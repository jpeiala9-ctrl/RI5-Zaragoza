...
const SessionInvites = {
  ...
  _infoTipoPaso(p) {
    if (p.tipoExtra === 'carrera') return { accent: 'var(--gold)', bg: 'rgba(192,160,96,0.12)', tag: '🏃 EXTRA · CARRERA — suma tiempo y distancia' };
  ...
  _renderPaso1() {
    ...
    const pasosHTML = d.pasosDetallados.map((p, i) => {
      ...
      <div class="sessgen-paso" data-idx="${i}" style="background:${info.bg}; border:1px solid ${info.accent}; border-left:4px solid ${info.accent}; border-radius:12px; padding:12px 12px 10px; margin-bottom:10px; box-shadow:var(--shadow-sm);">
        ...
        <!-- 🔥 Icono + título en su propia fila (ancho casi completo) para que
             palabras largas de una sola pieza como "CALENTAMIENTO" o
             "ENFRIAMIENTO" quepan enteras -- antes compartían fila con los
             minutos y el botón de eliminar y se quedaban sin sitio. -->
        <div style="display:flex; gap:8px; margin-bottom:8px; align-items:center;">
          <input class="sg-paso-icono" data-idx="${i}" value="${Utils.escapeHTML(p.icono || '')}" style="width:44px; flex-shrink:0; text-align:center; padding:8px 4px; font-size:16px;" maxlength="4">
          <input class="sg-paso-titulo" data-idx="${i}" value="${Utils.escapeHTML(p.titulo || '')}" placeholder="TÍTULO DEL PASO" style="flex:1; min-width:0; text-align:center; font-weight:bold; letter-spacing:0.3px; padding:8px 6px;">
        </div>
        <div style="display:flex; gap:8px; margin-bottom:8px; align-items:center;">
          <input class="sg-paso-min" data-idx="${i}" type="number" min="0" value="${p.duracionMin || ''}" placeholder="Minutos" title="Minutos" style="flex:1; min-width:0; text-align:center; padding:8px 6px;">
      ...
      <div style="padding:16px 20px; overflow-y:auto; flex:1;">
        ...
        <div id="sgPasosWrap">
          ...
          <div style="display:flex; gap:10px; margin-top:6px;">
            <button onclick="SessionInvites._anadirPaso('carrera')" style="flex:1; height:46px; padding:0 8px; margin:0; background:rgba(192,160,96,0.12); border:1.5px solid var(--gold); color:var(--gold); border-radius:12px; cursor:pointer; font-size:12px; font-weight:bold; letter-spacing:0.5px; box-shadow:var(--shadow-sm);">🏃 + CARRERA</button>
          ...
          <p style="font-size:10px; color:var(--text-secondary); text-align:center; margin:8px 0 0; line-height:1.5;"><span style="color:var(--gold);">🏃 Carrera</span> se coloca debajo de la parte principal y suma tiempo y distancia · <span style="color:var(--text-secondary);">💪 Fuerza</span> se coloca al final y solo suma tiempo</p>
        </div>
      </div>
...
console.log('✅ SessionInvites v11 - Rediseño visual completo del generador (pasos, calendario y usuarios)');