...
//   - El historial cuenta un pack POR DESTINATARIO (no por documento). De
//     momento los packs no tienen "REUTILIZAR".
//
...
const SessionInvites = {
  ...
  _abrirDetalleHistorial(idx) {
    ...
    modal.innerHTML = `
      ...
      <div style="padding:16px 20px; background:var(--bg-primary); border-top:1px solid var(--border-color); display:flex; justify-content:${lote.esPack ? 'center' : 'space-between'}; gap:12px;">
        ...
        ${lote.esPack ? '' : `<button onclick="SessionInvites._reutilizar(${idx})" class="action-button" style="width:auto; padding:0 24px; margin:0;">🔁 REUTILIZAR</button>`}
      </div>
    `;
  },
