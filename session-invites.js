...
const SessionInvites = {
    ...
    modal.innerHTML = `
      ...
      <div style="padding:16px 20px; overflow-y:auto; flex:1;">
        <div style="margin-bottom:16px;">
          ...
          <div id="sgGruposList" style="display:flex; flex-wrap:wrap; gap:8px;">⏳ Cargando grupos...</div>
        </div>
  ...
  // Pinta los grupos del admin como "chips" pulsables. Un chip se marca en
  // dorado cuando TODOS sus miembros (los que sigan existiendo en
  // _usuariosTodos) ya están seleccionados en la lista de abajo. Pulsarlo
  // marca de golpe a todos sus miembros; volver a pulsarlo (con todos ya
  ...
  _renderizarGrupos() {
    ...
    cont.innerHTML = grupos.map(g => {
      ...
      return `
        <span onclick="SessionInvites._toggleGrupo('${g.id}')" style="
          ...
          transition:all 0.15s ease;
        ">📁 ${Utils.escapeHTML(g.name)} · ${miembrosValidos.length}</span>`;
    }).join('');
  },
  ...
  _abrirGestionGrupos() {
    ...
    modal.innerHTML = `
      <div style="padding:16px 44px; background:var(--bg-primary); border-bottom:1px solid var(--border-color); text-align:center; position:relative;">
        <span style="font-size:16px; font-weight:bold; letter-spacing:1px; color:var(--text-primary);">📁 GESTIONAR GRUPOS</span>
        <button onclick="SessionInvites._cerrarModal('sessGroups')" style="position:absolute; right:12px; top:50%; transform:translateY(-50%); background:transparent; border:none; color:var(--text-secondary); font-size:20px; cursor:pointer; line-height:1;">✕</button>
      </div>
  ...
  async _renderizarGestionGrupos() {
    ...
    cont.innerHTML = grupos.map(g => `
      <div style="display:flex; align-items:center; gap:10px; padding:10px 12px; margin-bottom:8px; background:var(--stat-bg); border:1px solid var(--border-color); border-radius:12px; box-shadow:var(--shadow-sm);">
        <div style="flex:1; min-width:0;">
          <div style="font-size:14px; color:var(--text-primary); font-weight:bold;">📁 ${Utils.escapeHTML(g.name)}</div>
          <div style="font-size:11px; color:var(--text-secondary);">${(g.members || []).length} miembro(s)</div>
        </div>
        <button onclick="SessionInvites._abrirFormularioGrupo('${g.id}')" style="background:transparent; border:none; color:var(--gold); font-size:17px; cursor:pointer; padding:4px;">✏️</button>
        <button onclick="SessionInvites._eliminarGrupo('${g.id}')" style="background:transparent; border:none; color:var(--zone-5); font-size:17px; cursor:pointer; padding:4px;">🗑️</button>
      </div>
    `).join('');
  },
...
console.log('✅ SessionInvites v14 - Grupos de destinatarios en el Paso 3 (crear/editar/eliminar + selección rápida por chip)');