...
const SessionInvites = {
  ...
  _mostrarSiguienteDeCola() {
    ...
    if (!this._colaPendientes.length) {
      // No hay más invitaciones: aseguramos que el loading se oculte
      Utils.hideLoading();
      return;
    }
    ...
    // Mostrar loading SOLO si no está ya visible (evita reiniciar la animación)
  ...
  async _mostrarModalInvite(id, data) {
    ...
    if (sesion.tipo === 'descanso') {
      ...
      // Esperar a que el modal esté pintado y la animación de "EN PROCESO" termine
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          Utils.hideLoading();
        });
    ...
    // Esperar doble rAF para asegurar que el modal esté pintado y luego ocultar el loading
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        Utils.hideLoading();
      });
  ...
  async _rechazar(id, data) {
    // Mostrar loading y cerrar modal de invitación
    Utils.showLoading();
    this._cerrarModal('sessInvite');
    try {
      await firebaseServices.db.collection('sessionInvites').doc(id).update({ status: 'rejected' });
      // No ocultamos loading aquí; lo hará _mostrarSiguienteDeCola o el flujo final
      this._modalInviteAbierto = false;
  ...
  async _aceptar(id, data, personalizado) {
    ...
    try {
      ...
      if (!hayMas) {
        // Si no hay más, ocultamos loading y mostramos éxito
        Utils.hideLoading();
        this._modalInviteAbierto = false;
        ...
        // Si hay más, no ocultamos loading; lo hará la siguiente invitación
        this._modalInviteAbierto = false;
        this._mostrarSiguienteDeCola();
      }
...
console.log('✅ SessionInvites v8 - Animación "EN PROCESO" completa antes de mostrar siguiente invitación');