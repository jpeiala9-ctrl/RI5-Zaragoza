...
const SessionInvites = {
  ...
  async _confirmarEnvio() {
    ...
    try {
      ...
      // El listener en tiempo real de mostrarHistorial() recibirá los
      // documentos recién creados solo; si por lo que sea aún no está
      // activo (primera vez que se abre este panel en la sesión), lo
      // arranca ahora.
  ...
  // Muestra el historial y lo mantiene sincronizado EN TIEMPO REAL: antes
  // se leía una sola vez (.get()) y se guardaba en _historialCache hasta
  // el próximo envío/borrado, así que si un destinatario aceptaba o
  // rechazaba una sesión, el admin seguía viendo "⏳ Pendiente" en su
  // panel hasta recargar la app. Ahora se usa un listener (.onSnapshot)
  // que se queda escuchando mientras el panel "generar" esté activo, y
  // repinta la lista en cuanto cambia el status de cualquier destinatario.
  async mostrarHistorial() {
    ...
    if (this._historialUnsubscribe) {
      // El listener ya está activo (p.ej. se volvió a este subtab sin
      // haber cerrado sesión): repintamos con lo último que ya tengamos,
      // el propio listener seguirá refrescando solo si algo cambia.
      this._renderHistorial(container);
      return;
    }
    ...
    this._historialUnsubscribe = firebaseServices.db.collection('sessionInvites')
      ...
      .onSnapshot((snapshot) => {
        ...
        console.error('Error en listener de historial de sesiones enviadas:', error);
        if (!this._historialCache) {
  ...
  // Deja de escuchar el historial en tiempo real (llamado al salir del
  // panel de admin / cerrar sesión, ver detenerListeners() en app.js).
  detenerListenerHistorial() {
    if (this._historialUnsubscribe) {
      this._historialUnsubscribe();
      this._historialUnsubscribe = null;
    }
  },

  // Pinta _historialCache en el contenedor. Separado de mostrarHistorial()
  // para poder repintar tanto desde la carga inicial como desde cada
  // actualización del listener en tiempo real.
  _renderHistorial(container) {
    if (!container) container = document.getElementById('adminSesionesEnviadasList');
    if (!container) return;

    if (!this._historialCache || !this._historialCache.length) {
      container.innerHTML = '';
      return;
    }
