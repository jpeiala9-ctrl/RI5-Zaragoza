...
const SessionInvites = {
  ...
  _renderPaso1() {
    ...
    const pasosHTML = d.pasosDetallados.map((p, i) => {
      const tit = (p.titulo || '').toUpperCase();
      // 🔥 v9: etiqueta visual + borde de color para los pasos extra
      // añadidos con los botones "🏃 + CARRERA" / "💪 + FUERZA", para que
      // el admin distinga a simple vista cuáles suman distancia y cuáles
      // solo tiempo.
      const badge = p.tipoExtra === 'carrera'
        ? `<div style="font-size:9px; font-weight:bold; color:var(--gold); text-align:center; margin-bottom:6px; letter-spacing:0.5px;">🏃 PASO EXTRA · CARRERA (suma tiempo y distancia)</div>`
        : p.tipoExtra === 'fuerza'
          ? `<div style="font-size:9px; font-weight:bold; color:var(--text-secondary); text-align:center; margin-bottom:6px; letter-spacing:0.5px;">💪 PASO EXTRA · FUERZA (solo suma tiempo)</div>`
          : '';
      const borde = p.tipoExtra === 'carrera' ? 'var(--gold)' : (p.tipoExtra === 'fuerza' ? 'var(--text-secondary)' : 'var(--border-color)');
      ...
      const selectorZona = p.tipoExtra === 'carrera'
        ? `<select class="sg-paso-zona" data-idx="${i}" style="width:100%; margin-bottom:6px; text-align:center; text-align-last:center; font-size:12px;">
            ${this._ZONAS.map(z => `<option value="${z.codigo}" ${p.zona === z.codigo ? 'selected' : ''}>${z.codigo} · ${z.etiqueta}</option>`).join('')}
          </select>`
        : '';
      return `
      <div class="sessgen-paso" data-idx="${i}" style="background:var(--stat-bg); border:1px solid ${borde}; border-radius:10px; padding:10px; margin-bottom:8px;">
        ${badge}
      ...
      <div style="padding:16px 20px; overflow-y:auto; flex:1;">
        ...
        <div id="sgPasosWrap">
          ...
          <div style="display:flex; gap:8px; margin-top:4px;">
            <button onclick="SessionInvites._anadirPaso('carrera')" style="flex:1; padding:10px; background:transparent; border:1px dashed var(--gold); color:var(--gold); border-radius:10px; cursor:pointer; font-size:12px;">🏃 + CARRERA</button>
            <button onclick="SessionInvites._anadirPaso('fuerza')" style="flex:1; padding:10px; background:transparent; border:1px dashed var(--border-color-light); color:var(--text-secondary); border-radius:10px; cursor:pointer; font-size:12px;">💪 + FUERZA</button>
          </div>
          <p style="font-size:10px; color:var(--text-secondary); text-align:center; margin:6px 0 0; line-height:1.4;">🏃 Carrera se coloca justo debajo de la parte principal y suma tiempo y distancia. 💪 Fuerza se coloca al final (tras el enfriamiento) y solo suma tiempo.</p>
        </div>
      </div>
...
console.log('✅ SessionInvites v9 - Paso extra: elección explícita Carrera/Fuerza');