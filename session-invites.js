  ...
  _renderPaso1() {
    ...
    const pasosHTML = d.pasosDetallados.map((p, i) => {
      ...
      <div class="sessgen-paso" data-idx="${i}" style="background:${info.bg}; border:1px solid ${info.accent}; border-left:4px solid ${info.accent}; border-radius:12px; padding:12px 12px 10px; margin-bottom:10px; box-shadow:var(--shadow-sm);">
        ${info.tag ? `<div style="font-size:9px; font-weight:bold; color:${info.accent}; text-align:center; margin-bottom:8px; letter-spacing:0.8px;">${info.tag}</div>` : ''}
        <div style="display:flex; gap:8px; margin-bottom:8px; align-items:center;">
          <input class="sg-paso-icono" data-idx="${i}" value="${Utils.escapeHTML(p.icono || '')}" style="width:44px; text-align:center; padding:8px 4px; font-size:16px;" maxlength="4">
          <input class="sg-paso-titulo" data-idx="${i}" value="${Utils.escapeHTML(p.titulo || '')}" placeholder="TÍTULO DEL PASO" style="flex:1; text-align:center; font-weight:bold; letter-spacing:0.5px;">
...
console.log('✅ SessionInvites v11 - Rediseño visual del generador (tarjetas por color, sin cambios de lógica)');