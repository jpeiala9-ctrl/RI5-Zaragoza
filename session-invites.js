...
// VERSIÓN MODIFICADA: soporte para plan personalizado, orden de pasos,
// y orden cronológico de invitaciones + ajuste de fecha de inicio del plan.
// FIX v2: sesionesRealizadas/feedback se desplazan junto con diaGlobal al
// adelantar la fecha de inicio del plan.
// FIX v3: _construirDetalleSesion calculaba bien
// personalizado.segundosEsfuerzo para sesiones de SERIES (tiempo real de
// esfuerzo, sin los descansos entre repeticiones/bloques) pero lo perdía al
// construir el detalle final de la sesión guardada -- sin ese campo,
// calendar.js no podía separar esfuerzo de recuperación al marcar la sesión
// como hecha, y calculaba la zona con el ritmo MEDIO de toda la parte
// principal (diluido por los descansos), dando una zona muy por debajo de
// la real (ej. Z1 en vez de Z4/Z5).
// FIX v4 (esta ronda): al aceptar/rechazar una sesión enviada, el modal de
// la invitación (z-index 60000) se cerraba DESPUÉS de escribir en
// Firestore, tapando por completo la animación "EN PROCESO" de
// #loadingOverlay (z-index 40000) mientras esta corría -- con varias
// sesiones seguidas en la cola, cada una "flasheaba" y se cortaba en vez
// de completarse. Ahora _aceptar/_rechazar cierran el modal de la
// invitación ANTES de mostrar el loading, y esperan (await) a que la
// animación termine su duración mínima antes de avanzar a la siguiente
// invitación de la cola. Si la escritura falla, la invitación se
// reintroduce en la cola para poder reintentarla (antes se dejaba el
// modal a medio cerrar, ahora ya se había cerrado).
// FIX v5: cuando quedan más invitaciones pendientes tras aceptar una,
// no se oculta el loading ni se muestra el Dashboard entre medias;
// _mostrarModalInvite se encarga de ocultar el loading solo cuando el
// nuevo modal ya está visible, manteniendo el flujo encadenado sin saltos.
// ====================
...
const SessionInvites = {
  ...
  _calcularPersonalizacionSeries(seriesConfig, calculo, peso, calentamientoMin = 0, enfriamientoMin = 0) {
  ...
  // ========== NUEVAS FUNCIONES PARA PLAN PERSONALIZADO ==========
  ...
  // ========== DESPLAZAR CLAVES diaGlobal (sesionesRealizadas / feedback) ==========
  // Cuando una invitación con fecha anterior obliga a adelantar la fecha de
  // inicio del plan, TODAS las sesiones existentes desplazan su diaGlobal
  // hacia adelante (+diffDays). sesionesRealizadas y feedback son objetos
  // cuyas claves son ese mismo diaGlobal ({"3": true}, {"3": "..."}) -- si no
  // se desplazan igual, la marca de "completada"/el feedback de una sesión
  // ya hecha queda apuntando al diaGlobal ANTIGUO, que tras el desplazamiento
  // corresponde a otro día (o a ninguno).
  ...
  _renderPaso1() {
    ...
    const pasosHTML = d.pasosDetallados.map((p, i) => {
      const tit = (p.titulo || '').toUpperCase();
      const esExtremo = tit.includes('CALENTAMIENTO') || tit.includes('ENFRIAMIENTO') || tit.includes('ESTIRAMIENTO');
      const pedirMinutos = esExtremo || tipoActual === 'strength';
      return `
      <div class="sessgen-paso" data-idx="${i}" style="background:var(--stat-bg); border:1px solid var(--border-color); border-radius:10px; padding:10px; margin-bottom:8px;">
        <div style="display:flex; gap:8px; margin-bottom:6px; align-items:center;">
          ...
          ${pedirMinutos ? `<input class="sg-paso-min" data-idx="${i}" type="number" min="0" value="${p.duracionMin || ''}" placeholder="min" title="Minutos" style="width:56px; text-align:center; padding:8px 2px;">` : ''}
  ...
  _actualizarVisibilidadPorTipo() {
    ...
    if (pasosInfo) {
      pasosInfo.textContent = esFuerza
        ...
        : 'Pon los minutos de CALENTAMIENTO y ENFRIAMIENTO: así, al hacer la sesión con GPS, la app marca esos tramos automáticamente igual que en el resto de sesiones.';
    }
  },
  ...
  // ==================== FUNCIÓN MODIFICADA: _anadirPaso ====================
  _anadirPaso() {
    this._leerPasosDelDOM();
    const pasos = this._editable.detalle.pasosDetallados;
    
    // Buscar el índice del paso de enfriamiento (por título)
    ...
    const nuevoPaso = { icono: '📌', titulo: '', accion: '', porque: '' };
  ...
  // ========== HISTORIAL DE SESIONES ENVIADAS ==========
  ...
  // ========== DESTINATARIO: ESCUCHAR SESIONES PENDIENTES ==========
  // MODIFICADO: insertar invitaciones en orden cronológico por fecha de sesión
    ...
    this.unsubscribe = firebaseServices.db.collection('sessionInvites')
      ...
      .onSnapshot((snapshot) => {
        snapshot.docChanges().forEach(change => {
          ...
          // Insertar en la cola manteniendo orden por fecha (ascendente)
          const fecha = data.fecha || '2099-12-31'; // fecha por defecto al final
          let inserted = false;
  ...
  _mostrarSiguienteDeCola() {
    ...
    // Tomar el primero de la cola (ya ordenada por fecha ascendente)
  ...
  async _mostrarModalInvite(id, data) {
    ...
    if (sesion.tipo === 'descanso') {
      ...
      // Ocultar loading después de que el modal esté visible
      requestAnimationFrame(() => { Utils.hideLoading(); });
      return;
    }
  ...
  // FIX v4: el modal de la invitación (z-index 60000) se cerraba DESPUÉS
  // de terminar el trabajo en Firestore, así que tapaba por completo la
  // animación "EN PROCESO" de #loadingOverlay (z-index 40000) mientras
  // esta corría -- solo se veía un instante justo al cerrarse el modal,
  // cuando su duración mínima ya casi había pasado. Ahora el modal se
  // cierra YA (antes de mostrar el loading) y se espera de verdad
  // (await) a que la animación termine antes de tocar el toast o la
  // cola. _modalInviteAbierto se mantiene a true durante todo el
  // proceso (no solo mientras se ve el modal) para que la cola no
  // muestre la siguiente invitación mientras esta sigue en curso.
  async _rechazar(id, data) {
    ...
    try {
      ...
      // Reintroducimos la invitación en la cola para poder reintentar,
      // ya que el modal ya se cerró antes de saber si esto fallaría.
      this._colaPendientes.unshift({ id, data });
      this._mostrarSiguienteDeCola();
    }
  },

  // ========== ACEPTAR SESIÓN (MODIFICADO PARA AJUSTAR FECHA DE INICIO) ==========

  async _aceptar(id, data, personalizado) {
    // Ver comentario "FIX v4" en _rechazar: cerramos el modal YA, antes
    // de mostrar el loading, para que "EN PROCESO" no quede tapado por
    // el propio modal de la invitación.
    this._cerrarModal('sessInvite');
    Utils.showLoading();
    try {
      ...
      if (!personalizado) {
        if (data.sesion.tipo === 'descanso') {
          ...
          if (data.sesion.tipo === 'series' && det.seriesConfig) {
            personalizado = this._calcularPersonalizacionSeries(det.seriesConfig, calculo, peso, det.calentamientoMin || 0, det.enfriamientoMin || 0);
          } else {
      ...
      // --- SI EL PLAN YA EXISTE, AJUSTAR FECHA DE INICIO SI LA NUEVA SESIÓN ES ANTERIOR ---
      ...
      if (!planCreadoAhora && fechaSesion < fechaInicioActual) {
        // Calcular diferencia en días (redondear)
        diffDays = Math.round((fechaInicioActual - fechaSesion) / (1000 * 60 * 60 * 24));
        // Nueva fecha de inicio = fechaSesion
        planData.params.fechaInicio = fechaSesion.toISOString();
        // Desplazar diaGlobal de todas las sesiones existentes
        ...
        // Actualizar también en el objeto que vamos a guardar
        planData.sesiones = sesiones;
        // Guardamos la nueva fecha de inicio en el planData
        // (lo actualizaremos en Firestore más abajo)
      }

      // --- CALCULAR DIA GLOBAL PARA LA NUEVA SESIÓN ---
      ...
      if (diaGlobal < 1) {
        // Esto no debería ocurrir después del ajuste, pero por si acaso
        await Utils.hideLoading();
        this._modalInviteAbierto = false;
      ...
      // --- OBTENER PLAN COMPLETO PARA AÑADIR SESIÓN ---
      ...
      // Si hemos ajustado la fecha de inicio, también debemos actualizar las
      // sesiones EN FIRESTORE -- y desplazar igual las claves de
      // sesionesRealizadas/feedback, que están indexadas por el mismo
      // diaGlobal que acabamos de renumerar (ver _desplazarClavesDiaGlobal).
      if (diffDays > 0) {
        // Ya actualizamos planData.sesiones, pero necesitamos que sesiones refleje ese cambio
      ...
      // --- REFRESCAR CALENDARIO Y DASHBOARD (si no hay más invitaciones) ---
      const hayMas = this._colaPendientes.length > 0;
      if (!hayMas) {
        // Si no hay más, ocultamos loading y mostramos toast de éxito
        await Utils.hideLoading();
        this._modalInviteAbierto = false;
        ...
        // No llamamos a _mostrarSiguienteDeCola porque no hay más
      } else {
        // Si hay más, NO ocultamos loading; lo ocultará _mostrarModalInvite
        // cuando el siguiente modal esté visible
        this._modalInviteAbierto = false;
        // Llamamos a mostrar siguiente, que se encargará de ocultar loading cuando el modal esté listo
        this._mostrarSiguienteDeCola();
      }
...
console.log('✅ SessionInvites v5 - flujo encadenado sin saltos visuales entre invitaciones');