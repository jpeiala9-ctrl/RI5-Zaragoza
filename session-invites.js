...
const SessionInvites = {
  ...
  // entrenador (no admin) solo puede enviar sesiones a sus propios
  // amigos -- no es solo una restricción de la app: las reglas de
  // seguridad de Firestore ya no dejan a un no-admin leer usuarios que no
  // sean amigos suyos, así que una consulta sin filtro a toda la
  // colección 'users' fallaría directamente (permission-denied) para un
  // entrenador. Aquí se parte de su propia lista de amigos (que sí puede
  // leer, es su propio documento) y se piden esos usuarios por lotes de
  // hasta 30 (límite de Firestore para el operador 'in').
  _precargarUsuarios() {
    ...
    this._precargaUsuariosPromise = (async () => {
      try {
        ...
        const amigos = (miDoc.data()?.friendIds) || [];
        if (!amigos.length) { this._usuariosTodos = []; return; }
        const lotes = [];
        for (let i = 0; i < amigos.length; i += 30) lotes.push(amigos.slice(i, i + 30));
        ...
        console.error('Error cargando amigos para invitar:', e);
        this._usuariosTodos = [];
      }
    })().finally(() => { this._precargaUsuariosPromise = null; });
    return this._precargaUsuariosPromise;
  ...
  async _renderPaso3Usuarios() {
    ...
    // Los grupos de destinatarios guardados ("Gimnasio", "Amigos"...) siguen
    // siendo una comodidad exclusiva del admin -- un entrenador elige
    // siempre uno a uno entre sus amigos, más abajo.
    const puedeUsarGrupos = !!AppState.isAdmin;
    ...
    try {
      ...
      try {
        ...
        if (!hayMas) {
          ...
          // 🔥 FIX bug real: mostrarCalendario() solo PINTA el contenido
          // (el grid), pero nunca toca el display del contenedor
          // '#calendarioEntreno' -- ese empieza oculto por defecto
          // (style="display:none" en index.html) y solo lo hacían
          // visible mostrarUltimoPlanGuardado()/cargarPlanDesdeHistorial()
          // ANTES de llamar a mostrarCalendario(). Aquí, al aceptar la
          // PRIMERA sesión de un entrenador sin haber entrado antes en la
          // pestaña Plan, faltaban estas dos líneas -- así que el
          // calendario se dibujaba "a ciegas" detrás de una caja oculta:
          // el toast de éxito salía, pero no se veía nada al entrar en
          // Plan (ni error ni calendario, porque AppState.planActualId ya
          // estaba puesto en memoria y el auto-cargado al cambiar de
          // pestaña se saltaba por considerarlo innecesario).
...
console.log('✅ SessionInvites v14.3 - Grupos: usuarios y grupos se precargan en paralelo desde el Paso 2, así el Paso 3 aparece ya cargado sin saltos (con esqueleto animado como red de seguridad)');