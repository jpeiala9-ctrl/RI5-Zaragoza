...
const SessionInvites = {
  ...
  // Sesión de FUERZA: no hay distancia, ritmo ni zona (no es carrera), y
  // el tiempo de cada paso -- incluida la parte principal -- lo fija el
  // admin directamente en minutos, igual para todos los destinatarios
  // (no depende del ritmo de cada uno). La única personalización real es
  // la estimación de calorías, que sí depende del peso de cada usuario
  // (fórmula por MET: ~5 MET para trabajo de fuerza general).
  ...
  // Sesión de SERIES: sigue siendo carrera (SÍ tiene ritmo/distancia),
  // pero la parte principal no es un único tramo continuo, sino
  // numBloques x numSeries repeticiones a la zona de esfuerzo elegida,
  // con descanso entre repeticiones y entre bloques. El ritmo/tiempo de
  // cada repetición se calcula con EL ritmo real de cada destinatario en
  // esa zona, igual que en el resto de tipos de sesión.
  _calcularPersonalizacionSeries(seriesConfig, calculo, peso, calentamientoMin = 0, enfriamientoMin = 0) {
    ...
    if (calculo && Array.isArray(calculo.zones) && calculo.zones.length && calculo.ritmoBase && totalReps > 0) {
      ...
      if (zona) {
        const paceDecimal = calculo.ritmoBase * zona[4]; // min/km, en decimal, a la zona de esfuerzo
        resultado.ritmoStr = Utils.formatR(paceDecimal);
        ...
        // Los descansos entre repeticiones/bloques suman tiempo pero NO
        // distancia (trote muy suave o parado, no se contabiliza como km
        // recorridos de verdad).
  ...
  abrirGenerador() {
    this._editable = {
      ...
      detalle: {
        ...
        zona: 'Z2',
        // Solo se usa cuando tipo === 'series': número de bloques y de
        // repeticiones por bloque, cómo se define cada repetición
        // (distancia en metros o tiempo en segundos), zona de esfuerzo de
        // la repetición, y los dos descansos (entre repeticiones del
        // mismo bloque, y entre bloques). Con esto SÍ se puede calcular
        // ritmo/tiempo/distancia reales por usuario (sigue siendo
        // carrera), pero con la estructura real de series en vez de un
  ...
  _renderPaso1() {
    ...
    const pasosHTML = d.pasosDetallados.map((p, i) => {
      ...
      // En fuerza no hay distancia/ritmo que calcule el tiempo de la
      // parte principal: TODOS los pasos (calentamiento, ejercicios,
      // estiramientos) llevan sus propios minutos fijos. En el resto de
      // tipos, solo calentamiento/enfriamiento son minutos fijos -- la
      // parte principal se define aparte (por distancia/tiempo, o por la
    ...
    document.getElementById('sgTipo').addEventListener('change', () => {
      // Antes de cambiar de tipo, se guarda lo ya escrito en los campos
      // comunes y en los pasos (nombre, sensación, objetivo, porqué,
      // estructura), para no perderlo al re-renderizar el paso 1 con los
      // campos propios del nuevo tipo.
  ...
  // Si se cambia de tipo y los pasos siguen siendo justo los que trae
  // abrirGenerador() por defecto (sin editar), se ajustan a lo que tiene
  // más sentido para el nuevo tipo: en fuerza, "ENFRIAMIENTO" pasa a
  // llamarse "ESTIRAMIENTOS" y la parte principal recibe unos minutos de
  // partida. No toca nada si el admin ya personalizó el título/los pasos.
  ...
  // Muestra/oculta los bloques del formulario según el tipo de sesión
  // elegido: un descanso no admite nada personalizado; una sesión de
  // fuerza no usa distancia/ritmo/zona (todo es tiempo por paso); una de
  // series usa la estructura de bloques/repeticiones en vez de un único
  // tramo continuo; el resto (rodaje/tempo/tirada larga) usa el tramo
  // continuo de siempre (distancia o tiempo + zona).
  ...
  // Muestra el campo de metros o el de segundos según cómo se defina
  // cada repetición de la serie.
  ...
  _leerPaso1() {
    ...
    // para cada destinatario justo antes de mostrarle/aceptarle la
    // sesión, usando su propio ritmo base y su peso.
    this._leerPasosDelDOM();

    // El calentamiento y el enfriamiento SIEMPRE son minutos fijos
    // (iguales para todos): se sacan de los pasos que llevan ese título.
    d.calentamientoMin = 0;
    d.enfriamientoMin = 0;
    ...
    if (this._editable.tipo === 'strength') {
      // Sin distancia ni zona: no es una sesión de correr. El tiempo de
      // cada paso (incluida la parte principal) se lee directamente de
      // los pasos -- ver _calcularPersonalizacionFuerza.
    ...
    if (this._editable.tipo === 'series') {
      ...
      // Se guarda también en d.zona (misma zona de esfuerzo) para poder
      // reutilizar el mismo bloque de "zona/ritmo" que ya se muestra en
      // el resto de tipos de sesión, sin duplicar esa pantalla.
    ...
    // Tipo continuo (rodaje/tempo/tirada larga): comportamiento original,
    // parte principal por distancia o por tiempo + una única zona.
  ...
  _reutilizar(idx) {
    ...
    // Compatibilidad con sesiones del historial creadas antes de añadir
    // la estructura de series (bloques/repeticiones/descansos).
  ...
  async _mostrarModalInvite(id, data) {
    ...
    // Una sesión de FUERZA no tiene ritmo/km que calcular (no es
    // carrera): los minutos de cada paso los fijó el admin y son iguales
    // para todos, así que no hace falta esperar a las zonas del usuario,
    // solo a su peso (para la estimación de calorías). Se muestra una
    // tarjeta propia con cada paso (calentamiento/ejercicios/
    // estiramientos), sin distancia, ritmo, zona ni TSS.
    ...
    // calcula aquí para CADA usuario. Una sesión de series usa su propia
    // función (estructura de bloques/repeticiones en vez de un tramo
    // único), pero devuelve el mismo formato de resultado.
    let personalizado;
  ...
  _conValoresPorDefecto(detalleOriginal, personalizado, tipo) {
    ...
    // Fuerza no tiene zona/ritmo (no es carrera): textos por defecto
    // propios, y "ENFRIAMIENTO"/"ESTIRAMIENTOS" se tratan igual.
    if (tipo === 'strength') {
      const ACCION_FUERZA = {
