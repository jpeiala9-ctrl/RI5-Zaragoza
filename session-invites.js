...
const SessionInvites = {
  ...
  _construirDetalleSesion(sesionData, personalizado) {
    ...
    return {
      ...
      // FIX: segundosEsfuerzo se calcula bien en _calcularPersonalizacionSeries
      // (tiempo real corriendo, SIN los descansos entre reps/bloques) pero se
      // perdía al construir el detalle final -- sin este campo,
      // calendar.js/_desglosarSesion no puede distinguir esfuerzo de
      // recuperación al marcar la sesión como hecha, y cae al ritmo/zona
      // MEDIA de toda la parte principal (diluida por los descansos), dando
      // una zona muy por debajo de la real (ej. Z1 en vez de Z4).
  ...
  async _aceptar(id, data, personalizado) {
    ...
    try {
      ...
      // --- REFRESCAR CALENDARIO Y DASHBOARD ---
...
console.log('✅ SessionInvites v4 - animación "EN PROCESO" visible entera al aceptar/rechazar sesiones enviadas');