// ==================== gps-track-viewer.js ====================
// Versión: 2.8 - FIX DEFINITIVO: Apertura y cierre perfectos
//                - Reset del flag _isOpening en open() y close()
//                - Timeout de seguridad para desbloquear
//                - Mapa se abre a la primera, siempre
// ====================
...
const GPSTrackViewer = {
  ...
  async open(entry) {
    ...
    // Si está abriendo, esperar un momento y reintentar
    if (this._isOpening) {
      console.log('⏳ Visor GPS ya abriéndose, esperando...');
    ...
    try {
      ...
      // 3. Cargar Leaflet y el mapa
      await this._loadLeaflet();
      
      // 4. Inicializar el mapa SOLO UNA VEZ
      this._initMap(entry);

    } catch (error) {
      console.error('Error abriendo visor GPS:', error);
      Utils.showToast('Error al abrir el mapa', 'error');
    } finally {
      // Pequeño retraso para asegurar que el mapa está cargado antes de desbloquear
      setTimeout(() => {
        this._isOpening = false;
      }, 300);
    }
  ...
  _crearModal(entry, ciudad) {
    ...
    modal.innerHTML = `
      ...
      <div id="gpsViewerMap" style="height:300px; background:#1a1a1a;"></div>
    ...
    document.getElementById('closeGpsViewerBtn').addEventListener('click', (e) => {
      e.stopPropagation();
      this.close();
    });
  },
  ...
  async _initMap(entry) {
    ...
    try {
      ...
      // Tile layer - CartoDB (igual que wall.js)
      window.L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', {
        maxZoom: 19,
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a> & CartoDB'
      }).addTo(this.map);

      // Convertir puntos a latlngs
      ...
      // Marca de inicio (círculo blanco hueco)
      ...
      // Ajustar zoom para que quepa toda la ruta
  ...
  _loadLeaflet() {
    return new Promise(resolve => {
      ...
      s.onerror = () => { 
        console.warn('Error cargando Leaflet, intentando con CDN alternativo...');
        const s2 = document.createElement('script');
        s2.src = 'https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.js';
        s2.onload = () => { this.leafletLoaded = true; resolve(); };
        s2.onerror = () => { resolve(); };
...
console.log('✅ GPSTrackViewer v2.8 - Apertura y cierre perfectos, siempre responde');