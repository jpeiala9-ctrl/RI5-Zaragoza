...
const GPSTrackViewer = {
    ...
    modal.innerHTML = `
      ...
      <div style="padding:16px 20px; background:var(--bg-primary); border-top:1px solid var(--border-color);">
        <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:12px; margin-bottom:16px; text-align:center;">
          <div>
            <div style="font-size:22px; font-weight:bold; color:var(--level-color);">${(entry.gpsDistanceKm || entry.distancia || 0).toFixed(2)}</div>
  ...
  async _initMap(entry) {
    ...
    try {
      ...
      // Leaflet tampoco resuelve var(--level-color) solo (no es CSS
      // aplicado al DOM), así que se lee ya calculado, igual que en las
      // gráficas del dashboard.
      const rutaColor = (getComputedStyle(document.documentElement).getPropertyValue('--notification-color') || '').trim() || '#c0a060';
