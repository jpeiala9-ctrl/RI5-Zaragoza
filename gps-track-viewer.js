// ==================== gps-track-viewer.js ====================
// Versión: 2.10 - A petición del usuario: los números de portal/vivienda de
//                las calles quedaban visibles y recargaban el mapa,
//                sobre todo en tramos cortos o muy encuadrados (esos
//                números del callejero de OpenStreetMap solo aparecen a
//                partir de cierto nivel de zoom). Se limita el zoom
//                automático al encuadrar la ruta (maxZoom:16 en
//                fitBounds) para quedarse siempre por debajo de ese
//                nivel -- se siguen viendo los nombres de las calles con
//                normalidad. El usuario puede seguir haciendo zoom manual
//                más allá de ahí si quiere (no se ha tocado el maxZoom del
//                propio mapa, solo el del encuadre automático).
// Versión: 2.9 - FIX mapa tapado por marca de agua "API KEY REQUIRED":
//                CartoDB empezó a exigir clave de API para sus mosaicos
//                ('basemaps.cartocdn.com'). Se pasa a los mosaicos
//                estándar de OpenStreetMap, gratis y sin clave.
// Versión: 2.8 - FIX "latido" al abrir el modal: se sustituye la segunda
//                llamada fija a encuadrar() (300ms después, siempre, aunque
//                el tamaño no hubiera cambiado) por un ResizeObserver que
//                solo reencuadra si el contenedor cambia de tamaño de
//                verdad -- así el mapa no vuelve a hacer fitBounds sobre sí
//                mismo nada más abrirse, y sigue totalmente interactivo
//                (mover/zoom).
...
const GPSTrackViewer = {
  ...
  async _initMap(entry) {
    ...
    try {
      ...
      // cuenta ni clave.
      // 🔥 v3: vuelta a OpenStreetMap (a petición del usuario). La cajita
      // de atribución no se ve porque el mapa ya tiene
      // attributionControl:false puesto más arriba (ya estaba así antes).
      ...
      const encuadrar = () => {
        ...
        this.map.fitBounds(polyline.getBounds(), { padding: [30,30], animate: false, maxZoom: 16 });
      };

      encuadrar();

      // 🔥 v2.8: ANTES había aquí una segunda llamada a encuadrar() fija a
      // los 300ms "por si acaso" el layout no estuviera asentado en el
      // primer encuadre. El problema: esa llamada se disparaba SIEMPRE, en
      // cada apertura del modal, aunque el tamaño del contenedor no
      // hubiera cambiado ni un píxel desde el primer encuadre -- así que
      // en el caso normal (la inmensa mayoría de aperturas) el mapa hacía
      // fitBounds dos veces seguidas sobre el mismo contenedor, y ese
      // segundo salto -- aunque fuera con animate:false -- se percibía
      // como un pequeño "latido" o parpadeo del mapa justo después de
      // abrirse. Ahora, en su lugar, se usa un ResizeObserver: solo se
      // vuelve a encuadrar si el contenedor CAMBIA de tamaño de verdad
      // (p.ej. si el primer encuadre sí cayó antes de que el layout
      // terminara de asentarse, o si el usuario rota el móvil/cambia el
      // tamaño de la ventana con el visor abierto). Así se conserva la
      // robustez de antes sin el doble salto en la apertura normal, y el
      // mapa sigue totalmente interactivo (se puede mover/hacer zoom con
      // normalidad).
      if (window.ResizeObserver) {
        let ultimoAlto = mapContainer.clientHeight;
        let ultimoAncho = mapContainer.clientWidth;
        this._resizeObserver = new ResizeObserver(() => {
          ...
          if (Math.abs(alto - ultimoAlto) < 2 && Math.abs(ancho - ultimoAncho) < 2) return; // sin cambio real
          ultimoAlto = alto;
          ultimoAncho = ancho;
          encuadrar();
        });
...
console.log('✅ GPSTrackViewer v2.9 - Fix mapa tapado por marca de agua "API KEY REQUIRED" (OpenStreetMap en vez de CartoDB)');
