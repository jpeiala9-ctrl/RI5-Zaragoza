// ==================== gps-track-viewer.js ====================
// Versión: 2.1 - Modal con mapa claro, ciudad y datos de sesión (completo)
// ====================
...
const GPSTrackViewer = {
  ...
  _crearModal(entry) {
    ...
    modal.innerHTML = `
      <div style="padding:16px 20px; background:#111; border-bottom:1px solid #2a2a2a; display:flex; justify-content:space-between; align-items:center;">
        ...
        <button id="closeGpsViewerBtn" style="background:transparent; border:none; color:#888; font-size:24px; cursor:pointer;">&times;</button>
      </div>
  ...
  async _initMap(entry) {
    ...
    try {
      this.map = window.L.map(mapContainer, { zoomControl: true, attributionControl: false, tap: false });
...
console.log('✅ GPSTrackViewer v2.1 - Completo');