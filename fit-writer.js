// ==================== fit-writer.js - Generador FIT binario mínimo ====================
class FitWriter {
  constructor() {
    this.data = [];
    this.localMesgNum = 0;
  ...
  workout({ wktName, sport, subSport, numValidSteps }) {
    const mesgNum = 26;
    const fields = [
      { num: 3,  value: wktName,                                type: 7 },
      { num: 4,  value: this._enumSport(sport),                 type: 0x00 },
      { num: 5,  value: this._enumSubSport(subSport),           type: 0x00 },
      { num: 11, value: numValidSteps,                          type: 0x02 }
    ];
    this._writeMessage(mesgNum, fields);
  }
  ...
  workoutStep(index, { wktStepName, durationType, durationTime, targetType, targetValue, intensity }) {
    ...
    const fields = [
      ...
      { num: 2,   value: durationTime,                          type: 0x86 },
  ...
  _enumTargetType(t) { return t === 'heart_rate_zone' ? 0 : 0; }
  _enumIntensity(i) {
