// ==================== gps-tracker.js ====================
// Versión: 2.12 - Pre‑lock manual sin emoticonos + GPS perfecto
// ====================
...
const GPSTracker = {
  ...
  // ==================== FILTRO GPS (3 capas, sin saltos) ====================
  _filterGPS(lat, lng, accuracy, timestamp) {
    // Capa 1: descartar precisión muy mala (salvo los primeros puntos)
    if (accuracy > 45 && this.trackPoints.length > 5) return null;

    // Capa 2: anti‑salto (velocidad máxima 7.5 m/s)
    ...
    // Capa 3: buffer de últimas 4 lecturas → centroide ponderado
    this._rawBuffer.push({ lat, lng, acc: Math.max(1, accuracy) });
    if (this._rawBuffer.length > 4) this._rawBuffer.shift();

    // Si no hay suficientes lecturas, esperamos (pero la primera lectura la aceptamos directamente)
    if (this._rawBuffer.length < 2) {
      if (this.trackPoints.length === 0 && accuracy < 30) {
    ...
    // Descartar movimientos inferiores a 2m (ruido estático)
  ...
  // ==================== PANTALLA COMPLETA (sin emoticonos molestos) ====================
  _crearPantalla() {
    ...
    ov.innerHTML = `
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #gpsConfirm button { width: 100%; margin-bottom: 10px; border-radius: 40px; font-weight: bold; }
      ...
      <!-- Pantalla de pre‑lock (sin emoticono, solo texto y barra) -->
      ...
      <div id="gpsConfirm">
        <div class="confirm-content">
          ...
          <button id="confirmSaveBtn" style="background:#c0392b;color:#fff;">✅ GUARDAR Y SALIR</button>
          <button id="confirmForceBtn" style="display:none;background:#9BB5A0;color:#0a0a0a;">⭐ MARCAR IGUALMENTE</button>
          <button id="confirmExitBtn" style="background:transparent;border:2px solid #555;color:#aaa;">⚠️ SALIR SIN GUARDAR</button>
          <button id="confirmCancelBtn" style="background:transparent;border:none;color:#555;">CONTINUAR SESIÓN</button>
        </div>
      </div>
    `;
    document.body.appendChild(ov);

    // Asignación de eventos (evita conflictos con Leaflet)
  ...
  _iniciarPreLockManual() {
    ...
    // Iniciamos el watch para obtener actualizaciones de GPS y mostrar la precisión
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      // Si la precisión es buena (<20m), habilitamos el botón COMENZAR
      if (acc < 20) {
      ...
      // Mostrar botón "INICIAR IGUALMENTE" tras 20 segundos
  ...
  _comenzarManual() {
    ...
    // Obtenemos la última posición conocida para arrancar el mapa
  ...
  _arrancarSesion(lat, lng, acc) {
    ...
    // Ocultar pantalla de pre‑lock y mostrar la de sesión
    ...
    // IMPORTANTE: detener el watch anterior (el de pre‑lock) y crear uno nuevo
  ...
  // ==================== GUARDADO EN FIREBASE (compatible v8/v9) ====================
  _getCurrentUser() {
  ...
  async _guardarYPublicar(distKm, elapsedMs, forceMark = false) {
    ...
    // Decidir si marcar como realizada (90% o forzado)
...
console.log('✅ GPSTracker v2.12 - Pre‑lock sin emoticonos, GPS perfecto');