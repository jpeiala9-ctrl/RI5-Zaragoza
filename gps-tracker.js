// ==================== gps-tracker.js ====================
// Versión: 3.60 - DEFINITIVA: la línea termina EXACTAMENTE en la bandera (reemplaza último punto)
// ====================
...
const GPSTracker = {
  ...
  // ========== FUNCIONES CORREGIDAS (VERSIÓN 3.60) ==========
  _updatePolyline() {
    ...
    if (latlngs.length > 0) {
      // Reemplazar el último punto por la posición actual de la bandera
      latlngs[latlngs.length - 1] = [currentPos.lat, currentPos.lng];
    } else {
  ...
  _onPosition(pos) {
    ...
    if (puntoSuave) {
      ...
      // Reemplazar el último punto de trackPoints por la posición actual (evita duplicados)
      if (this.trackPoints.length > 0) {
        this.trackPoints[this.trackPoints.length - 1] = puntoSuave;
      } else {
...
console.log('✅ GPS Tracker v3.60 - Línea termina exactamente en la bandera (reemplaza último punto)');