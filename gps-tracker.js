// ==================== gps-tracker.js ====================
// Versión: 3.5 - Corrección del zigzag inicial: primeros puntos alineados en recta
// ====================
...
const GPSTracker = {
  ...
  _smoothAndSimplify(lat, lng, timestamp) {
    ...
    if (this.trackPoints.length === 1) {
      ...
      if (dist < 0.5) return null; // evitar puntos casi iguales
      this._lastSmoothPoint = { lat: smoothLat, lng: smoothLng, ts: timestamp };
      return this._lastSmoothPoint;
    }

    // A partir del tercer punto, aplicar la lógica normal con alineación a la recta inicial si es necesario
    if (this.trackPoints.length === 2) {
      // Proyectar el nuevo punto sobre la línea formada por los dos primeros puntos
      ...
      // Añadimos el punto proyectado al track
      this.trackPoints.push(this._lastSmoothPoint);
      this._updatePolyline();
      return null; // ya se añadió
    }

    // A partir del cuarto punto, usar el algoritmo normal de simplificación
    const dist = this._haversine(this._lastSmoothPoint.lat, this._lastSmoothPoint.lng, smoothLat, smoothLng);
    if (dist < this._minDistance) {
      // No mover el marcador
      return null;
    }

    // Detectar si estamos en línea recta
    if (this.trackPoints.length >= 3) {
      ...
      if (delta < this._minAngleChange) {
        // Actualizar el último punto sin añadir nuevo
    ...
    // Nuevo punto (cambio de dirección)
  ...
  _onPosition(pos) {
    ...
    // Siempre mover la bolita a la última posición suavizada
...
console.log('GPS Tracker v3.5 - Zigzag inicial corregido: primeros puntos alineados en recta');