// ==================== gps-tracker.js ====================
// Versión: 2.26 - GPS con precisión <10m, suavizado avanzado y mensajes amigables
// ====================
...
const GPSTracker = {
  ...
  // FILTRO GPS MEJORADO: Mayor precisión y suavizado
  _filterGPS(lat, lng, accuracy, timestamp) {
    // 1. Filtro de precisión: Requerimos menos de 10 metros para aceptar (antes era 45)
    if (accuracy > 10 && this.trackPoints.length > 5) {
      console.log(`📍 Punto descartado por precisión (${accuracy.toFixed(1)}m > 10m)`);
      return null;
    }

    // 2. Anti-salto (velocidad máxima 7.5 m/s)
    if (this._lastAccepted) {
      ...
      const velocidad = dist / dt;
      if (velocidad > 7.5 && dist > 20) {
        console.log(`📍 Punto descartado por velocidad (${velocidad.toFixed(1)} m/s)`);
        return null;
      }
    }

    // 3. Buffer de suavizado (centroide ponderado)
    this._rawBuffer.push({ lat, lng, acc: Math.max(1, accuracy) });
    if (this._rawBuffer.length > 5) this._rawBuffer.shift(); // Buffer más largo para mejor suavizado

    // Primera lectura: la aceptamos si la precisión es buena
    if (this.trackPoints.length === 0 && accuracy < 15) {
      const primerPunto = { lat, lng, ts: timestamp, acc: Math.round(accuracy) };
      this._lastAccepted = primerPunto;
      console.log(`📍 Primer punto aceptado (precisión ${accuracy.toFixed(1)}m)`);
      return primerPunto;
    }

    if (this._rawBuffer.length < 3) return null; // Esperamos al menos 3 lecturas para suavizar

    // Cálculo del centroide ponderado por precisión
    ...
    console.log(`📍 Punto suavizado añadido (movimiento ${movimiento?.toFixed(1)}m)`);
    return punto;
  },
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsConfirmForce">
        <div class="confirm-content">
          <div style="font-size:48px;margin-bottom:12px;">⚠️</div>
          <div style="font-size:13px;color:#f0a050;margin-bottom:16px;">NO HAS COMPLETADO EL ENTRENAMIENTO PROGRAMADO</div>
  ...
  async _terminarSesion() {
    ...
    if (alcanza90) {
      ...
      // Mensaje amigable (sin datos técnicos)
  ...
  async _finalizarConFuerza(forceMark, tipo = null) {
    ...
    this._limpiarRecursos();
    document.getElementById('gpsTrackerOverlay')?.remove();
    if (tipoReal === 'completo' && typeof Utils !== 'undefined' && Utils.launchConfetti) {
  ...
  async _guardarYPublicar(distKm, elapsedMs, forceMark, tipo = null) {
    ...
    if (forceMark && tipo) {
      ...
      if (typeof Utils !== 'undefined') Utils.showToast('📁 Track guardado, pero no se marcó la sesión', 'info', 3000);
    }
  },
...
console.log('✅ GPSTracker v2.26 - Precisión <10m, suavizado avanzado, mensajes amigables');