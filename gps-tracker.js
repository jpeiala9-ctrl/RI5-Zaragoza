// ==================== gps-tracker.js ====================
// Versión: 2.39 - Seguimiento GPS en tiempo real, filtro suavizado, guardado de puntos
// ====================
...
const GPSTracker = {
  ...
  // Filtro GPS mejorado: acepta puntos con precisión <30m, suavizado, anti-salto
  _filterGPS(lat, lng, accuracy, timestamp) {
    // Aceptamos precisión hasta 30m (antes era 10m) para que funcione en más dispositivos
    if (accuracy > 30 && this.trackPoints.length > 5) return null;

    // Anti-salto: velocidad máxima 7.5 m/s
    ...
    // Suavizado con buffer (centroide ponderado)
    this._rawBuffer.push({ lat, lng, acc: Math.max(1, accuracy) });
    if (this._rawBuffer.length > 4) this._rawBuffer.shift();

    // Primera lectura: la aceptamos directamente si la precisión es aceptable
    ...
    // Descartar si el movimiento es inferior a 1.5m (ruido estático)
  ...
  _iniciarPreLockManual() {
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      if (acc < 20) {  // Umbral de precisión más permisivo
...
console.log('✅ GPSTracker v2.39 - Seguimiento GPS en tiempo real, filtro mejorado');