// ==================== gps-tracker.js ====================
// Versión: 2.1 - GPS manual + confirmación robusta
// ====================
...
const GPSTracker = {
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        ...
        <!-- Botón COMENZAR (se habilita cuando la señal es buena) -->
      ...
      <!-- PANTALLA DE SESIÓN EN CURSO (oculta hasta que se pulse COMENZAR) -->
      ...
      <div id="gpsConfirm" style="display:none;position:absolute;inset:0;background:rgba(10,10,10,.95);z-index:10;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;">
        ...
        <button onclick="GPSTracker._confirmarFinalizar()" style="width:100%;max-width:280px;height:54px;background:#c0392b;border:none;color:#fff;border-radius:14px;font-size:16px;font-weight:bold;cursor:pointer;font-family:inherit;letter-spacing:1px;margin-bottom:12px;">✅ GUARDAR Y SALIR</button>
        <button onclick="GPSTracker._cancelarConfirm()" style="width:100%;max-width:280px;height:48px;background:transparent;border:2px solid #333;color:#666;border-radius:14px;font-size:14px;cursor:pointer;font-family:inherit;letter-spacing:1px;">CONTINUAR SESIÓN</button>
      </div>
  ...
  // ===== PRE-LOCK: esperar señal buena pero NO arrancar automáticamente =====
  _iniciarPreLockManual() {
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      // Señal suficientemente buena (menos de 20m)
      if (acc < 20) {
      ...
      const forceBtn = document.getElementById('preLockForceBtn');
      if (forceBtn) forceBtn.style.display = 'block';
      // Deshabilitar botón COMENZAR si hay error
      const startBtn = document.getElementById('preLockStartBtn');
      if (startBtn) startBtn.disabled = true;
    }, { enableHighAccuracy:true, maximumAge:1000, timeout:15000 });
  },

  // Botón COMENZAR manual (solo si señal buena)
  _comenzarManual() {
    const startBtn = document.getElementById('preLockStartBtn');
    ...
    // Obtener la última posición conocida para iniciar
  ...
  _arrancarSesion(lat, lng, acc) {
    ...
    // Reemplazar el watchId del pre-lock por el definitivo
  ...
  async _confirmarFinalizar() {
    ...
    // Deshabilitamos el botón para evitar doble clic (opcional)
    const confirmBtn = document.querySelector('#gpsConfirm button:first-child');
    ...
    try {
      ...
      // Si llegamos aquí, todo fue bien -> limpiamos y cerramos
      this._limpiarYFinalizar();
      ...
      Utils.hideLoading();
      Utils.showToast('⚠️ Error al guardar la sesión. Reintenta o contacta soporte.', 'error');
      // Reactivar botón por si quiere reintentar
      if (confirmBtn) confirmBtn.disabled = false;
      // No cerramos la pantalla, para que pueda reintentar
    }
  },

  // Limpieza total de la sesión (llamada solo cuando se guardó con éxito)
  _limpiarYFinalizar() {
    this.isRunning = false;
    if (this.timerInterval) clearInterval(this.timerInterval);
    if (this.stepInterval) clearInterval(this.stepInterval);
  ...
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    document.querySelector(`.calendario-dia[data-index="${this.diaIndex}"]`)?.classList.add('realizado');
...
console.log('✅ GPSTracker v2.1 - GPS manual + finalización robusta');