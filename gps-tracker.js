// ==================== gps-tracker.js ====================
// Versión: 3.22 - Mejorado sobre base funcional: estático, botón único, cuenta atrás, fondo claro
// ====================
...
const GPSTracker = {
  ...
  // ===== SUAVIZADO Y CONTROL DE MOVIMIENTO (evita puntos en estático) =====
  _smoothAndSimplify(lat, lng, timestamp) {
    ...
    // Si no hay movimiento significativo, no añadir punto
    if (dist < this._minDistance && speed < this._minSpeed) {
      return null;
    }

    // Actualizar último punto si apenas hay cambio de ángulo (línea recta)
    ...
    ov.innerHTML = `
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;">
        ...
        <div style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          ...
          <!-- SOLO DOS BOTONES: PAUSA y SIGUIENTE (FINALIZAR desaparece) -->
  ...
  _onPosition(pos) {
    ...
    if (puntoSuave) {
      // Control adicional: no añadir puntos si la distancia desde el inicio es muy pequeña (evita línea falsa al principio)
      if (this.trackPoints.length === 0) {
  ...
  renderTrackSVG(points, width = 320, height = 130) {
    ...
    // Fondo claro (#eaeaea) para que se vea bien en el muro
...
console.log('✅ GPS Tracker v3.22 - Mejorado sobre base funcional');