// ==================== gps-tracker.js ====================
// Versión: 3.35 - Auto‑bloqueo del slider tras 3s sin tocar botones, voz "go"
// ====================
...
const GPSTracker = {
  ...
  // Autoencuadre del mapa
  _mapAutoCenterTimer: null,
  _userMovedMap: false,

  // Auto‑bloqueo del slider
  _unlockTimeout: null,
  _isUnlocked: false,

  // ===== VOZ NATIVA DEL SISTEMA =====
  _speak(text) {
  ...
  // ===== SUAVIZADO CON CONTROL DE MOVIMIENTO REAL =====
  _smoothAndSimplify(lat, lng, timestamp) {
    ...
    if (dist < this._minDistance && speed < this._minSpeed) {
      return null;
    }
  ...
  _initMap(lat, lng) {
    ...
    try {
      ...
      this.map.on('movestart', () => {
        if (this._mapAutoCenterTimer) clearTimeout(this._mapAutoCenterTimer);
        this._userMovedMap = true;
      });
      this.map.on('moveend', () => {
        if (this._userMovedMap) {
          if (this._mapAutoCenterTimer) clearTimeout(this._mapAutoCenterTimer);
          this._mapAutoCenterTimer = setTimeout(() => {
            if (this.markerLayer && this._userMovedMap) {
              const pos = this.markerLayer.getLatLng();
              this.map.setView(pos, this.map.getZoom());
              this._userMovedMap = false;
            }
            this._mapAutoCenterTimer = null;
          }, 3000);
        }
      });
  ...
  _addFinishMarker(lat, lng) {
    if (!this.map || !window.L) return;
    if (this.finishMarker) this.finishMarker.remove();
    const finishIcon = window.L.divIcon({
      html: `<div style="font-size:24px; line-height:1; text-shadow:0 0 2px white;">🏁</div>`,
      className: '', iconAnchor: [12, 12]
    });
    this.finishMarker = window.L.marker([lat, lng], { icon: finishIcon }).addTo(this.map);
  },
  ...
  _updatePolyline() {
    ...
    if (this.trackPoints.length > 0) {
      ...
      this._addFinishMarker(lastPoint.lat, lastPoint.lng);
    }
  },
  ...
  // ===== PANTALLA COMPLETA (con slider estilo Apple y auto‑bloqueo) =====
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;">
        ...
        <div style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          ...
          <!-- Slider estilo Apple y botones con auto‑bloqueo -->
          <div id="gpsButtonContainer">
            ...
            <div id="gpsSliderContainer" style="margin-top:12px;">
              <div id="gpsAppleSlider" style="background:#2a2a2a; border-radius:30px; padding:3px; width:100%; display:flex; align-items:center; position:relative; cursor:pointer; touch-action:pan-x;">
                <div id="gpsSliderTrack" style="background:#444; border-radius:30px; height:40px; width:100%; position:relative; overflow:hidden;">
                  <div id="gpsSliderFill" style="background:#c0a060; width:0%; height:100%; border-radius:30px; transition:width 0.05s linear;"></div>
                </div>
                <div id="gpsSliderThumb" style="position:absolute; width:36px; height:36px; background:#fff; border-radius:50%; box-shadow:0 2px 8px rgba(0,0,0,0.3); left:2px; top:50%; transform:translateY(-50%); transition:left 0.05s linear; cursor:grab;"></div>
                <div style="position:absolute; left:0; right:0; top:0; bottom:0; display:flex; align-items:center; justify-content:center; pointer-events:none;">
                  <span style="font-size:12px; color:#aaa; letter-spacing:1px;">→ desliza para desbloquear →</span>
                </div>
              </div>
            </div>
    ...
    // Configurar el slider estilo Apple
    const sliderContainer = document.getElementById('gpsAppleSlider');
    const thumb = document.getElementById('gpsSliderThumb');
    const fill = document.getElementById('gpsSliderFill');
    const track = document.getElementById('gpsSliderTrack');
    ...
    const maxLeft = (track?.offsetWidth || 200) - 36; // ancho del track menos ancho del thumb

    // Función para resetear el slider (bloquear de nuevo)
    const resetSlider = () => {
    ...
    // Función para iniciar el temporizador de auto‑bloqueo
    ...
    const updatePosition = (clientX) => {
      ...
      if (percent >= 95 && !this._isUnlocked) {
        unlock();
      }
    };
    ...
    const onEnd = () => {
      ...
      // Si no se ha desbloqueado y el thumb no está al inicio, lo devolvemos suavemente
      if (!this._isUnlocked && thumb && parseFloat(thumb.style.left) > 5) {
    ...
    // Añadir listeners a los botones desbloqueados para reiniciar el temporizador
  ...
  // ===== CUENTA ATRÁS CON NÚMEROS NEGROS Y VOZ "go" =====
  _startCountdown() {
    let count = 3;
    ...
    numberDiv.id = 'gpsCountdownNumber';
    ...
    `;
    numberDiv.textContent = '3';
    overlay.appendChild(numberDiv);
    document.getElementById('gpsSessionScreen')?.appendChild(overlay);
    ...
    const animateNumber = (num) => {
      ...
      numberDiv.style.animation = 'countdownPop 0.3s ease-out';
    };

    if (!document.querySelector('#countdownKeyframes')) {
    ...
    const interval = setInterval(() => {
      count--;
      if (count > 0) {
        numberDiv.style.transform = 'scale(0.1)';
        ...
        this._speak(count.toString());
      } else {
        clearInterval(interval);
        this._speak('go');   // Cambiado de "a correr" a "go"
        setTimeout(() => {
          overlay.remove();
  ...
  _iniciarGrabacion() {
    ...
    if (primerBloque) {
      const duracionStr = primerBloque.duracionMin === 1 ? '1 minuto' : `${primerBloque.duracionMin} minutos`;
      this._speak(`${primerBloque.titulo}, ${duracionStr}`);
    }
  ...
  _onPosition(pos) {
    ...
    if (this._lastSmoothPoint) {
      this._updateMap(this._lastSmoothPoint.lat, this._lastSmoothPoint.lng);
    }
  ...
  _tick() {
    ...
    if (step) {
      ...
      const nextBtn = document.getElementById('gpsNextBtn');
      if (nextBtn && restante === 0) nextBtn.style.animation = 'stepPulse 0.8s ease infinite';
    }
  },
  ...
  togglePause() {
    ...
    if (this.isPaused) {
      ...
      this._speak('Sesión reanudada');
      // Reiniciar temporizador de auto‑bloqueo al reanudar (si estaba desbloqueado)
      if (this._isUnlocked) {
        if (this._unlockTimeout) clearTimeout(this._unlockTimeout);
        setTimeout(() => { if (this._isUnlocked) startAutoLockTimer(); }, 0);
      }
    } else {
      ...
      this._speak('Sesión pausada');
      // No cancelamos el temporizador, simplemente se mantiene
    }
  },
  ...
  nextStep() {
    ...
    if (esUltimo) {
      ...
      if (nuevoBloque) {
        const duracionStr = nuevoBloque.duracionMin === 1 ? '1 minuto' : `${nuevoBloque.duracionMin} minutos`;
        this._speak(`${nuevoBloque.titulo}, ${duracionStr}`);
      }
      if (typeof Utils !== 'undefined' && Utils.vibrate) Utils.vibrate(60);
    }
    // Reiniciar temporizador de auto‑bloqueo si estaba desbloqueado
    if (this._isUnlocked) {
      if (this._unlockTimeout) clearTimeout(this._unlockTimeout);
      setTimeout(() => { if (this._isUnlocked) startAutoLockTimer(); }, 0);
    }
...
console.log('✅ GPS Tracker v3.35 - Auto‑bloqueo del slider tras 3s inactividad, voz "go"');