// ==================== gps-tracker.js ====================
// Versión: 5.1 - VOZ NATURAL: selección automática de la mejor voz disponible
...
const GPSTracker = {
  ...
  // ============================================================
  //  ⭐ FUNCIÓN DE VOZ MEJORADA CON SELECCIÓN AUTOMÁTICA
  //  DE LA VOZ MÁS NATURAL DISPONIBLE
  // ============================================================
  _speak(text, preload = false) {
    if (!window.speechSynthesis) return;

    // Si las voces aún no están cargadas, esperar a que lo estén
    if (window.speechSynthesis.getVoices().length === 0) {
      window.speechSynthesis.onvoiceschanged = () => {
        this._speak(text, preload);
      };
      return;
    }
    ...
    // Priorizar voces humanas (Google WaveNet, Siri, etc.)
    const preferredNames = [
      'Google UK English Female',  // Google WaveNet (muy natural)
      'Google español',            // Google español
      'Google US English',         // Google US English
      'Samantha',                  // Siri (iOS, muy natural)
      'Mónica',                    // Siri español
      'es-ES',                     // Cualquier voz española
      'es-MX',                     // Cualquier voz mexicana
    ...
    // Si no se encontró ninguna voz preferida, usar cualquier voz en español
    if (!selectedVoice) {
    ...
    if (selectedVoice) {
      ...
      console.log(`🗣️ Usando voz: ${selectedVoice.name} (${selectedVoice.lang})`);
    }
  ...
  // ============================================================
  //  AJUSTE A CALLES (OSRM Map Matching)
  // ============================================================
...
console.log('✅ GPS Tracker v5.1 - Voz natural mejorada + Douglas-Peucker + ajuste a calles');