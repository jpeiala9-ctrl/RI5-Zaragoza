// ==================== gps-tracker.js ====================
// Versión: 2.3 - Completa: adquisición GPS previa, fases, botones corregidos, UI mejorada
// ====================
...
const GPSTracker = {
  ...
  _crearPantallaPreparacion() {
    ...
    overlay.innerHTML = `
      <div style="flex:1; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center; padding:20px;">
        ...
        <div style="font-size:20px; font-weight:bold; letter-spacing:2px;">ADQUIRIENDO GPS</div>
        ...
        <button id="gpsStartBtn" style="margin-top:30px; padding:12px 30px; background:#c0a060; border:none; border-radius:30px; color:#0a0a0a; font-weight:bold; font-size:16px; cursor:pointer; opacity:0.5; transition:opacity 0.3s;" disabled>COMENZAR</button>
      </div>
    `;
  ...
  _extraerDuraciones() {
    ...
    // Si no están definidos, usar valores por defecto (15% calentamiento, 10% enfriamiento)
  ...
  _actualizarBotonesSegunFase() {
    ...
    if (this.faseActual === 'enfriamiento') {
      // Ocultar botón siguiente, agrandar finalizar
  ...
  // ===== INICIAR EL ENTRENAMIENTO (DESPUÉS DE GPS LISTO) =====
  async comenzarEntreno() {
    ...
    // Reiniciar watch para el entrenamiento
    ...
    // Configurar botones con funciones flecha para mantener this
    const pauseBtn = document.getElementById('gpsPauseBtn');
    const siguienteBtn = document.getElementById('gpsSiguienteBtn');
    const finalizarBtn = document.getElementById('gpsFinalizarBtn');
  ...
  // ===== AVANZAR MANUALMENTE DE FASE O FINALIZAR =====
  async siguienteFase() {
    if (this.faseActual === 'enfriamiento') {
  ...
  // ===== BUCLE DE ACTUALIZACIÓN DE TEMPORIZADORES =====
  _updateFaseTimers() {
    ...
    if (totalFase > 0 && barElement) {
      ...
      barElement.style.width = `${Math.min(100, Math.max(0, porcentaje))}%`;
    } else if (barElement) {
  ...
  _onPosition(pos) {
    ...
    // Fase de preparación: solo actualizar estado
    ...
    // Durante el entrenamiento: aplicar filtros
    ...
    // Descartar puntos demasiado cercanos (< 4m)
  ...
  // ===== GUARDAR TRACK Y PUBLICAR EN MURO =====
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    // Decimar puntos para no sobrecargar
    ...
    // 1. Guardar track completo
    await firebaseServices.db
      .collection('users').doc(uid)
      .collection('gps_tracks')
      .add(trackData);
    
    // 2. Referencia en el plan
  ...
  async iniciar(sesion, diaIndex) {
    ...
    try {
      ...
      // timeout o sin señal: continuar de todas formas
    }
...
console.log('✅ GPSTracker v2.3 - Completo: preparación GPS + fases + botones corregidos');