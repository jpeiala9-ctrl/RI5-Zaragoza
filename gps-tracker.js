...
const GPSTracker = {
  ...
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    //    Envuelto en try-catch: si falta permiso de Firestore (regla gps_tracks)
    //    el error NO bloquea el guardado de la sesión principal.
    const trackData = {
    ...
    try {
      ...
      // Fallo no crítico: el track completo no se guardó en la subcolección,
      // pero la sesión sí se marcará. Añade la regla Firestore para gps_tracks.
      console.warn('⚠️ gps_tracks sin permiso (no crítico):', e.message);
    }

    // 2. Guardar metadata ligera en el plan (no incluye puntos → dentro del 1MB del doc)
    try {
