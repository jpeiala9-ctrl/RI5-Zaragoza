// ==================== gps-tracker.js ====================
// Versión: 3.69 - DEFINITIVA: Bandera orientable (palo como vanguardia) + auto-bloqueo 5s + toggle manual
// ====================
...
const GPSTracker = {
  ...
  _initMap(lat, lng) {
    ...
    try {
      ...
      this.flagIcon = window.L.divIcon({
        html: `<div style="font-size:28px; line-height:1; text-shadow:0 0 2px white; transform: rotate(0deg); transition: transform 0.1s linear;">🏁</div>`,
        className: '',
        iconAnchor: [14, 28]
      });
      this.currentMarker = window.L.marker([lat, lng], { icon: this.flagIcon }).addTo(this.map);
  ...
  _rotateFlag(bearingDeg) {
    if (!this.currentMarker || !this.flagIcon) return;
    const flagDiv = this.currentMarker.getElement();
    if (flagDiv) {
      const flagInner = flagDiv.querySelector('div');
      if (flagInner) {
        flagInner.style.transform = `rotate(${bearingDeg}deg)`;
      }
    }
  },
  ...
  _updateMap(lat, lng, bearing = null) {
    if (!window.L) return;
    if (!this.map) { this._initMap(lat, lng); return; }
    try {
      if (this.currentMarker) {
        this.currentMarker.setLatLng([lat, lng]);
        if (bearing !== null) this._rotateFlag(bearing);
        if (this.currentMarker.bringToFront) this.currentMarker.bringToFront();
      } else {
        this.flagIcon = window.L.divIcon({
          html: `<div style="font-size:28px; line-height:1; text-shadow:0 0 2px white; transform: rotate(0deg);">🏁</div>`,
  ...
  _onPosition(pos) {
    ...
    if (puntoSuave) {
      ...
      let bearing = null;
      if (lastPoint) {
        bearing = this._calculateBearing(lastPoint.lat, lastPoint.lng, puntoSuave.lat, puntoSuave.lng);
      }
      if (!lastPoint ||
      ...
      this._updateMap(puntoSuave.lat, puntoSuave.lng, bearing);
...
console.log('✅ GPS Tracker v3.69 - Bandera orientable (palo como vanguardia) + auto-bloqueo 5s');