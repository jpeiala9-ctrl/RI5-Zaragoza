// ==================== gps-tracker.js ====================
// Versión: 2.8 - Validación 90% + marcado automático
// ====================
...
const GPSTracker = {
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      <style>
        @media (orientation: portrait) {
          #gpsSessionScreen .map-container { flex: 0 0 40% !important; min-height: 200px; }
          #gpsSessionScreen .stats-container { padding: 8px 12px 16px !important; }
          #gpsTimer { font-size: 36px !important; }
          #gpsDistance, #gpsPace { font-size: 22px !important; }
          .btn-row { gap: 6px !important; }
          .btn-row button { height: 44px !important; font-size: 12px !important; }
        }
        #gpsConfirm {
          display: none;
          position: fixed;
        ...
        #gpsConfirm .confirm-content { width: 100%; max-width: 400px; margin: 0 auto; }
        #gpsConfirm button { width: 100%; margin-bottom: 12px; }
      ...
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        ...
        <button id="preLockStartBtn" disabled style="padding:12px 28px;border:2px solid #c0a060;background:#c0a060;color:#0a0a0a;border-radius:30px;font-size:14px;font-weight:bold;cursor:pointer;font-family:inherit;letter-spacing:2px;margin-bottom:12px;opacity:0.5;">COMENZAR</button>
        <button id="preLockForceBtn" style="display:none;padding:12px 24px;border:2px solid #666;background:transparent;color:#888;border-radius:12px;font-size:13px;cursor:pointer;margin-bottom:12px;">INICIAR IGUALMENTE</button>
        <button id="preLockCancelBtn" style="margin-top:8px;padding:10px 20px;border:none;background:transparent;color:#555;font-size:12px;cursor:pointer;">CANCELAR</button>
      </div>
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;min-height:0;">
        <div style="padding:10px 16px;background:#111;border-bottom:1px solid #222;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
          <div>
            ...
            <div id="gpsSesionNombre" style="font-size:13px;color:#c0a060;font-weight:bold;margin-top:1px;max-width:220px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"></div>
          </div>
          <div style="display:flex;align-items:center;gap:6px;">
            <div id="gpsSignalDot" style="width:8px;height:8px;border-radius:50%;background:#444;"></div>
            <span id="gpsSignalText" style="font-size:10px;color:#555;">—</span>
          </div>
        </div>
        <div id="gpsStepBar" style="padding:10px 16px;background:#141414;border-bottom:1px solid #1e1e1e;flex-shrink:0;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
            <div style="display:flex;align-items:center;gap:8px;">
              ...
              <span id="gpsStepTitle" style="font-size:13px;font-weight:bold;color:#fff;"></span>
            </div>
            <div style="text-align:right;">
              <div id="gpsStepCountdown" style="font-size:22px;font-weight:bold;color:#c0a060;">--:--</div>
              <div style="font-size:9px;color:#444;">RESTANTE</div>
            </div>
          </div>
        ...
        <div class="map-container" style="flex:1;min-height:0;position:relative;background:#0f0f0f;">
        ...
        <div class="stats-container" style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          <div style="text-align:center;margin-bottom:12px;">
            <div id="gpsTimer" style="font-size:52px;font-weight:bold;letter-spacing:4px;color:#fff;line-height:1;">00:00</div>
            <div style="font-size:9px;color:#333;letter-spacing:3px;">TIEMPO TOTAL</div>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
            <div style="text-align:center;background:#1a1a1a;border:1px solid #222;border-radius:12px;padding:10px 6px;">
              <div id="gpsDistance" style="font-size:28px;font-weight:bold;color:#c0a060;">0.00</div>
              <div style="font-size:9px;color:#444;">KM</div>
            </div>
            <div style="text-align:center;background:#1a1a1a;border:1px solid #222;border-radius:12px;padding:10px 6px;">
              <div id="gpsPace" style="font-size:28px;font-weight:bold;color:#9BB5A0;">--:--</div>
              <div style="font-size:9px;color:#444;">MIN/KM</div>
            </div>
          </div>
          <div class="btn-row" style="display:flex;gap:10px;">
            <button id="gpsPauseBtn" style="flex:1;height:50px;border:2px solid #c0a060;background:transparent;color:#c0a060;border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;">⏸ PAUSA</button>
            <button id="gpsNextBtn" style="flex:1;height:50px;border:2px solid #9BB5A0;background:#9BB5A0;color:#0a0a0a;border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;">SIGUIENTE →</button>
          </div>
          <div id="gpsPauseBanner" style="display:none;text-align:center;margin-top:10px;color:#c0a060;font-size:12px;animation:blink 1.2s step-start infinite;">⏸ EN PAUSA</div>
        </div>
      </div>
      <div id="gpsConfirm">
        <div class="confirm-content">
          <div style="font-size:48px;margin-bottom:16px;">⏹️</div>
          <div id="confirmMessage" style="font-size:14px;letter-spacing:2px;color:#c0a060;margin-bottom:20px;">FINALIZAR SESIÓN</div>
          <div id="gpsConfirmStats" style="font-size:22px;font-weight:bold;margin-bottom:30px;color:#fff;line-height:1.7;"></div>
          <div id="confirmWarning" style="display:none;background:#332211;color:#f0a050;padding:10px;border-radius:8px;margin-bottom:16px;font-size:12px;"></div>
          <button id="confirmSaveBtn" style="width:100%;height:54px;background:#c0392b;border:none;color:#fff;border-radius:14px;font-size:16px;font-weight:bold;cursor:pointer;margin-bottom:12px;">✅ GUARDAR Y SALIR</button>
          <button id="confirmForceBtn" style="display:none;width:100%;height:48px;background:#9BB5A0;border:none;color:#0a0a0a;border-radius:14px;font-size:14px;font-weight:bold;cursor:pointer;margin-bottom:12px;">⭐ MARCAR IGUALMENTE</button>
          <button id="confirmExitBtn" style="width:100%;height:48px;background:transparent;border:2px solid #555;color:#aaa;border-radius:14px;font-size:14px;cursor:pointer;margin-bottom:8px;">⚠️ SALIR SIN GUARDAR</button>
          <button id="confirmCancelBtn" style="width:100%;background:transparent;border:none;color:#555;font-size:12px;cursor:pointer;">CONTINUAR SESIÓN</button>
        </div>
      </div>
    `;
    ...
    const saveConfirmBtn = document.getElementById('confirmSaveBtn');
    const forceConfirmBtn = document.getElementById('confirmForceBtn');
    const exitConfirmBtn = document.getElementById('confirmExitBtn');
    ...
    if (saveConfirmBtn) saveConfirmBtn.onclick = () => this._confirmarFinalizar(false);
    if (forceConfirmBtn) forceConfirmBtn.onclick = () => this._confirmarFinalizar(true);
    if (exitConfirmBtn) exitConfirmBtn.onclick = () => this._salirSinGuardar();
    if (cancelConfirmBtn) cancelConfirmBtn.onclick = () => this._cancelarConfirm();
  },
  ...
  _mostrarConfirm() {
    ...
    const objetivoDist = this.sesion?.distanciaKm || null;
    const objetivoDur = this.sesion?.duracion || null;

    let cumple = true;
    let warningMsg = '';

    if (objetivoDist && distKm < 0.9 * objetivoDist) {
      cumple = false;
      warningMsg += `⚠️ Distancia recorrida (${distKm.toFixed(2)} km) es menor al 90% del objetivo (${objetivoDist} km).\n`;
    }
    if (objetivoDur && elapsedMin < 0.9 * objetivoDur) {
      cumple = false;
      warningMsg += `⚠️ Tiempo (${Math.floor(elapsedMin)} min) es menor al 90% de la duración prevista (${objetivoDur} min).\n`;
    }
    ...
    const forceBtn = document.getElementById('confirmForceBtn');
    const saveBtn = document.getElementById('confirmSaveBtn');

    if (!cumple && warningMsg) {
      if (warningDiv) {
        warningDiv.style.display = 'block';
        warningDiv.innerText = warningMsg + '¿Marcar sesión como completada de todas formas?';
      }
      if (forceBtn) forceBtn.style.display = 'block';
      if (saveBtn) saveBtn.style.background = '#555';
    } else {
      if (warningDiv) warningDiv.style.display = 'none';
      if (forceBtn) forceBtn.style.display = 'none';
      if (saveBtn) saveBtn.style.background = '#c0392b';
    }
  ...
  async _confirmarFinalizar(forceMark = false) {
    console.log('✅ Intentando guardar y finalizar (forceMark=' + forceMark + ')');
    if (!this.isRunning) return;
    ...
    try {
      if (typeof Utils !== 'undefined' && Utils.showLoading) Utils.showLoading('Guardando sesión...');
    } catch(e) {}

    try {
      const distKm = this._calcTotalDistance() / 1000;
      const elapsedMs = this._getElapsed();
      await this._guardarYPublicar(distKm, elapsedMs, forceMark);
      ...
      if (typeof Utils !== 'undefined') {
        if (Utils.hideLoading) Utils.hideLoading();
        Utils.showToast(`❌ Error al guardar: ${err.message || err}`, 'error', 5000);
      } else {
  ...
  async _guardarYPublicar(distKm, elapsedMs, forceMark = false) {
    ...
    let planId = null;
    if (window.AppState && AppState.planActualId) {
      planId = AppState.planActualId;
    } else {
      planId = localStorage.getItem('planActualId');
    }
    if (!planId) throw new Error('No se encontró planActualId');

    console.log(`Guardando para usuario ${uid}, plan ${planId}`);
    ...
    // Decidir si marcar como realizada (según forceMark o cumplimiento del 90%)
    let marcar = forceMark;
    if (!marcar) {
      const objetivoDist = this.sesion?.distanciaKm;
      const objetivoDur = this.sesion?.duracion; // en minutos
    ...
    if (marcar) {
      ...
      if (window.AppState) {
        ...
        const realizadas = JSON.parse(localStorage.getItem('sesionesRealizadas') || '{}');
        realizadas[this.diaIndex] = true;
        localStorage.setItem('sesionesRealizadas', JSON.stringify(realizadas));
      }
      if (typeof Utils !== 'undefined') Utils.showToast('✅ Sesión marcada como completada', 'success', 2000);
    } else {
      if (typeof Utils !== 'undefined') Utils.showToast('⚠️ No se alcanzó el 90% del objetivo. Sesión guardada pero NO marcada.', 'warning', 4000);
    }

    // Publicar en el muro si es entrenamiento (opcional)
    if (this.sesion && this.sesion.tipo !== 'descanso' && window.PlanGenerator) {
      ...
      const userData = (window.AppState && AppState.currentUserData) ? AppState.currentUserData : {};
...
console.log('✅ GPSTracker v2.8 - Validación 90% + marcado automático');