// ==================== gps-tracker.js ====================
// Versión: 3.59 - DEFINITIVA: la bandera SIEMPRE es el punto más avanzado (elimina cualquier punto futuro)
// ====================
...
const GPSTracker = {
  ...
  // ========== FUNCIONES MODIFICADAS PARA GARANTIZAR LA BANDERA COMO PUNTA ==========
  _updatePolyline() {
    if (!this.map || !this.polyline) return;
    if (!this.currentMarker) return;
    
    const currentPos = this.currentMarker.getLatLng();
    let validPoints = this.trackPoints.map(p => [p.lat, p.lng]);
    validPoints.push([currentPos.lat, currentPos.lng]);
    ...
    this.polyline.setLatLngs(validPoints);
  },
...
console.log('✅ GPS Tracker v3.59 - Bandera siempre como punta (elimina puntos futuros)');