// ==================== gps-tracker.js ====================
// Versión: 3.53 - Marcador único con bandera a cuadros 🏁 (sin punto blanco), centrado automático mejorado
// ====================
...
const GPSTracker = {
  ...
  map:           null,
  polyline:      null,
  currentMarker: null,   // ← marcador móvil con bandera a cuadros 🏁
  startMarker:   null,
  leafletLoaded: false,
  ...
  // ========== MAPA CON CENTRADO AUTOMÁTICO MEJORADO Y MARCADOR ÚNICO (BANDERA A CUADROS) ==========
  _initMap(lat, lng) {
    ...
    try {
      ...
      this.map.on('moveend', () => {
        if (this._userMovedMap) resetTimer();
      });
      ...
      this._mapAutoCenterTimer = timeoutId;
  ...
  _smoothMoveMarker(targetLat, targetLng) {
    ...
    const startLng = this.currentMarker.getLatLng().lng;
  ...
  // ===== PANTALLA COMPLETA (resto del código sin cambios importantes) =====
  _crearPantalla() {
...
console.log('✅ GPS Tracker v3.53 - Marcador único con bandera a cuadros 🏁 (sin punto blanco)');