// ==================== gps-tracker.js ====================
// Versión: 4.2 - Soporte para sesiones grupales (groupId) + bandera centrada
// ====================
...
const GPSTracker = {
  ...
  sesion:        null,
  diaIndex:      null,
  groupId:       null, // NUEVO: ID de la sesión grupal
  trackPoints:   [],
  watchId:       null,
  timerInterval: null,
  stepInterval:  null,
  ...
  _rawBuffer:    [],
  _lastAccepted: null,
  _lastGoodPoint: null, // Punto bueno para la bandera
  _velocities:   [],
  ...
  _filterGPS(lat, lng, accuracy, timestamp) {
    ...
    if (this._lastAccepted && this.trackPoints.length >= 2 && !punto.extrapolated) {
      const lastTwo = this.trackPoints.slice(-2);
      const dtLast = (lastTwo[1].ts - lastTwo[0].ts) / 1000;
      if (dtLast > 0.1) {
        const distLast = this._haversine(lastTwo[0].lat, lastTwo[0].lng, lastTwo[1].lat, lastTwo[1].lng);
        const speed = distLast / dtLast;
        if (speed > 1.0) {
  ...
  _initMap(lat, lng) {
    ...
    try {
      ...
      const resetAutoCenterTimer = () => {
        if (this._autoCenterTimer) clearTimeout(this._autoCenterTimer);
        this._userMovedMap = true;
        this._autoCenterTimer = setTimeout(() => {
          if (this._lastGoodPoint) {
            this._autoCentering = true;
            this.map.setView([this._lastGoodPoint.lat, this._lastGoodPoint.lng], this.map.getZoom(), { animate: true });
  ...
  _updateMap(lat, lng) {
    ...
    try {
      ...
      if (!this._userMovedMap && !this._autoCentering) {
        this.map.setView([targetLat, targetLng], this.map.getZoom(), { animate: true });
      }
  ...
  // ============================================================
  // INICIAR SESIÓN GPS (con soporte para groupId)
  // ============================================================
  async iniciar(sesion, diaIndex, groupId = null) {
    ...
    this.groupId     = groupId; // NUEVO: guardar el ID de la sesión grupal
    this.trackPoints = [];
    this._rawBuffer  = [];
  ...
  async _confirmarFinalizar() {
    ...
    Utils.showLoading();
    try {
      // ===== NUEVO: Si es una sesión grupal, subir track a GroupSessions =====
      if (this.groupId && typeof GroupSessions !== 'undefined') {
        await GroupSessions.subirTrack(this.groupId, AppState.currentUserId, {
          points: this.trackPoints.map(p => ({ lat: p.lat, lng: p.lng })),
          distanceKm: distKm,
          durationMs: elapsedMs
        });
        Utils.showToast(`✅ Track subido a la sesión grupal (${distKm.toFixed(2)} km)`, 'success');
      } else {
        // Publicación normal
        await this._guardarYPublicarNormal(distKm, elapsedMs);
      }
      Utils.hideLoading();
  ...
  // ============================================================
  // GUARDAR Y PUBLICAR NORMAL (sin grupo)
  // ============================================================
  async _guardarYPublicarNormal(distKm, elapsedMs) {
...
console.log('✅ GPS Tracker v4.2 - Soporte para sesiones grupales (groupId) + bandera centrada');