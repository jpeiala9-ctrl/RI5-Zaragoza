// ==================== gps-tracker.js ====================
// Versión: 5.4 - _extractZoneFromAction: fix de anuncio de voz en series --
//                antes solo se capturaba la PRIMERA zona mencionada en el
//                texto libre del paso "PARTE PRINCIPAL" (con estructuras
//                como "cuestas", donde el texto nombra antes la zona de
//                descanso Z2 que la de esfuerzo Z5, el aviso se quedaba
//                solo con "zona 2" y nunca mencionaba el esfuerzo real).
//                Ahora se capturan TODAS las zonas del texto, se
//                clasifican en esfuerzo (Z3+) / descanso (Z1-Z2) y se
//                anuncian ambas ("zona 4-5, descansos en zona 2"); además
//                se usa d.zona (zona de esfuerzo ya conocida de forma
//                fiable, ej. seriesConfig.zonaEsfuerzo) como respaldo
//                aunque el texto no la mencione.
...
const GPSTracker = {
  ...
  // 🔥 v2.8 fix: antes esta función solo capturaba la PRIMERA zona que
  // encontraba en el texto libre de "accion" (con match(), sin /g). En una
  // sesión de series el texto mezcla la zona de esfuerzo (Z4/Z5) y la de
  // descanso (Z2) en la misma frase -- si el texto nombraba antes la de
  // descanso (p.ej. estructura "cuestas", que describe primero la bajada/
  // trote de recuperación en Z2 y solo después el esfuerzo en Z5), el
  // anuncio se quedaba solo con "zona 2" y el esfuerzo real no se
  // mencionaba nunca. Ahora se capturan TODAS las zonas mencionadas en el
  // texto (con /g), se clasifican por convención de la app (Z1/Z2 =
  // calentamiento/descanso, Z3+ = esfuerzo) y se anuncian ambas por
  // separado. `zonaPreferida` (opcional) es la zona de esfuerzo ya
  // conocida de forma fiable (ej. d.zona / seriesConfig.zonaEsfuerzo del
  // paso PARTE PRINCIPAL) y se añade siempre al grupo de esfuerzo aunque
  // el texto libre no la mencione o la mencione distinto.
  _extractZoneFromAction(accion, zonaPreferida) {
    const esfuerzo = [];
    const descanso = [];
    const push = (n) => {
      n = parseInt(n, 10);
      if (isNaN(n)) return;
      const arr = n <= 2 ? descanso : esfuerzo;
      if (!arr.includes(n)) arr.push(n);
    };

    if (zonaPreferida) {
      const n = String(zonaPreferida).match(/\d+/);
      if (n) push(n[0]);
    }

    if (accion) {
      const matches = accion.matchAll(/zona?\s*(\d+)|Z(\d+)/gi);
      for (const m of matches) push(m[1] || m[2]);
    }

    if (esfuerzo.length && descanso.length) {
      return `zona ${esfuerzo.join('-')}, descansos en zona ${descanso.join('-')}`;
    }
    if (esfuerzo.length) return `zona ${esfuerzo.join('-')}`;
    if (descanso.length) return `zona ${descanso.join('-')}`;
    return 'zona 1';
  },
  ...
  _buildSteps(sesion) {
    ...
    if (pasos.length === 0) {
      return [
        ...
        { icono:'', titulo:'PARTE PRINCIPAL', duracionMin: d.partePrincipal || 25, accion: d.estructura || '', zona: this._extractZoneFromAction(d.estructura, d.zona) },
    ...
    return pasos.map(p => {
      ...
      return {
        ...
        duracionMin: durMin,
        accion: p.accion || '',
        zona: this._extractZoneFromAction(p.accion, d.zona)
      };
    });
  },
...
console.log('✅ GPS Tracker v5.4 - Fix anuncio de voz en series: zona de esfuerzo y descanso anunciadas por separado');