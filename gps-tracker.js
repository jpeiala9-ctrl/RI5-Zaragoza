// ==================== gps-tracker.js ====================
// Versión: 3.36 - Pitidos en cuenta atrás, slider reactivo y sin saltos
// ====================
...
const GPSTracker = {
  ...
  _beep(frequency, duration, volume = 0.2) {
    ...
    try {
      ...
      // Cerrar el contexto después de un tiempo para evitar fugas
  ...
  // ===== PANTALLA COMPLETA (con slider reactivo) =====
  _crearPantalla() {
    ...
    // Configurar el slider estilo Apple con seguimiento suave (sin saltos)
    ...
    let maxLeft = (track?.offsetWidth || 200) - 36; // ancho del track menos ancho del thumb

    // Recalcular maxLeft cuando cambie el tamaño (resize)
    const updateMaxLeft = () => {
      if (track) maxLeft = track.offsetWidth - 36;
    };
    ...
    const onStart = (e) => {
      ...
      // Mover el listener al documento para seguir incluso fuera del thumb
  ...
  // ===== CUENTA ATRÁS CON PITIDOS (sin voz) =====
  _startCountdown() {
    let count = 3;
    ...
    const interval = setInterval(() => {
      ...
      if (count > 0) {
        ...
        // Pitido final (880 Hz, 400ms) para indicar "go"
        this._beep(880, 400);
        setTimeout(() => {
          overlay.remove();
  ...
  _iniciarGrabacion() {
    ...
    // Anunciar el primer bloque con un pitido corto (opcional, pero lo dejamos)
    const primerBloque = this.steps[0];
    if (primerBloque) {
      // Podríamos poner un pitido, pero mejor no saturar
      // this._beep(660, 150);
    }
  ...
  nextStep() {
    ...
    if (esUltimo) {
      ...
      if (nuevoBloque) {
        // Pitido corto al cambiar de bloque
        this._beep(660, 100);
      }
...
console.log('✅ GPS Tracker v3.36 - Pitidos en cuenta atrás, slider reactivo sin saltos');