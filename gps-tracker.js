// ==================== gps-tracker.js ====================
// Versión: 3.62 - CORREGIDA: bandera como punta, track completo, centrado automático funcional, sin punto blanco
// ====================
...
const GPSTracker = {
  ...
  _updatePolyline() {
    ...
    const currentPos = this.currentMarker.getLatLng();
    let latlngs = this.trackPoints.map(p => [p.lat, p.lng]);
    // Añadir la posición actual de la bandera si no es ya el último punto
    if (latlngs.length === 0 || Math.abs(latlngs[latlngs.length-1][0] - currentPos.lat) > 1e-8 ||
        Math.abs(latlngs[latlngs.length-1][1] - currentPos.lng) > 1e-8) {
      latlngs.push([currentPos.lat, currentPos.lng]);
    } else {
      // Si ya es el último punto, no añadimos duplicado
    }
  ...
  _onPosition(pos) {
    ...
    if (puntoSuave) {
      // Mover la bandera primero
      this._updateMap(puntoSuave.lat, puntoSuave.lng);
      // Añadir el punto al track (no duplicados exactos)
      if (this.trackPoints.length === 0 ||
          Math.abs(this.trackPoints[this.trackPoints.length-1].lat - puntoSuave.lat) > 1e-8 ||
          Math.abs(this.trackPoints[this.trackPoints.length-1].lng - puntoSuave.lng) > 1e-8) {
...
console.log('✅ GPS Tracker v3.62 - Bandera como punta, track completo, centrado automático funcional');