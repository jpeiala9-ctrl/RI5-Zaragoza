// ==================== gps-tracker.js ====================
// Versión: 2.21 - Usa PlanGenerator.marcarSesionRealizada para unificar marcado y muro
// ====================
...
const GPSTracker = {
  ...
  async _guardarYPublicar(distKm, elapsedMs, forceMark = false) {
    ...
    if (tipoMarcado) {
      // Llamar a la función unificada de PlanGenerator para marcar (evita duplicar lógica)
      if (window.PlanGenerator && typeof PlanGenerator.marcarSesionRealizada === 'function') {
        await PlanGenerator.marcarSesionRealizada(this.diaIndex, true, tipoMarcado);
      } else {
        // Fallback: actualizar directamente (no debería ocurrir)
        console.warn('PlanGenerator no disponible, marcando directamente');
        ...
        if (diaElement) {
          ...
          diaElement.classList.add(tipoMarcado === 'completo' ? 'realizado' : 'realizado-parcial');
        }
        if (window.AppState) {
      ...
      if (typeof Utils !== 'undefined') {
        Utils.showToast('📁 Track guardado, pero no se marcó la sesión (no alcanza el 90%)', 'info', 3000);
      }
    }
  },
...
console.log('✅ GPSTracker v2.21 - Usa PlanGenerator para unificar marcado y muro');