// ==================== gps-tracker.js ====================
// Versión: 3.39 - Pitidos perfectos, voz automática, botón DESBLOQUEAR (sin slider)
// ====================
...
const GPSTracker = {
  ...
  _announceStep(step) {
    if (!step) return;
    let mensaje = `${step.titulo || ''}. `;
    if (step.accion) mensaje += step.accion;
    if (step.duracionMin) mensaje += `, ${step.duracionMin} minutos.`;
    this._speak(mensaje);
  },
  ...
  // ===== PANTALLA COMPLETA (con botón DESBLOQUEAR en lugar de slider) =====
  _crearPantalla() {
    ...
    const resetLock = () => {
      ...
      // Asegurar que el botón desbloquear esté visible
    ...
    // Listeners para resetear el temporizador cuando se usan los botones desbloqueados
  ...
  _iniciarGrabacion() {
    ...
    // Anunciar el primer bloque con voz automática
  ...
  nextStep() {
    ...
    if (esUltimo) {
      ...
      if (nuevoBloque) {
        this._beep(660, 100);
        // Anunciar el nuevo bloque con voz automática
  ...
  // Necesario para referenciar desde togglePause y nextStep
  _startAutoLockTimer() {
    if (this._unlockTimeout) clearTimeout(this._unlockTimeout);
    this._unlockTimeout = setTimeout(() => {
      if (this._isUnlocked) {
...
console.log('✅ GPS Tracker v3.39 - Pitidos perfectos, voz automática en cada bloque, botón DESBLOQUEAR centrado');