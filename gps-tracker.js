// ==================== gps-tracker.js ====================
// Versión: 2.38 - Botón "COMENZAR SESIÓN", confirmación modal instantánea, sin fuerza
// ====================
...
const GPSTracker = {
  ...
  _filterGPS(lat, lng, accuracy, timestamp) {
    this._firstReadings.push({ lat, lng, acc: accuracy, ts: timestamp });
    if (this._firstReadings.length > this._minReadingsBeforeStart * 2) this._firstReadings.shift();

    if (this._firstReadings.length < this._minReadingsBeforeStart) return null;

    if (this.trackPoints.length === 0) {
      let sumLat = 0, sumLng = 0, sumAcc = 0;
      for (let r of this._firstReadings) {
        sumLat += r.lat;
        sumLng += r.lng;
        sumAcc += r.acc;
      }
      const avgLat = sumLat / this._firstReadings.length;
      const avgLng = sumLng / this._firstReadings.length;
      const avgAcc = sumAcc / this._firstReadings.length;
      if (avgAcc > 15) return null;
      const primerPunto = { lat: avgLat, lng: avgLng, ts: timestamp, acc: Math.round(avgAcc) };
      this._lastAccepted = primerPunto;
      return primerPunto;
    }

    if (accuracy > 10 && this.trackPoints.length > 5) return null;
    ...
    if (this._rawBuffer.length < 3) return null;
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #gpsConfirm .confirm-content, #gpsConfirmTerminar .confirm-content {
          width: 100%;
          max-width: 400px;
        }
        #gpsConfirm button, #gpsConfirmTerminar button {
          width: 100%;
          margin-bottom: 10px;
          border-radius: 40px;
          font-weight: bold;
      ...
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        ...
        <button id="preLockStartBtn" disabled style="padding:12px 28px;background:#c0a060;color:#0a0a0a;border:none;border-radius:30px;font-size:14px;font-weight:bold;cursor:pointer;margin-bottom:12px;opacity:0.5;">COMENZAR SESIÓN</button>
      ...
      <div id="gpsConfirmTerminar">
        <div class="confirm-content">
          ...
          <div style="font-size:14px;color:#c0a060;margin-bottom:20px;">¿ESTÁS SEGURO DE QUE DESEAS TERMINAR LA SESIÓN?</div>
          <button id="terminarSiBtn" style="background:#c0392b;color:#fff;">✅ SÍ, TERMINAR</button>
          <button id="terminarNoBtn" style="background:transparent;border:2px solid #555;color:#aaa;">❌ NO, SEGUIR</button>
        </div>
      </div>
    `;
    ...
    const terminarSiBtn = document.getElementById('terminarSiBtn');
    const terminarNoBtn = document.getElementById('terminarNoBtn');
    ...
    if (terminarBtn) terminarBtn.onclick = () => this._mostrarConfirmTerminar();
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      if (acc < 10) {
        if (!goodSignalReceived) {
          ...
          if (status) status.innerHTML = 'GPS OK';
          if (startBtn) {
        ...
        if (goodSignalReceived) {
          ...
          if (status) status.innerHTML = 'buscando satélites';
          if (startBtn) {
  ...
  async _terminarSesionConfirmado() {
    document.getElementById('gpsConfirmTerminar').style.display = 'none';
    document.getElementById('gpsConfirm').style.display = 'none';
    const distKm = this._calcTotalDistance() / 1000;
    ...
    const tipo = alcanza90 ? 'completo' : 'parcial';
    await this._guardarYFinalizar(tipo);
  },

  async _guardarYFinalizar(tipo) {
    const distKm = this._calcTotalDistance() / 1000;
  ...
  async _guardarYPublicar(distKm, elapsedMs, tipo) {
    ...
    if (window.PlanGenerator && typeof PlanGenerator.marcarSesionRealizada === 'function') {
      ...
      if (diaElement) {
        diaElement.classList.remove('realizado');
        diaElement.classList.add('realizado');
      }
      if (window.AppState) {
...
console.log('✅ GPSTracker v2.38 - Botón "COMENZAR SESIÓN", confirmación modal instantánea');