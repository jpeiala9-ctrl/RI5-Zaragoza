// ==================== gps-tracker.js ====================
// Versión: 3.18 - Completo, con renderTrackSVG funcionando y puntos bien guardados
// ====================
...
const GPSTracker = {
  ...
  // ===== SUAVIZADO Y CONTROL DE MOVIMIENTO =====
  ...
  // ===== PANTALLA COMPLETA (solo dos botones: PAUSA y SIGUIENTE) =====
  _crearPantalla() {
  ...
  _renderStepInfo() {
    ...
    if (nextBtn) {
      ...
      if (esUltimo) {
        ...
        nextBtn.style.letterSpacing = '2px';
      } else {
  ...
  _startCountdown() {
    ...
    countDiv.id = 'gpsCountdownNumber';
  ...
  _onPosition(pos) {
    ...
    if (puntoSuave) {
      if (this.trackPoints.length === 0) {
        ...
        const distDesdeInicio = this._haversine(this.trackPoints[0].lat, this.trackPoints[0].lng, puntoSuave.lat, puntoSuave.lng);
        if (distDesdeInicio > 2.0 || this.trackPoints.length > 5) {
          this.trackPoints.push(puntoSuave);
          this._lastAccepted = puntoSuave;
          this._updatePolyline();
        } else if (distDesdeInicio > 0.5 && !this._staticWarningShown && Date.now() - this.startTime > 5000) {
          this._staticWarningShown = true;
          Utils.showToast('Movimiento muy pequeño, esperando desplazamiento real...', 'info', 3000);
        }
      }
    }
    this._updateStats();
  ...
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    // Asegurar que los puntos tienen lat y lng (estructura correcta)
    const puntosValidos = ptsFull.filter(p => p && typeof p.lat === 'number' && typeof p.lng === 'number');
    if (puntosValidos.length !== ptsFull.length) {
      console.warn('Algunos puntos no tienen lat/lng válidos', ptsFull);
    }
    
    const trackData = {
      points: puntosValidos.map(p => ({ lat: p.lat, lng: p.lng })),
    ...
    if (wallEntryId) {
      // Guardar los puntos decimados para el muro (asegurando formato correcto)
      const ptsWallValidos = ptsWall.filter(p => p && typeof p.lat === 'number' && typeof p.lng === 'number');
      await firebaseServices.db.collection('globalFeed').doc(wallEntryId).update({
        hasGPS: true,
        trackPoints: ptsWallValidos.map(p => ({ lat: p.lat, lng: p.lng })),
      ...
      console.log('Track guardado en muro con', ptsWallValidos.length, 'puntos');
    }
  },
  ...
  renderTrackSVG(points, width = 320, height = 130) {
    ...
    if (!points || !Array.isArray(points) || points.length < 2) {
      console.warn('renderTrackSVG: puntos insuficientes', points);
      return '';
    }
    const validPoints = points.filter(p => p && typeof p.lat === 'number' && typeof p.lng === 'number');
    if (validPoints.length < 2) {
      console.warn('renderTrackSVG: puntos inválidos', points);
      return '';
    }
    try {
      ...
      const pathD = 'M ' + validPoints.map(toXY).join(' L ');
      const first = toXY(validPoints[0]).split(',');
      const last = toXY(validPoints[validPoints.length - 1]).split(',');
      const svg = `<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"
        ...
        <circle cx="${first[0]}" cy="${first[1]}" r="5" fill="#9BB5A0" stroke="#fff" stroke-width="1.5"/>
        <circle cx="${last[0]}" cy="${last[1]}" r="5" fill="#c0392b" stroke="#fff" stroke-width="1.5"/>
      </svg>`;
      console.log('renderTrackSVG generado correctamente, puntos:', validPoints.length);
      return svg;
    } catch (error) {
      console.error('Error en renderTrackSVG:', error);
      return '';
    }
  }
};
...
console.log('✅ GPS Tracker v3.18 - Completo, renderTrackSVG funcionando');