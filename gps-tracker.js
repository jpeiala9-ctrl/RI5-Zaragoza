// ==================== gps-tracker.js ====================
// Versión: 3.46 - Anti-bloqueo total (Wake Lock + audio silencioso), paso automático, voz mejorada
// ====================
...
const GPSTracker = {
  ...
  // Wake Lock y audio silencioso
  _wakeLock: null,
  _keepAliveAudio: null,
  _keepAliveInterval: null,

  // ===== ANTI-BLOQUEO: AUDIO SILENCIOSO EN BUCLE =====
  _startKeepAliveAudio() {
    if (this._keepAliveAudio) return;
    try {
      // Creamos un elemento de audio con un sonido silencioso (usamos un DataURI de un silencio)
      const silentAudio = new Audio('data:audio/wav;base64,U3RlYWx0aCBXQVYgZm9ybWF0IGZha2U=');
      // No funcionará con ese base64 inválido, mejor usar un Web Audio Oscilador a frecuencia inaudible
      // Alternativa: usar un oscilador Web Audio a 1 Hz con ganancia 0
      const ctx = new (window.AudioContext || window.webkitAudioContext)();
      const gain = ctx.createGain();
      gain.gain.value = 0;
      gain.connect(ctx.destination);
      const osc = ctx.createOscillator();
      osc.frequency.value = 1; // frecuencia inaudible
      osc.connect(gain);
      osc.start();
      this._keepAliveAudio = { ctx, osc, gain };
      // No paramos nunca, mantiene el contexto de audio activo y evita el bloqueo de pantalla en muchos dispositivos
    } catch(e) {
  ...
  _stopKeepAliveAudio() {
    if (this._keepAliveAudio) {
      try {
        this._keepAliveAudio.osc.stop();
        this._keepAliveAudio.ctx.close();
      } catch(e) {}
  ...
  // Mantener despierto combinando Wake Lock + audio silencioso
  _startPreventSleep() {
    this._requestWakeLock();
    this._startKeepAliveAudio();
    // También renovamos Wake Lock cada 30 segundos por si se libera inesperadamente
  ...
  // ===== PANTALLA COMPLETA (con botón DESBLOQUEAR centrado) =====
  _crearPantalla() {
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;">
        ...
        <div style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          ...
          <div style="display:flex; justify-content:center; margin-top:12px;">
            <button id="gpsUnlockBtn" style="background:#2a2a2a; border:1px solid #c0a060; color:#c0a060; border-radius:30px; padding:12px 24px; font-size:14px; font-weight:bold; letter-spacing:2px; cursor:pointer; text-align:center;">🔓 DESBLOQUEAR</button>
          </div>
  ...
  _iniciarPreLock() {
    ...
    // Precargar AudioContext y voces
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      if (acc <= 5) {
        if (startBtn && startBtn.style.display !== 'block') {
          ...
          startBtn.onclick = () => {
            // Iniciar prevención de bloqueo ANTES de la cuenta atrás
  ...
  _onPosition(pos) {
    ...
    if (this._lastSmoothPoint) this._updateMap(this._lastSmoothPoint.lat, this._lastSmoothPoint.lng);
    if (puntoSuave) {
  ...
  _tick() {
    ...
    if (step) {
      ...
      if (restante === 0 && !this.isPaused && !this._autoNextPending && this.isRunning) {
...
console.log('✅ GPS Tracker v3.46 - Anti-bloqueo total (Wake Lock + audio silencioso)');