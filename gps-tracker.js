// ==================== gps-tracker.js ====================
// Versión: 3.47 - Voz "Sesión terminada" una sola vez, botón DESBLOQUEAR centrado, aviso anti-bloqueo
// ====================
...
const GPSTracker = {
  ...
  _endingSession: false,       // evita repetir "sesión terminada"
  ...
  // ===== ANTI-BLOQUEO: AUDIO SILENCIOSO INTERNO (sin Internet) =====
  _startKeepAliveAudio() {
    if (this._keepAliveAudio) return;
    try {
      const ctx = new (window.AudioContext || window.webkitAudioContext)();
      const gain = ctx.createGain();
      gain.gain.value = 0; // totalmente silencioso
  ...
  _announceSesionTerminada() {
    if (this._endingSession) return; // ya se anunció
  ...
  // ===== PANTALLA COMPLETA (con botón DESBLOQUEAR centrado y aviso anti-bloqueo) =====
  _crearPantalla() {
    ...
    ov.innerHTML = `
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        ...
        <div style="margin-top:12px; font-size:10px; color:#444; letter-spacing:1px;">⚠️ No bloquees el móvil durante la sesión</div>
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      if (acc <= 5) {
        if (startBtn && startBtn.style.display !== 'block') {
          ...
          startBtn.onclick = () => {
            // Activar anti-bloqueo antes de la cuenta atrás
  ...
  _tick() {
    ...
    if (step) {
      ...
      // Si el tiempo restante es 0 y no estamos en pausa, y no hay un autoNext pendiente
  ...
  nextStep(isAuto = false) {
    ...
    const esUltimo = this.stepIndex >= this.steps.length - 1;
    if (esUltimo) {
      // Evitar múltiples llamadas a finalizar
...
console.log('✅ GPS Tracker v3.47 - Voz "Sesión terminada" única, botón centrado, aviso anti-bloqueo');