// ==================== gps-tracker.js ====================
// Versión: 3.4 - Marcador móvil corregido, track suave tipo Garmin, adquisición sin números
// ====================
...
const GPSTracker = {
  ...
  _smoothAndSimplify(lat, lng, timestamp) {
    ...
    if (dist < this._minDistance) {
      // No mover el marcador (se mantiene donde está)
      return null;
    }
    ...
    if (this.trackPoints.length >= 2) {
      ...
      if (delta < this._minAngleChange) {
        // Estamos en recta: actualizar el último punto sin añadir nuevo
        this.trackPoints[this.trackPoints.length - 1] = { lat: smoothLat, lng: smoothLng, ts: timestamp };
        this._lastSmoothPoint = { lat: smoothLat, lng: smoothLng, ts: timestamp };
        this._updatePolyline();  // redibujar la polilínea
        return null; // no se añade punto
      }
    }
...
console.log('GPS Tracker v3.4 - Marcador móvil corregido, track suave tipo Garmin');