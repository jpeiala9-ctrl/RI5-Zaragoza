...
const GPSTracker = {
  ...
  _initMap(lat, lng) {
    ...
    try {
      ...
      // cuenta ni clave.
      // 🔥 v2: los mosaicos ESTÁNDAR de OpenStreetMap (el primer arreglo
      // tras lo de CartoDB) sirven de verdad, pero son mucho más
      // recargados -- nombres de calle, colores por uso de suelo,
      // etiquetas de comercios -- perdiendo el estilo limpio y
      // minimalista que tenía CartoDB "light_all" (pensado para que
      // destaque una línea de color encima, como el trazado dorado).
      // Esri "Light Gray Canvas" es gratis, sin clave, y visualmente es
      // el que más se parece a lo que había antes -- el único trade-off
      // es que no hace tanto zoom (hasta 16, antes 19).
