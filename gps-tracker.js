// ==================== gps-tracker.js ====================
// Versión: 3.0 - GPS robusto sin pausas, sin emojis, filtro mediana + extrapolación
// ====================
...
const GPSTracker = {
  ...
  // ===== PANTALLA COMPLETA (diseño serio, sin emojis) =====
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        ...
        <div id="preLockSignalBars" style="display:flex;gap:4px;margin:12px 0;">
          <div class="signal-bar" style="width:6px;height:6px;background:#222;"></div>
          <div class="signal-bar" style="width:6px;height:12px;background:#222;"></div>
          <div class="signal-bar" style="width:6px;height:18px;background:#222;"></div>
          <div class="signal-bar" style="width:6px;height:24px;background:#222;"></div>
        </div>
        <div id="preLockStatus" style="font-size:11px;color:#555;letter-spacing:1px;margin-bottom:32px;">buscando satélites...</div>
        <button id="preLockForceBtn" onclick="GPSTracker._forzarInicio()" style="display:none;padding:10px 20px;border:1px solid #666;background:transparent;color:#888;border-radius:0;font-size:12px;cursor:pointer;font-family:inherit;letter-spacing:1px;">FORZAR INICIO</button>
        <button onclick="GPSTracker.cancelar()" style="margin-top:14px;padding:8px 16px;border:none;background:transparent;color:#555;font-size:11px;cursor:pointer;font-family:inherit;letter-spacing:1px;">CANCELAR</button>
      </div>
      ...
      <div id="gpsConfirm" style="display:none;position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(10,10,10,.97);z-index:3000;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;">
        <div style="font-size:42px;font-weight:300;margin-bottom:16px;">⏹️</div>
        ...
        <button id="gpsConfirmNo"  style="width:100%;max-width:280px;height:48px;background:transparent;border:1px solid #444;color:#aaa;border-radius:14px;font-size:14px;cursor:pointer;font-family:inherit;letter-spacing:1px;">CONTINUAR</button>
      </div>
    `;
  ...
  _iniciarPreLock() {
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      // Actualizar barras de señal (simulación)
      const bars = document.querySelectorAll('#preLockSignalBars .signal-bar');
      if (bars.length) {
        let level = 0;
        if (acc < 15) level = 4;
        ...
        bars.forEach((bar, idx) => {
          bar.style.background = idx < level ? '#c0a060' : '#222';
        });
      }
      ...
      } else if (!forceBtnShown && Date.now() - preLockStarted > 15000) {
        forceBtnShown = true;
...
console.log('GPS Tracker v3.0 - GPS robusto sin pausas, sin emojis');