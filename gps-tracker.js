// ==================== gps-tracker.js ====================
// Versión: 3.43 - Pitidos nítidos, voz con zona Z3/Z4, sin mensajes de precarga visibles
// ====================
...
const GPSTracker = {
  ...
  async _beep(frequency, duration, volume = 0.2) {
    try {
      ...
      // fallback con contexto desechable
      try {
        ...
        osc.start();
        gain.gain.exponentialRampToValueAtTime(0.00001, fallbackCtx.currentTime + duration / 1000);
        osc.stop(fallbackCtx.currentTime + duration / 1000);
        setTimeout(() => fallbackCtx.close(), duration + 100);
      } catch(e2) {}
    }
  },

  // ===== VOZ (precarga silenciosa interna) =====
  ...
  // ===== PASOS DE SESIÓN (soporta Z3, Z4, etc.) =====
  _buildSteps(sesion) {
  ...
  // ===== PANTALLA COMPLETA (sin mensajes de precarga) =====
  _crearPantalla() {
  ...
  _iniciarPreLock() {
    ...
    // Inicializar AudioContext en segundo plano (sin mostrar mensajes)
    this._initAudioContext().then(() => {
      // Precargar todas las voces (en silencio)
...
console.log('✅ GPS Tracker v3.43 - Pitidos nítidos, voz con Z3/Z4, sin mensajes de precarga visibles');