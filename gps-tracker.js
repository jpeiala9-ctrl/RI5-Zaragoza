// ==================== gps-tracker.js ====================
// Versión: 3.2 - Precisión estricta (≤5m) para primera lectura, botón Finalizar, Salir sin guardar
// ====================
...
const GPSTracker = {
  ...
  // ===== PANTALLA COMPLETA (diseño serio, sin emojis, con botón COMENZAR solo cuando precisión ≤5m) =====
  _crearPantalla() {
    ...
    ov.innerHTML = `
      <!-- Pantalla de espera GPS (sin emojis) -->
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        ...
        <div style="font-size:12px;letter-spacing:3px;color:#c0a060;margin-bottom:8px;">INICIALIZANDO GPS</div>
        <div id="preLockAccuracy" style="font-size:28px;font-weight:bold;margin:12px 0;color:#c0a060;">— m</div>
        <div style="font-size:11px;color:#555;margin-bottom:20px;">precisión</div>
        <div style="width:200px;height:4px;background:#222;border-radius:2px;overflow:hidden;margin-bottom:8px;">
      ...
      <div id="gpsConfirm" style="display:none;position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(10,10,10,.97);z-index:3000;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;">
        ...
        <!-- Edición de distancia (se añade dinámicamente) -->
  ...
  _iniciarPreLock() {
    ...
    const accSpan = document.getElementById('preLockAccuracy');
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      if (accSpan) accSpan.textContent = `±${Math.round(acc)} m`;
      if (bar) {
      ...
      if (status) {
        if (acc <= 5) status.textContent = 'precisión óptima';
        else if (acc <= 15) status.textContent = 'precisión buena';
        else status.textContent = 'buscando señal...';
      }
      ...
      // Si hay error, igualmente permitir forzar? Mejor no, porque no tenemos posición.
      const startBtn = document.getElementById('preLockStartBtn');
      if (startBtn) startBtn.style.display = 'none';
  ...
  _arrancarSesion(lat, lng, acc) {
    ...
    if (acc > 5) {
      Utils.showToast('Precisión insuficiente. Espera a que sea ≤ 5m.', 'warning');
      return;
    }
  ...
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    const ptsFull = this.trackPoints;  // todos los puntos suavizados
...
console.log('GPS Tracker v3.2 - Precisión estricta (≤5m) para inicio, botón Finalizar, Salir sin guardar');