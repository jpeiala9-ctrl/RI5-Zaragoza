// ==================== gps-tracker.js ====================
// Versión: 3.21 - Definitiva: guarda puntos correctamente y genera SVG fiable
// ====================
...
const GPSTracker = {
  ...
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    // Asegurar que los puntos tienen lat/lng válidos
    const ptsFull = this.trackPoints.filter(p => p && typeof p.lat === 'number' && typeof p.lng === 'number' && !isNaN(p.lat) && !isNaN(p.lng));
    const ptsWall = this._decimarPuntos(ptsFull, 80);
    
    console.log(`📌 Guardando sesión GPS: ${ptsFull.length} puntos válidos de ${this.trackPoints.length} totales`);
    ...
    // Marcar sesión como realizada PASANDO los valores reales
    ...
    if (wallEntryId) {
      const ptsWallValidos = ptsWall.filter(p => p && typeof p.lat === 'number' && typeof p.lng === 'number' && !isNaN(p.lat) && !isNaN(p.lng));
      console.log(`📤 Actualizando muro con ${ptsWallValidos.length} puntos decimados`);
      ...
      console.warn('No se encontró wallEntryId para actualizar el track');
    }
  },
  ...
  renderTrackSVG(points, width = 320, height = 130) {
    console.log('🎨 renderTrackSVG llamado con', points?.length || 0, 'puntos');
    if (!points || !Array.isArray(points) || points.length < 2) {
      console.warn('renderTrackSVG: puntos insuficientes', points);
      return '';
    }
    const validPoints = points.filter(p => p && typeof p.lat === 'number' && typeof p.lng === 'number' && !isNaN(p.lat) && !isNaN(p.lng));
    if (validPoints.length < 2) {
      console.warn('renderTrackSVG: no hay puntos válidos tras filtro', points);
      return '';
    }
    try {
      ...
      console.log('✅ renderTrackSVG generado correctamente');
      return svg;
    } catch (error) {
...
console.log('✅ GPS Tracker v3.21 - Definitivo para el muro');