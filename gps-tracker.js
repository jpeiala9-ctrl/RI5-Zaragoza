...
const GPSTracker = {
      ...
      <div id="gpsConfirm" style="display:none;position:absolute;top:0;left:0;right:0;bottom:0;background:var(--bg-primary);z-index:3000;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;">
        ...
        <div id="gpsZonaBox" style="background:var(--stat-bg); border:1px solid var(--border-color); border-left:4px solid transparent; border-radius:12px; padding:12px; margin:0 0 20px 0; width:100%; max-width:280px; transition:border-color .2s ease;">
          <button type="button" id="gpsZonaBtn" style="width:100%; background:transparent; border:none; padding:0; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:6px; font-family:inherit;">
            ...
            <span id="gpsZonaCaret" style="font-size:9px; color:var(--text-secondary); display:inline-block; transition:transform .2s ease;">▾</span>
          </button>
          <div id="gpsZonaExpand" style="display:none; margin-top:10px; padding-top:10px; border-top:1px solid var(--border-color);">
            ...
            <div id="gpsZonaChips" style="display:flex; flex-wrap:wrap; gap:6px; justify-content:center;"></div>
          </div>
        </div>
  ...
  _mostrarConfirm() {
    ...
    const zonaCaretEl = document.getElementById('gpsZonaCaret');
    const zonaExpandEl = document.getElementById('gpsZonaExpand');
    const chipsCont = document.getElementById('gpsZonaChips');
    ...
    if (zonaBtnEl) zonaBtnEl.onclick = () => setZonaExpand(!zonaExpandAbierto);
    setZonaExpand(false); // por si se cancela y se vuelve a finalizar en la misma sesión
    ...
    if (chipsCont) {
      ...
      const pintarChips = () => {
        chipsCont.innerHTML = ZONAS_CHIPS.map(z => {
          const activo = this._zonaOverrideSeleccionada === z.codigo;
          return `<button type="button" class="gps-zona-chip" data-zona="${z.codigo || ''}" style="
            padding:8px 12px; border-radius:20px; font-family:'Courier New',monospace; font-size:12px; font-weight:bold;
        ...
        chipsCont.querySelectorAll('.gps-zona-chip').forEach(btn => {
          btn.onclick = () => {
