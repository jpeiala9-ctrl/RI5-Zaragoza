// ==================== gps-tracker.js ====================
// Versión: 3.33 - Cuenta atrás boomerang, slider de desbloqueo, autoencuadre GPS
// ====================
...
const GPSTracker = {
  ...
  _initMap(lat, lng) {
    ...
    try {
      ...
      // Autoencuadre: detectar cuando el usuario mueve el mapa
  ...
  // ===== PANTALLA COMPLETA (con overlay de bloqueo y slider) =====
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;">
        ...
        <div style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          ...
          <!-- Botones con slider de desbloqueo -->
          <div id="gpsButtonContainer">
            ...
            <div id="gpsSliderContainer" style="margin-top:12px;">
              <div style="background:#222; border-radius:30px; padding:2px; width:100%; position:relative;">
                <input type="range" id="gpsUnlockSlider" min="0" max="100" value="0" style="width:100%; -webkit-appearance:none; background:transparent;">
                <div style="position:absolute; top:0; left:0; right:0; bottom:0; display:flex; align-items:center; justify-content:center; pointer-events:none;">
                  <span id="gpsSliderLabel" style="font-size:12px; color:#aaa; letter-spacing:1px;">→ desliza para desbloquear →</span>
                </div>
              </div>
            </div>
    ...
    // Configurar el slider de desbloqueo
    const slider = document.getElementById('gpsUnlockSlider');
    ...
    const sliderLabel = document.getElementById('gpsSliderLabel');
    if (slider && buttonsLocked && buttonsUnlocked) {
      ...
      // Pequeño retoque visual para que el thumb del slider se vea
      slider.style.cssText += `; -webkit-appearance: none; background: #c0a060; height: 4px; border-radius: 2px;`;
      const thumbStyle = document.createElement('style');
      thumbStyle.textContent = `
        #gpsUnlockSlider::-webkit-slider-thumb {
  ...
  // ===== CUENTA ATRÁS CON EFECTO BOOMERANG Y VOZ "A CORRER" =====
  _startCountdown() {
    let count = 3;
    ...
    // Mostrar el primer número con animación
    animateNumber(3);
    this._speak('3');
    ...
    const interval = setInterval(() => {
      count--;
      if (count > 0) {
        // Animar la salida y entrada del siguiente número
        ...
        // Al llegar a 0, solo voz "a correr", sin texto en pantalla
        this._speak('a correr');
        // Eliminar el overlay después de un breve retardo
        setTimeout(() => {
          overlay.remove();
...
console.log('✅ GPS Tracker v3.33 - Slider de desbloqueo + autoencuadre GPS');