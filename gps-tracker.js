// ==================== gps-tracker.js ====================
// Versión: 3.40 - Precarga de pitidos y voces, formato de bloque: "Calentamiento, X minutos, zona Y"
// ====================
...
const GPSTracker = {
  ...
  // Precarga
  _preloadedAudio: false,
  _preloadedVoices: false,
  ...
  // ===== VOZ (con precarga silenciosa) =====
  _speak(text, preload = false) {
    if (!window.speechSynthesis) return;
    if (preload) {
      // Precargar: crear utterance con volumen 0 y cancelar inmediatamente
  ...
  _announceStep(step) {
    if (!step) return;
    // Formato: "Calentamiento, 10 minutos, zona 2"
    let mensaje = `${step.titulo}, ${step.duracionMin} minutos, ${step.zona}`;
    this._speak(mensaje);
  },

  // Extraer zona de la acción (ej: "Z2", "zona 2", "z1", etc.)
  _extractZoneFromAction(accion) {
    ...
    // Si no hay zona, por defecto zona 1
    return 'zona 1';
  },
  ...
  // ===== PANTALLA COMPLETA (con botón DESBLOQUEAR y precarga) =====
  _crearPantalla() {
    ...
    ov.innerHTML = `
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        ...
        <div id="preloadStatus" style="font-size:9px;color:#444;margin-top:4px;">precargando audio...</div>
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;">
        ...
        <div style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          ...
          <!-- Botones de acción: PAUSA y SIGUIENTE (inicialmente bloqueados) -->
          ...
          <!-- Botón DESBLOQUEAR (centrado, debajo de los botones) -->
          <div style="display:flex; justify-content:center; margin-top:12px;">
            <button id="gpsUnlockBtn" style="background:#2a2a2a; border:1px solid #c0a060; color:#c0a060; border-radius:30px; padding:12px 24px; font-size:14px; font-weight:bold; letter-spacing:2px; cursor:pointer;">🔓 DESBLOQUEAR</button>
          </div>
    ...
    // Configurar el botón DESBLOQUEAR
  ...
  _iniciarPreLock() {
    ...
    // Precargar recursos (audio y voces)
    this._preloadAudioAndVoices();
  ...
  _preloadAudioAndVoices() {
    if (this._preloadedAudio && this._preloadedVoices) return;
    const statusDiv = document.getElementById('preloadStatus');
    if (statusDiv) statusDiv.textContent = 'precargando audio...';

    // 1. Precargar AudioContext (crear uno y descartarlo)
    try {
      const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
      ...
      osc.start();
      gain.gain.setValueAtTime(0.0001, audioCtx.currentTime); // volumen casi nulo
      setTimeout(() => {
        osc.stop();
        audioCtx.close();
      }, 50);
      this._preloadedAudio = true;
    } catch(e) { console.warn('Precarga de audio falló', e); }

    // 2. Precargar todas las voces de los pasos (en silencio)
    if (this.steps && this.steps.length) {
      if (statusDiv) statusDiv.textContent = 'precargando instrucciones de voz...';
      for (let step of this.steps) {
        const mensaje = `${step.titulo}, ${step.duracionMin} minutos, ${step.zona}`;
        this._speak(mensaje, true); // preload = true
      }
      this._preloadedVoices = true;
    }
    if (statusDiv) statusDiv.textContent = 'todo listo';
    setTimeout(() => {
  ...
  _iniciarGrabacion() {
    ...
    // Anunciar el primer bloque con voz (ya precargada)
...
console.log('✅ GPS Tracker v3.40 - Precarga de pitidos y voces, formato de bloque con zona');