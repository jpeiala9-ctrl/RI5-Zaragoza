// ==================== gps-tracker.js ====================
// Versión: 2.10 - Layout fullscreen sin espacios negros + auth flexible
// ====================
...
const GPSTracker = {
  ...
  _crearPantalla() {
    ...
    // Aseguramos que ocupe toda la pantalla y no tenga márgenes
    ...
    ov.innerHTML = `
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Mapa: ocupa espacio disponible pero limitado en vertical */
        .map-container {
          flex: 1 1 auto;
          min-height: 180px;
          max-height: 40vh;
        ...
        /* Panel de estadísticas y botones: se coloca al final */
        .stats-container {
          flex-shrink: 0;
          margin-top: auto;
          background: #111;
        ...
        #gpsConfirm .confirm-content { width: 100%; max-width: 400px; }
        #gpsConfirm button { width: 100%; margin-bottom: 10px; border-radius: 40px; }
      ...
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        ...
        <div style="width:200px;height:6px;background:#222;border-radius:3px;overflow:hidden;margin-bottom:8px;">
          <div id="preLockBar" style="height:100%;width:0%;background:#c0a060;transition:width 0.5s,background 0.5s;"></div>
        </div>
        <div id="preLockStatus" style="font-size:11px;color:#555;margin-bottom:16px;">buscando satélites...</div>
        <button id="preLockStartBtn" disabled style="padding:12px 28px;border:2px solid #c0a060;background:#c0a060;color:#0a0a0a;border-radius:30px;font-size:14px;font-weight:bold;cursor:pointer;margin-bottom:12px;opacity:0.5;">COMENZAR</button>
  ...
  async _guardarYPublicar(distKm, elapsedMs, forceMark = false) {
    ...
    let planId = null;
    if (window.AppState && AppState.planActualId) planId = AppState.planActualId;
    else planId = localStorage.getItem('planActualId');
...
console.log('✅ GPSTracker v2.10 - Layout fullscreen sin espacios negros + auth flexible');