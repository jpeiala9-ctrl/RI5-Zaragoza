// ==================== gps-tracker.js ====================
// Versión: 3.57 - COMPLETA: bandera como punta del track (sin punto blanco, línea al final), centrado automático mejorado
// ====================
...
const GPSTracker = {
  ...
  // ========== FUNCIONES MODIFICADAS: línea termina en la bandera y movimiento instantáneo ==========
  _updatePolyline() {
    if (!this.map || !this.polyline) return;
    let latlngs = this.trackPoints.map(p => [p.lat, p.lng]);
    // Añadir posición actual de la bandera al final
  ...
  _smoothMoveMarker(targetLat, targetLng) {
    ...
    // Movimiento instantáneo (sin animación) para evitar desfase
  ...
  _onPosition(pos) {
    ...
    if (puntoSuave) {
      ...
      // No llamamos a _updatePolyline aquí porque la animación ya lo hará
    }
    this._updateStats();
  },
...
console.log('✅ GPS Tracker v3.57 - Bandera como punta del track, línea termina en la bandera');