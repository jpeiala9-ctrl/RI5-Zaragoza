// ==================== gps-tracker.js ====================
// Versión: 2.9 - Firebase auth flexible + layout móvil optimizado
// ====================
...
const GPSTracker = {
  ...
  _crearPantalla() {
    ...
    ov.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:#0a0a0a;z-index:999999;display:flex;flex-direction:column;font-family:"Courier New",monospace;color:#fff;user-select:none;';
    ...
    ov.innerHTML = `
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;min-height:0;height:100%;">
        <div style="padding:8px 12px;background:#111;border-bottom:1px solid #222;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
          <div>
        ...
        <div id="gpsStepBar" style="padding:8px 12px;background:#141414;border-bottom:1px solid #1e1e1e;flex-shrink:0;">
        ...
        <div class="map-container" style="flex:1;min-height:0;position:relative;background:#0f0f0f;">
          ...
          <div id="gpsNoGPS" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#0f0f0f;color:#555;font-size:12px;text-align:center;pointer-events:none;">
            <div style="font-size:32px;margin-bottom:8px;opacity:.5;">🗺️</div>
            <div>Esperando posición...</div>
          </div>
        </div>
        <div class="stats-container" style="background:#111;border-top:1px solid #222;padding:10px 12px 14px;flex-shrink:0;">
          <div style="text-align:center;margin-bottom:8px;">
            <div id="gpsTimer" style="font-size:40px;font-weight:bold;letter-spacing:3px;color:#fff;line-height:1;">00:00</div>
            <div style="font-size:8px;color:#333;letter-spacing:2px;">TIEMPO TOTAL</div>
          </div>
      ...
      <div id="gpsConfirm">
        <div class="confirm-content">
          ...
          <div id="confirmMessage" style="font-size:13px;letter-spacing:1px;color:#c0a060;margin-bottom:16px;">FINALIZAR SESIÓN</div>
          <div id="gpsConfirmStats" style="font-size:18px;font-weight:bold;margin-bottom:20px;color:#fff;line-height:1.5;"></div>
          <div id="confirmWarning" style="display:none;background:#332211;color:#f0a050;padding:8px;border-radius:8px;margin-bottom:16px;font-size:11px;"></div>
          <button id="confirmSaveBtn" style="width:100%;height:52px;background:#c0392b;border:none;color:#fff;border-radius:40px;font-size:15px;font-weight:bold;cursor:pointer;margin-bottom:10px;">✅ GUARDAR Y SALIR</button>
          <button id="confirmForceBtn" style="display:none;width:100%;height:48px;background:#9BB5A0;border:none;color:#0a0a0a;border-radius:40px;font-size:14px;font-weight:bold;cursor:pointer;margin-bottom:10px;">⭐ MARCAR IGUALMENTE</button>
          <button id="confirmExitBtn" style="width:100%;height:46px;background:transparent;border:2px solid #555;color:#aaa;border-radius:40px;font-size:14px;cursor:pointer;margin-bottom:8px;">⚠️ SALIR SIN GUARDAR</button>
  ...
  // Función para obtener el usuario actual de Firebase (compatible v8/v9)
  _getCurrentUser() {
    const fs = window.firebaseServices;
    if (!fs) throw new Error('firebaseServices no definido');
    // Si auth es una función (v8)
    if (typeof fs.auth === 'function') {
      return fs.auth().currentUser;
    }
    // Si auth es un objeto (v9 modular)
    if (fs.auth && typeof fs.auth === 'object') {
      return fs.auth.currentUser;
    }
    // Si existe getAuth (v9 compat)
    if (typeof fs.getAuth === 'function') {
      return fs.getAuth().currentUser;
    }
    throw new Error('No se pudo obtener auth de firebaseServices');
  },
...
console.log('✅ GPSTracker v2.9 - Firebase auth flexible + layout móvil optimizado');