// ==================== gps-tracker.js ====================
// Versión: 2.28 - Guarda puntos GPS, llama a PlanGenerator.marcarSesionRealizada con tipo 'completo'/'parcial'
// ====================
...
const GPSTracker = {
  ...
  // Filtro GPS: precisión <10m, buffer largo, anti-salto, suavizado
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        <div style="font-size:13px;letter-spacing:3px;color:#c0a060;margin-bottom:20px;">OBTENIENDO GPS...</div>
      ...
      <!-- Modal de confirmación forzada (sin datos técnicos) -->
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      if (acc < 10) {
        ...
        if (goodSignalReceived) {
          ...
          if (status) status.innerHTML = acc < 35 ? 'señal aceptable...' : 'obteniendo GPS...';
          if (startBtn) {
    ...
    if (forceMark && tipo) {
      if (window.PlanGenerator && typeof PlanGenerator.marcarSesionRealizada === 'function') {
        await PlanGenerator.marcarSesionRealizada(
          ...
          true  // silent para que no muestre toast duplicado
        );
...
console.log('✅ GPSTracker v2.28 - Guarda puntos, llama a PlanGenerator.marcarSesionRealizada');