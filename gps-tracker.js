// ==================== gps-tracker.js ====================
// Versión: 3.0 - Filtro GPS profesional, suavizado de trayectoria, mapa robusto
// ====================
...
const GPSTracker = {
  ...
  markerLayer: null,
  leafletLoaded: false,
  tileLayer: null,        // Para poder reintentar si falla
  mapRetryCount: 0,
  ...
  _rawBuffer: [],         // para filtro de centroide
  _lastAccepted: null,    // último punto aceptado
  _speedBuffer: [],       // para suavizado de velocidad (opcional)
  ...
  // ===== FILTRO GPS PROFESIONAL =====
  _filterGPS(lat, lng, accuracy, timestamp) {
    // 1. Descartar precisión muy mala (más de 25m después de los primeros puntos)
    if (accuracy > 25 && this.trackPoints.length > 3) return null;

    // 2. Speed-gate: velocidad máxima realista para corredor: 5.5 m/s (~20 km/h)
    if (this._lastAccepted) {
      ...
      if (dist / dt > 5.5 && dist > 12) {
        // Salto imposible, ignorar este punto
        return null;
      }
    }

    // 3. Buffer de centroide ponderado por precisión (últimas 4 lecturas)
    ...
    // 4. Si el movimiento es menor a 3 metros, ignorar (ruido de parada)
    if (this._lastAccepted) {
      const movimiento = this._haversine(this._lastAccepted.lat, this._lastAccepted.lng, sLat, sLng);
      if (movimiento < 3) return null;
    }

    // 5. Suavizado adicional: media móvil de los últimos 3 puntos aceptados (opcional, mejora la trayectoria)
    if (this.trackPoints.length >= 2) {
      const last = this.trackPoints[this.trackPoints.length - 1];
      const prev = this.trackPoints[this.trackPoints.length - 2];
      // Si el nuevo punto se desvía demasiado de la tendencia, lo ajustamos ligeramente
      const angleChange = Math.abs(Math.atan2(sLat - last.lat, sLng - last.lng) - 
                                   Math.atan2(last.lat - prev.lat, last.lng - prev.lng));
      if (angleChange > 0.8 && this.trackPoints.length > 5) {
        // Suavizado: 70% nuevo, 30% extrapolación lineal simple
        const extrapLat = last.lat + (last.lat - prev.lat);
        const extrapLng = last.lng + (last.lng - prev.lng);
        sLat = sLat * 0.7 + extrapLat * 0.3;
        sLng = sLng * 0.7 + extrapLng * 0.3;
      }
    }
  ...
  _initMap(lat, lng) {
    if (this.map) {
      // Si ya existe, solo movemos la vista
    ...
    try {
      ...
      this.map = window.L.map('gpsMap', { zoomControl: false, attributionControl: false, tap: false })
                         .setView([lat, lng], 16);
      // TileLayer con reintento automático
      this.tileLayer = window.L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', {
        maxZoom: 19,
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a> & CartoDB',
        errorTileUrl: '', // evitar icono roto
        // reintento en caso de error
      }).addTo(this.map);
      // Manejo de error de tiles: reintentar con otro subdominio o recargar
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsConfirm" style="display:none;position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(10,10,10,.97);z-index:3000;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;pointer-events:all;">
        ...
        <button id="gpsConfirmYes" style="width:100%;max-width:280px;height:54px;background:#c0392b;border:none;color:#fff;border-radius:14px;font-size:16px;font-weight:bold;cursor:pointer;font-family:inherit;margin-bottom:12px;">GUARDAR Y SALIR</button>
  ...
  _iniciarPreLock() {
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      const forceBtn = document.getElementById('preLockForceBtn');
      if (forceBtn) forceBtn.style.display = 'block';
    }, { enableHighAccuracy: true, maximumAge: 0, timeout: 15000 }); // maximumAge 0 para lecturas frescas
  },
...
console.log('GPSTracker v3.0 - filtro profesional, suavizado, mapa robusto');