// ==================== gps-tracker.js ====================
// Versión: 3.31 - Cuenta atrás minimalista, auto‑centrado del mapa, botón FINALIZAR con cancelar
// ====================
...
const GPSTracker = {
  ...
  _initMap(lat, lng) {
    ...
    try {
      ...
      // Detectar movimiento manual del mapa
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;">
        ...
        <div style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          ...
          <div id="gpsButtonContainer" style="display:flex; gap:10px; flex-direction:column;">
            <div style="display:flex; gap:10px;">
              <button id="gpsPauseBtn" onclick="GPSTracker.togglePause()" style="flex:1;height:50px;border:1px solid #c0a060;background:transparent;color:#c0a060;border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;letter-spacing:1px;font-family:inherit;">PAUSA</button>
              <button id="gpsNextBtn"  onclick="GPSTracker.nextStep()"   style="flex:1;height:50px;border:1px solid #9BB5A0;background:#9BB5A0;color:#0a0a0a;border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;letter-spacing:1px;font-family:inherit;">SIGUIENTE</button>
            </div>
            <div id="gpsCancelFinishContainer" style="display:none;">
              <button id="gpsCancelFinishBtn" style="width:100%;height:40px;background:transparent;border:1px solid #666;color:#aaa;border-radius:12px;font-size:12px;cursor:pointer;letter-spacing:1px;">CANCELAR</button>
            </div>
          </div>
    ...
    // Botón cancelar durante el final
    const cancelFinishBtn = document.getElementById('gpsCancelFinishBtn');
    if (cancelFinishBtn) {
      cancelFinishBtn.addEventListener('click', () => GPSTracker._cancelFinishMode());
    }
  },

  // Restaura el modo normal (con PAUSA y SIGUIENTE)
  _cancelFinishMode() {
    const pauseBtn = document.getElementById('gpsPauseBtn');
    const nextBtn = document.getElementById('gpsNextBtn');
    const cancelContainer = document.getElementById('gpsCancelFinishContainer');
    const buttonContainer = document.querySelector('#gpsButtonContainer > div:first-child');

    if (pauseBtn && nextBtn && buttonContainer && cancelContainer) {
      // Restaurar botón SIGUIENTE a su estado normal
      ...
      // Asegurar que PAUSA sea visible
      pauseBtn.style.display = 'inline-block';
      buttonContainer.style.display = 'flex';
      cancelContainer.style.display = 'none';
      // Resetear el estado de step (no se avanza)
      this._renderStepInfo(); // refresca para asegurar que el texto del título etc. esté bien
    }
  },
  ...
  _renderStepInfo() {
    ...
    if (nextBtn && pauseBtn && buttonContainer && cancelContainer) {
      if (esUltimo) {
        // Ocultar PAUSA y el contenedor de los dos botones, mostrar el grande y el cancelar
        pauseBtn.style.display = 'none';
        buttonContainer.style.display = 'none';
        nextBtn.style.display = 'block';
        nextBtn.style.width = '100%';
        nextBtn.style.flex = 'none';
  ...
  _startCountdown() {
    ...
    const interval = setInterval(() => {
      ...
      if (count > 0) {
        ...
        countDiv.textContent = 'COMENZAMOS';
        this._speak('Comenzamos');
        setTimeout(() => {
          overlay.remove();
  ...
  _tick() {
    ...
    if (step) {
      ...
      const nextBtn = document.getElementById('gpsNextBtn');
      if (nextBtn && restante === 0 && this.stepIndex < this.steps.length - 1) {
...
console.log('✅ GPS Tracker v3.31 - Cuenta atrás minimalista, auto‑centrado, botón FINALIZAR con cancelar');