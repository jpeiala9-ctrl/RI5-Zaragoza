// ==================== gps-tracker.js ====================
// Versión: 3.37 - Pitidos en cuenta atrás (sin números), slider exacto al dedo, voz en cada bloque
// ====================
...
const GPSTracker = {
  ...
  _speak(text) {
    ...
    // Cancelar cualquier locución en curso para evitar solapamiento
  ...
  _announceStep(step) {
    if (!step) return;
    const titulo = step.titulo || '';
    const accion = step.accion || '';
    let mensaje = `${titulo}. `;
    if (accion) mensaje += accion;
    // Añadir duración si está disponible
    if (step.duracionMin) {
      mensaje += `, ${step.duracionMin} minutos.`;
    }
    this._speak(mensaje);
  },
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;">
        ...
        <div style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          ...
          <!-- Slider estilo Apple sin transiciones (reactivo al dedo) -->
            ...
            <div id="gpsSliderContainer" style="margin-top:12px;">
              <div id="gpsAppleSlider" style="background:#2a2a2a; border-radius:30px; padding:3px; width:100%; display:flex; align-items:center; position:relative; cursor:pointer; touch-action:pan-x;">
                <div id="gpsSliderTrack" style="background:#444; border-radius:30px; height:40px; width:100%; position:relative; overflow:hidden;">
                  <div id="gpsSliderFill" style="background:#c0a060; width:0%; height:100%; border-radius:30px;"></div>
                </div>
                <div id="gpsSliderThumb" style="position:absolute; width:36px; height:36px; background:#fff; border-radius:50%; box-shadow:0 2px 8px rgba(0,0,0,0.3); left:2px; top:50%; transform:translateY(-50%); cursor:grab;"></div>
    ...
    // Configurar el slider estilo Apple con seguimiento exacto al dedo (sin transiciones)
    ...
    const startAutoLockTimer = () => {
      ...
      this._unlockTimeout = setTimeout(() => {
        if (this._isUnlocked) resetSlider();
      }, 3000);
    };
    ...
    const updatePosition = (clientX) => {
      ...
      if (percent >= 95 && !this._isUnlocked) unlock();
    };
  ...
  // ===== CUENTA ATRÁS CON PITIDOS (SIN NÚMEROS) =====
  _startCountdown() {
    let count = 3;
    // Solo pitidos, sin overlay de números
  ...
  _iniciarGrabacion() {
    ...
    // Anunciar el primer bloque con voz
  ...
  nextStep() {
    ...
    if (esUltimo) {
      ...
      if (nuevoBloque) {
        this._beep(660, 100);
        // Anunciar el nuevo bloque con voz
...
console.log('✅ GPS Tracker v3.37 - Pitidos en cuenta atrás (sin números), slider exacto al dedo, voz en cada bloque');