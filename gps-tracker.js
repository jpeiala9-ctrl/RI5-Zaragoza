// ==================== gps-tracker.js ====================
// Versión: 2.3 - GPS sobrio, sin emojis, descripciones claras, toast único
// ====================
...
const GPSTracker = {
  ...
  _filterGPS(lat, lng, accuracy, timestamp) {
    if (accuracy > 30 && this.trackPoints.length > 3) return null;
    if (this._lastAccepted) {
      ...
      if (dist / dt > 6.0 && dist > 15) return null;
    }
  ...
  _buildSteps(sesion) {
    ...
    let pasos = (d.pasosDetallados || []).filter(p => {
      ...
      return !titulo.includes('FUERZA') && p.icono !== '🏋️';
    });
    if (pasos.length === 0) {
      return [
        { titulo: 'CALENTAMIENTO', duracionMin: d.calentamiento || 10, accion: 'Trote suave en Z1 + ejercicios de movilidad' },
        { titulo: 'PARTE PRINCIPAL', duracionMin: d.partePrincipal || 25, accion: d.estructura || 'Mantén el ritmo indicado' },
        { titulo: 'ENFRIAMIENTO', duracionMin: d.enfriamiento || 5, accion: 'Trote suave + estiramientos' }
      ];
    }
    return pasos.map(p => {
      ...
      return { titulo: tit, duracionMin: durMin, accion: p.accion || p.porque || 'Sigue el ritmo indicado' };
    });
  },
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsConfirm" style="display:none;position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(10,10,10,.97);z-index:3000;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;pointer-events:all;">
        ...
        <button id="gpsConfirmNo" style="width:100%;max-width:280px;height:48px;background:transparent;border:2px solid #444;color:#aaa;border-radius:14px;font-size:14px;cursor:pointer;font-family:inherit;">CONTINUAR SESION</button>
      </div>
    ...
    document.getElementById('gpsConfirmYes')?.addEventListener('click', () => GPSTracker._confirmarFinalizar());
  ...
  async _confirmarFinalizar() {
    ...
    try {
      ...
      Utils.hideLoading();
      // UNICO TOAST: mostramos solo este
  ...
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    // Marcamos la sesión como realizada (esto ya muestra un toast, pero lo evitamos pasando un flag)
    await PlanGenerator.marcarSesionRealizada(this.diaIndex, true, true); // true = skipToast
...
console.log('GPSTracker v2.3 - sin emojis, descripciones claras, toast unico');