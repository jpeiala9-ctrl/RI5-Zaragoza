// ==================== gps-tracker.js ====================
// Versión: 3.64 - DEFINITIVA: centrado automático perfecto (sigue la bandera, vuelve tras 3s)
// ====================
...
const GPSTracker = {
  ...
  _userMovedMap: false,      // true si el usuario ha movido el mapa (desactiva centrado automático)
  _autoCenterTimer: null,    // temporizador para reactivar centrado
  _autoCentering: false,     // evita bucles durante el centrado forzado
  ...
  // ========== FUNCIÓN PRINCIPAL DEL MAPA (con centrado automático perfecto) ==========
  _initMap(lat, lng) {
    ...
    try {
      ...
      // Marcador único: bandera a cuadros
  ...
  _updateMap(lat, lng) {
    ...
    try {
      // Mover la bandera instantáneamente a la nueva posición
      if (this.currentMarker) {
        this.currentMarker.setLatLng([lat, lng]);
        this._updatePolyline();
      }
      // Solo centrar automáticamente si el usuario NO ha movido el mapa
      if (!this._userMovedMap && !this._autoCentering) {
  ...
  _updatePolyline() {
    ...
    if (latlngs.length === 0 ||
        Math.abs(latlngs[latlngs.length-1][0] - currentPos.lat) > 1e-8 ||
  ...
  // ===== PANTALLA COMPLETA (sin cambios, igual que en 3.62) =====
  _crearPantalla() {
...
console.log('✅ GPS Tracker v3.64 - Centrado automático perfecto (bandera siempre centrada, vuelve tras 3s)');