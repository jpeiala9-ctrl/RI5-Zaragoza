// ==================== gps-tracker.js ====================
// Versión: 2.20 - Marcado completo (verde) y parcial (naranja) con opción de forzar
// ====================
...
const GPSTracker = {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsConfirm">
        <div class="confirm-content">
          ...
          <button id="confirmForceBtn" style="display:none;background:#e67e22;color:#fff;">⭐ MARCAR IGUALMENTE</button>
          <button id="confirmExitBtn" style="background:transparent;border:2px solid #555;color:#aaa;">⚠️ SALIR SIN GUARDAR</button>
          <button id="confirmCancelBtn" style="background:transparent;border:none;color:#555;margin-top:5px;">CONTINUAR SESIÓN</button>
        </div>
      </div>
    `;
  ...
  _mostrarConfirm() {
    ...
    if (!cumple && warningMsg) {
      if (warningDiv) {
        ...
        warningDiv.innerText = warningMsg + '¿Marcar sesión como completada parcialmente? (naranja)';
      }
      if (forceBtn) forceBtn.style.display = 'block';
  ...
  async _confirmarFinalizar(forceMark = false) {
    console.log('✅ Guardando y saliendo (forceMark=' + forceMark + ')');
    if (!this.isRunning) return;
    const saveBtn = document.getElementById('confirmSaveBtn');
    const forceBtn = document.getElementById('confirmForceBtn');
    ...
    try {
      ...
      if (typeof Utils !== 'undefined') Utils.showToast(`❌ Error al guardar: ${err.message || err}`, 'error', 5000);
  ...
  async _guardarYPublicar(distKm, elapsedMs, forceMark = false) {
    ...
    if (!planId) throw new Error('No se encontró planActualId. Asegúrate de pasar planId a GPSTracker.iniciar()');
    ...
    // Determinar tipo de marcado: completo (verde) o parcial (naranja)
    ...
    let tipoMarcado = null; // null = no marcar, 'completo', 'parcial'
    if (forceMark) {
      tipoMarcado = 'parcial';
    } else if (alcanza90) {
      tipoMarcado = 'completo';
    }

    if (tipoMarcado) {
      const estado = { realizada: true, tipo: tipoMarcado };
      ...
      if (diaElement) {
        ...
        if (tipoMarcado === 'completo') diaElement.classList.add('realizado');
        else diaElement.classList.add('realizado-parcial');
      }
      if (window.AppState) {
        ...
        realizadas[this.diaIndex] = estado;
        localStorage.setItem('sesionesRealizadas', JSON.stringify(realizadas));
      }
      // No mostramos toast para no molestar
    } else {
      // Solo se guarda el track, sin marcar
      console.log('Sesión guardada pero no marcada (no alcanza 90% y no se forzó)');
    }

    // Publicar en el muro (solo si es completa, para evitar ruido)
    if (tipoMarcado === 'completo' && this.sesion && this.sesion.tipo !== 'descanso' && window.PlanGenerator) {
...
console.log('✅ GPSTracker v2.20 - Marcado completo (verde) y parcial (naranja)');