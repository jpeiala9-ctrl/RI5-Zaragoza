// ==================== gps-tracker.js ====================
// Versión: 3.6 - Marcador de inicio especial + voz en cada bloque (completa)
// ====================
...
const GPSTracker = {
  ...
  polyline:      null,
  markerLayer:   null,
  startMarker:   null,   // marcador especial de inicio
  leafletLoaded: false,
  ...
  _lastSmoothPoint:   null, // último punto suavizado (siempre actualizado)
  ...
  // ===== SUAVIZADO DE TRACK CON CORRECCIÓN DE ZIGZAG INICIAL =====
  _smoothAndSimplify(lat, lng, timestamp) {
    ...
    // Primer punto: siempre aceptar
    if (this.trackPoints.length === 0) {
      this._lastSmoothPoint = { lat: smoothLat, lng: smoothLng, ts: timestamp };
      return this._lastSmoothPoint;
    }

    // Segundo punto: aceptar sin restricciones (para establecer la primera dirección)
    ...
    // Tercer punto: proyectar sobre la recta inicial
    if (this.trackPoints.length === 2) {
      ...
      if (len2 > 0) {
        const t = ((smoothLng - p1.lng)*dx + (smoothLat - p1.lat)*dy) / len2;
        const projLng = p1.lng + t * dx;
        const projLat = p1.lat + t * dy;
        this._lastSmoothPoint = { lat: projLat, lng: projLng, ts: timestamp };
      } else {
        this._lastSmoothPoint = { lat: smoothLat, lng: smoothLng, ts: timestamp };
      }
      this.trackPoints.push(this._lastSmoothPoint);
      this._updatePolyline();
      return null;
    }

    // A partir del cuarto punto: algoritmo normal de simplificación
    const dist = this._haversine(this._lastSmoothPoint.lat, this._lastSmoothPoint.lng, smoothLat, smoothLng);
    if (dist < this._minDistance) return null;
  ...
  _addStartMarker(lat, lng) {
    ...
    const startIcon = window.L.divIcon({
      html: `<div style="width:18px;height:18px;background:#2ecc71;border:4px solid #fff;border-radius:50%;box-shadow:0 0 12px rgba(46,204,113,0.9);"></div>`,
  ...
  _arrancarSesion(lat, lng, acc) {
    ...
    // Anunciar el primer bloque por voz
...
console.log('GPS Tracker v3.6 - Marcador de inicio + voz en cada bloque (completa)');