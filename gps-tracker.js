// ==================== gps-tracker.js ====================
// Versión: 2.25 - Publica en muro verde y naranja, sin duplicar toasts, minimap clickeable
// ====================
...
const GPSTracker = {
  ...
  async _finalizarConFuerza(forceMark, tipo = null) {
    ...
    this._limpiarRecursos();
    document.getElementById('gpsTrackerOverlay')?.remove();
    // El toast lo muestra _guardarYPublicar a través de PlanGenerator (con silent=false en la última llamada)
  ...
  async _guardarYPublicar(distKm, elapsedMs, forceMark, tipo = null) {
    ...
    if (forceMark && tipo) {
      if (window.PlanGenerator && typeof PlanGenerator.marcarSesionRealizada === 'function') {
        // Llamamos con silent = true para que PlanGenerator no muestre toast, y nosotros ya mostramos el nuestro
...
console.log('✅ GPSTracker v2.25 - Publica verde y naranja, sin toasts duplicados');