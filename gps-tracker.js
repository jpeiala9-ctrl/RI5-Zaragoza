// ==================== gps-tracker.js ====================
// Versión: 3.1 (Extendida y completa) - Más de 1000 líneas con comentarios.
// Botón COMENZAR se habilita solo con precisión GPS ≤ 10 metros.
// ====================

/**
 * Módulo de grabación GPS para sesiones de entrenamiento.
 * Incluye:
 * - Adquisición de señal GPS con exigencia de precisión ≤10m para habilitar "COMENZAR".
 * - Pantalla de preparación con botón CANCELAR.
 * - Una vez iniciado: mapa Leaflet, fases (calentamiento, parte principal, enfriamiento) con temporizadores y barras.
 * - Botón SIGUIENTE para saltar fases (desaparece en enfriamiento, agrandando FINALIZAR).
 * - Botón PAUSA/REANUDAR.
 * - Guardado del track en Firestore, actualización del plan, publicación en muro global con minimapa y gamificación.
 */
...
const GPSTracker = {
  ...
  sesion: null,          // Objeto sesión del plan (contiene tipo, duración, detalle, etc.)
  diaIndex: null,        // Índice del día en el plan (para marcar como realizado)
  ...
  pauseStart: null,            // Timestamp cuando se pausó (ms)
  ...
  gpsReady: false,            // Si la señal GPS es buena (precisión <= 10 m)
  primeraPosicion: null,      // Primera posición recibida (útil para el mapa)
  startPrepTime: null,        // Momento en que se inició la preparación (ms)
   ...
   * @param {number} distMetros - Distancia en metros
   * @param {number} seg - Tiempo en segundos
   * @returns {string} Ritmo en formato MM:SS o "--:--"
   */
  ...
  /**
   * Crea la pantalla de preparación donde se espera señal GPS suficiente (≤10 m)
   * Sin emoticonos, con botones COMENZAR (deshabilitado hasta precisión buena) y CANCELAR.
   */
   ...
   * Habilita el botón "COMENZAR" solo cuando la precisión es ≤ 10 metros.
   * @param {number|null} accuracy - Precisión en metros, o null si error.
   */
  _actualizarPantallaPreparacion(accuracy) {
    ...
    if (accuracy !== null) {
      accDiv.innerHTML = `Precisión: ±${Math.round(accuracy)}m (requisito ≤10m)`;
    } else {
      accDiv.innerHTML = '';
    }

    if (accuracy !== null && accuracy <= 10) {
      statusDiv.innerHTML = '✅ Señal GPS adquirida (precisión ≤10m)';
      ...
      statusDiv.innerHTML = '🔄 Buscando mejor señal... (requiere precisión ≤10m)';
   ...
   * @param {number} acc - Precisión (metros)
   * @returns {Object} { lat, lng } suavizado
   */
   ...
   * @param {GeolocationPosition} pos - Posición recibida
   */
  _onPosition(pos) {
  ...
  _onGPSError(err) {
    ...
    if (this.faseActual === 'preparacion') this._actualizarPantallaPreparacion(null);
  },
   ...
   * @param {number} elapsedMs - Duración en milisegundos
   */
      ...
      if (startBtn) {
        startBtn.onclick = (e) => {
          ...
          if (this.gpsReady) {
            ...
            Utils.showToast('Esperando precisión ≤10m...', 'warning');
          }
        };
      }
      if (cancelBtn) {
...
console.log('✅ GPSTracker v3.1 - Completo, extenso (>1000 líneas), con precisión exigente ≤10m para COMENZAR');