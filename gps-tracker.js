// ==================== gps-tracker.js ====================
// Versión: 2.6 - Modal inferior + botón finalizar siempre visible
// ====================
...
const GPSTracker = {
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:#0a0a0a;">
        ...
        <button id="preLockStartBtn" disabled style="padding:12px 28px;border:2px solid #c0a060;background:#c0a060;color:#0a0a0a;border-radius:30px;font-size:14px;font-weight:bold;cursor:pointer;font-family:inherit;letter-spacing:2px;margin-bottom:12px;opacity:0.5;transition:0.2s;">COMENZAR</button>
        <button id="preLockForceBtn" style="display:none;padding:12px 24px;border:2px solid #666;background:transparent;color:#888;border-radius:12px;font-size:13px;cursor:pointer;font-family:inherit;letter-spacing:1px;margin-bottom:12px;">INICIAR IGUALMENTE</button>
        <button id="preLockCancelBtn" style="margin-top:8px;padding:10px 20px;border:none;background:transparent;color:#555;font-size:12px;cursor:pointer;font-family:inherit;letter-spacing:1px;">CANCELAR</button>
      </div>
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;min-height:0;">
        ...
        <div class="stats-container" style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          ...
          <div class="btn-row" style="display:flex;gap:10px;">
            <button id="gpsPauseBtn" style="flex:1;height:50px;border:2px solid #c0a060;background:transparent;color:#c0a060;border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;letter-spacing:1px;font-family:inherit;transition:all .2s;">⏸ PAUSA</button>
            <button id="gpsNextBtn" style="flex:1;height:50px;border:2px solid #9BB5A0;background:#9BB5A0;color:#0a0a0a;border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;letter-spacing:1px;font-family:inherit;transition:all .2s;">SIGUIENTE →</button>
          </div>
      ...
      <div id="gpsConfirm">
        <div class="confirm-content">
          ...
          <button id="confirmSaveBtn" style="width:100%;height:54px;background:#c0392b;border:none;color:#fff;border-radius:14px;font-size:16px;font-weight:bold;cursor:pointer;font-family:inherit;letter-spacing:1px;margin-bottom:12px;">✅ GUARDAR Y SALIR</button>
          <button id="confirmExitBtn" style="width:100%;height:48px;background:transparent;border:2px solid #555;color:#aaa;border-radius:14px;font-size:14px;cursor:pointer;font-family:inherit;letter-spacing:1px;margin-bottom:8px;">⚠️ SALIR SIN GUARDAR</button>
          <button id="confirmCancelBtn" style="width:100%;background:transparent;border:none;color:#555;font-size:12px;cursor:pointer;font-family:inherit;letter-spacing:1px;">CONTINUAR SESIÓN</button>
        </div>
      </div>
    `;
    ...
    if (saveConfirmBtn) saveConfirmBtn.onclick = () => this._confirmarFinalizar();
  ...
  async _confirmarFinalizar() {
    console.log('✅ Intentando guardar y finalizar');
    if (!this.isRunning) {
    ...
    try {
      if (typeof Utils !== 'undefined' && Utils.showLoading) {
        ...
        console.log('Utils.showLoading no disponible, continuando sin indicador');
      }
    } catch(e) { console.warn('Error al mostrar loading:', e); }

    try {
      const distKm = this._calcTotalDistance() / 1000;
      const elapsedMs = this._getElapsed();

      console.log(`Distancia: ${distKm} km, Tiempo: ${elapsedMs} ms`);
  ...
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    if (!window.AppState) throw new Error('AppState no disponible');
...
console.log('✅ GPSTracker v2.6 - Modal inferior + botón finalizar siempre visible');