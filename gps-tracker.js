// ==================== gps-tracker.js ====================
// Versión: 2.0 - Estabilización GPS + temporizadores por fases + botón siguiente + finalizar corregido
// ====================
...
const GPSTracker = {
  ...
  _actualizarFaseUI() {
    ...
    if (siguienteBtn) {
      // Cambiamos el comportamiento: el botón siguiente siempre visible, pero cambia de texto en la última fase
      if (this.faseActual === 'enfriamiento') {
        siguienteBtn.textContent = '🏁 FINALIZAR';
        siguienteBtn.style.borderColor = '#c0392b';
        siguienteBtn.style.color = '#c0392b';
      } else {
        siguienteBtn.textContent = '⏩ SIGUIENTE';
        siguienteBtn.style.borderColor = '#3498db';
        siguienteBtn.style.color = '#3498db';
      }
    }
  },

  // Avanza manualmente a la siguiente fase (o finaliza si es la última)
  async siguienteFase() {
    if (this.faseActual === 'enfriamiento') {
      // Si ya estamos en enfriamiento, el botón actúa como finalizar
      await this.finalizar();
      return;
    }
    
    // Confirmación opcional (se puede eliminar si molesta)
  ...
  _updateFaseTimers() {
    ...
    // Comprobar si la fase ha terminado (solo para pasar automáticamente, excepto la última)
  ...
  _onPosition(pos) {
    ...
    // Descartar puntos muy imprecisos (peor que 25m) excepto el primero
  ...
  async iniciar(sesion, diaIndex) {
    ...
    // Configurar botones con bind para asegurar el contexto
