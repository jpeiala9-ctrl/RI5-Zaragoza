// ==================== gps-tracker.js ====================
// Versión: 2.22 - Lógica de finalización: Terminar/Continuar, verde si 90%, naranja si se fuerza
// ====================
...
const GPSTracker = {
  ...
  // ========== HAVERSINE Y FILTRO (igual que antes) ==========
  ...
  // ========== PANTALLA COMPLETA (con nuevo modal de dos pasos) ==========
  _crearPantalla() {
  ...
  async _finalizarConFuerza(forceMark, tipo = null) {
    // Si forceMark = true y tipo es 'completo' o no se pasa, será verde; si forceMark = true sin tipo y viene del modal naranja, será 'parcial'
    const tipoReal = tipo || (forceMark ? 'parcial' : null);
    // Guardar track
    await this._guardarYPublicar(this._calcTotalDistance() / 1000, this._getElapsed(), forceMark, tipoReal);
    this._limpiarRecursos();
    document.getElementById('gpsTrackerOverlay')?.remove();
  ...
  async _guardarYPublicar(distKm, elapsedMs, forceMark, tipo = null) {
    ...
    if (forceMark && tipo) {
      // Llamar a la función unificada de PlanGenerator
      if (window.PlanGenerator && typeof PlanGenerator.marcarSesionRealizada === 'function') {
        await PlanGenerator.marcarSesionRealizada(this.diaIndex, true, tipo);
      } else {
        // Fallback directo
...
console.log('✅ GPSTracker v2.22 - Lógica Terminar/Continuar + verde/naranja');