// ==================== gps-tracker.js ====================
// Versión: 3.9 - Cuenta atrás de 3 segundos con voz + precarga de voz
// ====================
...
const GPSTracker = {
  ...
  _pendingStart: null, // para guardar lat,lng,acc durante la cuenta atrás
  ...
  _initVoice() {
    ...
    if (this._selectedVoice) {
      ...
      window.speechSynthesis.addEventListener('voiceschanged', () => this._initVoice(), { once: true });
    }
  },

  _speak(text) {
    if (!window.speechSynthesis) return;
    if (!this._voiceReady) this._initVoice();
    ...
    this.watchId = navigator.geolocation.watchPosition(pos => {
      ...
      if (acc <= 5) {
        if (startBtn && startBtn.style.display !== 'block') {
          ...
          // Precargar la voz cuando el GPS está listo
          if (!this._voiceReady) this._initVoice();
          startBtn.onclick = () => {
            // Preparar sesión (mapa, puntos) y luego hacer la cuenta atrás
...
console.log('GPS Tracker v3.9 - Cuenta atrás de 3 segundos con voz + precarga de voz');