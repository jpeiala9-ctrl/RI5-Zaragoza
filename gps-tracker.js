// ==================== gps-tracker.js ====================
// Versión: 3.3 - Adquisición GPS sin números, track suave tipo Garmin
// ====================
...
const GPSTracker = {
  ...
  sesion:        null,
  diaIndex:      null,
  trackPoints:   [],   // puntos ya filtrados y guardados (suavizados)
  watchId:       null,
  timerInterval: null,
  stepInterval:  null,
  startTime:     null,
  ...
  // Buffer robusto (mediana) para filtro de entrada
  _rawBuffer:    [],   // últimas lecturas crudas {lat, lng, acc, ts}
  _lastAccepted: null, // último punto aceptado {lat, lng, ts}
  _velocities:   [],   // últimas velocidades (para percentil)

  // ===== PARÁMETROS DE SUAVIZADO DE TRACK (estilo Garmin) =====
  _smoothBuffer:      [],   // cola para media móvil (máx 5 puntos)
  _lastSmoothPoint:   null, // último punto suavizado añadido al track
  _lastAngle:         null, // último ángulo para detección de rectas
  _minDistance:       3.0,  // metros: distancia mínima para añadir punto
  _minAngleChange:    10,   // grados: cambio mínimo para añadir punto en recta
  _smoothWindow:      5,    // ventana de media móvil (número de puntos)
  ...
  _filterGPS(lat, lng, accuracy, timestamp) {
    ...
    // 3. Detectar outlier: comparar la mediana con la última posición aceptada
    ...
    if (this._lastAccepted) {
      ...
      // Si la distancia es excesiva (> 40m), aplicamos extrapolación
      if (distToLast > 40 && this.trackPoints.length >= 2) {
        // Extrapolar usando la velocidad de los dos últimos puntos aceptados
    ...
    // 4. Speed-gate adaptativo (percentil de velocidades recientes)
    let maxSpeed = 8.0; // m/s (28.8 km/h) base
    ...
    if (this._lastAccepted && !punto.extrapolated) {
      ...
      const speed = dist / dt;
      if (speed > maxSpeed && dist > 15) {
        // Descartar punto errático, no actualizamos _lastAccepted
        return null;
      }
  ...
  _smoothAndSimplify(lat, lng, timestamp) {
    ...
    if (!this._lastSmoothPoint) {
      this._lastSmoothPoint = { lat: smoothLat, lng: smoothLng, ts: timestamp };
      return this._lastSmoothPoint;
    }

    // 1. Distancia mínima desde el último punto aceptado
    const dist = this._haversine(this._lastSmoothPoint.lat, this._lastSmoothPoint.lng, smoothLat, smoothLng);
    if (dist < this._minDistance) return null; // muy cerca, no añadir

    // 2. Detectar si estamos en una línea recta (cambio de ángulo pequeño)
    // Para ello necesitamos al menos dos puntos previos en el track final
    if (this.trackPoints.length >= 2) {
      ...
      if (delta < this._minAngleChange) {
        // Estamos en recta: no añadir punto nuevo, solo actualizar el último (lo alarga)
        this.trackPoints[this.trackPoints.length - 1] = { lat: smoothLat, lng: smoothLng, ts: timestamp };
        this._lastSmoothPoint = { lat: smoothLat, lng: smoothLng, ts: timestamp };
        // También actualizar la polilínea en el mapa (reescribir el último segmento)
        if (this.map && this.polyline) {
          const latlngs = this.trackPoints.map(p => [p.lat, p.lng]);
          this.polyline.setLatLngs(latlngs);
        }
        return null; // no se añade nuevo punto, solo se actualizó el último
      }
    }

    // Si llegamos aquí, se añade un nuevo punto (cambio de dirección significativo)
  ...
  // ===== PASOS DE SESIÓN (sin fuerza) =====
  _buildSteps(sesion) {
    const d = sesion.detalle;
    if (!d) return [{ icono:'', titulo:'SESION', duracionMin: sesion.duracion || 45, accion:'' }];
    ...
    return pasos.map(p => {
      ...
      return { icono: '', titulo: tit, duracionMin: durMin, accion: p.accion || '' };
    });
  },

  // ===== LEAFLET con mapa claro y movimiento suave =====
  _loadLeaflet() {
  ...
  _initMap(lat, lng) {
    ...
    try {
      ...
      const icon = window.L.divIcon({
        html:`<div style="width:14px;height:14px;background:#2ecc71;border:3px solid #fff;border-radius:50%;box-shadow:0 0 10px rgba(46,204,113,.9);"></div>`,
  ...
  _updateMap(lat, lng) {
    ...
    try {
      // La polilínea se actualiza desde _smoothAndSimplify cuando se añade o modifica un punto
      // Aquí solo movemos el marcador
      this._smoothMoveMarker(lat, lng);
  ...
  _smoothMoveMarker(targetLat, targetLng) {
    if (!this.markerLayer) return;
    const startLat = this.markerLayer.getLatLng().lat;
    const startLng = this.markerLayer.getLatLng().lng;
  ...
  // ===== PANTALLA COMPLETA (sin números de precisión) =====
  _crearPantalla() {
    ...
    ov.innerHTML = `
      <!-- Pantalla de espera GPS (sin números de precisión) -->
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;">
        ...
        <div style="background:#111;border-top:1px solid #222;padding:14px 16px 20px;flex-shrink:0;">
          ...
          <div style="display:flex;gap:10px;">
            <button id="gpsPauseBtn" onclick="GPSTracker.togglePause()" style="flex:1;height:50px;border:1px solid #c0a060;background:transparent;color:#c0a060;border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;letter-spacing:1px;font-family:inherit;transition:all .2s;">PAUSA</button>
            <button id="gpsNextBtn"  onclick="GPSTracker.nextStep()"   style="flex:1;height:50px;border:1px solid #9BB5A0;background:#9BB5A0;color:#0a0a0a;border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;letter-spacing:1px;font-family:inherit;transition:all .2s;">SIGUIENTE</button>
            <button id="gpsFinishBtn" onclick="GPSTracker._mostrarConfirm()" style="flex:1;height:50px;border:1px solid #666;background:transparent;color:#aaa;border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;letter-spacing:1px;font-family:inherit;transition:all .2s;">FINALIZAR</button>
          </div>
      ...
      <!-- Confirmación final (con tres botones, sin emoji) -->
      <div id="gpsConfirm" style="display:none;position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(10,10,10,.97);z-index:3000;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;">
        ...
        <div style="margin: 10px 0 20px 0;">
          ...
          <input type="number" id="gpsEditDistance" step="0.01" style="
            width:100%;
            max-width:180px;
            margin:8px auto;
            padding:8px 12px;
  ...
  _iniciarPreLock() {
    ...
    // Ocultar botón start inicialmente
    if (startBtn) startBtn.style.display = 'none';

    this.watchId = navigator.geolocation.watchPosition(pos => {
      const acc = pos.coords.accuracy;
      // Actualizar barra de progreso (solo visual, sin números)
      if (bar) {
        // Mapear precisión a ancho de barra: 0-60m -> 0-100%
      ...
      // Solo mostrar el botón COMENZAR cuando la precisión sea ≤ 5 metros
      if (acc <= 5) {
        if (startBtn && startBtn.style.display !== 'block') {
          startBtn.style.display = 'block';
          startBtn.onclick = () => {
            this._arrancarSesion(pos.coords.latitude, pos.coords.longitude, acc);
          };
        }
      } else {
        if (startBtn) startBtn.style.display = 'none';
  ...
  _arrancarSesion(lat, lng, acc) {
    ...
    // Si la precisión no es ≤5m, no arrancamos (por si llaman manualmente)
    if (acc > 5) {
      Utils.showToast('Precisión insuficiente. Espera a que el GPS esté listo.', 'warning');
      return;
    }
    ...
    // Inicializar mapa con la posición actual
    ...
    // Aceptar el primer punto con alta precisión (se añade manualmente)
    ...
    // Limpiar watch anterior y crear nuevo
  ...
  _onPosition(pos) {
    ...
    // Actualizar barras de señal en sesión
    ...
    if (bars.length) {
      ...
      bars.forEach((bar, idx) => {
        bar.style.background = idx < level ? '#c0a060' : '#333';
      });
    }
    ...
    // Paso 1: filtro robusto (elimina outliers, extrapola)
    const puntoFiltrado = this._filterGPS(lat, lng, accuracy, Date.now());
    if (!puntoFiltrado) return;

    // Paso 2: suavizado y simplificación (estilo Garmin)
    const puntoSuave = this._smoothAndSimplify(puntoFiltrado.lat, puntoFiltrado.lng, puntoFiltrado.ts);
    if (!puntoSuave) {
      // No se añadió nuevo punto, pero la polilínea pudo actualizarse (en caso de recta)
      // Actualizar estadísticas igualmente
      this._updateStats();
      return;
    }

    // Añadir el punto suavizado al track final
  ...
  async _confirmarFinalizar() {
    ...
    try {
      ...
      Utils.showToast(`Error GPS: ${msg}`, 'error', 6000);
    }
  },
  ...
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    const ptsFull = this.trackPoints;  // todos los puntos suavizados y simplificados
    ...
    try {
      ...
      console.warn('gps_tracks sin permiso (no crítico):', e.message);
    }
    ...
    try {
      ...
      console.warn('No se pudo guardar metadata GPS en el plan:', e.message);
    }
...
console.log('GPS Tracker v3.3 - Adquisición sin números, track suave tipo Garmin');