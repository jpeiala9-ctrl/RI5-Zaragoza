...
const GPSTracker = {
  ...
  _crearPantalla() {
    ...
    ov.innerHTML = `
      <div id="gpsPreLock" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;background:var(--bg-primary);">
        ...
        <div style="font-size:12px;letter-spacing:3px;color:var(--level-color);margin-bottom:8px;">ADQUIRIENDO GPS</div>
        <div style="width:200px;height:4px;background:var(--border-color);border-radius:2px;overflow:hidden;margin:20px auto 8px;">
          <div id="preLockBar" style="height:100%;width:0%;background:var(--level-color);transition:width 0.3s;"></div>
        </div>
        ...
        <div style="display:flex;gap:16px;margin-top:32px;">
          ...
          <button id="preLockStartBtn" style="display:none;padding:10px 24px;border:1px solid var(--level-color);background:transparent;color:var(--level-color);border-radius:0;font-size:14px;cursor:pointer;font-family:inherit;letter-spacing:2px;">COMENZAR</button>
        </div>
        <div style="margin-top: 24px; font-size: 14px; font-weight: bold; color: var(--level-color); background: color-mix(in srgb, var(--level-color) 10%, transparent); padding: 8px 16px; border-radius: 20px; letter-spacing: 1px;">
      ...
      <div id="gpsSessionScreen" style="flex:1;display:none;flex-direction:column;">
        <div style="padding: max(10px, env(safe-area-inset-top)) 16px 10px 16px; background:var(--bg-secondary); border-bottom:1px solid var(--border-color); display:flex; align-items:center; justify-content:space-between; flex-shrink:0;">
          <div>
            ...
            <div id="gpsSesionNombre" style="font-size:13px;color:var(--level-color);font-weight:bold;letter-spacing:1px;margin-top:1px;max-width:220px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"></div>
          </div>
        ...
        <div id="gpsStepBar" style="padding:10px 16px;background:var(--bg-secondary);border-bottom:1px solid var(--border-color);flex-shrink:0;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
            ...
            <div style="text-align:right;">
              <div id="gpsStepCountdown" style="font-size:22px;font-weight:bold;color:var(--level-color);font-variant-numeric:tabular-nums;">--:--</div>
        ...
        <div style="background:var(--bg-secondary);border-top:1px solid var(--border-color);padding:14px 16px 20px;flex-shrink:0;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;">
            <div style="text-align:center;background:var(--stat-bg);border:1px solid var(--border-color);border-radius:12px;padding:10px 6px;">
              <div id="gpsDistance" style="font-size:28px;font-weight:bold;color:var(--level-color);font-variant-numeric:tabular-nums;">0.00</div>
          ...
          <div id="gpsButtonsContainer">
            <div id="gpsButtonsLocked" style="display:flex; gap:10px; opacity:0.5;">
              <button disabled style="flex:1;height:50px;border:1px solid var(--level-color);background:transparent;color:var(--level-color);border-radius:12px;font-size:14px;font-weight:bold;letter-spacing:1px;">PAUSA</button>
            ...
            <div id="gpsButtonsUnlocked" style="display:none; gap:10px;">
              <button id="gpsPauseBtn" onclick="GPSTracker.togglePause()" style="flex:1;height:50px;border:1px solid var(--level-color);background:transparent;color:var(--level-color);border-radius:12px;font-size:14px;font-weight:bold;cursor:pointer;letter-spacing:1px;">PAUSA</button>
          ...
          <div style="display:flex; justify-content:center; margin-top:12px;">
            <button id="gpsUnlockBtn" style="display:flex; align-items:center; justify-content:center; background:var(--stat-bg); border:1px solid var(--level-color); color:var(--level-color); border-radius:14px; padding:12px 24px; font-size:14px; font-weight:bold; letter-spacing:2px; cursor:pointer; text-align:center;">🔓 DESBLOQUEAR</button>
          </div>

          <div id="gpsPauseBanner" style="display:none;text-align:center;margin-top:10px;color:var(--level-color);font-size:12px;letter-spacing:2px;">EN PAUSA</div>
        </div>
      </div>
      ...
      <div id="gpsConfirm" style="display:none;position:absolute;top:0;left:0;right:0;bottom:0;background:var(--bg-primary);z-index:3000;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;">
        <div style="font-size:14px;letter-spacing:2px;color:var(--level-color);margin-bottom:20px;">FINALIZAR SESION</div>
        ...
        <div style="margin: 10px 0 20px 0;">
          ...
          <input type="number" id="gpsEditDistance" step="0.01" style="width:100%;max-width:180px;margin:8px auto;padding:8px 12px;background:var(--stat-bg);border:1px solid var(--level-color);border-radius:10px;color:var(--text-primary);text-align:center;font-family:monospace;display:block;">
        </div>
  ...
  _renderStepDots() {
    ...
    container.innerHTML = this.steps.map((s, i) => {
      ...
      const bg       = done ? '#9BB5A0' : active ? 'var(--level-color)' : 'var(--border-color)';
  ...
  _onPosition(pos) {
    ...
    if (bars.length) {
      ...
      bars.forEach((bar, idx) => { bar.style.background = idx < level ? 'var(--level-color)' : 'var(--border-color)'; });
    }
  ...
  _tick() {
    ...
    if (step) {
      ...
      if (cdEl) {
        ...
        cdEl.style.color = restante === 0 ? '#e74c3c' : 'var(--level-color)';
      }
  ...
  togglePause() {
    ...
    if (this.isPaused) {
      ...
      this.pauseStart = null;
      this.isPaused = false;
      if (btn) { btn.innerHTML = 'PAUSA'; btn.style.color = 'var(--level-color)'; btn.style.borderColor = 'var(--level-color)'; }
...
console.log('✅ GPS Tracker v5.2 - Guarda recordsPorTramo por sesión para récords 100% fiables solo-GPS');