// ==================== gps-tracker.js ====================
// Versión: 3.8 - Voz Google español específica + todas las funciones completas
// ====================
...
const GPSTracker = {
  ...
  // ===== VOZ: Google español exclusivamente =====
  _selectedVoice: null,

  _initVoice() {
    if (!window.speechSynthesis) return;
    const voices = window.speechSynthesis.getVoices();
    // Buscar exactamente "Google español" o variantes de Google en español
    this._selectedVoice = voices.find(v => v.lang === 'es-ES' && v.name.includes('Google')) ||
                          voices.find(v => v.lang === 'es-ES' && v.name.toLowerCase().includes('google')) ||
                          voices.find(v => v.lang === 'es-ES' && v.name.includes('español')) ||
                          voices.find(v => v.lang === 'es-ES'); // fallback a cualquier voz española
    if (this._selectedVoice) {
      console.log('🎤 Voz seleccionada:', this._selectedVoice.name);
    } else {
      console.warn('⚠️ No se encontró voz Google español, se usará la predeterminada');
    }
  },

  _speak(text) {
    if (!window.speechSynthesis) return;
    // Inicializar voces si no se ha hecho
    if (!this._selectedVoice) {
      this._initVoice();
      // Si aún no hay voces disponibles, esperar a que se carguen
  ...
  _smoothAndSimplify(lat, lng, timestamp) {
    ...
    // Tercer punto: proyectar sobre la recta formada por los dos primeros
    ...
    // A partir del cuarto punto: algoritmo de simplificación
    const dist = this._haversine(this._lastSmoothPoint.lat, this._lastSmoothPoint.lng, smoothLat, smoothLng);
...
console.log('GPS Tracker v3.8 - Voz Google española específica + todas las funciones completas');