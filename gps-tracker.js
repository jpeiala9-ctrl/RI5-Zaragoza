// ==================== gps-tracker.js ====================
// Versión: 3.45 - Paso automático al llegar a cero, "Sesión terminada", Wake Lock, botón DESBLOQUEAR centrado
// ====================
...
const GPSTracker = {
  ...
  stepIndex:     0,
  stepStartTime: null,
  _autoNextPending: false, // evita múltiples pasos automáticos
  ...
  // ===== WAKE LOCK (evita bloqueo de pantalla) =====
  ...
  _iniciarPreLock() {
    ...
    // Inicializar AudioContext y precargar voces (sin mostrar mensajes)
  ...
  _tick() {
    ...
    if (step) {
      ...
      // Si el tiempo restante es 0 y no estamos en pausa y no hay un autoNext pendiente, pasar al siguiente bloque automáticamente
      if (restante === 0 && !this.isPaused && !this._autoNextPending && this.isRunning) {
        this._autoNextPending = true;
        this.nextStep(true); // true = automático
  ...
  nextStep(isAuto = false) {
    ...
    const esUltimo = this.stepIndex >= this.steps.length - 1;
    if (esUltimo) {
      // Si es el último bloque y llegamos aquí (por auto o por botón), finalizar sesión
...
console.log('✅ GPS Tracker v3.45 - Paso automático, "Sesión terminada", Wake Lock, botón DESBLOQUEAR centrado');