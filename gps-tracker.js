// ==================== gps-tracker.js ====================
// Versión: 2.4 - Guarda duración real en el muro, cámara centrada con flyTo
// ====================
...
const GPSTracker = {
  ...
  // ===== FILTRO GPS: 3 capas mejorado =====
  _filterGPS(lat, lng, accuracy, timestamp) {
    // Capa 1: descartar precisión mala (>30m)
    if (accuracy > 30 && this.trackPoints.length > 3) return null;

    // Capa 2: speed-gate – velocidad máxima 6 m/s (~21.6 km/h)
  ...
  _buildSteps(sesion) {
    ...
    // Filtrar pasos que NO sean de fuerza
  ...
  // *** MODIFICACIÓN: centrado suave con flyTo ***
  _updateMap(lat, lng) {
    if (!window.L) return;
  ...
  // ===== PANTALLA COMPLETA (con safe-area) =====
  _crearPantalla() {
    ...
    ov.innerHTML = `
      ...
      <!-- CONFIRM INLINE (con edición de distancia) -->
      <div id="gpsConfirm" style="display:none;position:absolute;top:0;left:0;right:0;bottom:0;background:rgba(10,10,10,.97);z-index:3000;flex-direction:column;align-items:center;justify-content:center;padding:30px;text-align:center;pointer-events:all;">
        ...
        <!-- Se añadirá input editable dinámicamente -->
  ...
  _arrancarSesion(lat, lng, acc) {
    ...
    this.watchId = navigator.geolocation.watchPosition(
      ...
      { enableHighAccuracy:true, maximumAge:1000, timeout:10000 }  // frecuencia 1s
    );
  ...
  async _guardarYPublicar(distKm, elapsedMs) {
    ...
    try {
      ...
      console.warn('⚠️ gps_tracks sin permiso (no crítico):', e.message);
    }
    ...
    try {
      ...
      console.warn('⚠️ No se pudo guardar metadata GPS en el plan:', e.message);
    }

    // *** MODIFICACIÓN: pasar skipToast = true para evitar doble notificación ***
    ...
    if (wallEntryId) {
      await firebaseServices.db.collection('globalFeed').doc(wallEntryId).update({
        ...
        duration:      Math.round(elapsedMs / 60000)   // duración real en minutos
      });
    }
  },
...
console.log('✅ GPSTracker v2.4 - guarda duración real en el muro, cámara centrada');