...
// Versión: 3.5 - Añadido marcarTodoLeido y optimización con Promise.all
// ====================
...
const Feed = {
  async cargarFeed(marcarLeido = false) {
    ...
    try {
      ...
      // Actualizar badge de novedades (si se pide marcado como leído, resetear a 0)
      if (marcarLeido) {
        await this.marcarTodoLeido();
      } else {
        const amigosConActividad = new Set(actividades.map(a => a.amigo.uid));
        AppState.actualizarNovedadesFeed(actividades.length, amigosConActividad);
      }
