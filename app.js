...
const UI = {
  ...
  guardarEstado() {
    ...
    // Guardar en localStorage específico del usuario para persistencia entre sesiones
  ...
  async restaurarEstado() {
    if (!AppState || !AppState.currentUserId) return;
    const uid = AppState.currentUserId;
    // Intentar restaurar primero desde localStorage (persistente)
      ...
      try {
        ...
        if (visible && planId) {
          ...
          if (planExiste && planExiste.sesiones && planExiste.sesiones.length > 0) {
            ...
            // Cambiar a la pestaña plan si no está activa
            const activeTab = document.querySelector('.tab-button.active')?.textContent.toLowerCase();
            if (activeTab !== 'plan') {
    ...
    // Fallback: restaurar desde sessionStorage
