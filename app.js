// ==================== app.js - VERSIÓN COMPLETA CON PERSISTENCIA DEL PLAN ====================
// Versión: 2.5 - Añadida persistencia del plan activo en localStorage
  ...
  async getFriends(uid) {
    try {
      ...
      for (const fid of friendIds) {
        const friendDoc = await firebaseServices.db.collection('users').doc(fid).get();
        if (friendDoc.exists) {
          friends.push({
            uid: fid,
            ...friendDoc.data()
...
const AppState = {
  ...
  // NUEVAS FUNCIONES PARA PERSISTENCIA DEL PLAN
  guardarPlanActivo(planId) {
    this.planActualId = planId;
    if (this.currentUserId) {
      localStorage.setItem(`ri5_plan_${this.currentUserId}`, planId);
    }
  },
  ...
  limpiarPlanActivo() {
    ...
    if (this.currentUserId) {
      localStorage.removeItem(`ri5_plan_${this.currentUserId}`);
    }
  },
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (uid) {
      ...
      // Restaurar plan activo si existe
      const savedPlanId = localStorage.getItem(`ri5_plan_${uid}`);
      if (savedPlanId) {
        this.planActualId = savedPlanId;
      }
    } else {
  ...
  puedeVerDetalleSesion() {
    ...
    if (!this.isPremium || (this.premiumExpiryDate && new Date() > this.premiumExpiryDate)) {
      if (this.planGeneradoActual && this.planGeneradoActual.fechaInicio) {
        const fechaInicio = new Date(this.planGeneradoActual.fechaInicio);
        const hoy = new Date();
...
const UI = {
  ...
  async switchTab(tab) {
    ...
    for(let b of btns) {
      if(b.textContent.includes(tab === 'entreno' ? 'ENTRENO' : tab === 'plan' ? 'PLAN' : tab === 'historial' ? 'HISTORIAL' : tab === 'soporte' ? 'SOPORTE' : tab === 'perfil' ? 'PERFIL' : tab === 'amigos' ? 'AMIGOS' : 'ADMIN')) { 
    ...
    try {
      ...
      if(tab === 'amigos') {
        if (window.Friends) {
          await Friends.cargarSolicitudes();
          await Friends.cargarListaAmigos();
          await Feed.cargarFeed();
        }
      }
  ...
  async cargarPlanDesdeHistorial(planId) {
    ...
    try {
      ...
      AppState.guardarPlanActivo(planId);
...
window.cambiarAmigosTab = function(tab) {
  document.querySelectorAll('.amigos-tab').forEach(t => t.classList.remove('active'));
  document.querySelectorAll('.amigos-panel').forEach(p => p.classList.remove('active'));
  ...
  document.querySelectorAll('.amigos-tab')[tabMap[tab]]?.classList.add('active');
  document.getElementById(`amigos-${tab}`)?.classList.add('active');
  
  if (tab === 'solicitudes' && window.Friends) Friends.cargarSolicitudes();
  if (tab === 'lista' && window.Friends) Friends.cargarListaAmigos();
  if (tab === 'feed' && window.Feed) Feed.cargarFeed();
};
    ...
    try {
      ...
      if (currentUser && currentUser.uid === savedUid) {
        ...
        if (userDoc.exists) {
          ...
          // Restaurar plan activo si existe
          const savedPlanId = localStorage.getItem(`ri5_plan_${savedUid}`);
          if (savedPlanId) {
            setTimeout(() => {
              UI.cargarPlanDesdeHistorial(savedPlanId);
            }, 500);
          }
