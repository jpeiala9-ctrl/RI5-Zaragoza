...
const AppState = {
  ...
  actualizarInterfazPremium() {
    // 🔥 FIX: antes se deshabilitaba (btn.disabled = true) "+ NUEVO PLAN"
    // para usuarios gratis. Un <button disabled> no dispara 'onclick' al
    // tocarlo, y en móvil el atributo 'title' que explicaba el motivo no
    // se ve NUNCA (no hay hover) -- así que un usuario gratis sin plan
    // veía la pestaña Plan completamente vacía (ni calendario ni
    // formulario) sin ninguna pista de qué pasaba. Ahora el botón se deja
    // siempre pulsable: es toggleCuestionario() (calendar.js) quien, al
    // pulsar, decide si abre el formulario (premium) o directamente el
    // aviso de premium con el contacto de Instagram (gratis) -- el
    // formulario para generar un plan nuevo sigue siendo solo premium.
