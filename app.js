...
// VERSIÓN: 4.50 - Tarjeta de conversación de soporte (admin) rediseñada:
//                 foto+nombre agrandados y centrados como bloque en el
//                 lado izquierdo, preview del último mensaje centrado en
//                 la parte baja de la tarjeta. Fecha/aviso/papelera sin
//                 tocar. Misma estructura de 3 columnas x 3 filas, sin
...
const Admin = {
  ...
  _renderizarListaSoporte(usuarios, container) {
    ...
    for (const user of usuarios) {
      ...
      const avatar = user.photoURL ? `<img src="${Utils.escapeHTML(user.photoURL)}" style="width:46px;height:46px;border-radius:50%;object-fit:cover;">` : `<div style="width:46px;height:46px;border-radius:50%;background:var(--bg-secondary);display:flex;align-items:center;justify-content:center;font-size:20px;">👤</div>`;
      ...
      html += `
        <div class="usuario-soporte-item" data-uid="${user.uid}" style="display:grid;grid-template-columns:46px 1fr auto;grid-template-rows:auto auto auto;column-gap:12px;row-gap:4px;padding:12px;background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:12px;margin-bottom:8px;cursor:pointer;">
          <div style="grid-column:1;grid-row:1 / 3;align-self:center;justify-self:center;">${avatar}</div>
          <span style="grid-column:2;grid-row:1 / 3;font-weight:700;font-size:15px;color:var(--accent-yellow);align-self:center;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${nombre}</span>
          <span style="grid-column:3;grid-row:1;font-size:11px;color:var(--text-secondary);align-self:start;justify-self:end;white-space:nowrap;">${fecha}</span>
          <div style="grid-column:3;grid-row:2;align-self:center;justify-self:end;">${badgeNoLeidos}</div>
          <div style="grid-column:1 / 3;grid-row:3;font-size:13px;color:var(--text-secondary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center;align-self:end;">${textoPreview}</div>
          <button type="button" class="borrar-conversacion-soporte" data-uid="${user.uid}" title="Eliminar conversación" style="grid-column:3;grid-row:3;justify-self:end;align-self:end;width:28px;height:28px;flex-shrink:0;border:none;background:transparent;color:var(--zone-5);padding:0;cursor:pointer;display:flex;align-items:center;justify-content:center;">${iconoPapelera}</button>
        </div>
      `;
    }
