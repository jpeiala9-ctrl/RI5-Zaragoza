// ==================== app.js - VERSIÓN COMPLETA CON PRECARGA TOTAL (PERFIL, AMIGOS, MURO, MENSAJES) ====================
// VERSIÓN: 4.19 - Añadida precarga al login y restauración de sesión
// ====================
...
// ==================== MÓDULO DE AUTENTICACIÓN (Auth) CON PRECARGA ====================
const Auth = {
  intentosLogin: 0,
  ultimoIntento: 0,

  _manejarErrorFirebase(error) { /* igual que original */ return 'Error'; },
  switchAuthTab(tab) { /* igual */ },
  async createUserDocument(userId, email, username, extraData = {}) { /* igual */ },
  showVerificationScreen(email) { /* igual */ },
  async reenviarVerificacion() { /* igual */ },
  async verificarAhora() { /* igual */ },
  async registerUser() { /* igual */ },
  ...
  async _enablePersistence() { try { await firebaseServices.db.enablePersistence({ synchronizeTabs: true }); console.log('✅ Persistencia offline habilitada'); } catch (err) { console.warn('⚠️ Persistencia no disponible:', err); } },

  async eliminarMiCuenta() { /* igual */ },
  async logoutUser() { /* igual */ },
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      document.getElementById("verificationPage").style.display = "none";
      document.getElementById("mainContent").style.display = "flex";
      // ========== PRECARGA TOTAL AL RESTAURAR SESIÓN ==========
      setTimeout(() => {
        if (AppState.currentUserId) {
          if (window.Profile) Profile.cargarPerfil().catch(e => console.warn('Precarga perfil:', e));
          if (window.Friends) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const ageInput = document.getElementById("age"); const timeInput = document.getElementById("time");
  ...
  UI.validarTodo(); UI.initDiasCheckboxes();
  ...
  document.addEventListener('click', function(e) {
    ...
    if (premiumElement) {
      ...
      if (perfilTab && perfilTab.classList.contains('active')) {
        ...
        Utils.confirm('Contactar con Administrador', '¿Quieres contactar con el administrador para obtener Premium?\n\nAceptar te llevará a Instagram (@navegacionpro)').then((resultado) => { if (resultado) window.open('https://www.instagram.com/navegacionpro', '_blank'); }).catch(() => {});
      }
    }
  });
  const savedUid = localStorage.getItem('ri5_current_user');
  if (!savedUid) { document.getElementById("loginPage").style.display = "flex"; document.getElementById("mainContent").style.display = "none"; } else { Utils.showLoading(); }
...
window.toggleTheme = function(btn) { /* igual */ };
window.togglePassword = function(inputId, element) { /* igual */ };
...
window.cerrarModalPremium = function() { Admin.cerrarModalPremium(); };
window.cerrarModalSesion = function() { document.getElementById("detalleSesion")?.classList.remove("visible"); document.getElementById("modalOverlay")?.classList.remove("visible"); if (AppState) AppState.currentSesionDetalle = null; };
window.cambiarAmigosTab = function(tab) { /* igual */ };
...
const ResetPassword = {
  abrirModal() { document.getElementById('resetOverlay').style.display = 'block'; document.getElementById('resetModal').style.display = 'block'; document.getElementById('resetEmail').value = ''; document.getElementById('resetError').classList.remove('visible'); },
  cerrarModal() { document.getElementById('resetOverlay').style.display = 'none'; document.getElementById('resetModal').style.display = 'none'; },
  async enviarEmailRecuperacion() {
    const email = document.getElementById('resetEmail')?.value.trim();
    const errorEl = document.getElementById('resetError');
    if (!email) { errorEl.textContent = 'Introduce tu correo electrónico'; errorEl.classList.add('visible'); return; }
    if (!Utils.isValidEmail(email)) { errorEl.textContent = 'Correo electrónico no válido'; errorEl.classList.add('visible'); return; }
    Utils.showLoading();
    try {
...
if (typeof PlanGenerator !== 'undefined') {
  ...
  window.cambiarTrimestre = async (delta) => { console.warn('PlanGenerator no disponible aún'); Utils.showToast('Cargando planificador...', 'info'); };
}