...
// VERSIÓN: 4.17 - Precarga de datos tras login y restauración (perfil, amigos, explorar, mensajes)
// ====================
...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (uid) {
      ...
      this.precargarDatos(); // <--- NUEVA PRECARGA
      // Restaurar último cálculo guardado de este usuario
      if (window.Training) {
        setTimeout(() => Training.restaurarUltimoCalculo(uid), 150);
      }
    } else {
  ...
  // ---------- NUEVO: PRECARGA DE DATOS EN SEGUNDO PLANO ----------
  precargarDatos() {
    if (!this.currentUserId) return;
    console.log('🔄 Precargando datos en segundo plano...');
    // Precargar perfil (ya tiene caché, pero forzamos la carga para llenar caché)
    if (window.Profile) {
      Profile.cargarPerfil().catch(e => console.warn('Error precargando perfil:', e));
    }
    // Precargar lista de amigos (con caché de 30s)
    if (window.Friends) {
      Friends.cargarListaAmigos().catch(e => console.warn('Error precargando amigos:', e));
      // Precargar explorar usuarios (primer lote)
  ...
  iniciarListeners() {
    ...
    this.unsubscribeConversations = firebaseServices.db
      ...
      .onSnapshot(async () => {
        ...
        // También actualizamos el badge de la pestaña amigos
  ...
  actualizarBadgeSolicitudes() {
    ...
    // También actualizar la subpestaña de solicitudes si está visible
  ...
  actualizarBadgeChat() {
    ...
    // Actualizar estilo de la subpestaña "Mis Amigos"
