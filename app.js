...
// VERSIÓN: 4.36 - Sistema de mensajes de soporte con eliminación sincronizada
// ====================
...
const AppState = {
  ...
  iniciarListeners() {
    ...
    // Listener para conversaciones de amigos
    ...
    // ===== LISTENER PARA MENSAJES DE SOPORTE (TIEMPO REAL) =====
    // Escucha cambios en la subcolección "mensajes" del usuario
    this.unsubscribeMensajesSoporte = firebaseServices.db
      ...
      .onSnapshot(async (snapshot) => {
        ...
        snapshot.forEach(doc => {
          const data = doc.data();
          // Solo contamos como no leído si el usuario es el destinatario y no está leído
        ...
        // Si el usuario está en la pestaña de soporte, recargar mensajes automáticamente
...
const Admin = {
  ...
  // ===== NUEVA FUNCIÓN DE ELIMINAR MENSAJE (SINCRONIZADA) =====
  async eliminarMensajeSoporte(mensajeId) {
    ...
    Utils.showLoading();
    try {
      // 1. Obtener el mensaje para saber los UIDs
      ...
      // 3. Eliminar de la subcolección del remitente (si no es admin)
      if (fromUid && fromUid !== adminUid) {
        // Buscar el mensaje en la subcolección del remitente por timestamp y texto
        ...
        await batch.commit();
        console.log(`🗑️ Mensaje eliminado de la subcolección de ${fromUid}`);
      }

      // 4. Eliminar de la subcolección del destinatario (si no es admin)
      if (toUid && toUid !== adminUid) {
        ...
        snapshot.forEach(doc => batch.delete(doc.ref));
        await batch.commit();
        console.log(`🗑️ Mensaje eliminado de la subcolección de ${toUid}`);
      }
  ...
  // ===== NUEVA VERSIÓN DE CARGAR MENSAJES (MINIMALISTA) =====
...
const UI = {
  ...
  async cargarMensajesRecibidos() {
    ...
    // Los recibidos son aquellos donde el usuario es el destinatario (toUid)
  ...
  async cargarMensajesEnviados() {
    ...
    const mensajes = await Storage.getMensajesSoporteUsuario(AppState.currentUserId);
    // Los enviados son aquellos donde el usuario es el remitente (fromUid)
  ...
  // ===== NUEVA FUNCIÓN PARA ELIMINAR MENSAJES (USUARIO) =====
  async borrarMensajeUsuario(idx, tipo) {
    if (!AppState.currentUserId) return;
    // Obtener la lista de mensajes (según el tipo)
    const mensajes = await Storage.getMensajesSoporteUsuario(AppState.currentUserId);
    // Filtramos por tipo (recibido o enviado) para obtener el mensaje correcto
