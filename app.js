...
const AppState = {
  ...
  actualizarBadgesCuenta() {
    const activo = !!(this.isPremium && this.premiumExpiryDate && new Date() <= this.premiumExpiryDate);
    ['dashboardAccountBadge', 'perfilAccountBadge'].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
      el.textContent = activo ? '⭐ PREMIUM' : 'STANDARD';
      el.classList.toggle('account-badge-premium', activo);
    });
  },
