...
// VERSIÓN: 4.10 - CORREGIDO: eliminado select() que causaba error al cargar usuarios
// ====================
...
this.unsubscribeConversations = firebaseServices.db
  ...
  .onSnapshot(async (snapshot) => {
    const prevUnread = this.mensajesAmigosNoLeidos || 0;
    if (window.Chat) {
      await Chat.updateUnreadBadge();
    }
    const newUnread = this.mensajesAmigosNoLeidos || 0;
    // Notificación toast si llegan nuevos mensajes y el chat no está abierto
    if (newUnread > prevUnread) {
      const chatModal = document.getElementById('chatModal');
      const chatVisible = chatModal && chatModal.style.display === 'flex';
      if (!chatVisible) {
        ...
        try {
          snapshot.docChanges().forEach(change => {
            if (change.type === 'modified' || change.type === 'added') {
              ...
              if (otherId && data.participantsData && data.participantsData[otherId]) {
                senderName = Utils.capitalizeUsername(data.participantsData[otherId].username || 'Un amigo');
              }
            }
          });
        } catch (e) {}
        Utils.showToast('\u{1F4AC} Nuevo mensaje de ' + senderName, 'info', 4000);
        Utils.vibrate(40);
      }
    }
  }, (error) => {
