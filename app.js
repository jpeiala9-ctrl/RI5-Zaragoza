...
// VERSIÓN: 4.19 - Botones "CARGAR MÁS" corregidos (permanecen visibles hasta el final)
// ====================
...
const Admin = {
  ...
  renderUsersList(usersData) {
    ...
    this._mostrarContadorAdminUsuarios();
  },
  ...
  _mostrarContadorAdminUsuarios() {
    ...
    const oldCounter = document.getElementById('contadorAdminUsuarios');
    if (oldCounter) oldCounter.remove();
    
    const counter = document.createElement('div');
    counter.id = 'contadorAdminUsuarios';
    ...
    counter.textContent = `👥 Total: ${totalItems} usuarios${!this.usersPagination.hasMore ? ' (todos cargados)' : ''}`;
    ...
    try {
      ...
      if (loadMoreBtn) {
        if (this.messagesPagination.hasMore) {
          ...
          loadMoreBtn.textContent = '📭 NO HAY MÁS MENSAJES';
          loadMoreBtn.disabled = true;
          loadMoreBtn.style.opacity = '0.5';
          setTimeout(() => {
            if (loadMoreBtn) loadMoreBtn.style.display = 'none';
