// ==================== app.js - VERSIÓN COMPLETA CORREGIDA (CON ADMIN) ====================
...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    this.isAdmin = userData?.isAdmin || false;        // <-- NUEVO
    ...
    if (adminTab) {
      adminTab.style.display = this.isAdmin ? 'block' : 'none';
    }
...
async function cargarUsuarios() {
  if (!AppState.currentUserId || !AppState.isAdmin) return;
  try {
    const snapshot = await firebaseServices.db.collection('users').orderBy('created', 'desc').get();
    AppState.usuarios = snapshot.docs.map(doc => ({ uid: doc.id, ...doc.data() }));
    // Contar mensajes no leídos de cada usuario (los que envió a admin)
    for (let u of AppState.usuarios) {
      const doc = await firebaseServices.db.collection('mensajes').doc('admin_' + u.uid).get();
      const mensajes = doc.exists ? doc.data().mensajes || [] : [];
      u.mensajesNoLeidos = mensajes.filter(m => !m.leido).length;
    }
...
async function enviarMensajeAdmin(uid, texto) {
  if (!AppState.currentUserId || !AppState.isAdmin || !texto.trim()) return;
  try {
    const docRef = firebaseServices.db.collection('mensajes').doc(uid);
    const doc = await docRef.get();
    const mensajes = doc.exists ? doc.data().mensajes || [] : [];
    ...
    // Si estamos en el chat, recargar mensajes
    if (AppState.usuarioSeleccionado && AppState.usuarioSeleccionado.uid === uid) {
      await cargarConversacionAdmin(uid);
    }
    // Actualizar lista de usuarios (para badge)
...
async function cargarConversacionAdmin(uid) {
  if (!AppState.currentUserId || !AppState.isAdmin) return;
  try {
    // Mensajes del admin hacia el usuario
    const docAdmin = await firebaseServices.db.collection('mensajes').doc(uid).get();
    const mensajesAdmin = docAdmin.exists ? docAdmin.data().mensajes || [] : [];
    // Mensajes del usuario hacia el admin
    const docUser = await firebaseServices.db.collection('mensajes').doc('admin_' + uid).get();
    const mensajesUser = docUser.exists ? docUser.data().mensajes || [] : [];
    
    // Combinar y ordenar por timestamp
    const todos = [...mensajesAdmin, ...mensajesUser].sort((a, b) => {
      const ta = a.timestamp?.seconds || 0;
      const tb = b.timestamp?.seconds || 0;
      return ta - tb;
    });
    
    AppState.conversacionActual = todos;
    if (window.UI && UI.renderConversacionAdmin) UI.renderConversacionAdmin();
    
    // Marcar como leídos los mensajes del usuario (los que envió a admin)
    if (mensajesUser.some(m => !m.leido)) {
      const nuevos = mensajesUser.map(m => ({ ...m, leido: true }));
      await firebaseServices.db.collection('mensajes').doc('admin_' + uid).set({ mensajes: nuevos });
    }
  } catch (error) {
    console.error('Error cargando conversación:', error);
    Utils.showToast('Error al cargar conversación', 'error');
  }
}

async function contarMensajesNoLeidosAdmin() {
  if (!AppState.currentUserId || !AppState.isAdmin) return 0;
  try {
    const snapshot = await firebaseServices.db.collectionGroup('mensajes').where('esUsuario', '==', true).where('leido', '==', false).get();
...
// Hacerlas globales para que admin.js pueda usarlas
window.cargarUsuarios = cargarUsuarios;
window.enviarMensajeAdmin = enviarMensajeAdmin;
window.cargarConversacionAdmin = cargarConversacionAdmin;
window.contarMensajesNoLeidosAdmin = contarMensajesNoLeidosAdmin;
...
const UI = {
    ...
    try {
      ...
      if(tab === 'admin') {
        if (AppState.isAdmin) {
          ...
          const adminTabBtn = document.getElementById('adminTabButton');
          if (total > 0) {
            adminTabBtn.classList.add('soporte-unread');
            adminTabBtn.innerText = `ADMIN (${total})`;
          } else {
            adminTabBtn.classList.remove('soporte-unread');
            adminTabBtn.innerText = 'ADMIN';
          }
        }
      }
    } catch (error) {
  ...
  // Métodos para admin UI (serán llamados desde admin.js)
  renderAdminUserList() {
    const container = document.getElementById('adminUserList');
    if (!container) return;
    
    if (!AppState.usuariosFiltrados || AppState.usuariosFiltrados.length === 0) {
      container.innerHTML = '<p style="text-align:center; padding:20px;">No hay usuarios</p>';
      return;
    }
    
    let html = '';
    AppState.usuariosFiltrados.forEach(u => {
      const fecha = u.created ? new Date(u.created).toLocaleDateString() : '?';
      const premium = u.premium && new Date(u.expires) > new Date() ? 'PREMIUM' : 'FREE';
      const premiumClass = premium === 'PREMIUM' ? 'premium' : '';
      
      const noLeidos = u.mensajesNoLeidos || 0;
      
      html += `
        <div class="user-card" onclick="abrirChatUsuario('${u.uid}')">
          <div class="user-info">
            <div class="user-name">${u.username || 'Sin nombre'}</div>
            <div class="user-email">${u.email || ''} · ${fecha}</div>
          </div>
          <div>
            <span class="user-badge ${premiumClass}">${premium}</span>
  ...
  renderConversacionAdmin() {
    const container = document.getElementById('adminChatMessages');
    if (!container || !AppState.conversacionActual) return;
    
    let html = '';
    AppState.conversacionActual.forEach(m => {
      const esAdmin = m.esAdmin === true;
      const clase = esAdmin ? 'admin' : 'user';
      const autor = esAdmin ? 'Tú' : (AppState.usuarioSeleccionado?.username || 'Usuario');
      html += `
        <div class="chat-message ${clase}">
          <div>${m.texto}</div>
          <small>${autor} · ${m.fecha}</small>
        </div>
      `;
    });
    container.innerHTML = html;
...
// Asignar métodos de UI para admin
UI.renderAdminUserList = UI.renderAdminUserList.bind(UI);
UI.renderConversacionAdmin = UI.renderConversacionAdmin.bind(UI);
