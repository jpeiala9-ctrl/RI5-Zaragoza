// ==================== app.js - VERSIÓN CORREGIDA + NOTIFICACIONES EN TIEMPO REAL ====================
// VERSIÓN: 4.6 - Añadido listener para mensajes del admin
// ====================
...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (uid) {
      ...
      // --- NUEVO: Listener en tiempo real para mensajes del admin ---
      if (this.unsubscribeMensajes) this.unsubscribeMensajes();
      this.unsubscribeMensajes = firebaseServices.db.collection('mensajes').doc(uid)
        .onSnapshot((doc) => {
          if (doc.exists) {
  ...
  async incrementarCalculo() {
    ...
    // Usar FieldValue.increment para evitar bypass en cliente
    try {
      ...
      // Si el error es de permisos, puede ser porque se superó el límite en el servidor
  ...
  // Método auxiliar para detener todos los listeners (se llama desde logout)
  detenerTodosLosListeners() {
    if (this.unsubscribeMensajes) {
      this.unsubscribeMensajes();
      this.unsubscribeMensajes = null;
    }
    if (window.Friends && typeof window.Friends.detenerListenerSolicitudes === 'function') {
      window.Friends.detenerListenerSolicitudes();
    }
    if (window.Chat && typeof window.Chat.detenerListenerChat === 'function') {
      window.Chat.detenerListenerChat();
    }
    if (window.Feed && typeof window.Feed.detenerListenerFeed === 'function') {
      window.Feed.detenerListenerFeed();
    }
  }
};
...
const Admin = {
  ...
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      // Para el toggle del detalle (se mantiene onclick en el header porque no lleva datos de usuario dinámicos)
  ...
  async cargarMensajesUsuarios() {
    ...
    try {
      ...
      // Añadir event listeners para los botones de responder y eliminar
      ...
      // Para el toggle del mensaje (usar el header o el contenedor)
...
const UI = {
  ...
  renderMessageList(container, mensajes, tipo, onToggleCallback) {
    ...
    container.querySelectorAll('.mensaje-item').forEach(el => {
      // El toggle se activa al hacer click en el header (o en el item mismo)
      ...
      // Los botones de eliminar y responder se manejan con event listeners
      ...
      if (responderBtn) {
        responderBtn.addEventListener('click', (e) => {
          ...
          if (onToggleCallback) {
            // Llamar al callback pasado (p.ej., responder mensaje)
            eval(onToggleCallback); // Esto es peligroso, mejor pasar una función. En el uso actual no se usa.
          }
        });
      }
