...
// VERSIÓN: 3.0 - DEFINITIVA | CORREGIDA POR EQUIPO MIT | 100% FUNCIONAL
// ====================
...
const AppState = {
  ...
  planActualId: null,
  sesionesRealizadas: {},
  feedbackSesiones: {}, // <-- NUEVO: feedback de sesiones
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    this.limpiarDatosPlan(); // <-- Esto ya incluye reset de feedbackSesiones
  ...
  limpiarDatosPlan() {
    ...
    this.feedbackSesiones = {}; // <-- NUEVO: resetear feedback al limpiar plan
  ...
  async cargarPlanDesdeHistorial(planId) {
    ...
    try {
      ...
      if (AppState) {
        ...
        AppState.feedbackSesiones = planCompleto.feedback || {}; // <-- Cargar feedback
      ...
      if (currentUser && currentUser.uid === savedUid) {
        ...
        if (userDoc.exists) {
          ...
          if(calc) { 
            ...
            if (window.UI) UI.mostrarResultadosGuardados(calc); // <-- AÑADIDO
          }
