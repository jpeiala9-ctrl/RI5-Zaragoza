...
// VERSIÓN: 4.11 - Notificaciones de nuevos mensajes en tiempo real con toast y vibración
// ====================
...
const AppState = {
  ...
  // Variable para controlar si el chat está abierto (para no mostrar notificaciones repetidas)
  isChatOpen: false,
  // Almacenar el último snapshot de conversaciones para detectar nuevos mensajes
  lastConversationsSnapshot: null,
  ...
  iniciarListeners() {
    ...
    this.unsubscribeFriendRequests = firebaseServices.db
      ...
      .onSnapshot((snapshot) => {
        ...
        if (this.solicitudesPendientesCount !== count) {
          ...
          if (count > (this.solicitudesPendientesCount - 1)) {
            Utils.vibrate(100);
            Utils.showToast(`📨 Nueva solicitud de amistad`, 'info', 4000);
          }
        }
      }, (error) => {
    ...
    this.unsubscribeConversations = firebaseServices.db
      ...
      .onSnapshot(async (snapshot) => {
        ...
        // Detectar si hay conversaciones nuevas o mensajes nuevos
        if (this.lastConversationsSnapshot) {
          // Comparar para encontrar nuevas conversaciones o mensajes nuevos
          snapshot.docChanges().forEach(change => {
            if (change.type === 'added' || change.type === 'modified') {
              const conv = change.doc.data();
              if (conv.lastMessage && conv.lastMessage.senderId !== this.currentUserId && !conv.lastMessage.read) {
                unreadCount++;
                if (!newMessageFrom) {
                  newMessageFrom = conv.otherUsername || 'Alguien';
                  newMessageText = conv.lastMessage.text;
        ...
        this.lastConversationsSnapshot = snapshot;
        this.mensajesAmigosNoLeidos = unreadCount;
        this.actualizarBadgeChat();
        
        // Mostrar notificación si hay un nuevo mensaje y el chat no está abierto
        if (newMessageFrom && !this.isChatOpen && newMessageText) {
          Utils.vibrate(200);
          Utils.showToast(`💬 Nuevo mensaje de ${newMessageFrom}`, 'info', 5000);
          // También cambiar el color del tab "AMIGOS" (ya lo hace actualizarBadgeChat)
        }
        
        // Actualizar lista de conversaciones si la pestaña de amigos está activa y el panel de chat está visible
        const activeTab = document.querySelector('.tab-button.active')?.textContent.toLowerCase();
        if (activeTab === 'amigos' && window.Chat && window.Chat.cargarConversaciones) {
          await window.Chat.cargarConversaciones();
        }
      }, (error) => {
  ...
  detenerListeners() {
    ...
    if (this.unsubscribeConversations) {
      ...
      this.lastConversationsSnapshot = null;
    }
  },
  ...
  document.addEventListener('click', function(e) {
    ...
    if (premiumElement) {
      ...
      if (perfilTab && perfilTab.classList.contains('active')) {
        ...
        if (confirm('¿Quieres contactar con el administrador para obtener Premium?\n\nAceptar te llevará a Instagram (@navegacionpro)')) {
