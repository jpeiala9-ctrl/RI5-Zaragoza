// ==================== app.js - VERSIÓN COMPLETA (TIEMPO REAL INMEDIATO) ====================
// VERSIÓN: 4.16 - OPTIMIZADO: Listeners con actualización instantánea sin delay
// ====================
...
const AppState = {
  ...
  iniciarListeners() {
    ...
    // ✅ SOLICITUDES DE AMISTAD - TIEMPO REAL INMEDIATO
    this.unsubscribeFriendRequests = firebaseServices.db
      ...
      .onSnapshot((snapshot) => {
        ...
        // ✅ SI ESTAMOS EN LA PESTAÑA DE SOLICITUDES, RECARGAR LISTA AL INSTANTE
        ...
        if (activeTab === 'amigos') {
          ...
          if (activeAmigosTab === 'solicitudes' && window.Friends) {
            Friends.cargarSolicitudesRecibidas();
          }
        }
      }, (error) => {
    ...
    // ✅ CONVERSACIONES - MENSAJES NO LEÍDOS EN TIEMPO REAL
    this.unsubscribeConversations = firebaseServices.db
      ...
      .onSnapshot(async () => {
        ...
        // ✅ FORZAR ACTUALIZACIÓN DE UI DE AMIGOS
        this.actualizarBadgeChat();
      }, (error) => {
        console.error('Error en listener de conversaciones:', error);
      });

    // ✅ ESTADO DE MENSAJES - DOBLE CHECK EN TIEMPO REAL
      ...
      .onSnapshot((snapshot) => {
        snapshot.docChanges().forEach((change) => {
          if (change.type === 'modified' || change.type === 'added') {
            ...
            // ✅ ACTUALIZAR CHAT ABIERTO AL INSTANTE
            ...
            // ✅ ACTUALIZAR BADGE AL INSTANTE
            this.actualizarBadgeChat();
            
            // ✅ SI ESTAMOS EN LISTA DE AMIGOS, RECARGAR PARA QUITAR VERDE
            const activeAmigosTab = document.querySelector('.amigos-tab.active')?.textContent.toLowerCase();
  ...
  actualizarBadgeSolicitudes() {
    ...
    const solicitudesTab = document.querySelector('.amigos-tab[onclick="cambiarAmigosTab(\'solicitudes\')"]');
    if (solicitudesTab) {
      if (this.solicitudesPendientesCount > 0) {
        solicitudesTab.classList.add('solicitudes-pendientes');
        solicitudesTab.style.color = '#25D366';
      } else {
        solicitudesTab.classList.remove('solicitudes-pendientes');
  ...
  actualizarBadgeChat() {
    // ✅ Pestaña AMIGOS en la barra principal
    const tab = document.querySelector('.tab-button[onclick="switchTab(\'amigos\')"]');
    if (tab) {
    ...
    // ✅ Pestaña MIS AMIGOS dentro de la sección Amigos
    const misAmigosTab = document.querySelector('.amigos-tab[onclick*="lista"]');
    if (misAmigosTab) {
      if (this.mensajesAmigosNoLeidos > 0) {
        misAmigosTab.style.color = '#25D366';
      } else {
        misAmigosTab.style.color = '';
      }
    }
    
    // ✅ FORZAR RECARGA DE LISTA DE AMIGOS SI ESTÁ VISIBLE
    const listaContainer = document.getElementById('listaAmigos');
    if (listaContainer && listaContainer.children.length > 0 && window.Friends) {
      Friends.cargarListaAmigos();
    }
  }
};
    ...
    try {
      ...
      if (currentUser && currentUser.uid === savedUid) {
        ...
        if (userDoc.exists) {
          ...
          // ✅ Verificar expiración de premium (SOLO LECTURA, NO ESCRITURA)
          if (userData.premium && userData.expires) {
            const expiryDate = new Date(userData.expires);
            const ahora = new Date();
            if (expiryDate <= ahora) {
              // Solo actualizar estado local, NO Firestore
