...
window.toggleTheme = function(btn) {
  ...
  // Ola circular: crece desde el propio botón (la bombilla) hasta cubrir
  // toda la pantalla, alcanzando de sobra la esquina opuesta. El tema real
  // se aplica solo cuando la ola ya lo cubre todo, así que no hay ningún
  // parpadeo: se revela ya con el tema nuevo puesto.
  ...
  overlay.style.cssText = `
    ...
    background:${newTheme === 'light' ? '#f5f5f5' : '#0f0f0f'};
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      overlay.style.clipPath = `circle(${radius}px at ${originX}px ${originY}px)`;
    });
  });
  ...
  // Red de seguridad por si transitionend no llega a disparar
