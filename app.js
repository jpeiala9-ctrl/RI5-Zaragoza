// ==================== app.js - VERSIÓN COMPLETA CON SOPORTE MEJORADO ====================
// VERSIÓN: 4.30 - Botones más pequeños, expansión sin indicador, optimización de carga
// ====================
...
const Admin = {
  ...
  async cargarMensajesUsuarios(reset = false) {
    ...
    try {
        ...
        html += `
          <div class="mensaje-soporte-item ${!leido ? 'no-leido' : ''}" data-id="${msg.id}" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:10px; padding:12px; margin-bottom:8px; cursor:pointer; ${!leido ? 'border-left:4px solid var(--notification-color);' : ''}">
            ...
            <div style="display:flex; justify-content:flex-end; gap:8px;">
              <button class="responder-soporte-btn" data-uid="${msg.fromUid}" data-username="${Utils.escapeHTML(nombre)}" style="background:transparent; border:1px solid var(--accent-blue); color:var(--accent-blue); padding:2px 10px; border-radius:20px; cursor:pointer; font-size:11px;">Responder</button>
              <button class="eliminar-soporte-btn" data-id="${msg.id}" style="background:transparent; border:1px solid var(--zone-5); color:var(--zone-5); padding:2px 10px; border-radius:20px; cursor:pointer; font-size:11px;">🗑️ Eliminar</button>
            </div>
          </div>
        `;
      }
      ...
      // Evento: clic en la tarjeta para expandir/colapsar (sin indicador)
