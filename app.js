...
// Versión: 2.5 - Añadidas notificaciones de actividades de amigos
...
const Storage = {
    ...
    try {
      ...
      friends.forEach(friend => {
        ...
        batch.set(feedRef, {
          ...
          leido: entry.leido || false  // Asegurar que incluimos el campo
        });
      });
  ...
  // NUEVA: Marcar entradas del feed como leídas
  ...
  // MODIFICADA: devuelve items y recuento de no leídos
...
const AppState = {
  ...
  // NUEVAS: para notificaciones de amigos
  ...
  // NUEVO: actualizar contador de novedades
  ...
  // NUEVO: actualizar badge en la pestaña AMIGOS
    ...
    try {
      ...
      if(tab === 'amigos') {
        if (window.Friends) {
          ...
          await Feed.cargarFeed(true); // Al entrar, marcar como leído
        }
      }
...
window.cambiarAmigosTab = function(tab) {
  ...
  if (tab === 'feed' && window.Feed) Feed.cargarFeed(false); // No marcar como leído al cambiar de pestaña, solo al entrar
};
