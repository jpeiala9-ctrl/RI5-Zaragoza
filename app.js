...
window.toggleTheme = function(btn) {
  ...
  // Ola: crece desde el propio botón (la bombilla) hasta cubrir toda la
  // pantalla, alcanzando de sobra la esquina opuesta. En vez de un simple
  // círculo con borde recto, se anima a mano con un gradiente radial que
  // lleva una franja más clara justo en el borde que avanza -- como la
  // cresta de una ola -- y el color del tema nuevo va quedando detrás.
  ...
  const DURATION = 1000;
  const CREST_WIDTH = 90; // ancho de la franja más clara que avanza
  const startTime = performance.now();
  const easeOutCubic = t => 1 - Math.pow(1 - t, 3);

  const paint = (r) => {
    const crestStart = Math.max(0, r - CREST_WIDTH);
    const pct = (v) => `${Math.min(100, (v / radius) * 100)}%`;
    overlay.style.background =
      `radial-gradient(circle at ${originX}px ${originY}px,
        rgba(${baseColor},1) 0%,
        rgba(${baseColor},1) ${pct(crestStart)},
        rgba(${crestColor},0.55) ${pct(r - CREST_WIDTH / 2)},
        rgba(${baseColor},1) ${pct(r)},
        transparent ${pct(r)})`;
  };
  paint(0);
