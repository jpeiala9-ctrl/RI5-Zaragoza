...
const Admin = {
  ...
  async cargarEstadisticas() {
    try {
      ...
      document.getElementById('statPremiumUsers').textContent = premiumUsers;
  ...
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      if (snapshot.empty) {
        if (reset) container.innerHTML = '<p style="text-align:center; padding:40px; color: var(--text-secondary);">No hay usuarios con estos filtros</p>';
