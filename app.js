...
// VERSIÓN: 4.24 - Sesiones Compartidas integradas (sin retos)
// ====================
...
const AppState = {
  ...
  // ===== NUEVAS VARIABLES PARA SESIONES COMPARTIDAS =====
  sesionesCompartidasPendientes: 0,
  sessionToShare: null,

  unsubscribeFriendRequests: null,
  unsubscribeConversations: null,
  unsubscribeMensajesSoporte: null,
  unsubscribeSharedSessions: null,
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (uid) {
      ...
      // Inicializar badge de sesiones compartidas
      const count = await SessionSharing.countPendingShares(uid);
      this.sesionesCompartidasPendientes = count;
      this.actualizarBadgeSesionesCompartidas();
    } else {
  ...
  iniciarListeners() {
    ...
    // ===== NUEVO LISTENER PARA SESIONES COMPARTIDAS =====
    this.unsubscribeSharedSessions = firebaseServices.db
      .collection('sharedSessions')
      .where('receiverIds', 'array-contains', this.currentUserId)
      .where(`receiverStatus.${this.currentUserId}`, '==', 'pending')
      .onSnapshot((snapshot) => {
        const count = snapshot.size;
        if (this.sesionesCompartidasPendientes !== count) {
          this.sesionesCompartidasPendientes = count;
          this.actualizarBadgeSesionesCompartidas();
          const activeTab = document.querySelector('.tab-button.active')?.textContent.toLowerCase();
          if (activeTab === 'sesiones compartidas' && window.cargarSesionesCompartidas) {
            cargarSesionesCompartidas();
          }
        }
      }, (error) => {
        console.error('Error en listener de sesiones compartidas:', error);
      });
  },
  ...
  detenerListeners() {
    ...
    if (this.unsubscribeSharedSessions) {
      this.unsubscribeSharedSessions();
      this.unsubscribeSharedSessions = null;
    }
  },
  ...
  // ===== NUEVA FUNCIÓN PARA ACTUALIZAR BADGE DE SESIONES COMPARTIDAS =====
  actualizarBadgeSesionesCompartidas() {
    const tab = document.querySelector('.tab-button[onclick="switchTab(\'sesionesCompartidas\')"]');
    if (tab) {
      if (this.sesionesCompartidasPendientes > 0) {
        tab.classList.add('sesion-shared-unread');
        tab.setAttribute('data-count', this.sesionesCompartidasPendientes);
      } else {
        tab.classList.remove('sesion-shared-unread');
        tab.removeAttribute('data-count');
      }
    }
  }
...
const Admin = {
  ...
  // --- Modal de detalle de usuario (optimizado) ---
  async verPerfil(uid) {
...
const UI = {
  ...
  async switchTab(tab) {
    ...
    for(let b of tabs) {
      if(b.textContent.includes(tab === 'entreno' ? 'ENTRENO' : 
                               ...
                               tab === 'amigos' ? 'AMIGOS' : 
                               tab === 'muro' ? 'MURO' : 
                               tab === 'sesionesCompartidas' ? 'SESIONES COMPARTIDAS' : 'ADMIN')) { 
    ...
    try {
      ...
      // ===== NUEVA PESTAÑA SESIONES COMPARTIDAS =====
      if (tab === 'sesionesCompartidas') {
        await cargarSesionesCompartidas();
      }
    } catch (error) {
  ...
  async restaurarEstado() {
    ...
    try {
      ...
      if (estado.activeTab) {
        const tabMap = { 'entreno': 'entreno', 'plan': 'plan', 'historial': 'historial', 'soporte': 'soporte', 'perfil': 'perfil', 'amigos': 'amigos', 'muro': 'muro', 'admin': 'admin', 'sesionescompartidas': 'sesionesCompartidas' };
        const tab = tabMap[estado.activeTab.toLowerCase()] || 'entreno';
  ...
  // ===== NUEVAS FUNCIONES PARA SESIONES COMPARTIDAS =====
  abrirSelectorAmigosParaCompartir() {
    const overlay = document.createElement('div');
    overlay.id = 'shareFriendOverlay';
    ...
    modal.style.cssText = 'background:var(--bg-card); border:1px solid var(--border-color); border-radius:16px; padding:24px; max-width:450px; width:90%; max-height:80vh; overflow-y:auto; box-shadow:var(--shadow-lg);';
    modal.innerHTML = `
      <h3 style="margin:0 0 16px 0; color:var(--accent-yellow);">📤 Compartir sesión</h3>
      <p style="font-size:13px; color:var(--text-secondary); margin-bottom:12px;">Selecciona a quién quieres enviar esta sesión:</p>
      <div id="shareFriendList" style="margin-bottom:12px;"></div>
      <textarea id="shareMessageInput" placeholder="Mensaje (opcional)" style="margin-bottom:12px; min-height:60px;"></textarea>
      <div style="display:flex; gap:8px; justify-content:flex-end;">
        <button class="action-button" onclick="UI.enviarSesionCompartida()" style="width:auto; margin:0;">ENVIAR</button>
        <button class="action-button" onclick="UI.cerrarSelectorAmigosCompartir()" style="width:auto; margin:0;">CANCELAR</button>
      </div>
    `;
    ...
    if (listContainer) {
      Storage.getFriends(AppState.currentUserId).then(friendsData => {
        if (friendsData.length === 0) {
          listContainer.innerHTML = '<p style="color:var(--text-secondary);">No tienes amigos para compartir.</p>';
        } else {
          let html = '';
          friendsData.forEach(f => {
            html += `
              <div style="display:flex; align-items:center; gap:10px; margin-bottom:6px; padding:6px; background:var(--bg-primary); border-radius:8px;">
                <input type="checkbox" class="share-friend-checkbox" value="${f.uid}" id="friend_${f.uid}" style="width:18px; height:18px; margin:0;">
                <label for="friend_${f.uid}" style="font-size:14px; cursor:pointer;">${Utils.capitalizeUsername(f.username)}</label>
              </div>
            `;
          });
  ...
  cerrarSelectorAmigosCompartir() {
    document.getElementById('shareFriendOverlay')?.remove();
  },

  async enviarSesionCompartida() {
    const checkboxes = document.querySelectorAll('.share-friend-checkbox:checked');
    const receiverIds = Array.from(checkboxes).map(cb => cb.value);
    ...
    const sessionInfo = AppState.sessionToShare;
    if (!sessionInfo) {
      Utils.showToast('No hay sesión para compartir', 'error');
      return;
    }
    const ok = await SessionSharing.shareSession(
      AppState.currentUserId,
      receiverIds,
      sessionInfo.sessionData,
      sessionInfo.targetDay,
      message
    );
    if (ok) {
      UI.cerrarSelectorAmigosCompartir();
      AppState.sessionToShare = null;
    }
  }
};

// ============================================================
//  FUNCIONES GLOBALES PARA SESIONES COMPARTIDAS
// ============================================================

async function cargarSesionesCompartidas() {
  const container = document.getElementById('sesionesCompartidasContainer');
  if (!container || !AppState.currentUserId) {
    if (container) container.innerHTML = '<p>Inicia sesión para ver tus sesiones compartidas</p>';
    return;
  }

  const shares = await SessionSharing.getPendingShares(AppState.currentUserId);
  if (shares.length === 0) {
    container.innerHTML = '<p style="text-align:center; padding:40px;">No tienes sesiones compartidas pendientes.</p>';
    return;
  }
  ...
  for (const share of shares) {
    const senderData = await Storage.getUser(share.senderId);
    const senderName = Utils.capitalizeUsername(senderData?.username || 'Usuario');
    const session = share.sessionData;
    const diaOriginal = share.targetDay;
    const diaOriginalNombre = diasSemana[diaOriginal];
    const tipoEmoji = { rodaje: '🏃', tempo: '⚡', series: '🔁', largo: '📏', strength: '💪' }[session.tipo] || '🏃';

    const optionsHtml = diasSemana.map((nombre, idx) =>
      idx === 0 ? '' : `<option value="${idx}" ${idx === diaOriginal ? 'selected' : ''}>${nombre}</option>`
    ).join('');

    html += `
      <div class="share-item" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:12px; padding:16px; margin-bottom:12px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
          <div>
            <strong style="color:var(--accent-yellow);">${senderName}</strong>
            <span style="font-size:12px; color:var(--text-secondary); margin-left:8px;">compartió una sesión</span>
          </div>
          <span style="font-size:12px; background:var(--zone-1); padding:2px 10px; border-radius:12px; color:var(--bg-primary);">
            📅 Día original: ${diaOriginalNombre}
          </span>
        </div>
        <div style="margin:8px 0;">
          <span>${tipoEmoji} <strong>${session.tipo.toUpperCase()}</strong> · ${session.duracion}'</span>
          <div style="font-size:12px; color:var(--text-secondary);">${session.detalle?.nombre || ''}</div>
          ${share.message ? `<div style="font-size:12px; color:var(--accent-blue); margin-top:4px;">💬 "${Utils.escapeHTML(share.message)}"</div>` : ''}
        </div>

        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center; margin-top:8px;">
          <button class="btn-amigo" style="background:var(--zone-2); border-color:var(--zone-2); color:var(--bg-primary); font-weight:bold;"
                  onclick="aceptarSesionDiaOriginal('${share.id}')">
            ✅ Aceptar (${diaOriginalNombre})
          </button>

          <div style="display:flex; align-items:center; gap:4px; background:var(--bg-primary); border-radius:8px; padding:4px; border:1px solid var(--border-color);">
            <select class="share-day-select" data-share-id="${share.id}" style="width:auto; margin:0; height:34px; padding:0 8px; font-size:13px; background:transparent; border:none; color:var(--text-primary);">
              ${optionsHtml}
            </select>
            <button class="btn-amigo" style="background:var(--accent-blue); border-color:var(--accent-blue); color:var(--bg-primary);"
                    onclick="aceptarSesionConDiaElegido('${share.id}')">
              📌 Colocar aquí
            </button>
          </div>

          <button class="btn-amigo eliminar" onclick="rechazarSesionCompartida('${share.id}')">✗ Rechazar</button>
        </div>

        <div style="font-size:11px; color:var(--text-secondary); margin-top:6px;">
          ⚠️ Al aceptar, reemplazarás tu sesión del día seleccionado por esta.
        </div>
      </div>
    `;
  }
...
window.aceptarSesionDiaOriginal = async function(shareId) {
  const shareRef = firebaseServices.db.collection('sharedSessions').doc(shareId);
  const shareDoc = await shareRef.get();
  if (!shareDoc.exists) {
    Utils.showToast('La sesión compartida ya no existe', 'error');
    return;
  }
  const shareData = shareDoc.data();
  const diaOriginal = shareData.targetDay;
  const diasSemana = ['', 'LUNES', 'MARTES', 'MIÉRCOLES', 'JUEVES', 'VIERNES', 'SÁBADO', 'DOMINGO'];
  const confirmed = await Utils.confirm(
    'Aceptar sesión',
    `¿Quieres colocar esta sesión en el día original (${diasSemana[diaOriginal]})?`
  );
  if (!confirmed) return;
  Utils.showLoading();
  try {
    await SessionSharing.acceptShare(shareId, AppState.currentUserId);
    await cargarSesionesCompartidas();
    Utils.showToast('✅ Sesión añadida en el día original', 'success');
  } catch (e) {
...
window.aceptarSesionConDiaElegido = async function(shareId) {
  const select = document.querySelector(`.share-day-select[data-share-id="${shareId}"]`);
  if (!select) {
    Utils.showToast('No se encontró el selector de día', 'error');
    return;
  }
  const targetDay = parseInt(select.value);
  const diasSemana = ['', 'LUNES', 'MARTES', 'MIÉRCOLES', 'JUEVES', 'VIERNES', 'SÁBADO', 'DOMINGO'];
  const confirmed = await Utils.confirm(
    'Colocar sesión',
    `¿Quieres colocar esta sesión en ${diasSemana[targetDay]}?`
  );
  if (!confirmed) return;
  Utils.showLoading();
  try {
    await SessionSharing.acceptShareWithDay(shareId, AppState.currentUserId, targetDay);
    await cargarSesionesCompartidas();
    Utils.showToast(`✅ Sesión añadida en ${diasSemana[targetDay]}`, 'success');
  } catch (e) {
    Utils.showToast('Error al aceptar', 'error');
  } finally {
    Utils.hideLoading();
...
window.rechazarSesionCompartida = async function(shareId) {
  const confirmed = await Utils.confirm('Rechazar sesión', '¿Quieres rechazar esta sesión compartida?');
  if (!confirmed) return;
  await SessionSharing.rejectShare(shareId, AppState.currentUserId);
  await cargarSesionesCompartidas();
};
...
console.log('✅ app.js v4.24 - Sesiones Compartidas integradas');