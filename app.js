  ...
  iniciarListeners() {
    ...
    this.unsubscribeConversations = firebaseServices.db
      ...
      .onSnapshot(async (snapshot) => {
        let unreadCount = 0;
        let lastUnreadSender = null;
        let lastUnreadText = null;
        ...
        snapshot.forEach(doc => {
          ...
          if (conv.lastMessage && conv.lastMessage.senderId !== this.currentUserId && !conv.lastMessage.read) {
            unreadCount++;
            if (!lastUnreadSender) {
              lastUnreadSender = conv.otherUsername || 'Alguien';
              lastUnreadText = conv.lastMessage.text;
            }
          }
        });
        
        const previousUnread = this.mensajesAmigosNoLeidos;
        this.mensajesAmigosNoLeidos = unreadCount;
        this.actualizarBadgeChat();
        
        if (unreadCount > previousUnread && !this.isChatOpen && lastUnreadSender) {
          Utils.vibrate(200);
          Utils.showToast(`💬 Nuevo mensaje de ${lastUnreadSender}`, 'info', 5000);
        }
        
        const activeTab = document.querySelector('.tab-button.active')?.textContent.toLowerCase();
        if (activeTab === 'amigos' && window.Chat && typeof window.Chat.cargarConversaciones === 'function') {
