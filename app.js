...
// VERSIÓN: 4.19 - Persistencia del plan en localStorage por usuario
// ====================
  ...
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      if (this.filtrosActuales.premium !== 'all') {
        query = query.where('premium', '==', this.filtrosActuales.premium === 'true');
      }

      // Aplicar filtros de fecha (rango)
      if (this.filtrosActuales.from) {
        const fromDate = new Date(this.filtrosActuales.from);
        fromDate.setHours(0,0,0,0);
        query = query.where('created', '>=', fromDate.toISOString());
      }
      if (this.filtrosActuales.to) {
        const toDate = new Date(this.filtrosActuales.to);
        toDate.setHours(23,59,59,999);
        query = query.where('created', '<=', toDate.toISOString());
      }

      // Ordenación (si no es por username, necesitamos otro índice)
      if (this.filtrosActuales.sort === 'created') {
        query = query.orderBy('created', 'desc');
      } else if (this.filtrosActuales.sort === 'premium') {
        query = query.orderBy('premium', 'desc');
      }
      ...
      if (snapshot.empty) {
        if (reset) {
          container.innerHTML = '<p style="text-align:center; padding:40px; color: var(--text-secondary);">No hay usuarios con estos filtros</p>';
        } else {
          this.usersPagination.hasMore = false;
          // Añadir mensaje de fin de lista si hay contenido previo
      ...
      if (reset) {
        container.innerHTML = '<p style="text-align:center; padding:40px; color: var(--zone-5);">Error al cargar usuarios. Revisa la consola (F12) para más detalles.</p>';
      } else {
        if (container.innerHTML && !container.innerHTML.includes('Error al cargar más')) {
          container.innerHTML += '<p style="text-align:center; padding:20px; color: var(--zone-5);">Error al cargar más usuarios. Revisa la consola.</p>';
        }
      }
      // Si el error es por índice, mostrar ayuda
      if (error.message && error.message.includes('index')) {
        Utils.showToast('⚠️ Se requiere un índice compuesto en Firestore. Revisa la consola para el enlace.', 'warning', 8000);
      }
    } finally {
      this.usersPagination.loading = false;
      this._actualizarBotonCargarMas();
