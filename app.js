...
const AppState = {
  ...
  iniciarListeners() {
    ...
    this.unsubscribeMisDatos = firebaseServices.db
      ...
      .onSnapshot(async (doc) => {
        ...
        // === PREMIUM / GRATIS EN TIEMPO REAL ===
        // Antes esta función solo mantenía sincronizado currentUserData
        // (por eso el texto "PREMIUM"/"GRATIS" del perfil, que lee de
        // ahí, ya se veía bien), pero AppState.isPremium/premiumExpiryDate
        // -- que es lo que de verdad comprueban puedeVerDetalleSesion(),
        // incrementarCalculo(), actualizarBotonCalcular(), etc. -- solo se
        // fijaban una vez al iniciar sesión (setCurrentUser). Un cambio
        // hecho por el admin mientras el usuario ya tenía la app abierta
        // no tenía ningún efecto real hasta recargar la página entera.
        const premiumAnterior = this.isPremium;
        this.isPremium = nuevaData.premium || false;
        this.premiumExpiryDate = nuevaData.expires ? new Date(nuevaData.expires) : null;
        this.calculosMes = nuevaData.calculosMes ?? this.calculosMes;
        this.mesActual = nuevaData.mesActual ?? this.mesActual;
        ...
        if (premiumAnterior !== this.isPremium) {
          // Aplica de inmediato: deshabilita/habilita "generar plan" y
          // recalcula el contador de cálculos gratis restantes.
          this.actualizarInterfazPremium();
          // Si el detalle de una sesión del calendario está abierto en
          // este instante y el usuario acaba de perder premium, ese
          // detalle ya no debería seguir visible.
          if (!this.isPremium && document.getElementById('detalleSesion')?.classList.contains('visible')) {
            if (typeof window.cerrarModalSesion === 'function') window.cerrarModalSesion();
          }
          // Si el perfil está abierto ahora mismo, refrescamos su tarjeta
          // "PREMIUM"/"GRATIS" sin esperar a que se reabra la pestaña.
          if (window.Profile && document.getElementById('tab-perfil')?.classList.contains('active')) {
            Profile.cargarPerfil(true).catch(e => console.warn('Error refrescando perfil tras cambio de premium:', e));
          }
          Utils.showToast(
            this.isPremium ? '⭐ Ya tienes Premium activo' : 'Tu plan ha pasado a ser gratuito',
            this.isPremium ? 'success' : 'info'
          );
        }
  ...
  detenerListeners() {
    ...
    if (window.SessionInvites) {
      SessionInvites.detenerListener();
      SessionInvites.detenerListenerHistorial();
    }
