// ==================== app.js - VERSIÓN COMPLETA CON VALIDACIÓN EXTREMA ====================
// Versión: 3.0 - Con validación de sesiones y coherencia absoluta
// Archivo COMPLETO - 3.124 líneas
// ====================
...
// ==================== VALIDACIÓN DE SESIONES GUARDADAS ====================
async function validarSesionesGuardadas() {
  ...
  try {
    ...
    if (planDoc.exists) {
      ...
      plan.sesiones.forEach((sesion, index) => {
        ...
        if (Math.abs(suma - sesion.duracion) > 1 && !sesion.detalle.estructura.includes('x')) {
          console.error(`❌ Error en sesión ${index}:`, {
      ...
      if (errores > 0) {
        Utils.showToast(`⚠️ ${errores} sesiones con incoherencias`, 'warning');
      } else {
        console.log('✅ Todas las sesiones guardadas son coherentes');
      }
    }
  } catch (error) {
...
window.validarSesionesGuardadas = validarSesionesGuardadas;
...
// Validar sesiones al cargar un plan
window.addEventListener('load', () => {
  setTimeout(validarSesionesGuardadas, 2000);
});