...
const Utils = {
  ...
  // Insignia de administrador: un icono pequeño para pegar justo después
  // del nombre de usuario, en cualquier sitio de la app donde aparezca
  // (muro, amigos, chat...). Independiente del color/insignia de NIVEL
  // (que sigue mostrando el número normal) -- esto es solo para
  // distinguir de un vistazo quién es el admin. Se pasa el propio dato ya
  // leído (isAdmin) en vez de volver a consultar Firestore aquí: en casi
  // todos los sitios donde se pinta un nombre ya se leyó el documento
  // completo del usuario (Storage.getUser / doc.data()), así que el
  // campo 'isAdmin' ya está disponible sin gasto extra.
  adminBadge(isAdmin) {
    if (!isAdmin) return '';
    return '<span title="Administrador" style="display:inline-flex; align-items:center; justify-content:center; margin-left:4px; font-size:0.85em; vertical-align:middle;">🛡️</span>';
  }
};
...
window.toggleTheme = function(btn) {
  ...
  const applyTheme = () => {
    ...
    // Gamification.getColorByLevel() devuelve un hex distinto según el
    // tema (la paleta clara existe para mantener el contraste sobre
    // fondos casi blancos -- ver gamification.js). Sin este re-pintado,
    // --notification-color/--level-color se quedaban con el color del
    // tema que había AL INICIAR SESIÓN hasta la próxima recarga, aunque
    // el usuario cambiara de tema a mano. Se usa el nivel cacheado en
    // localStorage (el mismo que ya se guarda para pintar de golpe en el
    // login) para no depender de una lectura a Firestore aquí.
    if (window.Gamification && typeof Gamification.applyNotificationColor === 'function') {
      const nivelCacheado = parseInt(localStorage.getItem('ri5_lastLevel'), 10) || 1;
      Gamification.applyNotificationColor(nivelCacheado);
    }
  };
...
window.addEventListener('storage', (e) => {
  ...
  // Mismo repintado que en toggleTheme(): el color de nivel depende del
  // tema, así que un cambio de tema hecho desde la guía también tiene que
  // refrescarlo aquí.
  if (window.Gamification && typeof Gamification.applyNotificationColor === 'function') {
    const nivelCacheado = parseInt(localStorage.getItem('ri5_lastLevel'), 10) || 1;
    Gamification.applyNotificationColor(nivelCacheado);
