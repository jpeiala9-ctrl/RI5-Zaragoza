...
const AppState = {
  ...
  precargarDatos() {
    ...
    // 🔥 v4.62: precarga del muro en segundo plano -- mismo patrón que la
    // precarga del historial de sesiones creadas (arriba). Wall.precargarMuro()
    // abre el listener onSnapshot una sola vez por sesión y pinta el HTML
    // en #wallContainer (que ya existe en el DOM, solo está oculto por CSS
    // mientras la subpestaña Muro no está activa). Cuando el usuario abra
    // de verdad la pestaña Muro, la lista ya está puesta desde antes, sin
    // parpadeo ni espera.
    if (window.Wall && Wall.precargarMuro) {
      Wall.precargarMuro().catch(e => console.warn('Error precargando el muro:', e));
    }
  },
...
const UI = {
  ...
  async switchTab(tab) {
    ...
    try {
      ...
      if(tab === 'muro') {
        if (window.Wall) {
          // 🔥 v4.62: ya NO se destruye el listener antes de llamar a
          // init() -- eso es justo lo que forzaba una recarga completa (y
          // el parpadeo) en cada visita a la pestaña. init() es ahora
          // idempotente: si el muro ya está pintado (precarga al iniciar
          // sesión o visita anterior), no toca el DOM ni pide nada a
          // Firestore; si aún no lo estaba, deja que el listener en vivo
          // (que ya estaba abierto de la precarga) pinte la lista.
          Wall.init();
        }
      }
