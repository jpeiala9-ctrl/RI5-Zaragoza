...
const Utils = {
  ...
  // Anima un texto corto letra a letra (entrando de izquierda a derecha,
  // cada letra con un color de la escala de niveles) y, cuando ya están
  // todas dentro, las vira todas a dorado -- mismo efecto que el splash
  // de entrada ("RI5 | Running LAB", ver el <script> junto a #ri5Splash
  // en index.html), pero como función genérica reutilizable para
  // cualquier palabra corta, sin la lógica de arranque de la app que es
  // exclusiva de ese splash. La usa showLoading() para el texto del
  // loadingOverlay en vez del círculo girando de toda la vida.
  // Timings más rápidos que los del splash (que solo se ve una vez, al
  // entrar): este texto puede verse muchas veces por sesión (cada guardado,
  // cada acción), así que conviene que se sienta ágil.
  // Devuelve la duración total (ms) que tarda la animación completa --
  // showLoading() la usa para saber cuánto tiene que esperar como mínimo
  // antes de poder ocultar el overlay (ver hideLoading), así ambas
  // funciones comparten la MISMA fórmula en vez de tener que mantenerla
  // sincronizada a mano en dos sitios.
  // colorPalette (opcional): para textos que se pintan encima de un fondo
  // ya dorado (los botones "Acceder"/"Registrarse", que usan var(--gold)
  // como fondo) -- ahí tanto los colores de nivel de paso como el dorado
  // final del efecto original se diluían contra ese mismo fondo, en los
  // dos temas. Cuando se pasa colorPalette se usa ESE color (en vez de la
  // paleta de niveles) para las letras y también como color final, para
  // garantizar contraste real en vez del efecto arcoíris->dorado.
  _animarTextoDorado(elemento, texto, colorPalette) {
    ...
    const coloresEntrada = colorPalette || LEVEL_COLORS;
    const colorFinal = colorPalette ? colorPalette[0] : GOLD;
    ...
    const spans = letras.map((ch, i) => {
      ...
      span.style.color = (ch === ' ') ? colorFinal : coloresEntrada[i % coloresEntrada.length];
      span.style.transition = `opacity ${ENTRADA_MS}ms ease, transform ${ENTRADA_MS}ms ease, color ${COLOR_MS}ms ease`;
      elemento.appendChild(span);
      return span;
    });

    // Doble requestAnimationFrame por el mismo motivo que en el splash:
    // asegurar que el navegador ha pintado ya el estado inicial (opacity:0)
    // antes de programar la entrada, o la primera letra puede aparecer de
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        setTimeout(() => {
          spans.forEach(span => { span.style.color = colorFinal; });
        }, finEntrada + PAUSA_MS);
      });
    });
  ...
  // 'texto' es opcional: por defecto "EN PROCESO" (genérico, vale para
  // cualquier acción). Sitios concretos pueden pasar algo más específico,
  // p.ej. Utils.showLoading('GENERANDO') al generar un plan.
  showLoading(texto) {
    ...
    // Token + hora de inicio: hideLoading() los usa para esperar, como
    // mínimo, a que la animación de letras termine de verse entera (con
    // el mismo margen de 150ms para que el "todo dorado" final se
    // aprecie) antes de quitar el overlay -- en vez de tocar uno a uno
    // los ~100 sitios de la app que llaman a showLoading/hideLoading para
    // decidir cuál necesita esta animación y cuál un simple fundido, con
    // esto todos quedan cubiertos automáticamente sin arriesgarse a
    // dejarse alguno o a que queden inconsistentes entre sí. Antes,
    // hideLoading() quitaba el overlay al instante en cuanto la acción
    // real terminaba -- si tardaba menos de ~1.5-2s (cambiar una foto,
    // p.ej.), la animación se veía cortada a media letra, muy feo.
  ...
  hideLoading() {
    ...
    // Devuelve una Promise que se resuelve cuando el overlay YA se ha
    // ocultado de verdad (tras esperar, si hacía falta, el resto del
    // mínimo). Ningún llamante existente usaba el valor de retorno (todos
    // llaman a Utils.hideLoading() sin await), así que añadir esto no
    // cambia el comportamiento en ningún otro sitio -- pero permite que un
    // llamante que sí lo necesite (p.ej. un modal con z-index por encima
    // del overlay) espere a que la animación termine ANTES de revelar su
    // contenido, en vez de mostrarlo por debajo/encima del overlay a medio
    // animar y "cortarla" visualmente.
    return new Promise(resolve => {
      const ocultarYa = () => {
        // Si mientras esperábamos se llamó a showLoading() de nuevo (otro
        // token), este hideLoading() ya no es el que manda -- no toca el
        // overlay que pertenece a la acción nueva.
...
const Admin = {
  ...
  async mostrarUsuariosPorFiltro(tipo) {
    ...
    let usuarios = [];
    let titulo = '';
    // El contenido se construye en esta variable y NO se vuelca al modal
    // hasta después de esperar a Utils.hideLoading() (ver más abajo) --
    // así se evita el fallo por el que el modal (z-index 100001, por
    // encima del loadingOverlay a propósito) se mostraba en cuanto
    // llegaba la respuesta de Firestore, tapando/cortando la animación
    // "EN PROCESO" si esta aún no había llegado a su duración mínima.
    let htmlContent = '';
    ...
    // Ahora sí: se espera a que el overlay de carga haya terminado del
    // todo (incluida su animación mínima) antes de volcar el contenido y
    // mostrar el modal -- así el modal siempre aparece DESPUÉS de que la
    // animación se vea completa, nunca tapándola a medias.
    await Utils.hideLoading();
...
const PWA = {
  ...
  registerServiceWorker() {
    ...
    // updateViaCache:'none' -- sin esto, el navegador puede fiarse de la
    // caché HTTP normal (la del propio navegador, o la de un CDN/hosting
    // por delante) al comprobar si sw.js ha cambiado, y seguir viendo
    // como "igual" un archivo que en el servidor ya es distinto. Con
    // 'none' cada comprobación (reg.update(), ver más abajo) fuerza una
    // petición de red real para sw.js, sin atajos de caché.
