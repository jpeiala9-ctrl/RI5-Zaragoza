// ==================== app.js - VERSIÓN COMPLETA (CON MURO, SIN FEED, CON LISTENERS EN TIEMPO REAL) ====================
// VERSIÓN: 4.8 - Fix escapeHTML acepta números; render muro blindado; funciones completas
// ====================
...
// ==================== UTILS ====================
const Utils = {
// FIX PRINCIPAL: convierte cualquier tipo a String antes de escapar.
// Evita TypeError: str.replace is not a function cuando llega un número (ej: entry.duration, entry.tss)
escapeHTML(str) {
...
if (!overlay || !modal || !msgEl || !yesBtn || !noBtn) { resolve(false); return; }
...
try { window.audioContext = new (window.AudioContext || window.webkitAudioContext)(); }
catch (e) { return; }
}
...
setLastCalc(d) {
if (!d) return;
this.lastName = d.name || “”; this.lastAge = d.age || 0;
this.lastFC = d.fcMax || 0; this.lastUL = d.ul || 0;
this.lastZones = d.zones || []; this.lastPred = d.pred || [];
this.lastRitmoBase = d.ritmoBase || 0; this.zonasCalculadas = true;
},

clearLastCalc() {
this.zonasCalculadas = false; this.lastName = “”; this.lastAge = 0;
this.lastFC = 0; this.lastUL = 0; this.lastZones = [];
this.lastPred = []; this.lastRitmoBase = 0;
},
...
if (uid) this.iniciarListeners();
else this.detenerListeners();
```

},

limpiarDatosPlan() {
this.planGeneradoActual = null; this.planActualId = null;
this.sesionesRealizadas = {}; this.feedbackSesiones = {};
...
planBtns.forEach(id => { const btn = document.getElementById(id); if(btn) btn.disabled = !isPremiumActive; });
...
this.isPremium = false; this.actualizarInterfazPremium();
} else {
...
return (this.isPremium && this.premiumExpiryDate && new Date() <= this.premiumExpiryDate);
},
...
if(this.mesActual !== mesActualKey) { this.calculosMes = 0; this.mesActual = mesActualKey; }
...
if (error.code === ‘permission-denied’) Utils.showToast(‘⚠️ Has alcanzado el límite de cálculos mensuales. Hazte premium para seguir usando la calculadora.’, ‘warning’);
else Utils.showToast(‘Error al registrar cálculo. Intenta de nuevo.’, ‘error’);
return false;
}
this.actualizarInterfazPremium();
return true;
...
btn.title = !hayCampos ? “Completa edad y tiempo” : “”;
} else {
btn.disabled = true;
...
if (tab) {
if (this.solicitudesPendientesCount > 0) { tab.classList.add(‘amigos-solicitudes-unread’); tab.setAttribute(‘data-count’, this.solicitudesPendientesCount); }
else { tab.classList.remove(‘amigos-solicitudes-unread’); tab.removeAttribute(‘data-count’); }
}
},
...
const tab = document.querySelector(’.tab-button[onclick=“switchTab('amigos')”]’);
if (tab) {
if (this.mensajesAmigosNoLeidos > 0) { tab.classList.add(‘chat-unread’); tab.setAttribute(‘data-chat-count’, this.mensajesAmigosNoLeidos); }
else { tab.classList.remove(‘chat-unread’); tab.removeAttribute(‘data-chat-count’); }
}
}
};
...
} else { Utils.showToast(‘❌ Mensaje no encontrado’, ‘error’); }
}
} catch (error) {
...
try {
let query = firebaseServices.db.collection(‘users’).orderBy(‘username’).limit(20);
...
if (this.usersPagination.lastDoc) query = query.startAfter(this.usersPagination.lastDoc);
...
const tieneMensajesNuevos = mensajesDoc.exists && mensajesDoc.data().mensajes && mensajesDoc.data().mensajes.some(m => !m.leido);
...
html += ` <div class="usuario-item ${esNuevo ? 'nuevo' : ''}" data-uid="${uid}"> <div class="usuario-header" data-uid="${uid}"> <span class="usuario-nombre">${Utils.escapeHTML(username)}</span> <span class="usuario-email">${email}</span> ${esNuevo ? '<span class="usuario-badge">NUEVO</span>' : ''} </div> <div class="usuario-detalle"> <div class="usuario-info"> <div class="info-item"><span class="info-label">Premium</span><span class="info-value">${premium}</span></div> <div class="info-item"><span class="info-label">Expira</span><span class="info-value">${expires}</span></div> <div class="info-item"><span class="info-label">Cálculos</span><span class="info-value">${calculos}</span></div> <div class="info-item"><span class="info-label">Registro</span><span class="info-value">${created}</span></div> </div> <div class="usuario-acciones"> <button class="ver-perfil-btn" data-uid="${uid}">VER</button> <button class="mensaje-usuario-btn" data-uid="${uid}" data-username="${Utils.escapeHTML(username)}">MENSAJE</button> <button class="premium-usuario-btn" data-uid="${uid}" data-username="${Utils.escapeHTML(username)}" data-premium="${user.premium}" data-expires="${user.expires || ''}">PREMIUM</button> <button class="eliminar-usuario-btn" data-uid="${uid}" data-username="${Utils.escapeHTML(username)}">ELIMINAR</button> </div> </div> </div>`;
}
container.innerHTML = html;
container.querySelectorAll(’.ver-perfil-btn’).forEach(btn => { btn.addEventListener(‘click’, (e) => { e.stopPropagation(); Admin.verPerfil(btn.dataset.uid); }); });
container.querySelectorAll(’.mensaje-usuario-btn’).forEach(btn => { btn.addEventListener(‘click’, (e) => { e.stopPropagation(); Admin.abrirMensajeUsuario(btn.dataset.uid, btn.dataset.username); }); });
container.querySelectorAll(’.premium-usuario-btn’).forEach(btn => { btn.addEventListener(‘click’, (e) => { e.stopPropagation(); Admin.abrirModalPremium(btn.dataset.uid, btn.dataset.username, btn.dataset.premium === ‘true’, btn.dataset.expires); }); });
container.querySelectorAll(’.eliminar-usuario-btn’).forEach(btn => { btn.addEventListener(‘click’, (e) => { e.stopPropagation(); Admin.eliminarUsuario(btn.dataset.uid, btn.dataset.username); }); });
container.querySelectorAll(’.usuario-header’).forEach(header => {
header.addEventListener(‘click’, () => {
const item = header.closest(’.usuario-item’);
if (item) Admin.toggleUsuario(item, item.dataset.uid);
});
});
const loadMoreBtn = document.getElementById(‘loadMoreUsersBtn’);
...
} finally { this.usersPagination.loading = false; }
},
...
buscarUsuarios() { this.cargarUsuarios(true); },
...
expiryEl.value = `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
} else {
...
allMessages.push({ …msg, fromUid: uid, username, mensajeIdx: idx, mensajeId, docId: doc.id, esNuevo: !msg.leido && !mensajesVistos.includes(mensajeId), texto: Utils.escapeHTML(msg.texto) });
});
}
allMessages.sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
if (allMessages.length === 0) { container.innerHTML = ‘<p style="text-align:center; padding:40px; color: var(--text-secondary);">No hay mensajes de usuarios</p>’; return; }
let html = ‘’;
allMessages.forEach(msg => {
const nuevoClass = msg.esNuevo ? ‘nuevo’ : ‘’;
html += ` <div class="mensaje-item ${nuevoClass}" data-mensaje-id="${msg.mensajeId}" data-uid="${msg.fromUid}" data-msg-idx="${msg.mensajeIdx}"> <div class="mensaje-header" data-mensaje-id="${msg.mensajeId}"> <span class="mensaje-fecha">📨 ${msg.fecha}</span> <span class="mensaje-remitente">${Utils.escapeHTML(msg.username)}</span> ${msg.esNuevo ? '<span class="nuevo-badge">NUEVO</span>' : ''} </div> <div class="mensaje-contenido"><p>${msg.texto}</p></div> <div class="mensaje-botones"> <button class="responder" data-uid="${msg.fromUid}" data-username="${Utils.escapeHTML(msg.username)}">✉️ RESPONDER</button> <button class="eliminar" data-uid="${msg.fromUid}" data-msg-idx="${msg.mensajeIdx}">🗑️ ELIMINAR</button> </div> </div>`;
});
container.innerHTML = html;
container.querySelectorAll(’.responder’).forEach(btn => { btn.addEventListener(‘click’, (e) => { e.stopPropagation(); Admin.responderMensaje(btn.dataset.uid, btn.dataset.username); }); });
container.querySelectorAll(’.eliminar’).forEach(btn => { btn.addEventListener(‘click’, (e) => { e.stopPropagation(); Admin.borrarMensajeUsuario(btn.dataset.uid, parseInt(btn.dataset.msgIdx)); }); });
container.querySelectorAll(’.mensaje-item’).forEach(item => {
const header = item.querySelector(’.mensaje-header’);
if (header) header.addEventListener(‘click’, () => Admin.toggleMensaje(item, item.dataset.mensajeId));
});
} catch (error) { console.error(‘Error cargando mensajes de usuarios:’, error); container.innerHTML = ‘<p style="text-align:center; padding:40px; color: var(--zone-5);">Error al cargar mensajes</p>’; }
},
...
if (element.classList.contains(‘nuevo’)) {
const mensajesVistos = JSON.parse(sessionStorage.getItem(‘admin_mensajes_vistos’) || ‘[]’);
if (!mensajesVistos.includes(mensajeId)) { mensajesVistos.push(mensajeId); sessionStorage.setItem(‘admin_mensajes_vistos’, JSON.stringify(mensajesVistos)); }
element.classList.remove(‘nuevo’);
...
marcarCampoTocado(c) {
if (!AppState) return;
AppState.camposTocados[c] = true; this.validarCampo(c); this.validarTodo();
},
...
```
// Detener listener del muro si salimos de esa pestaña
...
try {
  if(tab === 'historial') { if (AppState) AppState.resetHistorialPagination(); await this.cargarHistorialCompleto(true); }
  if(tab === 'plan') { await this.cargarHistorialPlanes(); }
  if(tab === 'soporte') { await this.cargarMensajesRecibidos(); await this.cargarMensajesEnviados(); }
  if(tab === 'perfil') { if (window.Profile) await Profile.cargarPerfil(); }
  if(tab === 'amigos') {
    if (window.Friends) {
      ...
      if (activeAmigosTab) {
        ...
        if (tabText.includes('buscar')) { if (!Friends.todosUsuariosPagination.lastDoc) await Friends.cargarTodosUsuarios(true); }
        else if (tabText.includes('solicitudes')) { await Friends.cargarSolicitudesRecibidas(); }
        else if (tabText.includes('mis amigos')) { await Friends.cargarListaAmigos(); }
      } else { await Friends.cargarTodosUsuarios(true); }
    }
  }
  if(tab === 'muro') {
    if (window.Wall) { Wall.detenerListener(); Wall.initListener(); }
  }
...
if (AppState.currentUserId) Storage.marcarLeido(AppState.currentUserId, idx);
...
e.stopPropagation();
UI.borrarMensajeUsuario(parseInt(eliminarBtn.dataset.msgIndex));
});
}
});
},
...
console.error(‘❌ Plan inválido, no se guarda en historial’); return;
}
try {
...
if(localStorage.getItem(‘pwa_installed’) !== ‘true’) { const banner = document.getElementById(‘pwa-banner’); if (banner) banner.style.display = ‘flex’; }
});
window.addEventListener(‘appinstalled’, () => {
const banner = document.getElementById(‘pwa-banner’); if (banner) banner.style.display = ‘none’;
AppState.deferredPrompt = null; localStorage.setItem(‘pwa_installed’, ‘true’);
...
if(choiceResult.outcome === ‘accepted’) { localStorage.setItem(‘pwa_installed’, ‘true’); Utils.showToast(‘✅ Instalando…’, ‘success’); }
AppState.deferredPrompt = null;
const banner = document.getElementById(‘pwa-banner’); if (banner) banner.style.display = ‘none’;
} catch (error) { console.error(‘Error instalando PWA:’, error); Utils.showToast(‘Error al instalar’, ‘error’); }
},
...
document.body.classList.remove(‘manual-light’); document.body.classList.add(‘manual-dark’); localStorage.setItem(‘ri5_theme’, ‘dark’);
} else if (document.body.classList.contains(‘manual-dark’)) {
document.body.classList.remove(‘manual-dark’); document.body.classList.add(‘manual-light’); localStorage.setItem(‘ri5_theme’, ‘light’);
} else {
const isDark = window.matchMedia(’(prefers-color-scheme: dark)’).matches;
if (isDark) { document.body.classList.add(‘manual-light’); localStorage.setItem(‘ri5_theme’, ‘light’); }
else { document.body.classList.add(‘manual-dark’); localStorage.setItem(‘ri5_theme’, ‘dark’); }
}
Utils.vibrate(30);
};
...
if (input) { const isPassword = input.type === ‘password’; input.type = isPassword ? ‘text’ : ‘password’; element.textContent = isPassword ? ‘ocultar’ : ‘ver’; }
else { console.error(‘No se pudo encontrar el input’); Utils.showToast(‘Error al mostrar/ocultar’, ‘error’); }
};
...
window.borrarMensajeUsuario = async function(i) { if (UI && UI.borrarMensajeUsuario) await UI.borrarMensajeUsuario(i); };
...
if (!email) { if (errorEl) { errorEl.textContent = ‘Introduce tu correo electrónico’; errorEl.classList.add(‘visible’); } return; }
if (!Utils.isValidEmail(email)) { if (errorEl) { errorEl.textContent = ‘Correo electrónico no válido’; errorEl.classList.add(‘visible’); } return; }
Utils.showLoading();
try {
await firebaseServices.auth.sendPasswordResetEmail(email);
Utils.hideLoading(); this.cerrarModal();
Utils.showToast(‘📧 Revisa tu correo para restablecer la contraseña’, ‘success’);
} catch (error) {
Utils.hideLoading();
if (error.code === ‘auth/user-not-found’) { Utils.showToast(‘Si el correo existe, recibirás instrucciones’, ‘info’); this.cerrarModal(); }
else Utils.handleFirebaseError(error);
}
}
};
...
console.log(‘🚀 Iniciando RI5 v4.8…’);
...
const unsubscribe = firebaseServices.auth.onAuthStateChanged((user) => { unsubscribe(); resolve(user); });
});
  ...
  if (currentUser && currentUser.uid === savedUid) {
    ...
    if (userDoc.exists) {
      ...
      if (!userData.username_lowercase && userData.username) {
        ...
        firebaseServices.db.collection('users').doc(savedUid).update({ username_lowercase: userData.username_lowercase })
      ...
      if (window.UI) { UI.changeDailyTip(); UI.startConsejoAutoChange(); }
      ...
      if(calc) {
        ...
        if (window.Training && Training.mostrarResultadosGuardados) Training.mostrarResultadosGuardados(calc);
      }
      ...
      if (estadoStr) {
        ...
        } catch (e) { console.warn('Error restaurando estado de pestaña', e); }
      }
...
setTimeout(() => {
if(AppState && AppState.deferredPrompt && localStorage.getItem(‘pwa_installed’) !== ‘true’ && localStorage.getItem(‘pwa_banner_closed’) !== ‘true’) {
const pwaBanner = document.getElementById(‘pwa-banner’);
...
window.audioEnabled = true; document.removeEventListener(‘click’, enableAudio);
}, { once: true });

await Storage.procesarCalculosPendientes();
window.addEventListener(‘online’, () => Storage.procesarCalculosPendientes());

console.log(‘✅ RI5 v4.8 inicializado correctamente’);
});

// ==================== ALIASES GLOBALES FINALES ====================
...
if (noMostrar) localStorage.setItem(‘ri5_info_visto’, ‘true’);
