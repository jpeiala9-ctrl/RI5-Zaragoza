// ==================== app.js - VERSIÓN COMPLETA CORREGIDA (CON MENSAJES UNIFICADOS Y RESTRICCIONES PREMIUM) ====================
...
const AppState = {
  ...
  // NUEVA FUNCIÓN PARA VERIFICAR SI PUEDE VER DETALLES DE SESIÓN
  puedeVerDetalleSesion() {
    // Usuarios premium activos pueden ver detalles
    if (this.isPremium && this.premiumExpiryDate && new Date() <= this.premiumExpiryDate) {
      return true;
    }
    // Usuarios no premium SOLO pueden ver los primeros 3 días del plan
    if (!this.isPremium || (this.premiumExpiryDate && new Date() > this.premiumExpiryDate)) {
      // Verificar si el plan existe y tiene fecha de inicio
      if (this.planGeneradoActual && this.planGeneradoActual.fechaInicio) {
        ...
        // Solo permitir ver detalles de los primeros 3 días
...
const Admin = {
  ...
  // ===== FUNCIÓN ACTUALIZADA: usa localStorage en lugar de sessionStorage =====
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      // CAMBIO IMPORTANTE: Usar localStorage en lugar de sessionStorage
      ...
      for (const doc of snapshot.docs) {
        ...
        // CAMBIO IMPORTANTE: Determinar si es nuevo para el admin
        // Un usuario es "nuevo" si:
        // 1. No ha sido visto antes POR ESTE ADMIN (almacenado en localStorage)
        // 2. Y además cumple UNA de estas condiciones:
        //    - Tiene mensajes no leídos para el admin
        //    - Se registró en los últimos 7 días
  ...
  toggleUsuario(element, uid) {
    ...
    // Marcar como visto en localStorage (persistente entre sesiones)
