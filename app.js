...
// Versión: 2.6 - Añadida sincronización offline de cálculos pendientes y optimización de getFriends
...
const Storage = {
  ...
  async getFriends(uid) {
    try {
      ...
      // Optimización: Promise.all en lugar de bucle secuencial
  ...
  // ==================== NUEVA FUNCIÓN: Procesar cálculos pendientes offline ====================
  async procesarCalculosPendientes() {
    ...
    for (const calc of pendientes) {
      try {
        ...
        // Si funciona, no lo añadimos a nuevosPendientes
      } catch (error) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // ===== PROCESAR CÁLCULOS PENDIENTES AL INICIAR =====
  await Storage.procesarCalculosPendientes();
  // Escuchar cuando la conexión vuelva
