...
// VERSIÓN: 4.20 - Admin: corrección de modales (lista y perfil) con restauración exacta
// ====================
...
const Admin = {
  ...
  // --- CORRECCIÓN: flag para controlar el estado del modal de lista ---
  _listModalOpen: false,
  ...
  // --- Modal de detalle de usuario (reemplaza alert) - CORREGIDO ---
  async verPerfil(uid) {
    if (!AppState.isAdmin || !uid) return;
    try {
      // Si el modal de lista está abierto, lo ocultamos y guardamos estado
      const listModal = document.getElementById('adminListModal');
      if (listModal && listModal.style.display !== 'none') {
        this._listModalOpen = true;
        listModal.style.display = 'none';
      } else {
        this._listModalOpen = false;
      }
  ...
  cerrarModalUsuario() {
    ...
    // Si venía de la lista, restaurar el modal de lista
    if (this._listModalOpen) {
      const listModal = document.getElementById('adminListModal');
  ...
  async mostrarUsuariosPorFiltro(tipo) {
    ...
    try {
      ...
      // Reiniciar flag al abrir la lista desde estadísticas
      this._listModalOpen = false;
