...
// VERSIÓN: 4.42 - CORREGIDO: mensajes de soporte se marcan como leídos con globalId y los badges se actualizan al instante
// ====================
...
const AppState = {
  ...
  iniciarListeners() {
    ...
    // Listener de mensajes de soporte para el ADMIN
    if (this.isAdmin) {
      this.unsubscribeMensajesSoporteAdmin = firebaseServices.db
        .collection('soporteMensajes')
        .where('toUid', '==', this.currentUserId)
        .onSnapshot((snapshot) => {
          let noLeidos = 0;
          snapshot.forEach(doc => {
            if (!doc.data().leido) noLeidos++;
          });
          if (this.mensajesSoporteAdminNoLeidos !== noLeidos) {
            this.mensajesSoporteAdminNoLeidos = noLeidos;
            if (window.UI) UI.actualizarBadgeMensajes();
          }
          // Si el admin tiene abierto el panel de soporte, refrescar la lista en vivo
          const adminSoportePanel = document.getElementById('adminSoportePanel');
          if (adminSoportePanel && adminSoportePanel.style.display !== 'none' && window.Admin) {
            Admin.cargarMensajesUsuarios(false);
          }
        }, (error) => {
          console.error('Error en listener de mensajes de soporte (admin):', error);
        });
    }
  },
  ...
  detenerListeners() {
    ...
    if (this.unsubscribeMensajesSoporteAdmin) {
      this.unsubscribeMensajesSoporteAdmin();
      this.unsubscribeMensajesSoporteAdmin = null;
    }
  },
...
const Admin = {
  ...
  async cargarMensajesUsuarios(reset = false) {
    ...
    try {
      ...
      // Evento click en el mensaje (expande y marca como leído)
      container.querySelectorAll('.mensaje-soporte-item').forEach(el => {
        el.addEventListener('click', async (e) => {
          ...
          if (id && el.classList.contains('no-leido')) {
            // Marcar como leído usando la función corregida de Storage
...
const UI = {
  ...
  renderMessageList(container, mensajes, tipo, onToggleCallback) {
    ...
    // Usar sessionStorage para recordar qué mensajes se han visto (ya no usamos el índice, usamos globalId)
    const mensajesVistos = JSON.parse(sessionStorage.getItem(`user_mensajes_vistos_${tipo}`) || '[]');
    let html = '';
    
    for (let i = 0; i < mensajes.length; i++) {
      const msg = mensajes[i];
      const mensajeId = msg.globalId || `local_${i}`;
      const esNuevo = !msg.leido && !mensajesVistos.includes(mensajeId);
      const nuevoClass = esNuevo ? 'nuevo' : '';
      const fecha = msg.fecha || msg.timestamp?.toDate?.().toLocaleString() || 'Fecha desconocida';
      const remitente = msg.esAdmin ? 'Admin' : (msg.esUsuario ? 'Tú' : 'Soporte');
      const icono = msg.esAdmin ? '📨' : (msg.esUsuario ? '📤' : '💬');
      const textoEscapado = Utils.escapeHTML(msg.texto || '');
      
      html += `
        <div class="mensaje-item ${nuevoClass}" data-mensaje-id="${mensajeId}" data-global-id="${msg.globalId || ''}" data-msg-index="${i}" data-tipo="${tipo}">
          ...
          <div class="mensaje-botones">
            ...
            <button class="eliminar" data-msg-index="${i}" data-tipo="${tipo}" onclick="UI.borrarMensajeUsuario(${i}, '${tipo}')">🗑️ ELIMINAR</button>
          </div>
        </div>
      `;
    }
    ...
    container.querySelectorAll('.mensaje-item').forEach(el => {
      ...
      if (header) {
        header.addEventListener('click', async (e) => {
          ...
          // Marcar como leído si tiene globalId
          if (globalId && el.classList.contains('nuevo')) {
            await Storage.marcarMensajeSoporteLeido(globalId, AppState.currentUserId);
            ...
            if (!vistos.includes(globalId)) {
              vistos.push(globalId);
  ...
  actualizarBadgeMensajes() {
    // Contador combinado: mensajes propios sin leer + (si es admin) mensajes de soporte entrantes sin leer.
    const totalNoLeidos = (AppState.mensajesNoLeidos || 0) + (AppState.isAdmin ? (AppState.mensajesSoporteAdminNoLeidos || 0) : 0);
    ...
    // Badge específico del subtab "💬 Soporte" dentro del panel de admin.
    if (AppState.isAdmin) {
      const adminBadge = document.getElementById('adminSoporteBadge');
      if (adminBadge) {
        const count = AppState.mensajesSoporteAdminNoLeidos || 0;
        if (count > 0) {
