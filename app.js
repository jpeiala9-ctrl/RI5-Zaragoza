// ==================== app.js - VERSIÓN COMPLETA CON SOPORTE INDEPENDIENTE ====================
// VERSIÓN: 4.29 - Tarjetas expandibles + botón eliminar en soporte
// ====================
...
const Admin = {
  ...
  // Eliminar mensaje de soporte (solo admin)
  async eliminarMensajeSoporte(mensajeId) {
    if (!AppState.isAdmin || !mensajeId) return;
    
    const confirmado = await Utils.confirm('ELIMINAR MENSAJE', '¿Eliminar este mensaje permanentemente?');
    if (!confirmado) return;
  ...
  // Cargar mensajes de soporte para el administrador (con preview y expandible)
  async cargarMensajesUsuarios(reset = false) {
    ...
    try {
      ...
      for (const msg of mensajes) {
        ...
        const badge = !leido ? '<span class="nuevo-badge" style="background:var(--notification-color); color:white; font-size:10px; padding:2px 8px; border-radius:10px; margin-left:8px;">NUEVO</span>' : '';
        ...
        html += `
          <div class="mensaje-soporte-item ${!leido ? 'no-leido' : ''}" data-id="${msg.id}" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:10px; padding:12px; margin-bottom:8px; cursor:pointer; ${!leido ? 'border-left:4px solid var(--notification-color);' : ''}">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
              <div style="font-weight:bold; color:var(--accent-yellow);">${nombre} ${badge}</div>
              <div style="font-size:11px; color:var(--text-secondary);">${fecha}</div>
            </div>
            <div class="mensaje-texto" style="font-size:14px; color:var(--text-primary); margin-bottom:6px; word-wrap:break-word;">
              <span class="preview">${preview}</span>
              ${esLargo ? `<span class="completo" style="display:none;">${textoCompleto}</span>` : ''}
              ${esLargo ? `<span class="expand-indicator" style="color:var(--accent-blue); font-size:12px; cursor:pointer;"> [Ver más]</span>` : ''}
            </div>
            <div style="display:flex; justify-content:flex-end; gap:8px;">
              <button class="responder-soporte-btn" data-uid="${msg.fromUid}" data-username="${Utils.escapeHTML(nombre)}" style="background:transparent; border:1px solid var(--accent-blue); color:var(--accent-blue); padding:4px 12px; border-radius:20px; cursor:pointer; font-size:12px;">Responder</button>
              <button class="eliminar-soporte-btn" data-id="${msg.id}" style="background:transparent; border:1px solid var(--zone-5); color:var(--zone-5); padding:4px 12px; border-radius:20px; cursor:pointer; font-size:12px;">🗑️ Eliminar</button>
            </div>
          </div>
        `;
      }
      ...
      // Evento: clic en la tarjeta para expandir/colapsar
      container.querySelectorAll('.mensaje-soporte-item').forEach(el => {
        el.addEventListener('click', async (e) => {
          // Ignorar si se hizo clic en un botón
          if (e.target.closest('.responder-soporte-btn') || e.target.closest('.eliminar-soporte-btn')) return;

          // Marcar como leído si no lo estaba
          const id = el.dataset.id;
          if (id && el.classList.contains('no-leido')) {
            await Storage.marcarMensajeSoporteLeido(id);
            el.classList.remove('no-leido');
            el.style.borderLeftColor = 'transparent';
            const badgeEl = el.querySelector('.nuevo-badge');
            if (badgeEl) badgeEl.remove();
          }

          // Expandir/colapsar texto
          const previewEl = el.querySelector('.preview');
          const completoEl = el.querySelector('.completo');
          const indicator = el.querySelector('.expand-indicator');
          if (completoEl) {
            if (completoEl.style.display === 'none') {
              previewEl.style.display = 'none';
              completoEl.style.display = 'inline';
              if (indicator) indicator.textContent = ' [Ver menos]';
            } else {
              previewEl.style.display = 'inline';
              completoEl.style.display = 'none';
              if (indicator) indicator.textContent = ' [Ver más]';
            }
          }
        });
      });
      ...
      container.querySelectorAll('.responder-soporte-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
          e.stopPropagation();
          const uid = btn.dataset.uid;
          const username = btn.dataset.username;
      ...
      container.querySelectorAll('.eliminar-soporte-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
          e.stopPropagation();
  ...
  // Enviar mensaje de soporte desde el administrador a un usuario
...
const UI = {
  ...
  async borrarMensajeUsuario(i, tipo) {
    Utils.showToast('Los mensajes se gestionan desde el administrador', 'info');
  },

  actualizarBadgeMensajes() {
    // Actualiza el badge de la pestaña "Soporte" en el perfil del usuario
  },
