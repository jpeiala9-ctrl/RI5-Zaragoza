...
// VERSIÓN: 4.26 - Admin: búsqueda de usuarios mejorada + listener mensajes + renombrado de pestaña
// ====================
...
const Admin = {
  ...
  // --- Mensajes de usuarios (con listener en tiempo real) ---

  // Inicia el listener en tiempo real para mensajes de usuarios
  _iniciarListenerMensajes() {
    ...
    // Escuchar todos los cambios en la colección mensajes
    this._unsubscribeMensajes = firebaseServices.db.collection('mensajes')
      .onSnapshot(() => {
        // Recargar la lista de mensajes (resetear paginación)
  ...
  async cargarMensajesUsuarios(reset = false) {
    ...
    try {
      const limit = 10;
      const result = await Storage.getAdminMessagesPaginated(limit, this.messagesPagination.lastDoc);
      
      // result.docs ya es un array de mensajes planos
      const messages = result.docs || [];
      this.messagesPagination.lastDoc = result.lastDoc;
      this.messagesPagination.hasMore = messages.length === limit;
      
      // Marcar nuevos mensajes (los que no están vistos)
      const mensajesVistos = JSON.parse(sessionStorage.getItem('admin_mensajes_vistos') || '[]');
      const messagesWithNew = messages.map(msg => {
        const mensajeId = `${msg.fromUid}_${msg.timestamp}`;
        return {
          ...msg,
          mensajeId,
          esNuevo: !msg.leido && !mensajesVistos.includes(mensajeId),
          texto: Utils.escapeHTML(msg.texto),
          username: Utils.capitalizeUsername(msg.username || 'Usuario')
        };
      });
      
      if (reset) {
        this.messagesPagination.allMessagesCache = messagesWithNew;
      } else {
        // Evitar duplicados
        const existingIds = new Set(this.messagesPagination.allMessagesCache.map(m => m.mensajeId));
        const newMessages = messagesWithNew.filter(m => !existingIds.has(m.mensajeId));
        this.messagesPagination.allMessagesCache = [...this.messagesPagination.allMessagesCache, ...newMessages];
      }
  ...
  async switchTab(tab) {
    ...
    try {
      ...
      if(tab === 'admin' && AppState && AppState.isAdmin) {
        ...
        Admin._iniciarListenerMensajes(); // Iniciar listener en tiempo real
      }
    } catch (error) {
