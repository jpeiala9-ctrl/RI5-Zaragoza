...
// VERSIÓN: 4.20 - Añadida función para agregar sesiones compartidas
// ====================
...
const Utils = {
  ...
  // NUEVA FUNCIÓN PARA UNIFICAR FORMATEO DE TIEMPO
...
const Admin = {
  ...
  aplicarFiltros() {
    ...
    this.usersPagination.lastDoc = null; // Resetear paginación al aplicar filtros
  ...
  async cargarUsuarios(reset = false) {
    ...
    try {
      // Consulta simple: solo orden por username_lowercase
      ...
      // 🔥 SCROLL AL CONTENEDOR CUANDO CARGAMOS MÁS
      if (!reset) {
  ...
  // --- Modal de detalle de usuario (reemplaza alert) ---
  async verPerfil(uid) {
    ...
    try {
      ...
      const gam = await Gamification.getData(uid);
      const entrenos = await firebaseServices.db.collection('globalFeed')
        .where('userId', '==', uid)
  ...
  // --- Mostrar lista de usuarios por filtro (al hacer clic en tarjetas) ---
    ...
    try {
      switch (tipo) {
        ...
        case 'sessions':
          ...
          if (userIds.length === 0) {
            ...
            // Obtener usuarios en lotes de 10
            const userPromises = [];
      ...
      if (usuarios.length === 0) {
        ...
        for (const user of usuarios) {
          ...
          html += `
            <div style="display:flex; align-items:center; justify-content:space-between; padding:10px 12px; background:var(--bg-secondary); border-radius:10px; cursor:pointer; border:1px solid var(--border-color);" onclick="Admin.verPerfil('${user.uid}')">
              <div>
  ...
  // --- Mensajes de usuarios (sin cambios) ---
...
// ==================== FUNCIÓN PARA AGREGAR SESIÓN COMPARTIDA ====================

/**
 * Función global para agregar una sesión compartida al plan del usuario
 * Se llama desde el chat cuando el usuario hace clic en "Añadir a mi plan"
 * @param {object} sessionData - Datos de la sesión
 * @param {string} fromUid - ID del amigo que compartió
 * @param {string} fromUsername - Nombre del amigo
 */
window.agregarSesionCompartidaAlPlan = async function(sessionData, fromUid, fromUsername) {
  if (!AppState.currentUserId) {
    Utils.showToast('⚠️ Inicia sesión para añadir la sesión.', 'warning');
    return false;
  }
  
  if (!AppState.isPremium) {
    Utils.showToast('⭐ Necesitas ser Premium para añadir sesiones compartidas.', 'warning');
    return false;
  }
  ...
  return await Storage.guardarSesionCompartidaEnPlan(
...
console.log('✅ app.js v4.20 - Función para agregar sesiones compartidas');