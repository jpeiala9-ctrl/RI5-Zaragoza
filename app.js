...
// VERSIÓN: 4.28 - Soporte usando colección separada soporteMensajes + subpestañas admin con líneas rectas
// ====================
...
const Admin = {
  ...
  _unsubscribeMensajes: null, // Para el listener en tiempo real (ya no se usa en soporte, pero se mantiene por compatibilidad)
  ...
  // Cargar mensajes de soporte para el administrador
  async cargarMensajesUsuarios(reset = false) {
    ...
    try {
      ...
      for (const msg of mensajes) {
        ...
        const badge = !leido ? '<span style="background:var(--notification-color); color:white; font-size:10px; padding:2px 8px; border-radius:10px; margin-left:8px;">NUEVO</span>' : '';
        const texto = Utils.escapeHTML(msg.texto || '');

        html += `
          <div class="mensaje-soporte-item" data-id="${msg.id}" style="background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:10px; padding:12px; margin-bottom:8px; cursor:pointer; ${!leido ? 'border-left:4px solid var(--notification-color);' : ''}">
            ...
            <div style="font-size:14px; color:var(--text-primary); margin-bottom:6px;">${texto}</div>
      ...
      // Evento clic para marcar como leído
      container.querySelectorAll('.mensaje-soporte-item').forEach(el => {
        el.addEventListener('click', async (e) => {
          if (e.target.closest('.responder-soporte-btn')) return;
          const id = el.dataset.id;
          if (id) {
            await Storage.marcarMensajeSoporteLeido(id);
            el.style.borderLeftColor = 'transparent';
            const badgeEl = el.querySelector('span[style*="background:var(--notification-color)"]');
  ...
  // (Opcional) Función para actualizar badge de soporte en el panel de administración
  actualizarBadgeSoporte() {
    // Si quieres mostrar un badge en el panel, puedes implementarlo aquí.
  },
...
const UI = {
  ...
  async switchTab(tab) {
    ...
    try {
      ...
      if(tab === 'soporte') { 
        // Soporte usa la nueva colección independiente
  ...
  async cargarMensajesRecibidos() {
    ...
    // Filtrar solo los mensajes recibidos (toUid == currentUserId)
  ...
  async cargarMensajesEnviados() {
    ...
    const mensajes = await Storage.getMensajesSoporteUsuario(AppState.currentUserId);
    // Filtrar solo los mensajes enviados (fromUid == currentUserId)
  ...
  async enviarMensajeUsuario() {
    ...
    // Enviar mensaje de soporte (colección independiente)
  ...
  async borrarMensajeUsuario(i, tipo) {
    // En la nueva lógica, los mensajes de soporte se borran desde el admin
  ...
  actualizarBadgeMensajes() {
    ...
    // (No es necesario para el administrador, que tiene su propio panel)
  },
