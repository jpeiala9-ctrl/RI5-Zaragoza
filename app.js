// ==================== app.js - VERSIÓN COMPLETA CON REDISEÑO ====================
// VERSIÓN: 4.19 - Añadidas funciones de navegación para bottom navigation
// ====================
...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (window.UI && UI.actualizarDashboard) {
...
const UI = {
  ...
  actualizarDashboard() {
    ...
    if (dashboardUserName && AppState.currentUserData) {
      dashboardUserName.innerText = Utils.capitalizeUsername(AppState.currentUserData.username);
    }
    
    // Actualizar estadísticas
    if (AppState.planActualId && window.PlanGenerator && PlanGenerator.planCompleto) {
      const sesiones = PlanGenerator.planCompleto?.sesiones || [];
      const semanaActual = AppState.trimestreActual * 12 + 1;
      const sesionesSemana = sesiones.filter(s => s.semana === semanaActual && s.tipo !== 'descanso');
      
      const kmSemana = sesionesSemana.reduce((acc, s) => acc + (s.detalle?.distanciaEstimada || 0), 0);
      const numSesiones = sesionesSemana.length;
      const completadas = Object.values(AppState.sesionesRealizadas || {}).filter(v => v === true).length;
      const totalSesiones = sesiones.length;
      const adherencia = totalSesiones > 0 ? Math.round((completadas / totalSesiones) * 100) : 0;
      
      document.getElementById('statKmSemana').innerText = Math.round(kmSemana);
      document.getElementById('statSesiones').innerText = numSesiones;
      document.getElementById('statAdherencia').innerText = adherencia;
      
      document.getElementById('planWeekLabel').innerText = `Semana ${semanaActual}/${sesiones.length / 7}`;
      document.getElementById('planCompleteLabel').innerText = `${adherencia}% completado`;
      document.getElementById('planProgressFill').style.width = `${adherencia}%`;
    }
    ...
    if (AppState.planActualId && window.PlanGenerator && PlanGenerator.planCompleto) {
      ...
      const sesiones = PlanGenerator.planCompleto.sesiones || [];
      const proximaSesion = sesiones.find(s => s.diaSemana >= diaSemana && s.tipo !== 'descanso' && !AppState.sesionesRealizadas?.[s.diaGlobal]);
      
      if (proximaSesion && proximaSesion.detalle) {
        document.getElementById('nextSessionName').innerText = `${proximaSesion.tipo.toUpperCase()} ${proximaSesion.detalle.zona || ''}`.trim();
        document.getElementById('nextSessionDetails').innerHTML = `${proximaSesion.detalle.nombre} · ${proximaSesion.duracion} min`;
        document.getElementById('nextSessionZones').innerHTML = `${proximaSesion.detalle.calentamiento}' Z1 + ${proximaSesion.detalle.partePrincipal}' principal + ${proximaSesion.detalle.enfriamiento}' Z1`;
      } else {
        document.getElementById('nextSessionName').innerText = '—';
        document.getElementById('nextSessionDetails').innerText = 'No hay sesiones programadas';
        document.getElementById('nextSessionZones').innerHTML = '';
      }
    }
  },
  
  async cargarMuroPreview() {
    const container = document.getElementById('wallPreview');
    if (!container) return;
    try {
      ...
      if (snapshot.empty) {
        container.innerHTML = '<p style="text-align:center; padding:20px;">No hay entrenamientos recientes</p>';
        return;
      }
      ...
      snapshot.forEach(doc => {
        ...
        html += `
          <div class="wall-post" onclick="switchToScreen('muro')">
            <div class="post-header">
              <div class="post-avatar">${entry.photoURL ? `<img src="${entry.photoURL}" style="width:44px; height:44px; border-radius:50%; object-fit:cover;">` : '👤'}</div>
              <div>
                <div class="post-user">${Utils.escapeHTML(usernameFormatted)}</div>
                <div class="post-time">${fecha}</div>
              </div>
            </div>
            <div class="post-training">
              ${tipoEmoji} ${Utils.escapeHTML(entry.trainingType || 'ENTRENO')} · ${entry.duration || 0}' · ${entry.distancia ? entry.distancia.toFixed(1) : '0'} km
            </div>
          </div>
        `;
      });
      ...
      console.error('Error cargando preview del muro:', error);
    }
  },
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Restaurar cálculo desde localStorage
  if (window.Training && Training._loadFromLocalStorage) {
    ...
    if (savedCalc && AppState && !AppState.zonasCalculadas) {
      ...
      if (document.getElementById('screen-entreno').classList.contains('active')) {
