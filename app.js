...
// VERSIÓN: 4.11 - CORREGIDO: eliminado hashPassword, verificación premium desde Firestore, confirm nativo reemplazado
// ====================
...
actualizarBadgeChat() {
const hayNoLeidos = this.mensajesAmigosNoLeidos > 0;

```
// 1. Tab principal AMIGOS — texto cambia de color
const tab = document.querySelector('.tab-button[onclick="switchTab(\'amigos\')"]');
if (tab) {
  tab.classList.toggle('chat-unread', hayNoLeidos);
  if (hayNoLeidos) tab.setAttribute('data-chat-count', this.mensajesAmigosNoLeidos);
  else tab.removeAttribute('data-chat-count');
}

// 2. Sub-pestaña "MIS AMIGOS" (btns[2]) — texto cambia de color
const amigosSubTabs = document.querySelectorAll('.amigos-tab');
if (amigosSubTabs[2]) {
  amigosSubTabs[2].classList.toggle('chat-messages-unread', hayNoLeidos);
}
```
