...
const AppState = {
  ...
  likesNuevosCount: 0,
  // 🔥 Entradas propias (globalFeed) con "me gusta" sin leer, más
  // reciente primero. Antes solo se guardaba el TOTAL (likesNuevosCount);
  // luego se guardó también el ID de cada una, para poder centrarla al
  // tocar la notificación en vez de solo abrir el Muro en general.
  // Ahora cada elemento es {id, ts} en vez de solo el id: el "ts" hace
  // falta para decidir, en el momento exacto de tocar la notificación (no
  // cuando llegó el snapshot), si esa tarjeta sigue dentro de las 24h del
  // Muro o si ya solo es alcanzable desde "mis últimos entrenamientos" en
  // Perfil -- el tiempo pasa entre que llega el like y que se toca la
  // campanita, así que hay que recalcularlo al vuelo (ver
  // irANotificacionDeLike() en index.html).
  ...
  iniciarListeners() {
    ...
    this.unsubscribeLikesPropios = firebaseServices.db
      ...
      .onSnapshot((snapshot) => {
        ...
        this.entriesConLikesNuevos = conNuevos; // {id, ts}[] -- ver comentario junto a la declaración
