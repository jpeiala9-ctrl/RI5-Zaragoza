...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    // Iniciar listeners de notificaciones en tiempo real si hay usuario
    if (uid) {
      this.iniciarListenersNotificaciones(uid);
    }
  },
  
  iniciarListenersNotificaciones(uid) {
    // Limpiar listeners anteriores si existen
    if (this.unsubscribeFriendRequests) this.unsubscribeFriendRequests();
    if (this.unsubscribeSoporte) this.unsubscribeSoporte();
    if (this.unsubscribeChat) this.unsubscribeChat();
    
    // 1. Solicitudes de amistad recibidas
    this.unsubscribeFriendRequests = firebaseServices.db
      .collection('friendRequests')
      .where('to', '==', uid)
      .where('status', '==', 'pending')
      .onSnapshot((snapshot) => {
        this.solicitudesPendientesCount = snapshot.size;
        this.actualizarBadgeSolicitudes();
      }, (error) => console.error('Error en listener de solicitudes:', error));
    
    // 2. Mensajes de soporte (admin)
    this.unsubscribeSoporte = firebaseServices.db
      .collection('mensajes')
      .doc(uid)
      .onSnapshot((doc) => {
        const data = doc.data();
        const mensajes = data?.mensajes || [];
        this.mensajesNoLeidos = mensajes.filter(m => !m.leido && m.esAdmin).length;
        if (window.UI) UI.actualizarBadgeMensajes();
      }, (error) => console.error('Error en listener de mensajes:', error));
    
    // 3. Mensajes de chat (conversaciones)
    this.unsubscribeChat = firebaseServices.db
      .collection('conversations')
      .where('participants', 'array-contains', uid)
      .onSnapshot(async () => {
        // Recargar conversaciones para contar no leídos
        const conversations = await Chat.getConversations(uid);
        const totalUnread = conversations.reduce((acc, c) => acc + (c.unreadCount || 0), 0);
        this.mensajesAmigosNoLeidos = totalUnread;
        this.actualizarBadgeChat();
      }, (error) => console.error('Error en listener de chat:', error));
  },
  
  limpiarListenersNotificaciones() {
    if (this.unsubscribeFriendRequests) this.unsubscribeFriendRequests();
    if (this.unsubscribeSoporte) this.unsubscribeSoporte();
    if (this.unsubscribeChat) this.unsubscribeChat();
    this.unsubscribeFriendRequests = null;
    this.unsubscribeSoporte = null;
  ...
  actualizarBadgeAmigos() {
    const tab = document.getElementById('amigosTabButton');
    if (tab) {
