...
// VERSIÓN: 3.6 - Corrección de historial, reset de notificaciones, cálculo ilimitado para todos
// ====================
...
const Storage = {
  ...
  // ========== HISTORIAL CORREGIDO CON MANEJO DE ERRORES ==========
  async getHistorial(uid, limit = 25, startAfter = null) {
    ...
    try {
      ...
      if (error.code === 'failed-precondition') {
        Utils.showToast('Error de índice en historial. Contacta al administrador.', 'error');
      } else {
...
const AppState = {
  ...
  actualizarInterfazPremium() {
    ...
    // Mostrar contador de cálculos para todos los usuarios (solo estadístico)
  ...
  // ========== MODIFICADO: Cálculos ilimitados para todos los usuarios ==========
  async incrementarCalculo() {
    ...
    // Siempre incrementamos el contador (sin límite)
    this.calculosMes++;
...
const UI = {
  ...
  async switchTab(tab) {
    ...
    try {
      ...
      // ========== MODIFICADO: resetear notificaciones al entrar en amigos ==========
      if(tab === 'amigos') {
        if (window.Friends) {
          // Limpiar badge de feed (ya no se usa, pero lo dejamos por seguridad)
          AppState.actualizarNovedadesFeed(0, new Set());
          // Resetear contador de solicitudes y actualizar badge
          AppState.solicitudesPendientesCount = 0;
          AppState.actualizarBadgeSolicitudes();   // <-- elimina el badge al entrar
          
          // Marcar feed como leído (solo para limpiar el estado interno)
