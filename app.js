...
// VERSIÓN: 4.12 - CORREGIDO: eliminado hashPassword, verificación premium desde Firestore, confirm nativo reemplazado
// CORREGIDO: creación automática de documento de mensajes en soporte
// ====================
...
const Utils = {
  ...
  // NOTA: función hashPassword eliminada por seguridad (base64 no es un hash real)
...
const Admin = {
  ...
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      // Eliminado select() para evitar errores por campos faltantes
...
const UI = {
  ...
  async cargarMensajesRecibidos() {
    ...
    Utils.showLoading();
    try {
      // ✅ Verificar si el documento de mensajes existe, si no, crearlo
      const mensajesRef = firebaseServices.db.collection('mensajes').doc(AppState.currentUserId);
      ...
      if (!doc.exists) {
        ...
        console.log('✅ Documento de mensajes creado desde cargarMensajesRecibidos para:', AppState.currentUserId);
      }
      ...
      container.innerHTML = '<p style="text-align:center; padding:20px; color: var(--zone-5);">Error al cargar mensajes</p>';
    } finally {
      Utils.hideLoading();
    }
  },
  ...
  async cargarMensajesEnviados() {
    ...
    try {
      ...
      container.innerHTML = '<p style="text-align:center; padding:20px; color: var(--zone-5);">Error al cargar mensajes enviados</p>';
    }
  },
  ...
  async enviarMensajeUsuario() {
    ...
    try {
      ...
      if (!doc.exists) {
        ...
        await mensajesRef.set({ mensajes: [mensajeBienvenida] });
        console.log('✅ Documento de mensajes creado desde enviarMensajeUsuario para:', AppState.currentUserId);
      }
      ...
      Utils.showToast('Error al enviar: ' + (error.message || 'Error desconocido'), 'error');
    } finally {
      Utils.hideLoading();
    }
  },
    ...
    try {
      ...
      if (currentUser && currentUser.uid === savedUid) {
        ...
        if (userDoc.exists) {
          ...
          // ✅ CORRECCIÓN: Verificar premium directamente desde Firestore
          // No se guardan datos sensibles en localStorage que puedan ser manipulados
          ...
          if (isPremiumValid && userData.expires) {
            ...
            if (premiumExpiryDate <= ahora) {
              // Premium expirado - actualizar Firestore
  ...
  document.addEventListener('click', function(e) {
    ...
    if (premiumElement) {
      ...
      if (perfilTab && perfilTab.classList.contains('active')) {
        ...
        // ✅ CORRECCIÓN: Reemplazar confirm nativo por Utils.confirm
        Utils.confirm(
