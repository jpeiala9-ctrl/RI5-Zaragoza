// ==================== app.js - VERSIÓN COMPLETA CON NUEVA LÓGICA PREMIUM ====================
// Versión: 2.2 - CORREGIDA (restauración de últimas zonas y activación de pestaña al recargar)
//
// Cambios respecto a la versión 2.1:
// - Se añade una llamada para activar la pestaña 'entreno' después de restaurar las zonas,
//   asegurando que se visualicen incluso si el estado guardado indicaba otra pestaña.
    ...
    try {
      ...
      if (currentUser && currentUser.uid === savedUid) {
        ...
        if (userDoc.exists) {
          ...
          // === CORRECCIÓN: usar Training.mostrarResultadosGuardados ===
          ...
          if(calc) { 
            ...
            // === NUEVO: Forzar la pestaña 'entreno' para que se vean las zonas ===
            // Esto evita que el estado guardado (por ejemplo, otra pestaña) oculte los resultados
            document.querySelectorAll('.tab-button').forEach(b => b.classList.remove('active'));
            document.querySelectorAll('.tab-content').forEach(c => c.classList.remove('active'));
            const entrenoBtn = Array.from(document.querySelectorAll('.tab-button')).find(b => b.textContent.includes('ENTRENO'));
            if (entrenoBtn) entrenoBtn.classList.add('active');
            document.getElementById('tab-entreno')?.classList.add('active');
          }
