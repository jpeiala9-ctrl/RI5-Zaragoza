...
const AppState = {
  ...
  unsubscribeSolicitudesEnviadasAceptadas: null,
  ...
  iniciarListeners() {
    ...
    // Lado del que ENVÍA la solicitud: antes solo se refrescaba quien
    // acepta (esa parte del código corre en su propia sesión); quien
    // envió la solicitud no se enteraba de que fue aceptada hasta que
    // refrescara la app a mano. Este listener detecta el cambio de
    // pending -> accepted en tiempo real y refresca amigos/perfil.
    // 'primerSnapshot' evita reaccionar a las solicitudes que YA estaban
    // aceptadas desde antes de abrir la app (solo interesan los cambios
    // que ocurren mientras la app está abierta).
    let primerSnapshotEnviadas = true;
    this.unsubscribeSolicitudesEnviadasAceptadas = firebaseServices.db
      .collection('friendRequests')
      .where('from', '==', this.currentUserId)
      .where('status', '==', 'accepted')
      .onSnapshot((snapshot) => {
        if (primerSnapshotEnviadas) { primerSnapshotEnviadas = false; return; }
        const huboNuevaAceptacion = snapshot.docChanges().some(c => c.type === 'added');
        if (!huboNuevaAceptacion) return;
        (async () => {
          try {
            if (window.Friends) {
            ...
            if (window.Profile && document.querySelector('.tab-button.active')?.textContent.toLowerCase() === 'perfil') {
              await Profile.cargarPerfil(true);
            }
            Utils.showToast('🎉 ¡Tenéis una nueva amistad!', 'success');
          } catch (e) { console.warn('Error refrescando tras amistad aceptada:', e); }
        })();
      }, (error) => {
        console.error('Error en listener de solicitudes enviadas aceptadas:', error);
      });
  ...
  detenerListeners() {
    ...
    if (this.unsubscribeSolicitudesEnviadasAceptadas) {
      this.unsubscribeSolicitudesEnviadasAceptadas();
      this.unsubscribeSolicitudesEnviadasAceptadas = null;
    }
...
const UI = {
  ...
  async restaurarEstado() {
    ...
    // La restauración de "pestaña activa" se quitó a propósito: en
    // móvil (PWA), sessionStorage puede sobrevivir a cerrar y volver a
    // abrir la app con más frecuencia de lo esperado (el navegador no
    // siempre destruye la sesión al cerrar por el gestor de apps), así
    // que esta restauración a veces dejaba al usuario en la pestaña
    // donde estaba antes en vez de en Inicio, justo lo contrario de lo
    // pedido: iniciar sesión (de la forma que sea) debe aterrizar
    // SIEMPRE en Inicio. guardarEstado() se deja tal cual por si algo
    // más lo necesita, simplemente ya no se usa para cambiar de pestaña.
  },
