// ==================== app.js - VERSIÓN ORIGINAL 4.16 CON PRECARGA AÑADIDA (SOLO ESO) ====================
// VERSIÓN: 4.16 + precarga en login y restauración de sesión
// ====================
...
const Admin = {
  ...
  renderUsersList(usersData) {
    ...
    container.querySelectorAll('.ver-perfil-btn').forEach(btn => {
      btn.addEventListener('click', (e) => { e.stopPropagation(); Admin.verPerfil(btn.dataset.uid); });
    });
    container.querySelectorAll('.mensaje-usuario-btn').forEach(btn => {
      btn.addEventListener('click', (e) => { e.stopPropagation(); Admin.abrirMensajeUsuario(btn.dataset.uid, btn.dataset.username); });
    });
    container.querySelectorAll('.premium-usuario-btn').forEach(btn => {
      btn.addEventListener('click', (e) => { e.stopPropagation(); Admin.abrirModalPremium(btn.dataset.uid, btn.dataset.username, btn.dataset.premium === 'true', btn.dataset.expires); });
    });
    container.querySelectorAll('.eliminar-usuario-btn').forEach(btn => {
      btn.addEventListener('click', (e) => { e.stopPropagation(); Admin.eliminarUsuario(btn.dataset.uid, btn.dataset.username); });
    });
  ...
  buscarUsuarios: Utils.debounce(function() { Admin.cargarUsuarios(true); }, 300),
  ...
  renderMessagesList(messages) {
    ...
    container.querySelectorAll('.responder').forEach(btn => {
      btn.addEventListener('click', (e) => { e.stopPropagation(); Admin.responderMensaje(btn.dataset.uid, btn.dataset.username); });
    });
    container.querySelectorAll('.eliminar').forEach(btn => {
      btn.addEventListener('click', (e) => { e.stopPropagation(); Admin.borrarMensajeUsuario(btn.dataset.uid, parseInt(btn.dataset.msgIdx)); });
    });
...
// Añadida función para cargar más mensajes en admin
...
// ========== CORRECCIÓN CRÍTICA: evitar error si PlanGenerator no está definido ==========
...
// ==================== AUTH (con precarga añadida) ====================
// NOTA: El objeto Auth se define aquí al final para que todo lo anterior esté disponible.
// Es exactamente igual que tu auth.js original, pero con los bloques de precarga dentro de loginUser y checkSavedSession.
...
const Auth = {
  ...
  _manejarErrorFirebase(error) {
    switch (error?.code) {
      ...
      case 'auth/user-disabled': return '🚫 Esta cuenta ha sido deshabilitada. Contacta con soporte.';
      ...
      case 'auth/email-already-in-use': return '📧 Este correo ya está registrado. Inicia sesión o usa otro.';
      case 'auth/weak-password': return '🔒 La contraseña debe tener al menos 6 caracteres.';
      case 'auth/network-request-failed': return '🌐 Error de conexión. Comprueba tu internet.';
      case 'auth/too-many-requests': return '⏳ Demasiados intentos. Espera un momento y vuelve a intentarlo.';
      case 'auth/requires-recent-login': return '⚠️ Por seguridad, debes volver a iniciar sesión antes de eliminar la cuenta.';
      case 'permission-denied': return '🔒 Error de permisos. Si persiste, contacta con soporte.';
      default: return '⚠️ Error al iniciar sesión. Revisa tus credenciales.';
    }
  },
  ...
  async createUserDocument(userId, email, username, extraData = {}) {
    ...
    const userData = {
      ...
      created: now.toISOString(), expires: expiry.toISOString(), premium: true,
      isAdmin: false, emailVerified: true, calculosMes: 0, mesActual,
      uid: userId, lastLogin: firebaseServices.Timestamp.now(),
    ...
    if (!mensajesDoc.exists) {
      await mensajesRef.set({ mensajes: [{ fecha: new Date().toLocaleString(), texto: "👋 ¡Bienvenido a RI5!...", leido: false, esAdmin: true, timestamp: firebaseServices.Timestamp.now() }] });
    }
    try {
      await firebaseServices.db.collection('gamification').doc(userId).set({
        totalXP: 0, level: 1, currentStreak: 0, longestStreak: 0, badges: [],
        totalDistance: 0, totalSessions: 0, lastSessionDate: null, lastUpdate: firebaseServices.Timestamp.now()
      });
    } catch(e) {}
    return userData;
  },
  ...
  async reenviarVerificacion() {
    ...
    if (!user) { Utils.showToast('No hay sesión activa', 'error'); return; }
    try { await user.sendEmailVerification(); Utils.showToast('📧 Correo de verificación reenviado.', 'success'); } catch (error) { Utils.handleFirebaseError(error); }
  },

  async verificarAhora() {
    const user = firebaseServices.auth.currentUser;
    if (!user) { document.getElementById("verificationPage").style.display = "none"; document.getElementById("loginPage").style.display = "flex"; Utils.hideLoading(); return; }
    Utils.showLoading();
    try {
      await user.reload();
      if (user.emailVerified) {
        ...
        if (!userDoc.exists) {
          ...
          if (usernameDoc.exists) finalUsername = username + '_' + Math.random().toString(36).substr(2,5);
          await firebaseServices.db.collection('usernames').doc(finalUsername).set({ uid: user.uid, createdAt: firebaseServices.Timestamp.now() });
        ...
        if (welcomeEl) welcomeEl.innerText = `> BIENVENIDO, ${userData.username.toUpperCase()} · ${userData.premium ? 'PREMIUM' : 'ACCESO'} HASTA ${new Date(userData.expires).toLocaleDateString()}`;
        if (document.getElementById('perfilContainer')) await Profile.cargarPerfil();
        Utils.showToast('✅ Correo verificado correctamente', 'success');
        if (window.UI) UI.startConsejoAutoChange();
      } else {
        Utils.showToast('❌ El correo aún no está verificado.', 'warning');
      }
    } catch (error) { console.error(error); Utils.showToast('Error al comprobar verificación', 'error'); }
    finally { Utils.hideLoading(); }
  },
  ...
  async registerUser() {
    ...
    if (!username || !email || !password) { errorEl.innerText = "⚠️ Completa todos los campos."; errorEl.classList.add('visible'); return; }
    if (username.length < 3) { errorEl.innerText = "⚠️ El nombre de usuario debe tener al menos 3 caracteres."; errorEl.classList.add('visible'); return; }
    if (password.length < 6) { errorEl.innerText = "⚠️ La contraseña debe tener al menos 6 caracteres."; errorEl.classList.add('visible'); return; }
    if (!Utils.isValidEmail(email)) { errorEl.innerText = "⚠️ El correo electrónico no es válido."; errorEl.classList.add('visible'); return; }
    if (!/^[a-zA-Z0-9_]+$/.test(username)) { errorEl.innerText = "⚠️ Solo se permiten letras, números y guión bajo en el usuario."; errorEl.classList.add('visible'); return; }
    btn.disabled = true; btn.textContent = 'REGISTRANDO...';
    try {
      const usernameDoc = await firebaseServices.db.collection('usernames').doc(username).get();
      if (usernameDoc.exists) { errorEl.innerText = "⚠️ El nombre de usuario ya está en uso. Elige otro."; errorEl.classList.add('visible'); btn.disabled = false; btn.textContent = '[ REGISTRARSE ]'; return; }
      const methods = await firebaseServices.auth.fetchSignInMethodsForEmail(email);
      if (methods.length > 0) { errorEl.innerText = "⚠️ El correo ya está registrado. Inicia sesión."; errorEl.classList.add('visible'); btn.disabled = false; btn.textContent = '[ REGISTRARSE ]'; return; }
      ...
      errorEl.innerText = errorMsg; errorEl.classList.add('visible');
  ...
  async loginUser() {
    ...
    errorEl.classList.remove('visible');
    if (!email || !password) { errorEl.innerText = "⚠️ Introduce correo y contraseña."; errorEl.classList.add('visible'); return; }
    if (!Utils.isValidEmail(email)) { errorEl.innerText = "⚠️ El correo electrónico no es válido."; errorEl.classList.add('visible'); return; }
    const ahora = Date.now();
    if (ahora - this.ultimoIntento < 2000) { errorEl.innerText = "⏳ Espera 2 segundos antes de reintentar."; errorEl.classList.add('visible'); return; }
    this.intentosLogin++; this.ultimoIntento = ahora;
    if (this.intentosLogin > 5) { errorEl.innerText = "⏳ Demasiados intentos. Espera 1 minuto."; errorEl.classList.add('visible'); setTimeout(() => { this.intentosLogin = 0; }, 60000); return; }
    ...
    try {
      ...
      const user = userCredential.user;
      if (!user.emailVerified) { this.showVerificationScreen(user.email); Utils.showToast('⚠️ Debes verificar tu correo antes de continuar', 'warning'); btn.disabled = false; btn.textContent = '[ ACCEDER ]'; return; }
      const userDoc = await firebaseServices.db.collection('users').doc(user.uid).get();
      if (!userDoc.exists) { await user.sendEmailVerification(); this.showVerificationScreen(user.email); Utils.showToast('⚠️ Completa la verificación de tu cuenta', 'warning'); btn.disabled = false; btn.textContent = '[ ACCEDER ]'; return; }
      const userData = userDoc.data();
      let isPremiumValid = userData.premium === true;
      if (isPremiumValid && userData.expires) {
        const expiryDate = new Date(userData.expires);
        if (expiryDate <= new Date()) { await firebaseServices.db.collection('users').doc(user.uid).update({ premium: false }); userData.premium = false; }
      }
      ...
      if (AppState.currentUserData.mesActual !== mesActual) { updates.calculosMes = 0; updates.mesActual = mesActual; AppState.currentUserData.calculosMes = 0; AppState.currentUserData.mesActual = mesActual; }
      ...
      // ========== PRECARGA TOTAL EN SEGUNDO PLANO (AÑADIDO) ==========
      setTimeout(() => {
        if (AppState.currentUserId) {
          ...
          if (window.UI) {
            UI.cargarMensajesRecibidos().catch(e => console.warn('Precarga mensajes:', e));
            UI.cargarMensajesEnviados().catch(e => console.warn('Precarga enviados:', e));
          }
        }
      }, 300);
      // ============================================================
      ...
      const welcomeEl = document.getElementById("userWelcome");
      if (welcomeEl) { const expiry = new Date(AppState.currentUserData.expires); welcomeEl.innerText = `> BIENVENIDO, ${AppState.currentUserData.username.toUpperCase()} · ${AppState.currentUserData.premium ? 'PREMIUM' : 'ACCESO'} HASTA ${expiry.toLocaleDateString()}`; }
      const nameField = document.getElementById('name');
      if (nameField) nameField.value = AppState.currentUserData.username;
      if (window.UI) { UI.changeDailyTip(); UI.startConsejoAutoChange(); await UI.cargarMensajesRecibidos(); await UI.cargarMensajesEnviados(); }
      const calc = await Storage.getUltimoCalculo(user.uid);
      if (calc) { AppState.setLastCalc(calc); if (window.UI) UI.mostrarResultadosGuardados(calc); }
  ...
  async _enablePersistence() {
    try { await firebaseServices.db.enablePersistence({ synchronizeTabs: true }); console.log('✅ Persistencia offline habilitada'); } catch (err) { console.warn('⚠️ Persistencia no disponible:', err); }
  },

  async eliminarMiCuenta() {
    const currentUser = firebaseServices.auth.currentUser;
    if (!currentUser) { Utils.showToast('❌ No hay sesión activa.', 'error'); return; }
    const email = currentUser.email, uid = currentUser.uid;
    let username = 'Usuario';
    try { const userDoc = await firebaseServices.db.collection('users').doc(uid).get(); if (userDoc.exists) username = userDoc.data().username || 'Usuario'; } catch(e) {}
    const confirmed = await Utils.confirm('SOLICITAR ELIMINACIÓN DE CUENTA', `¿SOLICITAR la eliminación de la cuenta "${email}"?\n\nSe enviará una notificación al administrador y se cerrará tu sesión.\n\nEl administrador procesará tu solicitud manualmente.`);
    if (!confirmed) return;
    Utils.showLoading();
    try {
      const adminMsgRef = firebaseServices.db.collection('mensajes').doc('admin_solicitudes');
      await adminMsgRef.set({ mensajes: firebaseServices.FieldValue.arrayUnion({ fecha: new Date().toLocaleString(), texto: `📢 SOLICITUD DE ELIMINACIÓN DE CUENTA\nUsuario: ${username}\nEmail: ${email}\nUID: ${uid}\n\nEl usuario solicita la eliminación de su cuenta.`, leido: false, esUsuario: true, timestamp: firebaseServices.Timestamp.now() }) }, { merge: true });
    ...
    } catch (error) { console.error('Error al enviar solicitud:', error); Utils.showToast(`❌ No se pudo enviar la solicitud. Error: ${error.message}`, 'error'); }
    finally { Utils.hideLoading(); }
  },
  ...
  async logoutUser() {
    ...
    } catch (error) { console.error('Error en logout:', error); Utils.showToast('Error al cerrar sesión', 'error'); }
    finally { Utils.hideLoading(); }
  },

  async checkSavedSession() {
    const uid = localStorage.getItem('ri5_current_user');
    if (!uid) { Utils.hideLoading(); return false; }
    try {
      if (!navigator.onLine) { localStorage.removeItem('ri5_current_user'); Utils.hideLoading(); document.getElementById("loginPage").style.display = "flex"; document.getElementById("mainContent").style.display = "none"; return false; }
      let user = firebaseServices.auth.currentUser;
      if (!user) { user = await new Promise((resolve) => { const unsubscribe = firebaseServices.auth.onAuthStateChanged((u) => { unsubscribe(); resolve(u); }); setTimeout(() => { unsubscribe(); resolve(null); }, 2000); }); }
      if (!user || user.uid !== uid) { localStorage.removeItem('ri5_current_user'); Utils.hideLoading(); document.getElementById("loginPage").style.display = "flex"; return false; }
      if (!user.emailVerified) { this.showVerificationScreen(user.email); return false; }
      const userDoc = await firebaseServices.db.collection('users').doc(uid).get();
      if (!userDoc.exists) { await firebaseServices.auth.signOut(); localStorage.removeItem('ri5_current_user'); Utils.hideLoading(); document.getElementById("loginPage").style.display = "flex"; return false; }
      const userData = userDoc.data();
      const now = new Date();
      if (userData.premium && new Date(userData.expires) <= now) { await firebaseServices.db.collection('users').doc(uid).update({ premium: false }); userData.premium = false; }
      localStorage.setItem('ri5_user_email', userData.email || '');
      if (userData.isAdmin) localStorage.setItem('ri5_is_admin', 'true'); else localStorage.removeItem('ri5_is_admin');
      await AppState.setCurrentUser(uid, userData.email, userData);
      document.getElementById("loginPage").style.display = "none";
      document.getElementById("verificationPage").style.display = "none";
      document.getElementById("mainContent").style.display = "flex";

      // ========== PRECARGA TOTAL AL RESTAURAR SESIÓN (AÑADIDO) ==========
      setTimeout(() => {
        if (AppState.currentUserId) {
          if (window.Profile) Profile.cargarPerfil().catch(e => console.warn('Precarga perfil:', e));
          if (window.Friends) {
      ...
      if (calc) { AppState.setLastCalc(calc); if (window.UI) UI.mostrarResultadosGuardados(calc); }
      await this._enablePersistence();
      await firebaseServices.db.collection('users').doc(uid).update({ lastLogin: firebaseServices.Timestamp.now() }).catch(e => console.warn(e));
      Utils.hideLoading();
      return true;
    } catch (error) { console.error('Error en checkSavedSession:', error); localStorage.removeItem('ri5_current_user'); Utils.hideLoading(); document.getElementById("loginPage").style.display = "flex"; return false; }
  },

  showPremiumBenefits() { document.getElementById('premiumOverlay').classList.add('active'); document.getElementById('premiumModal').classList.add('active'); }
};
...
firebaseServices.auth.onAuthStateChanged(async (user) => {
  if (user) {
    if (user.emailVerified) {
      const savedUid = localStorage.getItem('ri5_current_user');
      if (savedUid === user.uid || document.getElementById("mainContent").style.display !== "flex") {
        const userDoc = await firebaseServices.db.collection('users').doc(user.uid).get();
        if (userDoc.exists) {
          await AppState.setCurrentUser(user.uid, user.email, userDoc.data());
          document.getElementById("loginPage").style.display = "none";
          document.getElementById("verificationPage").style.display = "none";
          document.getElementById("mainContent").style.display = "flex";
          if (window.UI) UI.startConsejoAutoChange();
          Utils.hideLoading();
        } else { Auth.showVerificationScreen(user.email); }
      }
    } else { Auth.showVerificationScreen(user.email); }
  } else {
...
// Asignar funciones globales de Auth
