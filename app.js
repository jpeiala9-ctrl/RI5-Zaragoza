// ==================== app.js - VERSIÓN COMPLETA CON SOPORTE BASADO EN CHAT ====================
// VERSIÓN: 4.27 - Soporte usando conversaciones (chat entre amigos) + subpestañas admin
// ====================
...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    // Cambiar nombre de pestaña "Soporte" a "Administración" si es admin
...
const Admin = {
  ...
  _unsubscribeMensajes: null, // Para el listener en tiempo real
  ...
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      if (searchTerm) {
        // 🔍 BÚSQUEDA: combinamos resultados de username_lowercase y username
        const termLower = searchTerm.toLowerCase();
        // Consulta 1: por username_lowercase (insensible a mayúsculas)
        let query1 = firebaseServices.db.collection('users')
          .orderBy('username_lowercase')
          .startAt(termLower)
          .endAt(termLower + '\uf8ff')
          .limit(50);
        // Consulta 2: por username (sensible a mayúsculas, como fallback)
        ...
        // 📋 SIN BÚSQUEDA: consulta normal con paginación
      ...
      // ── A partir de aquí, el código es el mismo que ya tenías ──
  ...
  // --- MENSAJES DE SOPORTE (basados en conversaciones de chat) ---

  // Inicia el listener en tiempo real para conversaciones del admin
  _iniciarListenerMensajes() {
    if (this._unsubscribeMensajes) {
      this._unsubscribeMensajes();
      this._unsubscribeMensajes = null;
    }
    if (!AppState.isAdmin) return;
    
    this._unsubscribeMensajes = firebaseServices.db.collection('conversations')
      .where('participants', 'array-contains', AppState.currentUserId)
      .onSnapshot(() => {
        const soportePanel = document.getElementById('adminSoportePanel');
        if (soportePanel && soportePanel.style.display !== 'none') {
          this.cargarMensajesUsuarios(true);
        }
      }, (error) => {
        console.error('Error en listener de conversaciones de soporte:', error);
      });
  },

  _detenerListenerMensajes() {
    if (this._unsubscribeMensajes) {
      this._unsubscribeMensajes();
      this._unsubscribeMensajes = null;
    }
  ...
  // Cambiar entre subpestañas del panel de administración
  cambiarSubtab(subtab) {
    ...
    if (subtab === 'control') {
      ...
      this.cargarMensajesUsuarios(true);
      this._iniciarListenerMensajes();
    }
  },

  // Cargar conversaciones de soporte para el administrador
  async cargarMensajesUsuarios(reset = false) {
    ...
    try {
      // Obtener todas las conversaciones del administrador
      const conversations = await Chat.getConversations(AppState.currentUserId);
      
      if (!conversations || conversations.length === 0) {
        container.innerHTML = '<p style="text-align:center; padding:40px; color: var(--text-secondary);">No hay conversaciones de soporte</p>';
        return;
      }

      // Renderizar cada conversación como un elemento de soporte
      let html = '';
      for (const conv of conversations) {
        // Obtener el nombre del otro participante (que no es el admin)
        const otherUserId = conv.otherUserId;
        const otherUsername = conv.otherUsername || 'Usuario';
        ...
        html += `
          <div class="conversacion-item" data-conv-id="${conv.id}" data-other-user-id="${otherUserId}" data-other-username="${Utils.escapeHTML(otherUsername)}" style="cursor:pointer; padding:12px; background:var(--bg-secondary); border:1px solid var(--border-color); border-radius:10px; margin-bottom:8px; display:flex; align-items:center; justify-content:space-between;">
            <div>
              <div style="font-weight:bold; color:var(--accent-yellow);">${Utils.escapeHTML(otherUsername)}</div>
              <div style="font-size:12px; color:var(--text-secondary);">${Utils.escapeHTML(lastMessage)}</div>
            </div>
            <div style="display:flex; align-items:center; gap:8px;">
              ${unreadCount > 0 ? `<span style="background:var(--notification-color); color:white; border-radius:50%; padding:2px 8px; font-size:12px;">${unreadCount}</span>` : ''}
      ...
      // Añadir evento click para abrir el chat con ese usuario
      container.querySelectorAll('.conversacion-item').forEach(el => {
        el.addEventListener('click', () => {
          ...
          Chat.openChat(convId, otherUserId, otherUsername);
        });
      });

    } catch (error) {
      console.error('Error cargando conversaciones de soporte:', error);
      container.innerHTML = '<p style="text-align:center; padding:40px; color: var(--zone-5);">Error al cargar conversaciones</p>';
    }
  },

  // ... (resto de funciones de admin como cargarMasMensajesAdmin, toggleMensaje, etc.)
  // Nota: algunas de estas funciones pueden quedar obsoletas con la nueva lógica, pero se mantienen por si acaso.

  cargarMasMensajesAdmin() {
    // Esta función ya no es necesaria, pero se mantiene por compatibilidad.
    Utils.showToast('Los mensajes se cargan en tiempo real', 'info');
  },

  toggleMensaje(element, mensajeId) {
    // Función obsoleta, pero se mantiene.
    if (!element) return;
    element.classList.toggle('abierto');
  },
...
const UI = {
  ...
  async switchTab(tab) {
    ...
    try {
      ...
      if(tab === 'soporte') { 
        // En la pestaña de soporte, ahora usamos el chat, pero la UI sigue siendo la misma.
      ...
      if(tab === 'perfil') {
        ...
        if (AppState.isAdmin) {
          // Por defecto mostrar panel de control
          setTimeout(() => {
      ...
      if(tab === 'admin' && AppState && AppState.isAdmin) {
        ...
        // No cargamos mensajes aquí, se cargan al cambiar a la subpestaña de soporte.
        // Iniciamos el listener solo si la subpestaña de soporte está activa.
        if (document.getElementById('adminSoportePanel') && document.getElementById('adminSoportePanel').style.display !== 'none') {
          Admin._iniciarListenerMensajes();
        }
      }
    } catch (error) {
  ...
  // === NUEVAS FUNCIONES DE SOPORTE BASADAS EN CHAT ===
  ...
  async cargarMensajesRecibidos() {
    ...
    if (!adminUid) {
      container.innerHTML = '<p style="text-align:center; padding:20px;">Administrador no encontrado</p>';
      return;
    }

    // Obtener la conversación entre el usuario y el admin
    const conversationId = [AppState.currentUserId, adminUid].sort().join('_');
    try {
      ...
      // Renderizar los mensajes (solo los que son del admin hacia el usuario)
      const mensajesAdmin = mensajes.filter(m => m.senderId === adminUid);
      this.renderMessageList(container, mensajesAdmin, 'recibido', null);
      this.actualizarBadgeMensajes();
    } catch (error) {
  ...
  async cargarMensajesEnviados() {
    ...
    try {
      ...
      const mensajes = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      // Renderizar los mensajes enviados por el usuario al admin
      const mensajesEnviados = mensajes.filter(m => m.senderId === AppState.currentUserId);
      this.renderMessageList(container, mensajesEnviados, 'enviado', null);
    } catch (error) {
  ...
  async enviarMensajeUsuario() {
    ...
    // Si el usuario es el administrador, no puede enviarse mensajes a sí mismo
    if (AppState.currentUserId === adminUid) {
      Utils.showToast('No puedes enviarte un mensaje a ti mismo', 'error');
      return;
    }

    // Obtener o crear la conversación con el admin
    const conversationId = await Chat.getOrCreateConversation(AppState.currentUserId, adminUid);
    if (!conversationId) {
      Utils.showToast('Error al iniciar conversación con soporte', 'error');
      return;
    }

    // Enviar mensaje usando el chat
    const ok = await Chat.sendMessage(conversationId, texto);
    if (ok) {
      document.getElementById('mensajeUsuario').value = '';
      Utils.showToast('✅ Mensaje enviado a soporte', 'success');
      // Recargar mensajes enviados para actualizar la lista
      await this.cargarMensajesEnviados();
    } else {
  ...
  // === Funciones de borrado (adaptadas para chat) ===

  async borrarMensajeUsuario(i, tipo) {
    // En la nueva lógica, los mensajes se borran desde el chat, pero mantenemos esta función por compatibilidad.
    Utils.showToast('Los mensajes se gestionan desde el chat', 'info');
  },

  // === Funciones de historial y planes (sin cambios) ===
  ...
  actualizarBadgeMensajes() {
    // Esta función se mantiene para compatibilidad, pero ahora los mensajes de soporte se gestionan con el chat.
  ...
  // === Función para cambiar soporte tab (usuario) ===
