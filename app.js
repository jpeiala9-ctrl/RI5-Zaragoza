...
// VERSIÓN: 4.12 - CORREGIDO: Toasts amigables, envío de mensajes soporte, XSS
// ====================
...
const errorMap = {
  ...
  'auth/user-token-expired': 'Tu sesión ha expirado. Por favor, inicia sesión de nuevo.',
  'auth/invalid-credential': 'Credenciales inválidas. Verifica tu correo y contraseña.',
  'permission-denied': 'No tienes permisos para realizar esta acción.',
  'unavailable': 'El servicio no está disponible. Inténtalo más tarde.',
  'not-found': 'El documento solicitado no se encontró.',
  'already-exists': 'El documento ya existe.',
  'resource-exhausted': 'Has superado el límite de uso. Inténtalo más tarde.'
};

let friendlyMessage = errorMap[error.code];
...
if (!friendlyMessage) {
  if (error.message) {
    if (error.message.includes('permission-denied')) {
      ...
      friendlyMessage = 'Error de conexión. Comprueba tu red.';
    } else {
      friendlyMessage = 'Ha ocurrido un error inesperado. Si el problema persiste, contacta con soporte.';
    }
  } else {
...
if (uid) {
  ...
  // Limpiar entradas antiguas del muro tras login
  if (window.PlanGenerator && window.PlanGenerator.limpiarMuroGlobal) {
    PlanGenerator.limpiarMuroGlobal().catch(e => console.warn('limpiarMuroGlobal:', e));
  }
} else {
...
this.unsubscribeConversations = firebaseServices.db
  ...
  .onSnapshot(async () => {
    if (window.Chat) {
      ...
      // Refrescar lista de conversaciones en tiempo real si está visible
      const container = document.getElementById('listaConversaciones');
      if (container) await Chat.renderConversations(container);
    }
  }, (error) => {
...
const tab = document.querySelector(’.tab-button[onclick*=“amigos”]’);
if (tab) {
...
try {
  ...
  Utils.showToast(`✅ Usuario ${username} eliminado`, 'success');
...
```
const confirmado = await Utils.confirm('ENVÍO MASIVO', `¿Enviar este mensaje a TODOS los usuarios?`);
if (!confirmado) return;
...
if (e.target.closest(’.eliminar’)) return;
const idx = parseInt(el.dataset.msgIndex);
const tipoMsg = el.dataset.tipo;
const mensajeId = el.dataset.mensajeId;
el.classList.toggle(‘abierto’);
...
Utils.showLoading();
try {
const success = await Storage.enviarMensajeUsuario(AppState.currentUserId, t);
if (success) {
...
Utils.showToast(‘No se pudo enviar el mensaje’, ‘error’);
}
}
catch (error) {
console.error(‘Error enviando mensaje:’, error);
...
const confirmed = await Utils.confirm(‘Eliminar entrada’, ‘¿Eliminar esta entrada del historial?’);
if(!confirmed) return;
try {
...
if(!AppState || !AppState.currentUserId) return;
const confirmed = await Utils.confirm(‘Limpiar historial’, ‘¿Eliminar todo el historial de cálculos?’);
if(!confirmed) return;
Utils.showLoading();
try {
...
if (AppState) AppState.resetHistorialPagination();
await this.cargarHistorialCompleto(true);
Utils.showToast(‘✅ Historial limpiado’, ‘success’);
} catch (error) {
...
if (errorEl) { errorEl.textContent = ‘El formato del correo no es válido’; errorEl.classList.add(‘visible’); }
return;
}
Utils.showLoading();
try {
