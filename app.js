...
const AppState = {
  ...
  actualizarInterfazPremium() {
    // "nuevoPlanBtn" y "generarPlanBtn" ya NO se deshabilitan para
    // usuarios gratis: ahora cualquiera puede generar y ver su calendario
    // de plan (solo el detalle de cada sesión al pulsar un día queda
    // bloqueado tras el modal premium — ver abrirDetalleSesion en
    // calendar.js). "borrarPlanBtn" no existe en el HTML actual (el botón
    // se quitó), se deja en la lista sin efecto por si se reintroduce.
    const planBtns = ['borrarPlanBtn'];
