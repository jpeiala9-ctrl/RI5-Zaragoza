...
// VERSIÓN: 4.39 - Eliminado timeout forzado; hideLoading ahora se gestiona correctamente en auth.js (solo aviso de diagnóstico no intrusivo)
// ====================
...
const UI = {
  ...
  async switchTab(tab) {
    ...
    try {
      ...
      if(tab === 'perfil') {
        if (window.Profile) {
          Profile.cargarPerfil().catch(e => console.warn('Error cargando perfil:', e));
        }
        if (AppState.isAdmin) {
          setTimeout(() => {
