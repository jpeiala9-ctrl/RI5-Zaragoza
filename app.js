...
// VERSIÓN: 4.14 - NOTIFICACIONES CHAT CON COLOR VERDE #25D366 Y DOBLE CHECK AZUL EN TIEMPO REAL
// ====================
...
const AppState = {
  ...
  // Nuevo: Almacenar qué amigos tienen mensajes no leídos específicamente
  amigosConMensajesNoLeidos: new Set(),

  unsubscribeFriendRequests: null,
  unsubscribeConversations: null,
  unsubscribeNewMessages: null,
  unsubscribeMessageStatus: null, // Nuevo: para escuchar cambios en estado de lectura
  ...
  iniciarListeners() {
    ...
    // ✅ NUEVO: Escuchar mensajes nuevos y cambios de estado de lectura
    this.unsubscribeNewMessages = firebaseServices.db
      ...
      .onSnapshot((snapshot) => {
        snapshot.docChanges().forEach((change) => {
          ...
          if (change.type === 'added') {
            // Mensaje nuevo recibido (el otro usuario envió)
            if (msgData.senderId !== this.currentUserId) {
              // Obtener el otro usuario para saber quién envió
              const otherUserId = msgData.senderId;
              if (!this.amigosConMensajesNoLeidos.has(otherUserId)) {
                this.amigosConMensajesNoLeidos.add(otherUserId);
              }
              Utils.vibrate(100);
              Utils.showToast('📩 Nuevo mensaje de un amigo', 'info', 3000);
              this.actualizarBadgeChat();
              
              // Actualizar la lista de amigos si está visible
              const activeAmigosTab = document.querySelector('.amigos-tab.active')?.textContent.toLowerCase();
              if (activeAmigosTab === 'lista' && window.Friends) {
                Friends.cargarListaAmigos();
              }
            }
          } else if (change.type === 'modified') {
            // Cambio en el estado de lectura - actualizar UI del chat abierto
            if (window.Chat && window.Chat.currentConversationId === conversationId) {
              if (window.Chat.loadAndRenderMessages) {
                window.Chat.loadAndRenderMessages(window.Chat.currentConversationId, window.Chat.currentOtherUserId);
              }
            }
          }
        });
    ...
    // ✅ NUEVO: Escuchar específicamente cambios en readBy para actualizar doble check
    this.unsubscribeMessageStatus = firebaseServices.db
      .collectionGroup('messages')
      .where('readBy', 'array-contains', this.currentUserId)
      .onSnapshot((snapshot) => {
        snapshot.docChanges().forEach((change) => {
        ...
        console.error('Error en listener de estados de mensajes:', error);
      });
  },
  ...
  detenerListeners() {
    ...
    if (this.unsubscribeNewMessages) {
      this.unsubscribeNewMessages();
      this.unsubscribeNewMessages = null;
    }
    if (this.unsubscribeMessageStatus) {
      this.unsubscribeMessageStatus();
      this.unsubscribeMessageStatus = null;
    }
  },
  ...
  // ✅ MODIFICADO: Usar color #25D366 para notificaciones de chat
  actualizarBadgeChat() {
    const tab = document.querySelector('.tab-button[onclick="switchTab(\'amigos\')"]');
    if (tab) {
      if (this.mensajesAmigosNoLeidos > 0) {
        ...
        // Aplicar color verde WhatsApp
        tab.style.color = '#25D366';
      } else {
    ...
    // También actualizar la pestaña "MIS AMIGOS" dentro de la sección de amigos
    const misAmigosTab = document.querySelector('.amigos-tab[onclick*="lista"]');
    if (misAmigosTab) {
      if (this.mensajesAmigosNoLeidos > 0) {
        misAmigosTab.classList.add('tiene-mensajes');
      } else {
        misAmigosTab.classList.remove('tiene-mensajes');
      }
    }
  }
};
...
window.cambiarAmigosTab = function(tab) {
  ...
  if (tab === 'lista' && window.Friends) {
    Friends.cargarListaAmigos();
    // ✅ Al entrar a "MIS AMIGOS", marcar mensajes como leídos
    if (AppState && AppState.mensajesAmigosNoLeidos > 0) {
      AppState.amigosConMensajesNoLeidos.clear();
      AppState.mensajesAmigosNoLeidos = 0;
      ...
      if (currentUser && currentUser.uid === savedUid) {
        ...
        if (userDoc.exists) {
          ...
          if (isPremiumValid && userData.expires) {
            premiumExpiryDate = new Date(userData.expires);
            const ahora = new Date();
            if (premiumExpiryDate <= ahora) {
              console.log('Premium expirado, actualizando Firestore...');
              await firebaseServices.db.collection('users').doc(savedUid).update({
                premium: false
          ...
          if (welcomeEl) {
            const expiry = userData.expires ? new Date(userData.expires) : null;
            const usernameFormatted = Utils.capitalizeUsername(userData.username);
            const expiryText = expiry ? expiry.toLocaleDateString() : 'sin fecha';
            welcomeEl.innerText = `> BIENVENIDO, ${usernameFormatted.toUpperCase()} · ${userData.premium ? 'PREMIUM' : 'ACCESO'} HASTA ${expiryText}`;
          }
