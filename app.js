...
const UI = {
  ...
  async restaurarEstado() {
    ...
    // Pestaña activa: se lee de sessionStorage, NO de localStorage, a
    // propósito. sessionStorage sobrevive a una simple recarga (F5) de la
    // misma pestaña/sesión del navegador, pero se borra solo cuando de
    // verdad se cierra la pestaña/app. Eso da justo el comportamiento
    // pedido: recargar te deja donde estabas, pero cerrar y volver a
    // abrir (o cerrar sesión y entrar de nuevo, que además borra esta
    // misma clave explícitamente en el logout) aterriza siempre en Inicio,
    // porque no hay nada guardado que restaurar.
    const estadoStr = sessionStorage.getItem('ri5_estado');
    if (!estadoStr) return;
    try {
      const estado = JSON.parse(estadoStr);
      if (estado.activeTab && estado.activeTab !== 'inicio') {
        const tabMap = { 'plan': 'plan', 'comunidad': 'comunidad', 'perfil': 'perfil' };
        const tab = tabMap[estado.activeTab];
        if (tab) {
      ...
      console.warn('Error restaurando pestaña activa:', e);
    }
  },
...
window.toggleTheme = function(btn) {
  ...
  // Partículas de luz: en vez de un único círculo que crece, salen varias
  // decenas de partículas desde la propia bombilla hacia puntos repartidos
  // por toda la pantalla (las más lejanas salen un poco más tarde, para que
  // se note que "parten" desde la esquina de la bombilla). Al llegar a su
  // destino, cada una "salpica" una mancha del color del tema nuevo que
  // crece, como si tirásemos un cubo de pintura hecho de luz. Al final se
  // añade un desvanecido de seguridad para asegurar que no quede ningún
  // hueco sin cubrir.
  ...
  const maxDist = Math.hypot(Math.max(originX, vw - originX), Math.max(originY, vh - originY));

  const baseColor = newTheme === 'light' ? '245,245,245' : '15,15,15';
  const crestColor = newTheme === 'light' ? '255,255,255' : '192,160,96'; // dorado en oscuro, blanco en claro
  const particleColor = newTheme === 'light' ? '255,235,180' : '255,240,200'; // luz cálida

  const DPR = Math.min(window.devicePixelRatio || 1, 2);
  const canvas = document.createElement('canvas');
  canvas.width = vw * DPR;
  canvas.height = vh * DPR;
  canvas.style.cssText = `position:fixed; inset:0; width:${vw}px; height:${vh}px; z-index:99999; pointer-events:none;`;
  ...
  // Se generan los destinos de cada partícula, repartidos por toda la
  // pantalla, y se calcula cuándo "sale" y cuánto tarda en llegar según su
  // distancia al origen.
  const particles = [];
  for (let i = 0; i < N_PARTICLES; i++) {
    ...
    const launchDelay = distRatio * DURATION * 0.35 + Math.random() * 80;
    const flightTime = 220 + Math.random() * 180;
    const splashRadius = 140 + Math.random() * 140;
    const splashDuration = 380 + Math.random() * 220;
    particles.push({ targetX, targetY, launchDelay, flightTime, splashRadius, splashDuration, splashed: false });
  }
  ...
  function step(now) {
    ...
    particles.forEach(p => {
      const tSinceLaunch = elapsed - p.launchDelay;
      if (tSinceLaunch < 0) return; // aún no ha salido

      if (tSinceLaunch < p.flightTime) {
        // En vuelo: un cometa con estela hacia el destino
        const tf = easeOutQuad(Math.min(1, tSinceLaunch / p.flightTime));
        const x = originX + (p.targetX - originX) * tf;
        const y = originY + (p.targetY - originY) * tf;
        const grad = ctx.createRadialGradient(x, y, 0, x, y, 14);
        grad.addColorStop(0, `rgba(${particleColor},0.95)`);
        grad.addColorStop(1, `rgba(${particleColor},0)`);
        ctx.fillStyle = grad;
        ctx.beginPath(); ctx.arc(x, y, 14, 0, Math.PI * 2); ctx.fill();
      } else {
        // Ha llegado: mancha que crece (la "salpicadura")
        const tsSplash = tSinceLaunch - p.flightTime;
        const ts = Math.min(1, tsSplash / p.splashDuration);
        const r = easeOutCubic(ts) * p.splashRadius;
        if (r > 0) {
          const crestW = Math.min(40, r * 0.5);
          const grad = ctx.createRadialGradient(p.targetX, p.targetY, 0, p.targetX, p.targetY, r);
          grad.addColorStop(0, `rgba(${baseColor},1)`);
          grad.addColorStop(Math.max(0, 1 - crestW / r), `rgba(${baseColor},1)`);
          grad.addColorStop(Math.max(0, 1 - crestW / r / 2), `rgba(${crestColor},0.6)`);
          grad.addColorStop(1, `rgba(${baseColor},1)`);
          ctx.fillStyle = grad;
          ctx.beginPath(); ctx.arc(p.targetX, p.targetY, r, 0, Math.PI * 2); ctx.fill();
        }
      }
    });

    // Red de seguridad: en el último tramo se funde todo a una capa sólida
    // del tema nuevo, para que no quede ningún hueco sin cubrir entre
    // salpicaduras aunque la mala suerte deje algún resquicio.
    const SAFETY_START = 0.8;
    const t = elapsed / DURATION;
    if (t > SAFETY_START) {
      const safetyT = Math.min(1, (t - SAFETY_START) / (1 - SAFETY_START));
      ctx.fillStyle = `rgba(${baseColor},${safetyT})`;
