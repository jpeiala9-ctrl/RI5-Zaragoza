// ==================== app.js - VERSIÓN COMPLETA CORREGIDA (CON ADMIN Y MENSAJES FIX) ====================
...
const Storage = {
  ...
  // ===== FUNCIONES DE MENSAJERÍA CORREGIDAS =====
  async enviarMensajeUsuario(usuario, texto) {
    try {
      ...
      // Garantizar que mensajes es un array
      let mensajes = [];
      if (doc.exists) {
        const data = doc.data();
...
// ==================== MÓDULO ADMIN (CORREGIDO Y OPTIMIZADO) ====================
const Admin = {
  usersPagination: {
    lastDoc: null,
    hasMore: true,
  ...
  // ----- NUEVAS FUNCIONES PARA GESTIONAR PREMIUM -----
  abrirModalPremium(uid, username, esPremium, expires) {
    ...
    // Si hay fecha de expiración, la formateamos para input date (YYYY-MM-DD)
    if (expires && expires !== '-') {
  ...
  async guardarPremium() {
    ...
    // Si no hay fecha, establecer un mes por defecto
    if (!expiry) {
      const d = new Date();
      d.setMonth(d.getMonth() + 1);
      expiry = d.toISOString();
    } else {
      // Añadir hora para que sea ISO completo
    ...
    try {
      ...
      this.cargarUsuarios(true); // Recargar lista para reflejar cambios
    } catch (error) {
