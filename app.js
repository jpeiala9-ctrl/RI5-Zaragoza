...
const Admin = {
  ...
  async cargarMensajesUsuarios(forzarRecarga = false) {
    ...
    try {
      ...
      // 🔥 v4.56: primera tanda paginada (antes: colección entera sin
      // límite) -- ver Storage.getMensajesSoporteAdmin.
      const { mensajes, lastDoc, hayMas } = await Storage.getMensajesSoporteAdmin(150);
      ...
      const usuarios = await this._agruparMensajesSoportePorUsuario(mensajes, adminUid);
  ...
  // 🔥 v4.56: agrupación por usuario extraída a método aparte para
  // reutilizarla tanto en la carga inicial como en "cargar más antiguos"
  // (cargarMasMensajesSoporte), sin duplicar la lógica. `usuariosMapPrevio`
  // permite seguir mezclando sobre el Map ya construido en vez de empezar
  // de cero. Los datos de usuario (nombre/foto) que aún no se conocieran
  // se piden EN PARALELO (antes: uno a uno, en serie, con un await dentro
  // del bucle) y solo para uids que no estén ya en
  // _cacheSoporteUsuariosPorUid -- si ya se consultó a alguien antes en
  // esta sesión de admin (aquí o en una tanda anterior), no se vuelve a
  // leer su documento.
  async _agruparMensajesSoportePorUsuario(mensajes, adminUid, usuariosMapPrevio = null) {
    const usuariosMap = usuariosMapPrevio || new Map();
    ...
    usuarios._map = usuariosMap; // se conserva para poder seguir mezclando en la siguiente tanda
    return usuarios;
  },

  // Pide la siguiente tanda de mensajes de soporte más antiguos y los
  // mezcla con las conversaciones ya en pantalla (mismo patrón "cargar
  // más" que ya usa el Muro).
  async cargarMasMensajesSoporte() {
    if (!this._soporteHayMas || !this._soporteLastDoc) return;
    const btn = document.getElementById('cargarMasSoporteBtn');
    if (btn) { btn.disabled = true; btn.textContent = 'CARGANDO...'; }
    try {
      const adminUid = AppState.currentUserId;
      const { mensajes, lastDoc, hayMas } = await Storage.getMensajesSoporteAdmin(150, this._soporteLastDoc);
      this._soporteLastDoc = lastDoc;
      this._soporteHayMas = hayMas;
      const mapPrevio = this._cacheSoporteUsuarios?._map || new Map();
      const usuarios = await this._agruparMensajesSoportePorUsuario(mensajes, adminUid, mapPrevio);
      this._cacheSoporteUsuarios = usuarios;
      const container = document.getElementById('adminMessagesList');
      ...
      if (btn) { btn.disabled = false; btn.textContent = 'CARGAR CONVERSACIONES MÁS ANTIGUAS'; }
    }
  },
  ...
  _renderizarListaSoporte(usuarios, container) {
    ...
    if (this._soporteHayMas) {
      container.innerHTML += `<button id="cargarMasSoporteBtn" class="action-button" style="margin-top:8px; background:transparent; border:1px solid var(--border-color);" onclick="Admin.cargarMasMensajesSoporte()">CARGAR CONVERSACIONES MÁS ANTIGUAS</button>`;
    }
  ...
  // 🔥 v4.56: "ENVIAR A: TODOS / ELEGIDOS" -- _destinatariosSeleccionados
  // guarda los uids marcados en el selector (Admin.abrirSelectorDestinatarios).
  _destinatariosSeleccionados: new Set(),

  _actualizarDestinoBroadcast() {
    const elegidos = document.getElementById('broadcastDestinoElegidos')?.checked;
    const elegirBtn = document.getElementById('broadcastElegirBtn');
    const enviarBtn = document.getElementById('broadcastEnviarBtn');
    const countSpan = document.getElementById('broadcastCountElegidos');
    if (elegirBtn) elegirBtn.style.display = elegidos ? 'block' : 'none';
    if (countSpan) countSpan.textContent = this._destinatariosSeleccionados.size;
    if (enviarBtn) {
      enviarBtn.textContent = elegidos
        ? `📢 ENVIAR (${this._destinatariosSeleccionados.size})`
        : '📢 ENVIAR A TODOS';
    }
  },

  // Reutiliza la misma caché de usuarios que ya usa "Generar sesión para
  // usuarios" (SessionInvites._usuariosTodos/_precargarUsuarios): si esa
  // pantalla ya se abrió antes en esta sesión de admin, aquí no hace falta
  // volver a leer la colección 'users' entera otra vez.
  async abrirSelectorDestinatarios() {
    ...
    modal.style.cssText = `background:var(--bg-secondary); border-radius:20px; width:92%; max-width:500px; max-height:80vh; display:flex; flex-direction:column; overflow:hidden; box-shadow:0 20px 40px rgba(0,0,0,0.5); border:1px solid var(--border-color); font-family:'Courier New',monospace; opacity:0; transition:opacity 0.2s ease;`;
    modal.innerHTML = `
      <div style="padding:16px 20px; background:var(--bg-primary); border-bottom:1px solid var(--border-color); text-align:center;">
        <span style="font-size:15px; font-weight:bold; letter-spacing:1px; color:var(--text-primary);">👥 ELEGIR DESTINATARIOS</span>
      </div>
      <div style="padding:16px 20px; overflow-y:auto; flex:1;">
        <input id="broadcastDestBuscar" placeholder="> BUSCAR USUARIO_" style="width:100%; margin-bottom:12px; text-align:center;">
        <div id="broadcastDestList"><div style="text-align:center; padding:30px; color:var(--text-secondary);">⏳ Cargando...</div></div>
      </div>
      <div style="padding:16px 20px; background:var(--bg-primary); border-top:1px solid var(--border-color); display:flex; justify-content:space-between; align-items:center; gap:12px;">
        <button onclick="Admin._cerrarSelectorDestinatarios()" class="action-button" style="width:auto; padding:0 24px; margin:0; background:transparent;">LISTO</button>
        <span id="broadcastDestContador" style="font-size:12px; color:var(--gold);">0 elegidos</span>
      </div>
    `;
    overlay.appendChild(modal);
    document.body.appendChild(overlay);
    ...
    await SessionInvites._precargarUsuarios();
    const usuarios = SessionInvites._usuariosTodos || [];
    this._renderizarListaDestinatarios(usuarios);

    document.getElementById('broadcastDestBuscar')?.addEventListener('input', (e) => {
      const term = e.target.value.trim().toLowerCase();
      const filtrados = !term ? usuarios : usuarios.filter(u =>
  ...
  _renderizarListaDestinatarios(usuarios) {
    ...
    container.innerHTML = usuarios.map(u => {
      ...
      const nombre = Utils.escapeHTML(Utils.capitalizeUsername ? Utils.capitalizeUsername(u.username) : (u.username || '?'));
      return `<div class="broadcast-dest-item" data-uid="${u.uid}" style="display:flex; align-items:center; gap:10px; padding:10px 8px; border-radius:10px; cursor:pointer; background:${marcado ? 'rgba(192,160,96,0.14)' : 'transparent'};">
        <input type="checkbox" ${marcado ? 'checked' : ''} style="pointer-events:none; width:18px; height:18px; flex-shrink:0;">
        <span style="font-size:14px; color:var(--text-primary);">${nombre}</span>
      </div>`;
    }).join('');
    container.querySelectorAll('.broadcast-dest-item').forEach(el => {
      el.addEventListener('click', () => {
        const uid = el.dataset.uid;
    ...
    const contador = document.getElementById('broadcastDestContador');
    if (contador) {
      const n = this._destinatariosSeleccionados.size;
      contador.textContent = `${n} elegido${n === 1 ? '' : 's'}`;
    }
    this._actualizarDestinoBroadcast();
  },
  ...
  async enviarMensajeATodos() {
    ...
    const aElegidos = document.getElementById('broadcastDestinoElegidos')?.checked;
    let destinatarios = null;
    let mensajeConfirm;

    if (aElegidos) {
      destinatarios = Array.from(this._destinatariosSeleccionados);
      if (!destinatarios.length) { Utils.showToast('Elige al menos un destinatario', 'warning'); return; }
      mensajeConfirm = `¿Enviar este mensaje a ${destinatarios.length} usuario(s) elegido(s)?`;
    } else {
      mensajeConfirm = '¿Enviar este mensaje a TODOS los usuarios? (puede tardar unos segundos)';
    }
