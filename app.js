// ==================== app.js - VERSIÓN DEFINITIVA CON PERSISTENCIA DE CÁLCULO ====================
// Versión: 3.0 - Corregida: al recargar la página se muestra el último cálculo pase lo que pase
...
// ===== RESTAURAR SESIÓN Y ÚLTIMO CÁLCULO (VERSIÓN ROBUSTA) =====
window.addEventListener('load', async function () {
  ...
  try {
    ...
    // === RESTAURAR ÚLTIMO CÁLCULO Y MOSTRARLO SÍ O SÍ ===
    const calc = await Storage.getUltimoCalculo(savedUid);
    if (calc) {
      AppState.setLastCalc(calc);

      // Forzar que la pestaña de entrenamiento esté activa
      if (window.UI) {
        await UI.switchTab('entreno');
      }

      // Esperar un poco más a que la pestaña se active y el DOM se actualice
      setTimeout(() => {
        if (window.Training && typeof Training.mostrarResultados === 'function') {
          Training.mostrarResultados(calc);
          console.log('✅ Cálculo restaurado y mostrado');
        } else {
          console.error('❌ Training no disponible');
        }
      }, 300); // Aumentamos el tiempo para asegurar
    }
