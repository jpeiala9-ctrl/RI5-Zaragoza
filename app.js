...
const Admin = {
  ...
  async verPerfil(uid) {
    ...
    try {
      ...
      if (!overlay || !modal || !msgEl) {
        Utils.showToast(
          `${Utils.capitalizeUsername(user.username)} · ${user.email} · Premium: ${user.premium ? 'SÍ' : 'NO'}`,
          'info', 6000
        );
        return;
      }
      ...
      msgEl.textContent   = [
        `Email: ${user.email}`,
        `Premium: ${user.premium ? 'SÍ' : 'NO'} (expira ${expira})`,
        `Cálculos este mes: ${user.calculosMes || 0}`,
        `Registrado: ${registro}`,
  ...
  abrirMensajeUsuario(uid, username) {
    ...
    if (!modal) {
      ...
      modal.innerHTML = `
        <div style="background:var(--bg-primary,#1a1a1a);border:1px solid var(--border-color,#333);border-radius:12px;padding:24px;width:min(420px,90vw);display:flex;flex-direction:column;gap:16px;">
          <div id="adminMsgTitle" style="font-size:16px;font-weight:500;color:var(--text-primary,#fff);"></div>
          <textarea id="adminMsgInput" rows="4" placeholder="Escribe tu mensaje aquí..." style="background:var(--bg-secondary,#2a2a2a);border:1px solid var(--border-color,#444);border-radius:8px;color:var(--text-primary,#fff);font-size:14px;padding:10px 12px;resize:vertical;width:100%;box-sizing:border-box;"></textarea>
          <div style="display:flex;gap:10px;justify-content:flex-end;">
            <button id="adminMsgCancelBtn" style="padding:8px 18px;border-radius:8px;border:1px solid var(--border-color,#444);background:transparent;color:var(--text-primary,#fff);cursor:pointer;font-size:14px;">Cancelar</button>
            <button id="adminMsgSendBtn" style="padding:8px 18px;border-radius:8px;border:none;background:var(--accent,#7c3aed);color:#fff;cursor:pointer;font-size:14px;font-weight:500;">Enviar</button>
          </div>
        </div>
      `;
