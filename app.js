...
// VERSIÓN: 4.19 - Historial reparado y mejoras de estabilidad
// ====================
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (this.currentUser) {
      ...
      if(welcome) {
        ...
        welcome.innerHTML = `> BIENVENIDO, <strong style="color:var(--gold)">${this.currentUser.toUpperCase()}</strong>_ ${isPremiumActive ? '<span class="premium-badge-header">✨ PRO</span>' : ''}`;
      }
    }
...
const UI = {
  ...
  async switchTab(tab) {
    // Sync bottom navigation bar (si existe)
    const BNAV = { entreno:'bnav-entreno', plan:'bnav-plan', muro:'bnav-muro', amigos:'bnav-amigos', perfil:'bnav-perfil' };
    if (BNAV[tab]) {
  ...
  async cargarHistorialCompleto(reset = false) {
    ...
    if (reset) { container.innerHTML = '<div style="text-align:center; padding:20px;">⏳ Cargando historial…</div>'; AppState.historialPagination.lastDoc = null; AppState.historialPagination.hasMore = true; }
    try {
      ...
      if (result.items.length === 0) { 
        if (reset) container.innerHTML = '<p style="text-align:center; padding:20px;">No hay cálculos guardados. Calcula tus zonas primero.</p>'; 
      ...
      container.innerHTML = html;
    } catch (error) { console.error('Error cargando historial:', error); if (reset) container.innerHTML = '<p style="text-align:center; padding:20px;">Error al cargar. Intenta recargar la página.</p>'; }
