// ==================== app.js - VERSIÓN COMPLETA (PERSISTENCIA TOTAL DEL PLAN ABIERTO) ====================
// VERSIÓN: 4.21 - Usa localStorage para recordar el plan abierto incluso después de recargar o cerrar sesión
// ====================
...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (uid) {
      ...
      this.precargarDatos();
      // Restaurar el estado guardado (incluyendo el plan abierto)
...
const UI = {
  ...
  guardarEstado() {
    ...
    const estado = {
      ...
      sesionesRealizadas: AppState.sesionesRealizadas,
      sesionesForzadas: AppState.sesionesForzadas
    };
    // Guardar en localStorage para que persista entre recargas y cierres de navegador
    localStorage.setItem('ri5_estado', JSON.stringify(estado));
  },

  async restaurarEstado() {
    const estadoStr = localStorage.getItem('ri5_estado');
    if (!estadoStr) return;
    try {
      ...
      if (estado.planVisible && estado.planId && AppState && AppState.currentUserId) {
        if (AppState) {
          ...
          if (estado.sesionesRealizadas) AppState.sesionesRealizadas = estado.sesionesRealizadas;
          if (estado.sesionesForzadas) AppState.sesionesForzadas = estado.sesionesForzadas;
        }
        await this.cargarPlanDesdeHistorial(estado.planId);
        // Forzar la visibilidad del calendario
        const calendario = document.getElementById("calendarioEntreno");
        const cuestionario = document.getElementById("cuestionarioEntreno");
