// ==================== app.js - VERSIÓN REFACTORIZADA (DUPLICIDADES ELIMINADAS) ====================
// VERSIÓN: 4.0 - Consejos unificados, mensajes genéricos, administración modular
// ====================
...
const Utils = {
  ...
  vibrate(pattern) { if (window.navigator?.vibrate) window.navigator.vibrate(pattern); },
...
const AppState = {
  ...
  clearLastCalc() { 
    ...
    this.lastName = ""; this.lastAge = 0; this.lastFC = 0; this.lastUL = 0; 
    this.lastZones = []; this.lastPred = []; this.lastRitmoBase = 0; 
  },
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (userData && !userData.username_lowercase && userData.username) {
      ...
      firebaseServices.db.collection('users').doc(uid).update({ username_lowercase: newLowercase }).catch(e => console.warn('Error actualizando username_lowercase:', e));
  ...
  actualizarInterfazPremium() {
    ...
    if(counterDiv) {
      ...
      counterDiv.innerHTML = `📊 Cálculos este mes: ${this.calculosMes}`;
    }
  },
  ...
  async incrementarCalculo() {
    ...
    const limiteMensual = this.isPremium ? Infinity : 10;
    const ahora = new Date();
    ...
    if (this.calculosMes >= limiteMensual) {
      Utils.showToast('Has alcanzado el límite mensual de cálculos. Hazte premium para calcular más.', 'warning');
      return false;
    }
    ...
    } catch (error) { console.error('Error updating calculos:', error); }
...
const Admin = {
  ...
  async borrarMensajeUsuario(uid, idx) {
    ...
    } catch (error) { console.error('Error borrando mensaje:', error); Utils.showToast('Error al eliminar mensaje: ' + error.message, 'error'); }
  ...
  async cargarUsuarios(reset = false) {
          ...
          <div class="usuario-item ${esNuevo ? 'nuevo' : ''}" data-uid="${uid}" onclick="Admin.toggleUsuario(this, '${uid}')">
            ...
            <div class="usuario-detalle">
              <div class="usuario-info">
                <div class="info-item"><span class="info-label">Premium</span><span class="info-value">${premium}</span></div>
                <div class="info-item"><span class="info-label">Expira</span><span class="info-value">${expires}</span></div>
                <div class="info-item"><span class="info-label">Cálculos</span><span class="info-value">${calculos}</span></div>
                <div class="info-item"><span class="info-label">Registro</span><span class="info-value">${created}</span></div>
              </div>
    ...
    } catch (error) { console.error('Error cargando usuarios:', error); if (reset) container.innerHTML = '<p style="text-align:center; padding:40px; color: var(--zone-5);">Error al cargar</p>'; }
  ...
  async enviarMensajeUsuario(uid, texto) {
    ...
    Utils.showLoading();
    try { await Storage.enviarMensajeAdminAUsuario(uid, texto); Utils.showToast(`✅ Mensaje enviado a usuario`, 'success'); }
    ...
    try {
      ...
      for (const doc of snapshot.docs) {
        ...
        mensajes.forEach((msg, idx) => {
          ...
          allMessages.push({ ...msg, fromUid: uid, username, mensajeIdx: idx, mensajeId, docId: doc.id, esNuevo: !msg.leido && !mensajesVistos.includes(mensajeId) });
        });
      }
      ...
      allMessages.forEach(msg => {
        ...
        html += `
          <div class="mensaje-item ${nuevoClass}" data-mensaje-id="${msg.mensajeId}" onclick="Admin.toggleMensaje(this, '${msg.mensajeId}')">
            <div class="mensaje-header"><span class="mensaje-fecha">📨 ${msg.fecha}</span><span class="mensaje-remitente">${msg.username}</span>${msg.esNuevo ? '<span class="nuevo-badge">NUEVO</span>' : ''}</div>
            <div class="mensaje-contenido"><p>${msg.texto}</p></div>
            <div class="mensaje-botones">
              <button class="responder" onclick="event.stopPropagation(); Admin.responderMensaje('${msg.fromUid}', '${msg.username}')">✉️ RESPONDER</button>
              <button class="eliminar" onclick="event.stopPropagation(); Admin.borrarMensajeUsuario('${msg.fromUid}', ${msg.mensajeIdx})">🗑️ ELIMINAR</button>
            </div>
          </div>
        `;
      });
...
const UI = {
  ...
  validarCampo(c) {
    ...
    if(!ok) { err.innerText = c === 'age' ? 'Edad 14-85' : 'Formato MM:SS (12-90 min)'; err.classList.add('visible'); el.classList.add('error'); }
    ...
    try {
      ...
      if(tab === 'amigos') {
        if (window.Friends) {
          ...
          AppState.solicitudesPendientesCount = 0;
          AppState.actualizarBadgeSolicitudes();
          if (window.Feed && window.Feed.marcarTodoLeido) await window.Feed.marcarTodoLeido();
          if (!Friends.todosUsuariosPagination || !Friends.todosUsuariosPagination.lastDoc) await Friends.cargarTodosUsuarios(true);
          else {
            if (document.getElementById('listaSolicitudesRecibidas').style.display !== 'none') await Friends.cargarSolicitudesRecibidas();
            if (document.getElementById('listaSolicitudesEnviadas').style.display !== 'none') await Friends.cargarSolicitudesEnviadas();
            await Friends.cargarListaAmigos();
          }
          if (window.Feed) await Feed.cargarFeed(false);
        }
      }
      if(tab === 'admin' && AppState && AppState.isAdmin) { Admin.cargarUsuarios(true); Admin.cargarMensajesUsuarios(); }
    ...
    mensajes.forEach((msg, i) => {
      ...
      html += `
        <div class="mensaje-item ${nuevoClass}" data-mensaje-id="${mensajeId}" data-msg-index="${i}" data-tipo="${tipo}">
          ...
          <div class="mensaje-botones">
            ${onToggleCallback ? `<button class="responder" onclick="event.stopPropagation(); ${onToggleCallback}">✉️ RESPONDER</button>` : ''}
            <button class="eliminar" onclick="event.stopPropagation(); UI.borrarMensajeUsuario(${i})">🗑️ ELIMINAR</button>
          </div>
        </div>
      `;
    });
  ...
  guardarEstado() {
    ...
    const calendario = document.getElementById('calendarioEntreno');
    const estado = { activeTab, planVisible: calendario ? calendario.style.display === 'block' : false, planId: AppState.planActualId, trimestre: AppState.trimestreActual };
...
const ResetPassword = {
  abrirModal() { const overlay = document.getElementById('resetOverlay'); const modal = document.getElementById('resetModal'); if (overlay) overlay.style.display = 'block'; if (modal) modal.style.display = 'block'; const email = document.getElementById('resetEmail'); if (email) email.value = ''; const error = document.getElementById('resetError'); if (error) error.classList.remove('visible'); },
  cerrarModal() { const overlay = document.getElementById('resetOverlay'); const modal = document.getElementById('resetModal'); if (overlay) overlay.style.display = 'none'; if (modal) modal.style.display = 'none'; },
  async enviarEmailRecuperacion() {
    ...
    Utils.showLoading();
    try { await firebaseServices.auth.sendPasswordResetEmail(email); Utils.hideLoading(); this.cerrarModal(); Utils.showToast('📧 Revisa tu correo para restablecer la contraseña', 'success'); }
    catch (error) { Utils.hideLoading(); if (error.code === 'auth/user-not-found') { Utils.showToast('Si el correo existe, recibirás instrucciones', 'info'); this.cerrarModal(); } else Utils.handleFirebaseError(error); }
  }
};
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if(!localStorage.getItem('ri5_info_visto')) setTimeout(() => { const banner = document.getElementById('infoBanner'); if (banner) banner.style.display = 'block'; }, 1000);
  if (!window.firebaseServices) { console.error('❌ Firebase no está configurado'); Utils.showToast('Error de configuración', 'error'); return; }
  
  const savedUid = localStorage.getItem('ri5_current_user');
  if (savedUid) {
    Utils.showLoading();
    try {
      await new Promise((resolve) => { const unsubscribe = firebaseServices.auth.onAuthStateChanged((user) => { unsubscribe(); resolve(user); }); });
      ...
      if (currentUser && currentUser.uid === savedUid) {
        ...
        if (userDoc.exists) {
          ...
          if (!userData.username_lowercase && userData.username) { userData.username_lowercase = userData.username.toLowerCase(); firebaseServices.db.collection('users').doc(savedUid).update({ username_lowercase: userData.username_lowercase }).catch(e => console.warn('Error corrigiendo username_lowercase:', e)); }
          await AppState.setCurrentUser(savedUid, userData.email, userData);
          document.getElementById("loginPage").style.display = "none";
          document.getElementById("mainContent").style.display = "flex";
          const welcomeEl = document.getElementById("userWelcome");
          if (welcomeEl) { const expiry = new Date(userData.expires); welcomeEl.innerText = `> BIENVENIDO, ${Utils.capitalizeUsername(userData.username).toUpperCase()} · ${userData.premium ? 'PREMIUM' : 'ACCESO'} HASTA ${expiry.toLocaleDateString()}`; }
          const nameField = document.getElementById('name'); if(nameField) nameField.value = Utils.capitalizeUsername(userData.username);
          if (window.UI) { UI.updateTip('dailyTip'); UI.startConsejoAutoChange(); }
          const msgs = await Storage.getMensajesUsuario(savedUid); AppState.mensajesNoLeidos = msgs.filter(m => !m.leido && m.esAdmin).length; UI.actualizarBadgeMensajes();
          const calc = await Storage.getUltimoCalculo(savedUid); if(calc) { AppState.setLastCalc(calc); if (window.Training && Training.mostrarResultadosGuardados) Training.mostrarResultadosGuardados(calc); }
          const estadoStr = sessionStorage.getItem('ri5_estado');
          if (estadoStr) { try { const estado = JSON.parse(estadoStr); if (estado.activeTab) { const tabMap = { 'entreno': 'entreno', 'plan': 'plan', 'historial': 'historial', 'soporte': 'soporte', 'perfil': 'perfil', 'amigos': 'amigos', 'admin': 'admin' }; const tab = tabMap[estado.activeTab] || 'entreno'; await UI.switchTab(tab); } } catch (e) { console.warn('Error restaurando estado de pestaña', e); } }
          console.log('✅ Sesión restaurada correctamente');
        } else { localStorage.removeItem('ri5_current_user'); localStorage.removeItem('ri5_user_email'); localStorage.removeItem('ri5_is_admin'); }
      } else { localStorage.removeItem('ri5_current_user'); localStorage.removeItem('ri5_user_email'); localStorage.removeItem('ri5_is_admin'); }
    } catch (error) { console.error('❌ Error restaurando sesión:', error); localStorage.removeItem('ri5_current_user'); }
    finally { Utils.hideLoading(); }
  }
  if (!localStorage.getItem('ri5_current_user')) { document.getElementById("loginPage").style.display = "flex"; document.getElementById("mainContent").style.display = "none"; }
  
  const ageInput = document.getElementById("age");
  const timeInput = document.getElementById("time");
  if (ageInput) { ageInput.addEventListener("blur", () => UI.marcarCampoTocado('age')); ageInput.addEventListener("input", () => { if(AppState && AppState.camposTocados.age) UI.validarCampo('age'); UI.validarTodo(); }); }
  if (timeInput) { timeInput.addEventListener("blur", () => UI.marcarCampoTocado('time')); timeInput.addEventListener("input", () => { if(AppState && AppState.camposTocados.time) UI.validarCampo('time'); UI.validarTodo(); }); }
  UI.validarTodo();
  UI.initDiasCheckboxes();

  if(localStorage.getItem('pwa_installed') === 'true' || localStorage.getItem('pwa_banner_closed') === 'true') { const pwaBanner = document.getElementById('pwa-banner'); if (pwaBanner) pwaBanner.style.display = 'none'; }
  PWA.init(); 
  PWA.registerServiceWorker();
  setTimeout(() => { if(AppState && AppState.deferredPrompt && localStorage.getItem('pwa_installed') !== 'true' && localStorage.getItem('pwa_banner_closed') !== 'true') { const pwaBanner = document.getElementById('pwa-banner'); if (pwaBanner) pwaBanner.style.display = 'flex'; } }, 3000);
