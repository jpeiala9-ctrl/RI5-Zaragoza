// ==================== app.js - VERSIÓN COMPLETA CORREGIDA (CON ADMIN, ELIMINAR USUARIO Y PERSISTENCIA) ====================
      ...
      snapshot.docs.forEach(doc => {
        ...
        html += `
          <div class="historial-item" style="cursor:default;" onclick="event.stopPropagation()">
            ...
            <div style="display: flex; gap: var(--space-xs); margin-top: var(--space-sm); justify-content: center; flex-wrap: wrap;">
              ...
              <button class="action-button" style="width:auto; padding: var(--space-xs) var(--space-sm);" onclick="Admin.abrirModalPremium('${uid}', '${username}', ${user.premium}, '${user.expires || ''}')">⭐ PREMIUM</button>
              <button class="action-button" style="width:auto; padding: var(--space-xs) var(--space-sm); background: var(--zone-5); border-color: var(--zone-5);" onclick="Admin.eliminarUsuario('${uid}', '${username}')">🗑️ ELIMINAR</button>
            </div>
          </div>
        `;
      });
        ...
        html += `
          <div class="mensaje-item ${leido}" data-uid="${msg.fromUid}" data-idx="${msg.mensajeIdx}">
            <div class="mensaje-header" onclick="this.parentNode.classList.toggle('abierto')">
              <span style="text-align: center; flex: 1;">📨 ${msg.fecha} · ${msg.username}</span>
              <span class="flecha" style="margin: 0 5px;">▼</span>
            </div>
            <div class="mensaje-contenido" style="text-align: center;">${msg.texto}</div>
            <div style="display: flex; gap: 10px; justify-content: center; padding: 10px; border-top: 1px solid var(--border-color); margin-top: 5px;">
              <button class="delete-mensaje" style="min-width: 100px; justify-content: center;" onclick="event.stopPropagation(); Admin.responderMensaje('${msg.fromUid}', '${msg.username}')">✉️ RESPONDER</button>
              <button class="delete-mensaje" style="min-width: 100px; justify-content: center; background: var(--zone-5); border-color: var(--zone-5);" onclick="event.stopPropagation(); Admin.borrarMensajeUsuario('${msg.fromUid}', ${msg.mensajeIdx})">🗑️ ELIMINAR</button>
            </div>
          </div>
        `;
      });
