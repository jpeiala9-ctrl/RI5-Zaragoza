// ==================== app.js - VERSIÓN COMPLETA CON NUEVA LÓGICA PREMIUM Y SOCIAL ====================
// VERSIÓN: 4.3 - parseTime mejorado: acepta solo minutos (ej. 27) y formato MM:SS
// ====================
...
```
// Si no contiene ':', asumimos que son solo minutos (entero o decimal)
if (!t.includes(':')) {
  let minutos = parseFloat(t);
  if (isNaN(minutos)) return NaN;
  // Permitir valores entre 10 y 120 minutos (tiempo razonable para 6km)
...
// Badge unificado para la pestaña Amigos — combina feed, solicitudes y chat
_actualizarTabAmigos() {
const tab = document.querySelector(’.tab-button[onclick=“switchTab('amigos')”]’);
if (!tab) return;
const total = (this.nuevasActividadesAmigos || 0)
+ (this.solicitudesPendientesCount || 0)
+ (this.mensajesAmigosNoLeidos || 0);
if (total > 0) {
tab.classList.add(‘amigos-tiene-novedades’);
tab.setAttribute(‘data-amigos-count’, total);
} else {
tab.classList.remove(‘amigos-tiene-novedades’);
tab.removeAttribute(‘data-amigos-count’);
}
},

actualizarBadgeAmigos()      { this._actualizarTabAmigos(); },
actualizarBadgeSolicitudes() { this._actualizarTabAmigos(); },
actualizarBadgeChat()        { this._actualizarTabAmigos(); }
};
...
try {
  ...
  // Al paginar (reset=false) acumulamos; al buscar/reset reemplazamos
  if (reset) {
    container.innerHTML = html || '<p style="text-align:center; padding:40px; color: var(--text-secondary);">No se encontraron usuarios</p>';
  } else {
  ...
  const loadMoreBtn = document.getElementById('loadMoreUsersBtn');
  if (loadMoreBtn) {
    // Solo mostramos "Cargar más" si hay más páginas Y la última tuvo resultados visibles
    loadMoreBtn.style.display = (this.usersPagination.hasMore && html) ? 'block' : 'none';
  }
...
Utils.showToast(`${Utils.capitalizeUsername(user.username)} · ${user.email} · Premium: ${user.premium ? 'SÍ' : 'NO'}`, ‘info’, 6000);
return;
}
const expira    = user.expires ? new Date(user.expires).toLocaleDateString() : ‘-’;
const registro  = user.created ? new Date(user.created).toLocaleString() : ‘-’;
const lastLogin = user.lastLogin ? new Date(user.lastLogin.toDate()).toLocaleString() : ‘-’;
titleEl.textContent = `Perfil: ${Utils.capitalizeUsername(user.username)}`;
msgEl.textContent   = [`Email: ${user.email}`, `Premium: ${user.premium ? 'SÍ' : 'NO'} (expira ${expira})`, `Cálculos este mes: ${user.calculosMes || 0}`, `Registrado: ${registro}`, `Último login: ${lastLogin}`, `Admin: ${user.isAdmin ? 'SÍ' : 'NO'}`].join(’\n’);
noBtn.style.display = ‘none’;
yesBtn.textContent  = ‘Cerrar’;
overlay.classList.add(‘active’);
modal.classList.add(‘active’);
const cerrar = () => {
overlay.classList.remove(‘active’);
...
noBtn.style.display = ‘’;
yesBtn.removeEventListener(‘click’, cerrar);
};
yesBtn.addEventListener(‘click’, cerrar);
} catch (error) {
...
let modal = document.getElementById(‘adminMsgModal’);
if (!modal) {
modal = document.createElement(‘div’);
modal.id = ‘adminMsgModal’;
modal.style.cssText = ‘display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:1200;align-items:center;justify-content:center;’;
modal.innerHTML = `<div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:12px;padding:24px;width:min(420px,90vw);display:flex;flex-direction:column;gap:16px;"><div id="adminMsgTitle" style="font-size:16px;font-weight:500;color:var(--text-primary);"></div><textarea id="adminMsgInput" rows="4" placeholder="Escribe tu mensaje aquí..." style="background:var(--bg-secondary);border:1px solid var(--border-color);border-radius:8px;color:var(--text-primary);font-size:14px;padding:10px 12px;resize:vertical;width:100%;box-sizing:border-box;"></textarea><div style="display:flex;gap:10px;justify-content:flex-end;"><button id="adminMsgCancelBtn" style="padding:8px 18px;border-radius:8px;border:1px solid var(--border-color);background:transparent;color:var(--text-primary);cursor:pointer;font-size:14px;height:auto;line-height:normal;">Cancelar</button><button id="adminMsgSendBtn" style="padding:8px 18px;border-radius:8px;border:none;background:var(--accent-blue);color:var(--bg-primary);cursor:pointer;font-size:14px;font-weight:500;height:auto;line-height:normal;">Enviar</button></div></div>`;
document.body.appendChild(modal);
}
document.getElementById(‘adminMsgTitle’).textContent = `Mensaje para ${username}`;
const input = document.getElementById(‘adminMsgInput’);
input.value = ‘’;
modal.style.display = ‘flex’;
setTimeout(() => input.focus(), 50);
const sendBtn = document.getElementById(‘adminMsgSendBtn’);
const cancelBtn = document.getElementById(‘adminMsgCancelBtn’);
...
if (!texto) { Utils.showToast(‘Escribe un mensaje antes de enviar’, ‘warning’); return; }
cerrarModal();
this.enviarMensajeUsuario(uid, texto);
});
...
async switchTab(tab) {
// Cancelar listener de chat al cambiar de pestaña para evitar fugas de memoria
if (window.Chat && Chat.unsubscribeMessages) {
Chat.unsubscribeMessages();
Chat.unsubscribeMessages = null;
}
...
try {
  ...
  if(tab === 'amigos') {
    if (window.Friends) {
      ...
      if (window.Feed && window.Feed.marcarTodoLeido) {
        await window.Feed.marcarTodoLeido();
      }
...
html += `<div class="mensaje-item ${nuevoClass}" data-mensaje-id="${mensajeId}" data-msg-index="${i}" data-tipo="${tipo}"> <div class="mensaje-header"> <span class="mensaje-fecha">${icono} ${fecha}</span> <span class="mensaje-remitente">${remitente}</span> ${esNuevo ? '<span class="nuevo-badge">NUEVO</span>' : ''} </div> <div class="mensaje-contenido"> <p>${msg.texto}</p> </div> <div class="mensaje-botones"> ${onToggleCallback ?`<button class="responder" onclick="event.stopPropagation(); ${onToggleCallback}">✉️ RESPONDER</button>`: ''} <button class="eliminar" onclick="event.stopPropagation(); UI.borrarMensajeUsuario(${i})">🗑️ ELIMINAR</button> </div> </div>`;
});
container.innerHTML = html;
...
html += `<div class="historial-item" onclick="toggleHistorialDetalle(this)"> <div class="fecha">📅 ${it.date || ''}</div> <div class="resumen">${it.resumen || it.nombre + ' · ' + it.edad + ' años'}</div> <button class="delete-icon" onclick="event.stopPropagation(); borrarEntradaHistorial('${it.id}')">🗑️</button> <div class="detalle">${hora}${pred}${zonas}${it.fcMax ? `<div>❤️ FC Máx: ${it.fcMax} lpm</div>`: ''}${it.umbral ?`<div>⚡ Umbral: ${it.umbral} lpm</div>` : ''}</div> </div>`;
});
...
html += `<div class="plan-card" data-plan-id="${plan.id}"> <div class="plan-info" onclick="cargarPlanDesdeHistorial('${plan.id}')"> <div class="plan-fecha">📅 ${fecha}</div> <div class="plan-resumen">${distancia} · ${params.diasPorSemana || '?'} días · ${params.nivel || ''}</div> </div> <button class="delete-plan" onclick="event.stopPropagation(); eliminarPlanHistorial('${plan.id}')">🗑️</button> </div>`;
});
container.innerHTML = html;
...
if (tab === ‘chat’ && window.Chat) Chat.renderConversaciones(document.getElementById(‘conversacionesContainer’));
};
...
const overlay = document.getElementById(‘infoBannerOverlay’);
