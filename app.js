...
const UI = {
  ...
  async eliminarConversacionSoportePropia() {
    ...
    const confirmado = await Utils.confirm('Eliminar conversación', '¿Eliminar por completo tu conversación de soporte? Esta acción no se puede deshacer.');
    if (!confirmado) return;

    Utils.showLoading('Eliminando');
    try {
      const ok = await Storage.eliminarConversacionSoporte(AppState.currentUserId);
      if (ok) {
        Utils.showToast('✅ Conversación eliminada', 'success');
