...
// VERSIÓN: 4.41 - Añadido listener en tiempo real de notificación de soporte para el ADMIN (aparece al llegar mensaje, se quita sola al leerla)
// ====================
...
const AppState = {
  ...
  iniciarListeners() {
    ...
    // FIX: notificación de soporte para el ADMIN. Antes no existía ningún
    // listener en tiempo real para los mensajes que llegan a soporte del
    // admin (colección 'soporteMensajes'), así que no aparecía badge de
    // aviso al admin. Al ser un onSnapshot, cuando Storage.marcarMensajeSoporteLeido
    // pone leido:true en el mensaje, este mismo listener recalcula el
    // contador automáticamente y el badge se quita solo -- por eso basta
    // con este único listener para arreglar tanto "no aparece" como
    // "no se quita al leerla".
    if (this.isAdmin) {
      this.unsubscribeMensajesSoporteAdmin = firebaseServices.db
        ...
        .onSnapshot((snapshot) => {
          ...
          // Si el admin tiene abierto el panel de soporte, refrescar la
          // lista en vivo para que el mensaje nuevo aparezca sin recargar.
...
const UI = {
  ...
  actualizarBadgeMensajes() {
    // Contador combinado: mensajes propios sin leer + (si es admin)
    // mensajes de soporte entrantes de usuarios sin leer.
