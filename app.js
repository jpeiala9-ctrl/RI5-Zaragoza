...
// VERSIÓN: 4.13 - Persistencia de sesión garantizada y listeners en tiempo real
// ====================
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (savedUid) {
    // ✅ CORRECCIÓN: Delegar la restauración de sesión a Auth.checkSavedSession
    // que verifica emailVerified y muestra la pantalla de verificación si es necesario.
    const sesionRestaurada = await Auth.checkSavedSession();
    if (!sesionRestaurada) {
      // Si la sesión no es válida (email no verificado, etc.), Auth.checkSavedSession ya muestra
      // la pantalla correspondiente (verificación o login). Solo nos aseguramos de ocultar el loading.
      Utils.hideLoading();
    } else {
      // Si la sesión es válida, la UI ya está configurada por AppState.setCurrentUser.
      // Solo queda ocultar el loading.
      Utils.hideLoading();
      
      // Cargar cálculos pendientes
