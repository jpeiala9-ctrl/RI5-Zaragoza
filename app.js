...
const AppState = {
  ...
  async precargarDatos() {
    ...
    if (window.Wall && Wall.precargarMuro) {
      await Wall.precargarMuro().catch(e => console.warn('Error precargando el muro:', e));
    }
  },
      ...
      .onSnapshot(async (doc) => {
        ...
        if (alumnosGanados.length || alumnosPerdidos.length) {
          ...
          if (entrenadorTabActiva && this.isTrainer && !this.isAdmin && window.Friends && Friends.cargarPanelAlumnos) {
            Friends.cargarPanelAlumnos(true); // cambio real (alumno aceptado/perdido): forzar refresco de la caché (ver friends.js v3.55)
          }
        }
    ...
    this.unsubscribeSolicitudesEntrenadorEnviadas = firebaseServices.db
      ...
      .onSnapshot((snapshot) => {
        snapshot.docChanges().forEach(async (change) => {
          ...
          const entrenadorTabActiva = document.getElementById('subtab-perfil-entrenador')?.classList.contains('active');
          if (entrenadorTabActiva && this.isTrainer && !this.isAdmin && window.Friends && Friends.cargarPanelAlumnos) {
            Friends.cargarPanelAlumnos(true); // cambio real (invitación rechazada): forzar refresco de la caché (ver friends.js v3.55)
          }
        });
      }, (error) => {
