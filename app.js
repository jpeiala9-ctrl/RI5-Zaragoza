...
// VERSIÓN: 4.55 - 2º intento del fix de la papelera de soporte: incluso
//                 borrando SOLO la copia local (v4.54), Firestore seguía
//                 dando error de permiso porque las reglas no dejan que
//                 un usuario borre mensajes que no escribió él mismo
//                 (los del admin), aunque estén en su propia
//                 subcolección. Como marcar mensajes como leídos
//                 (update) sí funciona siempre, ahora en vez de borrar
//                 se OCULTAN (Storage.eliminarConversacionSoportePropia-
//                 Local hace update ocultoParaUsuario:true + leido:true
//                 en vez de delete; Storage.getMensajesSoporteUsuario
//                 filtra esos mensajes). Resultado igual para el
//                 usuario -- tarjeta y chat vacíos -- sin tocar ningún
//                 permiso, y el admin conserva su copia intacta.
// ====================
...
const UI = {
  ...
  // mensaje centrado en la parte baja del bloque central, fecha/aviso/
  // papelera a la derecha, tarjeta entera pintada del color de
  // notificación si hay mensajes no leídos) -- aquí para una sola
  // tarjeta fija: la conversación con el administrador.
  _renderizarTarjetaSoporteUsuario(datos, container) {
    ...
    const papelera = datos.hayConversacion
      ? `<button type="button" id="borrarConversacionSoportePropia" title="Eliminar conversación" style="grid-column:3;grid-row:3;justify-self:end;align-self:end;width:28px;height:28px;flex-shrink:0;border:none;background:transparent;color:${iconoPapeleraColor};padding:0;cursor:pointer;display:flex;align-items:center;justify-content:center;">${iconoPapelera}</button>`
      : '';
    ...
    const btnPapelera = document.getElementById('borrarConversacionSoportePropia');
    if (btnPapelera) {
      btnPapelera.addEventListener('click', (e) => {
        e.stopPropagation();
        this.eliminarConversacionSoportePropia();
      });
    }
  },

  async eliminarConversacionSoportePropia() {
    if (!AppState.currentUserId) return;
    const confirmado = await Utils.confirm('Eliminar conversación', '¿Eliminar esta conversación de tu bandeja? Seguirá disponible para soporte; si quieres puedes volver a escribirnos cuando quieras.');
    if (!confirmado) return;

    Utils.showLoading('Eliminando');
    try {
      // Solo borra la copia local del usuario (su subcolección privada).
      // No toca la colección global de soporte: un usuario normal no
      // tiene permisos para borrar de ahí (eso le corresponde al admin),
      // e intentarlo daba error de permiso.
      const ok = await Storage.eliminarConversacionSoportePropiaLocal(AppState.currentUserId);
      if (ok) {
        Utils.showToast('✅ Conversación eliminada de tu bandeja', 'success');
        await this.cargarSoporteUsuario();
      } else {
        Utils.showToast('Error al eliminar la conversación', 'error');
      }
    } catch (e) {
      console.error('Error eliminando conversación de soporte propia:', e);
      Utils.showToast('Error al eliminar la conversación', 'error');
    } finally {
      await Utils.hideLoading();
    }
