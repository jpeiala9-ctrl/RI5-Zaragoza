...
async function cargarUsuarios() {
  ...
  try {
    ...
    Utils.showToast('Error al cargar usuarios: ' + error.message, 'error');
  } finally {
    Utils.hideLoading();
  }
}
...
function renderConversacionAdmin() {
...
async function enviarMensajeAdminChat() {
  ...
  try {
    const docRef = firebaseServices.db.collection('mensajes').doc(AppState.usuarioSeleccionado.uid);
    const doc = await docRef.get();
    const mensajes = doc.exists ? doc.data().mensajes || [] : [];
    mensajes.push({
      fecha: new Date().toLocaleString(),
