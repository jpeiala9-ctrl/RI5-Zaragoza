// ==================== app.js - VERSIÓN COMPLETA CON PERSISTENCIA DE CÁLCULO ====================
// Versión: 2.1 - Corregida: al recargar la página se muestra el último cálculo en la pestaña de entrenamiento
...
// ===== RESTAURAR SESIÓN Y ÚLTIMO CÁLCULO (al cargar la página) =====
window.addEventListener('load', async function() {
  ...
  try {
    ...
    // === RESTAURAR ÚLTIMO CÁLCULO Y MOSTRARLO EN LA PESTAÑA DE ENTRENO ===
    const calc = await Storage.getUltimoCalculo(savedUid);
    if (calc) {
      AppState.setLastCalc(calc);

      // Asegurar que la pestaña de entrenamiento esté activa
      if (window.UI) {
        await UI.switchTab('entreno');
      }

      // Pequeño retraso para que el DOM de la pestaña se actualice
      setTimeout(() => {
        if (window.Training) {
          Training.mostrarResultados(calc);
        }
      }, 100);
    }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Inicializaciones locales (inputs, checkboxes, fecha, tema, PWA)
