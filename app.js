// ==================== app.js - VERSIÓN COMPLETA CON NUEVA LÓGICA PREMIUM Y TEMA MILITARY ====================
// Versión: 2.1 - Añadido tema military (MultiCam)
...
const AppState = {
  ...
  // FUNCIÓN ACTUALIZADA PARA LA NUEVA LÓGICA DE PREMIUM
  puedeVerDetalleSesion() {
    // Usuarios premium activos pueden ver todos los detalles
    if (this.isPremium && this.premiumExpiryDate && new Date() <= this.premiumExpiryDate) {
      return true;
    }
    // Usuarios no premium SOLO pueden ver las primeras 2 semanas del plan
    if (!this.isPremium || (this.premiumExpiryDate && new Date() > this.premiumExpiryDate)) {
      if (this.planGeneradoActual && this.planGeneradoActual.fechaInicio) {
        ...
        // Permitir ver detalles de las primeras 2 semanas (14 días)
...
// ==================== MÓDULO DE TEMA (3 temas: claro, oscuro, military) ====================
window.toggleTheme = function(btn) {
  ...
  if (body.classList.contains('manual-light')) currentTheme = 'light';
  else if (body.classList.contains('manual-dark')) currentTheme = 'dark';
  else if (body.classList.contains('military-theme')) currentTheme = 'military';
  
  // Ciclo: light → dark → military → light
  if (currentTheme === 'light') {
    body.classList.remove('manual-light');
    body.classList.add('manual-dark');
    localStorage.setItem('ri5_theme', 'dark');
    btn.textContent = '🌙'; // luna para dark
  } else if (currentTheme === 'dark') {
    body.classList.remove('manual-dark');
    body.classList.add('military-theme');
    localStorage.setItem('ri5_theme', 'military');
    btn.textContent = '🪖'; // casco para military
  } else {
    body.classList.remove('military-theme');
    body.classList.add('manual-light');
    localStorage.setItem('ri5_theme', 'light');
    btn.textContent = '💡'; // bombilla para light
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (savedTheme === 'light' || savedTheme === 'dark' || savedTheme === 'military') {
    document.body.classList.add(
      savedTheme === 'light' ? 'manual-light' :
      savedTheme === 'dark' ? 'manual-dark' : 'military-theme'
    );
    // Actualizar icono de los botones de tema
    const themeToggleLogin = document.getElementById('themeToggleLogin');
    const themeToggle = document.getElementById('themeToggle');
    const icon = savedTheme === 'light' ? '💡' : (savedTheme === 'dark' ? '🌙' : '🪖');
