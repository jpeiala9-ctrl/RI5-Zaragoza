// ==================== app.js - VERSIÓN COMPLETA CORREGIDA (SIN ADMIN) ====================
...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (uid) {
      ...
      // Guardar isAdmin para referencia rápida (opcional)
      if (this.isAdmin) {
...
// ==================== MÓDULO ADMIN ====================
// Nuevo objeto para la funcionalidad de administración
const Admin = {
  ...
  // Cargar lista de usuarios (paginated)
  async cargarUsuarios(reset = false) {
    ...
    try {
      ...
      if (snapshot.empty) {
        if (reset) container.innerHTML = '<p style="text-align:center; padding:20px;">No hay usuarios</p>';
      ...
      snapshot.docs.forEach(doc => {
        ...
        html += `
          <div class="historial-item" style="cursor:default;" onclick="event.stopPropagation()">
            <div class="fecha">${username} (${email})</div>
            <div class="resumen">Premium: ${premium} · Expira: ${expires} · Cálculos: ${calculos}</div>
            <div style="font-size: var(--text-xs); color: var(--text-secondary); margin-top: var(--space-xs);">
              Registro: ${created} · Último acceso: ${lastLogin}
            </div>
            <div style="display: flex; gap: var(--space-xs); margin-top: var(--space-sm); justify-content: center;">
              <button class="action-button" style="width:auto; padding: var(--space-xs) var(--space-sm);" onclick="Admin.verPerfil('${uid}')">VER PERFIL</button>
              <button class="action-button" style="width:auto; padding: var(--space-xs) var(--space-sm);" onclick="Admin.abrirMensajeUsuario('${uid}', '${username}')">MENSAJE</button>
            </div>
          </div>
        `;
      });
  ...
  async verPerfil(uid) {
    ...
    try {
      ...
      // Mostrar detalles en un modal simple (podríamos reutilizar el modal de confirmación adaptado)
      const message = `
      ...
      alert(message); // Por simplicidad, podemos mejorarlo después
    } catch (error) {
  ...
  // Cargar mensajes de todos los usuarios (desde admin_*)
  async cargarMensajesUsuarios() {
    ...
    try {
      // Obtener todos los documentos con ID que empiezan por "admin_"
      ...
      allMessages.forEach(msg => {
        const leido = msg.leido ? '' : 'nuevo';
        html += `
          <div class="mensaje-item ${leido}" data-uid="${msg.fromUid}" data-idx="${msg.mensajeIdx}">
            <div class="mensaje-header" onclick="this.parentNode.classList.toggle('abierto')">
              <span>📨 ${msg.fecha} · ${msg.username}</span>
              <span class="flecha">▼</span>
              <button class="delete-mensaje" onclick="event.stopPropagation(); Admin.responderMensaje('${msg.fromUid}', '${msg.username}')">✉️ RESPONDER</button>
            </div>
      ...
      container.innerHTML = '<p style="text-align:center; padding:20px;">Error al cargar mensajes</p>';
    }
  },
    ...
    try {
      ...
      for (let i = 0; i < users.length; i += batchSize) {
        ...
        for (const userDoc of chunk) {
          ...
          const userRef = firebaseServices.db.collection('mensajes').doc(uid);
          // Usar transacción no es necesario, usamos set con merge
          // Pero para batch, necesitamos leer primero? Mejor usar set con merge actualizando el array.
          // En Firestore no se puede hacer arrayUnion en batch sin leer antes. Alternativa: usar set con merge y arrayUnion, pero eso no es batchable.
          // Para simplificar, haremos una escritura por usuario con await (puede ser lento pero seguro).
          // O podemos usar una función cloud, pero no tenemos.
          // Lo haremos con Promise.all con límite de concurrencia.
        }
        
        // Como es complejo, optamos por una implementación simple con await secuencial y mostrar progreso.
      }
      
      // Implementación simple: enviar uno por uno con await (puede ser lento, pero funcional para pocos usuarios)
      ...
      Utils.showToast('Error al enviar mensajes', 'error');
    } finally {
      Utils.hideLoading();
    }
  }
