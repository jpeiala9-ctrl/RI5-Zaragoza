// ==================== app.js - VERSIÓN COMPLETA CON BARRA INFERIOR Y DASHBOARD ====================
// VERSIÓN: 5.1 - Compatible con index.html CORREGIDO
// ====================
...
const AppState = {
  ...
  async setCurrentUser(uid, email, userData = null) {
    ...
    if (uid) {
      ...
      this.precargarDatos();
      // Actualizar dashboard y perfil después del login
      setTimeout(() => {
        if (window.actualizarDashboard) window.actualizarDashboard();
        if (window.actualizarPerfil) window.actualizarPerfil();
      }, 500);
    } else {
...
// ==================== MÓDULO ADMIN (MEJORADO CON PAGINACIÓN Y TOTAL DE USUARIOS) ====================
const Admin = {
  usersPagination: {
    lastDoc: null,
    hasMore: true,
  ...
  async obtenerTotalUsuarios() {
    try {
      ...
      this.usersPagination.totalUsers = total;
      return total;
    } catch (error) {
      try {
        ...
        console.error('Error obteniendo total de usuarios:', e);
        return 0;
      }
    }
  },
  ...
  async cargarUsuarios(reset = false) {
    ...
    if (reset) {
      ...
      this.usersPagination.currentPage = 0;
      await this.obtenerTotalUsuarios();
  ...
  renderUsersList(usersData) {
    ...
    const loadMoreBtn = document.getElementById('loadMoreUsersBtn');
    if (loadMoreBtn) {
      if (this.usersPagination.hasMore) {
        loadMoreBtn.style.display = 'block';
        loadMoreBtn.disabled = false;
        loadMoreBtn.textContent = 'CARGAR MÁS (20)';
      } else {
        loadMoreBtn.style.display = 'none';
      }
    }

    this._mostrarInfoPaginaAdmin();
  },

  _mostrarInfoPaginaAdmin() {
    const container = document.getElementById('adminUsersList');
    const btn = document.getElementById('loadMoreUsersBtn');
    if (!container) return;
    const totalItems = container.querySelectorAll('.usuario-item').length;
    const pagina = this.usersPagination.currentPage || 1;
    const totalUsuarios = this.usersPagination.totalUsers || 0;
    const oldInfo = document.getElementById('infoPaginaAdmin');
    if (oldInfo) oldInfo.remove();
    const info = document.createElement('div');
    info.id = 'infoPaginaAdmin';
    info.style.cssText = 'text-align: center; font-size: 12px; color: var(--text-secondary); margin: 10px 0; padding: 8px; background: var(--bg-secondary); border-radius: 8px;';
    info.innerHTML = `📄 Página ${pagina} · Mostrando ${totalItems} usuarios · 👥 Total en app: <strong style="color:var(--accent-yellow);">${totalUsuarios}</strong> usuarios`;
    if (btn) {
  ...
  cargarMasUsuarios() {
    if (this.usersPagination.loading) {
    ...
    if (this.usersPagination.hasMore) {
      ...
      Utils.showToast('No hay más usuarios', 'info');
      const btn = document.getElementById('loadMoreUsersBtn');
  ...
  abrirMensajeUsuario(uid, username) {
    ...
    if (texto && texto.trim()) Admin.enviarMensajeUsuario(uid, texto.trim());
  },
  ...
  async cargarMensajesUsuarios(reset = false) {
    ...
    try {
      ...
      const {
        docs,
        lastDoc
      } = await Storage.getAdminMessagesPaginated(limit, this.messagesPagination.lastDoc);
      ...
      if (loadMoreBtn) {
        if (this.messagesPagination.hasMore) {
          loadMoreBtn.style.display = 'block';
          loadMoreBtn.disabled = false;
  ...
  cargarMasMensajesAdmin() {
    if (this.messagesPagination.loading) {
      Utils.showToast('Cargando...', 'info');
      return;
    }
    if (this.messagesPagination.hasMore && !this.messagesPagination.loading) {
      ...
      Utils.showToast('No hay más mensajes', 'info');
...
const UI = {
  ...
  changeDailyTip() {
    this.updateTip('dailyTip');
  },
  changeConsejo() {
    this.updateTip('curiosity');
  },
  ...
  startConsejoAutoChange() {
    ...
    this.dailyInterval = setInterval(() => {
      if (document.getElementById("loginPage")?.style.display !== "none") this.updateTip('dailyTip');
    }, 8000);
    this.consejoInterval = setInterval(() => {
      if (document.getElementById("mainContent")?.style.display !== "none") this.updateTip('curiosity');
    }, 8000);
  },
  ...
  async switchTab(tab) {
    ...
    // Actualizar barra inferior si existe
    if (window.switchToTab && (tab === 'inicio' || tab === 'plan' || tab === 'entreno' || tab === 'soporte' || tab === 'perfil')) {
    ...
    try {
      ...
      if (tab === 'perfil') {
        ...
        if (window.actualizarPerfil) await window.actualizarPerfil();
      }
      if (tab === 'amigos') {
        if (window.Friends) {
      ...
      if (tab === 'inicio' && window.actualizarDashboard) {
  ...
  renderMessageList(container, mensajes, tipo, onToggleCallback) {
    if (!container) return;
    if (!mensajes.length) {
      container.innerHTML = '<p style="text-align:center; padding:40px; color: var(--text-secondary);">No hay mensajes</p>';
      return;
    }
  ...
  async cargarHistorialCompleto(reset = false) {
    ...
    if (!AppState.historialPagination) AppState.historialPagination = {
      lastDoc: null,
      hasMore: true,
      loading: false
    };
    ...
    try {
      ...
      if (result.items.length === 0) {
        if (reset) container.innerHTML = '<p style="text-align:center; padding:20px;">Sin cálculos guardados</p>';
      ...
      if (reset) container.innerHTML = '<p style="text-align:center; padding:20px;">Error al cargar</p>';
    } finally {
      if (AppState && AppState.historialPagination) AppState.historialPagination.loading = false;
    }
  },
...
// ==================== FUNCIONES DE NAVEGACIÓN INFERIOR Y DASHBOARD ====================
window.switchToTab = function(tabId) {
  ...
  localStorage.setItem('ri5_active_tab', tabId);

  // Cargar contenido según el tab
  if (tabId === 'perfil' && window.Profile) Profile.cargarPerfil();
  if (tabId === 'inicio' && window.actualizarDashboard) window.actualizarDashboard();
  if (tabId === 'entreno') {
    const savedCalc = localStorage.getItem('ri5_last_calculation');
    if (savedCalc && window.Training) {
      try {
        window.Training.mostrarResultadosGuardados(JSON.parse(savedCalc));
      } catch (e) {}
    }
  }
  if (tabId === 'amigos' && window.Friends) {
    Friends.cargarTodosUsuarios(true);
    Friends.cargarListaAmigos();
  }
  if (tabId === 'muro' && window.Wall) Wall.cargarMuro();
  if (tabId === 'historial' && window.UI) UI.cargarHistorialCompleto(true);
  if (tabId === 'admin' && AppState?.isAdmin && window.Admin) {
    Admin.cargarUsuarios(true);
    Admin.cargarMensajesUsuarios(true);
  }
  if (tabId === 'plan') {
...
window.actualizarDashboard = async function() {
  ...
  const userName = AppState.currentUserData?.username || 'CORREDOR';
  const userNameSpan = document.getElementById('inicioUserName');
  if (userNameSpan) userNameSpan.textContent = userName.toUpperCase();

  // Intentar obtener estadísticas del plan actual
  let weeklyKm = 0,
    weeklySessions = 0,
    adherence = 0;
  try {
    if (AppState.planActualId) {
      const planDoc = await firebaseServices.db.collection('users').doc(AppState.currentUserId).collection('planes').doc(AppState.planActualId).get();
      if (planDoc.exists) {
        const plan = planDoc.data();
        const sesiones = plan.sesiones || [];
        const realizadas = plan.sesionesRealizadas || {};

        // Semana actual (aproximada)
        const now = new Date();
        ...
        sesiones.forEach(sesion => {
          // Usar semana actual (podría ser más preciso con fecha)
          if (sesion.semana === 8) { // Semana 8 como ejemplo
            if (sesion.tipo !== 'descanso') {
              weeklySessions++;
              if (sesion.detalle?.distanciaEstimada) weeklyKm += sesion.detalle.distanciaEstimada;
              if (realizadas[sesion.diaGlobal]) completedSessions++;
            }
          }
        });
        adherence = weeklySessions > 0 ? Math.round((completedSessions / weeklySessions) * 100) : 0;
      }
    }
  } catch (e) {
    console.warn('Error obteniendo stats del plan:', e);
  }

  const weeklyDistanceSpan = document.getElementById('weeklyDistance');
  const weeklySessionsSpan = document.getElementById('weeklySessions');
  const adherenceRateSpan = document.getElementById('adherenceRate');
  if (weeklyDistanceSpan) weeklyDistanceSpan.textContent = Math.round(weeklyKm);
  if (weeklySessionsSpan) weeklySessionsSpan.textContent = weeklySessions;
  if (adherenceRateSpan) adherenceRateSpan.textContent = adherence;

  // Cargar preview del muro
  await window.cargarPreviewMuro();
};

window.cargarPreviewMuro = async function() {
  const container = document.getElementById('muroPreviewContainer');
  if (!container) return;

  try {
    const snapshot = await firebaseServices.db.collection('globalFeed').orderBy('timestamp', 'desc').limit(3).get();
    if (snapshot.empty) {
      container.innerHTML = '<p style="text-align:center; color:var(--text-secondary); padding:20px;">Sé el primero en compartir un entrenamiento</p>';
      return;
    }
    ...
    snapshot.forEach(doc => {
      ...
      const username = Utils.capitalizeUsername(entry.username || 'Usuario');
      const timeAgo = getTimeAgo(entry.timestamp);
      html += `
        <div class="wall-entry" onclick="switchToTab('muro')">
          <img class="wall-avatar-small" src="${entry.photoURL || 'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22%3E%3Ccircle cx=%2250%22 cy=%2250%22 r=%2250%22 fill=%22%23ccc%22/%3E%3Ctext x=%2250%22 y=%2267%22 text-anchor=%22middle%22 fill=%22%23666%22 font-size=%2240%22%3E👤%3C/text%3E%3C/svg%3E'}">
          <div class="wall-content">
            <div class="wall-user">${username}</div>
            <div class="wall-time">${timeAgo}</div>
            <div class="wall-training-name">${entry.trainingName || entry.trainingType || 'Entreno'}</div>
            <div class="wall-details"><span>⏱️ ${entry.duration || 0}'</span><span>📏 ${entry.distancia?.toFixed(1) || 0} km</span></div>
            <div class="wall-likes">❤️ ${entry.likeCount || 0}</div>
          </div>
        </div>
      `;
    });
    ...
    console.warn('Error cargando preview del muro:', e);
    container.innerHTML = '<p style="text-align:center; color:var(--text-secondary);">Error al cargar el muro</p>';
  }
};
...
window.actualizarPerfil = async function() {
  ...
  if (userData) {
    const amigosSpan = document.getElementById('profileAmigos');
    if (amigosSpan) amigosSpan.textContent = userData.friendsCount || 0;
  }

  try {
    const gamification = await Gamification.getData(AppState.currentUserId);
    if (gamification) {
      const sesionesSpan = document.getElementById('profileSesiones');
      const kmSpan = document.getElementById('profileKmTotal');
      ...
      if (sesionesSpan) sesionesSpan.textContent = gamification.totalSessions || 0;
      if (kmSpan) kmSpan.textContent = Math.round(gamification.totalDistance || 0);
      if (levelSpan) levelSpan.textContent = gamification.level || 1;
      const progress = Gamification.getProgressToNextLevel(gamification.totalDistance);
      if (progressFill) progressFill.style.width = `${progress}%`;
      if (progressText) progressText.textContent = `${progress}% → Nivel ${(gamification.level || 1) + 1}`;
    }
  } catch (e) {}

  const ultimoCalc = AppState.lastCalc || await Storage.getUltimoCalculo(AppState.currentUserId);
  if (ultimoCalc) {
    const ageSpan = document.getElementById('profileAge');
    const fcSpan = document.getElementById('profileFcMax');
    const timeSpan = document.getElementById('profile6kTime');
    if (ageSpan) ageSpan.textContent = `${ultimoCalc.age} años`;
    if (fcSpan) fcSpan.textContent = `${ultimoCalc.fcMax} ppm`;
    if (timeSpan) timeSpan.textContent = Utils.formatR(ultimoCalc.ritmoBase);
  }
};

// Inicializar barra inferior después de que el DOM esté listo
function initBottomNav() {
  const navItems = document.querySelectorAll('.bottom-nav-item');
  navItems.forEach(item => {
    item.addEventListener('click', function() {
      const tabId = this.dataset.tab;
      if (tabId) window.switchToTab(tabId);
    });
  });

  const savedTab = localStorage.getItem('ri5_active_tab');
  if (savedTab && savedTab !== 'inicio') {
    const target = document.querySelector(`.bottom-nav-item[data-tab="${savedTab}"]`);
...
window.switchTab = async function(tab) {
  if (UI && UI.switchTab) await UI.switchTab(tab);
};
window.toggleCuestionario = function() {
  if (window.PlanGenerator) PlanGenerator.toggleCuestionario();
  else Utils.showToast('Cargando…', 'info');
};
window.mostrarUltimoPlanGuardado = function() {
  if (window.PlanGenerator) PlanGenerator.mostrarUltimoPlanGuardado();
  else Utils.showToast('Cargando…', 'info');
};
window.borrarPlanGuardado = function() {
  if (window.PlanGenerator) PlanGenerator.borrarPlanGuardado();
  else Utils.showToast('Cargando…', 'info');
};
window.generarCalendarioEntreno = function() {
  if (window.PlanGenerator) PlanGenerator.generarCalendarioEntreno();
  else Utils.showToast('Cargando…', 'info');
};
window.validarOpcionesPlan = function() {
  if (window.PlanGenerator) PlanGenerator.validarOpcionesPlan();
};
window.cargarHistorial = async function() {
  if (UI && UI.cargarHistorialCompleto) await UI.cargarHistorialCompleto(true);
};
window.cargarMasHistorial = async function() {
  if (UI && UI.cargarHistorialCompleto) await UI.cargarHistorialCompleto(false);
};
window.borrarHistorial = async function() {
  if (UI && UI.borrarHistorial) await UI.borrarHistorial();
};
window.borrarEntradaHistorial = async function(entryId) {
  if (UI && UI.borrarEntradaHistorial) await UI.borrarEntradaHistorial(entryId);
};
window.toggleHistorialDetalle = function(el) {
  if (UI && UI.toggleHistorialDetalle) UI.toggleHistorialDetalle(el);
};
window.enviarMensajeUsuario = async function() {
  if (UI && UI.enviarMensajeUsuario) await UI.enviarMensajeUsuario();
};
window.borrarMensajeUsuario = async function(i, tipo) {
  if (UI && UI.borrarMensajeUsuario) await UI.borrarMensajeUsuario(i, tipo);
};
window.cambiarSoporteTab = function(tab) {
  if (UI && UI.cambiarSoporteTab) UI.cambiarSoporteTab(tab);
};
window.cargarPlanDesdeHistorial = async function(planId) {
  if (UI && UI.cargarPlanDesdeHistorial) await UI.cargarPlanDesdeHistorial(planId);
};
window.eliminarPlanHistorial = async function(planId) {
  if (UI && UI.eliminarPlanHistorial) await UI.eliminarPlanHistorial(planId);
};
window.cerrarPlan = function() {
  if (UI && UI.cerrarPlan) UI.cerrarPlan();
};
...
if (typeof PlanGenerator !== 'undefined') {
  window.cambiarTrimestre = async (delta) => {
    await PlanGenerator.cambiarTrimestre(delta);
  };
} else {
  window.cambiarTrimestre = async (delta) => {
