...
// Versión: 4.3 - CORREGIDO: Doble check azul con logs de depuración
// ====================
...
const Chat = {
  ...
  listenMessages(conversationId, callback) {
    ...
    this.unsubscribeMessages = query.onSnapshot(
      snapshot => {
        console.log('🔄 Listener de mensajes activado. Docs:', snapshot.size);
        const messages = [];
  ...
  async markMessagesAsRead(conversationId) {
    ...
    console.log('👁️ Marcando mensajes como leídos para usuario:', userId);
    ...
    try {
      ...
      console.log('📊 Mensajes no leídos encontrados:', snapshot.size);
      ...
      snapshot.forEach(doc => {
        ...
        console.log('📝 Actualizando mensaje:', data.text, 'readBy antes:', currentReadBy);
      ...
      await batch.commit();
      console.log('✅ Mensajes marcados como leídos en Firestore');
      
      // Refrescar lista de amigos
      if (window.Friends && document.getElementById('listaAmigos')) {
  ...
  openChat(conversationId, otherUserId, otherUsername) {
    console.log('💬 Abriendo chat con:', otherUsername, 'conversationId:', conversationId);
  ...
  async loadAndRenderMessages(conversationId, otherUserId) {
    ...
    snapshot.forEach(doc => {
      const data = doc.data();
      console.log('📨 Mensaje cargado:', data.text, 'senderId:', data.senderId, 'readBy:', data.readBy);
  ...
  createMessageElement(msg, otherUserId) {
    ...
    if (isSent) {
      ...
      // ✅ VERIFICACIÓN CORRECTA DEL DOBLE CHECK
      const readBy = msg.readBy || [];
      console.log('📝 Renderizando mensaje enviado:', msg.text, 'readBy:', readBy, 'mi UID:', AppState.currentUserId);
      
      // Un mensaje está leído si hay ALGUIEN MÁS además del remitente en readBy
      const isRead = readBy.some(uid => uid !== AppState.currentUserId);
      
      console.log('✅ isRead calculado:', isRead);
      
      if (isRead) {
        statusSpan.innerHTML = '✓✓';
        statusSpan.title = 'Visto';
        statusSpan.classList.add('read');
        console.log('💙 Mostrando DOBLE CHECK AZUL');
      } else {
        statusSpan.innerHTML = '✓';
        statusSpan.title = 'Enviado';
        statusSpan.classList.add('sent');
        console.log('✓ Mostrando CHECK SIMPLE');
      }
  ...
  async updateUnreadBadge() {
    ...
    try {
      const conversations = await this.getConversations(AppState.currentUserId);
      const totalUnread = conversations.reduce((acc, c) => acc + (c.unreadCount || 0), 0);
      console.log('📬 [Chat] Total mensajes sin leer:', totalUnread);
