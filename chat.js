// ==================== chat.js - VERSIÓN 1.0.0 - DESDE CERO ====================
// Sin readBy, sin arrays, sin estados complejos. Solo un booleano: visto = true/false
// ====================
...
const Chat = {
  ...
  async sendMessage(conversationId, text) {
    ...
    try {
      // SIMPLE: Solo texto, remitente, timestamp y visto = false
  ...
  async marcarComoVisto(conversationId) {
    ...
    try {
      // Buscar mensajes NO VISTOS que NO envió el usuario actual
  ...
  async getConversations(userId) {
    ...
    try {
      ...
      for (const doc of snapshot.docs) {
        ...
        // Contar no leídos (visto = false Y senderId != userId)
  ...
  // ========== ESCUCHAR MENSAJES ==========
  escucharMensajes(conversationId, callback) {
  ...
  // ========== ABRIR CHAT ==========
  abrirChat(conversationId, otherUserId, otherUsername) {
    ...
    if (!modal) {
      ...
      document.getElementById('closeChatBtn').onclick = () => this.cerrarChat();
      document.getElementById('sendChatBtn').onclick = () => this.enviarMensajeActual();
      document.getElementById('chatInput').onkeypress = (e) => {
        if (e.key === 'Enter') this.enviarMensajeActual();
      };
    }
    ...
    // MARCAR COMO VISTO
    this.marcarComoVisto(conversationId);
    
    // CARGAR MENSAJES
    this.cargarMensajes(conversationId);
    
    // ESCUCHAR NUEVOS MENSAJES
    this.escucharMensajes(conversationId, (messages) => {
      this.renderizarMensajes(messages);
  ...
  // ========== CARGAR MENSAJES ==========
  async cargarMensajes(conversationId) {
    const snapshot = await firebaseServices.db
      .collection('conversations')
      .doc(conversationId)
      .collection('messages')
  ...
  renderizarMensajes(messages) {
    ...
    for (const [etiqueta, msgs] of Object.entries(grupos)) {
      ...
      msgs.forEach(msg => {
        container.appendChild(this.crearElementoMensaje(msg));
      });
    }
  },

  // ========== CREAR ELEMENTO DE MENSAJE ==========
  crearElementoMensaje(msg) {
    ...
    footer.style.cssText = 'display: flex; justify-content: flex-end; align-items: center; gap: 4px; font-size: 11px; margin-top: 4px;';
    
    const hora = document.createElement('span');
    const fecha = msg.timestamp?.toDate?.() || new Date(msg.timestamp);
    hora.textContent = `${fecha.getHours().toString().padStart(2, '0')}:${fecha.getMinutes().toString().padStart(2, '0')}`;
    hora.style.color = esMio ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)';
  ...
  async iniciarChatConAmigo(friendId, friendUsername) {
    ...
    if (conversationId) this.abrirChat(conversationId, friendId, friendUsername);
  },

  // ========== RENDERIZAR LISTA DE CONVERSACIONES ==========
  async renderizarListaConversaciones(container) {
    ...
    try {
      ...
      container.querySelectorAll('.conversacion-item').forEach(el => {
        el.addEventListener('click', () => {
          this.abrirChat(el.dataset.convId, el.dataset.otherId, el.dataset.otherName);
        });
      });
...
Chat.startChatWithFriend = Chat.iniciarChatConAmigo;
Chat.renderConversations = Chat.renderizarListaConversaciones;
Chat.updateUnreadBadge = async function() {
...
console.log('✅ Chat cargado - Versión 1.0.0 desde cero');