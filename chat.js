...
// Versión: 2.3 - Hora formateada y badge verde en conversaciones
// ====================
...
const Chat = {
  ...
  async cargarConversaciones() {
    ...
    try {
      ...
      if (snapshot.empty) {
        container.innerHTML = '<p style="text-align:center; padding:20px; color: var(--text-secondary);">No tienes conversaciones activas.</p>';
        return;
      }
        ...
        html += `
          <div class="conversacion-item ${noLeidosClass}" onclick="Chat.abrirChat('${otroId}', '${Utils.escapeHTML(username)}')">
            ...
            <div class="conversacion-info">
              <div class="conversacion-nombre ${tieneNoLeidos ? 'nombre-no-leido' : ''}">${Utils.escapeHTML(username)}</div>
