  ...
  async renderConversations(container) {
    ...
    try {
      ...
      for (const conv of conversations) {
        ...
        html += `
          <div class="conversacion-item ${unreadClass}" data-conv-id="${esc(conv.id)}" data-other-user-id="${esc(conv.otherUserId)}" data-other-username="${esc(conv.otherUsername)}">
            ${avatarHTML}
            <div class="conversacion-info">
              <div class="conversacion-nombre">${esc(conv.otherUsername)}</div>
              <div class="conversacion-preview">${esc(conv.lastMessage) || 'Sin mensajes'}</div>
            </div>
