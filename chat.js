...
? `<img src="${esc(conv.otherPhotoURL)}" class="conversacion-avatar" style="object-fit:cover;">`
: `<div class="conversacion-avatar">👤</div>`;
html += `<div class="conversacion-item ${unreadClass}" data-conv-id="${esc(conv.id)}" data-other-user-id="${esc(conv.otherUserId)}" data-other-username="${esc(conv.otherUsername)}"> ${avatarHTML} <div class="conversacion-info"> <div class="conversacion-nombre">${esc(conv.otherUsername)}</div> <div class="conversacion-preview">${esc(conv.lastMessage) || 'Sin mensajes'}</div> </div> <div class="conversacion-fecha">${fecha}</div> ${conv.unreadCount > 0 ?`<div class="conversacion-badge">${conv.unreadCount}</div>`: ''} </div>`;
}
...
modal.innerHTML = `<div class="modal-content" id="chatModalContent"> <div class="chat-header"> <span id="chatFriendName"></span> <button class="close-chat" onclick="Chat.closeChat()">✕</button> </div> <div id="chatMessages" class="chat-messages"></div> <div class="chat-input"> <input type="text" id="chatInput" placeholder="Escribe un mensaje..."> <button onclick="Chat.sendCurrentMessage()">📤</button> </div> </div>`;
