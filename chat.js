// ==================== chat.js - Mensajería entre amigos (CON ELIMINACIÓN DE CONVERSACIÓN) ====================
// Versión: 2.0 - Añadida función para eliminar conversación completa (mensajes + documento)
// ====================
...
const Chat = {
  ...
  async getOrCreateConversation(userId, friendId) {
    ...
    try {
      ...
      // Fallback: si no existe (por compatibilidad con amistades anteriores), la creamos ahora
  ...
  // ==================== NUEVA FUNCIÓN: ELIMINAR CONVERSACIÓN COMPLETA ====================
  async deleteConversation(conversationId) {
    ...
    try {
      ...
      // Eliminar el documento de la conversación
      ...
      // Refrescar la lista de conversaciones (si existe el contenedor)
      ...
      // También refrescar la lista de conversaciones si se muestra en otro sitio
  ...
  openChat(conversationId, otherUserId, otherUsername) {
    ...
    if (!modal) {
      ...
      // Evento para el botón eliminar conversación
      document.getElementById('deleteChatBtn').addEventListener('click', async () => {
        if (this.currentConversationId) {
          const deleted = await this.deleteConversation(this.currentConversationId);
          if (deleted) this.closeChat();
        }
      });
    } else {
      // Si ya existe el modal, actualizar el evento del botón eliminar (por si cambia la conversación)
