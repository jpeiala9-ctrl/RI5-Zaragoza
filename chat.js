// ==================== chat.js - Mensajería entre amigos (ELIMINACIÓN CIERRA MODAL AUTOMÁTICAMENTE) ====================
// Versión: 2.2 - Al hacer clic en eliminar, el modal se cierra de inmediato y luego se borra la conversación.
// ====================
...
const Chat = {
  ...
  async renderConversations(container) {
    ...
    try {
      ...
      for (const conv of conversations) {
        ...
        html += `
          <div class="conversacion-item ${unreadClass}" data-conv-id="${conv.id}" data-other-user-id="${conv.otherUserId}" data-other-username="${Utils.escapeHTML(conv.otherUsername)}">
            ${avatarHTML}
            <div class="conversacion-info">
              <div class="conversacion-nombre">${Utils.escapeHTML(conv.otherUsername)}</div>
              <div class="conversacion-preview">${conv.lastMessage ? Utils.escapeHTML(conv.lastMessage) : 'Sin mensajes'}</div>
            </div>
            <div class="conversacion-fecha">${Utils.escapeHTML(fecha)}</div>
  ...
  // ELIMINAR CONVERSACIÓN (cierra el modal inmediatamente)
  async deleteConversation(conversationId) {
    if (!conversationId) return false;
    const currentUid = AppState.currentUserId;
    if (!currentUid) return false;

    const confirmed = await Utils.confirm(
      'Eliminar conversación',
      '¿Eliminar permanentemente esta conversación y todos sus mensajes? Esta acción no se puede deshacer.'
    );
    if (!confirmed) return false;

    // CERRAR MODAL INMEDIATAMENTE (antes de borrar datos)
    ...
    try {
      ...
      if (!convDoc.exists) {
        Utils.showToast('La conversación ya no existe', 'error');
        return false;
      }

      const participants = convDoc.data().participants;
      if (!participants.includes(currentUid)) {
        Utils.showToast('No tienes permiso para eliminar esta conversación', 'error');
        return false;
      }

      // Eliminar todos los mensajes y el documento
      ...
      if (container && Friends && Friends.cargarListaAmigos) {
        await Friends.cargarListaAmigos();
      }
      const convContainer = document.getElementById('conversacionesList');
      if (convContainer) await this.renderConversations(convContainer);
  ...
  openChat(conversationId, otherUserId, otherUsername) {
    ...
    if (!modal) {
        ...
        <div class="modal-content" id="chatModalContent">
          <div class="chat-header">
            ...
            <div style="display: flex; gap: 8px;">
              <button id="deleteChatBtn" class="btn-amigo eliminar" style="background: transparent; border-color: var(--zone-5);">🗑️</button>
      ...
      // Botón eliminar: cierra el modal mediante deleteConversation (que ya cierra)
      ...
      // Actualizar evento del botón eliminar por si cambió el modal
