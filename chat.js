// ==================== chat.js - Mensajería entre amigos (CORREGIDO - sin get previo) ====================
// Versión: 1.5 - Corregido getOrCreateConversation para evitar errores de permisos
// ====================
...
const Chat = {
  ...
  // CORREGIDO: ya no hace get() antes de crear, usa set con merge
  async getOrCreateConversation(userId, friendId) {
    ...
    try {
      // Intentar crear/actualizar el documento sin leer primero
      // Si ya existe, merge: true evita sobrescribir los datos existentes
      await docRef.set({
      ...
      // Verificar si faltan los datos de participantesData (para casos antiguos)
      const doc = await docRef.get();
      if (!doc.exists) {
        // Esto no debería ocurrir porque acabamos de crearlo, pero por seguridad
        return null;
      }
      
      const data = doc.data();
      // Si no existe participantsData, lo creamos
