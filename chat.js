// ==================== chat.js - COMPLETO CON SOPORTE PARA SESIONES COMPARTIDAS ====================
// Versión: 1.1 - Añadida detección y acción para mensajes de tipo shared_session
// ====================
...
const Chat = {
  ...
  async getOrCreateConversation(userId, friendId) {
    ...
    try {
      ...
      Utils.showToast('❌ Error al iniciar la conversación. Reinténtalo.', 'error');
      return null;
    }
  },
  ...
  async sendMessage(conversationId, text) {
    ...
    try {
      ...
      Utils.showToast('❌ Error al enviar el mensaje. Reinténtalo.', 'error');
      return false;
    }
  },
  ...
  async markMessagesAsRead(conversationId) {
    ...
    try {
      ...
      await batch.commit();
      console.log(`✅ ${snapshot.size} mensajes marcados como leídos`);
    } catch (error) {
  ...
  openChat(conversationId, otherUserId, otherUsername) {
    ...
    this.markMessagesAsRead(conversationId).then(() => {
    ...
    this.listenMessages(conversationId, (updatedMessages) => {
      this.renderMessagesGrouped(updatedMessages, otherUserId);
  ...
  async clearChat(conversationId) {
    ...
    try {
      ...
      Utils.showToast('❌ Error al vaciar el chat. Reinténtalo.', 'error');
    }
  },
  ...
  // ==================== MODIFICACIÓN EN renderMessagesGrouped ====================
  ...
  renderMessagesGrouped(messages, otherUserId) {
    ...
    for (const [dateLabel, msgs] of Object.entries(grouped)) {
      ...
      msgs.forEach(msg => {
        // Verificar si es una sesión compartida
        if (msg.tipo === 'shared_session' && msg.sessionData) {
          // Renderizar elemento especial
          const sharedDiv = this._renderSharedSession(msg);
  ...
  // ==================== NUEVA FUNCIÓN PARA RENDERIZAR SESIÓN COMPARTIDA ====================

  _renderSharedSession(msg) {
    const container = document.createElement('div');
    container.className = 'chat-message received shared-session-message';
    container.style.cssText = `
      max-width:90%;
      background:var(--bg-card);
      border:1px solid var(--zone-2);
      border-radius:18px;
      padding:16px;
      margin:8px 0;
    ...
    const data = msg.sessionData;
    const fromUsername = msg.fromUsername || 'Alguien';
    const tipo = data.tipo || 'entreno';
    const duracion = data.duracion || '?';
    const nombre = data.detalle?.nombre || 'Sesión';
    const zona = data.detalle?.zona || 'Z2';
    
    container.innerHTML = `
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
        <span style="font-size:20px;">📨</span>
        <span style="font-weight:bold;color:var(--accent-yellow);">@${Utils.escapeHTML(fromUsername)}</span>
        <span style="font-size:12px;color:var(--text-secondary);">compartió una sesión</span>
      </div>
      <div style="background:var(--bg-secondary);border-radius:12px;padding:12px;margin:8px 0;">
        <div style="font-weight:bold;font-size:16px;">${Utils.escapeHTML(nombre)}</div>
        <div style="display:flex;gap:16px;margin-top:6px;font-size:13px;color:var(--text-secondary);">
          <span>🏃 ${Utils.escapeHTML(tipo.toUpperCase())}</span>
          <span>⏱️ ${duracion}'</span>
          <span>🔥 ${Utils.escapeHTML(zona)}</span>
        </div>
      </div>
      <button id="addSharedSessionBtn_${msg.sessionId}" 
              style="width:100%;padding:10px;background:var(--zone-2);border:none;border-radius:12px;color:var(--bg-primary);font-weight:bold;cursor:pointer;font-size:14px;letter-spacing:1px;margin-top:8px;transition:all 0.2s;">
        ➕ AÑADIR A MI PLAN
      </button>
    `;
    
    // Event listener para el botón
    const btn = container.querySelector(`#addSharedSessionBtn_${msg.sessionId}`);
    if (btn) {
      btn.addEventListener('click', async () => {
        ...
        try {
          const success = await Storage.guardarSesionCompartidaEnPlan(
            AppState.currentUserId,
            data,
            msg.fromUid || 'desconocido',
            fromUsername
          );
          
          if (success) {
            btn.textContent = '✅ AÑADIDA';
            btn.style.background = 'var(--realizado-color)';
            ...
            if (document.getElementById('tab-plan').classList.contains('active')) {
              if (window.PlanGenerator) {
                const planDoc = await firebaseServices.db.collection('users')
                  .doc(AppState.currentUserId)
                  .collection('planes')
                  .doc(AppState.planActualId)
                  .get();
                if (planDoc.exists) {
                  PlanGenerator.mostrarCalendario(planDoc.data().sesiones);
                }
              }
            }
          } else {
            btn.textContent = '❌ ERROR';
            btn.style.background = 'var(--zone-5)';
            setTimeout(() => {
              btn.textContent = '➕ REINTENTAR';
              btn.disabled = false;
              btn.style.background = 'var(--zone-2)';
            }, 3000);
          }
        } catch (error) {
          console.error('Error al añadir sesión compartida:', error);
          btn.textContent = '❌ ERROR';
          btn.style.background = 'var(--zone-5)';
          setTimeout(() => {
            btn.textContent = '➕ REINTENTAR';
  ...
  createMessageElement(msg, otherUserId) {
    ...
    if (isSent) {
      ...
      const isRead = msg.readBy && msg.readBy.includes(otherUserId);
      if (isRead) {
...
window.Chat = Chat;
console.log('✅ chat.js v1.1 - Soporte para sesiones compartidas');