...
// Funcionalidades: Hora en mensajes, nombre en verde si hay no leídos, palomitas ✓ (enviado) y ✓✓ (visto)
// ====================
...
const Chat = {
  ...
  async getOrCreateConversation(userId, friendId) {
    ...
    try {
      ...
      // Fallback: si no existe, la creamos ahora
  ...
  openChat(conversationId, otherUserId, otherUsername) {
    ...
    const loadMessages = async () => {
      ...
      chatMessages.innerHTML = '';
      snapshot.forEach(doc => {
        this.appendMessage(doc.data(), otherUserId);
      });
      chatMessages.scrollTop = chatMessages.scrollHeight;
    };
    loadMessages();
  ...
  appendMessage(msg, otherUserId) {
    ...
    // Palomitas (solo mensajes enviados)
    if (isSent) {
