// ==================== chat.js - Chat estilo Telegram (DOBLE CHECK FUNCIONAL) ====================
// Versión: 1.3 - CORREGIDO: Doble check garantizado
// ====================
...
const Chat = {
  ...
  async markMessagesAsRead(conversationId) {
    ...
    try {
      ...
      if (count > 0) {
        await batch.commit();
        console.log(`✅ ${count} mensajes marcados como leídos`);
      }
    } catch (error) {
  ...
  openChat(conversationId, otherUserId, otherUsername) {
    console.log(`💬 Abriendo chat con ${otherUsername} (${otherUserId})`);
    ...
    if (!modal) {
      ...
      document.getElementById('clearChatBtn').addEventListener('click', async () => {
        if (confirm('¿Seguro que quieres borrar todos los mensajes de esta conversación?')) {
    ...
    this.listenMessages(conversationId, (messages) => {
      console.log(`📨 ${messages.length} mensajes recibidos en listener`);
      this.renderMessagesGrouped(messages, otherUserId);
      chatMessages.scrollTop = chatMessages.scrollHeight;
      
      // Marcar como leídos los nuevos mensajes también
      this.markMessagesAsRead(conversationId);
    });
  },
  ...
  createMessageElement(msg, otherUserId) {
    ...
    if (isSent) {
      ...
      // CORRECCIÓN: Verificar si el otro usuario ha leído el mensaje
      const readBy = msg.readBy || [];
      const isRead = readBy.includes(otherUserId);
      
      console.log(`📊 Mensaje ${msg.id}: readBy=${JSON.stringify(readBy)}, otherUserId=${otherUserId}, isRead=${isRead}`);
