...
// CORREGIDO: verde para conversaciones no leídas y doble check azul funcionando
// ====================
...
const Chat = {
  ...
  async renderConversations(container) {
    ...
    try {
      ...
      let html = '';
      for (const conv of conversations) {
        // ✅ CORRECCIÓN: Aplicar clase 'conversacion-no-leida' si hay mensajes sin leer
  ...
  openChat(conversationId, otherUserId, otherUsername) {
    ...
    this.listenMessages(conversationId, (newMessages) => {
      if (newMessages.length) {
        this.appendMessagesGrouped(newMessages, otherUserId);
        // Actualizar badge de no leídos al recibir nuevos mensajes
  ...
  createMessageElement(msg, otherUserId) {
    ...
    if (isSent) {
      ...
      // ✅ CORRECCIÓN: Verificar si el mensaje ha sido leído por el otro usuario
      const isRead = msg.readBy && Array.isArray(msg.readBy) && msg.readBy.includes(otherUserId);
      if (isRead) {
        statusSpan.innerHTML = '✓✓';
        statusSpan.title = 'Visto';
        statusSpan.classList.add('read');
        statusSpan.style.color = '#4caf50'; // Verde para visto
      } else {
        statusSpan.innerHTML = '✓';
        statusSpan.title = 'Enviado';
        statusSpan.classList.add('sent');
        statusSpan.style.color = '#888888'; // Gris para enviado
      }
  ...
  async sendCurrentMessage() {
    ...
    if (success) {
      input.value = '';
      // Esperar un momento y luego marcar como leídos los mensajes del otro
      setTimeout(() => {
  ...
  closeChat() {
    ...
    // Actualizar badge al cerrar el chat
    this.updateUnreadBadge();
    this.updateConversationsList();
  },
