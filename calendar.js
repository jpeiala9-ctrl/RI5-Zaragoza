// ==================== calendar.js - VERSIÓN PRODUCCIÓN CON GAMIFICACIÓN + GPS ====================
// Versión: 2.49 - Corregido toggleCuestionario con clase hidden
// ============================================================================
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    if (!AppState.zonasCalculadas) {
      Utils.showToast("Primero calcula tus zonas", 'error');
      return;
    }
    if (!AppState.isPremium) {
      Utils.showToast("Solo usuarios premium", 'error');
      return;
    }
    try {
      Utils.showLoading();
      ...
      if (!diasEntreno.includes(diaLargo)) {
        Utils.showToast(`El día de tirada larga (${["", "LUN", "MAR", "MIÉ", "JUE", "VIE", "SÁB", "DOM"][diaLargo]}) no está marcado como día de entrenamiento.`, 'error');
        Utils.hideLoading();
        return;
      }
      if (nivel === 'avanzado' && diasEntreno.length < 5) {
        Utils.showToast("Nivel avanzado requiere mínimo 5 días de entreno", 'error');
        Utils.hideLoading();
        return;
      }
      if (diasEntreno.length === 0) {
        Utils.showToast("Selecciona al menos un día", 'error');
        Utils.hideLoading();
        return;
      }
      const diasMinimos = this.obtenerDiasMinimos(distancia);
      if (diasEntreno.length < diasMinimos) {
        Utils.showToast(`Para ${distancia === 'maraton' ? 'MARATÓN' : 'MEDIA'} necesitas mínimo ${diasMinimos} días`, 'error');
        Utils.hideLoading();
        return;
      }
      ...
      // Ocultar formulario y mostrar calendario usando clases
      const cuestionario = document.getElementById("cuestionarioEntreno");
      const calendario = document.getElementById("calendarioEntreno");
      if (cuestionario) cuestionario.classList.add("hidden");
      if (calendario) calendario.classList.remove("hidden");
  ...
  mostrarCalendario(sesiones) {
    ...
    if (!grid) {
      console.error('No se encontró el elemento calendarioGrid');
      return;
    }
    if (!sesiones || sesiones.length === 0) {
      console.warn('No hay sesiones para mostrar en el calendario');
      grid.innerHTML = '<div style="text-align:center; padding:20px;">No hay sesiones en este plan</div>';
      return;
    }
    // Asegurar que el formulario está oculto y el calendario visible
    const cuestionario = document.getElementById("cuestionarioEntreno");
    const calendario = document.getElementById("calendarioEntreno");
    if (cuestionario && !cuestionario.classList.contains('hidden')) {
  ...
  // ==================== FUNCIÓN CORREGIDA CON INTEGRACIÓN GPS ====================
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    // Limpiar elementos anteriores de GPS
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
          ...
          GPSTracker.onTrackingEnd = async (routeData) => {
            if (routeData.points.length >= 2) {
              ...
              statsDiv.innerHTML = `
                ...
                Distancia: ${(routeData.distanceMeters / 1000).toFixed(2)} km<br>
                Duración: ${Math.floor(routeData.durationSeconds / 60)} min ${routeData.durationSeconds % 60} seg<br>
                Ritmo medio: ${(routeData.durationSeconds / 60 / (routeData.distanceMeters / 1000)).toFixed(1)} min/km
              `;
      ...
      // Si la sesión ya está realizada y tiene ruta GPS, mostrarla
      if (realizada && sesion.gpsRoute) {
        ...
        mapContainer.style.marginTop = '16px';
        descripcion.parentNode.insertBefore(mapContainer, descripcion.nextSibling);
        await GPSTracker.displayRoute(mapContainer.id, sesion.gpsRoute.points);
  ...
  async guardarRutaGPS(diaIndex, routeData) {
    ...
    try {
      ...
      await planRef.update({
        [`gpsRoutes.${diaIndex}`]: routeData
      });
      
      // También actualizar en globalFeed si existe la entrada
  ...
  async mostrarUltimoPlanGuardado() {
    if (!AppState.currentUserId) {
      Utils.showToast("No hay usuario", 'error');
      return;
    }
    try {
      ...
      if (!ultimoPlanId) {
        Utils.showToast("No hay plan guardado", 'error');
        return;
      }
      const planDoc = await firebaseServices.db
        .collection('users')
  ...
  async borrarPlanGuardado() {
    if (!AppState.currentUserId) return;
    if (!AppState.isPremium) {
      Utils.showToast("Solo premium puede eliminar planes", 'error');
      return;
    }
    const confirmed = await Utils.confirm('ELIMINAR PLAN', "¿Eliminar plan guardado?");
    if (!confirmed) return;
    Utils.showLoading();
    try {
      ...
      // Ocultar calendario y mostrar formulario
      const cuestionario = document.getElementById("cuestionarioEntreno");
      const calendario = document.getElementById("calendarioEntreno");
      if (cuestionario) cuestionario.classList.remove("hidden");
  ...
  toggleCuestionario() {
    console.log('toggleCuestionario ejecutado');
    Utils.showToast('Cargando formulario...', 'info', 500);
    const cuestionario = document.getElementById('cuestionarioEntreno');
    if (cuestionario) {
      const wasHidden = cuestionario.classList.contains('hidden');
      console.log('Estado actual del formulario:', wasHidden ? 'oculto' : 'visible');
      if (wasHidden) {
        cuestionario.classList.remove('hidden');
        // Ocultar calendario si está visible
        const calendario = document.getElementById('calendarioEntreno');
        if (calendario && !calendario.classList.contains('hidden')) {
      ...
      console.error('No se encontró el elemento #cuestionarioEntreno');
...
console.log('✅ PlanGenerator v2.49 - con toggleCuestionario corregido');