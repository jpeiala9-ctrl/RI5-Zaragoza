...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      const planParaGuardar = {
        ...
        feedback: {}, // Nuevo campo para guardar feedback de sesiones
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad) {
    ...
    if (tipo === 'series' && sesionMatriz.repeticiones && sesionMatriz.distanciaSerie) {
      ...
      estructuraDetallada = `${calentamiento}' calentamiento Z1 + ` +
                           `${tiempoSeries}' series (${sesionMatriz.repeticiones}x${sesionMatriz.distanciaSerie}m) + ` +
      ...
      pasosDetallados = [
        {
          ...
          accion: `${calentamiento}' Z1`,
          porque: 'Activar sistema cardiovascular y preparar músculos'
        },
        {
          icono: '💪',
          titulo: 'SERIES',
          accion: `${tiempoSeries}' (${sesionMatriz.repeticiones}x${sesionMatriz.distanciaSerie}m)`,
          porque: 'Mejorar potencia aeróbica y velocidad'
        },
        {
          icono: '🧘',
          titulo: 'ENFRIAMIENTO',
          accion: `${enfriamiento}' Z1`,
          porque: 'Facilitar recuperación y eliminar lactato'
        }
      ];
    } else {
      ...
      switch(tipo) {
        case 'rodaje':
          ...
          pasoPrincipal = {
            ...
            accion: `${partePrincipal}' Z2`,
            porque: 'Desarrollar base aeróbica y eficiencia'
          };
          break;
        case 'tempo':
          ...
          pasoPrincipal = {
            icono: '⚡',
            titulo: 'TEMPO',
            accion: `${partePrincipal}' Z3-Z4`,
            porque: 'Elevar umbral de lactato y mejorar resistencia'
          };
          break;
        case 'largo':
          ...
          pasoPrincipal = {
            ...
            titulo: 'TIRADA LARGA',
            accion: `${partePrincipal}' Z2`,
            porque: 'Aumentar capacidad aeróbica y resistencia específica'
          };
          break;
        default:
          ...
          pasoPrincipal = {
            ...
            accion: `${partePrincipal}'`,
            porque: 'Mejorar condición física general'
          };
      }
  ...
  renderizarPagina(sesiones) {
    ...
    for (let pos = 0; pos < celdas.length; pos++) {
      ...
      if (sesion.tipo !== 'descanso' && sesion.detalle) {
        contenidoHtml = `<strong>${sesion.letra}</strong><div>${diaSemanaAbr}</div>`;
      } else {
        contenidoHtml = `<strong>D</strong><div>${diaSemanaAbr}</div>`;
      }
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // Pasos detallados usando pasosDetallados
      ...
      // Mostrar feedback existente si lo hay
  ...
  async guardarFeedback(diaIndex, valor) {
    ...
    try {
      ...
      // Actualizar UI (marcar el botón seleccionado)
      const feedbackContainer = document.getElementById("sesionFeedbackContainer");
...
console.log('✅ PlanGenerator v14.0 - STABLE (sin fecha de inicio)');