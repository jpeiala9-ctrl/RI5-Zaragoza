...
// 
// VERSIÓN: 5.1 - ÉLITE ADAPTATIVO | SIN FECHA DE INICIO | SIEMPRE LUNES
// ============================================================================
...
const PlanGenerator = {
  ...
  // ==========================================================================
  // REGLAS DE ENTRENADOR PROFESIONAL
  // ==========================================================================
  REGLAS: {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // Generar plan completo (SIEMPRE EMPIEZA EN LUNES)
      ...
      document.getElementById("resumenObjetivo").innerHTML = `
        <strong>${mapaDist[distanciaKey]}</strong> · ${diasEntreno.length} días · Nivel ${nivelReal}
      `;
  ...
  // ==========================================================================
  // GENERAR PLAN PERIODIZADO
  // ==========================================================================
  async generarPlanPeriodizado(semanasTotales, distancia, nivel, diasEntreno, datos, atleta) {
    const planCompleto = [];
    ...
    for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
      ...
      const semana = await this.generarSemana(
        ...
        diaGlobalCounter,
        atleta,
        planCompleto // Pasamos el plan completo para consultar semanas anteriores
      );
  ...
  // ==========================================================================
  async generarSemana(semanaGlobal, fase, nivel, distancia, diasEntreno, datos, factorVolumen, factorIntensidad, esDescarga, diaGlobalCounter, atleta, planCompleto) {
    const semana = [];
    ...
    // Asignar a días disponibles con REGLAS PROFESIONALES
    const tiposPorDia = await this.asignarTiposConReglas(
      diasEntreno, 
      tiposSemana, 
      fase, 
      semanaGlobal, 
  ...
  // ==========================================================================
  // ASIGNAR TIPOS CON REGLAS (NUEVA FUNCIÓN MEJORADA)
  // ==========================================================================
  async asignarTiposConReglas(diasEntreno, tiposSemana, fase, semanaGlobal, planCompleto) {
    const tiposPorDia = {};
    const tiposDisponibles = [...tiposSemana];
    const calidad = ['series', 'tempo', 'largo'];
    
    // Ordenar días para priorizar fin de semana para largos
    const diasOrdenados = [...diasEntreno].sort((a, b) => {
      // Priorizar domingo (7) y sábado (6) para largos
      if (a === 7) return -1;
      if (b === 7) return 1;
      if (a === 6) return -1;
      if (b === 6) return 1;
    ...
    // Encontrar el último largo en semanas anteriores
    let ultimoLargo = null;
    if (planCompleto && planCompleto.length > 0) {
      for (let i = planCompleto.length - 1; i >= 0; i--) {
        if (planCompleto[i].tipo === 'largo') {
          ultimoLargo = planCompleto[i].diaSemana;
          break;
        }
      }
    }

    // Primero, colocar el largo si existe
    const largoIndex = tiposDisponibles.indexOf('largo');
    if (largoIndex !== -1) {
      // Buscar el mejor día para largo que respete separación
      let diaLargo = null;
      
      for (const dia of diasOrdenados) {
        // Si no hay último largo, cualquier día vale
        if (!ultimoLargo) {
          diaLargo = dia;
          break;
        }
        
        // Verificar separación mínima de 3 días
        if (Math.abs(dia - ultimoLargo) >= 3) {
          diaLargo = dia;
          break;
        }
      }
      
      // Si encontramos día, asignamos largo
      if (diaLargo) {
        tiposPorDia[diaLargo] = 'largo';
        tiposDisponibles.splice(largoIndex, 1);
      } else {
        // No se puede poner largo esta semana, convertir a rodaje
        tiposDisponibles[largoIndex] = 'rodaje';
      }
    }

    // Asignar el resto de sesiones respetando no calidad consecutiva
    for (const dia of diasOrdenados) {
      // Saltar si ya tiene sesión asignada (el largo)
      if (tiposPorDia[dia]) continue;
      
      // Si no quedan tipos disponibles, rodaje
      ...
      // Si el día anterior fue calidad, este debe ser rodaje
      if (tipoAnterior && calidad.includes(tipoAnterior)) {
        tiposPorDia[dia] = 'rodaje';
        continue;
      }

      // Verificar día siguiente (para no poner calidad antes de descanso)
      const diaSiguiente = dia + 1;
      const hayCalidadPendiente = tiposDisponibles.some(t => calidad.includes(t));
      
      // Si mañana no es día de entreno y hoy es calidad, puede ser
      // Si mañana es día de entreno, hay que pensar
      if (diasEntreno.includes(diaSiguiente) && hayCalidadPendiente) {
        // Intenta no poner calidad si mañana también podría ser calidad
        const primerTipo = tiposDisponibles[0];
        if (calidad.includes(primerTipo)) {
          // Mejor poner un rodaje hoy y guardar calidad para mañana
          tiposPorDia[dia] = 'rodaje';
          continue;
        }
      }

      // Asignar el siguiente tipo disponible
      tiposPorDia[dia] = tiposDisponibles.shift();
    }
  ...
  personalizarSesion(sesionBase, fase, nivel, distancia, datos, factorVolumen, factorIntensidad, atleta) {
    ...
    // Calcular FC para cada paso (CADA PASO CON SU ZONA)
  ...
  // ==========================================================================
  // CALCULAR FC PARA UN PASO ESPECÍFICO
  // ==========================================================================
  ...
  // ==========================================================================
  // CALCULAR RITMO PARA UN PASO ESPECÍFICO
  // ==========================================================================
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      const pasos = [
        {
          ...
          texto: detalle.estructura?.calentamiento?.descripcion || 'Trote suave + ejercicios de técnica. Prepara músculos y activa la circulación.'
        },
        {
          numero: 2,
          ...
          texto: detalle.estructura?.principal?.descripcion || detalle.comentariosPrevios || 'Mantén el ritmo objetivo durante toda la sesión.'
        },
        {
          numero: 3,
          ...
          texto: detalle.estructura?.vueltaCalma?.descripcion || 'Trote muy suave para eliminar el lactato acumulado. Termina con estiramientos.'
        }
      ];
      ...
      descripcion.innerHTML = `
        ...
        <div style="margin-bottom: 25px;">
          ...
          <p style="margin: 0; font-size: 14px; line-height: 1.6; color: var(--text-primary);">
            ${detalle.comentariosPrevios || 'Sesión de entrenamiento. Concéntrate en mantener la técnica y el ritmo objetivo.'}
          </p>
        </div>
      ...
      descripcion.innerHTML = `
        ...
        <div style="margin-bottom: 15px;">
          ...
          <p style="font-style: italic; color: var(--text-secondary);">> Paseo de 20-30 minutos a ritmo muy suave.</p>
        </div>

        <div style="margin-bottom: 15px;">
          <h5 style="margin: 0 0 10px 0; font-size: 18px; font-weight: 500;">2. Movilidad y estiramientos</h5>
          <p style="font-style: italic; color: var(--text-secondary);">> 10-15 minutos de estiramientos suaves.</p>
        </div>

        <div style="margin-bottom: 15px;">
          <h5 style="margin: 0 0 10px 0; font-size: 18px; font-weight: 500;">3. Foam roller</h5>
          <p style="font-style: italic; color: var(--text-secondary);">> Automasaje suave en piernas.</p>
        </div>
      `;
  ...
  // ==========================================================================
  // MARCAR SESIÓN REALIZADA CON FEEDBACK
  // ==========================================================================
  ...
  // ==========================================================================
  // GUARDAR FEEDBACK Y ACTUALIZAR MODELO DE ATLETA
  // ==========================================================================
  async guardarFeedbackSesion(diaIndex, sesion, feedback) {
    ...
    try {
      ...
      modelo.feedbackPromedio = ultimas7.length > 0 ? sumaFeedback / ultimas7.length : 3;
  ...
  // ==========================================================================
  // ACTUALIZAR NAVEGACIÓN TRIMESTRAL
  // ==========================================================================
  ...
  // ==========================================================================
  // CAMBIAR TRIMESTRE (CORREGIDO - ERROR DE VARIABLE SOLUCIONADO)
  // ==========================================================================
    ...
    if (AppState.planActualId && AppState.currentUserId) {
      try {
        ...
        if (planDoc.exists) {
          ...
          // CORREGIDO: Antes ponía planCompleto.sesiones, ahora usa planData.sesiones
          if (planData?.sesiones) {
            this.renderizarPagina(planData.sesiones);
  ...
  async mostrarUltimoPlanGuardado() {
    ...
    try {
      ...
      document.getElementById("calendarioEntreno").style.display = "block";
      document.getElementById("cuestionarioEntreno").style.display = "none";
      this.mostrarCalendario(planData.sesiones);
    } catch (error) {
...
console.log('✅ PlanGenerator v5.1 - ÉLITE ADAPTATIVO | SIN FECHA INICIO | SIEMPRE LUNES');