// ==================== calendar.js - VERSIÓN FINAL CON MURO Y DEPURACIÓN ====================
// Versión: 2.35 - Incluye gestión de muro global con alertas para móvil
// MODIFICADO: Eliminados todos los alert() al marcar sesión, solo se usan toasts.
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== MARCAR SESIÓN REALIZADA (CON MURO GLOBAL, SIN ALERTS) ==========
  async marcarSesionRealizada(diaIndex, realizada) {
    ...
    try {
      ...
      // ========== GESTIÓN DEL MURO GLOBAL (SIN MODALES NI ALERTS) ==========
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          // Eliminado alert("📝 Guardando en muro...")
          ...
          try {
            ...
            // Eliminado alert("✅ Entrada guardada...")
          } catch (err) {
            console.error('Error al guardar en muro:', err);
            Utils.showToast('Error al publicar en el muro', 'error');
            // Eliminado alert("❌ Error al guardar...")
          }
        } else {
          // Eliminado alert("⚠️ No se pudo crear entrada...")
        ...
        if (wallEntryId) {
          try {
            ...
            // Eliminado alert("🗑️ Entrada eliminada del muro")
          } catch (err) {
...
console.log('✅ PlanGenerator v2.35 - Sin alertas al marcar sesión, solo toasts');