// ==================== calendar.js - VERSIÓN PRODUCCIÓN CON GAMIFICACIÓN Y GPS INTEGRADO ====================
// Versión: 2.52 - Marcado naranja en calendario funcionando correctamente
// ============================================================================
...
const PlanGenerator = {
  ...
  // ==================== FUNCIÓN RENDERIZAR PÁGINA (CORREGIDA PARA NARANJA) ====================
  renderizarPagina(sesiones, semanasTotales) {
    ...
    for (let pos = 0; pos < celdas.length; pos++) {
      ...
      if (estado && estado.realizada) {
        // CORRECCIÓN: si tipo es 'parcial' se pone 'realizado-parcial', si no 'realizado'
        if (estado.tipo === 'parcial') {
          realizadaClase = 'realizado-parcial';
        } else if (estado.tipo === 'completo') {
          realizadaClase = 'realizado';
        } else {
          // Compatibilidad con formato antiguo (booleano simple)
          realizadaClase = 'realizado';
        }
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // Mostrar texto adicional si es parcial
  ...
  // ==================== FUNCIÓN PRINCIPAL DE MARCADO (ACTUALIZADA v2.52) ====================
  async marcarSesionRealizada(diaIndex, realizada, tipo = 'completo', trackPoints = null, gpsDistance = null, silent = false) {
    ...
    try {
      ...
      // ACTUALIZACIÓN INMEDIATA DEL CALENDARIO (CORREGIDA)
      ...
      // Publicar en muro si hay trackPoints (GPS) O si es completo (sin GPS)
      ...
      if (debePublicar) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          // Calcular ritmo medio si hay GPS
          let ritmoMedio = null;
          ...
          const entry = {
            ...
            ciudad: userData.profile?.ciudad || 'Ubicación no disponible'
          };
          ...
          // Gamificación solo si es completo
...
console.log('✅ PlanGenerator v2.52 - Marcado naranja en calendario funcionando correctamente');