// ==================== calendar.js - VERSIÓN FINAL COMPLETA ====================
// - Calendario siempre visible al recargar
// - Sesiones forzadas (naranja) persistentes
// - GPS con pantalla negra y suavizado de puntos
// - Marcadores sutiles y mapa CartoDB Voyager
// ============================================================================
...
const PlanGenerator = {
  ...
  _smoothPoints(rawPoints) {
    ...
    // Eliminar puntos con velocidad anómala
    const filtered = [];
    for (let i = 0; i < rawPoints.length; i++) {
      ...
      if (speedPrev > 12 || speedPrev < 0.3 || speedNext > 12 || speedNext < 0.3) {
        console.log(`🗑️ Punto ${i} descartado por velocidad anómala`);
        continue;
      }
      filtered.push(curr);
    }
  ...
  renderizarPagina(sesiones, semanasTotales) {
    ...
    for (let i = 0; i < sesiones.length; i++) {
      ...
      if (!sesion) continue;
      const semanaSesion = Math.floor((i) / 7);
    ...
    grid.innerHTML = html;
    console.log(`✅ Calendario renderizado: ${celdas.length} celdas, ${grid.children.length} elementos`);
