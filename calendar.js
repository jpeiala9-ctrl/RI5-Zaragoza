// ==================== calendar.js - VERSIÓN COMPLETA CON SOPORTE PARA GPS REAL ====================
// Versión: 2.48 - Ahora acepta distancia y duración reales para gamificación
// ============================================================================
...
const PlanGenerator = {
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      if (window.GPSTracker) {
        ...
        // Crear sesión limpia (sin fuerza)
        ...
        if (sesion.tieneFuerza && sesion.detalle && sesion.detalle.pasosDetallados) {
          ...
          // Recalcular duración total sin fuerza
  ...
  // ==================== FUNCIÓN MODIFICADA (acepta distancia y duración reales) ====================
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          const entry = {
            ...
            duration: Math.floor(duracionUsadaMs / 60000), // minutos reales o estimados
            distancia: distancia,
            tss: tss,
          ...
          // Si hay datos reales, marcamos que tiene GPS (para mostrar minimapa después)
          if (realDistance !== null && realDurationMs !== null) {
            entry.hasGPS = true;
            // Los puntos GPS se añadirán más tarde desde GPSTracker, pero ya dejamos la marca
          }
          ...
          // Sumar a gamificación usando la distancia REAL (si existe) o la estimada
          if (window.Gamification) {
            try {
              const oldData = await Gamification.getData(AppState.currentUserId);
              const oldLevel = oldData.level;
              // Pasar las métricas ya calculadas (con distancia real o estimada)
        ...
        if (window.Gamification && sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          try {
            // Para desmarcar, necesitamos saber qué distancia se usó al marcar.
            let distanciaRemovida = null;
            if (wallEntryId) {
              const wallDoc = await firebaseServices.db.collection('globalFeed').doc(wallEntryId).get();
              if (wallDoc.exists) {
                distanciaRemovida = wallDoc.data().distancia;
              }
            }
            if (distanciaRemovida === null) {
              const metricas = this.calcularMetricasSesion(sesion);
  ...
  // Función auxiliar para calcular TSS a partir de distancia y duración reales (aproximación)
  calcularTSSdesdeReal(distanciaKm, duracionMs, sesion) {
    const duracionHoras = duracionMs / (1000 * 60 * 60);
    if (duracionHoras <= 0) return 0;
    const ritmoBase = AppState.lastRitmoBase || 5; // min/km
...
console.log('✅ PlanGenerator v2.48 - Soporte para distancia y tiempo real de GPS');