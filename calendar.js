...
// Versión: 3.8 - DEFINITIVA CON DETALLE PROFESIONAL DE SESIONES
//
// CARACTERÍSTICAS:
...
// - Visualización profesional de cada sesión con tiempo, distancia y TSS
...
const PlanGenerator = {
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, faseActual, semanaEnFase, libreriaPerfil, datos) {
    ...
    if (tipo === 'series' && sesionMatriz.repeticiones && sesionMatriz.distanciaSerie) {
      ...
      // Aplicar límite de cordura si es necesario
  ...
  // ==========================================================================
  // DETALLE DE SESIÓN (MODAL) - VERSIÓN PROFESIONAL
  // ==========================================================================
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // Calcular distancia estimada y TSS
      const distancia = this.calcularDistanciaDesdeSesion(sesion);
      const tss = this.calcularTSS(sesion);

      // Formatear tiempo total (duración en minutos a HH:MM:SS)
      const tiempoTotal = this.formatearTiempo(sesion.duracion);

      // Generar los pasos a partir de la estructura
      const pasosHTML = this.generarPasosSesion(sesion);

      let descHtml = `
        <div class="sesion-resumen">
          <div><span>🕒</span> ${tiempoTotal}</div>
          <div><span>📏</span> ${distancia.toFixed(2)} km</div>
          <div><span>⚡</span> ${tss} TSS</div>
        </div>
        <div class="sesion-comentarios">
          <h4>COMENTARIOS PREVIOS</h4>
          <p>${sesion.detalle.descripcion || `Sesión de ${sesion.tipo} en fase ${AppState.planGeneradoActual?.fase || 'base'}.`}</p>
        </div>
  ...
  // ==========================================================================
  // FUNCIONES AUXILIARES PARA EL DETALLE PROFESIONAL
  // ==========================================================================
  ...
  calcularDistanciaDesdeSesion(sesion) {
    if (!sesion.detalle || !sesion.detalle.ritmoObjetivo) return 0;
    // Extraer el ritmo en min/km (formato "MM:SS")
    const ritmoStr = sesion.detalle.ritmoObjetivo;
    const match = ritmoStr.match(/(\d+):(\d+)/);
    if (!match) return 0;
    const ritmoMin = parseInt(match[1]) + parseInt(match[2]) / 60;
    if (ritmoMin <= 0) return 0;

    // Intentar obtener la duración de la parte principal (si es continua, usar toda)
    let duracionPrincipal = sesion.duracion;
    // Si la estructura tiene calentamiento y enfriamiento, intentar extraer la parte principal
    const estructura = sesion.detalle.estructura || '';
    const matchCal = estructura.match(/(\d+)' calentamiento/i);
    const matchEnf = estructura.match(/(\d+)' enfriamiento/i);
    if (matchCal && matchEnf) {
      const cal = parseInt(matchCal[1]);
      const enf = parseInt(matchEnf[1]);
      duracionPrincipal = sesion.duracion - cal - enf;
    } else {
      // Si hay varias partes con '+', tomar la de mayor duración como principal
      ...
      partes.forEach(p => {
        ...
        if (m) {
          ...
          if (d > maxDuracion) maxDuracion = d;
        }
      });
      if (maxDuracion > 0) duracionPrincipal = maxDuracion;
    }

    return duracionPrincipal / ritmoMin;
  },

  calcularTSS(sesion) {
    if (!sesion.detalle || !sesion.detalle.zona) return 0;
    const zona = sesion.detalle.zona;
    // Factores de intensidad aproximados por zona (IF)
    let ifactor = 0.7; // por defecto Z2
    if (zona.includes('Z1')) ifactor = 0.6;
    else if (zona.includes('Z2')) ifactor = 0.7;
    else if (zona.includes('Z3')) ifactor = 0.85;
    else if (zona.includes('Z4')) ifactor = 0.95;
    else if (zona.includes('Z5')) ifactor = 1.05;
    else if (zona.includes('Z6')) ifactor = 1.15;

    // TSS simplificado = duración (min) * IF^2
    const tss = sesion.duracion * ifactor * ifactor;
    return Math.round(tss);
  },

  generarPasosSesion(sesion) {
    if (!sesion.detalle || !sesion.detalle.estructura) return '<p>No hay detalles de pasos</p>';
    ...
    // Dividir la estructura en partes (separadas por '+')
    const partes = estructura.split('+').map(p => p.trim());
    let pasos = [];
    let pasoIndex = 1;

    partes.forEach(parte => {
      // Buscar patrones como "10' calentamiento" o "20' Z2" o "5x1000m"
      const matchTiempo = parte.match(/^(\d+)'/);
      const matchSeries = parte.match(/(\d+)x(\d+)m/i);
      if (matchTiempo) {
        const minutos = parseInt(matchTiempo[1]);
        const descripcion = parte.replace(/^\d+'/, '').trim() || 'continuo';
        let zonaPaso = zona;
        let ritmoPaso = ritmo;
        // Si la descripción contiene una zona, usarla
        if (descripcion.includes('Z1')) zonaPaso = 'Z1';
        else if (descripcion.includes('Z2')) zonaPaso = 'Z2';
        else if (descripcion.includes('Z3')) zonaPaso = 'Z3';
        else if (descripcion.includes('Z4')) zonaPaso = 'Z4';
        else if (descripcion.includes('Z5')) zonaPaso = 'Z5';

        pasos.push(`
          <div class="paso">
            <div class="paso-titulo">${pasoIndex++}. ${descripcion.charAt(0).toUpperCase() + descripcion.slice(1)}</div>
            <div class="paso-detalle">
              <span>⏱️ ${minutos} min</span>
              <span>❤️ ${fc}</span>
              <span>📈 ${zonaPaso}</span>
            </div>
            <div class="paso-ritmo">🎯 Ritmo: ${ritmoPaso}</div>
          </div>
        `);
      } else if (matchSeries) {
        const repeticiones = parseInt(matchSeries[1]);
        const distancia = parseInt(matchSeries[2]);
        const descripcion = parte.replace(matchSeries[0], '').trim() || 'series';
        // Para series, podemos mostrar un sub-paso o agrupar
        pasos.push(`
          <div class="paso">
            <div class="paso-titulo">${pasoIndex++}. ${descripcion}</div>
            <div class="paso-detalle">
              <span>🔁 ${repeticiones} x ${distancia}m</span>
              <span>❤️ ${fc}</span>
              <span>📈 ${zona}</span>
            </div>
            <div class="paso-ritmo">🎯 Ritmo: ${ritmo}</div>
          </div>
        `);
      } else {
        // Parte sin tiempo (ej: "enfriamiento" suelto) - lo ignoramos o lo tratamos como descanso
        pasos.push(`
          <div class="paso">
            <div class="paso-titulo">${pasoIndex++}. ${parte}</div>
          </div>
        `);
      }
    });
...
console.log('✅ PlanGenerator v3.8 - Detalle profesional de sesiones');