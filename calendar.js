...
// Versión: 5.10 - CORREGIDO: alineación de días por semana y visualización del día del mes
// ============================================================================
...
const PlanGenerator = {
  ...
  renderizarPagina(sesiones) {
    ...
    const semanaInicio = Math.floor(inicioPagina / 7);
    const semanaFin = Math.floor((finPagina - 1) / 7);
    ...
    // Crear mapa de sesiones por semana y día
    const sesionesMap = new Map();
    for (let i = inicioPagina; i < finPagina; i++) {
      const sesion = sesiones[i];
      if (!sesion) continue;
      const semana = sesion.semana;
      const dia = sesion.diaSemana;
      if (!sesionesMap.has(semana)) sesionesMap.set(semana, new Map());
      sesionesMap.get(semana).set(dia, { sesion, index: i });
    }

    for (let semana = semanaInicio; semana <= semanaFin; semana++) {
      const semanaMap = sesionesMap.get(semana) || new Map();
      for (let dia = 1; dia <= 7; dia++) {
        const entrada = semanaMap.get(dia);
        if (entrada) {
          const sesion = entrada.sesion;
          const i = entrada.index;
          ...
          // Día sin sesión (no debería ocurrir, pero por seguridad)
...
console.log('✅ PlanGenerator v5.10 - Calendario alineado por día de la semana y fecha real');