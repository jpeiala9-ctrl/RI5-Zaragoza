// ==================== calendar.js - VERSIÓN CORREGIDA (PERSISTENCIA NARANJA + GAMIFICACIÓN CONSISTENTE) ====================
// GPS con botón COMENZAR, marcadores sutiles, CartoDB Voyager, corrección de persistencia de sesiones forzadas y gamificación
// ============================================================================
...
const PlanGenerator = {
  ...
  async marcarSesionRealizada(diaIndex, realizada, routeData = null) {
    ...
    try {
      ...
      if (realizada) {
        ...
        // Desmarcar: eliminar del muro y restar de gamificación usando la misma distancia que se sumó
        const planData = planDoc.data();
      ...
      // Forzar recarga del perfil para actualizar gamificación
      if (window.Profile) {
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      if (!realizada) {
        console.log('🟢 Mostrando controles GPS para sesión no realizada');
        const gpsControls = document.createElement('div');
        gpsControls.className = 'gps-controls';
        gpsControls.style.cssText = 'display: flex; flex-direction: column; gap: 12px; margin: 16px 0; align-items: center;';
        ...
        startBtn.style.cssText = `
          ...
          box-shadow: 0 4px 8px rgba(0,0,0,0.2);
        `;
        startBtn.innerHTML = 'Iniciar sesión';
        
        const wakeLockMsg = document.createElement('div');
        wakeLockMsg.style.cssText = 'font-size: 12px; color: var(--text-secondary); text-align: center; margin: 4px 0 0 0;';
        wakeLockMsg.innerHTML = '🔋 La pantalla permanecerá encendida mientras grabes. No bloquees el teléfono manualmente o el GPS se detendrá.';
        ...
        startBtn.onclick = async () => {
          ...
          try {
            ...
            console.warn('Wake Lock no soportado:', err);
          }
          
          let mapContainer = document.getElementById('gpsMapContainer');
          if (!mapContainer) {
            mapContainer = document.createElement('div');
            mapContainer.id = 'gpsMapContainer';
          ...
          const lat = position ? position.coords.latitude : 40.416775;
          const lng = position ? position.coords.longitude : -3.703790;
          const map = L.map(mapContainer.id, { attributionControl: false }).setView([lat, lng], 16);
          ...
          const updateRoute = () => {
            ...
            if (points.length >= 2) {
              polyline = L.polyline(points, { color: '#C9A96E', weight: 4, opacity: 0.8 }).addTo(map);
          ...
          watchId = navigator.geolocation.watchPosition(
            (pos) => {
              ...
              updateRoute();
              userMarker.setLatLng([newPoint.lat, newPoint.lng]);
            },
          ...
          startBtn.disabled = true;
          startBtn.innerHTML = '⏳ Grabando...';
          const stopBtn = document.createElement('button');
          stopBtn.className = 'action-button';
          stopBtn.style.cssText = `
            width: auto;
            background: linear-gradient(135deg, var(--zone-5), #a55a5a);
            border: none;
            color: white;
            font-weight: bold;
            box-shadow: 0 4px 8px rgba(0,0,0,0.2);
          `;
          stopBtn.innerHTML = 'Finalizar sesión';
          stopBtn.onclick = async () => {
            ...
            if (porcentaje <= 10) {
              ...
              Utils.showToast('Usa el botón "Marcar como realizada" para finalizar la sesión (se marcará en naranja).', 'info', 3000);
            }
          ...
          gpsControls.appendChild(stopBtn);
          
          const startTime = Date.now();
          mapContainer._leaflet_map = map;
        };
        
        gpsControls.appendChild(startBtn);
        gpsControls.appendChild(wakeLockMsg);
        descripcion.parentNode.insertBefore(gpsControls, descripcion.nextSibling);
      }
...
console.log('✅ PlanGenerator v2.78 - Corrección definitiva de gamificación y persistencia naranja');