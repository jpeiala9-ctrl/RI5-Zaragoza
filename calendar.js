// ==================== calendar.js - VERSIÓN FINAL 2.25 ====================
// - Adaptación dinámica de series con números redondos:
//   * Distancias de serie redondeadas a múltiplos de 50 m
//   * Tiempos de esfuerzo y recuperación redondeados a múltiplos de 5 o 10 segundos
// - Descripciones solo con zonas (sin ritmos absolutos)
// - Conserva toda la lógica profesional de periodización, feedback, etc.
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== FUNCIONES DE REDONDEO PARA NÚMEROS LIMPIOS ==========
  _redondearDistancia(distanciaMetros) {
    // Redondea a múltiplos de 50m, con mínimo 50m
    if (distanciaMetros < 50) return 50;
    const redondeado = Math.round(distanciaMetros / 50) * 50;
    return Math.min(redondeado, 2000); // máximo 2000m (2km)
  },

  _redondearTiempo(segundos) {
    // Redondea a múltiplos de 5 segundos (para esfuerzos y recuperaciones)
    if (segundos < 10) return 10;
    let redondeado = Math.round(segundos / 5) * 5;
    // Para recuperaciones largas, múltiplo de 15 segundos (más natural)
    if (segundos > 60) redondeado = Math.round(segundos / 15) * 15;
    return redondeado;
  },

  _redondearDuracionPartePrincipal(segundos) {
    // Redondea el total de la parte principal a múltiplos de 15 segundos
    return Math.round(segundos / 15) * 15;
  },

  // ========== OBTENER RITMO DESDE ZONAS (solo uso interno) ==========
  ...
  // ================== SESIONES AVANZADAS DE SERIES CON NÚMEROS REDONDOS ==================
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    switch(estructura) {
      case 'piramide': {
        ...
        // Redondear distancias a múltiplos de 50m
      ...
      case 'rotas': {
        ...
        if (factor < 0.8) {
          // Reducir distancia de serie y recuperaciones
        ...
        // Asegurar que distanciaSerie y recIntra sean redondos
        distanciaSerie = this._redondearDistancia(distanciaSerie);
        recIntra = this._redondearTiempo(recIntra);
...
console.log('✅ PlanGenerator v2.25 - Series con distancias y tiempos redondeados, solo zonas.');