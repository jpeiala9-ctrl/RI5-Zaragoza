...
// Versión: 3.0 - Utiliza ENTRENAMIENTOS_DB (1.344 entrenamientos) con periodización profesional
...
const PlanGenerator = {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          ...
          // Crear sesión personalizada (AHORA USA LA MATRIZ)
  ...
  // --- FUNCIÓN CLAVE: Obtiene sesiones REALES de la matriz ---
  async crearSesionDesdeMatriz(sesionBase, esActivo, faseActual, semanaEnFase, libreriaPerfil, datos) {
    ...
    // Adaptar la estructura con la duración personalizada
    let estructura = sesionMatriz.estructura;
    if (estructura) {
      // Reemplazar números de minutos por la duración calculada
      estructura = estructura.replace(/\b\d{2,3}\b/g, duracion.toString());
    } else {
  ...
  aplicarFactorLargo(sesion, progresion, semanaEnFase) {
    ...
    return {
      ...
      detalle: {
        ...sesion.detalle,
        duracion: nuevaDuracion,
        estructura: sesion.detalle.estructura.replace(/\b\d{2,3}\b/g, nuevaDuracion.toString()),
...
console.log('✅ PlanGenerator v3.0 - Usando matriz completa de 1.344 entrenamientos');