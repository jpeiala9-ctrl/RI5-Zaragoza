// ==================== calendar.js - VERSIÓN FINAL 2.16 (COMPLETA) ====================
// - Periodización ondulatoria (oscilación semanal de intensidad/volumen)
// - Estructuras avanzadas de series (pirámides, series rotas, fartlek, cuestas)
// - Simulación de competición en fases específica y pico
// - Límites por distancia, volumen ajustado para 2k/5k, control de repetición y fuerza variada
// ============================================================================
...
const PlanGenerator = {
  ...
  // Patrones ondulatorios: [intensidad, volumen] relativos a la fase
  ONDULACION_PATRONES: [
    { intensidad: 0.8, volumen: 1.2 },  // Semana de volumen alto, intensidad baja
    { intensidad: 1.0, volumen: 1.0 },  // Semana equilibrada
    { intensidad: 1.2, volumen: 0.8 },  // Semana de intensidad alta, volumen bajo
    { intensidad: 0.9, volumen: 0.9 }   // Semana de descarga
  ],
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        if (minutosPorTipo.largo > 0) {
          ...
          const esSimulacion = this.debeHacerSimulacion(fase, semanaGlobal, semanasTotales, distancia, nivelActual);
        ...
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          if (!diasEntreno.includes(diaSemana)) {
            ...
            const sesion = await this.crearSesionDesdeMatriz(
              ...
              semanaEnFase,
              nivelActual,
              { modalidad, distancia, ritmoBase, fcUmbral, semanasTotales, semanaGlobal },
              ajusteVolumen,
              ajusteIntensidad,
              minutos
            );
  ...
  debeHacerSimulacion(fase, semanaGlobal, semanasTotales, distancia, nivel) {
    ...
    const semanaEnFase = semanaGlobal - this.obtenerInicioFase(fase, distancia, semanasTotales);
    return semanaEnFase % 3 === 0 && semanaEnFase >= 2 && semanaEnFase <= (fase === 'ESPECIFICA' ? 6 : 4);
  },

  obtenerInicioFase(fase, distancia, semanasTotales) {
    const fases = this.generarFases(semanasTotales, 'mejorar', distancia);
    for (let f of fases) {
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    const { modalidad, distancia, ritmoBase, fcUmbral, semanasTotales, semanaGlobal } = datos;
  ...
  crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    const ritmoRapido = Utils.formatR(ritmoBaseAjustado * 0.95);
    const ritmoModerado = Utils.formatR(ritmoBaseAjustado * 1.1);
...
console.log('✅ PlanGenerator v2.16 - Con ondulación, series avanzadas y simulación de competición');