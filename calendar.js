...
const PlanGenerator = {
    ...
    switch(estructura) {
      case 'piramide': {
        ...
        if (factor >= 0.8 && factor <= 1.2) {
          ...
          // Tope de 3 pirámides completas seguidas. Sin este límite, una
          // sesión larga de verdad (p.ej. avanzado + maratón en fase
          // ESPECIFICA, con hasta 120' de serie disponibles) da un
          // factor > 7 y el código repetía la pirámide entera factor
          // veces -- 7 pirámides seguidas = 49 repeticiones en una sola
          // sesión, algo que ni un atleta de élite haría. Con el tope,
          // la parte principal real (recalculada más abajo a partir de
          // las distancias que de verdad se han metido) sale más corta
          // que la duración nominal del día en sesiones muy largas -- se
          // deja así a propósito en vez de forzar contenido de más.
          const repeticiones = Math.min(3, Math.floor(factor));
          if (repeticiones >= 2) {
            distancias = [];
  ...
  _mejorarDistribucionCalidad(tiposPorDia, diasEntreno, diaLargo) {
    // Reescrito para no arrastrar una lista de "días de calidad" que se
    // queda desactualizada en cuanto se mueve o rebaja una sesión. La
    // versión anterior (y un primer intento de arreglo) calculaban esa
    // lista UNA vez al principio; en cuanto una sesión se movía a otro
    // día, las comprobaciones siguientes seguían comparando contra su
    // posición vieja -- y si el movimiento la colocaba MÁS TARDE que la
    // siguiente sesión de calidad de la lista, la comparación de
    // adyacencia dejaba de tener sentido (comparaba "día 4" contra "día
    // 3" como si 3 fuera el siguiente a 4) y una pareja de días
    // consecutivos de verdad se quedaba sin corregir. Ahora cada pasada
    // vuelve a leer el estado ACTUAL de tiposPorDia (ya ordenado, porque
    // diasEntreno siempre va de día 1 a 7) y corrige solo la primera
    // pareja consecutiva que encuentra, hasta que no quede ninguna o se
    // agoten los intentos (nunca más pasadas que días de entreno, así
    // que no hay riesgo de bucle infinito).
    for (let intento = 0; intento < diasEntreno.length; intento++) {
      const diasCalidad = diasEntreno.filter(d => {
        const s = tiposPorDia[d];
        return s && (s.tipo === 'tempo' || s.tipo === 'series');
      });

      let parAResolver = null;
      for (let i = 0; i < diasCalidad.length - 1; i++) {
        const actual = diasCalidad[i];
        const siguiente = diasCalidad[i + 1];
        if (siguiente === actual + 1 || (actual === 7 && siguiente === 1)) {
          parAResolver = { actual, siguiente };
          break;
        }
      }
      if (!parAResolver) break; // sin días de calidad consecutivos: terminado

      const { actual, siguiente } = parAResolver;
      let nuevoDia = null;
      for (let d = actual + 1; d <= diaLargo; d++) {
        if (!tiposPorDia[d] && diasEntreno.includes(d)) { nuevoDia = d; break; }
      }
      if (nuevoDia) {
    ...
    for (let dia of diasEntreno) {
      ...
      if (sesion && (sesion.tipo === 'tempo' || sesion.tipo === 'series') && dia > diaLargo) {
        ...
        if (nuevoDia) {
          tiposPorDia[nuevoDia] = tiposPorDia[dia];
          delete tiposPorDia[dia];
        }
      }
    }
  },
