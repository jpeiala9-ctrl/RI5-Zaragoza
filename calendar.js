...
// Versión: 5.8 - Fechas correctas día a día + persistencia con sesionesRealizadas
// ============================================================================
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      const planCompleto = [];
      let fechaActual = new Date(fechaInicio); // ← Se irá incrementando día a día
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          ...
          semana.push({
            ...
            semana: semanaGlobal,
            diaSemana: diaSemana,
            fecha: new Date(fechaActual),   // ← Fecha real del día
            ...sesion
          });
...
console.log('✅ PlanGenerator v5.8 - Fechas correctas día a día + persistencia con sesionesRealizadas');