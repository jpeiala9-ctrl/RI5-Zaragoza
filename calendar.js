...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // Determinar nivel REAL basado en progresión
      const nivelReal = this.determinarNivelReal(nivelInput, semanasTotales);
      ...
      document.getElementById("resumenObjetivo").innerHTML = `
        <strong>${mapaDist[distanciaKey]}</strong> · ${diasEntreno.length} DÍAS/SEMANA · NIVEL ${nivelReal.toUpperCase()}<br>
        <span style="color: var(--text-secondary); font-size: 13px;">
          OBJ: ${objetivo.toUpperCase()} · VOLUMEN: ${configDist.volumenBase[nivelReal]}km/sem
        </span>
      `;
  ...
  // ==========================================================================
  async generarPlanPeriodizado(semanasTotales, distancia, nivel, diasEntreno, datos, factorProgresion) {
    const planCompleto = [];
    ...
    for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
      ...
      const factorVolumen = this.calcularFactorVolumen(mesociclo, semanaEnMesociclo, esDescarga);
      const factorIntensidad = this.calcularFactorIntensidad(mesociclo, semanaEnMesociclo, esDescarga);
  ...
  // ==========================================================================
  // GENERAR UNA SEMANA DE ENTRENAMIENTO
  // ==========================================================================
  async generarSemana(semanaGlobal, fase, nivel, distancia, diasEntreno, datos, factorVolumen, factorIntensidad, esDescarga, diaGlobalCounter) {
    const semana = [];
    ...
    const indexLargo = diasDisponibles.indexOf(diaLargo);
    if (indexLargo !== -1 && tiposSemana.includes('largo')) {
      diasDisponibles.splice(indexLargo, 1);
    }

    // Mapa de tipos por día
    const tiposPorDia = {};
    
    // Asignar largo al día óptimo
    ...
    for (const dia of diasDisponibles) {
      if (tipoIndex < tiposSemana.length) {
        // Saltar si ya es largo
        if (tiposPorDia[dia] === 'largo') continue;
        
        let tipo = tiposSemana[tipoIndex];
        // Si es cuestas, tratar como series pero con sesión específica
  ...
  // ==========================================================================
  // PERSONALIZAR SESIÓN CON DATOS DEL USUARIO
  // ==========================================================================
  personalizarSesion(sesionBase, fase, nivel, distancia, datos, factorVolumen, factorIntensidad) {
    ...
    sesion.estructura.principal.duracionReal = duracionPrincipal;
    sesion.estructura.principal.detalle = sesion.estructura.principal.detalle.replace(
      /\d+'/g, 
      duracionPrincipal + "'"
    );
    
    // Añadir métricas personalizadas
    sesion.duracionTotal = duracionTotal;
    sesion.zonaPrincipal = zonaPrincipal;
    sesion.ritmoObjetivo = Utils.formatR(ritmoObjetivo);
    sesion.fcObjetivo = fcObjetivo;
    
    // Calcular TSS aproximado
    const ifactor = this.calcularIFactor(zonaPrincipal);
    sesion.tss = Math.round(duracionTotal * ifactor * ifactor);
    
    return sesion;
  },

  // ==========================================================================
  // CALCULAR FC COHERENTE CON LA ZONA (NUEVA FUNCIÓN MEJORADA)
  // ==========================================================================
  ...
  calcularRitmoObjetivo(ritmoBase, zona, factorIntensidad) {
    ...
    return ritmoBase * factor * (0.95 + (factorIntensidad * 0.05));
  },
  ...
  // ==========================================================================
  // MOSTRAR CALENDARIO (VERSIÓN VISUAL - SOLO LETRAS)
  // ==========================================================================
  ...
  renderizarPagina(sesiones) {
    ...
    for (let pos = 0; pos < celdas.length; pos++) {
      ...
      // SOLO LETRA - sin emoticonos, sin texto adicional
      html += `<div class="calendario-dia ${sesion.color} ${realizada}" data-index="${sesion.diaGlobal}"${faseIndicator}>
        <span style="font-size: 20px; font-weight: 400;">${sesion.letra}</span>
      </div>`;
    }
  ...
  // ==========================================================================
  // ABRIR DETALLE DE SESIÓN (TARJETA PERFECTA)
  // ==========================================================================
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      titulo.innerHTML = `
        <div style="margin-bottom: 15px;">
          <span style="font-size: 14px; color: var(--accent-yellow); letter-spacing: 2px;">${faseInfo.nombre} · SEMANA ${sesion.semana}</span>
          <h3 style="font-size: 24px; margin: 5px 0 0 0; padding: 0; border: none;">${detalle.nombre || sesion.tipo.toUpperCase()}</h3>
        </div>
      `;

      // Métricas principales
      const tiempoFormateado = this.formatearTiempo(detalle.duracionTotal || sesion.duracion || 50);
      ...
      descripcion.innerHTML = `
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; margin-bottom: 25px;">
          <div style="background: var(--bg-primary); border-radius: 12px; padding: 12px; text-align: center;">
            <div style="font-size: 20px; color: var(--accent-blue);">⏱️</div>
            <div style="font-size: 16px; font-weight: 500;">${tiempoFormateado}</div>
            <div style="font-size: 11px; color: var(--text-secondary);">DURACIÓN</div>
          </div>
          <div style="background: var(--bg-primary); border-radius: 12px; padding: 12px; text-align: center;">
            <div style="font-size: 20px; color: var(--accent-blue);">❤️</div>
            <div style="font-size: 16px; font-weight: 500;">${detalle.fcObjetivo || '—'}</div>
            <div style="font-size: 11px; color: var(--text-secondary);">FC OBJETIVO</div>
          </div>
          <div style="background: var(--bg-primary); border-radius: 12px; padding: 12px; text-align: center;">
            <div style="font-size: 20px; color: var(--accent-blue);">⚡</div>
            <div style="font-size: 16px; font-weight: 500;">${detalle.ritmoObjetivo || '—'}</div>
            <div style="font-size: 11px; color: var(--text-secondary);">RITMO</div>
          </div>
          <div style="background: var(--bg-primary); border-radius: 12px; padding: 12px; text-align: center;">
            <div style="font-size: 20px; color: var(--accent-blue);">📊</div>
            <div style="font-size: 16px; font-weight: 500;">${detalle.tss || 0}</div>
            <div style="font-size: 11px; color: var(--text-secondary);">TSS</div>
          </div>
        </div>

        <!-- OBJETIVO FISIOLÓGICO -->
        <div style="background: var(--bg-primary); border-radius: 12px; padding: 20px; margin-bottom: 20px;">
          <h4 style="margin: 0 0 10px 0; color: var(--accent-yellow); font-size: 14px;">🎯 OBJETIVO FISIOLÓGICO</h4>
          <p style="margin: 0; font-size: 15px; line-height: 1.5;">${detalle.objetivoFisiologico || 'Mejora del rendimiento aeróbico'}</p>
          <p style="margin: 10px 0 0 0; font-size: 13px; color: var(--text-secondary);">
            Zona principal: <strong>${detalle.zonaPrincipal || 'Z2'}</strong> · 
            Percepción: <strong>${this.ZONAS_FISIOLOGICAS[detalle.zonaPrincipal?.split('-')[0]]?.percepcion || 'Controlado'}</strong>
          </p>
        </div>

        <!-- ESTRUCTURA PASO A PASO (CON EXPLICACIONES) -->
        <div style="margin-bottom: 20px;">
          <h4 style="margin: 0 0 15px 0; color: var(--accent-yellow); font-size: 14px;">📋 EJECUCIÓN DE LA SESIÓN</h4>
          
          <!-- PASO 1: CALENTAMIENTO -->
          <div style="background: var(--bg-primary); border-radius: 12px; padding: 15px; margin-bottom: 10px;">
            <div style="display: flex; align-items: center; margin-bottom: 10px;">
              <span style="background: var(--accent-blue); color: var(--bg-primary); width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 12px; font-weight: 500;">1</span>
              <span style="font-weight: 500; font-size: 16px;">CALENTAMIENTO (${detalle.estructura?.calentamiento?.duracion || 15}')</span>
            </div>
            <div style="margin-left: 40px;">
              <p style="margin: 0 0 8px 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 POR QUÉ:</span> ${detalle.estructura?.calentamiento?.porQue || 'Preparar el sistema cardiovascular y muscular'}</p>
              <p style="margin: 0 0 8px 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 CÓMO:</span> ${detalle.estructura?.calentamiento?.como || detalle.estructura?.calentamiento?.detalle || 'Trote suave progresivo + ejercicios de técnica'}</p>
              <p style="margin: 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 PARA QUÉ:</span> ${detalle.estructura?.calentamiento?.paraQue || 'Aumentar temperatura muscular y prevenir lesiones'}</p>
            </div>
          </div>
          
          <!-- PASO 2: PARTE PRINCIPAL -->
          <div style="background: var(--bg-primary); border-radius: 12px; padding: 15px; margin-bottom: 10px; border-left: 4px solid var(--accent-blue);">
            <div style="display: flex; align-items: center; margin-bottom: 10px;">
              <span style="background: var(--accent-blue); color: var(--bg-primary); width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 12px; font-weight: 500;">2</span>
              <span style="font-weight: 500; font-size: 16px;">PARTE PRINCIPAL (${detalle.estructura?.principal?.duracionReal || detalle.estructura?.principal?.duracionBase || 40}')</span>
            </div>
            <div style="margin-left: 40px;">
              <p style="margin: 0 0 8px 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 POR QUÉ:</span> ${detalle.estructura?.principal?.porQue || 'Estimular las adaptaciones específicas'}</p>
              <p style="margin: 0 0 8px 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 CÓMO:</span> ${detalle.estructura?.principal?.como || detalle.estructura?.principal?.detalle || 'Trabajo específico según objetivo'}</p>
              <p style="margin: 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 PARA QUÉ:</span> ${detalle.estructura?.principal?.paraQue || 'Mejorar capacidad aeróbica o umbral'}</p>
            </div>
          </div>
          
          <!-- PASO 3: VUELTA A LA CALMA -->
          <div style="background: var(--bg-primary); border-radius: 12px; padding: 15px;">
            <div style="display: flex; align-items: center; margin-bottom: 10px;">
              <span style="background: var(--accent-blue); color: var(--bg-primary); width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 12px; font-weight: 500;">3</span>
              <span style="font-weight: 500; font-size: 16px;">VUELTA A LA CALMA (${detalle.estructura?.vueltaCalma?.duracion || 10}')</span>
            </div>
            <div style="margin-left: 40px;">
              <p style="margin: 0 0 8px 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 POR QUÉ:</span> ${detalle.estructura?.vueltaCalma?.porQue || 'Facilitar la recuperación'}</p>
              <p style="margin: 0 0 8px 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 CÓMO:</span> ${detalle.estructura?.vueltaCalma?.como || detalle.estructura?.vueltaCalma?.detalle || 'Trote suave + estiramientos'}</p>
              <p style="margin: 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 PARA QUÉ:</span> ${detalle.estructura?.vueltaCalma?.paraQue || 'Eliminar lactato y acelerar recuperación'}</p>
            </div>
          </div>
        </div>

        <!-- NOTAS TÉCNICAS ADICIONALES -->
        ${detalle.notasTecnicas ? `
        <div style="background: var(--bg-primary); border-radius: 12px; padding: 15px; margin-bottom: 10px;">
          <h4 style="margin: 0 0 10px 0; color: var(--accent-yellow); font-size: 14px;">📌 NOTAS TÉCNICAS</h4>
          <p style="margin: 0; font-size: 14px;">${detalle.notasTecnicas}</p>
        </div>
        ` : ''}
      `;
      ...
      titulo.innerHTML = `
        <div style="margin-bottom: 10px;">
          <span style="font-size: 14px; color: var(--accent-yellow);">SEMANA ${sesion.semana}</span>
          <h3 style="font-size: 24px; margin: 5px 0 0 0; padding: 0; border: none;">DESCANSO</h3>
        </div>
      `;
      
      descripcion.innerHTML = `
        <div style="background: var(--bg-primary); border-radius: 12px; padding: 25px; text-align: center;">
          ...
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin-bottom: 20px;">
            <div>
              ...
              <div style="font-size: 13px;">Estiramientos</div>
            </div>
            <div>
              <div style="font-size: 24px; margin-bottom: 5px;">🚶</div>
              <div style="font-size: 13px;">Paseo activo</div>
            </div>
            <div>
              <div style="font-size: 24px; margin-bottom: 5px;">🌀</div>
              <div style="font-size: 13px;">Foam roller</div>
            </div>
          </div>
          
          <div style="background: var(--bg-secondary); border-radius: 10px; padding: 15px;">
            <p style="margin: 0 0 8px 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 POR QUÉ:</span> Permitir la supercompensación y reparación muscular</p>
            <p style="margin: 0 0 8px 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 CÓMO:</span> Descanso activo con movilidad suave</p>
            <p style="margin: 0; font-size: 14px;"><span style="color: var(--accent-yellow);">🔹 PARA QUÉ:</span> Maximizar las adaptaciones del entrenamiento</p>
          </div>
        </div>
      `;
...
console.log('✅ PlanGenerator v4.0 - ÉLITE MUNDIAL | Ciencia real | Tarjetas perfectas | Progresión natural');