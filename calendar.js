...
const PlanGenerator = {
  ...
  _estimarRitmoParteEfectiva(sesion, totalMin, totalKm) {
    const fallback = (totalKm > 0) ? totalMin / totalKm : NaN;
    const det = sesion?.detalle;
    const duracionPlan = sesion?.duracion || 0;
    if (!det || duracionPlan <= 0 || (!det.calentamiento && !det.enfriamiento)) return fallback;

    const proporcion = totalMin / duracionPlan;
    const calMinReal = (det.calentamiento || 0) * proporcion;
    const enfMinReal = (det.enfriamiento || 0) * proporcion;
    const mainMinReal = totalMin - calMinReal - enfMinReal;
    if (mainMinReal <= 0) return fallback;

    const ritmoZ1MinPerKm = this._ritmoToSeg(this.obtenerRitmoParaZona('Z1')) / 60;
    if (!ritmoZ1MinPerKm || !isFinite(ritmoZ1MinPerKm)) return fallback;

    const distCalReal = calMinReal / ritmoZ1MinPerKm;
    const distEnfReal = enfMinReal / ritmoZ1MinPerKm;
    const distMainReal = totalKm - distCalReal - distEnfReal;
    if (distMainReal <= 0) return fallback;

    return mainMinReal / distMainReal;
  },
    ...
    try {
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          const paceParaZona = (distanciaUsada > 0 && duracionUsadaMs > 0)
            ? this._estimarRitmoParteEfectiva(sesion, duracionUsadaMs / 60000, distanciaUsada)
            : null;
          const zonaReal = paceParaZona
            ? this._detectarZonaPorRitmo(paceParaZona)
            : null;
