...
// Versión: 2.65 - Dos cambios pedidos: 1) el suelo mínimo de "series" (lo
//                pendiente de la v2.64) ahora es propio y más bajo
//                (25-30' según nivel, antes 45-55' como rodaje/tempo/
//                largo) -- probado: en 10K avanzado, series pasa de
//                40'/48'/49'/45' de media (BASE/CONSTRUCCION/ESPECIFICA/
//                PICO, con progresión real) en vez de 57' fijo en las 4
//                fases. 2) Añadida la regla 80/20 (entrenamiento
//                polarizado, Seiler): tempo+series no debería superar un
//                ~20% del volumen semanal. Se aplica como última palabra,
//                después de todo lo demás que toca esos dos tipos.
//                BUG ENCONTRADO probando la regla 80/20: con solo 2-3
//                días de entreno (típico en principiante) y tempo+series
//                gastando todos los días libres, el rodaje planificado
//                (a veces la mayoría del volumen semanal: un caso real,
//                153 de 227') no tenía ningún día donde programarse y
//                desaparecía entero -- la semana entrenada de verdad
//                acababa siendo pura calidad, hasta un 62% tempo+series
//                sobre el total. Arreglado reservando siempre al menos un
//                día libre para rodaje cuando hay más de uno disponible;
//                si tiposCalidad no cabe entero, el que sobra se pliega
//                sobre el rodaje de la semana (mecanismo que ya existía
//                para otro caso, reutilizado aquí). Con esto, en las
//                mismas 480 semanas de prueba: media de tempo+series baja
//                de 34.1% a 22.8%, y el peor caso de 62% a 31% (6 de 480
//                semanas por encima del 30%; ya ninguna por encima del
//                62-63% de antes). No queda perfecto al 20.0% exacto en
//                todas las semanas -- el suelo mínimo por sesión y la
//                granularidad de días siguen dejando algún margen -- pero
//                ya no hay ninguna semana donde el rodaje desaparezca.
// Versión: 2.64 - Revisado el cambio de base: DISTRIBUCION_TIPOS dejó de
//                fijar la duración de largo/tempo/series (ahora solo
//                decide si se programan o no esa fase) y pasaron a usar
//                una duración de referencia propia de la distancia
//                (DURACION_NATIVA_REFERENCIA). El comentario de la v2.63
//                de más abajo se quedó desactualizado con ese cambio --
//                seguía describiendo el sistema por porcentaje que ya no
//                se usa para la duración. Probado: para avanzado en 10K,
//                media maratón y maratón, ya no hace falta alargar la
//                tirada larga en NINGUNA de las 24 semanas de esos planes
//                (antes, con el % fijo, hacía falta en 9-23 de 24 según
//                el caso). Mejor que el intento anterior en volumen alto.
//                BUG ENCONTRADO probando este cambio: la duración de
//                tempo/series/largo dependía solo de nivel + la ola de 4
//                semanas, sin ningún factor de fase -- así que apenas
//                progresaba de BASE a PICO (en 10K avanzado, series salía
//                57' de media en BASE y 57' en PICO). Se pierde la idea
//                de que las sesiones de calidad se vuelven más
//                sustanciales según se acerca la fase específica. Añadido
//                DURACION_NATIVA_FACTOR_FASE para devolver esa
//                progresión sin volver a atar la duración al volumen
//                semanal total.
//                PENDIENTE (no arreglado en esta revisión, a la espera de
//                que confirmes si quieres tocarlo): con el factor de fase
//                ya puesto, "largo" y "tempo" progresan con claridad de
//                fase en fase, pero "series" casi no lo hace para nivel
//                avanzado -- de BASE a PICO su media pasa de 57' a solo
//                57'. La causa: _minimoDuracionSesion pone un suelo de 55'
//                para avanzado, y la duración nativa de series en fases
//                tempranas (34-46') cae por debajo de ese suelo, así que
//                el suelo manda en vez de la progresión -- hasta que la
//                fase avanza lo suficiente como para superarlo por sí
//                sola. Rodaje/tempo/largo no sufren esto porque sus
//                valores de referencia suelen quedar por encima del suelo
//                casi siempre. Si quieres que lo arregle, lo más limpio
//                sería que series (una sesión de intensidad, no de
//                volumen) tuviera su propio suelo más bajo en vez de
//                heredar el mismo que rodaje/tempo/largo.
...
const PlanGenerator = {
  ...
  // (45 -> 35).
  _minimoDuracionSesion(nivel, esDescargaOTaper, tipo) {
    // Series y tempo son sesiones de INTENSIDAD, no de volumen: una
    // sesión de calidad corta pero exigente es legítima (lo que importa
    // son las repeticiones o el tiempo en ritmo fuerte, no rellenar
    // minutos) y no tienen por qué compartir el suelo de rodaje/largo.
    // BUG ENCONTRADO probando la regla 80/20 (ver más abajo en
    // generarCalendarioEntreno): con el mismo suelo que el resto (45-55'
    // según nivel), tempo por sí solo podía superar ya todo el
    // presupuesto del 20% en semanas de volumen bajo (2K/5K,
    // principiante) -- el suelo genérico deshacía el recorte del 80/20
    // en cuanto se creaba la sesión de verdad. Con su propio suelo, más
    // bajo, el recorte del 80/20 se sostiene.
    if (tipo === 'series') {
      const normalSeries = { principiante: 25, intermedio: 28, avanzado: 30 }[nivel] || 25;
      return esDescargaOTaper ? Math.max(18, normalSeries - 5) : normalSeries;
    }
    if (tipo === 'tempo') {
      const normalTempo = { principiante: 22, intermedio: 25, avanzado: 28 }[nivel] || 22;
      return esDescargaOTaper ? Math.max(15, normalTempo - 5) : normalTempo;
    }
  ...
  async crearSesionBasica(tipo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null, esRecuperacion = false, maximoForzado = null) {
    ...
    if (duracionExacta !== null) {
      ...
      const min = this._minimoDuracionSesion(nivel, semanaEnFase % 4 === 0 || fase === 'TAPER', tipo);
      if (duracion < min) {
  ...
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    const minSeries = this._minimoDuracionSesion(nivel, fase === 'TAPER', 'series');
  ...
  // BUG ENCONTRADO probando este archivo: sin esto, la duración de
  // tempo/series/largo dependía solo de nivel + la ola de 4 semanas, así
  // que apenas cambiaba entre fases (en un plan de 10k avanzado de 24
  // semanas, series salía 57' en la semana 1 de BASE y 57' en PICO --
  // básicamente plana). DISTRIBUCION_TIPOS pasó a decidir solo SI se
  // programa un tipo esa fase, no cuánto dura, así que se perdió la
  // progresión de "las sesiones de calidad se vuelven más sustanciales
  // según se acerca la fase específica" que sí tenía el sistema por
  // porcentaje. Este factor devuelve esa progresión sin volver a atar la
  // duración al volumen semanal total (que era el problema original).
  DURACION_NATIVA_FACTOR_FASE: {
    BASE: 0.75,
    CONSTRUCCION: 0.90,
    ESPECIFICA: 1.05,
    PICO: 1.15,
  ...
  _duracionNativaPorDistancia(tipo, nivel, distancia, volumenOnd, fase) {
    ...
    const factorFase = this.DURACION_NATIVA_FACTOR_FASE[fase] ?? 1;
    ...
    return Math.round(base * factorNivel * factorFase * ondaAmortiguada);
  },
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        let minutosPorTipo = {
          rodaje: 0,
          tempo: distribucion.tempo > 0 ? this._duracionNativaPorDistancia('tempo', nivelActual, distancia, volumenOnd, fase) : 0,
          series: distribucion.series > 0 ? this._duracionNativaPorDistancia('series', nivelActual, distancia, volumenOnd, fase) : 0,
          largo: distribucion.largo > 0 ? this._duracionNativaPorDistancia('largo', nivelActual, distancia, volumenOnd, fase) : 0
        };
        ...
        const minSeries = this._minimoDuracionSesion(nivelActual, fase === 'TAPER', 'series');
        ...
        // REGLA 80/20 (entrenamiento polarizado, Seiler): del volumen
        // semanal, el trabajo de intensidad moderada-alta (tempo+series)
        // no debería superar ~20%, dejando al menos ~80% a ritmo fácil
        // (rodaje + la tirada larga, que se corre sobre todo en Z2). Sin
        // este límite, tempo+series podían juntar bastante más de un 20%
        // en fases de mucha calidad (avanzado en ESPECIFICA/PICO llegaba
        // a ~35-45%) -- eso ya no es entrenamiento polarizado, es pasar
        // demasiado tiempo en "zona gris" (ni claramente fácil ni
        // claramente fuerte), justo lo que este modelo intenta evitar.
        // BUG EVITADO: iba antes justo después de calcular las duraciones
        // nativas, pero el suelo mínimo de series de aquí arriba puede
        // volver a subirlo después y deshacer el recorte -- en una
        // semana de descarga llegaba a un 31% real aunque el recorte de
        // más arriba lo hubiera dejado en 20%. Puesta aquí, después de
        // TODO lo demás que toca tempo/series, es la última palabra: si
        // en una semana de volumen muy bajo ni siquiera cabe el suelo
        // mínimo de series dentro del 20%, gana el 80/20 y la sesión de
        // series sale más corta que su suelo habitual -- una sesión de
        // calidad breve en una semana de descarga es coherente; superar
        // el 20% de intensidad no lo es.
        const presupuestoIntensidad = Math.round(volumenSemana * 0.20);
        const sumaIntensidad = minutosPorTipo.tempo + minutosPorTipo.series;
        if (sumaIntensidad > presupuestoIntensidad && sumaIntensidad > 0) {
          const factor8020 = presupuestoIntensidad / sumaIntensidad;
          const tempoAntes = minutosPorTipo.tempo;
          const seriesAntes = minutosPorTipo.series;
          minutosPorTipo.tempo = Math.round(minutosPorTipo.tempo * factor8020);
          minutosPorTipo.series = Math.round(minutosPorTipo.series * factor8020);
          const recuperado = (tempoAntes - minutosPorTipo.tempo) + (seriesAntes - minutosPorTipo.series);
          minutosPorTipo.rodaje += recuperado;
        }
        ...
        if (tiposCalidad.length > 0 && diasLibres.length > 0) {
          ...
          // en vez de perderlos.
          // BUG ENCONTRADO probando la regla 80/20: si tiposCalidad.length
          // == diasLibres.length (típico con solo 2-3 días de entreno,
          // frecuente en principiante), tempo+series se comían TODOS los
          // días libres y no quedaba ninguno para rodaje -- sus minutos
          // planificados (a menudo la mayoría del volumen semanal: en un
          // caso real, 153 de 227') no se repartían en ningún sitio, se
          // perdían enteros. La semana entrenada de verdad acababa siendo
          // solo tempo+series+largo, sin nada de fácil -- rompiendo el
          // 80/20 mucho más de lo que el propio recorte de más arriba
          // podía prevenir, porque el recorte actuaba sobre un total
          // planificado que luego no se cumplía. Ahora, si hay más de un
          // día libre, se reserva siempre al menos uno para rodaje.
          const diasReservadosParaRodaje = diasLibres.length > 1 ? 1 : 0;
          const numCalidad = Math.min(tiposCalidad.length, Math.max(0, diasLibres.length - diasReservadosParaRodaje));
          ...
          for (let i = 0; i < numCalidad; i++) {
            ...
            const minutosDia = Math.max(20, Math.min(max, Math.round(minutosBase * variacionCalidad)));
        ...
        // diaGlobal se numera en el orden real de la semana (1=lunes...
        // 7=domingo), no en el orden en que se crearon las sesiones arriba.
        const semana = [];
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
...
console.log('✅ PlanGenerator v2.65 - Suelo propio para series (progresa por fase) + regla 80/20 tempo+series, con reserva de día para rodaje cuando antes se quedaba sin ninguno');