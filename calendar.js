...
// Versión: 2.51 - Añadido compartir sesiones con amigos
// ============================================================================
...
const PlanGenerator = {
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // ---- BOTÓN COMPARTIR ----
      if (AppState.currentUserId && sesion.tipo !== 'descanso') {
        ...
        shareBtn.style.cssText = `
          ...
          transition: all 0.2s ease;
        `;
        shareBtn.innerHTML = '👥 COMPARTIR CON AMIGOS';
        shareBtn.addEventListener('mouseenter', () => {
          shareBtn.style.background = '#9BB5A0';
          shareBtn.style.color = '#0a0a0a';
        });
        shareBtn.addEventListener('mouseleave', () => {
          shareBtn.style.background = 'transparent';
          shareBtn.style.color = '#9BB5A0';
        });
        shareBtn.addEventListener('click', () => {
          PlanGenerator.abrirModalCompartirSesion(sesion, diaIndex);
        });
  ...
  // ==================== COMPARTIR SESIÓN ====================
  async abrirModalCompartirSesion(sesion, diaIndex) {
    if (!AppState.currentUserId) {
      Utils.showToast('Inicia sesión primero', 'error');
      return;
    }
    // Obtener lista de amigos
    const amigos = await Storage.getFriends(AppState.currentUserId);
    if (!amigos || amigos.length === 0) {
      Utils.showToast('No tienes amigos para compartir', 'warning');
      return;
    }

    // Crear un modal simple para seleccionar amigos
    ...
    let html = `<h3 style="margin:0 0 16px; color:var(--accent-yellow);">👥 Compartir sesión</h3>
      <p style="margin-bottom:16px;">Selecciona los amigos con los que quieres compartir este entrenamiento:</p>
      <div style="display:flex; flex-direction:column; gap:8px;">`;

    for (const amigo of amigos) {
      const name = Utils.capitalizeUsername(amigo.username);
      html += `
        <label style="display:flex; align-items:center; gap:12px; padding:8px; background:var(--bg-secondary); border-radius:10px; cursor:pointer;">
          <input type="checkbox" class="share-friend-checkbox" data-uid="${amigo.uid}" value="${amigo.uid}">
          <span>${Utils.escapeHTML(name)}</span>
          <span style="font-size:12px; color:var(--text-secondary);">@${Utils.escapeHTML(amigo.username)}</span>
        </label>
      `;
    }
    ...
    html += `</div>
      <div style="display:flex; gap:12px; margin-top:20px; justify-content:flex-end;">
        <button id="shareCancelBtn" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:8px 24px; border-radius:30px; cursor:pointer;">Cancelar</button>
        <button id="shareSendBtn" style="background:var(--accent-blue); border:none; color:var(--bg-primary); padding:8px 24px; border-radius:30px; cursor:pointer;">Enviar</button>
      </div>
    `;
    ...
    overlay.querySelector('#shareCancelBtn').addEventListener('click', () => overlay.remove());
    overlay.addEventListener('click', (e) => { if (e.target === overlay) overlay.remove(); });

    overlay.querySelector('#shareSendBtn').addEventListener('click', async () => {
      const checkboxes = overlay.querySelectorAll('.share-friend-checkbox:checked');
      ...
      const diaSemana = sesion.diaSemana || new Date().getDay() || 7;
      // Asegurarnos de que la sesión tenga todos los campos necesarios
      const sessionToShare = {
        ...
        color: sesion.color,
        letra: sesion.letra,
        tieneFuerza: sesion.tieneFuerza || false,
      };
      const shareId = await Storage.createSharedSession(
      ...
      if (shareId) {
        Utils.showToast(`✅ Sesión compartida con ${receiverIds.length} amigo(s)`, 'success');
        overlay.remove();
        // Actualizar badge de compartidos (si existe la función)
        if (window.Friends && typeof Friends.actualizarBadgeCompartidos === 'function') {
          Friends.actualizarBadgeCompartidos();
        }
      } else {
...
console.log('✅ PlanGenerator v2.51 - Añadido compartir sesiones con amigos');