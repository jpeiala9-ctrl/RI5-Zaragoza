...
// Versión: 2.52 - Compartir sesiones entre amigos (sin retos)
// ============================================================================
...
const PlanGenerator = {
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      if (AppState.isPremium && AppState.currentUserId) {
        const shareBtn = document.createElement('button');
        shareBtn.id = 'shareSessionBtn';
        shareBtn.style.cssText = [
          'margin-top:12px', 'width:100%', 'height:48px',
          'background:transparent', 'border:2px solid #c0a060',
          'color:#c0a060', 'font-size:13px', 'font-weight:bold',
          'letter-spacing:1px', 'border-radius:14px', 'cursor:pointer',
          "font-family:'Courier New',monospace",
          'display:flex', 'align-items:center',
          'justify-content:center', 'gap:8px',
          'transition:all 0.2s ease'
        ].join(';');
        shareBtn.innerHTML = '📤 ENVIAR A AMIGO';
        shareBtn.addEventListener('mouseenter', () => {
          shareBtn.style.background = '#c0a060';
          shareBtn.style.color = '#0a0a0a';
        });
        shareBtn.addEventListener('mouseleave', () => {
          shareBtn.style.background = 'transparent';
          shareBtn.style.color = '#c0a060';
        });
        shareBtn.addEventListener('click', () => {
          AppState.sessionToShare = {
            sessionData: {
              tipo: sesion.tipo,
            ...
            targetDay: sesion.diaSemana
          };
          if (window.UI) UI.abrirSelectorAmigosParaCompartir();
        });
...
console.log('✅ calendar.js v2.52 - Compartir sesiones entre amigos integrado');