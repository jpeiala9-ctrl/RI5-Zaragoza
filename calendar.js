...
// Versión: 2.41 - Exportación .FIT para relojes GPS (fit-file-writer con reintentos)
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== FUNCIÓN DE EXPORTACIÓN .FIT (con reintentos) ==========
  exportarWorkoutAFIT(sesion) {
    ...
    const intentarExportar = () => {
      const fitLib = window.fitFileWriter;
      if (!fitLib || !fitLib.FitWriter) {
        if (intentos < maxIntentos) {
          intentos++;
          setTimeout(intentarExportar, 100);
          return;
        }
        Utils.showToast('Librería FIT no disponible. Recarga la página.', 'error');
        return;
      }

      const { FitWriter } = fitLib;
      ...
      fit.writeMessage('file_id', {
        ...
        product: 0,
        time_created: fit.time(now),
        serial_number: 0
      });

      fit.writeMessage('workout', {
        wkt_name: `RI5 - ${sesion.detalle.nombre}`,
        sport: 'running',
        sub_sport: 'generic',
        num_valid_steps: pasos.length
      });

      pasos.forEach((p, idx) => {
        fit.writeMessage('workout_step', {
          message_index: idx,
          wkt_step_name: p.nombre,
          duration_type: 'time',
          duration_time: p.duracionSeg,
      ...
      const blob = new Blob([fit.finish()], { type: 'application/octet-stream' });
...
console.log('✅ PlanGenerator v2.41 - Exportación .FIT con reintentos');