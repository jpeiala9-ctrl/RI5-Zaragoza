...
// Versión: 5.1 - ADAPTADA PARA TU PROYECTO
//
// CARACTERÍSTICAS:
...
const PlanGenerator = {
  ...
  async guardarPlanEnFirebase(planId, planData) {
    ...
    try {
      // Usar Storage en lugar de firebaseServices directamente para mantener consistencia
      await Storage.savePlanCompleto(AppState.currentUserId, planId, planData);
  ...
  async cambiarTrimestre(delta) {
    ...
    if (AppState.planActualId && AppState.currentUserId) {
      try {
        const planCompleto = await Storage.getPlanCompleto(AppState.currentUserId, AppState.planActualId);
  ...
  async marcarSesionRealizada(diaIndex, realizada) {
    ...
    try {
      // Usar Storage en lugar de acceder directamente a Firebase
      await Storage.marcarSesionRealizada(AppState.currentUserId, AppState.planActualId, diaIndex, realizada);
  ...
  async mostrarUltimoPlanGuardado() {
    ...
    try {
      ...
      const realizadas = await Storage.getSesionesRealizadas(AppState.currentUserId, ultimoPlanId);
      AppState.sesionesRealizadas = realizadas || {};
      AppState.trimestreActual = 0;
  ...
  async borrarPlanGuardado() {
    ...
    try {
      ...
      if (AppState.planActualId) {
        await Storage.deletePlan(AppState.currentUserId, AppState.planActualId);
      }
...
console.log('✅ PlanGenerator v5.1 - Adaptado para tu proyecto');