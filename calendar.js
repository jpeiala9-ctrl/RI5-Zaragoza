...
const PlanGenerator = {
  ...
  // guardan. Devuelve {km, ms} o null si se cancela.
  abrirModalDatosReales(sesion) {
    return new Promise((resolve) => {
      ...
      modal.innerHTML = `
        ...
        <p style="margin:0 0 18px 0; font-size:12px; color:var(--text-secondary);">Confirma o corrige lo que has hecho de verdad. Si has entrenado distinto a lo planificado, actualízalo aquí.</p>
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      checkbox.onchange = async (e) => {
        ...
        if (marcando && sesion && sesion.tipo !== 'descanso' && sesion.tipo !== 'strength') {
          ...
          await this.marcarSesionRealizada(diaIndex, true, datos.km, datos.ms, null, false);
        } else {
  ...
  async marcarSesionRealizada(diaIndex, realizada, realDistance = null, realDurationMs = null, realMaxSpeedKmh = null, esGPS = false, saltarComprobacionFatiga = false) {
    if (!AppState.currentUserId || !AppState.planActualId) return;
