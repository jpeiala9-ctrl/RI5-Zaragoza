...
const PlanGenerator = {
  ...
  async limpiarMuroGlobal() {
    ...
    try {
      // ANTES: se borraba todo lo anterior al inicio de HOY, en cada
      // recarga y cada vez que se marcaba una sesión como realizada. Eso
      // significaba que 'globalFeed' nunca podía tener nada de antes de
      // hoy, así que cualquier estadística "esta semana" (lunes-domingo)
      // se quedaba vacía en cuanto pasaba la medianoche del primer día con
      // datos. Ahora se conserva toda la semana actual (desde el lunes) y
      // solo se poda lo anterior a esta semana.
      const hoy = new Date();
      const diaSemana = hoy.getDay();
      const inicioSemana = new Date(hoy);
      inicioSemana.setDate(hoy.getDate() - (diaSemana === 0 ? 6 : diaSemana - 1));
      inicioSemana.setHours(0, 0, 0, 0);
      const inicioSemanaTimestamp = firebaseServices.Timestamp.fromDate(inicioSemana);
      
      const antiguas = await firebaseServices.db
        .collection('globalFeed')
        .where('timestamp', '<', inicioSemanaTimestamp)
        .get();
      ...
      if (antiguas.size > 0) {
        await batch.commit();
        console.log(`🗑️ Muro limpiado: se eliminaron ${antiguas.size} entradas anteriores a esta semana.`);
      }
    } catch (error) {
