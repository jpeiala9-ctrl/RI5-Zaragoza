// ==================== calendar.js - VERSIÓN DEFINITIVA 2.19 ====================
// - Corregido cálculo de ritmo rápido para series (función obtenerRitmoParaTipo)
// - Mejoradas las explicaciones de series avanzadas (pirámide, rotas, fartlek, cuestas)
// - Añadida duración de la parte principal y asociación a zonas
// - Corregido cálculo de distancia en sesiones avanzadas usando ritmos reales
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== FUNCIÓN PARA OBTENER RITMO REALISTA SEGÚN TIPO, NIVEL Y DISTANCIA ==========
  ...
  // ================== SESIONES AVANZADAS DE SERIES (MEJORADAS) ==================
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    // Función auxiliar para convertir ritmo (min:seg) a segundos por km
    const ritmoToSeg = (ritmoStr) => {
    ...
    switch(estructura) {
      ...
      case 'piramide':
        ...
        const distanciasPiramide = [200, 400, 600, 800, 600, 400, 200];
        const recuperacionesPiramide = [60, 90, 120, 120, 90, 60];
        let textoPir = '';
        for (let i = 0; i < distanciasPiramide.length; i++) {
          const rec = i < recuperacionesPiramide.length ? recuperacionesPiramide[i] : 60;
          textoPir += `${distanciasPiramide[i]}m (rec ${rec}") `;
        }
        // Duración total de las series (sin recuperaciones)
        const ritmoRapidoSeg = ritmoToSeg(ritmoRapido);
        let tiempoSeriesSeg = 0;
        distanciasPiramide.forEach(d => tiempoSeriesSeg += (d / 1000) * ritmoRapidoSeg);
        const minutosSeries = Math.floor(tiempoSeriesSeg / 60);
        const segundosResto = Math.round(tiempoSeriesSeg % 60);
        const duracionPartePrincipal = `${minutosSeries}:${segundosResto.toString().padStart(2, '0')}`;
        
        accion = `Parte principal (${duracionPartePrincipal} minutos): ${textoPir}. Realiza cada repetición a ritmo rápido (${ritmoRapido} min/km) – Z4-Z5 – recuperando caminando o trotando suave – Z2.`;
        ...
        // Guardar datos para calcular distancia real
        const detallePiramide = {
          nombre, descripcion, estructura: `${calentamiento}' calentamiento + ${accion} + ${enfriamiento}' enfriamiento`,
          sensacion: 'Muy intenso', zona, duracion, ritmoObjetivo: ritmoRapido,
          fcObjetivo: `> ${Math.round(fcUmbral * 1.02)} lpm`,
          calentamiento, partePrincipal, enfriamiento, objetivo: nombre, porque,
          pasosDetallados,
          // Datos para calcular distancia
          tipoEstructura: 'piramide',
          distancias: distanciasPiramide,
          ritmoSerie: ritmoRapido,
          ritmoRecuperacion: ritmoModerado,
          recuperaciones: recuperacionesPiramide
        };
        return { tipo: 'series', duracion, detalle: detallePiramide };
      ...
      case 'rotas':
        ...
        // Calcular duración total de la parte principal (series + recuperaciones)
        const ritmoRapidoSegR = ritmoToSeg(ritmoRapido);
        const tiempoSeries = (repPorBloque * bloques) * (distanciaSerie / 1000) * ritmoRapidoSegR;
        const tiempoRecIntra = (repPorBloque - 1) * recIntra * bloques;
        const tiempoRecEntre = recEntreBloques * (bloques - 1);
        const totalSegundos = tiempoSeries + tiempoRecIntra + tiempoRecEntre;
        const minutosPP = Math.floor(totalSegundos / 60);
        const segundosPP = Math.round(totalSegundos % 60);
        const duracionPartePrincipalR = `${minutosPP}:${segundosPP.toString().padStart(2, '0')}`;
        
        accion = `Parte principal (${duracionPartePrincipalR} minutos): ${bloques} bloques de ${repPorBloque}x${distanciaSerie}m a ritmo rápido (${ritmoRapido} min/km) – Z4-Z5 – con recuperación de ${recIntra}" entre series y ${Math.floor(recEntreBloques/60)}' entre bloques (recuperación activa Z2).`;
        ...
        const detalleRotas = {
          ...
          tipoEstructura: 'rotas',
          distanciaSerie,
          repeticionesPorBloque: repPorBloque,
          bloques,
          recIntra,
          recEntreBloques,
          ritmoSerie: ritmoRapido,
        ...
        return { tipo: 'series', duracion, detalle: detalleRotas };

      // ------------------ FARTLEK ------------------
      case 'fartlek':
        nombre = 'Fartlek estructurado';
        descripcion = 'Cambios de ritmo sin parar.';
        const tiempoFuerte = 60;   // segundos
        const tiempoSuave = 90;    // segundos
        const duracionCiclo = (tiempoFuerte + tiempoSuave) / 60; // minutos por ciclo
        let repeticiones = Math.floor(partePrincipal / duracionCiclo);
        let sobrante = partePrincipal - (repeticiones * duracionCiclo);
        
        let detalleCiclos = '';
        let segundosFuertes = 0;
        let segundosSuaves = 0;
        if (repeticiones > 0) {
          detalleCiclos = `${repeticiones} ciclos de ${tiempoFuerte}" fuerte + ${tiempoSuave}" suave`;
          segundosFuertes = repeticiones * tiempoFuerte;
          segundosSuaves = repeticiones * tiempoSuave;
          if (sobrante > 0) {
            const sobranteSeg = Math.round(sobrante * 60);
            detalleCiclos += ` + ${sobranteSeg}" de ritmo suave`;
            segundosSuaves += sobranteSeg;
          }
        } else {
          repeticiones = 1;
          const fuerteReal = Math.min(tiempoFuerte, partePrincipal * 60 * 0.4);
          const suaveReal = partePrincipal * 60 - fuerteReal;
          detalleCiclos = `${Math.round(fuerteReal)}" fuerte + ${Math.round(suaveReal)}" suave`;
          segundosFuertes = fuerteReal;
          segundosSuaves = suaveReal;
        }

        accion = `Parte principal (${partePrincipal} minutos): ${detalleCiclos}. Realiza el fuerte a ritmo rápido (${ritmoRapido} min/km) – Z4-Z5 – y el suave a ritmo aeróbico (${ritmoModerado} min/km) – Z2. El cambio de ritmo debe ser continuo, sin detenerte.`;
        ...
        const detalleFartlek = {
          ...
          pasosDetallados,
          // Datos para distancia
          tipoEstructura: 'fartlek',
          segundosFuertes,
          segundosSuaves,
          ritmoFuerte: ritmoRapido,
          ritmoSuave: ritmoModerado
        };
        return { tipo: 'series', duracion, detalle: detalleFartlek };
      ...
      case 'cuestas':
        ...
        const recCuesta = 90;
        const duracionTotalCuestas = numCuestas * duracionCuesta; // segundos
        const minutosTotal = Math.floor(duracionTotalCuestas / 60);
        const segundosTotal = duracionTotalCuestas % 60;
        
        accion = `Parte principal (${minutosTotal}:${segundosTotal.toString().padStart(2, '0')} minutos): ${numCuestas} repeticiones de ${duracionCuesta}" en cuesta con desnivel moderado, recuperando trotando suave ${recCuesta}" – Z2. Esfuerzo máximo (Z5) en la subida.`;
        ...
        const detalleCuestas = {
          nombre, descripcion, estructura: `${calentamiento}' calentamiento + ${accion} + ${enfriamiento}' enfriamiento`,
          sensacion: 'Muy intenso', zona, duracion, ritmoObjetivo: ritmoRapido,
          fcObjetivo: `> ${Math.round(fcUmbral * 1.02)} lpm`,
          calentamiento, partePrincipal, enfriamiento, objetivo: nombre, porque,
          pasosDetallados,
          // Datos para distancia (se estima con ritmo de series, aunque en cuesta la distancia no es lineal, se puede omitir o estimar)
          tipoEstructura: 'cuestas',
          numRepeticiones: numCuestas,
          duracionRepeticion: duracionCuesta,
          ritmoRepeticion: ritmoRapido
        };
        return { tipo: 'series', duracion, detalle: detalleCuestas };
  ...
  // ========== CÁLCULO DE MÉTRICAS CON RITMOS REALES (si están disponibles) ==========
  calcularMetricasSesion(sesion, factorIntensidad = 1.0) {
    ...
    // Si la sesión es de series avanzadas y tiene datos específicos, calcular distancia real
    if (detalle.tipoEstructura === 'fartlek' && detalle.segundosFuertes !== undefined && detalle.segundosSuaves !== undefined) {
      const ritmoFuerteSeg = ritmoToSeg(detalle.ritmoFuerte);
      const ritmoSuaveSeg = ritmoToSeg(detalle.ritmoSuave);
      ...
      // Calentamiento y enfriamiento con ritmo suave (Z2)
      const ritmoSuaveSegGen = ritmoBase * 1.25 / factorIntensidad * 60; // aprox ritmo Z2 en seg/km
      ...
      // TSS (se mantiene el cálculo genérico por zona)
    ...
    if (detalle.tipoEstructura === 'piramide' && detalle.distancias && detalle.ritmoSerie) {
      const ritmoSerieSeg = ritmoToSeg(detalle.ritmoSerie);
      let distanciaSeries = 0;
      detalle.distancias.forEach(d => distanciaSeries += d / 1000);
      // Las series se recorren una vez (la pirámide incluye todas las distancias)
      // Multiplicamos por el número de repeticiones (1 de cada distancia)
      distanciaSeries = distanciaSeries; // ya suma todas
      // Calcular tiempo de series
      const tiempoSeriesSeg = distanciaSeries * 1000 / (ritmoSerieSeg / 1000) * ritmoSerieSeg; // no, mejor: distanciaSeries * ritmoSerieSeg
      const tiempoSeriesReal = distanciaSeries * ritmoSerieSeg; // segundos
      // Las recuperaciones no aportan distancia significativa (se hacen a pie)
    ...
    if (detalle.tipoEstructura === 'cuestas' && detalle.numRepeticiones) {
      // Estimación muy aproximada: la distancia en cuesta se calcula como si fuera rodaje a ritmo rápido (no es exacto, pero mejor que nada)
      const ritmoSerieSeg = ritmoToSeg(detalle.ritmoRepeticion);
    ...
    // Fallback: usar método genérico por zona
...
console.log('✅ PlanGenerator v2.19 - Con series avanzadas mejoradas y cálculo de distancia real');