...
// Versión: 2.41 - ELIMINADA la limpieza automática al marcar sesión
// ============================================================================
...
const PlanGenerator = {
  ...
  // ✅✅✅ FUNCIÓN CORREGIDA - SIN LIMPIEZA AUTOMÁTICA ✅✅✅
  async marcarSesionRealizada(diaIndex, realizada) {
    ...
    try {
      ...
      if (realizada) {
        // ✅ CORRECCIÓN: NO BORRAR EL MURO AQUÍ. Solo guardar.
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          try {
            ...
            // ✅ FORZAR RECARGA DEL MURO PARA QUE APAREZCA AL INSTANTE
            if (window.Wall) {
              setTimeout(() => {
                if (window.Wall.recargar) Wall.recargar();
              }, 300);
            }
          } catch (err) {
...
// Ejecutar limpieza del muro al iniciar la app (solo una vez)
setTimeout(() => {
...
console.log('✅ PlanGenerator v2.41 - Muro corregido (sin limpieza automática al marcar)');