// ==================== calendar.js - VERSIÓN COMPLETA CON GPS Y MAPA SATELITAL ====================
// Versión: 2.53 - Añadida verificación de precisión antes de grabar y capa de imagen aérea
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== FUNCIÓN ABRIR DETALLE CON GPS (MEJORADA CON PRECISIÓN Y MAPA SATELITAL) ==========
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // ========== CONTROLES DE GPS CON VERIFICACIÓN DE PRECISIÓN ==========
      if (!realizada) {
        ...
        startBtn.onclick = async () => {
          ...
          // Verificar precisión antes de comenzar
          Utils.showToast('Obteniendo posición precisa...', 'info', 2000);
          let posicionPrecisa = null;
          let precisionAceptable = false;
          
          try {
            posicionPrecisa = await new Promise((resolve, reject) => {
            ...
            const precision = posicionPrecisa.coords.accuracy;
            console.log(`Precisión obtenida: ${precision} metros`);
            if (precision <= 30) {
              precisionAceptable = true;
              Utils.showToast(`✅ Precisión buena (${precision} m). Iniciando grabación...`, 'success', 2000);
            } else {
              Utils.showToast(`⚠️ Precisión baja (${precision} m). Es posible que la ruta no sea precisa.`, 'warning', 3000);
              const continuar = await Utils.confirm('Precisión baja', `La precisión actual es de ${precision} metros. ¿Deseas continuar igualmente?`);
              if (!continuar) {
            ...
            console.error('Error obteniendo posición precisa:', error);
            Utils.showToast('No se pudo obtener una posición precisa. Comprueba los permisos de GPS.', 'error');
            return;
          }
          
          let mapContainer = document.getElementById('gpsMapContainer');
          if (!mapContainer) {
            console.log('Creando contenedor de mapa...');
          ...
          GPSTracker.onPointAdded = (point, index) => {
            console.log(`📍 Punto ${index}: ${point.lat}, ${point.lng}`);
          };
          
          GPSTracker.onTrackingEnd = async (routeData) => {
            ...
            if (routeData.points.length >= 2) {
              ...
              const distanciaReal = routeData.distanceMeters / 1000;
              const diferencia = Math.abs(distanciaReal - distanciaEstimada);
              const porcentaje = (diferencia / distanciaEstimada) * 100;
              
              await this.guardarRutaGPS(diaIndex, routeData);
              
              if (porcentaje <= 10) {
                await this.marcarSesionRealizada(diaIndex, true);
                Utils.showToast(`✅ Sesión dentro del 10% (${porcentaje.toFixed(1)}% de diferencia). Marcada como realizada.`, 'success');
              } else {
                Utils.showToast(`⚠️ La distancia real (${distanciaReal.toFixed(2)} km) difiere en un ${porcentaje.toFixed(1)}% de lo planificado (${distanciaEstimada.toFixed(2)} km). No se marca automáticamente.`, 'warning');
              }
              
              await GPSTracker.displayRoute('gpsMapContainer', routeData.points);
              const statsDiv = document.createElement('div');
              statsDiv.className = 'gps-stats';
              statsDiv.style.cssText = 'margin-top: 12px; padding: 12px; background: var(--bg-secondary); border-radius: var(--r-md); text-align: center;';
              statsDiv.innerHTML = `
                <strong>📊 Datos reales del entrenamiento</strong><br>
                Distancia: ${distanciaReal.toFixed(2)} km<br>
                Duración: ${Math.floor(routeData.durationSeconds / 60)} min ${routeData.durationSeconds % 60} seg<br>
                Ritmo medio: ${(routeData.durationSeconds / 60 / distanciaReal).toFixed(1)} min/km
                ${porcentaje > 10 ? `<br><span style="color: var(--no-realizado-color);">⚠️ Diferencia del ${porcentaje.toFixed(1)}% respecto al plan</span>` : ''}
              `;
              mapContainer.parentNode.insertBefore(statsDiv, mapContainer.nextSibling);
            } else {
              Utils.showToast('No se pudo grabar la ruta (puntos insuficientes)', 'warning');
            }
          ...
          console.log('Llamando a GPSTracker.startTracking...');
          const started = await GPSTracker.startTracking('gpsMapContainer');
          console.log('Resultado de startTracking:', started);
          if (started) {
            ...
            stopBtn.onclick = async () => {
              console.log('🛑 Finalizando grabación manualmente');
            ...
            console.error('No se pudo iniciar la grabación (startTracking devolvió false)');
          }
        };
        
        gpsControls.appendChild(startBtn);
        descripcion.parentNode.insertBefore(gpsControls, descripcion.nextSibling);
        console.log('✅ Controles GPS añadidos al DOM');
      }
      
      if (realizada && sesion.gpsRoute) {
        console.log('🗺️ Mostrando ruta guardada para sesión realizada');
        const mapContainer = document.createElement('div');
        mapContainer.id = `gpsMap_${diaIndex}`;
        mapContainer.className = 'route-map';
        mapContainer.style.marginTop = '16px';
        descripcion.parentNode.insertBefore(mapContainer, descripcion.nextSibling);
        GPSTracker.displayRoute(mapContainer.id, sesion.gpsRoute.points);
        const statsDiv = document.createElement('div');
        statsDiv.className = 'gps-stats';
        statsDiv.style.cssText = 'margin-top: 12px; padding: 12px; background: var(--bg-secondary); border-radius: var(--r-md); text-align: center;';
        statsDiv.innerHTML = `
          <strong>📊 Datos reales del entrenamiento</strong><br>
          Distancia: ${(sesion.gpsRoute.distanceMeters / 1000).toFixed(2)} km<br>
          Duración: ${Math.floor(sesion.gpsRoute.durationSeconds / 60)} min ${sesion.gpsRoute.durationSeconds % 60} seg<br>
          Ritmo medio: ${(sesion.gpsRoute.durationSeconds / 60 / (sesion.gpsRoute.distanceMeters / 1000)).toFixed(1)} min/km
        `;
        descripcion.parentNode.insertBefore(statsDiv, mapContainer.nextSibling);
      }
...
console.log('✅ PlanGenerator v2.53 - con GPS, verificación de precisión y mapa satelital');