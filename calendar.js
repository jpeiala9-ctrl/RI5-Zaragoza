  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // GARANTÍA FINAL: la tirada larga tiene que ser, sin excepción, la
        // sesión más larga de la semana. Había DOS intentos anteriores de
        // asegurar esto (el bloque de más arriba que capaba tempo/series, y
        // _garantizarLargoMaximo que capaba rodaje) pero los dos se
        // ejecutaban ANTES de crear las sesiones definitivas -- y el suelo
        // mínimo de duración por nivel (_minimoDuracionSesion, más alto
        // para intermedio/avanzado) se aplica DESPUÉS, dentro de
        // crearSesionBasica/crearSesionAvanzadaSeries, así que en semanas
        // de poco volumen (fase BASE) ese suelo podía volver a subir una
        // sesión por encima de la tirada larga sin que nada se enterase.
        // Esta comprobación se hace aquí, con TODAS las duraciones ya
        // definitivas, así que es la última palabra: nada después de esto
        // puede volver a romper la garantía.
        const sesionLargo = semana.find(s => s.tipo === 'largo');
        if (sesionLargo && sesionLargo.duracion) {
          const largoMinutos = sesionLargo.duracion;
          for (const s of semana) {
            if (s.tipo !== 'largo' && s.tipo !== 'descanso' && s.duracion && s.duracion >= largoMinutos) {
              // BUG CORREGIDO: recortar siempre a EXACTAMENTE largoMinutos-5
              // (sin variación) recreaba el mismo problema que el suelo
              // mínimo -- si varias sesiones de la semana necesitaban
              // recortarse (típico en una semana de poco volumen con el
              // suelo por nivel ya subiendo varias por encima de la tirada
              // larga), todas acababan con el mismo número exacto. El
              // margen aleatorio evita esa coincidencia sin dejar de
              // garantizar que se queda por debajo de la tirada larga
              // (el resultado nunca supera largoMinutos-5).
              const nuevaDuracion = Math.max(20, largoMinutos - 4 - Math.floor(Math.random() * 12));
              s.duracion = nuevaDuracion;
              if (s.detalle) {
                s.detalle.duracion = nuevaDuracion;
                const metricasAjustadas = this.calcularMetricasSesion({ tipo: s.tipo, duracion: nuevaDuracion, detalle: s.detalle }, intensidadTotal);
                s.detalle.distanciaEstimada = metricasAjustadas.distanciaTotal;
                s.detalle.tssEstimada = metricasAjustadas.tssTotal;
              }
            }
          }
        }
