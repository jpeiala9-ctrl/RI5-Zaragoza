// ============================================================================
// calendar.js - VERSIÓN ÉLITE (con gestor de planes adaptativo por volumen semanal)
// ============================================================================
...
const PlanGenerator = {
  ...
  DISTRIBUCION_TIPOS: {
    // Se ajustará dinámicamente según la distancia, estos son valores base
    principiante: {
  ...
  // ==========================================================================
  // FUNCIÓN PARA CALIBRAR NIVEL SEGÚN TIEMPO REAL
  // ==========================================================================
  calibrarNivelPorTiempo(mejorTiempoStr) {
    if (!mejorTiempoStr) return null;
    // Buscar números en el string (ej. "5k: 22:30" -> 22:30)
    const match = mejorTiempoStr.match(/(\d{1,2}):(\d{2})/);
    if (!match) return null;
    const minutos = parseInt(match[1]);
    const segundos = parseInt(match[2]);
    const totalMinutos = minutos + segundos / 60;
    // Tabla de referencia aproximada (media maratón)
    if (totalMinutos < 90) return 'avanzado';
    if (totalMinutos < 135) return 'intermedio';
    return 'principiante';
  },
  ...
  calcularVolumenSemanal(nivel, experiencia, objetivo, distancia) {
    ...
    // Primera vez en esa distancia
    if (experiencia === 'no') volumen = Math.round(volumen * 0.9);
    // Limitar rangos razonables (2h a 12h)
    return Math.min(720, Math.max(120, volumen));
  },

  // ==========================================================================
  // GENERACIÓN PRINCIPAL CON VOLUMEN SEMANAL
  // ==========================================================================
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // Calibrar nivel según mejor tiempo si se proporcionó
      const mejorTiempo = document.getElementById("mejorTiempo")?.value.trim();
      if (mejorTiempo) {
        const nivelSugerido = this.calibrarNivelPorTiempo(mejorTiempo);
        if (nivelSugerido) {
          nivel = nivelSugerido;
          // Opcional: mostrar sugerencia
          Utils.showToast(`Nivel sugerido: ${nivelSugerido.toUpperCase()} según tu mejor marca`, 'info');
        }
      }
      ...
      if (!diasEntreno.includes(diaLargo)) {
        Utils.showToast(`⚠️ El día de tirada larga (${["", "LUN", "MAR", "MIÉ", "JUE", "VIE", "SÁB", "DOM"][diaLargo]}) no está marcado como día de entrenamiento. Selecciona un día válido o elige "Automático".`, 'error');
        Utils.hideLoading();
        return;
      }
      ...
      // ===== CALCULAR FECHA DE INICIO REAL (próximo lunes) =====
      ...
      // ===== PREPARAR DISTRIBUCIÓN DE TIPOS SEGÚN DISTANCIA =====
      ...
      // Variables para feedback adaptativo
      let ajusteVolumen = 1.0;
      let ajusteIntensidad = 1.0;
      if (AppState.planActualId) {
        // Leer feedback de las últimas semanas (se implementa en función aparte)
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // Calcular volumen objetivo para esta semana (minutos totales)
        ...
        // Si es semana de descarga, reducir volumen un 30%
        if (esDescarga) volumenSemana = Math.round(volumenSemana * 0.7);

        // Distribuir volumen entre los días disponibles según los porcentajes de la fase
        ...
        // Ajustar redondeos para que la suma sea exacta
        ...
        // Asignar tipos a los días, respetando día largo
        const tiposPorDia = {};
        const diasDisponibles = [...diasEntreno];
        // Asignar largo al día elegido
        ...
        // Preparar lista de tipos restantes con sus minutos
        ...
        for (let i = 0; i < numDiasRestantes; i++) {
          ...
          if (tipoObj) {
            const minutosDia = Math.min(tipoObj.minutos, 90); // máx por sesión
        ...
        // Generar las sesiones de la semana
        const semana = [];
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          if (!diasEntreno.includes(diaSemana)) {
            ...
            // Crear sesión con la duración calculada
            const sesion = await this.crearSesionDesdeMatriz(
              ...
              1, // factorVolumen (ya incluido en minutos)
              1  // factorIntensidad
            );
            // Sobrescribir duración con la calculada
            sesion.duracion = minutos;
            sesion.detalle.duracion = minutos;
            semana.push({
              diaGlobal: diaGlobalCounter++,
              semana: semanaGlobal,
              diaSemana,
      ...
      const planParaGuardar = {
        params: {
          ...
          mejorTiempo: mejorTiempo || null
        },
        sesiones: planCompleto,
        feedback: {},
        ajustes: {}, // para guardar ajustes adaptativos
  ...
  ajustarDistribucionPorDistancia(baseDistribucion, distancia) {
    ...
    const factorRodaje = 1.0;

    for (let nivel in nueva) {
      for (let fase in nueva[nivel]) {
        let d = nueva[nivel][fase];
        // Normalizar para que suma sea 1 después de ajustes
  ...
  // ==========================================================================
  // ANÁLISIS DE FEEDBACK ADAPTATIVO
  // ==========================================================================
  async analizarFeedbackAdaptativo() {
    ...
    try {
      ...
      // Contar frecuencias de cada valor (1=Muy dura, 2=Justa, 3=Bien, 4=Excelente)
  ...
  // ==========================================================================
  // CREAR SESIÓN DESDE MATRIZ (con duración mínima 45 min y ajuste por descarga)
  // ==========================================================================
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad) {
    ...
    const MINIMO = { rodaje: 45, tempo: 45, series: 45, largo: 45 };
    ...
    // Calcular zona mostrada a partir de los pasos
  ...
  // ==========================================================================
  // GUARDADO Y VISUALIZACIÓN (resto del código sin cambios)
  // ==========================================================================
  ...
  async mostrarUltimoPlanGuardado() {
    ...
    // Permitir ver plan aunque no sea premium (solo cargar, no generar)
    try {
...
console.log('✅ PlanGenerator v15.0 - ÉLITE: volumen semanal, feedback adaptativo, calibración por tiempo');