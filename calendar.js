...
// Versión: 5.12 - CORREGIDO: intercambio de día largo sin rotación de plantilla
// ============================================================================
...
const PlanGenerator = {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        if (idxLargoPlantilla === -1) {
          // Si no hay largo, asignamos uno al día que tenga algún tipo (o al último)
        ...
        // Recalcular índice por si cambió
        const idxLargoFinal = plantillaArray.findIndex(dia => dia.tipo === 'largo');

        // --- INTERCAMBIO: si el día elegido es diferente al que tiene el largo, intercambiamos ---
        if (diaLargo - 1 !== idxLargoFinal) {
          [plantillaArray[diaLargo - 1], plantillaArray[idxLargoFinal]] = [plantillaArray[idxLargoFinal], plantillaArray[diaLargo - 1]];
        }

        // Ahora plantillaArray tiene la distribución correcta: el día elegido tiene tipo 'largo'
        const semana = [];
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          const sesionBase = plantillaArray[diaSemana - 1];
  ...
  renderizarPagina(sesiones) {
    ...
    // getDay(): 0=domingo, 1=lunes, ..., 6=sábado. Convertimos:
...
console.log('✅ PlanGenerator v5.12 - Día largo por intercambio, sin rotación');