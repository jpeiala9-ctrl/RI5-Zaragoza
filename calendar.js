// ==================== calendar.js - VERSIÓN FINAL 2.14 (COMPLETA) ====================
// - Paginación dinámica según duración real del plan
// - Pre‑cálculo de métricas para feed asegurado
// - Rodajes con variabilidad controlada (±10%), tirada larga coherente, series con mínimo de repeticiones
// - Factores de volumen e intensidad aplicados en creación de sesiones
// - Al marcar sesión realizada, se añade al feed de los amigos
// ============================================================================
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // Rodajes con variabilidad controlada (±10%)
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    ...
    const obtenerRitmoObjetivo = () => {
      let base = ritmoBase;
      // Ajustar por intensidad (factorIntensidad): mayor intensidad -> ritmo más rápido (menor valor)
      let ritmo = base * (1.05 / factorIntensidad);
      if (distancia === '10k') ritmo = base * 0.98;
      if (distancia === 'medio') ritmo = base * 1.03;
      if (distancia === 'maraton') ritmo = base * 1.07;
      return ritmo;
    };
    ...
    if (incluirRitmoObjetivo) {
      ...
      switch(tipo) {
        ...
        case 'series': {
          ...
          const ritmoSeries = ritmoBase * 0.95 / factorIntensidad;
    ...
    // Ajustar ritmo por intensidad: si factorIntensidad > 1, ritmo más rápido (menor valor)
    let ritmoBaseAjustado = ritmoBase / factorIntensidad;
    const ritmoObjetivo = Utils.formatR(ritmoBaseAjustado * (factoresRitmo[zonaPrincipal] || 1.25));
    ...
    // Ajustar fc objetivo por intensidad: factorIntensidad > 1 -> mayor fc
  ...
  async marcarSesionRealizada(diaIndex, realizada) {
    ...
    try {
      ...
      // Si se marca como realizada, añadir al feed de los amigos
      if (realizada) {
  ...
  calcularMetricasSesion(sesion, factorIntensidad = 1.0) {
    ...
    // Ajustar ritmo por intensidad: mayor intensidad -> ritmo más rápido (menor valor)
    ...
    // Intensidad también afecta TSS (mayor intensidad -> más TSS)
...
console.log('✅ PlanGenerator v2.14 - Con feed real, variabilidad controlada y factores adaptativos');