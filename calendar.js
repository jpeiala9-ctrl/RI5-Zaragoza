...
// CORREGIDO: error this.generatorPasosSimples, añadida nota sobre inicio en lunes.
// ============================================================================
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // Mostrar resumen con información clara sobre inicio en lunes y día largo
      ...
      document.getElementById("resumenObjetivo").innerHTML = `
        ...
        <div style="margin-top: 8px; font-size: 12px; color: var(--accent-yellow);">
          📅 El plan comienza en LUNES (la fecha de inicio seleccionada no se utiliza, el plan se organiza por semanas desde el lunes)
        </div>
      `;
   ...
   * CORREGIDO: ya no depende de ninguna función externa inexistente
   */
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad) {
    ...
    // La sensación se obtiene siempre de la fase
...
console.log('✅ PlanGenerator v14.3 - CORREGIDO: error de función eliminado, nota sobre inicio en lunes, día largo respetado');