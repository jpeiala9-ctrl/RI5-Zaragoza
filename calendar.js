...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    // Generar el plan y ver su calendario ya no requiere premium: un
    // usuario gratis puede crear su plan y ver la cuadrícula de días con
    // normalidad. Lo único que sigue bloqueado tras el modal premium es
    // el DETALLE de cada sesión al pulsar un día (ver abrirDetalleSesion
    // más abajo).
    try {
      Utils.showLoading();
  ...
  mostrarCalendario(sesiones) {
    ...
    if (!puedeVerDetalle) {
      ...
      if (notaPlan) {
        ...
        // Antes decía "puedes ver detalles de las primeras 2 semanas",
        // pero eso nunca se llegó a implementar de verdad (puedeVerDetalleSesion
        // bloquea SIEMPRE el detalle si no eres premium). Se corrige el
        // texto para que no prometa algo que la app no hace: con el plan
        // gratis se ve el calendario pero ninguna sesión en detalle.
        msgExpirado.innerHTML = '⚠️ Versión gratuita: puedes ver tu calendario, pero para abrir el detalle de las sesiones necesitas ser Premium. <button onclick="showPremiumBenefits()">HAZTE PREMIUM</button>';
