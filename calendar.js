// ============================================================================
// calendar.js - VERSIÓN DEFINITIVA CORREGIDA
// Días anteriores: "YA PASÓ" con número real
// Métricas en una línea
// Descripciones mejoradas por tipo y zona
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // ===== DÍAS ANTERIORES A LA FECHA DE INICIO (CORREGIDO) =====
      ...
      if (diasDesdeLunes > 0) {
        ...
        for (let i = 0; i < diasDesdeLunes; i++) {
          ...
          diasRelleno.push({
            ...
            fase: 'PREVIO',
            nivel: nivelInicial,
            tipo: esDiaEntreno ? 'pasado' : 'descanso',
            color: esDiaEntreno ? 'sesion-pasada' : 'sesion-descanso',
            ...
            pasado: true,
            bloqueado: false // NO está bloqueado, solo es informativo
          });
        }
      }
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad) {
    ...
    if (sesionMatriz) {
      ...
      estructuraDetallada = sesionMatriz.estructura || `${duracion}' continuos`;
    } else {
      ...
      estructuraDetallada = `${duracion}' continuos`;
    }
  ...
  // ==========================================================================
  // ABRIR DETALLE DE SESIÓN (VERSIÓN CORREGIDA - MINIMALISTA)
  // ==========================================================================
  abrirDetalleSesion(sesion, diaIndex) {
    if (!sesion) return;
    if (!AppState.puedeVerDetalleSesion() && !sesion.pasado) {
    ...
    // ===== SESIONES PASADAS (días anteriores a la fecha de inicio) =====
    if (sesion.tipo === 'pasado') {
      wrapper.classList.add('sesion-pasada');
      
      // Obtener fecha formateada
      let fechaStr = '';
      if (sesion.fecha) {
        const fecha = new Date(sesion.fecha);
        fechaStr = fecha.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
      }
      
      titulo.innerText = "📅 DÍA ANTERIOR AL INICIO";
      
      let descHtml = `
        <div style="text-align: center; padding: 10px;">
          <div style="font-size: 48px; margin-bottom: 10px;">📅</div>
          <p style="font-size: 16px; margin-bottom: 5px; color: var(--text-secondary);">${fechaStr}</p>
          <p style="margin: 15px 0;">Este día ya pasó antes de comenzar el plan.</p>
      `;
      
      // Si hay detalle de la sesión planificada, mostrarlo
      if (sesion.detalle) {
        descHtml += `
          <div style="margin-top: 20px; padding: 15px; background: var(--bg-primary); border-radius: 10px; text-align: left;">
            <h4 style="margin: 0 0 10px 0; color: var(--accent-blue);">ENTRENO PLANIFICADO PARA ESTE DÍA:</h4>
    ...
    // ===== SESIONES DE ENTRENO =====
      ...
      titulo.innerText = `${icono} ${sesion.tipo.toUpperCase()}${nivelTexto}${faseTexto}: ${sesion.detalle.nombre}`;

      // ===== MÉTRICAS EN UNA SOLA LÍNEA (MINIMALISTA) =====
      const tiempoTotal = this.formatearTiempo(sesion.duracion);
      const metricas = this.calcularMetricasSesion(sesion);

      const metricasHTML = `
        <div style="background: var(--bg-primary); padding: 6px 12px; margin: 10px 0; border-radius: 20px; display: inline-block; font-size: 13px; color: var(--text-secondary);">
          <span style="margin-right: 12px;">🕒 ${tiempoTotal}</span>
          <span style="margin-right: 12px;">📏 ${metricas.distanciaTotal.toFixed(2)} km</span>
          <span>⚡ ${metricas.tssTotal} TSS</span>
        </div>
      `;

      // ===== DESCRIPCIÓN MEJORADA (coherente con zonas) =====
      let descripcionMejorada = sesion.detalle.descripcion || '';
      
      // Si la descripción es genérica, la mejoramos según el tipo y zona
      if (!descripcionMejorada || descripcionMejorada.includes('Sesión de')) {
        const zona = sesion.detalle.zona || 'Z2';
        const ritmo = sesion.detalle.ritmoObjetivo;
        const fc = sesion.detalle.fcObjetivo;
        
        const descripcionesPorTipo = {
          rodaje: `Rodaje aeróbico en ${zona}. Mantén un ritmo conversacional durante toda la sesión. El objetivo es desarrollar base aeróbica y mejorar la capilarización muscular.`,
          tempo: `Entrenamiento de tempo en ${zona}. El esfuerzo debe ser "cómodamente duro". Debes poder decir frases cortas pero no mantener una conversación. Mejora el umbral de lactato.`,
          series: `Trabajo de calidad en ${zona}. Las repeticiones se realizan a alta intensidad con recuperaciones completas. Mejora la potencia aeróbica y la tolerancia al lactato.`,
          largo: `Tirada larga en ${zona}. Mantén un ritmo estable y controlado. El objetivo es desarrollar resistencia específica para la distancia objetivo.`
        };
        
        descripcionMejorada = descripcionesPorTipo[sesion.tipo] || `Sesión de ${sesion.tipo} en ${zona}.`;
        
        // Añadir información de ritmo y FC si están disponibles
        if (ritmo && ritmo !== '--:--') {
          descripcionMejorada += ` Ritmo objetivo: ${ritmo} min/km.`;
        }
        if (fc && !fc.includes('--')) {
          descripcionMejorada += ` Frecuencia cardíaca: ${fc}.`;
        }
      }

      // ===== ESTRUCTURA DE LA SESIÓN (respetando la original) =====
      let estructuraHTML = '';
      if (sesion.detalle.estructura) {
        // Mostrar la estructura original sin modificaciones
        const partes = sesion.detalle.estructura.split('+').map(p => p.trim());
        
        estructuraHTML = '<div style="margin-top: 20px;"><h4 style="color: var(--accent-blue); margin-bottom: 10px; font-size: 14px;">📋 ESTRUCTURA</h4>';
        
        partes.forEach((parte, index) => {
          let icono = '';
          if (parte.toLowerCase().includes('calentamiento')) icono = '🔥';
          else if (parte.toLowerCase().includes('enfriamiento')) icono = '🧘';
          else icono = '💪';
          
          estructuraHTML += `
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px; padding: 5px 0; border-bottom: 1px solid var(--border-color-light);">
              <span style="font-size: 16px;">${icono}</span>
              <span style="font-size: 13px;">${parte}</span>
            </div>
          `;
        });
      ...
      let descHtml = `
        ...
        <div style="background: var(--bg-primary); padding: 15px; margin: 15px 0; border-radius: 10px; text-align: left;">
          <p style="line-height: 1.6; color: var(--text-primary); margin: 0;">${descripcionMejorada}</p>
        </div>
        ${estructuraHTML}
      `;
    ...
    // ===== DÍAS DE DESCANSO =====
      ...
      descripcion.innerHTML = `
        <div style="text-align: center; padding: 10px;">
          <div style="font-size: 48px; margin-bottom: 10px;">😴</div>
          <p style="font-size: 16px; margin-bottom: 5px; color: var(--text-secondary);">${fechaStr}</p>
          <p style="font-size: 18px; margin: 15px 0;">Día de descanso</p>
          <div style="text-align: left; background: var(--bg-primary); padding: 15px; border-radius: 10px;">
            ...
            <ul style="list-style: none; padding: 0;">
              <li style="margin: 8px 0; display: flex; align-items: center; gap: 8px;">
                <span>🧘</span> Estiramientos suaves (15-20 min)
              </li>
              <li style="margin: 8px 0; display: flex; align-items: center; gap: 8px;">
                <span>🌀</span> Foam roller para liberar tensiones
              </li>
              <li style="margin: 8px 0; display: flex; align-items: center; gap: 8px;">
                <span>🚶</span> Paseo activo de 30-45 minutos
              </li>
            </ul>
          </div>
        </div>
...
console.log('✅ PlanGenerator v16.0 - DEFINITIVO: días pasados con 📅, métricas en línea, descripciones mejoradas');