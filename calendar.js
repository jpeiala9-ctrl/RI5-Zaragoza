...
const PlanGenerator = {
  ...
  // reales o planificados) en sus 3 tramos -- calentamiento, parte
  // principal y enfriamiento -- devolviendo minutos, km y zona de cada
  // uno. Es la base tanto de la detección de zona real al marcar una
  // sesión como del desglose que se guarda en cada entrada del muro/perfil/
  // dashboard para que las tarjetas puedan mostrarlo (ver
  ...
  // es la parte principal real, cuya zona se detecta por su propio ritmo.
  // Si la sesión no tiene calentamiento/enfriamiento definidos, o el
  // resultado no sale coherente (tiempo/distancia negativos), se devuelve
  // un único tramo de "parte principal" con el ritmo medio de toda la
  // sesión -- igual que se hacía antes de tener este desglose.
  _desglosarSesion(sesion, totalMin, totalKm) {
    ...
    if (paceMain > paceMedia * 1.001) return soloPartePrincipal;
  ...
  calcularMetricasSesion(sesion, factorIntensidad = 1.0) {
    ...
    if (detalle.tipoEstructura === 'fartlek' && detalle.segundosFuertes !== undefined && detalle.segundosSuaves !== undefined) {
      ...
      const ritmoSuaveSegGen = ritmoBase * 1.35 / factorIntensidad * 60;
    ...
    const ritmoSuaveMin = ritmoBase * factoresRitmo['Z1'] / factorIntensidad;
  ...
  // Recalcula SOLO detalle.distanciaEstimada/detalle.tssEstimada de cada
  // sesión del plan ACTUAL del usuario, con la fórmula corregida de
  // calcularMetricasSesion (calentamiento/parte principal/enfriamiento con
  // su propio ritmo, en vez de un ritmo único para toda la sesión -- ver
  // el comentario en calcularMetricasSesion). No toca nada más de cada
  // sesión (nombre, estructura, pasos, zona, fechas...) ni las sesiones
  // que YA estén marcadas como realizadas: esas guardan su resultado real
  // en globalFeed/sesionesRealizadas por separado, y no se ven afectadas.
  // Sirve para poner al día un plan que se generó ANTES de esa corrección,
  // sin tener que generarlo de nuevo desde cero (lo que perdería el
  // progreso/historial ya marcado).
  //
  // NOTA: el factor de intensidad real usado al generar cada sesión
  // (periodización/ACWR/feedback de esa semana concreta) no se guarda por
  // sesión, así que aquí se recalcula con factorIntensidad=1.0 (el mismo
  // valor por defecto que ya usaba calcularMetricasSesion). Es una
  // aproximación razonable: el efecto de esto es mucho menor que el fallo
  // estructural que se corrige (que ni siquiera separaba correctamente
  // calentamiento/parte principal/enfriamiento).
  async recalcularDistanciasPlanActual() {
    if (!AppState.currentUserId || !AppState.planActualId) {
      Utils.showToast('No hay un plan activo que recalcular', 'info');
      return;
    }
    try {
      Utils.showLoading('Recalculando distancias del plan...');
      ...
      if (!planDoc.exists) { Utils.hideLoading(); return; }
      ...
      sesiones.forEach(s => {
        ...
        const tssNuevo = metricas.tssTotal || 0;
        if (distNueva !== s.detalle.distanciaEstimada || tssNuevo !== s.detalle.tssEstimada) {
          s.detalle.distanciaEstimada = distNueva;
          s.detalle.tssEstimada = tssNuevo;
          cambios++;
        }
      });
      ...
      // los km/TSS nuevos sin tener que salir y volver a entrar a la
      // pestaña PLAN.
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          if (realDistance !== null && realDurationMs !== null) {
            ...
            // Desglose real en calentamiento/parte principal/enfriamiento
            // (min/km/zona de cada tramo, ver _desglosarSesion) a partir de
          ...
          const entry = {
            ...
            // enfriamiento (min/km/zona de cada tramo, ver
  ...
  calcularTSSdesdeReal(distanciaKm, duracionMs, sesion, desglose = null) {
    ...
    if (desglose && desglose.partePrincipal && desglose.partePrincipal.min > 0) {
      let tssAcumulado = 0;
      ['calentamiento', 'partePrincipal', 'enfriamiento'].forEach(fase => {
