// ==================== calendar.js - VERSIÓN FINAL 2.16 (CORREGIDA) ====================
// - Ondulación de intensidad ahora se aplica correctamente
// - Simulación de competición usa el objetivo real del plan
// ============================================================================
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // Aplicar ondulación de intensidad combinada con ajuste por feedback
        ...
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          if (!diasEntreno.includes(diaSemana)) {
            ...
            const sesion = await this.crearSesionDesdeMatriz(
              ...
              ajusteVolumen,
              intensidadTotal,   // ← aquí se aplica la intensidad ondulatoria
              minutos
            );
            semana.push({
              diaGlobal: diaGlobalCounter++,
...
console.log('✅ PlanGenerator v2.16 - Con ondulación corregida, simulación alineada al objetivo real, series avanzadas y fuerza variada');