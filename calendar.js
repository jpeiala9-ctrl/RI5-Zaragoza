// ==================== calendar.js - VERSIÓN FINAL REFACTORIZADA (SIN FEED, CON MURO) ====================
// Versión: 2.34 - Añadida gestión del muro global (globalFeed) al marcar/desmarcar sesión
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== MARCAR SESIÓN REALIZADA (CON MURO GLOBAL) ==========
  async marcarSesionRealizada(diaIndex, realizada) {
    ...
    try {
      ...
      // ========== GESTIÓN DEL MURO GLOBAL ==========
      if (realizada) {
        // Obtener la sesión completa para extraer métricas
        ...
        if (sesion && sesion.detalle) {
          ...
          // Guardar en globalFeed y guardar el ID para poder eliminarlo después
          const globalRef = await firebaseServices.db.collection('globalFeed').add(entry);
          // Guardar referencia en el plan para poder eliminar después
          await planRef.update({ [`wallEntryId.${diaIndex}`]: globalRef.id });
        }
      } else {
        // Eliminar del muro global si existía
        const planDoc = await planRef.get();
...
console.log('✅ PlanGenerator v2.34 - Con gestión del muro global');