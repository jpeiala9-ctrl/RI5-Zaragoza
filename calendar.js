...
const PlanGenerator = {
  ...
  // Suma los minutos de un desglose (ver _desglosarSesion) cuyos tramos
  // (calentamiento/parte principal/recuperación/enfriamiento) tengan
  // exactamente la zona pedida (p.ej. 'Z4' o 'Z5'). Se usa para las
  // insignias ZONE_4_60/ZONE_5_30 (ver Gamification.updateAfterSession):
  // antes nada en la app calculaba estos minutos, así que esas insignias
  // eran imposibles de conseguir por mucho que se entrenara en esas zonas.
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // La fuerza no es una carrera: no hay distancia/ritmo/zona que medir
      // con GPS, así que el botón no debe aparecer para tipo 'strength'
      // (antes salía igualmente, y si el usuario llegaba a usarlo la
      // sesión se registraba con datos de GPS que no le correspondían).
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          if (sesion.tipo === 'strength') {
            // La fuerza no es una carrera: no hay km/ritmo/zona que
            // calcular ni desglose que reconstruir. La duración es
            // siempre la planificada (no se pide corregirla al marcar,
            // ver el checkbox más arriba), y lo único que sí depende del
            // usuario son las calorías (por su peso), ya calculadas al
            // aceptar la sesión (detalle.caloriasEstimadas) o, si no
            // existieran (sesión antigua), estimadas aquí mismo.
            ...
            metricas = {
              ...
              // Minutos reales en zona 4 y zona 5 para las insignias
              // ZONE_4_60/ZONE_5_30 (ver Gamification.updateAfterSession):
              // se leen del desglose real de ESTA sesión (con o sin GPS),
              // sumando los tramos (calentamiento/parte principal/
              // recuperación/enfriamiento) cuya zona sea exactamente esa.
              // Antes esto no se calculaba en ningún sitio, así que esas
              // dos insignias eran imposibles de conseguir.
          ...
          const entry = {
            ...
            // Solo relevante en sesiones de fuerza (no hay km que
            // convertir en kcal como en carrera, así que se guarda la
            // estimación calculada arriba). null en el resto de tipos.
            calorias: metricas.calorias || null
          };

          // La fuerza no tiene desglose de km/zona que reconstruir al
          // abrir el detalle de la sesión ya completada (ver
          // abrirDetalleEntrenamientoCompletado en index.html): se
          // copian tal cual los pasos ORIGINALES que escribió el admin
          // (calentamiento/ejercicios/estiramientos con sus minutos),
          // para que esa sesión siga siendo legible después de marcarla.
        ...
        if (wallEntryId) {
          try {
            ...
            if (wallDoc.exists) {
              ...
              // Minutos en Z4/Z5 que esta sesión había aportado (ver
              // ZONE_4_60/ZONE_5_30), para poder restarlos igual que la
              // distancia -- si no, desmarcar dejaría esos minutos
              // "pegados" para siempre en el total.
