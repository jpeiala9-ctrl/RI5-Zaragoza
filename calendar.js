// ==================== calendar.js - VERSIÓN FINAL 2.21 ====================
// - CORREGIDO: Tiempo de sesión coherente con estructura real de series rotas.
// - Adaptación dinámica de series: pirámide, rotas, fartlek, cuestas ajustan
//   repeticiones/distancia según duración asignada por la progresión semanal.
// - Métricas de distancia y TSS calculadas con ritmos realistas.
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== FUNCIÓN PARA OBTENER RITMO REALISTA ==========
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // Mínimo de 45 minutos para series (excepto en taper)
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    ...
    if (tipo === 'series' && (nivel !== 'principiante' || fase !== 'BASE')) {
      // Decidir estructura aleatoriamente entre las disponibles
    ...
    // Sesiones básicas (rodaje, tempo, largo)
  ...
  // ================== SESIONES AVANZADAS DE SERIES (ADAPTATIVAS) ==================
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    // Calentamiento y enfriamiento fijos para sesiones de calidad
    const calentamiento = Math.min(20, Math.max(12, Math.floor(duracion * 0.2)));
    const enfriamiento = Math.min(15, Math.max(8, Math.floor(duracion * 0.15)));
    ...
    // Variable para guardar el tiempo real en zona (solo series)
    let tiempoEnZonaSeg = 0;

    switch(estructura) {
      case 'piramide': {
        nombre = 'Pirámide de series';
        descripcion = 'Estructura en pirámide ascendente y descendente, adaptada al tiempo disponible.';
        // Definir una pirámide base (distancias en metros)
        const baseDistancias = [200, 400, 600, 800, 600, 400, 200];
        // Tiempo total estimado de series si se hace una vez
        const tiempoSeriesBase = baseDistancias.reduce((acc, d) => acc + (d / 1000) * ritmoRapidoSeg, 0);
        // Si el tiempo disponible es mayor, repetir la pirámide o aumentar distancias; si es menor, reducir distancias o número de escalones
        let factor = partePrincipalSeg / tiempoSeriesBase;
        let distancias = [];
        if (factor >= 0.8 && factor <= 1.2) {
          distancias = baseDistancias;
        } else if (factor < 0.8) {
          // Reducir la pirámide: quitar escalones centrales o reducir distancias
          if (factor < 0.5) {
            distancias = [200, 400, 200];
          } else if (factor < 0.7) {
            distancias = [200, 400, 400, 200];
          } else {
            distancias = [200, 400, 600, 400, 200];
          }
        } else {
          // Aumentar la pirámide: repetir la pirámide completa
          const repeticiones = Math.floor(factor);
          if (repeticiones >= 2) {
            distancias = [];
            for (let r = 0; r < repeticiones; r++) {
              distancias.push(...baseDistancias);
            }
          } else {
            // Aumentar las distancias proporcionalmente
            distancias = baseDistancias.map(d => Math.round(d * factor));
          }
        }
        // Calcular recuperaciones entre series (en segundos)
        const recuperaciones = distancias.map((d, i) => {
          if (i === distancias.length - 1) return 0;
          // Recuperación proporcional a la distancia
          return Math.min(120, Math.max(40, d / 10));
        });
        ...
        const totalPartePrincipal = tiempoSeries + tiempoRec;
        partePrincipalSeg = totalPartePrincipal; // ajustar al real
        tiempoEnZonaSeg = tiempoSeries;
        ...
        accion = `Parte principal: ${Math.floor(totalPartePrincipal / 60)}:${Math.round(totalPartePrincipal % 60).toString().padStart(2,'0')} minutos. Pirámide: ${textoDistancias}. Realiza cada repetición a ritmo rápido (${ritmoRapido} min/km) – Z4-Z5 – recuperando caminando o trotando suave – Z2.`;
        ...
        const detallePiramide = {
          ...
          sensacion: 'Muy intenso', zona, duracion,
          ritmoObjetivo: ritmoRapido,
          fcObjetivo: `> ${Math.round(fcUmbral * 1.02)} lpm`,
          calentamiento, partePrincipal: Math.floor(totalPartePrincipal / 60), enfriamiento,
          objetivo: nombre, porque,
          pasosDetallados,
          ...
          tiempoEnZona: Utils.formatR(tiempoEnZonaSeg / 60) // minutos
        };
      ...
      case 'rotas': {
        ...
        // Elegir distancia de serie según distancia objetivo y tiempo disponible
        ...
        // Calcular tiempo que consumiría una configuración estándar
        const tiempoPorSerie = (distanciaSerie / 1000) * ritmoRapidoSeg;
        const tiempoSeriesBase = repPorBloque * bloques * tiempoPorSerie;
        const tiempoRecIntraBase = (repPorBloque - 1) * recIntra * bloques;
        const tiempoRecEntreBase = recEntreBloques * (bloques - 1);
        let totalBase = tiempoSeriesBase + tiempoRecIntraBase + tiempoRecEntreBase;

        // Ajustar dinámicamente: si el tiempo disponible es mayor, aumentar bloques o repeticiones
        ...
        if (factor > 1.2) {
          ...
          // Recalcular
          const tiempoSeries = repPorBloque * bloques * tiempoPorSerie;
          const tiempoRecIntra = (repPorBloque - 1) * recIntra * bloques;
          const tiempoRecEntre = recEntreBloques * (bloques - 1);
          totalBase = tiempoSeries + tiempoRecIntra + tiempoRecEntre;
          factor = partePrincipalSeg / totalBase;
        }
        if (factor < 0.8) {
          // Reducir distancia de serie
          if (distanciaSerie > 200) distanciaSerie -= 200;
          ...
          factor = partePrincipalSeg / totalBase;
          if (factor < 0.8) {
            // Reducir repeticiones por bloque
        ...
        const totalPartePrincipalReal = tiempoSeriesReal + tiempoRecIntraReal + tiempoRecEntreReal;
        partePrincipalSeg = totalPartePrincipalReal;
        tiempoEnZonaSeg = tiempoSeriesReal;
        ...
        const detalleRotas = {
          ...
          ritmoObjetivo: ritmoRapido,
          fcObjetivo: `> ${Math.round(fcUmbral * 1.02)} lpm`,
          calentamiento, partePrincipal: minutosPP, enfriamiento,
          objetivo: nombre, porque,
          pasosDetallados,
          ...
          ritmoSerie: ritmoRapido,
          ritmoRecuperacion: ritmoModerado,
          tiempoEnZona: Utils.formatR(tiempoEnZonaSeg / 60)
        };
      ...
      case 'fartlek': {
        ...
        let tiempoFuerte = 60; // segundos
        let tiempoSuave = 90;
        let duracionCiclo = (tiempoFuerte + tiempoSuave) / 60;
        ...
        if (repeticiones < 2) {
          // Ajustar duraciones para tener al menos 2 ciclos
        ...
        tiempoEnZonaSeg = segundosFuertes;

        accion = `Parte principal: ${Math.floor(totalSegundos / 60)}:${(totalSegundos % 60).toString().padStart(2,'0')} minutos. ${repeticiones} ciclos de ${tiempoFuerte}" fuerte + ${tiempoSuave}" suave${sobrante ? ` + ${sobrante}" suave final` : ''}. Realiza el fuerte a ritmo rápido (${ritmoRapido} min/km) – Z4-Z5 – y el suave a ritmo aeróbico (${ritmoModerado} min/km) – Z2. El cambio de ritmo debe ser continuo, sin detenerte.`;
        ...
        const detalleFartlek = {
          ...
          ritmoObjetivo: ritmoRapido,
          fcObjetivo: `> ${Math.round(fcUmbral * 1.02)} lpm`,
          calentamiento, partePrincipal: Math.floor(totalSegundos / 60), enfriamiento,
          objetivo: nombre, porque,
          pasosDetallados,
      ...
      case 'cuestas': {
        ...
        let duracionCuesta = 60; // segundos
        let recCuesta = 90;
        ...
        partePrincipalSeg = totalSegundos;
        tiempoEnZonaSeg = numCuestas * duracionCuesta;

        accion = `Parte principal: ${Math.floor(totalSegundos / 60)}:${(totalSegundos % 60).toString().padStart(2,'0')} minutos. ${numCuestas} repeticiones de ${duracionCuesta}" en cuesta con desnivel moderado, recuperando trotando suave ${recCuesta}" – Z2. Esfuerzo máximo (Z5) en la subida.`;
  ...
  // ========== CÁLCULO DE MÉTRICAS CON RITMOS REALES ==========
  calcularMetricasSesion(sesion, factorIntensidad = 1.0) {
    ...
    // Si es una sesión avanzada con datos específicos, calcular distancia real
    ...
    document.querySelectorAll('.calendario-dia[data-index]').forEach(dia => {
      dia.addEventListener('click', (e) => {
        ...
        if (diaIndex && sesiones[diaIndex - 1]) {
          ...
          console.warn('No se encontró sesión para el día', diaIndex);
        }
      });
    });
  },
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      const tiempoEnZona = sesion.detalle.tiempoEnZona ? `${sesion.detalle.tiempoEnZona} min` : '—';
      const headerHTML = `
        <div class="sesion-resumen-horizontal">
          ...
          <div class="resumen-item"><span>🔥 Z4‑Z5</span> ${tiempoEnZona}</div>
        </div>
      `;
      const objetivoHTML = `
...
console.log('✅ PlanGenerator v2.21 - Adaptativo: series ajustadas a tiempo disponible, tiempos coherentes');