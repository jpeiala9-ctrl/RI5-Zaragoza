// ============================================================================
// calendar.js - VERSIÓN DEFINITIVA CON RELLENO DE DÍAS PREVIOS
// El calendario siempre empieza en lunes
// Los días antes de la fecha inicio aparecen como COMPLETADOS
// El primer día REAL es el que el usuario elige
// ============================================================================
...
const PlanGenerator = {
  ...
  FASES: {
    BASE: { nombre: 'Base', color: '#707070', intensidad: 0.7, volumen: 0.8 },
    CONSTRUCCION: { nombre: 'Construcción', color: '#8A8A8A', intensidad: 0.85, volumen: 0.9 },
    ESPECIFICA: { nombre: 'Específica', color: '#A0A0A0', intensidad: 0.95, volumen: 1.0 },
    PICO: { nombre: 'Pico', color: '#B8B8B8', intensidad: 1.0, volumen: 0.8 },
    TAPER: { nombre: 'Taper', color: '#585858', intensidad: 0.6, volumen: 0.5 }
  },
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // ===== ASIGNACIÓN QUE RESPETA LOS DÍAS =====
      ...
      // ===== MAGIA: RELLENAR DÍAS ANTES DE LA FECHA DE INICIO =====
      const fechaInicio = new Date(fechaInicioStr + 'T12:00:00');
      const diaSemanaInicio = fechaInicio.getDay(); // 0 domingo, 1 lunes...
      
      // Calcular días desde el lunes anterior hasta la fecha de inicio
      let diasDesdeLunes = 0;
      if (diaSemanaInicio === 0) { // Domingo
        diasDesdeLunes = 6;
      } else {
        diasDesdeLunes = diaSemanaInicio - 1; // Lunes=0, Martes=1, etc.
      }

      // Crear días "completados" para antes de empezar
      const diasRelleno = [];
      if (diasDesdeLunes > 0) {
        const fechaRelleno = new Date(fechaInicio);
        fechaRelleno.setDate(fechaRelleno.getDate() - diasDesdeLunes);
        
        for (let i = 0; i < diasDesdeLunes; i++) {
          const fechaRellenoActual = new Date(fechaRelleno);
          fechaRellenoActual.setDate(fechaRelleno.getDate() + i);
          
          // Determinar si este día era de entrenamiento según los días marcados
          const diaSemanaRelleno = fechaRellenoActual.getDay(); // 0 domingo, 1 lunes...
          const diaSemanaRellenoNum = diaSemanaRelleno === 0 ? 7 : diaSemanaRelleno; // Convertir a 1-7 (lunes=1)
          ...
          diasRelleno.push({
            ...
            fase: 'PREVIO',
            nivel: nivel,
            tipo: esDiaEntreno ? 'completado' : 'descanso',
            color: esDiaEntreno ? 'sesion-completada' : 'sesion-descanso',
            letra: esDiaEntreno ? '✓' : 'D',
            detalle: esDiaEntreno ? { 
              nombre: 'Día completado', 
              descripcion: 'Este día ya pasó. Se marca como completado.',
              estructura: 'Entreno realizado antes de empezar el plan'
            } : null,
            completado: true,
            bloqueado: true
          });
      ...
      // Ajustar diaGlobal del planCompleto
  ...
  // ==========================================================================
  // RENDERIZADO DE PÁGINA - SIN OFFSET, USA diaGlobal
  // ==========================================================================
  renderizarPagina(sesiones) {
    ...
    for (let i = inicioPagina; i < finPagina; i++) {
      ...
      // diaGlobal empieza en 1 para el primer día del plan (incluyendo relleno)
      // Calculamos la posición absoluta en el grid
    ...
    // Generar grid en orden (fila por fila)
    for (let pos = 0; pos < celdas.length; pos++) {
      ...
      let contenidoHtml = '';
      if (sesion.tipo !== 'descanso' && sesion.tipo !== 'completado' && sesion.detalle) {
        contenidoHtml = `<strong>${sesion.letra}</strong><div>${diaDelMes} ${diaSemanaAbr}</div>`;
      } else if (sesion.tipo === 'completado') {
        contenidoHtml = `<strong>✓</strong><div>${diaDelMes} ${diaSemanaAbr}</div>`;
      } else {
        contenidoHtml = `<strong>D</strong><div>${diaDelMes} ${diaSemanaAbr}</div>`;
      }

      html += `<div class="calendario-dia ${sesion.color} ${realizada} ${bloqueado}" data-index="${sesion.diaGlobal}"${faseIndicator} data-bloqueado="${sesion.bloqueado || false}">${contenidoHtml}</div>`;
    }

    grid.innerHTML = html;
    this.agregarLeyendaFases();

    // Event listeners (solo para días no bloqueados)
    document.querySelectorAll('.calendario-dia[data-index]:not(.bloqueado)').forEach(dia => {
      dia.addEventListener('click', (e) => {
        ...
        if (diaIndex && sesiones[diaIndex - 1]) { // diaGlobal empieza en 1, array en 0
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.tipo !== 'completado' && sesion.detalle) {
      ...
      wrapper.classList.add('sesion-completada');
      titulo.innerText = "✓ DÍA COMPLETADO";
      descripcion.innerHTML = `
        <div class="descanso-container">
          <div class="descanso-icono">✓</div>
          <p class="descanso-texto">Este día ya pasó</p>
          <p>El entrenamiento correspondiente a este día ya fue realizado antes de comenzar el plan.</p>
        </div>
      `;
...
console.log('✅ PlanGenerator v10.0 - DEFINITIVO: días previos completados, primer día real respetado');