// ==================== calendar.js - VERSIÓN ESTABLE CON GPS PROTEGIDO ====================
// Versión: 2.50 - Corregido error de carga del plan y botón "Nuevo Plan"
// ============================================================================
...
const PlanGenerator = {
  ...
  calcularMetricasSesion(sesion, factorIntensidad = 1.0) {
    ...
    const ritmoBase = AppState.planGeneradoActual?.ritmoBase || AppState.lastRitmoBase || 5;
    const fcUmbral = AppState.planGeneradoActual?.fcUmbral || AppState.lastUL || 150;
  ...
  mostrarCalendario(sesiones) {
    ...
    if (!grid) {
      console.error('No se encontró calendarioGrid');
      return;
    }
  ...
  // ==================== FUNCIÓN CORREGIDA CON INTEGRACIÓN GPS (PROTEGIDA) ====================
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // ========== CONTROLES DE GPS (PROTEGIDOS) ==========
      if (!realizada && typeof GPSTracker !== 'undefined' && GPSTracker.startTracking) {
        try {
          ...
          console.error('Error al inicializar GPS:', gpsError);
          Utils.showToast('No se pudo cargar el módulo GPS', 'warning');
        }
      }
      
      // Si la sesión ya está realizada y tiene ruta GPS, mostrarla
      if (realizada && sesion.gpsRoute && typeof GPSTracker !== 'undefined' && GPSTracker.displayRoute) {
        try {
          ...
          console.error('Error mostrando ruta GPS guardada:', gpsError);
        }
      }
  ...
  toggleCuestionario() {
    ...
    if (!cuestionario) {
      ...
      Utils.showToast('Error interno, recarga la página', 'error');
      return;
    }
    ...
    Utils.showToast(isVisible ? 'Cuestionario oculto' : 'Completa los datos del plan', 'info', 1000);
  },
...
console.log('✅ PlanGenerator v2.50 - con GPS protegido y correcciones de carga');