// ==================== calendar.js - VERSIÓN FINAL REFACTORIZADA ====================
// Versión: 2.30 - Al desmarcar sesión, elimina la entrada del feed (removeFromFeed)
// ============================================================================
...
// ========== FUNCIONES DE REDONDEO CENTRALIZADAS ==========
...
// ========== OBTENER RITMO DESDE ZONAS CON FALLBACK ==========
...
// ========== HELPERS COMUNES PARA CREACIÓN DE SESIONES ==========
...
const detalle = {
nombre, descripcion, estructura, sensacion, zona: zonaPrincipal, duracion,
ritmoObjetivo, fcObjetivo, calentamiento, partePrincipal, enfriamiento,
objetivo, porque, pasosDetallados,
…metricasExtra
};
...
// ========== OBTENER OBJETIVO TEXTO Y PORQUÉ (unificados) ==========
...
// ========== SESIÓN BÁSICA (rodaje, tempo, largo) ==========
async crearSesionBasica(tipo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
...
if (zonaPrincipal === 'Z5') fcObjetivo = `> ${Math.round(fcRangos.Z5[0] * fcAjuste)} lpm`;
else if (zonaPrincipal === 'Z4') fcObjetivo = `${Math.round(fcRangos.Z4[0] * fcAjuste)}-${Math.round(fcRangos.Z4[1] * fcAjuste)} lpm`;
else if (zonaPrincipal === 'Z3') fcObjetivo = `${Math.round(fcRangos.Z3[0] * fcAjuste)}-${Math.round(fcRangos.Z3[1] * fcAjuste)} lpm`;
else if (zonaPrincipal === 'Z2') fcObjetivo = `${Math.round(fcRangos.Z2[0] * fcAjuste)}-${Math.round(fcRangos.Z2[1] * fcAjuste)} lpm`;
else fcObjetivo = `${Math.round(fcRangos.Z1[0] * fcAjuste)}-${Math.round(fcRangos.Z1[1] * fcAjuste)} lpm`;
...
const datosBasicos = {
  ...
  ritmoObjetivo: this.obtenerRitmoParaZona('Z5'), fcObjetivo: `> ${Math.round(fcUmbral * 1.02)} lpm`,
...
// Sesiones básicas
return this.crearSesionBasica(tipo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta);
```
...
const headerHTML = `<div class="sesion-resumen-horizontal"> <div class="resumen-item"><span>🕒</span> ${tiempoTotal}</div> <div class="resumen-item"><span>📏</span> ${metricas.distanciaTotal.toFixed(2)} km</div> <div class="resumen-item"><span>⚡</span> ${metricas.tssTotal} TSS</div> <div class="resumen-item"><span>🔥 Z4‑Z5</span> ${tiempoEnZona}</div> </div>`;
const objetivoHTML = `<div class="sesion-objetivo-principal"> <h4>🎯 OBJETIVO PRINCIPAL</h4> <p><strong>${sesion.detalle.objetivo || 'Sesión de calidad'}</strong></p> <p class="porque">${sesion.detalle.porque || ''}</p> </div>`;
const zonasHTML = `<div class="sesion-zonas"> <div class="zona-item"><span>⏱️ Ritmo</span><strong>${sesion.detalle.ritmoObjetivo} min/km</strong></div> <div class="zona-item"><span>❤️ FC</span><strong>${sesion.detalle.fcObjetivo}</strong></div> <div class="zona-item"><span>😌 Sensación</span><strong>${sesion.detalle.sensacion}</strong></div> <div class="zona-item"><span>📊 Zona</span><strong>${sesion.detalle.zona}</strong></div> </div>`;
...
pasosHTML += `<div class="paso-detalle-sesion"> <div class="paso-header"><span>${paso.icono}</span><strong>${paso.titulo}</strong></div> <p class="paso-accion">${paso.accion}</p> <p class="paso-porque"><em>${paso.porque}</em></p> </div>`;
});
...
pasosHTML += `<div class="paso-detalle-sesion"> <div class="paso-header"><span>${iconoPaso}</span><strong>${tituloPaso}</strong></div> <p class="paso-accion">${parte}</p> </div>`;
});
}
pasosHTML += ‘</div>’;
...
descripcion.innerHTML = `<div class="descanso-container"> <div class="descanso-icono">😴</div> <p class="descanso-texto">Día de descanso</p> <div class="descanso-objetivo"> <h4>🎯 OBJETIVO</h4> <p>${objetivoDescanso}</p> <p class="porque">${porqueDescanso}</p> </div> <ul class="descanso-recomendaciones"> <li>🧘 Estiramientos suaves</li> <li>🌀 Foam roller</li> <li>🚶 Paseo activo</li> <li>💧 Hidratación adecuada</li> </ul> </div>`;
...
// ========== MARCAR SESIÓN REALIZADA (con eliminación del feed al desmarcar) ==========
...
```
  ...
  if (realizada) {
    ...
    // ELIMINAR DEL FEED AL DESMARCAR
    await Storage.removeFromFeed(AppState.currentUserId, AppState.planActualId, diaIndex);
  }
  
  Utils.showToast(realizada ? '✅ Sesión marcada' : '📝 Sesión desmarcada (feed actualizado)', 'success');
} catch (error) {
...
// ========== VALIDACIÓN DE OPCIONES DEL PLAN ==========
validarOpcionesPlan() {
const distancia = document.getElementById(“distObjetivo”)?.value;
...
// cerrarPlan: oculta el calendario sin borrar el plan guardado
window.cerrarPlan = () => {
document.getElementById(‘calendarioEntreno’).style.display = ‘none’;
document.getElementById(‘cuestionarioEntreno’).style.display = ‘block’;
};
...
console.log(‘✅ PlanGenerator v2.30 - Al desmarcar sesión se elimina del feed’);