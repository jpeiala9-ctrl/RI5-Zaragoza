...
// Versión: 2.50 - Marcado completo (verde) y parcial (naranja) con publicación en muro para ambos
// ============================================================================
...
const PlanGenerator = {
  ...
  renderizarPagina(sesiones, semanasTotales) {
    ...
    for (let pos = 0; pos < celdas.length; pos++) {
      ...
      if (estado && estado.realizada) {
        realizadaClase = estado.tipo === 'completo' ? 'realizado' : 'realizado-parcial';
      } else if (estado && estado === true) { // Compatibilidad con formato antiguo
  ...
  // ==================== FUNCIÓN PRINCIPAL DE MARCADO (ACTUALIZADA v2.50) ====================
  async marcarSesionRealizada(diaIndex, realizada, tipo = 'completo', trackPoints = null, gpsDistance = null, silent = false) {
    ...
    try {
      ...
      if (celda) {
        ...
        if (realizada && tipo === 'completo') celda.classList.add('realizado');
        else if (realizada && tipo === 'parcial') celda.classList.add('realizado-parcial');
      }
      ...
      if (debePublicar) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          if (window.Gamification && tipo === 'completo') {
            ...
            // Si es parcial y tiene GPS, refrescamos el muro
            if (document.getElementById('tab-muro')?.classList.contains('active') && window.Wall)
              await Wall.cargarMuro();
          }
        }
      } else if (!realizada) {
        // Desmarcar: eliminar del muro
...
console.log('✅ PlanGenerator v2.50 - Marcado completo (verde) y parcial (naranja) con publicación en muro');