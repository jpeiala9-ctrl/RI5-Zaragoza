...
// Solo respeta los días marcados. El plan empieza el lunes.
// ============================================================================
...
const PlanGenerator = {
   ...
   * CORREGIDO: la sensación se obtiene siempre de la fase, no de la matriz.
   * CORREGIDO: los pasos se generan respetando la estructura real de la sesión (múltiples zonas)
   */
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad) {
    ...
    let objetivo = '';
    let porque = '';
    let zonaPrincipal = sesionMatriz.zona?.split('-')[0] || 'Z2';

    // Si la sesión de la matriz tiene una estructura con múltiples partes (ej. "25' Z2 + 15' Z3")
    // intentamos extraer partes para crear pasos más detallados
    if (sesionMatriz.estructura && sesionMatriz.estructura.includes('+')) {
      // Extraer partes de la estructura
      const partes = sesionMatriz.estructura.split('+').map(p => p.trim());
      // El primer paso es calentamiento, el último enfriamiento, los del medio son parte principal
      if (partes.length >= 3) {
        // Calentamiento
        const calentamientoParte = partes[0];
        pasosDetallados.push({
          icono: '🔥',
          titulo: 'CALENTAMIENTO',
          accion: calentamientoParte,
          porque: 'Preparar músculos, articulaciones y sistema cardiovascular.'
        });
        // Partes intermedias (principales)
        for (let i = 1; i < partes.length - 1; i++) {
          let icono = '💪';
          let titulo = 'TRABAJO PRINCIPAL';
          if (partes[i].toLowerCase().includes('rodaje')) titulo = 'RODAJE CONTINUO';
          else if (partes[i].toLowerCase().includes('tempo')) titulo = 'TEMPO';
          else if (partes[i].toLowerCase().includes('series')) titulo = 'SERIES';
          else if (partes[i].toLowerCase().includes('largo')) titulo = 'TIRADA LARGA';
        ...
        // Enfriamiento
        const enfriamientoParte = partes[partes.length - 1];
        pasosDetallados.push({
          icono: '🧘',
          titulo: 'ENFRIAMIENTO',
          accion: enfriamientoParte,
          porque: 'Facilitar la vuelta a la calma, eliminar lactato y prevenir lesiones.'
        });
        estructuraDetallada = sesionMatriz.estructura;
      } else {
        // Fallback a estructura simple
        this.generarPasosSimples();
      }
    } else {
      this.generarPasosSimples();
    }

    function generarPasosSimples() {
    ...
    // La zona que se mostrará en el resumen será la de la parte principal (la más intensa si hay varias)
    let zonaMostrada = zonaPrincipal;
    if (pasosDetallados.length > 2) {
      // Buscar la zona más intensa entre los pasos principales
      const zonasEnPasos = pasosDetallados.filter(p => p.titulo !== 'CALENTAMIENTO' && p.titulo !== 'ENFRIAMIENTO')
        .map(p => {
          const match = p.accion.match(/Z([1-6])/);
          return match ? match[1] : null;
        }).filter(z => z);
      if (zonasEnPasos.length) {
        const maxZona = Math.max(...zonasEnPasos.map(z => parseInt(z)));
        zonaMostrada = `Z${maxZona}`;
        if (zonasEnPasos.length > 1 && Math.max(...zonasEnPasos.map(z => parseInt(z))) !== Math.min(...zonasEnPasos.map(z => parseInt(z)))) {
          zonaMostrada = `Z${Math.min(...zonasEnPasos.map(z => parseInt(z)))}-Z${Math.max(...zonasEnPasos.map(z => parseInt(z)))}`;
        }
      }
    }
...
console.log('✅ PlanGenerator v14.1 - CORREGIDO: sensación según fase, pasos con múltiples zonas, métricas horizontales');