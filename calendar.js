// ==================== calendar.js - VERSIÓN COMPLETA 2.65 ====================
// Calendario siempre verde, fuerza excluida de sesiones GPS, parciales suman a gamificación
// ============================================================================
...
const PlanGenerator = {
  ...
  async crearSesionBasica(tipo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null, esRecuperacion = false, esGPS = false) {
    ...
    const sesionCompleta = this._buildSesionDetalle(tipo, fase, datosBasicos, pasosDetallados);
    // Si es sesión GPS y tiene fuerza, la eliminamos (no debe aparecer en el tracking)
    if (esGPS && sesionCompleta.tieneFuerza) {
      sesionCompleta.tieneFuerza = false;
      // Eliminar el paso de fuerza de los pasosDetallados
      if (sesionCompleta.detalle.pasosDetallados) {
        sesionCompleta.detalle.pasosDetallados = sesionCompleta.detalle.pasosDetallados.filter(paso => paso.titulo !== 'FUERZA COMPLEMENTARIA');
      }
    }
    return sesionCompleta;
  },

  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta, esGPS = false) {
    ...
    switch(estructura) {
      ...
      default:
        return this.crearSesionBasica('series', fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta, false, esGPS);
    }
    ...
    const sesionCompleta = this._buildSesionDetalle('series', fase, datosBasicos, pasosDetallados, metricasExtra);
    if (esGPS && sesionCompleta.tieneFuerza) {
      sesionCompleta.tieneFuerza = false;
      if (sesionCompleta.detalle.pasosDetallados) {
        sesionCompleta.detalle.pasosDetallados = sesionCompleta.detalle.pasosDetallados.filter(paso => paso.titulo !== 'FUERZA COMPLEMENTARIA');
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null, esGPS = false) {
    const { modalidad, distancia, ritmoBase, fcUmbral, semanasTotales, semanaGlobal, objetivo } = datos;
    ...
    if (tipo === 'series' && (nivel !== 'principiante' || fase !== 'BASE')) {
      ...
      return this.crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta, esGPS);
    }

    return this.crearSesionBasica(tipo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta, esRecuperacion, esGPS);
  },
        ...
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          if (!diasEntreno.includes(diaSemana)) {
            ...
            const sesion = await this.crearSesionDesdeMatriz(
              ...
              intensidadTotal,
              minutos,
              false // esGPS = false en la generación del plan (la fuerza sí puede aparecer)
            );
            semana.push({
              diaGlobal: diaGlobalCounter++,
              semana: semanaGlobal,
  ...
  // ========== RENDERIZADO DE CALENDARIO (SIEMPRE VERDE) ==========
  ...
  // ========== ABRIR DETALLE DE SESIÓN (CON BOTÓN GPS PASANDO PLANID Y SIN FUERZA) ==========
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // === BOTÓN GPS: iniciar sesión con grabación de track en vivo (sin fuerza) ===
      if (window.GPSTracker) {
        ...
        // Al iniciar GPS, pasamos un flag para que la sesión no incluya fuerza
        // La función GPSTracker.iniciar no necesita el flag, pero la sesión que se pasa ya no tiene fuerza
        // Si la sesión actual tiene fuerza, la ignoramos para el tracking
        const sesionSinFuerza = { ...sesion, tieneFuerza: false };
        if (sesionSinFuerza.detalle && sesionSinFuerza.detalle.pasosDetallados) {
          sesionSinFuerza.detalle.pasosDetallados = sesionSinFuerza.detalle.pasosDetallados.filter(paso => paso.titulo !== 'FUERZA COMPLEMENTARIA');
        }
        gpsBtn.addEventListener('click', () => GPSTracker.iniciar(sesionSinFuerza, diaIndex, AppState.planActualId));
    ...
    try {
      ...
      if (debePublicar) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          let ritmoMedio = null;
          if (tieneGPS && gpsDistance > 0.1 && sesion.duracion) {
          ...
          // Las sesiones parciales también suman a gamificación
...
console.log('✅ PlanGenerator v2.65 - Calendario siempre verde, fuerza excluida de GPS, parciales suman a gamificación');