// ==================== calendar.js - VERSIÓN COMPLETA Y CORREGIDA ====================
// Versión: 2.71 - Checkbox verde solo con GPS válido, naranja automático si falla, wake lock
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== OBTENER POSICIÓN PRECISA SIN CUENTA ATRÁS ==========
    ...
    return new Promise((resolve) => {
      ...
      const finalizar = (cancelled = false) => {
        ...
        if (cancelled) {
          ...
          if (bestPosition) {
            const finalPrecision = Math.round(bestAccuracy);
            const precisionMsg = finalPrecision <= 10 ? '✅ GPS preciso' : (finalPrecision <= 20 ? '⚠️ GPS aceptable' : '❌ GPS poco preciso');
            Utils.showToast(`GPS: ${precisionMsg} (${finalPrecision} m)`, finalPrecision <= 20 ? 'success' : 'warning', 2000);
          } else {
            Utils.showToast('⚠️ No se pudo obtener posición GPS. Se usará la última conocida.', 'warning');
          }
      ...
      watchId = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          if (accuracy < bestAccuracy) {
            ...
            bestPosition = pos;
            precisionSpan.innerText = `Precisión: ${Math.round(accuracy)} m (mejorando...)`;
          } else {
        ...
        (err) => {
          ...
          if (bestPosition) finalizar(false);
        },
        { enableHighAccuracy: true, maximumAge: 0, timeout: 10000 }
      );
      
      timeoutId = setTimeout(() => finalizar(false), 8000);
  ...
  // ========== ABRIR DETALLE CON GPS MEJORADO, WAKE LOCK Y MENSAJE ==========
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // Deshabilitar checkbox si no se ha grabado ruta válida (se activará solo tras grabación)
      checkbox.disabled = !realizada && !window._ultimaGrabacionGPSValida;
      if (!realizada && !window._ultimaGrabacionGPSValida) {
        checkbox.title = "Debes grabar la ruta con GPS para marcar como realizada (verde)";
      }
      checkbox.checked = realizada;
      checkbox.onchange = async (e) => {
        if (e.target.checked) {
          // Si intenta marcar sin GPS válido, lo convertimos en naranja
          if (!window._ultimaGrabacionGPSValida) {
            Utils.showToast('No hay datos GPS válidos. La sesión se marcará como completada manualmente (naranja).', 'warning');
            await this.marcarSesionForzada(diaIndex, true, null, false);
            e.target.checked = false;
            e.target.disabled = true;
            window._ultimaGrabacionGPSValida = false;
            return;
          }
          await this.marcarSesionRealizada(diaIndex, true);
        } else {
      ...
      if (!realizada) {
        ...
        let forceButtonCreated = false;
        let wakeLock = null;
        
        startBtn.onclick = async () => {
          console.log('🖱️ Click en Iniciar Grabación GPS');
          ...
          stopBtn.innerHTML = '⏹️ Finalizar Grabación';
          stopBtn.onclick = async () => {
            ...
            if (porcentaje <= 10) {
              window._ultimaGrabacionGPSValida = true;
              ...
              // Habilitar botón naranja para marcar forzado
              if (!forceButtonCreated) {
                const forceBtn = document.createElement('button');
                forceBtn.className = 'action-button';
                forceBtn.style.cssText = 'width: auto; background: #FF9800; border-color: #FF9800; color: var(--bg-primary); margin-top: 8px;';
                forceBtn.innerHTML = '🟠 Marcar como completada (manual)';
                forceBtn.onclick = async () => {
                  await this.marcarSesionForzada(diaIndex, true, lastRouteData, true);
                  Utils.showToast('🟠 Sesión marcada manualmente (naranja)', 'success');
                  cerrarModalSesion();
                };
                gpsControls.appendChild(forceBtn);
                forceButtonCreated = true;
              }
            }
  ...
  async _displayRoute(containerId, points) {
    ...
    L.circleMarker(latlngs[0], { radius: 6, color: '#4CAF50', fillColor: '#4CAF50', fillOpacity: 1 }).addTo(map);
    L.circleMarker(latlngs[latlngs.length-1], { radius: 6, color: '#F44336', fillColor: '#F44336', fillOpacity: 1 }).addTo(map);
  ...
  async mostrarUltimoPlanGuardado() {
    ...
    try {
      ...
      AppState.sesionesForzadas = planCompleto.sesionesForzadas || {};
...
console.log('✅ PlanGenerator v2.71 - Checkbox verde solo con GPS válido, naranja automático si falla, wake lock con mensaje');