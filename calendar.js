...
// Versión: 2.31 - Mejoras en distribución: descanso después de largo, largo máximo, calidad separada
// ============================================================================
...
const PlanGenerator = {
  ...
  async crearSesionBasica(tipo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null, esRecuperacion = false) {
    ...
    // Si es recuperación, forzar duración baja (25-30 min) y zona Z1
  ...
  // ========== NUEVAS FUNCIONES PARA MEJORAR DISTRIBUCIÓN ==========
  _ajustarDiaPostLargo(tiposPorDia, diaLargo, diasEntreno) {
    ...
    if (diasEntreno.includes(diaSiguiente) && tiposPorDia[diaSiguiente]) {
      ...
      // Si no es descanso, lo convertimos en rodaje de recuperación (muy suave)
  ...
  _mejorarDistribucionCalidad(tiposPorDia, diasEntreno, diaLargo) {
    ...
    for (let i = 0; i < diasCalidad.length - 1; i++) {
      ...
      if (siguiente === actual + 1 || (actual === 7 && siguiente === 1)) {
        // Buscar un día entre el actual y el largo que esté libre
        let nuevoDia = null;
        ...
        if (nuevoDia) {
          ...
          // Si no se puede mover, al menos reducir a rodaje suave
    ...
    // Intentar que la calidad esté antes del largo
...
console.log('✅ PlanGenerator v2.31 - Mejoras en distribución de entrenamientos');