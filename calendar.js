...
// Versión: 5.13 - CORREGIDO: días de entrenamiento fijos por día de la semana
// ============================================================================
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // Convertir la plantilla en un array de 7 elementos (lunes a domingo)
        const plantillaArray = [];
        for (let i = 1; i <= 7; i++) {
          plantillaArray.push(plantillaSemana[i] || { tipo: 'descanso' });
        }

        // Ahora generamos la semana día a día
        const semana = [];
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          // Determinar si este día es de entrenamiento
          const esDiaActivo = diasEntreno.includes(diaSemana);

          // Obtener el tipo base de la plantilla para este día
          let tipoBase = plantillaArray[diaSemana - 1].tipo;

          // Si es el día elegido para el largo, forzamos que el tipo sea 'largo'
          if (diaSemana === diaLargo) {
            tipoBase = 'largo';
          }

          // Si no es día activo, la sesión será descanso
          const tipoFinal = esDiaActivo ? tipoBase : 'descanso';

          // Crear la sesión (si es descanso, se crea sin detalle)
          let sesion;
          if (tipoFinal === 'descanso') {
            ...
            // Buscar en la matriz de entrenamientos una sesión del tipo correspondiente
            // Usamos la función crearSesionDesdeMatriz, pero necesitamos pasarle el tipo y la variante adecuada
            // Para simplificar, creamos un objeto sesionBase con el tipo y fromMatrix=true
            const sesionBase = { tipo: tipoFinal, fromMatrix: true };
          ...
          if (modalidad === 'trail' && libreriaPerfil.progresionDesnivel && sesion.tipo !== 'descanso') {
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, faseActual, semanaEnFase, libreriaPerfil, datos) {
    ...
    // Buscar en la base de datos de entrenamientos
    const dbTipo = this.ENTRENAMIENTOS[modalidad]?.[distancia]?.[nivel]?.[tipo];
    let sesionMatriz = null;
    if (dbTipo?.length) {
      // Si hay una variante específica, intentamos encontrarla
    ...
    if (tipo === 'series' && sesionMatriz.repeticiones && sesionMatriz.distanciaSerie) {
      // Series con repeticiones definidas
  ...
  // ==========================================================================
  // VISUALIZACIÓN DEL CALENDARIO (con offset por día de la semana)
  // ==========================================================================
  ...
  renderizarPagina(sesiones) {
    ...
    // Obtener la fecha de la primera sesión en esta página
    const primeraSesion = sesiones[inicioPagina];
    if (!primeraSesion) return;

    const fechaPrimera = new Date(primeraSesion.fecha);
    // Calcular offset: 0=lunes, 1=martes, ..., 6=domingo
    let diaSem = fechaPrimera.getDay();
    let offset;
    if (diaSem === 0) offset = 6; // domingo
    else offset = diaSem - 1; // lunes=0, martes=1, ..., sábado=5

    let html = '';
    let sesionIndex = inicioPagina;

    // Añadir celdas vacías iniciales según offset
    for (let i = 0; i < offset; i++) {
      html += '<div class="calendario-dia sesion-descanso"></div>';
    }

    // Añadir las sesiones reales
    while (sesionIndex < finPagina) {
      ...
      html += `<div class="calendario-dia ${sesion.color} ${realizada} ${bloqueado ? 'bloqueado' : ''}" data-index="${sesionIndex}">${contenidoHtml}</div>`;
    ...
    // Asignar eventos solo a las celdas con data-index
  ...
  // ==========================================================================
  // FUNCIONES AUXILIARES PARA EL MODAL EXPLICATIVO
  // ==========================================================================
...
console.log('✅ PlanGenerator v5.13 - Días fijos por semana, largo forzado');