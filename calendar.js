// ==================== calendar.js - VERSIÓN PRODUCCIÓN CON GAMIFICACIÓN ====================
// Versión: 2.47 - Añadido método getProximasSesiones para dashboard
// ============================================================================
...
const PlanGenerator = {
  ...
  async getProximasSesiones(limit = 3) {
    ...
    try {
      ...
      const semanaActual = AppState.planGeneradoActual?.semanaActual || 8;
      
      // Filtrar sesiones futuras
      const proximas = plan.sesiones.filter(s => {
        if (s.semana < semanaActual) return false;
        if (s.semana === semanaActual && s.diaSemana < hoy) return false;
        return s.tipo !== 'descanso';
      }).slice(0, limit);
          ...
          if (window.Gamification) {
            try {
              ...
              if (newLevel !== oldLevel) {
                ...
                if (document.getElementById('tab-inicio').classList.contains('active') && window.Dashboard) {
...
console.log('✅ PlanGenerator v2.47 - Con método getProximasSesiones para dashboard');