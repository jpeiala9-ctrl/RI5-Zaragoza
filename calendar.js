// ==================== calendar.js - VERSIÓN CORREGIDA MURO ====================
// Versión: 2.41 - AÑADIDA recarga forzada del muro al marcar sesión
// ============================================================================
...
try {
// ✅ CORREGIDO: conservar entradas de los últimos 7 días (antes solo se guardaba el día actual)
const hace7dias = new Date();
hace7dias.setDate(hace7dias.getDate() - 7);
hace7dias.setHours(0, 0, 0, 0);
const limiteTimestamp = firebaseServices.Timestamp.fromDate(hace7dias);
...
```
  ...
  if (antiguas.size === 0) return;

  // Firestore batch soporta máximo 500 operaciones
  const chunks = [];
  let chunk = firebaseServices.db.batch();
  let count = 0;
  antiguas.forEach(doc => {
    ...
    if (count === 499) {
      chunks.push(chunk);
      chunk = firebaseServices.db.batch();
      count = 0;
    }
  });
  ...
  console.log(`🗑️ Muro limpiado: se eliminaron ${antiguas.size} entradas con más de 7 días.`);
} catch (error) {
...
// ✅✅✅ FUNCIÓN CORREGIDA - CON RECARGA FORZADA DEL MURO ✅✅✅
...
```
  ...
  if (realizada) {
    ...
    if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
      ...
      // ✅ CORREGIDO: proteger contra userData nulo
      if (!userData || !userData.username) {
        console.warn('No hay datos de usuario disponibles, no se puede publicar en el muro');
        Utils.showToast('Sesión marcada, pero no se pudo publicar en el muro (datos de usuario no cargados)', 'warning');
        return;
      }
      ...
      try {
        ...
        console.log('✅ Entrada guardada en muro con ID:', globalRef.id);
        
        // ✅ NUEVO: Forzar recarga del muro para asegurar que aparece
        if (window.Wall) {
          setTimeout(() => {
            if (window.Wall.recargar) {
              Wall.recargar();
              console.log('🔄 Muro recargado manualmente');
            }
          }, 500);
        }
      } catch (err) {
...
// ✅ CORREGIDO: eliminada limpieza automática al arrancar
// Borraba entradas antes de que el muro las cargara.
// La limpieza solo ocurre al marcar una sesión como realizada.
...
console.log(‘✅ PlanGenerator v2.41 - Muro corregido con recarga forzada’);