// ==================== calendar.js - VERSIÓN CORREGIDA (v2.74) ====================
// Soluciona el error de calendario vacío al cargar planes guardados.
// Mantiene toasts únicos, GPS con botón "COMENZAR", marcadores sutiles.
// ============================================================================
...
const PlanGenerator = {
  ...
  mostrarCalendario(sesiones) {
    ...
    // Calcular semanas totales a partir de las sesiones (más fiable que duracionMeses)
  ...
  renderizarPagina(sesiones, semanasTotales) {
    ...
    for (let i = 0; i < sesiones.length; i++) {
      ...
      if (!sesion) continue;
      const semanaSesion = Math.floor((i) / 7); // índice de semana (0-based)
  ...
  // ========== OBTENER POSICIÓN PRECISA CON BOTÓN COMENZAR (versión corregida) ==========
  ...
  // ========== MARCAR SESIÓN REALIZADA (VERDE) CON DISTANCIA REAL ==========
    ...
    try {
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          const distanciaReal = (routeData && routeData.distanceMeters) ? routeData.distanceMeters / 1000 : metricas.distanciaTotal;
          ...
          if (window.Gamification) {
            try {
              await Gamification.updateAfterSession(AppState.currentUserId, sesion, metricas, distanciaReal);
        ...
        if (window.Gamification && sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          try {
            const metricas = this.calcularMetricasSesion(sesion);
            await Gamification.removeSession(AppState.currentUserId, sesion, metricas, diaIndex);
          } catch (e) { console.error(e); }
        }
      }
  ...
  // ========== MARCAR SESIÓN FORZADA (NARANJA) CON DISTANCIA PLANIFICADA ==========
  ...
  // ========== ABRIR DETALLE CON UN SOLO CHECKBOX (VERDE/NARANJA AUTOMÁTICO) ==========
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      if (!realizada) {
        ...
        startBtn.style.cssText = 'width: auto; background: var(--zone-2); border-color: var(--zone-2); color: var(--bg-primary);';
        ...
        startBtn.onclick = async () => {
          ...
          stopBtn.style.cssText = 'width: auto; background: var(--zone-5); border-color: var(--zone-5); color: var(--bg-primary);';
          stopBtn.innerHTML = '⏹️ Finalizar sesión';
  ...
  async limpiarMuroGlobal() {
    // Esta función se puede dejar vacía o implementar si es necesario
  },
...
console.log('✅ PlanGenerator v2.74 - Calendario corregido, GPS con botón COMENZAR, marcadores sutiles, toasts únicos');