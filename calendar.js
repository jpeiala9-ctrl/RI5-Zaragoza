// ============================================================================
// calendar.js - VERSIÓN ÉLITE 2.8
// Pre‑cálculo de métricas en sesión para feed independiente
// Progresión intra‑fase, distribución no uniforme, máximos dinámicos,
// series corregidas, fuerza complementaria.
// ============================================================================
...
const PlanGenerator = {
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    ...
    if (incluirRitmoObjetivo) {
      ...
      switch(tipo) {
        ...
        case 'series':
          ...
          let repeticionesBase = 6;
          if (sesionMatriz && sesionMatriz.distanciaSerie) {
            distanciaSerie = sesionMatriz.distanciaSerie;
            repeticionesBase = sesionMatriz.repeticiones || 6;
          } else {
            if (distancia === '2k' || distancia === '5k') {
              distanciaSerie = 300;
              repeticionesBase = 8;
            } else if (distancia === '10k') {
              distanciaSerie = 800;
              repeticionesBase = 5;
            } else if (distancia === 'medio' || distancia === 'maraton') {
              distanciaSerie = 1000;
              repeticionesBase = 4;
            }
          }
          ...
          const factorDescanso = distanciaSerie <= 400 ? 1.0 : 1.5;
          const tiempoTotalPorCiclo = tiempoRepeticion + (tiempoRepeticion * factorDescanso);
          
          let repeticiones = Math.floor(partePrincipal / tiempoTotalPorCiclo);
          if (repeticiones < 2) repeticiones = 2;
          
          const tiempoSeriesReal = repeticiones * tiempoRepeticion;
          const tiempoDescansoReal = repeticiones * (tiempoRepeticion * factorDescanso);
          const tiempoTotalReal = tiempoSeriesReal + tiempoDescansoReal;
          let tiempoExtra = partePrincipal - tiempoTotalReal;
          if (tiempoExtra < 0) {
            repeticiones--;
            if (repeticiones < 1) repeticiones = 1;
            const nuevoTiempoTotal = repeticiones * (tiempoRepeticion * (1 + factorDescanso));
            tiempoExtra = partePrincipal - nuevoTiempoTotal;
          }
          ...
          descripcionPrincipal = `${Math.round(tiempoSeriesReal)}' series (${repeticiones}x${distanciaSerie}m) Z4-Z5${descripcionExtra}`;
          pasoPrincipal = {
            icono: '💪',
            titulo: 'SERIES',
            accion: `${repeticiones} repeticiones de ${distanciaSerie}m a ritmo rápido (aprox. ${ritmoObjetivoStr} min/km) con recuperación de ${Math.round(tiempoRepeticion * factorDescanso)}' entre series. ${tiempoExtra > 5 ? `+ ${Math.round(tiempoExtra)}' de trote suave.` : ''}`,
    ...
    // Precalcular métricas para usar en feed
...
console.log('✅ PlanGenerator v16.8 - Pre‑cálculo de métricas para feed independiente');