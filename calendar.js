// ==================== calendar.js - VERSIÓN FINAL 2.24 ====================
// - SESIONES DESCRITAS ÚNICAMENTE CON ZONAS (sin ritmos absolutos)
// - El usuario conoce sus ritmos asociados a cada zona desde el cálculo inicial
// - Adaptación dinámica de series: pirámide, rotas, fartlek, cuestas
// - Métricas de distancia y TSS calculadas internamente con ritmos realistas
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== FUNCIÓN PARA OBTENER RITMO DESDE ZONAS (solo uso interno) ==========
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    ...
    if (incluirRitmoObjetivo) {
      ...
      descripcionPrincipal = `${parteZ2}' Z2 + ${bloqueRitmo}' a ritmo objetivo (Z4)`;
      pasoPrincipal = {
        icono: '🎯',
    ...
    // Para mostrar en el modal, obtenemos el ritmo (solo visual)
    let ritmoObjetivo = '';
    if (tipo === 'rodaje') ritmoObjetivo = this.obtenerRitmoParaZona('Z2');
    ...
    } else if (zonaPrincipal === 'Z4') {
      fcObjetivo = `${Math.round(fcRangos.Z4[0] * fcAjuste)}-${Math.round(fcRangos.Z4[1] * fcAjuste)} lpm`;
    } else if (zonaPrincipal === 'Z3') {
      fcObjetivo = `${Math.round(fcRangos.Z3[0] * fcAjuste)}-${Math.round(fcRangos.Z3[1] * fcAjuste)} lpm`;
    } else if (zonaPrincipal === 'Z2') {
      fcObjetivo = `${Math.round(fcRangos.Z2[0] * fcAjuste)}-${Math.round(fcRangos.Z2[1] * fcAjuste)} lpm`;
    } else {
      fcObjetivo = `${Math.round(fcRangos.Z1[0] * fcAjuste)}-${Math.round(fcRangos.Z1[1] * fcAjuste)} lpm`;
    }
    ...
    const detalle = {
      ...
      zona: zonaMostrada,
      duracion: duracion,
      ritmoObjetivo: ritmoObjetivo,   // solo para mostrar en modal
  ...
  crearSesionSimulacion(fase, semanaEnFase, nivel, datos, duracionExacta) {
    ...
    let accionBloques = '';
    if (numBloques >= 2) {
      descripcionBloques = `${numBloques}x${bloqueRitmo}' a ritmo objetivo (Z4) con ${recuperacion}' de trote suave entre bloques`;
      ...
      descripcionBloques = `${partePrincipal}' a ritmo objetivo (Z4)`;
    ...
    const detalle = {
      ...
      duracion: duracion,
      ritmoObjetivo: this.obtenerRitmoParaZona('Z4'), // solo para modal
  ...
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    // Ritmos solo para cálculo interno, no se muestran en texto
    ...
    switch(estructura) {
      case 'piramide': {
        ...
        const detallePiramide = {
          ...
          ritmoObjetivo: this.obtenerRitmoParaZona('Z5'), // solo para modal
          ...
          ritmoSerie: this.obtenerRitmoParaZona('Z5'),
          ritmoRecuperacion: this.obtenerRitmoParaZona('Z2'),
      ...
      case 'rotas': {
        ...
        const detalleRotas = {
          ...
          estructura: `${calentamiento}' calentamiento + ${accion} + ${enfriamiento}' enfriamiento`,
          sensacion: 'Muy intenso', zona, duracion,
          ritmoObjetivo: this.obtenerRitmoParaZona('Z5'), // solo modal
          fcObjetivo: `> ${Math.round(fcUmbral * 1.02)} lpm`,
      ...
      case 'fartlek': {
        ...
        accion = `Parte principal: ${Math.floor(totalSegundos / 60)}:${(totalSegundos % 60).toString().padStart(2,'0')} minutos. ${repeticiones} ciclos de ${tiempoFuerte}" fuerte + ${tiempoSuave}" suave${sobrante ? ` + ${sobrante}" suave final` : ''}. Realiza el fuerte a ritmo rápido – Z4-Z5 – y el suave a ritmo aeróbico – Z2. El cambio de ritmo debe ser continuo, sin detenerte.`;
        ...
        const detalleFartlek = {
          ...
          estructura: `${calentamiento}' calentamiento + ${accion} + ${enfriamiento}' enfriamiento`,
          sensacion: 'Muy intenso', zona, duracion,
          ritmoObjetivo: this.obtenerRitmoParaZona('Z5'),
          fcObjetivo: `> ${Math.round(fcUmbral * 1.02)} lpm`,
          ...
          segundosFuertes,
          segundosSuaves,
          ritmoFuerte: this.obtenerRitmoParaZona('Z5'),
          ritmoSuave: this.obtenerRitmoParaZona('Z2'),
          tiempoEnZona: Utils.formatR(tiempoEnZonaSeg / 60)
        };
      ...
      case 'cuestas': {
        ...
        const detalleCuestas = {
          ...
          ritmoRepeticion: this.obtenerRitmoParaZona('Z5'),
          tiempoEnZona: Utils.formatR(tiempoEnZonaSeg / 60)
        };
  ...
  // ========== CÁLCULO DE MÉTRICAS CON RITMOS REALES (INTERNO) ==========
  calcularMetricasSesion(sesion, factorIntensidad = 1.0) {
    ...
    // Para sesiones avanzadas que guardaron ritmos internos, los usamos
...
console.log('✅ PlanGenerator v2.24 - Descripciones sin ritmos absolutos, solo zonas.');