// ============================================================================
// calendar.js - VERSIÓN SIMPLIFICADA: RESPETA ESTRICTAMENTE LOS DÍAS ELEGIDOS
// ============================================================================
...
const PlanGenerator = {
  ...
  // ==========================================================================
  // GENERACIÓN PRINCIPAL (simplificada)
  // ==========================================================================
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // Días marcados (1=lunes, ..., 7=domingo)
      ...
      // Si el día largo no está en los días de entreno, lo añadimos
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // ----- ASIGNACIÓN SIMPLIFICADA: SOLO A LOS DÍAS ELEGIDOS -----
        // Creamos un array con los tipos disponibles (sin el largo)
        ...
        // Mezclamos los tipos para que no siempre sea el mismo orden (opcional, pero mantiene variedad)
        this.mezclarArray(tiposDisponibles);

        // Días que no son el largo (para asignar el resto de tipos)
        const otrosDias = diasEntreno.filter(d => d !== diaLargo).sort((a, b) => a - b);

        // Construimos el objeto tiposPorDia
        const tiposPorDia = {};
        // El día largo siempre es 'largo'
        tiposPorDia[diaLargo] = 'largo';

        // Asignamos los tipos disponibles a los otros días en orden
        for (let i = 0; i < otrosDias.length; i++) {
          const dia = otrosDias[i];
          if (i < tiposDisponibles.length) {
            tiposPorDia[dia] = tiposDisponibles[i];
          } else {
            // Si hay más días que tipos, rellenamos con rodaje
            tiposPorDia[dia] = 'rodaje';
          }
        }

        // Ahora generamos los 7 días de la semana
        const semana = [];
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          // Si el día NO está en diasEntreno -> descanso
          ...
          const tipo = tiposPorDia[diaSemana]; // Siempre debe existir porque el día está en diasEntreno
      ...
      // Guardar el plan (igual que antes)
  ...
  // ==========================================================================
  // FUNCIONES AUXILIARES (sin cambios, excepto mezclarArray)
  // ==========================================================================
  ...
  // ==========================================================================
  // CREACIÓN DE SESIONES (sin cambios, excepto la inclusión de mezclarArray)
  // ==========================================================================
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad) {
    // (Esta función se mantiene igual que en tu código original, con toda la lógica de entrenamientos)
    // Por brevedad, la incluyo completa pero sin cambios respecto a la anterior.
  ...
  // ==========================================================================
  // GUARDADO Y VISUALIZACIÓN (con la mejora de mostrar el día de la semana)
  // ==========================================================================
  ...
  renderizarPagina(sesiones) {
    ...
    let diaSem = fechaPrimera.getDay(); // 0 domingo, 1 lunes...
    let offset = diaSem === 0 ? 6 : diaSem - 1; // para que lunes sea 0
    ...
    while (sesionIndex < finPagina) {
      ...
      const diaSemanaAbr = diasSemanaAbr[sesion.diaSemana - 1]; // sesion.diaSemana va de 1 a 7
  ...
  // ==========================================================================
  // MÉTRICAS Y EXPLICACIONES (sin cambios)
  // ==========================================================================
...
console.log('✅ PlanGenerator v8.0 - SIMPLIFICADO: solo entreno en días marcados');