// ==================== calendar.js - VERSIÓN CORREGIDA (CALENDARIO SIEMPRE VISIBLE) ====================
// Versión: 5.4 - Renderizado robusto basado en diaGlobal y reinicio de paginación
// ============================================================================
...
const PlanGenerator = {
  ...
  _smoothPoints(rawPoints) {
    ...
    for (let i = 0; i < rawPoints.length; i++) {
      ...
      const distToPrev = this._haversineDistance(prev.lat, prev.lng, curr.lat, curr.lng);
      const timeDiffPrev = (curr.timestamp - prev.timestamp) / 1000;
      const speedPrev = distToPrev / timeDiffPrev;
      const distToNext = this._haversineDistance(curr.lat, curr.lng, next.lat, next.lng);
      const timeDiffNext = (next.timestamp - curr.timestamp) / 1000;
      const speedNext = distToNext / timeDiffNext;
      if (speedPrev > 12 || speedPrev < 0.3 || speedNext > 12 || speedNext < 0.3) continue;
      filtered.push(curr);
    }
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      const avgLng = (prev.lng + curr.lng + next.lng) / 3;
      smoothed.push({ lat: avgLat, lng: avgLng, timestamp: curr.timestamp });
    }
    return smoothed;
  },
  ...
  obtenerObjetivoTexto(tipo, fase) {
    const objetivos = {
      rodaje: { BASE: 'Construir base aeróbica', CONSTRUCCION: 'Mantener volumen con calidad', ESPECIFICA: 'Preparar para ritmos de competición', PICO: 'Mantener forma sin fatiga', TAPER: 'Recuperación activa' },
      tempo: { BASE: 'Introducir ritmos sostenidos', CONSTRUCCION: 'Mejorar umbral de lactato', ESPECIFICA: 'Simular ritmos de competición', PICO: 'Ajustar ritmos objetivo', TAPER: 'Mantener agilidad' },
      series: { BASE: 'Desarrollar velocidad básica', CONSTRUCCION: 'Aumentar tolerancia al lactato', ESPECIFICA: 'Estimular VO2max', PICO: 'Afinar velocidad específica', TAPER: 'Mantener explosividad' },
      largo: { BASE: 'Aumentar capacidad aeróbica', CONSTRUCCION: 'Mejorar resistencia específica', ESPECIFICA: 'Simular condiciones de carrera', PICO: 'Mantener confianza', TAPER: 'Cargar glucógeno' }
    };
  ...
  obtenerPorque(tipo, fase) {
    const porqueMap = {
      rodaje: { BASE: 'Construir la base aeróbica, fundamental para soportar volúmenes mayores.', CONSTRUCCION: 'Mantener el volumen mientras se introduce calidad.', ESPECIFICA: 'Preparar el cuerpo para los ritmos de competición.', PICO: 'Mantener la forma sin generar fatiga adicional.', TAPER: 'Activar la circulación y mantener la frescura muscular.' },
      tempo: { BASE: 'Introducir el cuerpo a ritmos sostenidos por encima del aeróbico.', CONSTRUCCION: 'Elevar el umbral de lactato para poder mantener ritmos más rápidos.', ESPECIFICA: 'Simular los ritmos de carrera y acostumbrar al cuerpo a la fatiga.', PICO: 'Ajustar el ritmo objetivo y ganar confianza.', TAPER: 'Mantener la agilidad sin acumular ácido láctico.' },
      series: { BASE: 'Desarrollar velocidad básica y eficiencia neuromuscular.', CONSTRUCCION: 'Aumentar la tolerancia al lactato y la capacidad de eliminar desechos.', ESPECIFICA: 'Estimular el VO2máx y mejorar la potencia aeróbica.', PICO: 'Afinar la velocidad específica para la distancia objetivo.', TAPER: 'Mantener la explosividad sin fatiga.' },
      largo: { BASE: 'Aumentar la capacidad aeróbica y la resistencia general.', CONSTRUCCION: 'Mejorar la resistencia específica para la distancia objetivo.', ESPECIFICA: 'Simular las condiciones de carrera (ritmo, nutrición, hidratación).', PICO: 'Mantener la confianza y la resistencia sin llegar al agotamiento.', TAPER: 'Cargar los depósitos de glucógeno y mantener la motivación.' }
    };
  ...
  async crearSesionBasica(tipo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null, esRecuperacion = false) {
    ...
    if (esRecuperacion && tipo === 'rodaje') duracion = 25;
  ...
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    switch(estructura) {
      ...
      default:
        return this.crearSesionBasica('series', fase, semanaEnFase, nivel, dados, factorVolumen, factorIntensidad, duracionExacta);
    }
  ...
  async guardarPlanEnFirebase(planId, planData) {
    ...
    try {
      const dataToSave = { ...planData, sesionesRealizadas: {}, sesionesForzadas: {} };
  ...
  mostrarCalendario(sesiones) {
    ...
    if (!grid) {
      console.error("mostrarCalendario: elemento calendarioGrid no encontrado");
      return;
    }
    if (!sesiones || sesiones.length === 0) {
      console.warn("mostrarCalendario: sesiones vacías");
      grid.innerHTML = '<div style="text-align:center; padding:20px;">No hay sesiones para mostrar</div>';
      return;
    }

    // Asegurar que trimestreActual es 0 al mostrar un nuevo plan
    if (typeof AppState.trimestreActual !== 'number' || isNaN(AppState.trimestreActual)) {
      AppState.trimestreActual = 0;
    }

    const semanasTotales = Math.ceil(sesiones.length / 7);
    console.log(`📅 Mostrando calendario con ${semanasTotales} semanas y ${sesiones.length} sesiones`);
  ...
  renderizarPagina(sesiones, semanasTotales) {
    ...
    if (semanasTotales <= 0) {
      grid.innerHTML = '<div style="text-align:center; padding:20px;">No hay semanas para mostrar</div>';
      return;
    }
    
    // Validar y resetear trimestreActual si es necesario
    if (typeof AppState.trimestreActual !== 'number' || isNaN(AppState.trimestreActual)) {
      AppState.trimestreActual = 0;
    }
    ...
    if (semanasAMostrar <= 0) {
      grid.innerHTML = '<div style="text-align:center; padding:20px;">No hay semanas en esta página</div>';
      return;
    }
    ...
    // Recorrer todas las sesiones y colocarlas en la celda correcta
    for (let i = 0; i < sesiones.length; i++) {
      const sesion = sesiones[i];
      if (!sesion) continue;
      
      // Calcular semana absoluta (basada en diaGlobal si existe, si no, usar índice)
      let semanaAbsoluta;
      if (sesion.diaGlobal) {
        semanaAbsoluta = Math.floor((sesion.diaGlobal - 1) / 7);
      } else {
        semanaAbsoluta = Math.floor(i / 7);
      }
      
      if (semanaAbsoluta >= inicioPagina && semanaAbsoluta < finPagina) {
        const diaSemana = sesion.diaSemana || ((sesion.diaGlobal - 1) % 7 + 1);
        if (diaSemana >= 1 && diaSemana <= 7) {
          const pos = (semanaAbsoluta - inicioPagina) * 7 + (diaSemana - 1);
    ...
    for (let pos = 0; pos < celdas.length; pos++) {
      ...
      const forzada = AppState.sesionesForzadas?.[sesion.diaGlobal];
      let claseAdicional = '';
      if (realizada) {
        claseAdicional = forzada ? 'realizado-forzado' : 'realizado';
      }
      ...
      html += `<div class="calendario-dia ${sesion.color} ${claseAdicional}" data-index="${sesion.diaGlobal}"${faseIndicator}>${contenidoHtml}</div>`;
    }
    
    grid.innerHTML = html;
    console.log(`✅ Calendario renderizado: ${celdas.length} celdas, ${grid.children.length} elementos visibles`);
    ...
    document.querySelectorAll('.calendario-dia[data-index]').forEach(dia => {
      dia.addEventListener('click', (e) => {
        ...
        const sesionEncontrada = sesiones.find(s => s.diaGlobal == diaIndex);
        if (sesionEncontrada) this.abrirDetalleSesion(sesionEncontrada, parseInt(diaIndex));
  ...
  async guardarRutaGPS(diaIndex, routeData) {
    ...
    try {
      const planRef = firebaseServices.db
        ...
        .collection('planes')
        .doc(AppState.planActualId);
      await planRef.update({ [`gpsRoutes.${diaIndex}`]: routeData });
      const planDoc = await planRef.get();
      const wallEntryId = planDoc.data()?.wallEntryId?.[diaIndex];
      if (wallEntryId) {
        await firebaseServices.db.collection('globalFeed').doc(wallEntryId).update({ gpsRoute: routeData });
      }
      console.log('✅ Ruta GPS guardada correctamente');
    } catch (error) {
      console.error('Error guardando ruta GPS:', error);
    }
  },

  _obtenerPosicionPrecisa() {
    return new Promise((resolve) => {
      const overlay = document.createElement('div');
      overlay.id = 'gpsPrecisionOverlay';
      ...
      message.innerText = '🔍 Buscando señal GPS...';
      
      const precisionSpan = document.createElement('div');
      precisionSpan.style.fontSize = '14px';
      precisionSpan.style.marginTop = '10px';
      precisionSpan.style.color = '#ccc';
      precisionSpan.innerText = 'Precisión: --- m';
      
      const buttonContainer = document.createElement('div');
      buttonContainer.style.cssText = 'display: flex; gap: 16px; justify-content: center; margin-top: 20px;';
      ...
      startBtn.style.cssText = `
        background: linear-gradient(135deg, var(--zone-2), #6b8c6b);
        border: none;
        color: white;
        font-weight: bold;
        padding: 8px 24px;
      ...
      content.appendChild(precisionSpan);
      ...
      const finalizar = (cancelled = false) => {
        if (finished) return;
        finished = true;
        ...
        overlay.remove();
        if (cancelled) {
          resolve({ position: null, cancelled: true });
        } else {
          resolve({ position: bestPosition, cancelled: false });
        }
      };
      ...
      watchId = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          precisionSpan.innerText = `Precisión: ${Math.round(accuracy)} m`;
          if (accuracy <= 20 && startBtn.disabled) {
            startBtn.disabled = false;
            startBtn.style.opacity = '1';
            message.innerText = '✅ GPS listo. Presiona COMENZAR.';
          }
        },
        (err) => {
          console.warn('Error GPS:', err);
          precisionSpan.innerText = `⚠️ Error: ${err.message}`;
          if (bestPosition && startBtn.disabled) {
            startBtn.disabled = false;
            startBtn.style.opacity = '1';
            message.innerText = '⚠️ Precisión mejorable, puedes comenzar igualmente.';
          }
        },
      ...
      timeoutId = setTimeout(() => {
        if (startBtn.disabled && bestPosition) {
          startBtn.disabled = false;
          startBtn.style.opacity = '1';
          message.innerText = '⏱️ Tiempo agotado, puedes comenzar con la posición actual.';
        }
      }, 5000);
      
      startBtn.onclick = () => finalizar(false);
      cancelBtn.onclick = () => finalizar(true);
    });
  },

  async marcarSesionRealizada(diaIndex, realizada, routeData = null) {
    if (!AppState.currentUserId || !AppState.planActualId) return;
    try {
      const planRef = firebaseServices.db
        .collection('users')
      ...
      if (!realizada) {
        await planRef.update({ [`sesionesForzadas.${diaIndex}`]: firebaseServices.FieldValue.delete() });
        if (AppState.sesionesForzadas) delete AppState.sesionesForzadas[diaIndex];
      }
      ...
      if (celda) {
        if (realizada) {
          ...
          celda.classList.remove('realizado-forzado');
        } else {
          celda.classList.remove('realizado', 'realizado-forzado');
          const planDoc = await planRef.get();
          const planData = planDoc.data();
          const sesion = planData.sesiones[diaIndex - 1];
          if (sesion && sesion.color) {
            celda.classList.remove('sesion-rodaje', 'sesion-tempo', 'sesion-series', 'sesion-largo', 'sesion-strength', 'sesion-descanso');
            celda.classList.add(sesion.color);
          }
        }
      }
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          let distanciaUsada = metricas.distanciaTotal;
          if (routeData && routeData.porcentaje !== undefined && routeData.porcentaje <= 10) {
            distanciaUsada = routeData.distanceMeters / 1000;
          }
          const tss = isFinite(metricas.tssTotal) ? metricas.tssTotal : 0;
          
          let gpsRoute = null;
          const planDocGps = await planRef.get();
          if (planDocGps.exists && planDocGps.data().gpsRoutes && planDocGps.data().gpsRoutes[diaIndex]) {
            gpsRoute = planDocGps.data().gpsRoutes[diaIndex];
          }
          ...
          const entry = {
            ...
            gpsRoute: routeData || gpsRoute,
            forcedComplete: false,
            distanciaUsadaOriginal: distanciaUsada
          };
          try {
          ...
          if (window.Gamification) {
            try {
              await Gamification.updateAfterSession(AppState.currentUserId, sesion, metricas, distanciaUsada);
        ...
        let distanciaARestar = null;
        if (wallEntryId) {
          try {
            const muroDoc = await firebaseServices.db.collection('globalFeed').doc(wallEntryId).get();
            if (muroDoc.exists) {
              distanciaARestar = muroDoc.data().distancia;
              await muroDoc.ref.delete();
            }
        ...
        if (window.Gamification && sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          try {
            const metricas = this.calcularMetricasSesion(sesion);
            const distancia = distanciaARestar !== null ? distanciaARestar : metricas.distanciaTotal;
            await Gamification.removeSession(AppState.currentUserId, sesion, metricas, diaIndex, distancia);
          } catch (e) { console.error(e); }
        }
      }
      ...
      console.error('Error marcando/desmarcando sesión:', error);
      Utils.showToast('Error al actualizar la sesión', 'error');
    }
  },

  async marcarSesionForzada(diaIndex, realizada, routeData = null, skipToast = false) {
    if (!AppState.currentUserId || !AppState.planActualId) return;
    try {
      const planRef = firebaseServices.db
        .collection('users')
      ...
      await planRef.update({
        [`sesionesRealizadas.${diaIndex}`]: true,
        [`sesionesForzadas.${diaIndex}`]: true
      });
      
      const verifyDoc = await planRef.get();
      const forzadas = verifyDoc.data()?.sesionesForzadas || {};
      if (!forzadas[diaIndex]) {
        console.warn(`⚠️ No se guardó sesionesForzadas para ${diaIndex}, reintentando...`);
        await planRef.update({ [`sesionesForzadas.${diaIndex}`]: true });
      } else {
        console.log(`✅ sesionesForzadas[${diaIndex}] guardado correctamente`);
      }
      
      if (!AppState.sesionesRealizadas) AppState.sesionesRealizadas = {};
      if (!AppState.sesionesForzadas) AppState.sesionesForzadas = {};
      AppState.sesionesRealizadas[diaIndex] = true;
      AppState.sesionesForzadas[diaIndex] = true;
      
      const celda = document.querySelector(`.calendario-dia[data-index="${diaIndex}"]`);
      if (celda) {
        celda.classList.remove('realizado');
        celda.classList.add('realizado-forzado');
      }
      ...
      if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
        ...
        const metricas = this.calcularMetricasSesion(sesion);
        const userData = AppState.currentUserData;
        const distanciaPlanificada = metricas.distanciaTotal;
        const tss = isFinite(metricas.tssTotal) ? metricas.tssTotal : 0;
        ...
        const entry = {
          ...
          trainingType: sesion.tipo,
          duration: sesion.duracion || 0,
          distancia: distanciaPlanificada,
          tss: tss,
          timestamp: firebaseServices.Timestamp.now(),
          planId: AppState.planActualId,
          sesionIndex: diaIndex,
          ...
          zone: sesion.detalle.zona || '',
          trainingName: sesion.detalle.nombre || '',
          gpsRoute: routeData || null,
          forcedComplete: true,
          distanciaUsadaOriginal: distanciaPlanificada
        };
        const globalRef = await firebaseServices.db.collection('globalFeed').add(entry);
        await planRef.update({ [`wallEntryId.${diaIndex}`]: globalRef.id });
        ...
        if (window.Gamification) {
          try {
            await Gamification.updateAfterSession(AppState.currentUserId, sesion, metricas, distanciaPlanificada);
          } catch (e) { console.error(e); }
        }
      }
      ...
      if (!skipToast) Utils.showToast('🟠 Sesión marcada manualmente (naranja)', 'success');
    } catch (error) {
      console.error('Error marcando sesión forzada:', error);
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    const oldGpsControls = document.querySelector('.gps-controls');
    if (oldGpsControls) oldGpsControls.remove();
    const oldMapContainer = document.getElementById('gpsMapContainer');
    if (oldMapContainer) oldMapContainer.remove();
    const oldStatsDiv = document.querySelector('.gps-stats');
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      const zonasHTML = `
        <div class="sesion-zonas">
          ...
          <div class="zona-item"><span>📊 Zona</span><strong>${Utils.escapeHTML(zonaMostrada)}</strong></div>
        </div>
      `;
      ...
      const realizada = AppState.sesionesRealizadas?.[diaIndex] || false;
      ...
      checkbox.onchange = async (e) => {
        ...
        if (e.target.checked) {
          if (lastRouteData && lastRouteData.porcentaje !== undefined && lastRouteData.porcentaje <= 10) {
            await this.marcarSesionRealizada(diaIndex, true, lastRouteData);
          } else {
            await this.marcarSesionForzada(diaIndex, true, lastRouteData || null, false);
            await new Promise(resolve => setTimeout(resolve, 500));
          }
          cerrarModalSesion();
        } else {
          await this.marcarSesionRealizada(diaIndex, false);
        }
      };
      ...
      feedbackButtons.forEach(btn => {
        ...
        if (AppState.feedbackSesiones?.[diaIndex] == btn.getAttribute('data-value')) {
          btn.classList.add('active');
        }
      });
      
      // === NUEVA INTERFAZ DE GRABACIÓN GPS (PANTALLA NEGRA) ===
      if (!realizada) {
        console.log('🟢 Mostrando interfaz de grabación GPS (pantalla negra)');
        
        const recordOverlay = document.createElement('div');
        recordOverlay.id = 'gpsRecordingOverlay';
        ...
        statusMsg.style.cssText = 'font-size: 18px; margin-bottom: 20px; letter-spacing: 2px;';
        statusMsg.innerText = '🏃 SESIÓN EN CURSO';
        
        const timerSpan = document.createElement('div');
        timerSpan.style.cssText = 'font-size: 48px; font-weight: bold; margin: 20px 0; font-family: monospace;';
        timerSpan.innerText = '00:00';
        
        const distanceSpan = document.createElement('div');
        distanceSpan.style.cssText = 'font-size: 24px; margin: 10px 0;';
        distanceSpan.innerText = '📏 0.00 km';
        
        const paceSpan = document.createElement('div');
        paceSpan.style.cssText = 'font-size: 20px; margin: 10px 0; color: #ccc;';
        paceSpan.innerText = '⚡ --:-- min/km';
        
        const stopButton = document.createElement('button');
        stopButton.innerText = 'FINALIZAR SESIÓN';
        stopButton.style.cssText = `
          background: linear-gradient(135deg, #a55a5a, #8a3a3a);
          border: none;
          color: white;
          font-weight: bold;
          padding: 14px 28px;
        ...
        `;
        stopButton.onmouseover = () => stopButton.style.transform = 'scale(1.02)';
        stopButton.onmouseout = () => stopButton.style.transform = 'scale(1)';
        
        infoContainer.appendChild(statusMsg);
        infoContainer.appendChild(timerSpan);
        infoContainer.appendChild(distanceSpan);
        infoContainer.appendChild(paceSpan);
        infoContainer.appendChild(stopButton);
        recordOverlay.appendChild(infoContainer);
        ...
        const updateUI = (distanceKm, durationSec, currentPace) => {
          const mins = Math.floor(durationSec / 60);
          const secs = Math.floor(durationSec % 60);
          timerSpan.innerText = `${mins.toString().padStart(2,'0')}:${secs.toString().padStart(2,'0')}`;
          distanceSpan.innerText = `📏 ${distanceKm.toFixed(2)} km`;
          if (currentPace && isFinite(currentPace)) {
            const paceMin = Math.floor(currentPace);
            const paceSec = Math.round((currentPace - paceMin) * 60);
            paceSpan.innerText = `⚡ ${paceMin}:${paceSec.toString().padStart(2,'0')} min/km`;
          } else {
            paceSpan.innerText = `⚡ --:-- min/km`;
          }
        };
        ...
        const startRecording = async () => {
          try {
            const { position, cancelled } = await this._obtenerPosicionPrecisa();
            if (cancelled) {
            ...
            watchId = navigator.geolocation.watchPosition(
              (pos) => {
                ...
                for (let i = 1; i < points.length; i++) {
                  totalDistance += this._haversineDistance(points[i-1].lat, points[i-1].lng, points[i].lat, points[i].lng);
                }
                const durationSec = (Date.now() - startTime) / 1000;
                const avgSpeedKmh = (totalDistance / 1000) / (durationSec / 3600);
                const paceMinPerKm = avgSpeedKmh > 0 ? 60 / avgSpeedKmh : 0;
                updateUI(totalDistance / 1000, durationSec, paceMinPerKm);
              }.bind(this),
            ...
            timerInterval = setInterval(() => {
              if (startTime) {
                ...
                const avgSpeedKmh = (totalDistance / 1000) / (durationSec / 3600);
                const pace = avgSpeedKmh > 0 ? 60 / avgSpeedKmh : 0;
                updateUI(totalDistance / 1000, durationSec, pace);
              }
            }, 1000);
          } catch (error) {
            console.error('Error iniciando grabación:', error);
            Utils.showToast('No se pudo iniciar la grabación', 'error');
            recordOverlay.remove();
          }
        };
        ...
        stopButton.onclick = async () => {
          ...
          const smoothed = this._smoothPoints(points);
          console.log(`Puntos originales: ${points.length}, suavizados: ${smoothed.length}`);
          let distanceMeters = 0;
          for (let i = 1; i < smoothed.length; i++) {
            distanceMeters += this._haversineDistance(smoothed[i-1].lat, smoothed[i-1].lng, smoothed[i].lat, smoothed[i].lng);
          }
          const distanciaReal = distanceMeters / 1000;
          const distanciaEstimada = metricas.distanciaTotal;
          const porcentaje = Math.abs((distanciaReal - distanciaEstimada) / distanciaEstimada) * 100;
          lastRouteData = {
            points: smoothed.map(p => ({ lat: p.lat, lng: p.lng })),
            distanceMeters: distanceMeters,
            durationSeconds: durationSeconds,
            porcentaje: porcentaje
          };
          await this.guardarRutaGPS(diaIndex, lastRouteData);
          recordOverlay.remove();
          if (porcentaje <= 10) {
            autoMarked = true;
            await this.marcarSesionRealizada(diaIndex, true, lastRouteData);
            ...
            Utils.showToast(`✅ Sesión marcada (${porcentaje.toFixed(1)}% diferencia)`, 'success');
            cerrarModalSesion();
          } else {
            Utils.showToast(`⚠️ Diferencia del ${porcentaje.toFixed(1)}%`, 'warning');
            Utils.showToast('Usa el botón "Marcar como realizada" para finalizar la sesión (se marcará en naranja).', 'info', 4000);
          }
        };
      ...
      if (realizada && sesion.gpsRoute) {
        const mapContainer = document.createElement('div');
        mapContainer.id = `gpsMap_${diaIndex}`;
        mapContainer.className = 'route-map';
        mapContainer.style.marginTop = '16px';
        mapContainer.style.height = '300px';
        descripcion.parentNode.insertBefore(mapContainer, descripcion.nextSibling);
        this._displayRoute(mapContainer.id, sesion.gpsRoute.points);
      }
  ...
  async _displayRoute(containerId, points) {
    if (!points || points.length < 2) return;
    await this._loadLeaflet();
    const container = document.getElementById(containerId);
    if (!container) return;
    const latlngs = points.map(p => [p.lat, p.lng]);
    const map = L.map(containerId, { attributionControl: false }).fitBounds(latlngs, { padding: [30, 30] });
    L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', {
      attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a> &copy; <a href="https://carto.com/attributions">CARTO</a>',
      subdomains: 'abcd',
      maxZoom: 19
    }).addTo(map);
    L.polyline(latlngs, { color: '#C9A96E', weight: 4 }).addTo(map);
  ...
  async guardarFeedback(diaIndex, valor) {
    ...
    try {
      await firebaseServices.db
        ...
        .collection('planes')
        .doc(AppState.planActualId)
        .update({ [`feedback.${diaIndex}`]: valor });
    } catch (error) {
  ...
  mostrarFeedbackExistente(diaIndex) {
    const feedback = AppState.feedbackSesiones?.[diaIndex];
    if (feedback) {
      const btns = document.querySelectorAll('.feedback-btn');
      btns.forEach(btn => {
        if (btn.getAttribute('data-value') == feedback) btn.classList.add('active');
      });
    }
  },

  obtenerObjetivoDescanso(sesion) {
    if (sesion.fase === 'TAPER') return 'Recuperación activa y descarga muscular.';
    if (sesion.fase === 'PICO') return 'Mantener frescura antes de competición.';
    return 'Recuperación completa de entrenamientos previos.';
  },

  obtenerPorqueDescanso(sesion) {
    return 'El descanso es parte fundamental del entrenamiento. Es cuando el cuerpo repara tejidos y mejora su rendimiento.';
  },
  ...
  async mostrarUltimoPlanGuardado() {
    ...
    Utils.showLoading();
    try {
      const ultimoPlanId = (await firebaseServices.db.collection('users').doc(AppState.currentUserId).get()).data()?.ultimoPlanId;
      if (!ultimoPlanId) { Utils.hideLoading(); Utils.showToast('No hay plan guardado', 'info'); return; }
      const planCompleto = await Storage.getPlanCompleto(AppState.currentUserId, ultimoPlanId);
      if (!planCompleto) { Utils.hideLoading(); Utils.showToast('El plan ya no existe', 'error'); return; }
      if (window.UI && UI.cargarPlanDesdeHistorial) await UI.cargarPlanDesdeHistorial(ultimoPlanId);
      else Utils.showToast('Error al cargar el plan', 'error');
    } catch (error) { console.error('Error cargando último plan:', error); Utils.showToast('Error al cargar', 'error'); }
    finally { Utils.hideLoading(); }
  },

  async borrarPlanGuardado() {
    if (!AppState.currentUserId) return;
    const confirmado = await Utils.confirm('Eliminar plan', '¿Eliminar el plan guardado actual?');
    ...
    try {
      ...
      if (ultimoPlanId) {
        await firebaseServices.db.collection('users').doc(AppState.currentUserId).collection('planes').doc(ultimoPlanId).delete();
        await userRef.update({ ultimoPlanId: null });
      }
      ...
      if (window.UI && UI.cargarHistorialPlanes) await UI.cargarHistorialPlanes();
    } catch (error) { console.error('Error borrando plan:', error); Utils.showToast('Error al eliminar', 'error'); }
    finally { Utils.hideLoading(); }
  },
  ...
  toggleCuestionario() {
    const cuestionario = document.getElementById("cuestionarioEntreno");
    if (cuestionario) {
      cuestionario.style.display = cuestionario.style.display === "none" ? "block" : "none";
    }
  },

  async analizarFeedbackAdaptativo() {
    if (!AppState.planActualId) return { volumen: 1.0, intensidad: 1.0 };
    const feedback = AppState.feedbackSesiones || {};
    const valores = Object.values(feedback);
    if (valores.length === 0) return { volumen: 1.0, intensidad: 1.0 };
    const media = valores.reduce((a, b) => a + parseInt(b), 0) / valores.length;
    if (media >= 3.5) return { volumen: 1.05, intensidad: 1.02 };
    if (media <= 2) return { volumen: 0.95, intensidad: 0.98 };
  ...
  validarOpcionesPlan() {
    ...
    if (msgDiv) {
      if ((distancia === "medio" || distancia === "maraton") && duracion === 1) {
        msgDiv.style.display = "block";
        msgDiv.innerHTML = "⚠️ No se puede generar un plan de 1 mes para media o maratón. Selecciona al menos 3 meses.";
      } else if ((distancia === "medio" || distancia === "maraton") && duracion === 3 && experiencia === "no") {
        msgDiv.style.display = "block";
        msgDiv.innerHTML = "⚠️ Para media o maratón en 3 meses es recomendable tener experiencia previa. Selecciona 6 meses o cambia a \"CON EXPERIENCIA\".";
      } else {
...
console.log('✅ PlanGenerator v2.81 - Calendario visible, persistencia naranja y pantalla negra GPS');