// ==================== calendar.js - VERSIÓN COMPLETA CON COMPARTIR SESIONES ====================
// Versión: 2.51 - Añadida funcionalidad de compartir sesiones con amigos
// ============================================================================
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    if (!AppState.zonasCalculadas) {
      Utils.showToast("⚠️ Primero calcula tus zonas de entrenamiento.", 'error');
      return;
    }
    if (!AppState.isPremium) {
      Utils.showToast("⭐ Esta función es exclusiva para usuarios Premium.", 'error');
      return;
    }
    try {
      Utils.showLoading();
      ...
      if (nivel === 'avanzado' && diasEntreno.length < 5) {
        Utils.showToast("⚠️ El nivel avanzado requiere al menos 5 días de entrenamiento.", 'error');
        Utils.hideLoading();
        return;
      }
      if (diasEntreno.length === 0) {
        Utils.showToast("⚠️ Selecciona al menos un día de entrenamiento.", 'error');
        Utils.hideLoading();
        return;
      }
      const diasMinimos = this.obtenerDiasMinimos(distancia);
      if (diasEntreno.length < diasMinimos) {
        Utils.showToast(`⚠️ Para ${distancia === 'maraton' ? 'MARATÓN' : 'MEDIA'} necesitas al menos ${diasMinimos} días de entrenamiento.`, 'error');
        Utils.hideLoading();
        return;
      }
      if (meses === 1 && !['2k', '5k', '10k'].includes(distancia)) {
        Utils.showToast("⚠️ El plan de 1 mes solo está disponible para 2 km, 5 km y 10 km.", 'error');
        Utils.hideLoading();
        return;
      }
      if (['medio', 'maraton'].includes(distancia) && meses === 3 && experiencia === 'no') {
        Utils.showToast("⚠️ Para media maratón o maratón en 3 meses necesitas experiencia previa.", 'error');
        Utils.hideLoading();
        return;
      }
      ...
      Utils.showToast('✅ Plan generado correctamente.', 'success');
  ...
  // ==================== NUEVAS FUNCIONES PARA COMPARTIR SESIONES ====================

  /**
   * Muestra el modal de selección de amigo para compartir una sesión
   * @param {object} sesion - Objeto de la sesión
   * @param {number} diaIndex - Índice del día en el plan
   */
  async compartirSesion(sesion, diaIndex) {
    if (!AppState.currentUserId) {
      Utils.showToast('⚠️ Inicia sesión para compartir.', 'warning');
      return;
    }
    
    // Verificar que la sesión tenga datos válidos
    if (!sesion || !sesion.detalle) {
      Utils.showToast('⚠️ No se puede compartir esta sesión.', 'error');
      return;
    }
    
    // Obtener lista de amigos
    const amigos = await Friends.getAmigosLista();
    if (!amigos || amigos.length === 0) {
      Utils.showToast('ℹ️ No tienes amigos para compartir. Añade amigos primero.', 'info');
      return;
    }
    
    // Mostrar modal de selección
    const modal = document.getElementById('modalCompartirAmigo');
    const overlay = document.getElementById('modalCompartirAmigoOverlay');
    const listContainer = document.getElementById('compartirAmigosList');
    
    if (!modal || !overlay || !listContainer) {
      Utils.showToast('❌ Error al abrir el modal de compartir.', 'error');
      return;
    }
    
    // Construir lista de amigos
    let html = '';
    const currentUsername = AppState.currentUserData?.username || 'Usuario';
    const sessionData = {
      tipo: sesion.tipo,
    ...
    for (const amigo of amigos) {
      ...
      const photoHTML = amigo.photoURL 
        ? `<img src="${Utils.escapeHTML(amigo.photoURL)}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;">`
      ...
      html += `
        <div class="compartir-amigo-item" data-uid="${amigo.uid}" data-username="${Utils.escapeHTML(amigo.username)}" 
             style="display:flex;align-items:center;gap:12px;padding:12px;border-bottom:1px solid var(--border-color);cursor:pointer;transition:background 0.2s;">
          ${photoHTML}
          <div style="flex:1;">
            <div style="font-weight:500;color:var(--text-primary);">${Utils.escapeHTML(usernameFormatted)}</div>
            <div style="font-size:12px;color:var(--text-secondary);">@${Utils.escapeHTML(amigo.username)}</div>
          </div>
          <button class="btn-amigo enviar-btn" data-uid="${amigo.uid}" data-username="${Utils.escapeHTML(amigo.username)}" 
                  style="padding:6px 16px;font-size:12px;">Enviar</button>
        </div>
      `;
    }
    ...
    listContainer.querySelectorAll('.enviar-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        // Cerrar modal
        PlanGenerator.cerrarModalCompartir();
        
        // Enviar sesión (envío múltiple)
        Utils.showLoading();
        try {
          // Recoger todos los UIDs seleccionados (por ahora solo uno, pero podemos extender a múltiple)
          const selectedUids = [uid];
          const success = await Storage.enviarSesionCompartida(selectedUids, sessionData, currentUsername);
          if (success) {
            Utils.showToast(`✅ Sesión compartida con @${username}`, 'success');
          } else {
            Utils.showToast('❌ No se pudo compartir la sesión. Reinténtalo.', 'error');
          }
        } catch (error) {
          console.error('Error compartiendo sesión:', error);
          Utils.showToast('❌ Error al compartir la sesión.', 'error');
        } finally {
          Utils.hideLoading();
        }
      });
    ...
    overlay.onclick = () => PlanGenerator.cerrarModalCompartir();
    document.getElementById('cerrarCompartirModal')?.addEventListener('click', () => PlanGenerator.cerrarModalCompartir());
  },
  ...
  // ==================== FUNCIONES PARA SESIONES COMPARTIDAS EN EL CALENDARIO ====================

  async obtenerSesionesCompartidasParaCalendario(uid, planId) {
    if (!uid || !planId) return [];
    try {
      const shared = await Storage.obtenerSharedSessions(uid, planId);
      return shared.filter(s => !s.realizado); // Solo las no realizadas
    } catch (error) {
      console.error('Error obteniendo shared sessions:', error);
      return [];
    }
  },

  // ==================== MOSTRAR CALENDARIO (MODIFICADO) ====================
  ...
  mostrarCalendario(sesiones) {
    ...
    // Obtener sesiones compartidas del plan actual y añadirlas al calendario
    this._cargarSharedSessionsYRenderizar(sesiones);
  ...
  async _cargarSharedSessionsYRenderizar(sesiones) {
    if (!AppState.currentUserId || !AppState.planActualId) return sesiones;
    try {
      const sharedSessions = await this.obtenerSesionesCompartidasParaCalendario(AppState.currentUserId, AppState.planActualId);
      if (sharedSessions.length === 0) return sesiones;
      
      // Convertir shared sessions a objetos de sesión para el calendario
      const sharedSessionsMapeadas = sharedSessions.map((s, index) => ({
        diaGlobal: 'shared_' + s.id,
        semana: 0, // Se pondrá en la semana actual
        diaSemana: new Date().getDay() || 7, // Hoy
        tipo: s.sessionData.tipo || 'rodaje',
        color: this.getColor(s.sessionData.tipo || 'rodaje'),
        letra: this.getLetra(s.sessionData.tipo || 'rodaje'),
        tieneFuerza: false,
        duracion: s.sessionData.duracion || 30,
        detalle: s.sessionData.detalle || {},
        esCompartida: true,
      ...
      // Añadir al final del array de sesiones
      return [...sesiones, ...sharedSessionsMapeadas];
    } catch (error) {
      console.error('Error cargando shared sessions:', error);
      return sesiones;
    }
  },

  // ==================== RENDERIZAR PÁGINA (MODIFICADO) ====================
  ...
  renderizarPagina(sesiones, semanasTotales) {
    ...
    for (let pos = 0; pos < celdas.length; pos++) {
      ...
      if (sesion.tipo !== 'descanso' && sesion.detalle) {
        ...
        // Si es compartida, añadir badge
        let badgeHtml = '';
        if (sesion.esCompartida) {
          badgeHtml = `<div style="font-size:8px;color:var(--zone-2);background:rgba(155,181,160,0.15);border-radius:10px;padding:1px 4px;margin-top:2px;">📨 @${Utils.escapeHTML(sesion.fromUsername || '')}</div>`;
        }
        contenidoHtml = `<strong>${Utils.escapeHTML(letra)}</strong><div>${tiempo}'</div>${badgeHtml}`;
      } else {
    ...
    document.querySelectorAll('.calendario-dia[data-index]').forEach(dia => {
      dia.addEventListener('click', (e) => {
        ...
        if (diaIndex) {
          // Buscar la sesión correspondiente (puede ser shared)
          let sesion = sesiones.find(s => s.diaGlobal == diaIndex);
          if (!sesion && typeof diaIndex === 'string' && diaIndex.startsWith('shared_')) {
            // Buscar entre shared sessions
            const sharedId = diaIndex.replace('shared_', '');
            sesion = sesiones.find(s => s.diaGlobal === diaIndex);
          }
          if (sesion) this.abrirDetalleSesion(sesion, diaIndex);
      ...
      try {
        ...
        if (planDoc.exists) {
          ...
          if (planCompleto?.sesiones) {
            // Cargar shared sessions y renderizar
            const sesionesConShared = await this._cargarSharedSessionsYRenderizar(planCompleto.sesiones);
            this.renderizarPagina(sesionesConShared, semanasTotales);
            this.actualizarNavegacionTrimestral(totalPaginas);
        ...
        Utils.showToast('❌ Error al cambiar de página. Reintenta.', 'error');
      }
    }
  },

  // ==================== ABRIR DETALLE DE SESIÓN (MODIFICADO) ====================
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // Añadir badge de sesión compartida si procede
      let sharedBadge = '';
      if (sesion.esCompartida) {
        sharedBadge = `
          <div style="background:rgba(155,181,160,0.15);border:1px solid var(--zone-2);border-radius:12px;padding:8px;margin:12px 0;text-align:center;">
            📨 Sesión compartida por <strong>@${Utils.escapeHTML(sesion.fromUsername)}</strong>
          </div>
        `;
      }
      
      descripcion.innerHTML = headerHTML + sharedBadge + objetivoHTML + zonasHTML + pasosHTML;
      ...
      shareBtn.id = 'shareSessionBtn';
      shareBtn.style.cssText = [
        'margin-top:12px',
        'width:100%',
        'height:48px',
        'background:transparent',
        'border:2px solid var(--zone-2)',
      ...
      shareBtn.innerHTML = '📤 COMPARTIR CON AMIGO';
      shareBtn.addEventListener('mouseenter', () => {
        shareBtn.style.background = 'var(--zone-2)';
        shareBtn.style.color = 'var(--bg-primary)';
      });
      shareBtn.addEventListener('mouseleave', () => {
        shareBtn.style.background = 'transparent';
        shareBtn.style.color = 'var(--text-primary)';
      });
      shareBtn.addEventListener('click', () => PlanGenerator.compartirSesion(sesion, diaIndex));
      descripcion.appendChild(shareBtn);
      
      checkboxContainer.style.display = 'flex';
      // Si es compartida, el checkbox debe reflejar su estado
      const isRealizada = sesion.realizado || AppState.sesionesRealizadas?.[diaIndex] || false;
      checkbox.checked = isRealizada;
  ...
  async guardarFeedback(diaIndex, valor) {
    ...
    try {
      ...
      Utils.showToast('✅ Feedback guardado correctamente.', 'success');
    } catch (error) {
      console.error('Error guardando feedback:', error);
      Utils.showToast('❌ Error al guardar feedback. Reinténtalo.', 'error');
    }
  },
  ...
  async marcarSesionRealizada(diaIndex, realizada, realDistance = null, realDurationMs = null) {
    // Verificar si es una sesión compartida (diaIndex empieza con 'shared_')
    if (typeof diaIndex === 'string' && diaIndex.startsWith('shared_')) {
      const sharedId = diaIndex.replace('shared_', '');
      await Storage.marcarSharedSessionRealizada(AppState.currentUserId, sharedId);
      // Actualizar UI
      ...
      Utils.showToast(realizada ? '✅ Sesión compartida realizada' : '📝 Sesión compartida desmarcada', 'success');
      return;
    }

    // Si no es compartida, proceder con la lógica normal
    if (!AppState.currentUserId || !AppState.planActualId) return;
    try {
      const planRef = firebaseServices.db
        .collection('users')
      ...
      Utils.showToast(realizada ? '✅ Sesión marcada como realizada.' : '📝 Sesión desmarcada. Los datos se han actualizado.', 'success');
    } catch (error) {
  ...
  async mostrarUltimoPlanGuardado() {
    if (!AppState.currentUserId) {
      Utils.showToast("⚠️ No hay usuario autenticado.", 'error');
      return;
    }
    try {
      ...
      if (!ultimoPlanId) {
        Utils.showToast("ℹ️ No hay ningún plan guardado.", 'info');
        return;
      }
      const planDoc = await firebaseServices.db
        .collection('users')
      ...
      document.getElementById("calendarioEntreno").style.display = "block";
      document.getElementById("cuestionarioEntreno").style.display = "none";
      // Cargar con shared sessions
      const sesionesConShared = await this._cargarSharedSessionsYRenderizar(planCompleto.sesiones);
      this.mostrarCalendario(sesionesConShared);
    } catch (error) {
      console.error('Error al cargar último plan:', error);
      Utils.showToast('❌ Error al cargar el plan. Reinténtalo.', 'error');
    }
  },
  ...
  async borrarPlanGuardado() {
    if (!AppState.currentUserId) return;
    if (!AppState.isPremium) {
      Utils.showToast("⭐ Solo los usuarios Premium pueden eliminar planes.", 'warning');
      return;
    }
    const confirmed = await Utils.confirm('ELIMINAR PLAN', "⚠️ ¿Eliminar plan guardado?");
    if (!confirmed) return;
    Utils.showLoading();
    try {
      ...
      Utils.showToast("✅ Plan eliminado correctamente.", 'success');
      if (window.UI) {
        UI.guardarEstado();
...
console.log('✅ PlanGenerator v2.51 - Compartir sesiones con amigos');