...
// Versión: 3.6 - DEFINITIVA (series dinámicas + día de descanso óptimo)
// 
// CARACTERÍSTICAS:
...
// - Cálculo dinámico de series según ritmo del usuario (calentamiento y enfriamiento fijos)
// - Día de descanso alineado con el día de tirada larga (no fijo)
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // Convertir la plantilla (objeto con claves 1-7) en un array ordenado
        const ordenSemana = [];
        for (let i = 1; i <= 7; i++) {
          ordenSemana.push(plantillaSemana[i] || { tipo: 'descanso' });
        }
        
        // Encontrar la posición del día largo en la plantilla (primer día con tipo 'largo')
        let posLargo = ordenSemana.findIndex(dia => dia.tipo === 'largo');
        if (posLargo === -1) {
          // Si no hay largo (no debería ocurrir), usamos el primer día no descanso como fallback
          posLargo = ordenSemana.findIndex(dia => dia.tipo !== 'descanso');
          if (posLargo === -1) posLargo = 0;
        }
        
        const semana = [];
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          // Calcular el índice de la plantilla según el día largo elegido
          const indicePlantilla = (posLargo + (diaSemana - diaLargo) + 7) % 7;
          const sesionBase = ordenSemana[indicePlantilla];
          ...
          // Aplicar factor de progresión al día largo (si es el día elegido y es largo)
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, faseActual, semanaEnFase, libreriaPerfil, datos) {
    ...
    if (tipo === 'series' && sesionMatriz.repeticiones && sesionMatriz.distanciaSerie) {
      ...
      // Calcular duración base con factores de fase
      ...
      // Escalar la estructura original
    ...
    // Calcular ritmos y FC (común para todos los tipos)
    ...
    // Añadir repeticiones si es una serie
  ...
  escalarEstructura(estructura, duracionObjetivo) {
    ...
    // Dividir la estructura por '+' y limpiar espacios
    const segmentos = estructura.split('+').map(s => s.trim());
    const partes = []; // Array de objetos { texto, minutos (opcional) }

    for (let seg of segmentos) {
      // Intentar extraer minutos al inicio del segmento (ej: "15' calentamiento")
      const match = seg.match(/^(\d+)'(.*)$/);
      if (match) {
        partes.push({
          texto: match[2],      // texto después de los minutos
          minutos: parseInt(match[1])
        });
      } else {
        // Segmento sin minutos (ej: "3x5000m") - en teoría no debería aparecer aquí,
        // pero por si acaso lo conservamos sin escalar
        partes.push({
          texto: seg,
          minutos: null
        });
    ...
    // Separar las partes con minutos de las que no
    const partesConMinutos = partes.filter(p => p.minutos !== null);
    const partesSinMinutos = partes.filter(p => p.minutos === null);

    // Si no hay partes con minutos, devolvemos la estructura original
    if (partesConMinutos.length === 0) {
      return estructura;
    }

    // Calcular la suma original de minutos (solo de las partes con minutos)
    const sumaOriginal = partesConMinutos.reduce((acc, p) => acc + p.minutos, 0);

    // Si la suma original ya es la duración objetivo, devolvemos la estructura sin cambios
    ...
    // Reconstruir la estructura en orden original
    for (let i = 0; i < partes.length; i++) {
      const p = partes[i];
      if (p.minutos !== null) {
        // Parte con minutos: escalar
        let nuevosMinutos;
        // Calcular cuántas partes con minutos quedan por procesar (incluyendo esta)
        const restantesConMinutos = partes.slice(i).filter(p => p.minutos !== null).length;
        if (restantesConMinutos === 1) {
          // Última parte con minutos: ajustar para que la suma total sea exacta
        ...
        // Parte sin minutos: se añade tal cual
  ...
  aplicarFactorLargo(sesion, progresion, semanaEnFase) {
    ...
    return {
      ...
      detalle: {
        ...sesion.detalle,
        duracion: nuevaDuracionLimitada,
        // Volvemos a escalar la estructura con la nueva duración (solo si no es series, porque en series ya tenemos estructura fija)
...
console.log('✅ PlanGenerator v3.6 - DEFINITIVA: series dinámicas y día de descanso óptimo');