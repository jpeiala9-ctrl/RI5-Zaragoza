...
const PlanGenerator = {
  ...
  abrirModalDatosReales(sesion) {
    return new Promise((resolve) => {
      ...
      // 🔥 La zona ya no se elige entre 7 botones siempre visibles, ni con
      // un panel que se despliega DENTRO de la tarjeta (eso seguía siendo
      // igual de grande, solo que metido dentro). Ahora, al tocar la
      // zona, se abre el mismo tipo de ventanita pequeña y centrada que
      // ya se usa aquí mismo para elegir el TIPO de sesión
      // (abrirSelectorTipo): un cuadradito flotante encima de todo, no un
      // bloque que empuja el resto de la tarjeta hacia abajo.
      const abrirSelectorZona = () => {
        document.getElementById('datosRealesZonaPopupOverlay')?.remove();
        const popupOverlay = document.createElement('div');
        popupOverlay.id = 'datosRealesZonaPopupOverlay';
        ...
        popup.innerHTML = `
          ...
          <div style="display:flex; flex-wrap:wrap; gap:8px; justify-content:center;">
            ${ZONAS_CHIPS.map(z => {
              ...
              return `<button type="button" class="datos-reales-zona-opcion" data-zona="${z.codigo || ''}" style="
                padding:10px 16px; border-radius:20px; font-family:'Courier New',monospace; font-size:13px; font-weight:bold;
                cursor:pointer; letter-spacing:1px;
                background:${activo ? 'var(--gold)' : 'var(--bg-primary)'};
        ...
        popup.querySelectorAll('.datos-reales-zona-opcion').forEach(btn => {
      ...
      if (zonaBtnEl) zonaBtnEl.addEventListener('click', abrirSelectorZona);
