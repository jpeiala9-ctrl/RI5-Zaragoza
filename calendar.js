...
// Versión: 5.2 - CORREGIDA (respetar días seleccionados por el usuario)
//
// CARACTERÍSTICAS:
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          let sesionBase = semanaOrdenada[diaSemana - 1];
          const esDiaActivo = diasEntreno.includes(diaSemana);

          // FIX: Si el día está activo pero la plantilla indica descanso, lo reemplazamos por un rodaje suave
          if (esDiaActivo && sesionBase.tipo === 'descanso') {
            sesionBase = { tipo: 'rodaje', variante: 'suave', fromMatrix: true };
          }
...
console.log('✅ PlanGenerator v5.2 - Corregido: respeta días seleccionados');