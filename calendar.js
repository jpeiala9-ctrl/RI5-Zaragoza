...
// Versión: 3.1 - CORREGIDO: estructuras de sesiones con múltiples componentes
// 
// CORRECCIÓN IMPORTANTE: Ya no reemplaza TODOS los números por la duración total
// Ahora mantiene los números originales de los componentes y solo muestra
// la duración total como dato aparte
...
const PlanGenerator = {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          ...
          // Crear sesión personalizada (VERSIÓN CORREGIDA)
  ...
  // --- FUNCIÓN CLAVE CORREGIDA: respeta los números originales en la estructura ---
  async crearSesionDesdeMatriz(sesionBase, esActivo, faseActual, semanaEnFase, libreriaPerfil, datos) {
    ...
    // Guardar la estructura original para usarla después
    const estructuraOriginal = sesionMatriz.estructura;
    ...
    // === CORRECCIÓN IMPORTANTE: Mantener la estructura original ===
    // Ya NO reemplazamos todos los números por la duración calculada
    // Solo mostramos la duración total como un dato aparte
    let estructura = estructuraOriginal || `${duracion}' continuos`;
    ...
    const detalle = {
      ...
      estructura: estructura,  // ← AHORA USA LA ESTRUCTURA ORIGINAL SIN MODIFICAR
      ...
      duracion: duracion,      // ← La duración total se muestra aparte
  ...
  aplicarFactorLargo(sesion, progresion, semanaEnFase) {
    ...
    return {
      ...
      detalle: {
        ...sesion.detalle,
        duracion: nuevaDuracion,
        // La estructura NO se modifica porque ya no reemplazamos números
...
console.log('✅ PlanGenerator v3.1 - CORREGIDO: estructuras de sesiones respetadas');