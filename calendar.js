...
// MODIFICACIÓN: marcarSesionRealizada acepta skipToast para evitar duplicados
// ============================================================================
...
const PlanGenerator = {
  ...
  // ==================== FUNCIÓN CORREGIDA (con skipToast) ====================
  async marcarSesionRealizada(diaIndex, realizada, skipToast = false) {
    if (!AppState.currentUserId || !AppState.planActualId) return;
    try {
      const planRef = firebaseServices.db
        .collection('users')
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          try {
            ...
            if (!skipToast) Utils.showToast('Error al publicar en el muro', 'error');
          }
      ...
      if (!skipToast) Utils.showToast(realizada ? 'Sesión marcada' : 'Sesión desmarcada', 'success');
    } catch (error) {
      console.error('Error marcando sesión:', error);
      if (!skipToast) Utils.showToast('Error al marcar la sesión', 'error');
    }
  },
...
console.log('✅ PlanGenerator v2.47 - GPS tracking integrado (con skipToast)');