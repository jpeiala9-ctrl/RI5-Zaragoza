...
// Versión: 3.2 - CORREGIDO: Estructuras escaladas para que sumen la duración total
...
const PlanGenerator = {
  ...
  // ==================== FUNCIÓN CORREGIDA - CON ESCALADO DE ESTRUCTURA ====================
  async crearSesionDesdeMatriz(sesionBase, esActivo, faseActual, semanaEnFase, libreriaPerfil, datos) {
    ...
    // ===== NUEVO: ESCALAR LA ESTRUCTURA PARA QUE SUMA SEA LA DURACIÓN TOTAL =====
    ...
    const detalle = {
      ...
      estructura: estructuraEscalada,  // ← AHORA ESTÁ ESCALADA
  ...
  // ===== NUEVA FUNCIÓN: ESCALA UNA ESTRUCTURA PARA QUE SUMA SEA LA DURACIÓN OBJETIVO =====
  escalarEstructura(estructura, duracionObjetivo) {
    if (!estructura) return `${duracionObjetivo}' continuos`;

    // Caso 1: Estructura simple de un solo número (ej: "45' continuos")
    const matchSimple = estructura.match(/^(\d+)'(.+)$/);
    if (matchSimple) {
      return duracionObjetivo + "'" + matchSimple[2];
    }

    // Caso 2: Estructura con múltiples partes separadas por '+'
    ...
    partes.forEach(parte => {
      ...
      if (match) {
        ...
        textos.push(match[2]); // guardamos el texto después de los minutos (ej: " Z2")
      } else {
        // Si no tiene número, lo dejamos igual (ej: "descanso")
        textos.push(parte);
      }
    });

    if (numeros.length === 0) {
      // No hay números escalables, devolvemos original
      return estructura;
    }

    const sumaOriginal = numeros.reduce((a, b) => a + b, 0);
    const factor = duracionObjetivo / sumaOriginal;

    // Escalar cada número y reconstruir
    let resultado = [];
    let idxNum = 0;
    let sumaParcial = 0;
    ...
    for (let i = 0; i < partes.length; i++) {
      ...
      if (match) {
        let nuevoNum;
        if (idxNum === numeros.length - 1) {
          // Último número: ajustar para que la suma sea exacta
          nuevoNum = duracionObjetivo - sumaParcial;
        } else {
          nuevoNum = Math.round(numeros[idxNum] * factor);
          sumaParcial += nuevoNum;
        }
        resultado.push(`${nuevoNum}'${textos[idxNum]}`);
        idxNum++;
      } else {
  ...
  aplicarFactorLargo(sesion, progresion, semanaEnFase) {
    ...
    return {
      ...
      detalle: {
        ...sesion.detalle,
        duracion: nuevaDuracionLimitada,
        // También hay que escalar la estructura de nuevo
  ...
  // ===== FUNCIÓN MODIFICADA PARA MOSTRAR LA ESTRUCTURA ESCALADA =====
...
console.log('✅ PlanGenerator v3.2 - Corregido: estructuras escaladas para que sumen la duración total');