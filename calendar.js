// ==================== calendar.js - VERSIÓN CORREGIDA (CON RESTRICCIONES PREMIUM) ====================
...
const PlanGenerator = {
  ...
  // FUNCIÓN MODIFICADA PARA MOSTRAR MENSAJE DE PREMIUM EXPIRADO
  mostrarCalendario(sesiones) {
    ...
    if (!puedeVerDetalle) {
      ...
      if (notaPlan) {
        ...
        msgExpirado.innerHTML = '⚠️ Tu premium ha expirado. Solo puedes ver los detalles de los primeros 3 días del plan. <button onclick="showPremiumBenefits()">HAZTE PREMIUM</button>';
        notaPlan.parentNode.insertBefore(msgExpirado, notaPlan.nextSibling);
      }
    }
  },

  // FUNCIÓN MODIFICADA PARA RENDERIZAR PÁGINA CON BLOQUEO DE DÍAS
  renderizarPagina(sesiones) {
    ...
    // Calcular días desde inicio del plan
    ...
    for (let i = inicioPagina; i < finPagina; i++) {
      ...
      // Determinar si este día está bloqueado
      let bloqueado = false;
  ...
  // FUNCIÓN MODIFICADA CON VERIFICACIÓN PREMIUM
  abrirDetalleSesion(sesion, diaIndex) {
    if (!sesion) return;

    // Verificar si puede ver el detalle
...
console.log('✅ PlanGenerator con restricciones premium y calendario simplificado');