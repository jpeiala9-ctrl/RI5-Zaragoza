    ...
    return new Promise((resolve) => {
      ...
      modal.innerHTML = `
        ...
        <div id="datosRealesZonaBox" style="background:var(--bg-primary); border:1px solid var(--border-color); border-left:4px solid transparent; border-radius:12px; padding:12px; margin-bottom:18px; transition:border-color .2s ease;">
          ...
          <button type="button" id="datosRealesZonaBtn" style="margin-top:6px; width:100%; background:transparent; border:none; padding:6px 0 0; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:6px; font-family:inherit;">
            <span id="datosRealesZona" style="font-size:12px; color:var(--text-secondary);">&nbsp;</span>
            <span id="datosRealesZonaCaret" style="font-size:9px; color:var(--text-secondary); display:inline-block; transition:transform .2s ease;">▾</span>
          </button>
          <div id="datosRealesZonaExpand" style="display:none; margin-top:10px; padding-top:10px; border-top:1px solid var(--border-color);">
            <div style="font-size:10px; color:var(--text-secondary); letter-spacing:0.5px; margin-bottom:8px;">Si fue distinta (desnivel, más de pulso...), elige la zona real:</div>
            <div id="datosRealesZonaChips" style="display:flex; flex-wrap:wrap; gap:6px; justify-content:center;"></div>
          </div>
        </div>
      ...
      const zonaBtnEl = document.getElementById('datosRealesZonaBtn');
      const zonaCaretEl = document.getElementById('datosRealesZonaCaret');
      const zonaExpandEl = document.getElementById('datosRealesZonaExpand');

      // 🔥 La zona ya no se elige entre 7 botones siempre visibles (AUTO +
      // Z1..Z6, todos a la vista todo el tiempo). Ahora el propio
      // indicador de la zona automática -- el que ya se recalculaba solo
      // al cambiar km/tiempo -- ES el botón: se toca, se despliega el
      // selector de zona real debajo, y al elegir una se recoge solo. Así
      // es un único control (mostrar zona / corregirla) en vez de dos
      // bloques separados y siempre visibles.
      let zonaExpandAbierto = false;
      const setZonaExpand = (abierto) => {
        zonaExpandAbierto = abierto;
        if (zonaExpandEl) zonaExpandEl.style.display = abierto ? 'block' : 'none';
        if (zonaCaretEl) zonaCaretEl.style.transform = abierto ? 'rotate(180deg)' : 'rotate(0deg)';
      };
      if (zonaBtnEl) zonaBtnEl.addEventListener('click', () => setZonaExpand(!zonaExpandAbierto));

      // null = automática (por ritmo, como antes). El ritmo medio no
      // distingue un llano de una sesión con desnivel o con el pulso más
      // alto de lo normal -- aquí se puede decir "en realidad esto fue
      // Z3/Z4", y esa zona (no la calculada) es la que se usa luego para
      // guardar la sesión y recalcular el TSS.
      ...
      const chipsZonaCont = document.getElementById('datosRealesZonaChips');
      const pintarChipsZona = () => {
        if (!chipsZonaCont) return;
        chipsZonaCont.innerHTML = ZONAS_CHIPS.map(z => {
          const activo = zonaOverride === z.codigo;
          return `<button type="button" class="datos-reales-zona-chip" data-zona="${z.codigo || ''}" style="
            padding:6px 12px; border-radius:16px; font-family:'Courier New',monospace; font-size:11px; font-weight:bold;
        ...
        chipsZonaCont.querySelectorAll('.datos-reales-zona-chip').forEach(btn => {
          btn.addEventListener('click', () => {
            zonaOverride = btn.dataset.zona || null;
            pintarChipsZona();
            recalcular();
            setZonaExpand(false); // elegida la zona, el selector se recoge solo
          });
        });
      };
      pintarChipsZona();
      ...
      const recalcular = () => {
        ...
        if (km > 0 && totalMin > 0) {
          ...
          if (zonaOverride) {
            const infoOverride = (AppState.lastZones || []).find(z => z[0] === zonaOverride);
            const etiquetaOverride = infoOverride ? infoOverride[1] : '';
            zonaEl.innerHTML = `Zona de trabajo: <strong>${Utils.escapeHTML(zonaOverride)}</strong>${etiquetaOverride ? ' · ' + Utils.escapeHTML(etiquetaOverride) : ''} <span style="opacity:0.75;">(corregida a mano)</span>`;
            zonaBoxEl.style.borderLeftColor = `var(--zone-${zonaOverride.replace('Z', '')})`;
          } else {
