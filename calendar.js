...
// Versión: 2.65 - Mapa aéreo Esri, cuenta atrás, marcado naranja, desmarcado funcional
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== FUNCIÓN CORREGIDA: MARCAR/DESMARCAR SESIÓN (CON CHECKBOX) ==========
  async marcarSesionRealizada(diaIndex, realizada) {
    ...
    try {
      ...
      if (!realizada) {
        // Si se desmarca, eliminar también el flag de forzada si existe
      ...
      // Actualizar la celda del calendario visualmente
      const celda = document.querySelector(`.calendario-dia[data-index="${diaIndex}"]`);
      if (celda) {
        if (realizada) {
          // Si se marca, añadir clase 'realizado' y quitar 'realizado-forzado'
          celda.classList.add('realizado');
          celda.classList.remove('realizado-forzado');
        } else {
          // Si se desmarca, eliminar ambas clases
          celda.classList.remove('realizado', 'realizado-forzado');
          // Restaurar el color original según el tipo de sesión (sin recargar)
          const planDoc = await planRef.get();
          const planData = planDoc.data();
          const sesion = planData.sesiones[diaIndex - 1];
          if (sesion && sesion.color) {
            // Eliminar todas las clases de color y añadir la original
            ...
            // Fallback: recargar la página para restaurar colores (no debería ocurrir)
            location.reload();
          }
        }
      }
      ...
      if (realizada) {
        // MARCADA: publicar en el muro y actualizar gamificación
        ...
        // DESMARCADA: eliminar del muro y revertir gamificación
        const planData = planDoc.data();
  ...
  // ========== MARCAR SESIÓN FORZADA (NARANJA) - SIN RECARGA ==========
  ...
  // ========== FUNCIÓN ABRIR DETALLE CON MAPA AÉREO ESRI Y BOTÓN NARANJA ==========
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // ========== CONTROLES DE GPS CON MAPA AÉREO ESRI ==========
      if (!realizada) {
...
console.log('✅ PlanGenerator v2.65 - Mapa aéreo Esri, cuenta atrás, marcado naranja, desmarcado funcional');