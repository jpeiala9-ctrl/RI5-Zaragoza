// ============================================================================
// calendar.js - VERSIÓN ÉLITE 2.12
// - Rodajes con variabilidad natural (aleatoria + escalado)
// - Tirada larga con mínimo coherente (45' en base, progresión hasta 210')
// - Feed: pre‑cálculo de distancia y TSS garantizado
// - Series: mínimo 4 repeticiones (3 en taper), ajuste dinámico
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== FUNCIÓN DE MÁXIMOS POR TIPO (ajustada) ==========
  getMaximosPorTipo(tipo, nivel, fase) {
    ...
    if (tipo === 'series' && max > 90) max = 90;
    // Para tirada larga, mínimo 45 en fase base, 60 en construcción, 90 en específica/pico
    if (tipo === 'largo') {
      if (fase === 'BASE') max = Math.max(max, 45);
      if (fase === 'CONSTRUCCION') max = Math.max(max, 60);
      if (fase === 'ESPECIFICA' || fase === 'PICO') max = Math.max(max, 90);
    }
  ...
  // ========== PROGRESIÓN INTRA‑FASE (sin taper) ==========
  obtenerFactorProgresion(semanaEnFase, duracionFase, fase) {
    if (fase === 'TAPER') return 1.0;
  ...
  // ==========================================================================
  // GENERACIÓN DEL PLAN (se mantiene igual, solo se modifica la asignación interna)
  // ==========================================================================
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        const factorProgresion = this.obtenerFactorProgresion(semanaEnFase, duracionFase, fase);
        ...
        // ========== ASIGNACIÓN DE TIRADA LARGA ==========
        if (minutosPorTipo.largo > 0) {
          // Asegurar un mínimo coherente según fase
          let largoMinimo = 45;
          ...
          // Aplicar máximo de la fase
          const maxLargo = this.getMaximosPorTipo('largo', nivelActual, fase);
        ...
        // ========== ASIGNACIÓN DE CALIDAD (tempo y series) ==========
        const tiposCalidad = [];
        ...
        // ========== RODAJES CON VARIABILIDAD NATURAL ==========
        ...
        if (numDiasRodaje > 0 && minutosRodajeTotal > 0) {
          const maxRodaje = this.getMaximosPorTipo('rodaje', nivelActual, fase);
          const minRodaje = 35; // valor mínimo absoluto
          // Generar valores aleatorios dentro de un rango
          let valores = [];
          let sumaAleatoria = 0;
          for (let i = 0; i < numDiasRodaje; i++) {
            let rangoSuperior = Math.min(maxRodaje, minutosRodajeTotal); // no más del total
            let valor = Math.floor(Math.random() * (rangoSuperior - minRodaje + 1) + minRodaje);
            valores.push(valor);
            sumaAleatoria += valor;
          }
          // Escalar para que la suma coincida exactamente con minutosRodajeTotal
          if (sumaAleatoria > 0) {
            let factor = minutosRodajeTotal / sumaAleatoria;
            let sumaEscalada = 0;
            let valoresEscalados = valores.map(v => {
              let escalado = Math.floor(v * factor);
              sumaEscalada += escalado;
              return escalado;
            });
            // Ajustar la diferencia por redondeo
            let diferencia = minutosRodajeTotal - sumaEscalada;
            for (let i = 0; i < diferencia && i < valoresEscalados.length; i++) {
              valoresEscalados[i]++;
            }
            // Asegurar que ningún valor supere el máximo ni sea menor que el mínimo
            for (let i = 0; i < valoresEscalados.length; i++) {
              valoresEscalados[i] = Math.min(valoresEscalados[i], maxRodaje);
              valoresEscalados[i] = Math.max(valoresEscalados[i], minRodaje);
            }
            // Asignar a los días
            for (let i = 0; i < numDiasRodaje; i++) {
              const dia = diasLibres[i];
              tiposPorDia[dia] = { tipo: 'rodaje', minutos: valoresEscalados[i] };
            }
          } else {
            // Caso de seguridad: si la suma aleatoria es 0, asignar rodaje mínimo
            for (let i = 0; i < numDiasRodaje; i++) {
              const dia = diasLibres[i];
  ...
  // ==========================================================================
  // CREACIÓN DE SESIÓN DESDE MATRIZ (con duración exacta y pre‑cálculo de métricas)
  // ==========================================================================
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    ...
    const obtenerRitmoObjetivo = () => {
      if (distancia === '10k') return ritmoBase * 0.98;
      if (distancia === 'medio') return ritmoBase * 1.03;
      if (distancia === 'maraton') return ritmoBase * 1.07;
      return ritmoBase * 1.05;
    };
    ...
    if (incluirRitmoObjetivo) {
      ...
      const ritmoObjetivoStr = Utils.formatR(obtenerRitmoObjetivo());
      ...
      switch(tipo) {
        ...
        case 'largo':
          ...
          break;
        case 'series': {
          let distanciaSerie = 400;
          if (sesionMatriz && sesionMatriz.distanciaSerie) {
            distanciaSerie = sesionMatriz.distanciaSerie;
          } else {
            if (distancia === '2k' || distancia === '5k') distanciaSerie = 300;
            else if (distancia === '10k') distanciaSerie = 800;
            else if (distancia === 'medio' || distancia === 'maraton') distanciaSerie = 1000;
          }

          const ritmoSeries = ritmoBase * 0.95;
          const tiempoRepeticion = (distanciaSerie / 1000) * ritmoSeries;
          const factorDescanso = distanciaSerie <= 400 ? 1.0 : (distanciaSerie <= 800 ? 1.2 : 1.5);
          const tiempoRecuperacion = tiempoRepeticion * factorDescanso;
          const tiempoCiclo = tiempoRepeticion + tiempoRecuperacion;

          const esTaper = (fase === 'TAPER');
          const minReps = esTaper ? 3 : 4;
          
          let repeticiones = Math.max(minReps, Math.floor(partePrincipal / tiempoCiclo));
          let distanciaFinal = distanciaSerie;
          
          if (repeticiones < minReps && partePrincipal > 0) {
            const tiempoNecesario = minReps * tiempoCiclo;
            const factor = Math.min(1, partePrincipal / tiempoNecesario);
            distanciaFinal = Math.max(200, Math.round(distanciaSerie * factor));
            const nuevoTiempoRep = (distanciaFinal / 1000) * ritmoSeries;
            const nuevoTiempoRec = nuevoTiempoRep * factorDescanso;
            const nuevoTiempoCiclo = nuevoTiempoRep + nuevoTiempoRec;
            repeticiones = Math.floor(partePrincipal / nuevoTiempoCiclo);
            if (repeticiones < minReps) repeticiones = minReps;
          }
          
          let tiempoRecuperacionFinal = tiempoRecuperacion;
          let tiempoCicloFinal = tiempoRepeticion + tiempoRecuperacionFinal;
          let tiempoTotal = repeticiones * tiempoCicloFinal;
          let tiempoExtra = partePrincipal - tiempoTotal;
          if (tiempoExtra < 0) {
            const tiempoObjetivo = partePrincipal / repeticiones;
            const nuevaRecuperacion = Math.max(0.5, tiempoObjetivo - tiempoRepeticion);
            tiempoRecuperacionFinal = nuevaRecuperacion;
            tiempoCicloFinal = tiempoRepeticion + nuevaRecuperacion;
            tiempoTotal = repeticiones * tiempoCicloFinal;
            tiempoExtra = partePrincipal - tiempoTotal;
          }
          
          if (tiempoExtra > 10) tiempoExtra = 10;
          
          const tiempoSeriesReal = repeticiones * (distanciaFinal / 1000) * ritmoSeries;
          const ritmoObjetivoStr = Utils.formatR(ritmoSeries);
          let descripcionExtra = '';
          if (tiempoExtra > 5) {
            descripcionExtra = ` + ${Math.round(tiempoExtra)}' de rodaje suave entre series`;
          }
          
          descripcionPrincipal = `${Math.round(tiempoSeriesReal)}' series (${repeticiones}x${distanciaFinal}m) Z4-Z5${descripcionExtra}`;
          pasoPrincipal = {
            icono: '💪',
            titulo: 'SERIES',
            accion: `${repeticiones} repeticiones de ${distanciaFinal}m a ritmo rápido (aprox. ${ritmoObjetivoStr} min/km) con recuperación de ${Math.round(tiempoRecuperacionFinal)}' entre series. ${tiempoExtra > 5 ? `+ ${Math.round(tiempoExtra)}' de trote suave.` : ''}`,
    ...
    // ========== PRE‑CÁLCULO DE MÉTRICAS PARA FEED ==========
    const metricas = this.calcularMetricasSesion({ tipo, duracion, detalle });
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    // Código completo como en versiones anteriores (no se modifica)
    if (!sesion) return;
  ...
  calcularMetricasSesion(sesion) {
    ...
    // Para sesiones de fuerza, no calculamos distancia ni TSS
...
console.log('✅ PlanGenerator v16.12 - Rodajes con variabilidad, tirada larga coherente, feed con métricas garantizadas');