...
// Versión: 3.1 - CORREGIDO: Estructura de sesiones con duraciones proporcionales
// ====================
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // Validar que la fecha no sea demasiado antigua
      const hoy = new Date();
      hoy.setHours(0, 0, 0, 0);
      if (fechaInicio < hoy) {
        const confirmado = await Utils.confirm('FECHA PASADA', 'La fecha de inicio es anterior a hoy. ¿Continuar de todas formas?');
        if (!confirmado) {
          Utils.hideLoading();
          return;
        }
      ...
      if (nivel === 'principiante' && diasEntreno.length > 4) {
        const confirmado = await Utils.confirm('DEMASIADOS DÍAS', 'Para principiante, recomendamos máximo 4 días. ¿Continuar?');
        if (!confirmado) {
          Utils.hideLoading();
          return;
        }
      }
      if (nivel === 'intermedio' && diasEntreno.length > 5) {
        const confirmado = await Utils.confirm('DEMASIADOS DÍAS', 'Para nivel intermedio, recomendamos máximo 5 días. ¿Continuar?');
        if (!confirmado) {
          Utils.hideLoading();
          return;
        }
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          ...
          // Crear sesión personalizada (AHORA USA LA MATRIZ CON ESTRUCTURA CORREGIDA)
  ...
  // --- FUNCIÓN CLAVE CORREGIDA: Obtiene sesiones REALES de la matriz con estructura coherente ---
  async crearSesionDesdeMatriz(sesionBase, esActivo, faseActual, semanaEnFase, libreriaPerfil, datos) {
    ...
    // Ajustar por semana dentro de la fase (progresión)
    const progresoFase = semanaEnFase / faseActual.duracion;
    duracion = Math.round(duracion * (0.9 + (progresoFase * 0.3)));

    // --- CORRECCIÓN CRÍTICA: Personalizar estructura manteniendo proporciones ---
    let estructura = sesionMatriz.estructura;
    if (estructura) {
        if (estructura.includes('+')) {
            // Caso: estructura con múltiples partes (ej: "30' Z2 + 20' Z3 + 10' enfriamiento")
            const partes = estructura.split('+').map(p => p.trim());
            
            // Extraer duraciones originales de cada parte
            const duracionesOriginales = partes.map(p => {
                const match = p.match(/\b(\d{1,3})\b/);
                return match ? parseInt(match[1]) : 0;
            });
            
            const sumaOriginal = duracionesOriginales.reduce((a, b) => a + b, 0);
            
            if (sumaOriginal > 0) {
                // Escalar cada parte proporcionalmente a la nueva duración total
                const nuevasPartes = partes.map((parte, idx) => {
                    // Calcular duración proporcional para esta parte
                    const proporcion = duracionesOriginales[idx] / sumaOriginal;
                    const duracionParte = Math.round(duracion * proporcion);
                    
                    // Reemplazar SOLO el número de esta parte
                    return parte.replace(/\b\d{1,3}\b/, duracionParte.toString());
                });
                estructura = nuevasPartes.join(' + ');
            } else {
                // Fallback seguro: reemplazar solo el primer número
                estructura = estructura.replace(/\b\d{1,3}\b/, duracion.toString());
            }
        } else if (estructura.includes('x')) {
            // Caso: series (ej: "8x400m con recuperación 2'")
            // Mantener el número de repeticiones igual, solo ajustar si es necesario
            // Las series no deberían escalarse linealmente
            estructura = estructura; // Mantener estructura original para series
        } else {
            // Caso simple: un solo número (ej: "40' continuos")
            estructura = estructura.replace(/\b\d{1,3}\b/, duracion.toString());
        }
    } else {
        estructura = `${duracion}' continuos`;
    }
    ...
    if (tipo === 'rodaje') {
      ...
      ritmoObjetivo = Utils.formatR(ritmo);
      fcObjetivo = `${Math.round(fcUmbral * 0.92)}-${Math.round(fcUmbral * 0.98)} lpm`;
    } else if (tipo === 'series') {
    ...
    } else if (tipo === 'series' && estructura.includes('x')) {
      // Intentar extraer repeticiones de la estructura
      const match = estructura.match(/(\d+)x/);
      if (match) detalle.repeticiones = parseInt(match[1]);
    }

    // Validar que la estructura sea coherente con la duración
    const sesionFinal = {
      tipo,
    ...
    // Validación de coherencia (opcional, para debug)
    this.validarCoherenciaSesion(sesionFinal);
    
    return sesionFinal;
  },

  // --- FUNCIÓN AUXILIAR PARA VALIDAR COHERENCIA DE SESIONES ---
  validarCoherenciaSesion(sesion) {
    if (!sesion.detalle || !sesion.detalle.estructura) return;
    
    // Extraer todos los números de la estructura
    const numeros = sesion.detalle.estructura.match(/\b\d{1,3}\b/g) || [];
    let suma = 0;
    numeros.forEach(n => suma += parseInt(n));
    
    // Si es una sesión de series, la lógica es diferente
    if (sesion.tipo === 'series') {
      // Para series, la suma puede ser muy superior a la duración total
      // porque incluye recuperaciones, no validamos
      return;
    }
    
    // Para sesiones continuas, la suma debería ser aproximadamente la duración
    if (Math.abs(suma - sesion.duracion) > 15 && suma > 0) {
      console.warn(`⚠️ Posible incoherencia en sesión ${sesion.tipo}:`, {
        estructura: sesion.detalle.estructura,
        sumaNumeros: suma,
        duracionEsperada: sesion.duracion
      });
    }
  },
  ...
  aplicarFactorLargo(sesion, progresion, semanaEnFase) {
    ...
    // Actualizar también la estructura si es necesario
    let nuevaEstructura = sesion.detalle.estructura;
    if (nuevaEstructura && !nuevaEstructura.includes('x')) {
      nuevaEstructura = nuevaEstructura.replace(/\b\d{1,3}\b/, nuevaDuracion.toString());
    }
...
console.log('✅ PlanGenerator v3.1 - CORREGIDO: Estructura de sesiones proporcional');