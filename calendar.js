...
// Versión: 2.61 - Corregida la lógica de la tirada larga: ahora se aumenta
//                si es más corta que otra sesión, en lugar de recortar las demás.
//                Añadido _ajustarDuracionSesion() para mantener consistencia
//                interna (suma de bloques = duración total) en cualquier ajuste.
//                Eliminado el antiguo bloque de recorte de otras sesiones.
...
const PlanGenerator = {
  ...
  async crearSesionBasica(tipo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null, esRecuperacion = false) {
    ...
    if (duracionExacta !== null) {
      ...
      if (duracion < min) duracion = min + Math.floor(Math.random() * 12);
    } else {
  ...
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    if (duracion < minSeries) duracion = minSeries + Math.floor(Math.random() * 12);
    ...
    switch(estructura) {
      case 'piramide': {
        ...
        if (factor >= 0.8 && factor <= 1.2) {
          ...
          // Límite de 3 pirámides completas para evitar sesiones de series interminables.
          const maxPiramides = 3;
          if (repeticiones >= 2 && repeticiones <= maxPiramides) {
            distancias = [];
  ...
  // Ajusta la duración de una sesión y reajusta todos sus campos internos
  // (calentamiento, parte principal, enfriamiento y pasos) para mantener
  // la consistencia. También recalcula distanciaEstimada y tssEstimada.
  _ajustarDuracionSesion(sesion, nuevaDuracion, factorIntensidad) {
    if (!sesion || !sesion.detalle || nuevaDuracion <= 0) return;
    nuevaDuracion = Math.round(nuevaDuracion);
    sesion.duracion = nuevaDuracion;
    sesion.detalle.duracion = nuevaDuracion;

    const { calentamiento, enfriamiento, partePrincipal } =
      this._calcularCalentamientoEnfriamiento(nuevaDuracion);
    sesion.detalle.calentamiento = calentamiento;
    sesion.detalle.enfriamiento = enfriamiento;
    sesion.detalle.partePrincipal = partePrincipal;

    if (Array.isArray(sesion.detalle.pasosDetallados)) {
      sesion.detalle.pasosDetallados.forEach(paso => {
        const tit = (paso.titulo || '').toUpperCase();
        if (tit.includes('CALENTAMIENTO')) {
          paso.duracionMin = calentamiento;
        } else if (tit.includes('ENFRIAMIENTO')) {
          paso.duracionMin = enfriamiento;
        } else if (tit.includes('PARTE PRINCIPAL') && paso.duracionMin !== undefined) {
          paso.duracionMin = partePrincipal;
        }
      });
    }

    const metricas = this.calcularMetricasSesion(
      { tipo: sesion.tipo, duracion: nuevaDuracion, detalle: sesion.detalle },
      factorIntensidad
    );
    sesion.detalle.distanciaEstimada = metricas.distanciaTotal;
    sesion.detalle.tssEstimada = metricas.tssTotal;
  },
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // GARANTÍA FINAL: la tirada larga tiene que ser, sin excepción, la sesión más larga de la semana.
        // En lugar de recortar las demás, aumentamos la tirada larga si es necesario.
        const sesionLargo = semana.find(s => s.tipo === 'largo');
        if (sesionLargo && sesionLargo.duracion) {
          let maxOtra = 0;
          for (const s of semana) {
            if (s.tipo !== 'largo' && s.tipo !== 'descanso' && s.duracion && s.duracion > maxOtra) {
              maxOtra = s.duracion;
            }
          }
          // Si la tirada larga no es estrictamente mayor que la siguiente más larga
          if (maxOtra >= sesionLargo.duracion) {
            const maxLargo = this.getMaximosPorTipo('largo', nivelActual, fase, distancia);
            // Intentamos aumentar la tirada larga para que sea al menos maxOtra + 5 (margen)
            let nuevoLargo = Math.min(maxLargo, maxOtra + 5 + Math.floor(Math.random() * 5));
            // Asegurar que al menos aumente 5' respecto a la actual
            if (nuevoLargo < sesionLargo.duracion + 5) nuevoLargo = sesionLargo.duracion + 5;
            // Si el nuevo largo es mayor que el máximo, lo ponemos al máximo
            if (nuevoLargo > maxLargo) nuevoLargo = maxLargo;
            // Solo ajustamos si realmente podemos superar a maxOtra
            if (nuevoLargo > maxOtra) {
              this._ajustarDuracionSesion(sesionLargo, nuevoLargo, intensidadTotal);
            } else {
              // Si no podemos superar a maxOtra (porque maxLargo es demasiado bajo),
              // entonces no hacemos nada (dejamos la tirada larga como está, aunque sea más corta)
              console.warn(`No se pudo aumentar la tirada larga para superar a ${maxOtra}'. Máximo permitido: ${maxLargo}'`);
            }
          }
        }
...
console.log('✅ PlanGenerator v2.61 - Tirada larga se aumenta en lugar de recortar; consistencia de duraciones garantizada');