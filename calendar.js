...
const PlanGenerator = {
  ...
  abrirModalDatosReales(sesion) {
    return new Promise((resolve) => {
      ...
      const zonaAutoOption = zonaSelectEl?.querySelector('option[value=""]');
      ...
      const recalcular = () => {
        ...
        if (km > 0 && totalMin > 0) {
          ...
          if (zona && zonaAutoOption) zonaAutoOption.textContent = `AUTO (${zona[0]} · ${zona[1]})`;
          if (zonaOverride) {
          ...
          ritmoEl.textContent = '--:--';
          if (zonaAutoOption) zonaAutoOption.textContent = 'AUTO';
