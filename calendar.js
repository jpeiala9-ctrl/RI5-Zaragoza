...
const PlanGenerator = {
  ...
  async mostrarUltimoPlanGuardado(silencioso = false) {
    ...
    try {
      ...
      if (!ultimoPlanId) {
        console.log('ℹ️ mostrarUltimoPlanGuardado: este usuario no tiene ultimoPlanId guardado (normal si nunca ha tenido plan)');
      ...
      const planDoc = await firebaseServices.db
        ...
        .get();
      if (!planDoc.exists) {
        // 🔥 Esto NO es "usuario sin plan" (caso normal y silencioso) --
        // es que el usuario SÍ tiene un ultimoPlanId guardado pero el
        // documento al que apunta ya no existe. Eso es una anomalía real
        // (el plan se borró por otro lado sin limpiar el puntero, o
        // algo se corrompió) y antes se tragaba en silencio si la
        // llamada venía con silencioso=true (como la del cambio de
        // pestaña) -- así que un usuario con un plan "fantasma" veía la
        // pestaña Plan vacía sin ningún aviso, ni en pantalla ni en
        // consola. Ahora se avisa siempre, aunque sea silencioso.
      ...
      // 🔥 Igual que arriba: un error de verdad (p.ej. permiso denegado)
      // no debe desaparecer en silencio solo porque la llamada venía con
      // silencioso=true -- ese flag es para el caso normal de "usuario
      // sin plan todavía", no para tapar fallos reales.
      Utils.showToast('Error al cargar el plan: ' + (error.message || error.code || 'desconocido'), 'error');
    }
  },
  ...
  toggleCuestionario() {
    ...
    // abre -- generar un plan nuevo sigue siendo solo premium. En vez de
    // deshabilitar el botón (como antes: sin feedback en móvil, ver nota
    // en actualizarInterfazPremium en app.js) se deja siempre pulsable y
    // se muestra directamente el aviso de premium con el contacto de
