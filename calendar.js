...
const PlanGenerator = {
  ...
  // ========== MARCAR SESIÓN REALIZADA (CON MURO GLOBAL Y ALERTAS) ==========
    ...
    try {
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          // Alerta para depuración en móvil
          alert(`📝 Guardando en muro: ${entry.trainingType} - ${entry.distancia} km`);
          ...
          try {
            ...
            alert(`✅ Entrada guardada en muro con ID: ${globalRef.id}`);
          } catch (err) {
          ...
          alert('⚠️ No se pudo crear entrada en muro: sesión inválida o es descanso');
        }
      } else {
        const planDoc = await planRef.get();
        ...
        if (wallEntryId) {
          try {
            ...
            alert(`🗑️ Entrada eliminada del muro`);
          } catch (err) {
...
console.log('✅ PlanGenerator v2.35 - Con gestión de muro global y alertas de depuración');