...
const PlanGenerator = {
  ...
  // Tabla de factor de intensidad por zona, usada tanto al planificar una
  // sesión (varias copias locales por el código de generación, sin tocar)
  // como al calcular el TSS real por tramos (ver calcularTSSdesdeReal).
  _factoresIF: { 'Z1': 0.6, 'Z2': 0.7, 'Z3': 0.85, 'Z4': 0.95, 'Z5': 1.05 },
  ...
  //
  // Se apoya en _desglosarSesionReal (ver más abajo), que hace el mismo
  // cálculo pero devolviendo el desglose completo de los tres tramos, no
  // solo el ritmo de la parte principal.
  _estimarRitmoParteEfectiva(sesion, totalMin, totalKm) {
    const desglose = this._desglosarSesionReal(sesion, totalMin, totalKm);
    if (desglose && desglose.partePrincipal.km > 0) {
      return desglose.partePrincipal.min / desglose.partePrincipal.km;
    }
    return (totalKm > 0) ? totalMin / totalKm : NaN;
  },

  // Desglosa una sesión REAL (tiempo y distancia totales introducidos por
  // el usuario, o medidos por GPS) en sus tres tramos -- calentamiento,
  // parte principal y enfriamiento -- cada uno con sus minutos, km y zona
  // estimados. Antes solo se guardaba una única "zona" para toda la
  // sesión (calculada ya con el ritmo ajustado de _estimarRitmoParteEfectiva),
  // así que la tarjeta de "última sesión"/muro/perfil solo podía mostrar
  // "60 min, Z2" sin más detalle -- ni el propio usuario podía comprobar
  // que su calentamiento y enfriamiento sí contaban aparte, en Z1.
  //
  // Se asume que calentamiento/enfriamiento se corrieron al ritmo Z1 del
  // usuario; se escala su duración PLANIFICADA (sesion.detalle.calentamiento/
  // enfriamiento, en minutos) a la duración TOTAL real introducida
  // (proporcionalmente, por si el usuario tardó más o menos de lo previsto)
  // para estimar cuánto tiempo/distancia se llevó cada tramo, y se obtiene
  // el de la parte principal por diferencia.
  //
  // Devuelve null (sin desglose posible, cae de vuelta al comportamiento
  // de antes: una sola zona para toda la sesión) si la sesión no tiene
  // calentamiento/enfriamiento definidos, o si el resultado no es
  // coherente (tiempo/distancia que saldrían negativos -- p.ej. el
  // usuario introdujo mucho menos tiempo del que tardaría solo en
  // calentar+enfriar).
  _desglosarSesionReal(sesion, totalMin, totalKm) {
    const det = sesion?.detalle;
    const duracionPlan = sesion?.duracion || 0;
    if (!det || duracionPlan <= 0 || (!det.calentamiento && !det.enfriamiento) || !(totalKm > 0) || !(totalMin > 0)) {
      return null;
    }
    ...
    if (mainMin <= 0) return null;

    const ritmoZ1MinPerKm = this._ritmoToSeg(this.obtenerRitmoParaZona('Z1')) / 60;
    if (!ritmoZ1MinPerKm || !isFinite(ritmoZ1MinPerKm)) return null;

    const calKm = calMin / ritmoZ1MinPerKm;
    const enfKm = enfMin / ritmoZ1MinPerKm;
    const mainKm = totalKm - calKm - enfKm;
    if (mainKm <= 0) return null;

    const zonaCalEnf = this._detectarZonaPorRitmo(ritmoZ1MinPerKm);
    const zonaMain = this._detectarZonaPorRitmo(mainMin / mainKm);

    return {
      calentamiento: { min: Math.round(calMin), km: +calKm.toFixed(2), zona: zonaCalEnf ? zonaCalEnf[0] : 'Z1' },
      partePrincipal: { min: Math.round(mainMin), km: +mainKm.toFixed(2), zona: zonaMain ? zonaMain[0] : (det.zona || '') },
      enfriamiento: { min: Math.round(enfMin), km: +enfKm.toFixed(2), zona: zonaCalEnf ? zonaCalEnf[0] : 'Z1' }
    };
  },
    ...
    return new Promise((resolve) => {
      ...
      const recalcular = () => {
        ...
        if (km > 0 && totalMin > 0) {
          ...
          // La ZONA se detecta con el ritmo estimado de la parte
          // principal (descontando calentamiento/enfriamiento a Z1), no
          // con el ritmo medio de toda la sesión -- ver
          // _estimarRitmoParteEfectiva. El "ritmo medio" de arriba sigue
          // mostrando el real, sin tocar.
          const paceParaZona = this._estimarRitmoParteEfectiva(sesion, totalMin, km);
          const zona = this._detectarZonaPorRitmo(paceParaZona);
          if (zona) {
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          if (realDistance !== null && realDurationMs !== null) {
            ...
            // Desglose por tramos de la sesión REAL (calentamiento/parte
            // principal/enfriamiento -- ver _desglosarSesionReal), calculado
            // una sola vez aquí y reutilizado para: el TSS real (más
            // preciso, pondera cada tramo por SU propia intensidad en vez
            // de la intensidad media de toda la sesión diluida por el
            // calentamiento/enfriamiento), la zona registrada más abajo, y
            // el detalle que se muestra al pulsar la tarjeta de la sesión
            // en Dashboard/Muro/Perfil. null si la sesión no tiene
            // calentamiento/enfriamiento definidos, o si el resultado no
            // es coherente -- en ese caso todo cae de vuelta al cálculo
            // anterior basado en la media de toda la sesión.
            const desgloseReal = this._desglosarSesionReal(sesion, realDurationMs / 60000, realDistance);
            metricas = {
          ...
          // _desglosarSesionReal): el ritmo medio (metricas.bestPace,
          // usado tal cual para el resto de métricas: récords, insignias
          // PACE_SUB5/PACE_SUB4...) se queda igual, pero para decidir EN
          // QUÉ ZONA se entrenó de verdad hay que descontar el
          ...
          //
          // Si esta marcación viene con GPS/datos reales ya se calculó el
          // desglose una vez (metricas.desglose); si no, se calcula aquí
          // (p.ej. al reutilizar directamente la estimación del plan).
          const desgloseFinal = metricas.desglose !== undefined
            ? metricas.desglose
            : ((distanciaUsada > 0 && duracionUsadaMs > 0)
                ? this._desglosarSesionReal(sesion, duracionUsadaMs / 60000, distanciaUsada)
                : null);
          const zonaReal = desgloseFinal
            ? this._detectarZonaPorRitmo(desgloseFinal.partePrincipal.min / desgloseFinal.partePrincipal.km)
            : null;
          ...
          // Desglose por tramos de la sesión REAL (calentamiento/parte
          // principal/enfriamiento, cada uno con sus minutos/km/zona) --
          // ver _desglosarSesionReal. Se guarda además de 'zone' (que solo
          // tiene la zona predominante) para poder mostrarlo al pulsar la
          // tarjeta de la sesión en Dashboard/Muro/Perfil, y para que las
          // gráficas de "zonas usadas" repartan minutos/km entre zonas de
          // forma exacta en vez de la aproximación anterior (a partes
          // iguales entre las zonas de una etiqueta compuesta "Z2-Z3").
          // null si no se pudo desglosar (ver _desglosarSesionReal) -- en
          // ese caso las pantallas afectadas caen de vuelta a su
          // comportamiento anterior con solo 'zone'.
          if (desgloseFinal) {
            entry.desgloseZonas = desgloseFinal;
          }
  ...
  // Función auxiliar para calcular TSS a partir de distancia y duración
  // reales. Si se dispone del desglose por tramos (ver
  // _desglosarSesionReal) se suma el TSS de CADA tramo por separado,
  // ponderado por su propio factor de intensidad (misma tabla que usa la
  // planificación, _factoresIF) -- mucho más preciso que el método
  // anterior, que aplicaba un único "factor de ritmo" calculado con la
  // media de TODA la sesión: una sesión de tempo en Z3 con calentamiento/
  // enfriamiento en Z1 salía con un TSS más bajo del real, porque la
  // media general se acercaba más al ritmo suave que al ritmo realmente
  // exigente de la parte principal. Y ese TSS insuficiente es justo lo
  // que alimenta la carga aguda/crónica (ACWR, ver calcularCargaACWR) y
  // el tiempo de recuperación estimado (ver calcularRecuperacion), así
  // que ambos arrastraban el mismo error.
  // Si no hay desglose disponible (sesión sin calentamiento/enfriamiento
  // definidos, o resultado no coherente) cae de vuelta al método anterior
  // basado en el ritmo medio de toda la sesión, igual que antes.
  calcularTSSdesdeReal(distanciaKm, duracionMs, sesion, desglose = null) {
    const duracionHoras = duracionMs / (1000 * 60 * 60);
    if (duracionHoras <= 0) return 0;

    if (desglose) {
      const tss = ['calentamiento', 'partePrincipal', 'enfriamiento'].reduce((acc, tramo) => {
        const t = desglose[tramo];
        if (!t || !(t.min > 0)) return acc;
        const ifactor = this._factoresIF[(t.zona || '').split('-')[0]] || 0.7;
        return acc + (t.min * ifactor * ifactor);
      }, 0);
      if (tss > 0) return Math.round(tss);
    }
