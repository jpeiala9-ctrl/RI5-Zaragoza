...
const PlanGenerator = {
  ...
  async crearSesionBasica(tipo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null, esRecuperacion = false) {
    ...
    if (duracionExacta !== null) {
      ...
      if (duracion < min) duracion = min + Math.floor(Math.random() * 6);
    } else {
  ...
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    if (duracion < minSeries) duracion = minSeries + Math.floor(Math.random() * 6);
        ...
        if (sesionLargo && sesionLargo.duracion) {
          ...
          for (const s of semana) {
            if (s.tipo !== 'largo' && s.tipo !== 'descanso' && s.duracion && s.duracion >= largoMinutos) {
              const nuevaDuracion = Math.max(20, largoMinutos - 5);
