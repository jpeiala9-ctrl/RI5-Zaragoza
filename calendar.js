// ============================================================================
// calendar.js - VERSIÓN ÉLITE 2.3
// Fuerza como complemento extra (no resta minutos de carrera)
// ============================================================================
...
const PlanGenerator = {
  ...
  FASES: {
    BASE: { nombre: 'Base', color: '#8AA0B0', intensidad: 0.7, volumen: 1.0 },
    CONSTRUCCION: { nombre: 'Construcción', color: '#9BB5A0', intensidad: 0.85, volumen: 1.15 },
    ESPECIFICA: { nombre: 'Específica', color: '#C9A78B', intensidad: 0.95, volumen: 1.2 },
    PICO: { nombre: 'Pico', color: '#C99BA5', intensidad: 1.0, volumen: 1.1 },
    TAPER: { nombre: 'Taper', color: '#9AA5A5', intensidad: 0.6, volumen: 0.7 }
  },

  // Distribución solo para carrera (sin fuerza)
  DISTRIBUCION_TIPOS: {
    principiante: {
  ...
  // Duración fija de las sesiones de fuerza (minutos)
  DURACION_FUERZA: 40,

  // Ajustar distribución por distancia (sin fuerza)
  ...
  // Cálculo de volumen semanal BASE (minutos) – solo carrera
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // Descarga microciclo 3:1 (excepto en taper)
        const esDescarga = (semanaEnFase % 4 === 0) && fase !== 'TAPER';
        
        let volumenSemana = this.calcularVolumenSemanal(nivelActual, experiencia, objetivo, distancia);
        volumenSemana = Math.round(volumenSemana * this.FASES[fase].volumen);
        volumenSemana = Math.round(volumenSemana * ajusteVolumen);
        volumenSemana = Math.min(720, Math.max(180, volumenSemana));
        if (esDescarga) volumenSemana = Math.round(volumenSemana * 0.7);
        ...
        // Asignar tirada larga al día elegido
        ...
        // Resto de tipos (rodaje, tempo, series) se distribuyen en los días restantes
        const tiposRestantes = [];
        if (minutosPorTipo.rodaje > 0) tiposRestantes.push({ tipo: 'rodaje', minutos: minutosPorTipo.rodaje });
        if (minutosPorTipo.tempo > 0) tiposRestantes.push({ tipo: 'tempo', minutos: minutosPorTipo.tempo });
        if (minutosPorTipo.series > 0) tiposRestantes.push({ tipo: 'series', minutos: minutosPorTipo.series });
        
        const numDiasRestantes = diasDisponibles.length;
        let indiceTipo = 0;
        for (let i = 0; i < numDiasRestantes; i++) {
          const dia = diasDisponibles[i];
          const tipoObj = tiposRestantes[indiceTipo % tiposRestantes.length];
          if (tipoObj) {
            const minutosDia = Math.min(tipoObj.minutos, 90);
            tipoObj.minutos -= minutosDia;
            if (tipoObj.minutos <= 0) indiceTipo++;
            tiposPorDia[dia] = { tipo: tipoObj.tipo, minutos: minutosDia };
          } else {
            tiposPorDia[dia] = { tipo: 'rodaje', minutos: 45 };
          }
        }

        // Ahora añadimos fuerza como sesiones extra (sin restar minutos de carrera)
        // Elegimos 2 días adecuados para fuerza (evitar días de series/tempo intensas si es posible)
        const diasFuerza = this.seleccionarDiasFuerza(tiposPorDia, diaLargo);
        for (const dia of diasFuerza) {
          // Si ya hay una sesión ese día, la dejamos y añadimos fuerza como extra (mismo día, pero duplicado en el calendario? No, solo una sesión por día)
          // Mejor: si ya hay sesión, la fuerza se marca como parte de la sesión (en el detalle se mostrará como complemento)
          // Pero para no complicar, podemos añadir la fuerza como una sesión aparte en el mismo día, pero el calendario solo muestra una casilla por día.
          // Alternativa: incluir la fuerza como parte del detalle de la sesión de ese día (si es rodaje) o crear un día extra (pero el usuario dijo "aparte de las sesiones de carrera", puede ser el mismo día).
          // Lo más limpio: añadir la fuerza como una sesión separada, pero eso duplicaría el día en el calendario. Para evitar duplicados, podemos marcar el día con "R+F" o similar.
          // En esta versión, añadiremos un campo "fuerza" a la sesión existente si ya hay una, o crearemos una sesión de fuerza si el día estaba vacío.
          // Pero lo más sencillo: añadir una sesión de fuerza en días que no tengan sesión de carrera (si hay días libres) o en días de rodaje suave.
          // Si no hay días libres, se añade en un día de rodaje (no en tempo/series).
          if (tiposPorDia[dia]) {
            // Si ya hay sesión de carrera, la marcamos como "con fuerza" en el detalle (añadimos un campo)
            // Pero para simplificar, no modificamos la estructura; la fuerza se verá en el detalle como parte de la sesión.
            // Para que el calendario lo refleje, podríamos cambiar la letra a "R+F" pero es complicado.
            // Mejor: añadir la fuerza como una sesión aparte en el mismo día, pero el calendario actual no soporta dos sesiones por día.
            // Dado que el usuario quiere que sea "aparte", probablemente espera que sea en el mismo día pero como complemento.
            // Por ahora, añadimos la fuerza como parte del detalle de la sesión de ese día.
            // Añadimos un flag para que en el detalle se muestre.
            // Esto requiere modificar la sesión existente.
            // Para no complicar, en esta versión simplemente añadimos la fuerza en días sin sesión de carrera, y si no hay suficientes, en días de rodaje (pero no en tempo/series).
            // Lo haré así: buscar días sin sesión de carrera (descanso) y asignar fuerza allí. Si no hay suficientes, asignar en días de rodaje (preferentemente los de menor duración).
          } else {
            tiposPorDia[dia] = { tipo: 'strength', minutos: this.DURACION_FUERZA };
          }
        }
  ...
  // Selecciona 2 días para fuerza (prioriza días con rodaje suave o descanso)
  seleccionarDiasFuerza(tiposPorDia, diaLargo) {
    const candidatos = [];
    // Días con rodaje (no tempo ni series) y que no sean el día de tirada larga
    for (let dia = 1; dia <= 7; dia++) {
      const sesion = tiposPorDia[dia];
      if (sesion && (sesion.tipo === 'rodaje' || sesion.tipo === 'descanso')) {
        if (dia !== diaLargo) candidatos.push(dia);
      }
    }
    // Si no hay suficientes, añadir días sin sesión asignada (descanso)
    for (let dia = 1; dia <= 7; dia++) {
      if (!tiposPorDia[dia] && candidatos.length < 2) {
        candidatos.push(dia);
      }
    }
    // Si aún faltan, usar días de tempo o series (pero evitando largo)
    for (let dia = 1; dia <= 7; dia++) {
      const sesion = tiposPorDia[dia];
      if (sesion && sesion.tipo !== 'largo' && candidatos.length < 2 && !candidatos.includes(dia)) {
        candidatos.push(dia);
      }
    }
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    ...
    const LIMITES = { rodaje: 90, tempo: 75, series: 120, largo: 210, strength: 45 };
    const MINIMO = { rodaje: 45, tempo: 45, series: 45, largo: 45, strength: 30 };
    ...
    if (!sesionMatriz) {
      ...
      switch(tipo) {
        ...
        case 'strength': nombreBase = 'Fuerza funcional'; duracionBase = 40; break;
      }
    ...
    if (duracionExacta !== null) {
      ...
      const min = (semanaEnFase % 4 === 0 || fase === 'TAPER') ? 35 : MINIMO[tipo] || 45;
    ...
    if (partePrincipal < 5 && tipo !== 'descanso' && tipo !== 'strength') {
    ...
    if (incluirRitmoObjetivo) {
      ...
      switch(tipo) {
        ...
        case 'series':
          ...
          break;
        case 'strength':
          descripcionPrincipal = `${partePrincipal}' entrenamiento de fuerza`;
          pasoPrincipal = {
            icono: '🏋️',
            titulo: 'FUERZA FUNCIONAL',
            accion: `${partePrincipal}' de ejercicios de fuerza (sentadillas, planchas, zancadas, core)`,
    ...
    } else if (tipo === 'strength') {
      objetivo = 'Fortalecimiento muscular y prevención de lesiones';
      porque = 'El trabajo de fuerza complementa el running y mejora la economía de carrera.';
    } else {
    ...
    else if (tipo === 'strength') zonaPrincipal = 'FUERZA';

    const factoresRitmo = { 'Z1': 1.35, 'Z2': 1.25, 'Z3': 1.15, 'Z4': 1.05, 'Z5': 0.95 };
    const ritmoObjetivo = tipo === 'strength' ? '—' : Utils.formatR(ritmoBase * (factoresRitmo[zonaPrincipal] || 1.25));
    
    let fcObjetivo = '';
    if (tipo === 'strength') {
      fcObjetivo = 'No aplica (esfuerzo moderado)';
    } else {
      const fcRangos = {
    ...
    if (tipo === 'strength') zonaMostrada = 'FUERZA';
  ...
  obtenerSensacion(tipo, fase) {
    if (tipo === 'strength') return 'Esfuerzo muscular controlado';
    const sensaciones = {
  ...
  // ==========================================================================
  // GUARDADO Y VISUALIZACIÓN (sin cambios)
  // ==========================================================================
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      else if (sesion.tipo === 'strength') icono = "🏋️";
...
console.log('✅ PlanGenerator v16.3 - Fuerza como complemento extra, no resta minutos de carrera');