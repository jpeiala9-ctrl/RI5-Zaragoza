...
const PlanGenerator = {
  ...
  _desglosarSesion(sesion, totalMin, totalKm) {
    ...
    // sesion.detalle.segundosEsfuerzo (solo presente en las estructuras
    // avanzadas: pirámide/rotas/fartlek/cuestas, ver
    // crearSesionAvanzadaSeries) es el tiempo PLANIFICADO que de verdad se
    // corre al ritmo objetivo, sin contar las recuperaciones. Con la misma
    // técnica que ya se usa para calentamiento/enfriamiento -- escalar la
    // duración PLANIFICADA a la duración REAL de la parte principal --
    // se separa el tiempo/km de recuperación (asumido a ritmo Z2, igual
    // que el resto de la app) del de esfuerzo real, y la zona final de la
    // sesión pasa a detectarse sobre el esfuerzo real, no sobre la media.
    if (sesion?.tipo === 'series' && det.segundosEsfuerzo > 0 && duracionPlan > 0) {
      ...
      if (plannedMainSec > 0 && plannedRecSec > 0) {
        ...
        const recMinEstimado = (plannedRecSec / 60) * proporcionMain;
        const ritmoZ2MinPerKm = this._ritmoToSeg(this.obtenerRitmoParaZona('Z2')) / 60;
        if (ritmoZ2MinPerKm && isFinite(ritmoZ2MinPerKm) && recMinEstimado > 0 && recMinEstimado < mainMin) {
          const recKmEstimado = recMinEstimado / ritmoZ2MinPerKm;
          const esfuerzoMin = mainMin - recMinEstimado;
          const esfuerzoKm = mainKm - recKmEstimado;
          // Misma comprobación de coherencia física que arriba: el
          // esfuerzo no puede salir con un ritmo más lento que la propia
          // recuperación asumida (Z2) ni que la media de la parte
          // principal -- si sale así, la calibración de zonas del usuario
          // no encaja con esta sesión concreta y es más seguro no partir
          // el tramo (se deja el bloque único ya calculado arriba).
          if (esfuerzoKm > 0 && esfuerzoMin > 0) {
            const paceEsfuerzo = esfuerzoMin / esfuerzoKm;
            if (paceEsfuerzo <= paceMain * 1.001 && paceEsfuerzo <= ritmoZ2MinPerKm * 1.001) {
              const zonaEsfuerzo = this._detectarZonaPorRitmo(paceEsfuerzo);
              resultado.partePrincipal = { min: esfuerzoMin, km: esfuerzoKm, zona: zonaEsfuerzo ? zonaEsfuerzo[0] : 'Z4' };
              resultado.recuperacion = { min: recMinEstimado, km: recKmEstimado, zona: 'Z2' };
            }
          }
        }
      }
