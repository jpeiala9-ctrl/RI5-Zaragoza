// ==================== calendar.js - VERSIÓN FINAL CON MAPA AÉREO IGN Y MARCADO NARANJA ====================
// Versión: 2.60 - Mapa aéreo IGN + respaldo OSM, marcado forzado en naranja (mismo comportamiento que verde)
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== MARCAR SESIÓN FORZADA (NARANJA) - MISMO COMPORTAMIENTO QUE LA VERDE ==========
  async marcarSesionForzada(diaIndex, realizada, routeData = null) {
    if (!AppState.currentUserId || !AppState.planActualId) return;
    try {
      const planRef = firebaseServices.db
        .collection('users')
      ...
      // Marcar como realizada Y forzada
      ...
      // Actualizar celda del calendario con la clase correspondiente
      const celda = document.querySelector(`.calendario-dia[data-index="${diaIndex}"]`);
      if (celda) {
        celda.classList.add('realizado-forzado');
        celda.classList.remove('realizado'); // Quita verde si lo tenía
      }
      
      // Publicar en el muro (igual que con verde)
      const planDoc = await planRef.get();
      const planCompleto = planDoc.data();
      ...
      Utils.showToast('✅ Sesión marcada como completada (manual)', 'success');
    } catch (error) {
  ...
  // ========== FUNCIÓN ABRIR DETALLE CON GPS Y MAPA AÉREO (IGNA MAPA) ==========
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // ========== CONTROLES DE GPS CON CUENTA ATRÁS Y MAPA AÉREO ==========
      if (!realizada) {
        ...
        startBtn.onclick = async () => {
          ...
          // Mostrar cuenta atrás y obtener posición precisa
          const { position, cancelled } = await this._mostrarCuentaAtrasGPS();
          if (cancelled) {
            Utils.showToast('Grabación cancelada', 'info');
            return;
          }
          
          // Crear contenedor del mapa con altura fija
          ...
          // Crear mapa con capa aérea IGN (y respaldo OpenStreetMap si falla)
          const lat = position ? position.coords.latitude : 40.416775;
          const lng = position ? position.coords.longitude : -3.703790;
          const map = L.map(mapContainer.id).setView([lat, lng], 16);
          
          // Intentar usar ortofoto IGN, si falla, usar OpenStreetMap
          const ignLayer = L.tileLayer('https://www.ign.es/wmts/mapa-raster?request=GetTile&service=WMTS&version=1.0.0&layer=OI.OrthoimageCoverage&style=default&tilematrixset=GoogleMapsCompatible&tilematrix={z}&tilerow={y}&tilecol={x}&format=image/jpeg', {
            attribution: '© IGN España (CC BY 4.0)',
            maxZoom: 19
          });
          ...
          ignLayer.addTo(map);
          // Si IGN falla (evento de error), cambiar a OSM
          ...
          stopBtn.onclick = async () => {
            ...
            if (porcentaje <= 10) {
              ...
              forceBtn.onclick = async () => {
                await this.marcarSesionForzada(diaIndex, true, lastRouteData);
                location.reload();
              };
      ...
      // Mostrar ruta guardada si existe (con mapa aéreo IGN)
  ...
  // Mostrar ruta con mapa aéreo IGN (con respaldo OSM)
  async _displayRoute(containerId, points) {
    ...
    const map = L.map(containerId).fitBounds(latlngs, { padding: [30, 30] });
  ...
  // Cuenta atrás con obtención de posición precisa
  _mostrarCuentaAtrasGPS() {
    return new Promise((resolve) => {
      const overlay = document.createElement('div');
      overlay.id = 'gpsCountdownOverlay';
      ...
      countdownDisplay.style.color = 'var(--accent-yellow)';
      
      const message = document.createElement('div');
      message.style.marginBottom = '20px';
      message.style.fontSize = '18px';
      message.innerText = 'Obteniendo posición GPS precisa...';
      ...
      watchId = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          if (accuracy < bestAccuracy) {
            ...
            bestPosition = pos;
            precisionSpan.innerText = `Precisión actual: ${Math.round(accuracy)} m (mejorando...)`;
          } else {
        ...
        (err) => {
          ...
          precisionSpan.innerText = `⚠️ Error: ${err.message}. Usando última posición.`;
        },
        { enableHighAccuracy: true, maximumAge: 0, timeout: 10000 }
      );
      ...
      setTimeout(() => {
        if (!bestPosition && !cancelled) {
          if (watchId) navigator.geolocation.clearWatch(watchId);
          if (intervalId) clearInterval(intervalId);
          overlay.remove();
          resolve({ position: null, cancelled: false });
        }
      }, 6000);
    });
  },
  ...
  async marcarSesionRealizada(diaIndex, realizada) {
    ...
    try {
      ...
      // Si se desmarca, eliminar también la flag forzada
      if (!realizada && AppState.sesionesForzadas?.[diaIndex]) {
        await planRef.update({ [`sesionesForzadas.${diaIndex}`]: false });
        delete AppState.sesionesForzadas[diaIndex];
      }
      ...
      if (realizada) {
          ...
          if (window.Gamification) {
            try {
              ...
              if (newLevel !== oldLevel) {
                ...
                if (Gamification.clearCache) {
                  await Gamification.clearCache(AppState.currentUserId);
                }
                ...
                if (document.getElementById('tab-amigos').classList.contains('active') && window.Friends) {
                  ...
                  if (activeAmigosTab && activeAmigosTab.textContent.includes('BUSCAR')) {
        ...
        if (window.Gamification && sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          try {
            ...
            console.error('Error revirtiendo gamificación:', e);
          }
        }
      }
...
console.log('✅ PlanGenerator v2.60 - Mapa aéreo IGN con respaldo OSM, marcado forzado en naranja (mismo comportamiento que verde)');