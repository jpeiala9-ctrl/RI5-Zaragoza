...
// Versión: 2.67 - GPS alta precisión (≤5m), sin popup "tu ubicación", cuenta atrás 5s
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== ABRIR DETALLE CON GPS DE ALTA PRECISIÓN Y SIN POPUP ==========
    ...
    return new Promise((resolve) => {
      ...
      const updateCountdown = () => {
        ...
        if (countdown === 0) {
          ...
          overlay.remove();
          if (bestAccuracy > 5 && bestPosition) {
            Utils.confirm('Precisión GPS baja', `La mejor precisión obtenida es ${Math.round(bestAccuracy)} metros (se recomienda ≤5m). ¿Continuar de todas formas?`)
              .then(continuar => {
                if (continuar) resolve({ position: bestPosition, cancelled: false });
                else resolve({ position: null, cancelled: true });
              });
          } else {
...
console.log('✅ PlanGenerator v2.67 - GPS alta precisión (≤5m), sin popup, mapa limpio');