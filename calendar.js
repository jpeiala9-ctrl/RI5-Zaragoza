// ==================== calendar.js - VERSIÓN COMPLETA (PANTALLA NEGRA + SUAVIZADO GPS) ====================
// GPS con pantalla negra durante grabación, suavizado de puntos, y mapa solo al final
// ============================================================================
...
const PlanGenerator = {
  ...
  _smoothPoints(rawPoints) {
    ...
    // 1. Eliminar puntos con velocidad instantánea anómala (> 10 m/s = 36 km/h, o < 0.5 m/s)
    const filtered = [];
    for (let i = 0; i < rawPoints.length; i++) {
      ...
      // Si la velocidad es muy alta o muy baja, descartar este punto
      if (speedPrev > 12 || speedPrev < 0.3 || speedNext > 12 || speedNext < 0.3) {
        console.log(`🗑️ Punto ${i} descartado por velocidad anómala (${speedPrev.toFixed(1)} m/s, ${speedNext.toFixed(1)} m/s)`);
        continue;
      }
      filtered.push(curr);
    }
    ...
    // 2. Aplicar media móvil (ventana de 3) a lat/lng
    const smoothed = [];
  ...
  // ========== FUNCIONES DE REDONDEO Y CÁLCULOS ==========
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      if (!realizada) {
        ...
        // Crear overlay de grabación (pantalla negra)
        ...
        const startRecording = async () => {
          try {
            // Pedir posición inicial con buena precisión
            ...
            // Timer para actualizar tiempo aunque no lleguen puntos
        ...
        stopButton.onclick = async () => {
          ...
          const smoothed = this._smoothPoints(rawPoints);
          console.log(`Puntos originales: ${rawPoints.length}, suavizados: ${smoothed.length}`);
          
          // Calcular distancia real con puntos suavizados
          ...
          // Marcar sesión automáticamente si la diferencia es <=10%
          if (porcentaje <= 10) {
            autoMarked = true;
            await this.marcarSesionRealizada(diaIndex, true, lastRouteData);
            const celda = document.querySelector(`.calendario-dia[data-index="${diaIndex}"]`);
...
console.log('✅ PlanGenerator v2.79 - Pantalla negra GPS y suavizado de puntos');