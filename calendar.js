// ==================== calendar.js - VERSIÓN ESTABLE CON MAPA OSM Y BOTÓN NARANJA SIN RECARGA ====================
// Versión: 2.62 - Mapa OpenStreetMap (siempre visible), cuenta atrás visual, marcado forzado sin recarga
// ============================================================================
...
const PlanGenerator = {
  ...
  async marcarSesionForzada(diaIndex, realizada, routeData = null, skipToast = false) {
    ...
    try {
      ...
      // Actualizar celda del calendario sin recargar
      ...
      if (!skipToast) Utils.showToast('🟠 Sesión marcada manualmente', 'success');
    } catch (error) {
  ...
  // ========== FUNCIÓN ABRIR DETALLE CON MAPA OSM (SIEMPRE VISIBLE) ==========
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // ========== CONTROLES DE GPS CON MAPA OSM (SIEMPRE VISIBLE) ==========
      if (!realizada) {
        ...
        startBtn.innerHTML = 'COMENZAR SESIÓN';
        ...
        startBtn.onclick = async () => {
          ...
          // Usar OpenStreetMap (siempre funciona)
          ...
          stopBtn.onclick = async () => {
            ...
            if (porcentaje <= 10) {
              ...
              cerrarModalSesion();
              // Recargar la página para actualizar el calendario (opcional)
              // location.reload(); // Comentado para evitar recarga, solo se actualiza la celda
              // Actualizar la celda manualmente
              const celda = document.querySelector(`.calendario-dia[data-index="${diaIndex}"]`);
              if (celda) {
              ...
              if (!forceButtonCreated) {
                ...
                forceBtn.onclick = async () => {
                  ...
                  cerrarModalSesion();
                  // No recargar, la celda ya se actualizó en marcarSesionForzada
                };
      ...
      // Mostrar ruta guardada si existe (con OSM)
...
console.log('✅ PlanGenerator v2.62 - Mapa OSM siempre visible, marcado forzado sin recarga');