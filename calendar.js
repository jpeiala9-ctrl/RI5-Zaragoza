    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        if (numDiasRodaje > 0 && minutosRodajeTotal > 0) {
          ...
          // -- p.ej. "81, 81, 81, 81"): cuando el volumen semanal exige más
          // rodaje del que cabe por día dentro del tope de este tipo/fase
          // (típico en nivel avanzado, donde el tope de rodaje en
          // CONSTRUCCION es justo 81'), el 'valor' de CADA día tras aplicar
          // la variación seguía superando maxRodaje, así que el
          // Math.min(maxRodaje,...) de abajo recortaba TODOS los días
          // exactamente al mismo número -- deshaciendo la variación que se
          // acababa de calcular una línea antes. Un entrenador real nunca
          // repite la tirada fácil con el minuto exacto sesión tras sesión;
          // cuando toca recortar al tope, se recorta a un punto variable
          // (93%-100% del tope) en vez de siempre al tope exacto, así el
          // recorte sigue respetando el límite de seguridad del tipo/fase
          // pero no produce sesiones clónicas.
          const clampRodaje = (valor) => {
            if (valor > maxRodaje) return Math.round(maxRodaje * (0.93 + Math.random() * 0.07));
            return Math.max(minRodaje, valor);
          };
          let valores = [];
          let sumaValores = 0;
