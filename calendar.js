...
const PlanGenerator = {
  ...
  abrirModalDatosReales(sesion) {
    return new Promise((resolve) => {
      ...
      modal.style.cssText = `
        ...
        border-radius:20px; max-width:380px; width:90%; padding:24px;
      ...
      modal.innerHTML = `
        ...
        <div id="datosRealesTipoBox" style="cursor:pointer; background:var(--bg-primary); border:1px solid var(--border-color); border-radius:12px; padding:10px 12px; margin-bottom:8px; display:flex; align-items:center; justify-content:center; gap:8px;">
          ...
          <span style="font-size:11px; color:var(--text-secondary);">✏️ cambiar</span>
        </div>
        <div id="datosRealesTipoLista" style="display:none; flex-wrap:wrap; gap:6px; justify-content:center; margin-bottom:10px;">
          ${opcionesTipo.map(o => `<button type="button" class="datos-reales-tipo-opcion" data-tipo="${o.tipo}" style="background:var(--bg-primary); border:1px solid var(--border-color); color:var(--text-primary); padding:6px 12px; border-radius:20px; cursor:pointer; font-size:12px;">${o.icono} ${o.label}</button>`).join('')}
        </div>
        ...
        <div style="text-align:left; margin-bottom:14px;">
          <label style="display:block; font-size:12px; color:var(--text-secondary); margin-bottom:4px;">📏 Kilómetros</label>
          <input type="number" id="datosRealesKm" inputmode="decimal" step="0.01" min="0" value="${distanciaDefecto ? distanciaDefecto.toFixed(2) : ''}" style="width:100%; padding:10px; border-radius:10px; background:var(--bg-primary); border:1px solid var(--border-color); color:var(--text-primary); font-size:16px; box-sizing:border-box;">
        </div>
        <div style="display:flex; gap:10px; margin-bottom:14px;">
          <div style="flex:1; text-align:left;">
            <label style="display:block; font-size:12px; color:var(--text-secondary); margin-bottom:4px;">⏱️ Minutos</label>
            <input type="number" id="datosRealesMin" inputmode="numeric" min="0" value="${Math.floor(duracionDefecto)}" style="width:100%; padding:10px; border-radius:10px; background:var(--bg-primary); border:1px solid var(--border-color); color:var(--text-primary); font-size:16px; box-sizing:border-box;">
          </div>
          <div style="flex:1; text-align:left;">
            <label style="display:block; font-size:12px; color:var(--text-secondary); margin-bottom:4px;">Segundos</label>
            <input type="number" id="datosRealesSeg" inputmode="numeric" min="0" max="59" value="0" style="width:100%; padding:10px; border-radius:10px; background:var(--bg-primary); border:1px solid var(--border-color); color:var(--text-primary); font-size:16px; box-sizing:border-box;">
          </div>
        </div>
        <div style="background:var(--bg-primary); border:1px solid var(--border-color); border-radius:12px; padding:12px; margin-bottom:20px;">
      ...
      const tipoLista = document.getElementById('datosRealesTipoLista');
      ...
      tipoBox.addEventListener('click', () => {
        tipoLista.style.display = tipoLista.style.display === 'none' ? 'flex' : 'none';
      });
      tipoLista.querySelectorAll('.datos-reales-tipo-opcion').forEach(btn => {
      ...
      const recalcular = () => {
        ...
        if (km > 0 && totalMin > 0) {
          ...
          zonaEl.innerHTML = zona ? `Zona de trabajo: <strong>${Utils.escapeHTML(zona[0])}</strong> · ${Utils.escapeHTML(zona[1])}` : '&nbsp;';
        } else {
          ritmoEl.textContent = '--:--';
          zonaEl.innerHTML = '&nbsp;';
        }
  ...
  // Tipos de entrenamiento que existen de verdad en el plan actual
  // (deduplicados, en el orden en que aparecen), para ofrecer solo
  // opciones que tienen sentido dentro de ESTE plan concreto en vez de
  // una lista genérica fija. Si por lo que sea el calendario aún no se ha
  // renderizado y no hay sesiones cargadas, cae a los 4 tipos base.
  _tiposDisponiblesEnPlan() {
    const sesiones = this._sesionesActuales || [];
    const vistos = new Set();
    const opciones = [];
    sesiones.forEach(s => {
      if (!s || !s.tipo || s.tipo === 'descanso' || s.tipo === 'strength') return;
      if (vistos.has(s.tipo)) return;
      vistos.add(s.tipo);
      opciones.push({ tipo: s.tipo, ...this._infoTipo(s.tipo) });
    });
    if (opciones.length === 0) {
      ['rodaje', 'tempo', 'series', 'largo'].forEach(t => opciones.push({ tipo: t, ...this._infoTipo(t) }));
    }
    return opciones;
  },
