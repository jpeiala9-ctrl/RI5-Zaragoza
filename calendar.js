...
const PlanGenerator = {
  ...
  // usuario solo confirma o corrige km y tiempo -- sin selector de tipo,
  // que se quitó por simplicidad -- y esos son los que se guardan. Si el
  // tiempo real difiere del programado (p.ej. programada a 45' y se
  // introducen 55'), esa diferencia se entiende SIEMPRE como parte
  // principal, al ritmo resultante de los km/tiempo introducidos: ver
  // calcularTSSdesdeReal, que calcula la carga sobre TODA la duración
  // real (no solo sobre los minutos planificados) usando la zona real
  // detectada aquí mismo. Devuelve {km, ms} o null si se cancela.
  abrirModalDatosReales(sesion) {
    return new Promise((resolve) => {
      ...
      modal.innerHTML = `
        ...
        <p style="margin:0 0 14px 0; font-size:12px; color:var(--text-secondary);">Confirma o corrige los km y el tiempo reales. Si has entrenado más o menos de lo previsto, esa diferencia cuenta como parte principal, al ritmo que indiques.</p>
        <div style="background:var(--bg-primary); border:1px solid var(--border-color); border-radius:12px; padding:10px 12px; margin-bottom:14px; display:flex; align-items:center; justify-content:center; gap:8px;">
          <span style="font-size:16px;">${infoTipoOriginal.icono}</span>
          <strong style="font-size:14px; color:var(--text-primary);">${Utils.escapeHTML(nombreCabecera)}</strong>
        </div>
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      checkbox.onchange = async (e) => {
        ...
        if (marcando && sesion && sesion.tipo !== 'descanso' && sesion.tipo !== 'strength') {
          ...
          // km y el tiempo reales antes de guardar nada -- solo esos dos
          // datos, sin selector de tipo. Si el tiempo real difiere del
          // programado (45' programados, 55' reales), esa diferencia se
          // entiende siempre como parte principal: calcularTSSdesdeReal
          // calcula la carga sobre TODA la duración real, al ritmo real
          // introducido aquí, en vez de anclarla a los minutos que decía
          // el plan. Las sesiones iniciadas con GPS no pasan por aquí: el
          // GPS ya es autosuficiente y va directo por gps-tracker.js con
          // sus propios datos medidos por el track (solo se afinan unos
          // metros de distancia ahí, no el tiempo).
          const datos = await this.abrirModalDatosReales(sesion);
          if (!datos) {
            checkbox.checked = false;
            return;
          }
          await this.marcarSesionRealizada(diaIndex, true, datos.km, datos.ms);
        } else {
  ...
  // reales. Antes: tssBase = sesion.duracion (los minutos PLANIFICADOS,
  // fijos) escalado por un factor de ritmo real vs ritmo base -- así, si
  // programabas 45' y corrías más tiempo (55') al mismo esfuerzo, el TSS
  // se quedaba anclado a esos 45' de todas formas, sin contar la carga
  // real de los 10' de más. Ahora: la duración REAL completa cuenta como
  // parte principal, a la zona/ritmo real que resulta de los km y el
  // tiempo introducidos (misma detección que ya usa el modal de "datos
  // de la sesión" para mostrar la zona en pantalla) -- así que 10' de
  // más a un ritmo similar sí añaden carga real, en vez de quedarse
  // fuera solo porque el plan decía menos tiempo.
  calcularTSSdesdeReal(distanciaKm, duracionMs, sesion) {
    ...
    const zonaPlanCod = (sesion?.detalle?.zona || '').split('-')[0].trim();
    const ifactorPlan = factoresIF[zonaPlanCod] || 0.7;

    // Zona REAL entrenada, según el ritmo medio real (km/tiempo
    // introducidos). Si no hay km válidos, se cae de vuelta a la zona
    // planificada para no dejar el cálculo sin intensidad alguna.
    const ritmoRealMinPerKm = distanciaKm > 0 ? duracionMin / distanciaKm : 0;
    const zonaRealArr = ritmoRealMinPerKm > 0 ? this._detectarZonaPorRitmo(ritmoRealMinPerKm) : null;
    const ifactorReal = zonaRealArr ? (factoresIF[zonaRealArr[0]] || ifactorPlan) : ifactorPlan;

    // factorIntensidad de la semana/fase actual: se recupera a partir de
    // lo que ya se estimó al generar el plan (tssEstimada = duracionPlan
    // · ifactorPlan² · factorIntensidad) en vez de recalcular en qué
    // punto del ondulatorio 3:1 estamos -- así una semana de descarga
    // sigue pesando como descarga también en la carga REAL registrada.
    const duracionPlanMin = sesion?.duracion || duracionMin;
    const tssEstimadaPlan = sesion?.detalle?.tssEstimada;
    const factorIntensidad = (tssEstimadaPlan && duracionPlanMin > 0 && ifactorPlan > 0)
      ? tssEstimadaPlan / (duracionPlanMin * ifactorPlan * ifactorPlan)
      : 1.0;

    return Math.round(duracionMin * ifactorReal * ifactorReal * factorIntensidad);
  },
