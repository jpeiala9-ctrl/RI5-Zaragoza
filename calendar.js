// ==================== calendar.js - VERSIÓN FINAL 2.15 (CON MEJORAS PROFESIONALES) ====================
// - Límites de duración por distancia (2k,5k,10k,medio,maraton)
// - Volumen base ajustado para distancias cortas
// - Control de repetición de sesiones (evita misma sesión seguida)
// - Descripciones de fuerza variadas
// ============================================================================
...
const PlanGenerator = {
  ...
  // Array de descripciones de fuerza (variantes)
  ...
  // Control de repetición: almacena la última sesión usada por tipo y distancia
  ultimaSesionPorTipo: {},

  getMaximosPorTipo(tipo, nivel, fase, distancia) {
    // Límites máximos base (minutos)
    const maxBase = {
    ...
    // Factor de fase (igual que antes)
    const factorFase = {
      BASE: 0.8,
      CONSTRUCCION: 0.9,
      ESPECIFICA: 1.0,
    ...
    // Ajustes por distancia (máximos más restrictivos para distancias cortas)
    if (distancia === '2k') {
    ...
    // Ajustes de mínimos (ya se hace después en la creación)
    return max;
  },
  ...
  calcularVolumenSemanal(nivel, experiencia, objetivo, distancia) {
    ...
    // Ajuste por distancia (2k y 5k requieren menos volumen)
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // Reiniciar control de repetición por tipo para este plan
  ...
  agregarFuerzaASesion(sesion) {
    ...
    // Elegir una descripción de fuerza aleatoria
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    ...
    if (dbTipo?.length) {
      // Control de repetición: evitar elegir la misma sesión que la última vez para este tipo
      const ultima = this.ultimaSesionPorTipo[tipo];
      let candidatas = dbTipo;
      if (ultima) {
        candidatas = dbTipo.filter(s => s.nombre !== ultima);
      }
      if (candidatas.length === 0) candidatas = dbTipo; // fallback
...
console.log('✅ PlanGenerator v2.15 - Con mejoras profesionales: límites por distancia, volumen ajustado, control de repetición y fuerza variada');