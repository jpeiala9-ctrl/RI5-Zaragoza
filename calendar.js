...
// Versión: 3.10 - DEFINITIVA CON FC POR ZONA EN CADA PASO
//
// CARACTERÍSTICAS:
...
// - Visualización profesional de cada sesión con tiempo, distancia, TSS y FC por paso
...
const PlanGenerator = {
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, faseActual, semanaEnFase, libreriaPerfil, datos) {
    ...
    // Calcular ritmos y FC (para el resumen general, aunque luego se sobreescribirá por paso)
  ...
  // ==========================================================================
  // DETALLE DE SESIÓN (MODAL) - VERSIÓN PROFESIONAL CON FC POR ZONA
  // ==========================================================================
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      const pasosHTML = this.generarPasosSesion(metricas.pasos);
  ...
  // ==========================================================================
  // FUNCIONES AUXILIARES PARA EL DETALLE PROFESIONAL MEJORADAS
  // ==========================================================================
  ...
  calcularMetricasSesion(sesion) {
    ...
    // Factores de intensidad para TSS
    const factoresIF = {
      'Z1': 0.6,
      'Z2': 0.7,
      'Z3': 0.85,
    ...
    // Rangos de FC por zona (porcentajes del umbral)
    const fcRangos = {
      'Z1': [Math.round(fcUmbral * 0.60), Math.round(fcUmbral * 0.70)],
      'Z2': [Math.round(fcUmbral * 0.70), Math.round(fcUmbral * 0.80)],
      'Z3': [Math.round(fcUmbral * 0.80), Math.round(fcUmbral * 0.90)],
      'Z4': [Math.round(fcUmbral * 0.90), Math.round(fcUmbral * 0.95)],
      'Z5': [Math.round(fcUmbral * 0.95), Math.round(fcUmbral * 1.02)],
      'Z6': [Math.round(fcUmbral * 1.02), Math.round(fcUmbral * 1.10)]
    };
    ...
    partes.forEach(parte => {
      ...
      if (!matchTiempo) return; // ignorar partes sin tiempo
      const minutos = parseInt(matchTiempo[1]);

      const factorRitmo = factoresRitmo[zona] || 1.25;
      const ritmoMin = ritmoBase * factorRitmo;
      const ritmoStr = Utils.formatR(ritmoMin);
      const distanciaPaso = minutos / ritmoMin;
      ...
      let fcRange = '';
      if (zona === 'Z5' || zona === 'Z6') {
        // Para Z5 y Z6, mostrar como "> X" (límite inferior)
  ...
  generarPasosSesion(pasosCalculados) {
    if (!pasosCalculados || pasosCalculados.length === 0) {
      return '<p>No hay detalles de pasos</p>';
    }

    let html = '';
    pasosCalculados.forEach((paso, index) => {
      // Iconos fijos para mantener consistencia
      const iconoDuracion = '🔴';
      const iconoFC = '🟠';
      const iconoZona = '☑';
      ...
      html += `
        <div class="paso">
          <div class="paso-titulo">${index + 1}. ${paso.titulo}</div>
          <div class="paso-detalle">
            <span>${iconoDuracion} ${paso.minutos} min</span>
            <span>${iconoFC} ${paso.fc}</span>
            <span>${iconoZona} ${paso.zona}</span>
          </div>
          <div class="paso-ritmo">🎯 Ritmo: ${paso.ritmo}</div>
          <div class="paso-distancia">📏 ${paso.distancia.toFixed(2)} km</div>
        </div>
      `;
    });
...
console.log('✅ PlanGenerator v3.10 - FC por zona en cada paso');