// ==================== calendar.js - VERSIÓN COMPLETA 2.56 ====================
// Generación de plan, fases, sesiones, marcado en verde/naranja, integración GPS
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== RENDERIZADO DE CALENDARIO (CORREGIDO PARA NARANJA) ==========
  renderizarPagina(sesiones, semanasTotales) {
    ...
    for (let pos = 0; pos < celdas.length; pos++) {
      ...
      const estado = AppState.sesionesRealizadas?.[sesion.diaGlobal];
      let realizadaClase = '';
      if (estado && estado.realizada) {
        if (estado.tipo === 'parcial') {
          realizadaClase = 'realizado-parcial';
        } else {
          realizadaClase = 'realizado';
        }
      } else if (estado && estado === true) {
        realizadaClase = 'realizado';
      }
      let faseColor = '';
      ...
      html += `<div class="calendario-dia ${sesion.color} ${realizadaClase}" data-index="${sesion.diaGlobal}"${faseIndicator}>${contenidoHtml}</div>`;
    }
    grid.innerHTML = html;
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      // === BOTÓN GPS: iniciar sesión con grabación de track en vivo ===
      if (window.GPSTracker) {
        ...
        // Pasar el planId como tercer parámetro
        gpsBtn.addEventListener('click', () => GPSTracker.iniciar(sesion, diaIndex, AppState.planActualId));
        descripcion.appendChild(gpsBtn);
      }
      checkboxContainer.style.display = 'flex';
      const estadoActual = AppState.sesionesRealizadas?.[diaIndex];
      const realizada = estadoActual && estadoActual.realizada === true;
      const tipoActual = estadoActual?.tipo || 'completo';
      checkbox.checked = realizada;
      const existingSpan = checkbox.parentNode.querySelector('.parcial-text');
      if (existingSpan) existingSpan.remove();
      if (realizada && tipoActual === 'parcial') {
        const parcialText = document.createElement('span');
        parcialText.className = 'parcial-text';
        parcialText.textContent = ' (parcial)';
        parcialText.style.color = '#e67e22';
        parcialText.style.fontSize = '12px';
        parcialText.style.marginLeft = '8px';
        checkbox.parentNode.appendChild(parcialText);
      }
      checkbox.onchange = async (e) => {
        const nuevaRealizada = e.target.checked;
        const nuevoTipo = nuevaRealizada ? 'completo' : null;
        await this.marcarSesionRealizada(diaIndex, nuevaRealizada, nuevoTipo);
      };
  ...
  // ==================== FUNCIÓN PRINCIPAL DE MARCADO (ACTUALIZADA) ====================
  async marcarSesionRealizada(diaIndex, realizada, tipo = 'completo', trackPoints = null, gpsDistance = null, silent = false) {
    if (!AppState.currentUserId || !AppState.planActualId) return;
    try {
      const planRef = firebaseServices.db
        .collection('users')
      ...
      let estado = null;
      if (realizada) {
        estado = { realizada: true, tipo: tipo };
      }
      
      await planRef.update({ [`sesionesRealizadas.${diaIndex}`]: estado });
      
      if (!AppState.sesionesRealizadas) AppState.sesionesRealizadas = {};
      AppState.sesionesRealizadas[diaIndex] = estado;
      
      const celda = document.querySelector(`.calendario-dia[data-index="${diaIndex}"]`);
      if (celda) {
        celda.classList.remove('realizado', 'realizado-parcial');
        if (realizada && tipo === 'completo') {
          celda.classList.add('realizado');
        } else if (realizada && tipo === 'parcial') {
          celda.classList.add('realizado-parcial');
        }
      }
      ...
      const tieneGPS = (trackPoints !== null && trackPoints.length > 0);
      const debePublicar = (realizada && (tieneGPS || tipo === 'completo'));
      ...
      if (debePublicar) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          const metricas = this.calcularMetricasSesion(sesion);
          const distancia = (gpsDistance !== null && gpsDistance > 0) ? gpsDistance : (isFinite(metricas.distanciaTotal) ? metricas.distanciaTotal : 0);
          const tss = isFinite(metricas.tssTotal) ? metricas.tssTotal : 0;
          
          let ritmoMedio = null;
          if (tieneGPS && gpsDistance > 0 && sesion.duracion) {
            const ritmoSeg = (sesion.duracion * 60) / gpsDistance;
            const mm = Math.floor(ritmoSeg / 60);
            const ss = Math.floor(ritmoSeg % 60);
            ritmoMedio = `${mm}:${ss.toString().padStart(2,'0')}`;
          }
          ...
          const entry = {
            ...
            hasGPS: tieneGPS,
            trackPoints: tieneGPS ? trackPoints : [],
            gpsDistanceKm: tieneGPS ? distancia : null,
            gpsDurationMs: null,
            tipoMarcado: tipo,
            ritmoMedio: ritmoMedio,
            zapatillas: userData.profile?.zapatillas || 'No especificadas',
            firstPointLat: (tieneGPS && trackPoints.length > 0) ? trackPoints[0].lat : null,
            firstPointLng: (tieneGPS && trackPoints.length > 0) ? trackPoints[0].lng : null
          };
          ...
          if (window.Gamification && tipo === 'completo') {
        ...
        const wallEntryId = planCompleto?.wallEntryId?.[diaIndex];
        if (wallEntryId) {
      ...
      if (!silent) {
        Utils.showToast(realizada ? (tipo === 'completo' ? '✅ Sesión completada' : '🟠 Sesión parcialmente completada') : '📝 Sesión desmarcada', 'success');
      }
    } catch (error) {
      console.error('Error marcando sesión:', error);
      if (!silent) Utils.showToast('Error al marcar la sesión', 'error');
    }
  },
...
console.log('✅ PlanGenerator v2.56 - Marcado naranja en calendario, GPS con planId, muro con datos completos');