...
// Versión: 5.6 - Mejora en elección de día largo (prioriza que el día siguiente no esté marcado)
//
// CARACTERÍSTICAS:
...
// - Día de tirada larga optimizado: elige día cuyo día siguiente NO esté marcado si es posible
...
// - Al marcar una sesión como realizada, se publica en el feed de amigos (sin foto)
// - VALIDACIÓN ADICIONAL: las series siempre tienen zona ≥ Z4, tempo zona Z3-Z4, etc.
// ============================================================================
...
const PlanGenerator = {
  ...
  // ==========================================================================
  // GENERACIÓN PRINCIPAL DEL PLAN (con mejora en día largo)
  // ==========================================================================
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // --- MEJORA: Manejo del día de tirada larga (con opción automática) ---
      let diaLargoInput = document.getElementById("diaLargo").value;
      let diaLargo;

      // Primero obtenemos los días de entreno (necesarios para la opción automática)
      ...
      if (diaLargoInput === 'auto') {
        // Elegir automáticamente el mejor día (priorizando que el día siguiente NO esté marcado)
      ...
      // Verificar que el día largo esté en días de entreno
      if (!diasEntreno.includes(diaLargo)) {
        // No preguntamos, lo añadimos automáticamente para garantizar coherencia
  ...
  // Prioriza días cuyo día siguiente NO esté marcado como entrenamiento.
  // Si no existe, prioriza fin de semana o último día.
  // ==========================================================================
  elegirDiaLargoOptimo(diasEntreno) {
    // 1. Buscar un día marcado cuyo día siguiente (día+1, con wrap domingo->lunes) NO esté marcado
    for (let dia of diasEntreno) {
      let diaSiguiente = dia + 1;
      if (diaSiguiente > 7) diaSiguiente = 1; // domingo -> lunes
      if (!diasEntreno.includes(diaSiguiente)) {
        return dia;
      }
    }

    // 2. Si no, priorizar fin de semana
    if (diasEntreno.includes(6)) return 6; // sábado
    if (diasEntreno.includes(7)) return 7; // domingo

    // 3. Por defecto, el último día de la semana (mayor número)
  ...
  // ==========================================================================
  // MARCAR SESIÓN COMO REALIZADA (con publicación en feed, SIN FOTO)
  // ==========================================================================
  async marcarSesionRealizada(diaIndex, realizada) {
    ...
    try {
      ...
      // === AÑADIR AL FEED DE AMIGOS (con campo 'leido' = false) ===
      if (realizada) {
        const sesion = planData.sesiones[diaIndex];
        const userData = AppState.currentUserData;
        
        let distancia = 0;
        if (sesion.detalle && sesion.detalle.estructura) {
          const match = sesion.detalle.estructura.match(/(\d+(?:\.\d+)?)\s*k?m/i);
          if (match) distancia = parseFloat(match[1]);
        }

        const feedEntry = {
          friendUid: AppState.currentUserId,
          friendUsername: userData.username,
          trainingDate: firebaseServices.Timestamp.now(),
          trainingType: sesion.tipo,
          duration: sesion.duracion || 0,
          distancia: distancia,
          tss: sesion.detalle?.tssTotal || 0,
        ...
        if (window.Storage && typeof Storage.addToFeed === 'function') {
          await Storage.addToFeed(AppState.currentUserId, feedEntry);
        }
      }
...
console.log('✅ PlanGenerator v5.6 - Mejora en elección de día largo (prioriza día siguiente no marcado)');