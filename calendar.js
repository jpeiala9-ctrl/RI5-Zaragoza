...
// Versión: 5.3 - VALIDACIÓN DE ZONAS ASEGURADA
//
// CARACTERÍSTICAS:
...
const PlanGenerator = {
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, faseActual, semanaEnFase, libreriaPerfil, datos) {
    ...
    // Aseguramos que la zona sea coherente con el tipo de sesión
    if (tipo === 'series') {
      // Si la zona es Z2 o Z3, la corregimos a Z4 (valor por defecto para series)
      ...
      // Rodaje y largo deben ser Z1-Z2
  ...
  async marcarSesionRealizada(diaIndex, realizada) {
    ...
    try {
      ...
      // === NUEVO: AÑADIR AL FEED DE AMIGOS (sin foto) ===
      if (realizada) {
        const sesion = planData.sesiones[diaIndex];
        const userData = AppState.currentUserData;
        
        // Calcular distancia aproximada (podrías tenerla en sesion.detalle)
        let distancia = 0;
        if (sesion.detalle && sesion.detalle.estructura) {
          // Intentar extraer distancia de la estructura (simplificado)
...
console.log('✅ PlanGenerator v5.3 - Con validación de zonas y feed de amigos');