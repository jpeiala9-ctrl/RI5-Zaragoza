// ==================== calendar.js - VERSIÓN FINAL 2.22 ====================
// - CORREGIDO: error "Attempted to assign to readonly property" en rotas.
  ...
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    switch(estructura) {
      ...
      case 'rotas': {
        ...
        let repPorBloque = 4;        // <--- CAMBIO: de const a let
        let bloques = 2;
        ...
        // Ajustar dinámicamente: si el tiempo disponible es mayor, aumentar bloques
        if (factor > 1.2) {
          // Aumentar bloques
          bloques = Math.min(4, Math.floor(bloques * factor));
          if (bloques < 2) bloques = 2;
          // Recalcular totalBase
        ...
        // Si falta tiempo, reducir distancia de serie o repeticiones
        if (factor < 0.8) {
          ...
          if (nuevoFactor < 0.8) {
            // Aún falta tiempo: reducir repeticiones por bloque
          ...
          // Actualizar totalBase con los valores ajustados
          totalBase = total;
          factor = nuevoFactor;
        }

        // Calcular tiempos reales finales
        ...
        const tiempoEnZonaSeg = tiempoSeriesReal;
...
console.log('✅ PlanGenerator v2.22 - Adaptativo: series ajustadas a tiempo disponible, tiempos coherentes');