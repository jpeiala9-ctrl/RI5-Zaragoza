// ============================================================================
// calendar.js - VERSIÓN SIN DÍAS DEL MES
// Solo días de la semana: LUN, MAR, MIÉ, JUE, VIE, SÁB, DOM
// Respeta ESTRICTAMENTE los días marcados
// ============================================================================
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // ===== NO USAMOS FECHA DE INICIO =====
      // Generamos plan basado SOLO en días de la semana
  ...
  // ==========================================================================
  // RENDERIZADO DE PÁGINA - SIN DÍAS DEL MES
  // ==========================================================================
  renderizarPagina(sesiones) {
    ...
    for (let pos = 0; pos < celdas.length; pos++) {
      ...
      // SIN NÚMERO DE DÍA - solo letra del día y tipo de sesión
      let contenidoHtml = '';
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      let descHtml = `
        ...
        <div class="sesion-objetivos">
          <h4>🎯 OBJETIVOS</h4>
          <p><strong>Ritmo:</strong> ${sesion.detalle.ritmoObjetivo} min/km</p>
...
console.log('✅ PlanGenerator v13.0 - SIN DÍAS DEL MES');