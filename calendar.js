  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // MEJORA: antes tempo iba siempre primero y series siempre segundo,
        // toda la duración del plan -- eso, combinado con que diasLibres
        // nunca cambiaba de orden, hacía que el mismo día de la semana
        // recibiera siempre el mismo tipo de calidad (p.ej. "martes tempo,
        // miércoles series" fijo de la semana 1 a la última). Alternar el
        // orden por paridad de semana hace que qué tipo cae en el slot
        // "temprano" de la semana y cuál en el "tardío" vaya cambiando,
        // como haría un entrenador real variando el microciclo.
        const alternarOrdenCalidad = semanaGlobal % 2 === 0;
        const tiposCalidad = [];
        ...
        // MEJORA: rotación de diasLibres según la semana. diasDisponibles
        // (los días de entreno menos el de la tirada larga) es el MISMO
        // conjunto de días durante todo el plan, así que sin esto la
        // selección de índices de más abajo -- que es determinista dado un
        // mismo diasLibres -- acababa eligiendo siempre exactamente los
        // mismos días de la semana para calidad, plan entero. Rotando el
        // ORDEN del array antes de seleccionar (con un desplazamiento fijo
        // por semana, no aleatorio: reproducible y fácil de razonar, como
        // una periodización ondulante real) cambia qué días concretos les
        // tocan sin tocar ni una línea de la lógica de selección/colisión
        // que ya hay más abajo -- esa lógica solo trabaja con índices, así
        // que le da igual en qué orden vengan los días.
        if (diasLibres.length > 1) {
          const offsetSemana = (semanaGlobal - 1) % diasLibres.length;
          diasLibres = [...diasLibres.slice(offsetSemana), ...diasLibres.slice(0, offsetSemana)];
        }
