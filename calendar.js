...
// Versión: 7.1 - PERIODIZACIÓN AVANZADA + FEEDBACK VISUAL DE FASES
// ============================================================================
...
const PlanGenerator = {
  ...
  // ==========================================================================
  // CONSTANTES DE PERIODIZACIÓN (basadas en modelos científicos)
  // ==========================================================================
  FASES: {
  ...
  // Distribución de tipos de sesión por fase y nivel
  DISTRIBUCION_TIPOS: {
    principiante: {
  ...
  // Progresión de nivel (en semanas)
  PROGRESION_NIVEL: {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      // --- Generar la estructura de fases (macrociclos) ---
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // Obtener distribución de tipos para esta semana
        ...
        // Asignar los tipos a los días concretos
        const tiposPorDia = this.asignarTiposADias(diasEntreno, diaLargo, tiposSemana);

        // Generar las 7 sesiones de la semana
        const semana = [];
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          ...
          if (tipo === 'descanso') {
            ...
            // Buscar una sesión concreta en la biblioteca
  ...
  // Devuelve el mínimo de días recomendado según distancia
  ...
  // Genera la estructura de fases basada en la duración total
  generarFases(semanasTotales, objetivo, distancia) {
    const fases = [];
    let semanaInicio = 1;

    // Ajustar duraciones según objetivo
  ...
  // Obtiene la fase correspondiente a una semana global
  ...
  // Calcula el nivel según la semana (progresión)
  ...
  // Calcula el factor de volumen para una semana
  ...
  // Calcula el factor de intensidad para una semana
  ...
  // Calcula cuántas sesiones de cada tipo debe haber en la semana
  calcularTiposSemana(distribucion, numSesiones, esDescarga, fase) {
    ...
    // En semanas de descarga, reducir o eliminar tempo/series
    if (esDescarga) {
  ...
  // Asigna los tipos a los días concretos de la semana
  asignarTiposADias(diasEntreno, diaLargo, tiposSemana) {
    const tipos = [];
    for (let i = 0; i < tiposSemana.rodaje; i++) tipos.push('rodaje');
    for (let i = 0; i < tiposSemana.tempo; i++) tipos.push('tempo');
    for (let i = 0; i < tiposSemana.series; i++) tipos.push('series');
    for (let i = 0; i < tiposSemana.largo; i++) tipos.push('largo');

    // Mezclar aleatoriamente (excepto el largo)
    const tiposSinLargo = tipos.filter(t => t !== 'largo');
    this.mezclarArray(tiposSinLargo);
    ...
    for (let i = 0; i < otrosDias.length; i++) {
      if (i < tiposSinLargo.length) {
        tiposPorDia[otrosDias[i]] = tiposSinLargo[i];
      } else {
  ...
  // ==========================================================================
  // CREACIÓN DE SESIONES INDIVIDUALES
  // ==========================================================================
  ...
  aplicarFactorLargo(sesion, fase, semanaEnFase, factorVolumen) {
    // El largo ya tiene el factor de volumen aplicado, pero podemos añadir un extra
    const extra = 1.0 + (semanaEnFase * 0.02); // pequeño incremento semanal
  ...
  elegirDiaLargoOptimo(diasEntreno) {
    for (let dia of diasEntreno) {
      let sig = dia + 1;
      if (sig > 7) sig = 1;
      if (!diasEntreno.includes(sig)) return dia;
    }
  ...
  // ==========================================================================
  // VISUALIZACIÓN DEL CALENDARIO (con feedback visual de fases)
  // ==========================================================================
  ...
  renderizarPagina(sesiones) {
    ...
    let offset;
    if (diaSem === 0) offset = 6;
    else offset = diaSem - 1;

    let html = '';
    let sesionIndex = inicioPagina;

    // Añadir celdas vacías iniciales
    for (let i = 0; i < offset; i++) {
      html += '<div class="calendario-dia sesion-descanso"></div>';
    }

    // Añadir sesiones reales
    while (sesionIndex < finPagina) {
      ...
      // Determinar color de fase (si la sesión tiene fase)
      let faseColor = '';
      if (sesion.fase && this.FASES[sesion.fase]) {
        faseColor = this.FASES[sesion.fase].color;
      }

      // Clase adicional para el indicador de fase
    ...
    // Añadir leyenda de fases (opcional, se puede poner en el HTML)
  ...
  // Agrega una pequeña leyenda con los colores de las fases
  agregarLeyendaFases() {
  ...
  // ==========================================================================
  // FUNCIONES AUXILIARES PARA MÉTRICAS Y EXPLICACIONES
  // ==========================================================================
...
console.log('✅ PlanGenerator v7.1 - Periodización avanzada con feedback visual de fases');