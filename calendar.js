...
// Versión: 2.43 - Exportación .FIT autónoma (corregida – ingeniería senior)
// ============================================================================
...
const PlanGenerator = {
  ...
  abrirDetalleSesion(sesion, diaIndex) {
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      pasosHTML += '</div>';
      // *** BOTÓN DE EXPORTACIÓN .FIT ***
      const exportButtonHTML = `
        <button class="action-button" onclick="PlanGenerator.exportarWorkoutAFIT(AppState.currentSesionDetalle.sesion)"
          style="margin-top:16px; border-color: var(--accent-blue); color: var(--accent-blue);">
          ⌚ EXPORTAR A RELOJ (.FIT)
        </button>
      `;
      descripcion.innerHTML = headerHTML + objetivoHTML + zonasHTML + pasosHTML + exportButtonHTML;
  ...
  // ========== FUNCIÓN DE EXPORTACIÓN .FIT AUTÓNOMA ==========
  exportarWorkoutAFIT(sesion) {
    if (!sesion || !sesion.detalle) {
      Utils.showToast('No hay datos de sesión para exportar', 'error');
      return;
    }

    try {
      const extraerZona = (zonaStr) => {
        const match = String(zonaStr).match(/Z(\d)/);
        return match ? parseInt(match[1]) : 1;
      };

      const intensidadPorIcono = {
        '🔥': 'warmup',
        '💪': 'active',
        '🏁': 'active',
        '⛰️': 'active',
      ...
      const calSec = (sesion.detalle.calentamiento || 10) * 60;
      const enfSec = (sesion.detalle.enfriamiento || 5) * 60;
      const partePrincipalSec = ((sesion.detalle.partePrincipal || sesion.duracion - 10 - 5) || 30) * 60;

      const pasos = [];
      if (sesion.detalle.pasosDetallados && sesion.detalle.pasosDetallados.length > 0) {
        for (const paso of sesion.detalle.pasosDetallados) {
          let duracion = calSec;
          if (paso.icono === '🔥') duracion = calSec;
          else if (paso.icono === '🧘') duracion = enfSec;
          else if (paso.icono === '💪' || paso.icono === '🏁' || paso.icono === '⛰️') duracion = partePrincipalSec;
          else continue;

          pasos.push({
            nombre: paso.titulo,
            duracionSeg: duracion,
            zona: paso.icono === '🔥' || paso.icono === '🧘' ? 1 : extraerZona(sesion.detalle.zona),
            intensidad: intensidadPorIcono[paso.icono] || 'active'
          });
        }
      }

      if (pasos.length === 0) {
        pasos.push({ nombre: 'Calentamiento', duracionSeg: calSec, zona: 1, intensidad: 'warmup' });
        pasos.push({ nombre: 'Parte principal', duracionSeg: partePrincipalSec, zona: extraerZona(sesion.detalle.zona), intensidad: 'active' });
        pasos.push({ nombre: 'Enfriamiento', duracionSeg: enfSec, zona: 1, intensidad: 'cooldown' });
      }

      // ----- GENERACIÓN MANUAL DE FIT -----
      const writer = new FitWriter();
      const now = new Date();

      writer.fileHeader();
      writer.fileId({ type: 'workout', manufacturer: 'development', product: 0, timeCreated: now });
      writer.workout({ wktName: `RI5 - ${sesion.detalle.nombre}`, sport: 'running', subSport: 'generic', numValidSteps: pasos.length });

      pasos.forEach((p, idx) => {
        writer.workoutStep(idx, {
          wktStepName: p.nombre,
          durationType: 'time',
          durationTime: p.duracionSeg,
      ...
      a.href = url;
      a.download = `RI5_${sesion.detalle.nombre.replace(/\s+/g, '_')}.fit`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      Utils.showToast('✅ Archivo .FIT generado. Cópialo a tu reloj o impórtalo en tu plataforma.', 'success');

    } catch (error) {
      console.error('Error generando FIT:', error);
      Utils.showToast('Error al generar el archivo .FIT', 'error');
    }
  },
...
// ==================== ESCRITOR FIT AUTÓNOMO (CORREGIDO – INGENIERÍA SENIOR) ====================
class FitWriter {
  constructor() {
    this.data = [];
    this.localMesgNum = 0;
  ...
  fileHeader() {
    ...
    new DataView(header.buffer).setUint16(2, 2132, true); // profile version (21.32)
    header[8] = 0x2E;        // '.'
    header[9] = 0x46;        // 'F'
    header[10]= 0x49;        // 'I'
    header[11]= 0x54;        // 'T'
  ...
  fileId({ type, manufacturer, product, timeCreated }) {
    const mesgNum = 0;
    const fields = [
      { num: 0, value: this._enumType(type) },
      { num: 1, value: this._enumManufacturer(manufacturer) },
      { num: 2, value: product },
      { num: 4, value: this._timestamp(timeCreated) }
    ];
  ...
  workout({ wktName, sport, subSport, numValidSteps }) {
    ...
    const fields = [
      { num: 3, value: wktName },
      { num: 4, value: this._enumSport(sport) },
      { num: 5, value: this._enumSubSport(subSport) },
      { num: 11, value: numValidSteps }
    ];
    this._writeMessage(mesgNum, fields);
  }
  ...
  workoutStep(index, { wktStepName, durationType, durationTime, targetType, targetValue, intensity }) {
    ...
    const fields = [
      { num: 254, value: index },
      { num: 0, value: wktStepName },
      { num: 1, value: this._enumDurationType(durationType) },
      { num: 2, value: durationTime },
      { num: 3, value: this._enumTargetType(targetType) },
      { num: 4, value: targetValue },
      { num: 7, value: this._enumIntensity(intensity) }
    ];
    this._writeMessage(mesgNum, fields);
  }

  finish() {
    // 1. Construir el buffer de datos (definiciones + registros)
    let dataBuffer = new Uint8Array(0);
    this.messages.forEach(msg => {
      const def = msg.definition;
      const record = msg.record;   // incluye el byte de cabecera 0x00 + payload
      const combined = new Uint8Array(def.length + record.length);
      combined.set(def);
      combined.set(record, def.length);   // concatenación correcta
    ...
    // 2. Actualizar tamaño de datos en el header
  ...
  _writeMessage(mesgNum, fields) {
    // --- Parte 1: Mensaje de definición (Definition Message) ---
    const fieldDefs = fields.map(f => ({
      num: f.num,
      size: this._fieldSize(f.value),
      type: this._fieldType(f.value)
    }));
    ...
    // --- Parte 2: Registro de datos (Data Record) ---
    let payloadSize = 0;
    fieldDefs.forEach(fd => payloadSize += fd.size);
    const record = new Uint8Array(1 + payloadSize);  // 1 byte extra para la cabecera
    record[0] = 0x00;  // header byte: registro normal

    const view = new DataView(record.buffer);
    let payloadOffset = 1;  // empezamos después del header byte
    fields.forEach(f => {
    ...
    // Guardamos la definición y el registro completo
    this.messages.push({ definition: def, record: record });
  }

  _fieldSize(value) {
    if (typeof value === 'string') {
      return Math.min(value.length + 1, 64);  // strings: máximo 64 bytes, incluye terminador nulo
    } else if (typeof value === 'number') {
      if (Number.isInteger(value) && value <= 255 && value >= 0) return 1;
      else if (Number.isInteger(value) && value <= 65535) return 2;
      else return 4;
    }
    return 1;
  }
  ...
  _fieldType(value) {
    if (typeof value === 'string') return 7;  // string
    return 0x86;  // uint32 (funciona para uint8/uint16 también)
  }
  ...
  // ---- ENUMS ----
  _enumType(type) { const map = { activity: 4, workout: 5 }; return map[type] || 5; }
  _enumManufacturer(m) { return 255; }  // development
  _enumSport(sport) { return sport === 'running' ? 1 : 1; }
  _enumSubSport(sub) { return 0; }  // generic
  _enumDurationType(d) { return d === 'time' ? 0 : 0; }
  _enumTargetType(t) { return t === 'heart_rate_zone' ? 0 : 0; }  // 0 = heart rate zone (CORRECTO)
  _enumIntensity(i) {
...
console.log('✅ PlanGenerator v2.43 - Exportación .FIT definitiva (ingeniería senior)');