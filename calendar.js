// ==================== calendar.js - VERSIÓN FINAL 2.23 ====================
// - AHORA usa las zonas de entrenamiento calculadas por el usuario (edad + 6km)
// - Ritmos de rodaje, tempo, series y fartlek se toman de Z2, Z3, Z5, etc.
// - Eliminada la función obsoleta `obtenerRitmoParaTipo`
// - Se mantiene toda la adaptación dinámica de volumen, series, progresión.
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== NUEVO: OBTENER RITMO DESDE LAS ZONAS CALCULADAS POR EL USUARIO ==========
  obtenerRitmoParaZona(zonaNombre) {
    ...
    if (!zonas || zonas.length === 0) {
      ...
      return Utils.formatR(AppState.lastRitmoBase);
    }
    ...
    const factorPace = zona[4]; // índice 4 contiene el factor de ritmo (1.25 para Z2, 1.05 para Z4, 0.95 para Z5, etc.)
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        const intensidadTotal = ajusteIntensidad * intensidadOnd; // solo se usa para volumen, no para ritmos
  ...
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    ...
    // Obtener ritmo objetivo desde las zonas según el tipo de sesión
    const obtenerRitmoObjetivo = () => {
      if (tipo === 'rodaje') return this.obtenerRitmoParaZona('Z2');
      if (tipo === 'tempo') return this.obtenerRitmoParaZona('Z3'); // podríamos usar Z4 en fases avanzadas, pero de momento Z3
      if (tipo === 'series') return this.obtenerRitmoParaZona('Z5');
      if (tipo === 'largo') return this.obtenerRitmoParaZona('Z2');
      return this.obtenerRitmoParaZona('Z2');
    };
    ...
    if (incluirRitmoObjetivo) {
      ...
      const ritmoObjetivoStr = this.obtenerRitmoParaZona('Z4'); // ritmo objetivo más rápido
      ...
      const ritmoObj = obtenerRitmoObjetivo();
      switch(tipo) {
        case 'rodaje':
          ...
          pasoPrincipal = {
            ...
            accion: `${partePrincipal}' a ritmo aeróbico (Z2), conversación fluida (${ritmoObj} min/km)`,
        ...
        case 'tempo':
          ...
          pasoPrincipal = {
            icono: '⚡',
            titulo: 'TEMPO',
            accion: `${partePrincipal}' a ritmo sostenido (Z3-Z4), "cómodamente duro" (${ritmoObj} min/km)`,
        ...
        case 'largo':
          ...
          pasoPrincipal = {
            icono: '📏',
            titulo: 'TIRADA LARGA',
            accion: `${partePrincipal}' a ritmo aeróbico (Z2), manteniendo buena técnica (${ritmoObj} min/km)`,
  ...
  async crearSesionAvanzadaSeries(estructura, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta) {
    ...
    const ritmoRapido = this.obtenerRitmoParaZona('Z5');   // series a ritmo de VO2max
    const ritmoModerado = this.obtenerRitmoParaZona('Z2'); // recuperación activa
...
console.log('✅ PlanGenerator v2.23 - Usa zonas calculadas por el usuario, ritmos coherentes');