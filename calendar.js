// ============================================================================
// calendar.js - VERSIÓN ÉLITE 2.1
// Correcciones: taper sin doble reducción, redondeo en ritmo objetivo,
// fuerza con dos sesiones, métricas para fuerza, CSS añadido.
// ============================================================================
...
const PlanGenerator = {
  ...
  // ========== DISTRIBUCIÓN CON STRENGTH ==========
  DISTRIBUCION_TIPOS: {
    principiante: {
      base: { rodaje: 0.7, tempo: 0.1, series: 0.0, largo: 0.1, strength: 0.1 },
      construccion: { rodaje: 0.6, tempo: 0.15, series: 0.05, largo: 0.1, strength: 0.1 },
      especifica: { rodaje: 0.55, tempo: 0.2, series: 0.1, largo: 0.1, strength: 0.05 },
      pico: { rodaje: 0.45, tempo: 0.25, series: 0.15, largo: 0.1, strength: 0.05 },
      taper: { rodaje: 0.7, tempo: 0.1, series: 0.0, largo: 0.1, strength: 0.1 }
    },
    intermedio: {
      base: { rodaje: 0.6, tempo: 0.15, series: 0.05, largo: 0.1, strength: 0.1 },
      construccion: { rodaje: 0.5, tempo: 0.2, series: 0.1, largo: 0.1, strength: 0.1 },
      especifica: { rodaje: 0.4, tempo: 0.2, series: 0.2, largo: 0.1, strength: 0.1 },
      pico: { rodaje: 0.3, tempo: 0.25, series: 0.25, largo: 0.1, strength: 0.1 },
      taper: { rodaje: 0.55, tempo: 0.15, series: 0.05, largo: 0.1, strength: 0.15 }
    },
    avanzado: {
      base: { rodaje: 0.5, tempo: 0.2, series: 0.1, largo: 0.1, strength: 0.1 },
      construccion: { rodaje: 0.4, tempo: 0.2, series: 0.2, largo: 0.1, strength: 0.1 },
      especifica: { rodaje: 0.3, tempo: 0.2, series: 0.3, largo: 0.1, strength: 0.1 },
      pico: { rodaje: 0.2, tempo: 0.25, series: 0.35, largo: 0.1, strength: 0.1 },
      taper: { rodaje: 0.45, tempo: 0.2, series: 0.1, largo: 0.1, strength: 0.15 }
    }
  },
  ...
  // ==========================================================================
  // AJUSTE POR DISTANCIA (con factor rodaje reducido para maratón)
  // ==========================================================================
  ajustarDistribucionPorDistancia(baseDistribucion, distancia) {
    ...
    for (let nivel in nueva) {
      for (let fase in nueva[nivel]) {
        ...
        let sum = d.rodaje + d.tempo + d.series + d.largo + d.strength;
        ...
        d.strength = d.strength / sum;
        sum = d.rodaje + d.tempo + d.series + d.largo + d.strength;
        d.rodaje /= sum;
        d.tempo /= sum;
        d.series /= sum;
        d.largo /= sum;
  ...
  // ==========================================================================
  // CÁLCULO DE VOLUMEN SEMANAL BASE (minutos)
  // ==========================================================================
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // MICROCICLO 3:1 – descarga cada 4 semanas, excluyendo taper (ya tiene su propia reducción)
        ...
        const minutosPorTipo = {
          ...
          strength: Math.round(volumenSemana * distribucion.strength)
        };

        // Ajuste por redondeo
        let suma = minutosPorTipo.rodaje + minutosPorTipo.tempo + minutosPorTipo.series + minutosPorTipo.largo + minutosPorTipo.strength;
        ...
        // Asignar fuerza (priorizar dos sesiones si hay días suficientes)
        if (minutosPorTipo.strength > 0) {
          const minutosFuerza = minutosPorTipo.strength;
          if (diasDisponibles.length >= 2) {
            const minutosPorSesion = Math.floor(minutosFuerza / 2);
            for (let i = 0; i < 2; i++) {
              const dia = diasDisponibles[i];
              tiposPorDia[dia] = { tipo: 'strength', minutos: minutosPorSesion };
            }
            // Eliminar los dos días usados
            diasDisponibles.splice(0, 2);
          } else if (diasDisponibles.length === 1) {
            const dia = diasDisponibles[0];
            tiposPorDia[dia] = { tipo: 'strength', minutos: minutosFuerza };
            diasDisponibles.splice(0, 1);
          } else {
            // No hay días disponibles, se perderá la fuerza (no debería ocurrir)
            console.warn('No hay días disponibles para fuerza');
          }
        }

        // Resto de tipos (rodaje, tempo, series)
  ...
  // ==========================================================================
  // CREAR SESIÓN DESDE MATRIZ (con duración exacta y corrección de tiempos)
  // ==========================================================================
  async crearSesionDesdeMatriz(sesionBase, esActivo, fase, semanaEnFase, nivel, datos, factorVolumen, factorIntensidad, duracionExacta = null) {
    ...
    // ========== CÁLCULO DE CALENTAMIENTO Y ENFRIAMIENTO (CORREGIDO) ==========
    ...
    // Para tirada larga con ritmo objetivo (solo en fases específica o pico, para media/maratón)
  ...
  calcularMetricasSesion(sesion) {
    // Fuerza no aporta distancia ni TSS
...
console.log('✅ PlanGenerator v16.1 - ÉLITE 2.1: todas las correcciones aplicadas');