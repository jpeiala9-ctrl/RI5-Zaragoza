      ...
      const abrirSelectorTipo = () => {
        ...
        popup.innerHTML = `
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
            ${opcionesTipo.map(o => `
              <button type="button" class="datos-reales-tipo-opcion" data-tipo="${o.tipo}" style="background:var(--bg-primary); border:1px solid var(--border-color); color:var(--text-primary); padding:14px 8px; border-radius:12px; cursor:pointer; font-size:13px; display:flex; flex-direction:column; align-items:center; gap:4px;">
                <span style="font-size:20px;">${o.icono}</span>${o.label}
              </button>
            `).join('')}
          </div>
        `;
