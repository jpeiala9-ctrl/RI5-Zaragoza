...
// Versión: 2.70 - A petición del usuario, la recuperación de fuerza
//                seguía siendo demasiado alta (factor 0.9x TSS x1.1, tope
//                72h) para sesiones de intensidad moderada, y encima el
//                aviso de "aún no recuperado" bloqueaba SALIR A CORRER solo
//                por haber hecho fuerza el día anterior. Dos cambios en
//                calcularRecuperacion()/comprobarFatigaAntesDeSesion(): (1)
//                factor de 'strength' bajado de 1.1 a 0.75 y tope propio de
//                24h (antes 72h, compartido con todos los tipos) -- como
//                mucho "1 día" para volver a hacer fuerza; (2) el aviso de
//                fatiga ahora compara el GRUPO de la sesión anterior con el
//                de la que se va a empezar (fuerza vs. carrera son grupos
//                independientes) y solo avisa si se repite el mismo grupo,
//                así que no recuperado de fuerza ya no impide salir a
//                correr (ni viceversa).
// Versión: 2.69 - Nuevo calcularTSSFuerza(): el TSS de una sesión de
//                fuerza al marcarla como hecha ya no aplica un RPE=6 fijo
//                a TODOS sus minutos (incluido calentamiento y estiramientos/
//                enfriamiento, de esfuerzo mucho más bajo que la parte
//                principal). Ahora se reparte el RPE por fase -- igual que
//                ya se hace con las zonas en las sesiones de carrera --
//                identificando calentamiento/enfriamiento por el título del
//                paso (mismo criterio que SessionInvites._calcularPersonalizacionFuerza).
//                Antes, una sesión de fuerza con buen calentamiento y vuelta
//                a la calma salía con el mismo TSS que si esos minutos
//                también fueran a tope, y como calcularRecuperacion() usa el
//                factor más alto de todos los tipos para 'strength' (1.1),
//                eso disparaba la recomendación de descanso a 2 días para
//                sesiones de intensidad en realidad moderada.
...
const PlanGenerator = {
    ...
    if (extrasPlanMin > 0) {
      ...
      if (esSerieConDatos) {
        ...
        if (plannedMainSec > 0 && plannedRecSec > 0) {
          ...
          subBloques.push({ nombre: 'partePrincipal', planMin: det.segundosEsfuerzo / 60, zona: zonaPlanEsfuerzo || 'Z4' });
      ...
      if (totalPlanSub > 0) {
        ...
        subBloques.forEach(b => {
          const prop = b.planMin / totalPlanSub;
          const bloque = { min: mainMin * prop, km: mainKm * prop, zona: b.zona };
    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      if (window.GPSTracker && sesion.tipo !== 'strength') {
        ...
        gpsBtn.addEventListener('click', async () => {
          const continuar = await this.comprobarFatigaAntesDeSesion(sesionLimpia.tipo || sesion.tipo);
          if (!continuar) return;
  ...
  // 🔥 FIX: el aviso de fatiga era genérico -- si la ÚLTIMA sesión había
  // sido de fuerza, bloqueaba con el mismo aviso el querer SALIR A CORRER
  // aunque sean disciplinas distintas (el usuario lo reportó explícitamente:
  // "necesito descanso para volver a hacer fuerza, pero si puedo correr
  // antes, ¿no?"). Ahora se compara el GRUPO de la última sesión con el de
  // la sesión que se va a empezar (`tipoProximo`): fuerza y carrera son
  // grupos independientes, así que solo se avisa si se repite el MISMO
  // grupo (fuerza→fuerza, o carrera→carrera). Si no se pasa `tipoProximo`
  // (compatibilidad con llamadas antiguas) se mantiene el aviso genérico
  // de antes, por seguridad.
  _grupoRecuperacion(tipo) {
    return tipo === 'strength' ? 'fuerza' : 'carrera';
  },
  ...
  async comprobarFatigaAntesDeSesion(tipoProximo = null) {
    ...
    if (estado && estado.pct < 100) {
      if (tipoProximo && this._grupoRecuperacion(tipoProximo) !== this._grupoRecuperacion(estado.tipo)) {
      ...
      const esFuerza = estado.tipo === 'strength';
      const mensajeTipo = esFuerza ? 'tu última sesión de fuerza' : `tu última sesión (${estado.tipo})`;
      const continuar = await Utils.confirm(
        '⚠️ Aún no estás recuperado al 100%',
        `Tu recuperación de ${mensajeTipo} está al ${estado.pct}% (${estado.tss} TSS). Faltan ~${horasStr} para la recuperación completa. Entrenar ahora aumenta el riesgo de fatiga acumulada y lesión. ¿Quieres continuar igualmente?`
      );
      if (!continuar) {
  ...
  async _marcarSesionRealizadaInterno(diaIndex, realizada, realDistance = null, realDurationMs = null, realMaxSpeedKmh = null, esGPS = false, saltarComprobacionFatiga = false, tipoCorregido = null) {
    if (realizada && !saltarComprobacionFatiga) {
      const tipoSesionAMarcar = tipoCorregido || this._sesionesPorDiaGlobal?.[diaIndex]?.tipo || null;
      const continuar = await this.comprobarFatigaAntesDeSesion(tipoSesionAMarcar);
      if (!continuar) {
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          if (sesion.tipo === 'strength') {
            ...
            // calcularTSSdesdeReal() ya tenía su propio factor para
            // 'strength' (1.1) esperando precisamente este dato.
            //
            // 🔥 FIX: ya no se aplica un RPE=6 plano a toda la sesión --
            // ver calcularTSSFuerza() para el porqué (calentamiento y
            // estiramientos/enfriamiento cuentan con un RPE más bajo que
            // la parte principal).
            const tssEquivalenteFuerza = this.calcularTSSFuerza(sesion);
            metricas = {
              distanciaTotal: 0,
  ...
  // 🔥 FIX: el TSS de fuerza (ver el sitio que llama a esto, más abajo)
  // aplicaba un único RPE=6 a TODOS los minutos de la sesión, incluidos
  // el calentamiento y los estiramientos/vuelta a la calma -- fases de
  // esfuerzo mucho más bajo que la parte principal (el trabajo de fuerza
  // en sí). Eso inflaba el TSS de cualquier sesión con un calentamiento/
  // enfriamiento decentes: una de 50' con, por ejemplo, 10' calentamiento
  // + 30' parte principal + 10' estiramientos salía con el mismo TSS que
  // 50' seguidos a RPE 6 -- y como calcularRecuperacion() usa el factor
  // más alto de todos los tipos para 'strength' (1.1), eso disparaba la
  // recomendación de descanso a 2 días para lo que en realidad es una
  // sesión de intensidad moderada.
  //
  // Ahora se reparte el RPE por fase, igual que ya se hace con las zonas
  // en las sesiones de carrera (ver calcularTSSdesdeReal arriba): se
  // identifican los pasos de calentamiento/estiramientos-enfriamiento con
  // el mismo criterio que ya usa SessionInvites._calcularPersonalizacionFuerza
  // (por el título del paso), y solo el resto de minutos -- la parte
  // principal, el trabajo de fuerza real -- se queda en el RPE 6 asumido.
  // Si la sesión no tiene pasos identificables como calentamiento/
  // enfriamiento (p.ej. viene sin desglose), esto se reduce exactamente
  // al cálculo de antes (RPE 6 en toda la duración).
  calcularTSSFuerza(sesion) {
    const duracionTotal = sesion.duracion || 0;
    if (duracionTotal <= 0) return 0;

    const RPE_CALENTAMIENTO = 3;  // activación/movilidad: esfuerzo bajo
    const RPE_PRINCIPAL = 6;      // trabajo de fuerza en sí ("duro, pero no al fallo")
    const RPE_ENFRIAMIENTO = 2;   // estiramientos/vuelta a la calma

    let minCalentamiento = 0;
    let minEnfriamiento = 0;
    (sesion.detalle?.pasosDetallados || []).forEach(p => {
      const tit = (p.titulo || '').toUpperCase();
      if (tit.includes('CALENTAMIENTO')) minCalentamiento += (p.duracionMin || 0);
      else if (tit.includes('ENFRIAMIENTO') || tit.includes('ESTIRAMIENTO')) minEnfriamiento += (p.duracionMin || 0);
    });
    // Los pasos son datos manuales del admin y podrían no cuadrar del
    // todo con la duración total: no dejar la parte principal en negativo.
    minCalentamiento = Math.min(minCalentamiento, duracionTotal);
    minEnfriamiento = Math.min(minEnfriamiento, duracionTotal - minCalentamiento);
    const minPrincipal = Math.max(0, duracionTotal - minCalentamiento - minEnfriamiento);

    const tss = (minCalentamiento / 60) * RPE_CALENTAMIENTO * 10
              + (minPrincipal / 60) * RPE_PRINCIPAL * 10
              + (minEnfriamiento / 60) * RPE_ENFRIAMIENTO * 10;
  ...
  calcularRecuperacion(tss, tipo) {
    // 🔥 FIX: 'strength' tenía el factor MÁS ALTO de todos los tipos (1.1,
    // por encima incluso de tempo/largo) y compartía el mismo tope de 72h
    // que una tirada larga muy dura. Para una sesión de fuerza normal (RPE
    // 6 en la parte principal, moderada) eso disparaba fácilmente más de
    // 24h de "recuperación" -- el usuario confirmó que es demasiado para
    // el tipo de sesión que hace. Bajado a 0.75 (recuperación muscular
    // localizada, no el mismo desgaste cardio-metabólico que una sesión de
    // calidad en carrera) y con un tope propio de 24h en vez de 72h: como
    // mucho "un día" para poder repetir fuerza, nunca más.
...
console.log('✅ PlanGenerator v2.69 - TSS de fuerza ponderado por fase (calentamiento/principal/enfriamiento), ya no un RPE plano para toda la sesión');