...
// Versión: 2.42 - Exportación .FIT autónoma (sin dependencias externas)
// ============================================================================
...
// ==================== ESCRITOR FIT AUTÓNOMO (sin dependencias) ====================
class FitWriter {
  constructor() {
    this.data = [];
    this.localMesgNum = 0;
  ...
  fileHeader() {
    ...
    header[0] = 14;
    header[1] = 0;
    new DataView(header.buffer).setUint16(2, 1664, true);
    header[8] = 0x2E;
    header[9] = 0x46;
    header[10]= 0x49;
    header[11]= 0x54;
  ...
  finish() {
    ...
    this.messages.forEach(msg => {
      ...
      const combined = new Uint8Array(def.length + payload.length);
      combined.set(def);
      combined.set(payload, def.length);
    ...
    const crc = 0x0000;
    new DataView(headerWithSize.buffer).setUint16(12, crc, true);

    const result = new Uint8Array(headerWithSize.length + dataBuffer.length);
  ...
  _writeMessage(mesgNum, fields) {
    const fieldDefs = fields.map(f => ({ num: f.num, size: this._fieldSize(f.value), type: this._fieldType(f.value) }));
    ...
    let payloadOffset = 0;
    fields.forEach(f => {
      this._writeValue(view, payloadOffset, f.value, fieldDefs.find(d => d.num === f.num).size);
      payloadOffset += fieldDefs.find(d => d.num === f.num).size;
    });

    this.messages.push({ definition: def, payload: payload });
  }

  _fieldSize(value) {
    if (typeof value === 'string') {
      return Math.min(value.length, 64);
  ...
  _enumType(type) {
    const map = { activity: 4, workout: 5 };
    return map[type] || 5;
  }
  _enumManufacturer(m) {
    return 255;
...
console.log('✅ PlanGenerator v2.42 - Exportación .FIT autónoma (sin dependencias)');