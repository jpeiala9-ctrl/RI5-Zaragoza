// ============================================================================
// calendar.js - VERSIÓN ÉLITE 2.5
// Asignación inteligente de sesiones (intercalado de calidad, rodajes equitativos)
// Fuerza como complemento en días de rodaje
// ============================================================================
...
const PlanGenerator = {
  ...
  async generarCalendarioEntreno() {
    ...
    try {
      ...
      for (let semanaGlobal = 1; semanaGlobal <= semanasTotales; semanaGlobal++) {
        ...
        // Días disponibles excluyendo el día de tirada larga
        let diasDisponibles = [...diasEntreno];
        const indexLargo = diasDisponibles.indexOf(diaLargo);
        if (indexLargo !== -1) diasDisponibles.splice(indexLargo, 1);
        ...
        // --- Asignación inteligente de calidad (tempo y series) y rodaje ---
        const tiposCalidad = [];
        ...
        // Asignar calidad en días alternos si es posible
        if (tiposCalidad.length > 0 && diasLibres.length > 0) {
          const numCalidad = tiposCalidad.length;
          // Calcular un paso para espaciar las sesiones de calidad
          ...
          for (let i = 0; i < numCalidad; i++) {
            ...
            // Evitar consecutividad si hay espacio
          ...
          // Asignar cada calidad a su día
          for (let i = 0; i < tiposCalidad.length; i++) {
            const dia = diasLibres[indicesAsignados[i]];
            const calidad = tiposCalidad[i];
            // Limitar a 90 minutos por sesión (no debería superarse, pero por seguridad)
            const minutosDia = Math.min(calidad.minutos, 90);
            tiposPorDia[dia] = { tipo: calidad.tipo, minutos: minutosDia };
            calidad.minutos -= minutosDia; // (si sobra, se tratará como rodaje, pero no debería ocurrir)
          }
          // Eliminar los días usados de la lista de libres
          diasLibres = diasLibres.filter((_, idx) => !indicesAsignados.includes(idx));
        }

        // Ahora repartir minutos de rodaje (y posibles restos de calidad) en los días libres restantes
        let minutosRodajeTotal = minutosPorTipo.rodaje;
        // Si alguna calidad no se asignó completamente (por límite de 90), añadir esos minutos al rodaje
        ...
        if (numDiasRodaje > 0 && minutosRodajeTotal > 0) {
          let minutosPorDiaRodaje = Math.floor(minutosRodajeTotal / numDiasRodaje);
          let resto = minutosRodajeTotal % numDiasRodaje;
          for (let i = 0; i < numDiasRodaje; i++) {
            const dia = diasLibres[i];
            let minutos = minutosPorDiaRodaje + (i < resto ? 1 : 0);
            minutos = Math.min(minutos, 90);
            if (minutos > 0) {
        ...
        // Si algún día libre quedó sin sesión (por falta de minutos), asignar rodaje mínimo
        ...
        // Elegir días para fuerza (2 días, preferentemente rodaje)
        ...
        for (let diaSemana = 1; diaSemana <= 7; diaSemana++) {
          if (!diasEntreno.includes(diaSemana)) {
            ...
            if (!info) {
              // No debería ocurrir, pero por seguridad asignamos descanso
              semana.push({
                diaGlobal: diaGlobalCounter++,
                semana: semanaGlobal,
                diaSemana,
  ...
  seleccionarDiasFuerza(tiposPorDia, diaLargo) {
    ...
    // Si no hay suficientes, añadir días con tempo o series (evitando largo)
    ...
    // Si aún no hay 2, usar cualquier día con sesión (excepto largo)
    if (candidatos.length < 2) {
      for (let dia = 1; dia <= 7; dia++) {
        const sesion = tiposPorDia[dia];
  ...
  agregarFuerzaASesion(sesion) {
    ...
    // Opcional: marcar que tiene fuerza (para mostrar algo en el calendario)
...
console.log('✅ PlanGenerator v16.5 - Asignación inteligente de sesiones (intercalado calidad, rodajes equitativos)');