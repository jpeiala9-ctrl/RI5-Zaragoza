    ...
    if (sesion.tipo !== 'descanso' && sesion.detalle) {
      ...
      if (labelCheckbox) {
        labelCheckbox.textContent = !checkbox.disabled
          ...
          : (sesionEsFutura ? '⏱️ Día futuro: aún no se puede marcar' : '⏱️ Día anterior: ya no se puede marcar');
      }
    ...
    if (realizada && this._fechaInicioPlan) {
      ...
      if (fechaSesionCheck.getTime() !== hoyCheck.getTime()) {
        const mensaje = fechaSesionCheck < hoyCheck
          ? '⏱️ No se puede marcar como realizada una sesión de un día anterior'
          : '⏱️ Todavía no puedes marcar una sesión de un día futuro';
