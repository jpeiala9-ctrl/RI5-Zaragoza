// ==================== calendar.js - VERSIÓN PRODUCCIÓN CON GAMIFICACIÓN (SIN RECARGAS DE PERFIL) ====================
// Versión: 2.45 - Refresca perfil y amigos tras subir de nivel
// ============================================================================
    ...
    try {
      ...
      if (realizada) {
        ...
        if (sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          ...
          // Sumar a gamificación (actualiza XP, distancia, nivel y suma km a la zapatilla)
          if (window.Gamification) {
            try {
              const oldLevel = (await Gamification.getData(AppState.currentUserId)).level;
              await Gamification.updateAfterSession(AppState.currentUserId, sesion, metricas);
              const newLevel = (await Gamification.getData(AppState.currentUserId)).level;
              
              // Si el nivel cambió, refrescar perfil y listas de amigos
              if (newLevel !== oldLevel) {
                console.log(`🎉 Nivel cambiado de ${oldLevel} a ${newLevel}. Refrescando UI...`);
                // Recargar perfil si la pestaña está activa
                if (document.getElementById('tab-perfil').classList.contains('active') && window.Profile) {
                  await Profile.cargarPerfil(true); // true para forzar recarga (borrar caché)
                }
                // Recargar lista de amigos si está visible
                if (document.getElementById('tab-amigos').classList.contains('active') && window.Friends) {
                  await Friends.cargarListaAmigos(true);
                  // También actualizar explorar usuarios si está visible
                ...
                // También actualizar el muro si está visible (por si muestra niveles en avatares)
        ...
        // Restar de gamificación (resta XP, distancia, km de zapatilla)
        if (window.Gamification && sesion && sesion.detalle && sesion.tipo !== 'descanso') {
          try {
            ...
            // También refrescar UI al desmarcar por si baja de nivel
            if (document.getElementById('tab-perfil').classList.contains('active') && window.Profile) {
...
console.log('✅ PlanGenerator v2.45 - Refresca perfil y amigos tras subir de nivel');