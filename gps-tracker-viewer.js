// ==================== gps-track-viewer.js ====================
// Versión: 1.0 - Bottom sheet con mapa Leaflet real + stats GPS + ciudad
// Se abre al pulsar el minimap en muro o perfil
// ====================
...
const GPSTrackViewer = {
  ...
  async open(entry) {
    ...
    // Reverse geocoding en paralelo (no bloquea el mapa)
  ...
  _crearSheet(entry) {
    ...
    sheet.innerHTML = `
      ...
      <!-- Cabecera: nombre + fecha + botón cerrar -->
      ...
      <div style="flex-shrink:0;padding:8px 16px;display:flex;align-items:center;gap:16px;background:#0d0d0d;border-top:1px solid #1e1e1e;">
        <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:#555;">
          <div style="width:10px;height:10px;border-radius:50%;background:#9BB5A0;border:1.5px solid #fff;"></div> Inicio
        </div>
        <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:#555;">
  ...
  async _initMap(entry) {
    ...
    try {
      ...
      const poly    = window.L.polyline(latlngs, {
        color: '#c0a060', weight: 4.5, opacity: 0.92,
        lineJoin: 'round', lineCap: 'round'
      }).addTo(map);

      // Marcador inicio (verde)
      window.L.marker(latlngs[0], { icon: window.L.divIcon({
        html: '<div style="width:12px;height:12px;background:#9BB5A0;border:2.5px solid #fff;border-radius:50%;box-shadow:0 0 6px rgba(155,181,160,.6);"></div>',
  ...
  // ===== REVERSE GEOCODING (Nominatim, gratis, sin API key) =====
  async _getCity(lat, lng) {
    try {
      ...
      // Intentar de más específico a más general
...
console.log('✅ GPSTrackViewer listo');
