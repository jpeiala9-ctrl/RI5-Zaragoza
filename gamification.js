// ==================== gamification.js - VERSIÓN CON HISTORIAL DE ZAPATILLAS ====================
// Versión: 5.1 - Añadido historial de cambios de zapatilla, mismo resto de funciones.
// ====================
...
const Gamification = {
  ...
  getDefaultData() {
    return {
      ...
      shoeHistory: []    // <-- NUEVO: array de { name, km, changedAt }
    };
  },
  ...
  // ========== MÉTODOS PARA ZAPATILLA CON HISTORIAL ==========
  ...
  async setCurrentShoe(uid, newShoeName) {
    ...
    try {
      ...
      // Solo guardar en historial si la zapatilla anterior tiene nombre distinto al por defecto o ha acumulado km
      if (oldShoe.name !== 'Zapatilla actual' || oldShoe.km > 0) {
        ...
        // Limitar a 15 entradas para no crecer demasiado
        ...
        // Si es la primera zapatilla (la de defecto sin km), simplemente actualizar el nombre
        await firebaseServices.db.collection('gamification').doc(uid).update({
          currentShoe: { name: newShoeName, km: 0 }
        });
      }
  ...
  // ========== ACTUALIZAR TRAS MARCAR SESIÓN (sin racha) ==========
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // Actualizar kilómetros de la zapatilla actual
      ...
      const newData = {
        ...
        lastUpdate: firebaseServices.Timestamp.now()
        // currentShoe y shoeHistory se actualizan aparte
      };
  ...
  async removeSession(uid, sesion, metricas, diaIndex) {
    ...
    try {
      ...
      // Restar kilómetros de la zapatilla actual
