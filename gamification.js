// ==================== gamification.js - VERSIÓN DEFINITIVA CON RACHA LOCAL ROBUSTA ====================
// Versión: 2.8 - Cálculo de fecha local con getDate() y recarga forzada del perfil
// ====================
...
const Gamification = {
  ...
  // ========== RECALCULAR RACHA USANDO FECHA LOCAL (ROBUSTO) ==========
    ...
    try {
      ...
      snapshot.forEach(doc => {
        ...
        if (data.timestamp) {
          ...
          // Obtener fecha local YYYY-MM-DD usando getFullYear, getMonth, getDate
      ...
      while (fechasSet.has(current)) {
        ...
        current = `${prevYear}-${prevMonth}-${prevDay}`;
      }
      return streak;
    } catch (error) {
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // Mostrar notificaciones de subida de nivel o insignias
      ...
      // Recargar el perfil si la pestaña está activa
      setTimeout(() => {
