...
// Versión: 3.3 - Recalcula siempre la racha desde globalFeed con fechas locales. Sin botones.
// ====================
...
const Gamification = {
  ...
  async _recalculateStreak(uid) {
    ...
    try {
      ...
      const hoy = new Date();
      const hoyStr = this._toLocalDateString(hoy);
      let streak = 0;
      let current = hoyStr;
      ...
      while (fechasSet.has(current)) {
        ...
        current = this._toLocalDateString(prev);
      }
      return streak;
    } catch (error) {
