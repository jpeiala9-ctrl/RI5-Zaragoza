...
const Gamification = {
  ...
  // Repara YA (sin esperar a la próxima sesión marcada/desmarcada) el km
  // de la zapatilla actual de un usuario ya afectado por la desviación
  // descrita en _recalcularDerivadosDesdeHistorial (v5.19). Pensada para
  // un botón manual en Perfil ("recalcular km reales"), útil para quien ya
  // tenga la zapatilla desviada y no vaya a marcar/desmarcar nada pronto.
  async repararKmZapatilla(uid) {
    if (!uid) return null;
    try {
      const derivados = await this._recalcularDerivadosDesdeHistorial(uid);
      if (!derivados || !isFinite(derivados.currentShoeKm)) {
        Utils.showToast('No se pudo recalcular el km de la zapatilla. Inténtalo de nuevo.', 'error');
        return null;
      }
      await this._fijarKmZapatilla(uid, derivados.currentShoeKm);
      Utils.showToast(`✅ Km de zapatilla recalculados: ${derivados.currentShoeKm.toFixed(1)} km`, 'success', 3500);
      return derivados.currentShoeKm;
    } catch (error) {
      console.error('Error reparando km de zapatilla:', error);
      Utils.showToast('Error al recalcular el km de la zapatilla', 'error');
      return null;
    }
  },
...
console.log('✅ gamification.js v5.19 - los km de la zapatilla actual también se recalculan desde la verdad (al marcar y al desmarcar), igual que el nivel/totalDistance -- ya no se quedan desviados');