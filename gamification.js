// ==================== gamification.js - VERSIÓN DEFINITIVA (RACHA AUTOMÁTICA Y ROBUSTA) ====================
// Versión: 3.4 - Fix racha: arranca desde ayer si hoy no hay actividad. Aritmética de fechas en hora local.
// ====================
...
const Gamification = {
  ...
  // new Date("YYYY-MM-DD") parsea como UTC midnight, lo que en zonas UTC-N
  // desplaza la fecha un día hacia atrás. Parseamos los componentes manualmente.
  _subtractOneDay(dateStr) {
    const [year, month, day] = dateStr.split('-').map(Number);
    const d = new Date(year, month - 1, day); // fecha local, sin UTC offset
  ...
  // ========== RECALCULAR RACHA SIEMPRE DESDE GLOBALFEED ==========
  async _recalculateStreak(uid) {
    ...
    try {
      ...
      // La racha no se rompe hasta que pase medianoche sin entrenar.
      let current = fechasSet.has(hoyStr)
        ? hoyStr
        : this._subtractOneDay(hoyStr);
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // Recalcular racha desde cero (siempre)
  ...
  async removeSession(uid, sesion, metricas, diaIndex) {
    ...
    try {
      ...
      const newStreak = await this._recalculateStreak(uid);
      const newLongestStreak = oldData.longestStreak; // no puede aumentar al eliminar
