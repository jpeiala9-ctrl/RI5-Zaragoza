// ==================== gamification.js - VERSIÓN 3.5 (FINAL, CON RECALCULO AUTOMÁTICO AL INICIAR) ====================
// Corrige racha: recalcula al cargar la app y siempre refresca el perfil tras marcar/desmarcar.
// ====================
...
const Gamification = {
  ...
  // ========== FUNCIÓN ROBUSTA PARA OBTENER FECHA LOCAL YYYY-MM-DD ==========
  ...
  // ========== RESTA UN DÍA A UNA FECHA YYYY-MM-DD SIN PROBLEMAS DE ZONA HORARIA ==========
  ...
  // ========== RECALCULAR RACHA SIEMPRE DESDE GLOBALFEED (USANDO FECHAS LOCALES) ==========
  async _recalculateStreak(uid) {
    if (!uid) return 0;
    try {
      ...
      // Si hoy no hay sesión, empezamos desde ayer (porque la racha se mantiene hasta que pase medianoche sin entrenar)
      ...
      console.error('Error recalculando streak:', error);
      return 0;
    }
  },

  // ========== FORZAR RECALCULO GENERAL (SE EJECUTA UNA VEZ AL INICIAR) ==========
  async _forceRecalculate(uid) {
    if (!uid) return;
    try {
      const newStreak = await this._recalculateStreak(uid);
      const userData = await this.getData(uid);
      if (userData.currentStreak !== newStreak || userData.longestStreak < newStreak) {
        ...
        console.log(`🔄 Racha forzada: ahora ${newStreak} días (máxima ${newLongest})`);
        // Si el perfil está visible, recargar
        const activeTab = document.querySelector('.tab-button.active')?.textContent.toLowerCase();
        if (activeTab === 'perfil' && window.Profile) {
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // SIEMPRE recargar el perfil después de marcar (para que se actualice la racha al volver)
      setTimeout(() => {
  ...
  async removeSession(uid, sesion, metricas, diaIndex) {
    ...
    try {
      ...
      const newStreak = await this._recalculateStreak(uid);
      const newLongestStreak = oldData.longestStreak;
      ...
      // SIEMPRE recargar el perfil después de desmarcar
      setTimeout(() => {
        if (window.Profile) Profile.cargarPerfil();
      }, 500);
...
// ========== INICIALIZACIÓN: forzar recálculo de racha al cargar la app ==========
setTimeout(async () => {
  if (AppState && AppState.currentUserId) {
    await Gamification._forceRecalculate(AppState.currentUserId);
  }
}, 3000);
