// ==================== gamification.js - VERSIÓN 5.0 (SIN RACHAS, CON ZAPATILLA ACTUAL) ====================
// Se eliminan currentStreak, longestStreak y las insignias de racha.
// Se añade "currentShoe" (objeto con nombre y km) que suma los kilómetros de cada sesión.
// ====================
...
const Gamification = {
  ...
  getDefaultData() {
    return {
      ...
      currentShoe: { name: 'Zapatilla actual', km: 0 }   // <-- NUEVO
    };
  },
  ...
  // ---------- ACTUALIZAR TRAS MARCAR SESIÓN (SIN RACHAS) ----------
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // Actualizar la zapatilla: sumar kilómetros
      ...
      const newData = {
        ...
        lastUpdate: firebaseServices.Timestamp.now()
        // currentShoe se actualiza aparte, no se toca aquí
      };
  ...
  // ---------- ACTUALIZAR TRAS DESMARCAR SESIÓN (SIN RACHAS) ----------
  async removeSession(uid, sesion, metricas, diaIndex) {
    ...
    try {
      ...
      // Restar kilómetros de la zapatilla
