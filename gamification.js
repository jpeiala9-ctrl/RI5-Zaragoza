...
// Versión: 2.5 - Racha recalculada desde globalFeed (corrección crítica)
// ====================
...
const Gamification = {
  ...
  // ========== NUEVA FUNCIÓN: recalcular racha desde globalFeed ==========
  async _recalculateStreak(uid) {
    ...
    try {
      const snapshot = await firebaseServices.db
        ...
        .where('userId', '==', uid)
        .select('timestamp')  // solo traemos timestamp para optimizar
        .get();
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // Rachas recalculadas desde globalFeed
  ...
  async removeSession(uid, sesion, metricas, diaIndex) {
    ...
    try {
      ...
      // Recalcular racha desde cero (ya no dependemos de lastSessionDate)
      const newStreak = await this._recalculateStreak(uid);
      const newLongestStreak = oldData.longestStreak; // no puede aumentar al eliminar

      // Las insignias no se eliminan, solo se mantienen las que siguen cumpliéndose
      // Para simplificar, conservamos las badges actuales (podríamos recalcular, pero no es necesario)
