// ==================== gamification.js ====================
// Versión: 5.11 - Récords sin GPS: quitado el margen del 15% (una sesión
//                de 8.5 km ya no colaba como récord de "10 km"). Ahora
//                cuenta solo si llegas AL MENOS a la distancia estándar,
//                y el tiempo se extrapola por ritmo medio a esa distancia
//                exacta (no el tiempo total si corriste de más).
...
const Gamification = {
  ...
  // Para cada distancia estándar que el recorrido llegue a cubrir, busca
  // el mejor tramo (_mejorTramo) y, si bate el récord ya guardado, lo
  // actualiza. No toca los récords que no mejora. Devuelve la lista de
  // distancias en las que se ha batido récord, para poder avisar al
  // usuario.
  async actualizarRecordsPorTramos(uid, trackPoints) {
    if (uid !== AppState.currentUserId && !AppState.isAdmin) return [];
    if (!trackPoints || trackPoints.length < 2) return [];
    if (!trackPoints[0] || typeof trackPoints[0].ts !== 'number') return []; // sin marcas de tiempo, no se puede analizar por tramos
    try {
      ...
      this.RECORD_DISTANCES.forEach(d => {
        ...
        if (!actual || tramo.durationMs < actual.durationMs) {
          personalRecords[key] = {
            ...
            distanciaKm: d,
            fecha: new Date(tramo.inicioTs).toISOString(),
            gps: true
          };
          mejorados.push(d);
        }
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // Récord por distancia estándar, sin margen: cuenta si corriste AL
      // MENOS esa distancia (antes contaba con solo quedarte dentro de un
      // 15%, así que un 8.5 podía "colarse" como récord de 10 km). El
      // tiempo que se guarda es el equivalente EXACTO a esa distancia,
      // extrapolado con tu ritmo medio de la sesión -- no el tiempo total
      // de la sesión completa (que sería más lento si corriste más de lo
      // necesario). Sin GPS no hay forma de saber el tiempo real en el km
      // exacto, así que esta extrapolación por ritmo medio es la mejor
      // aproximación posible; con GPS, el récord de verdad (con el tramo
      // real más rápido) lo pone actualizarRecordsPorTramos aparte.
      if (distance > 0 && duracionMs > 0) {
        this.RECORD_DISTANCES.forEach(d => {
          if (distance < d) return;
          const duracionEquivalente = Math.round(duracionMs * (d / distance));
          const key = String(d);
          const actual = personalRecords[key];
          if (!actual || duracionEquivalente < actual.durationMs) {
            personalRecords[key] = { durationMs: duracionEquivalente, distanciaKm: d, fecha: now.toISOString() };
          }
        });
      }
  ...
  async _recalcularDerivadosDesdeHistorial(uid) {
    ...
    try {
      ...
      snap.forEach(doc => {
        ...
        // Mismo criterio "sin margen" que updateAfterSession: cuenta si la
        // sesión llegó al menos a esa distancia, con el tiempo
        // extrapolado por ritmo medio a la distancia exacta (no el
        // margen del 15% de antes).
        if (dist > 0 && duracionMs > 0) {
          this.RECORD_DISTANCES.forEach(dr => {
            if (dist < dr) return;
            const duracionEquivalente = Math.round(duracionMs * (dr / dist));
            const rk = String(dr);
            if (!personalRecords[rk] || duracionEquivalente < personalRecords[rk].durationMs) {
              personalRecords[rk] = { durationMs: duracionEquivalente, distanciaKm: dr, fecha: fecha.toISOString() };
            }
          });
        }
      });
  ...
  async removeSession(uid, sesion, metricas, diaIndex, badgesGanadas = []) {
    ...
    try {
      ...
      if (derivados) {
        // _recalcularDerivadosDesdeHistorial reconstruye personalRecords
        // SOLO con el método antiguo (distancia total de la sesión
        // dentro de un margen del 15%), porque es lo único que puede
        // deducir de globalFeed. Los récords por TRAMO GPS (ver
        // actualizarRecordsPorTramos, marcados con gps:true) se calculan
        // aparte, con el track completo, y no se pueden reconstruir
        // desde globalFeed -- si se dejaran pisar por este recálculo,
        // desmarcar CUALQUIER sesión los borraría sin motivo. Se
        // conservan tal cual estaban y solo se completan/mejoran con lo
        // recalculado para las distancias que no tuvieran ya un récord
        // por tramo.
        const recordsPrevios = oldData.personalRecords || {};
        const recordsMezclados = { ...derivados.personalRecords };
        Object.keys(recordsPrevios).forEach(key => {
          if (recordsPrevios[key]?.gps) recordsMezclados[key] = recordsPrevios[key];
        });
        derivados.personalRecords = recordsMezclados;
