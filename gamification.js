// ==================== gamification.js - NIVEL POR KILÓMETROS (con resta de sesiones corregida) ====================
// Versión: 2.2 - Asegura la resta de sesiones al desmarcar y añade logs
// ====================
...
const Gamification = {
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      console.log(`✅ Sesión añadida: +${distance.toFixed(1)} km, +${xpGained} XP, sesiones: ${oldData.totalSessions} → ${newTotalSessions}`);
      return newData;
    } catch (error) {
  ...
  async removeSession(uid, sesion, metricas, diaIndex) {
    ...
    try {
      ...
      // Ajuste simple de racha: si la sesión desmarcada corresponde a la última fecha, decrementamos racha
      ...
      const newData = {
        ...
        totalSessions: newTotalSessions,
        lastUpdate: firebaseServices.Timestamp.now()
        // No cambiamos lastSessionDate porque puede haber otras sesiones hoy
      };
      
      await firebaseServices.db.collection('gamification').doc(uid).set(newData, { merge: true });
      
      console.log(`❌ Sesión eliminada: -${distanceRemoved.toFixed(1)} km, -${xpRemoved} XP, sesiones: ${oldData.totalSessions} → ${newTotalSessions}`);
