...
const Gamification = {
  ...
  //
  // DOS VERSIONES, una por tema: la paleta original se diseñó pensando en
  // fondos oscuros (tarjeta ~#1a1a1a) y ahí funciona bien, pero sobre el
  // tema CLARO (tarjetas casi blancas) esos mismos tonos suaves no pasan
  // el contraste mínimo de accesibilidad (WCAG AA, 4.5:1 para texto) --
  // por ejemplo nivel 5 daba apenas 2:1 sobre blanco, prácticamente
  // ilegible. La versión clara usa el mismo matiz (mismo "color base" por
  // nivel, para que se siga reconociendo cuál es cuál) pero más oscuro y
  // algo más saturado, calculado para quedar en ~4.65:1 sobre blanco.
  LEVEL_COLORS_DARK: {
  ...
  LEVEL_COLORS_LIGHT: {
    1: '#747474', 2: '#4377b3', 3: '#3c7f7a', 4: '#4a8145',
    5: '#6a7c31', 6: '#8a7226', 7: '#a46629', 8: '#b95835',
    9: '#bb4f79', 10: '#885eca'
  },
  getColorByLevel(level) {
    const isLight = (typeof document !== 'undefined') && document.body &&
      document.body.classList.contains('manual-light');
    const colors = isLight ? this.LEVEL_COLORS_LIGHT : this.LEVEL_COLORS_DARK;
  ...
  async getData(uid) {
    ...
    try {
      ...
      let data;
      if (doc.exists) {
        data = await this._limpiarRecordsNoGPS(uid, doc.data());
      } else {
        data = this.getDefaultData();
        await firebaseServices.db.collection('gamification').doc(uid).set(data);
        console.log('✅ Documento de gamificación creado para', uid);
      }
      return this._aplicarNivelAdmin(uid, data);
    } catch (error) {
  ...
  // ==================================================================
  //  ADMIN = NIVEL 10 SIEMPRE (guiño de gamificación de "máximo nivel")
  // ==================================================================
  //
  // El nivel del admin NO se guarda falseado en Firestore: su km/XP real
  // se sigue calculando y guardando con total normalidad (ver
  // updateAfterSession/removeSession), por si algún día deja de ser
  // admin y su progreso real debe volver a aparecer solo. Lo que se hace
  // aquí es sobrescribir el campo 'level' SOLO en lo que se devuelve a
  // pantalla, y SOLO para el propio usuario admin que ha iniciado sesión
  // ahora mismo (no para cómo ven otros usuarios cosas ajenas). Como el
  // color de nivel, la insignia y las gráficas de todos los sitios parten
  // de este 'level', el morado de nivel 10 llega solo con este cambio.
  _aplicarNivelAdmin(uid, data) {
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // Admin = nivel 10 fijo (ver _aplicarNivelAdmin más abajo para el
      // detalle); el resto de esta función sigue calculando distancia/XP
      // reales con total normalidad, solo el campo 'level' que se guarda
      // se fuerza aquí para que no dependa de que alguien vuelva a leer
      // los datos para que se vea bien.
      const newLevel = (uid === AppState.currentUserId && AppState.isAdmin)
        ? 10
        : this.getLevelByDistance(newTotalDistance);
...
console.log('✅ gamification.js v5.13 - Fix zona horaria en racha (updateAfterSession) + blindaje de marcado fuera de orden');