// ==================== gamification.js - NIVEL POR KILÓMETROS (con racha local) ====================
// Versión: 2.7 - Depuración forzada de racha + recarga explícita
// ====================
...
const Gamification = {
  ...
  // ========== RECALCULAR RACHA USANDO FECHA LOCAL (CON LOGS) ==========
  async _recalculateStreak(uid) {
    if (!uid) return 0;
    try {
      console.log('🔁 Recalculando racha para', uid);
      ...
      console.log('📅 Fechas de entrenamiento encontradas:', Array.from(fechasSet));
      ...
      while (fechasSet.has(current)) {
        ...
        current = prev.toLocaleDateString('en-CA');
      }
      console.log(`✅ Racha calculada: ${streak} días`);
      return streak;
    } catch (error) {
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // Forzar actualización del perfil si está visible
      setTimeout(() => {
