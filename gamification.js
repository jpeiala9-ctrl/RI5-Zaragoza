...
const Gamification = {
  ...
  // getCached(uid): lectura SÍNCRONA e instantánea desde sessionStorage.
  // La app ya escribía `gamification_${uid}` en sessionStorage en cada
  // entrada (login, sesión guardada...) y tras cada sesión marcada, pero
  // nada la leía nunca -- era una caché de solo escritura. Esto es lo que
  // permite que un modal (récords del perfil, p.ej.) se pinte con datos
  // reales la primera vez que se abre, sin pasar antes por un "Cargando…"
  // si la app ya lleva un rato abierta.
  getCached(uid) {
    if (!uid) return null;
    try {
  ...
  async getData(uid) {
    ...
    try {
      ...
      // Mantiene la caché de sessionStorage siempre al día con lo último
      // que se ha leído de Firestore, para que getCached() nunca sirva
      // datos más viejos que la última vez que se llamó a getData().
      try { sessionStorage.setItem(`gamification_${uid}`, JSON.stringify(data)); } catch (e) {}
      return data;
    } catch (error) {
      console.error('Error obteniendo datos de gamificación:', error);
      // Sin red: mejor devolver la última copia conocida que los valores
      // por defecto (que borrarían de la vista el nivel/XP/insignias
      // reales del usuario solo por un fallo temporal de conexión).
      return this.getCached(uid) || this.getDefaultData();
    }
  },
  ...
  async updateAfterSession(uid, sesion, metricas) {
    ...
    try {
      ...
      // Refresca la caché de sessionStorage que usan los modales (récords,
      // carga y recuperación...) para que la próxima vez que se abran ya
      // tengan el dato bueno sin esperar. Como el guardado de arriba es un
      // merge parcial, no se reconstruye la caché a mano aquí -- se relee
      // de Firestore en segundo plano (no bloquea el resto de esta función).
      this.getData(uid).catch(() => {});
  ...
  async removeSession(uid, sesion, metricas, diaIndex, badgesGanadas = []) {
    ...
    try {
      ...
      await firebaseServices.db.collection('gamification').doc(uid).set(newData, { merge: true });
      // Mismo motivo que en updateAfterSession: refresca la caché en
      // segundo plano para que los modales sigan mostrando datos al día.
      this.getData(uid).catch(() => {});
