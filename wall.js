...
const Wall = {
      ...
      try {
        ...
        html += `
          <div class="wall-item" data-entry-id="${entry.id}">
            ...
            <div class="wall-stats-box" data-entry-id="${entry.id}" style="border:1px solid var(--border-color); border-radius:12px; padding:14px; text-align:center; background:var(--bg-primary); margin-top:4px; cursor:pointer;">
  ...
  _handleItemClick(e) {
    ...
    // Pulsar el recuadro de estadísticas (tipo/duración/km/TSS/zona) abre
    // el detalle real de la sesión -- con su desglose calentamiento/parte
    // principal/enfriamiento si lo tiene, ver
    // abrirDetalleEntrenamientoCompletado en index.html -- en vez de la
    // lista de "me gusta" (que sigue abriéndose al pulsar el resto de la
    // tarjeta, como antes).
    const statsBox = e.target.closest('.wall-stats-box');
    if (statsBox) {
      const entryId = statsBox.dataset.entryId;
