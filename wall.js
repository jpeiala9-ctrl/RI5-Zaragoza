...
const Wall = {
  ...
  _renderMiniMap(trackPoints) {
    ...
    // Fallback local si GPSTracker no está cargado aún
        ...
        if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
          ...
          if (mapSVG) {
            const distReal = entry.gpsDistanceKm
              ? `<span style="color:#9BB5A0; font-weight:600;">📍 ${Number(entry.gpsDistanceKm).toFixed(2)} km GPS</span>`
              : '';
            miniMapHTML = `
              <div style="margin-top:10px; border-radius:10px; overflow:hidden; border:1px solid #2a2a2a;">
                ${mapSVG}
              </div>
              ${distReal ? `<div style="text-align:center; font-size:12px; margin-top:6px;">${distReal}</div>` : ''}
            `;
          }
        }
