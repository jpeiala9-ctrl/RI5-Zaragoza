// ==================== wall.js ====================
// Versión: 4.10 - No mostrar minimapa si distancia < 0.1 km (100 m)
// ====================
...
const Wall = {
  ...
  render(entries) {
    ...
    for (const entry of entries) {
      try {
        ...
        let miniMapHTML = '';
        // Solo mostrar minimapa si la distancia GPS es >= 0.1 km (100 m)
        const gpsDist = entry.gpsDistanceKm || entry.distancia;
        const distanciaNumerica = parseFloat(gpsDist);
        if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2 && distanciaNumerica >= 0.1) {
  ...
  async _handleMinimapClick(e) {
    ...
    const distanciaRaw = container.dataset.dist;
    const tiempo = container.dataset.tiempo;
    const ritmo = container.dataset.ritmo;
    const lat = parseFloat(container.dataset.lat);
    const lng = parseFloat(container.dataset.lng);
    
    const distancia = parseFloat(distanciaRaw).toFixed(2);
  ...
  abrirModalMapa(points, distancia, entryData = {}) {
    ...
    let ritmoFormateado = '—';
    if (entryData.ritmo && entryData.ritmo !== '—' && !entryData.ritmo.includes('2727')) {
      const partes = entryData.ritmo.split(':');
      if (partes.length === 2 && parseInt(partes[0]) < 20) {
        ritmoFormateado = `${entryData.ritmo} min/km`;
      }
    }
...
window.Wall = Wall;
console.log('✅ Wall v4.10 - No mostrar minimapa si distancia < 0.1 km');