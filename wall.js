// ==================== wall.js ====================
// Versión: 4.17 - FIX mini-mapa del track GPS desaparecido. Causa: al
//                terminar una sesión con GPS, calendar.js hace primero
//                un add() del entry con hasGPS:true pero SIN trackPoints
//                (los puntos GPS se añaden con un update() posterior,
//                hecho desde gps-tracker.js después de calcular los
//                récords por tramo). Desde v4.16, el muro decide no
//                repintar si "no ha cambiado nada" comparando solo id/
//                likeCount/likes.length (_sonIgualesEntradas), así que
//                no se enteraba de que el update() posterior con los
//                trackPoints ya había llegado, y la tarjeta se quedaba
//                sin mini-mapa, sin capa de tap, y sin poder abrir el
//                modal del visor GPS al pulsarla. Ahora _sonIgualesEntradas
//                compara también hasGPS/trackPoints.length/mapSnapshot.
...
const Wall = {
  ...
  // 🔥 v4.17: compara dos tandas de entradas del muro de forma LIGERA
  // (id + likeCount + likes.length + hasGPS + trackPoints.length +
  // mapSnapshot). Con esto, cuando el listener onSnapshot dispara por
  // cualquier motivo (una reconexión tras el móvil bloqueado, un
  // visibilitychange, un snapshot idéntico al anterior...), pero el
  // contenido visible no ha cambiado realmente, no se reconstruye el
  // HTML -- que es exactamente lo que se veía como "parpadeo" al
  // volver a la pestaña Muro.
  //
  // 🔥 v4.17: se añaden los tres campos de GPS. MOTIVO REAL: al terminar
  // una sesión con GPS, el guardado se hace en DOS escrituras separadas
  // sobre el mismo documento de globalFeed. Primero calendar.js hace un
  // add() con hasGPS:true pero SIN trackPoints (los puntos GPS todavía
  // no están calculados). Más tarde, _guardarYPublicar() en gps-tracker.js
  // hace un update() posterior añadiendo trackPoints, gpsDistanceKm,
  // gpsDurationMs y recordsPorTramo. Sin comparar trackPoints en esta
  // función, _sonIgualesEntradas devolvía true (mismos ids y contadores
  // de likes), el muro NO repintaba, y la tarjeta se quedaba sin el
  // mini-mapa del track (que se construye justo a partir de
  // entry.trackPoints) y sin la capa de tap que abre el visor GPS al
  // pulsarla. Comparando los tres campos, el segundo snapshot (el del
  // update) ya se detecta como "cambio real" y el muro repinta la
  // tarjeta con su mini-mapa y su capa de tap.
    ...
    this._unsubscribe = firebaseServices.db
      ...
      .onSnapshot(async (snapshot) => {
        ...
        try {
          ...
          // mismos datos de GPS), no se toca el DOM. Sin esto, cada
          // reconexión del listener reconstruía el HTML entero con las
          // mismas entradas, y eso se veía como un parpadeo al entrar en
          // la pestaña.
  ...
  async _crearMiniMapa(containerId, trackPoints, entryId = null, esPropia = false) {
    ...
    const map = window.L.map(container, {
      ...
      boxZoom: false,
      tap: false
    }).setView([íncenter.lat, center.lng], 13);
    ...
    // la ruta (por si el tamaño cambió un pel entre el rAF y ahora,
...
window.Wall = Wall;
console.log('✅ Wall v4.17 - Fix mini-mapa del track GPS: _sonIgualesEntradas compara también hasGPS/trackPoints/mapSnapshot para detectar el update posterior que añade los puntos GPS');