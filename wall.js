// ==================== wall.js - Muro global con carga manual, refresco periódico, actualización local de likes y mapas GPS ====================
// Versión: 3.2 - Añadido soporte para mostrar rutas GPS con Leaflet
// ====================
...
const Wall = {
  ...
  render(entries) {
    ...
    // Inicializar mapas para las entradas que tienen ruta
    if (window.GPSTracker) {
      for (const entry of entries) {
        if (entry.gpsRoute && entry.gpsRoute.points && entry.gpsRoute.points.length >= 2) {
          ...
          if (mapContainer) {
            GPSTracker.displayRoute(mapId, entry.gpsRoute.points, { showMarkers: true });
          }
        }
      }
    }
