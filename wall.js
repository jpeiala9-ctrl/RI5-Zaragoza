...
// Versión: 1.5.2 - Pastillas de zona coloreadas (zona-pastilla) sin cambios en el HTML
// ====================
  ...
  render(entries) {
    ...
    for (const entry of entries) {
      try {
        ...
        // Construir pastilla de zona coloreada
        let zonaPastillaHTML = '';
        if (zone) {
          const zonaSimple = zone.split('-')[0].trim().toLowerCase(); // p.ej. "z2"
          zonaPastillaHTML = `<span class="zona-pastilla ${zonaSimple}"><span></span> ${Utils.escapeHTML(zone)}</span>`;
        }

        // Métricas básicas (siempre)
        let detallesHTML = `<span>⏱️ ${duracion}'</span>
        ...
        html += `
          <div class="wall-item" data-entry-id="${entry.id}">
            ...
            <div class="wall-entreno">
              <div class="wall-entreno-tipo">${tipoEmoji} ${tipoMostrado}</div>
              <div class="wall-entreno-detalles">
                ${detallesHTML}
              </div>
              ${zonaPastillaHTML ? `<div style="display:flex; align-items:center; gap:8px; margin-top:6px; flex-wrap:wrap;">
                ${zonaPastillaHTML}
                ${hora ? `<span style="font-size:12px; color:var(--text-secondary);">🕒 ${hora}</span>` : ''}
              </div>` : ''}
            </div>
