// ==================== wall.js ====================
// Versión: 5.0 - Miniaturas con caché persistente (localStorage) y expiración de 24 horas
// ====================
...
const Wall = {
  ...
  // ========== GENERAR MINIATURA CON CACHÉ Y EXPIRACIÓN ==========
  async _getOrGenerateThumbnail(entryId, trackPoints) {
    const cacheKey = `map_thumb_${entryId}`;
    const now = Date.now();
    ...
    try {
      ...
      if (item) {
        const { imageData, timestamp } = JSON.parse(item);
        if (now - timestamp < twentyFourHours) {
          cached = imageData; // Válido, usar caché
        } else {
          localStorage.removeItem(cacheKey); // Eliminar expirado
        }
      }
    } catch (e) {
    ...
    const imageData = await this._generarMiniaturaMapa(entryId, trackPoints);
    if (imageData) {
      try {
        ...
        console.warn('No se pudo guardar en localStorage (posiblemente lleno)', e);
        // Si está lleno, intentar limpiar algunas entradas antiguas
        this._limpiarCacheAntigua();
        // Reintentar guardar
        try {
          localStorage.setItem(cacheKey, JSON.stringify({
            imageData: imageData,
  ...
  // Limpiar cachés de más de 24 horas para liberar espacio
  _limpiarCacheAntigua() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith('map_thumb_')) {
        try {
          const item = JSON.parse(localStorage.getItem(key));
          if (now - item.timestamp >= twentyFourHours) {
    ...
    console.log(`🗑️ Limpiadas ${keysToRemove.length} miniaturas expiradas del caché`);
  },

  async _generarMiniaturaMapa(entryId, trackPoints) {
    // Asegurar que Leaflet está cargado
    if (!window.L) {
    ...
    // Inicializar mapa con el mismo tile layer que GPSTrackViewer
    const map = window.L.map(container).setView([trackPoints[0].lat, trackPoints[0].lng], 13);
    ...
    window.L.polyline(latlngs, { color: '#c0a060', weight: 5 }).addTo(map);
    map.fitBounds(window.L.polyline(latlngs).getBounds(), { padding: [20, 20] });

    // Esperar a que los tiles carguen (mínimo 500ms)
    await new Promise(resolve => setTimeout(resolve, 500));

    // Cargar html2canvas si no está presente
    if (typeof html2canvas === 'undefined') {
      await this._cargarHtml2Canvas();
    }
  ...
  async _renderThumbnail(containerDiv, entry) {
    const entryId = entry.id;
    const trackPoints = entry.trackPoints;
    if (!trackPoints || trackPoints.length < 2) return;

    // Mostrar placeholder "Cargando..."
    containerDiv.innerHTML = '<div style="color:#888; padding:20px; text-align:center;">Cargando mapa...</div>';

    // Obtener imagen desde caché o generarla
    const imgSrc = await this._getOrGenerateThumbnail(entryId, trackPoints);
    if (imgSrc) {
      containerDiv.innerHTML = `<img src="${imgSrc}" style="width:100%; display:block;">`;
      // Añadir overlay de "VER RECORRIDO"
      const overlay = document.createElement('div');
      overlay.style.cssText = 'position:absolute; inset:0; display:flex; flex-direction:column; align-items:center; justify-content:flex-end; padding-bottom:10px; pointer-events:none;';
      overlay.innerHTML = `<div style="background:rgba(0,0,0,0.72); color:#c0a060; font-size:10px; letter-spacing:1.5px; padding:5px 14px; border-radius:20px; border:1px solid rgba(192,160,96,0.35); font-family:'Courier New',monospace;">🗺 VER RECORRIDO</div>`;
      containerDiv.appendChild(overlay);
      // Evento de clic para abrir GPSTrackViewer
      containerDiv.addEventListener('click', (e) => {
        e.stopPropagation();
        if (window.GPSTrackViewer) GPSTrackViewer.open(entry);
      });
    } else {
      containerDiv.innerHTML = '<div style="color:#888; padding:10px;">Error al cargar mapa</div>';
    }
  },
  ...
  render(entries) {
    ...
    for (const entry of entries) {
      try {
        ...
        // Contenedor para el minimapa (placeholder)
        let miniMapContainer = '';
        if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
          miniMapContainer = `
            <div class="gps-minimap-container" data-entry-id="${entry.id}" style="margin-top:10px; border-radius:10px; overflow:hidden; border:1px solid #2a2a2a; cursor:pointer; position:relative; min-height:130px; background:#1a1a1a;">
            </div>
          `;
        }
    ...
    // Cargar miniaturas (con caché) para cada entrada
    for (const entry of entries) {
      if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
        const containerDiv = this.container.querySelector(`.gps-minimap-container[data-entry-id="${entry.id}"]`);
        if (containerDiv) {
          this._renderThumbnail(containerDiv, entry);
        }
      }
    }

    // Limpiar caché de miniaturas que ya no están en el muro (para ahorrar espacio)
    this._limpiarCachesHuérfanas(entries.map(e => e.id));
  ...
  // Limpiar cachés de entradas que ya no están en el muro
  _limpiarCachesHuérfanas(activeEntryIds) {
    const activeSet = new Set(activeEntryIds);
    const keysToRemove = [];
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('map_thumb_')) {
        const entryId = key.replace('map_thumb_', '');
        if (!activeSet.has(entryId)) {
          keysToRemove.push(key);
        }
      }
    }
    ...
    if (keysToRemove.length > 0) {
      console.log(`🗑️ Limpiadas ${keysToRemove.length} miniaturas de entradas eliminadas del muro`);
    }
  },
  ...
  _handleItemClick(e) {
    ...
    if (e.target.closest('.gps-minimap-container')) return;
...
window.Wall = Wall;
console.log('✅ Wall v5.0 - Miniaturas con caché persistente, expiración 24h y limpieza automática');