// ==================== wall.js - Muro global de entrenamientos (CORREGIDO Y FUNCIONAL) ====================
// Versión: 1.6 - Carga manual como fallback y recarga forzada
// ====================
...
const Wall = {
  ...
  initListener() {
    ...
    this.unsubscribe = query.onSnapshot((snapshot) => {
      ...
      console.log('📱 Muro actualizado en tiempo real:', entries.length, 'entradas');
      this.render(entries);
    }, (error) => {
      console.error('Error en listener del muro:', error);
      // Si falla el tiempo real (ej. permisos), cargamos una vez
      this.cargarManual();
    });
  },
  ...
  async cargarManual() {
    try {
      ...
      console.log('📱 Muro (carga manual):', entries.length, 'entradas');
      this.render(entries);
    } catch (error) {
