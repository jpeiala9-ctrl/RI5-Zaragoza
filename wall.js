// ==================== wall.js ====================
// Versión: 4.20 - Fundido de entrada para las fotos de usuario del Muro (a
//                petición del usuario): antes la <img> del avatar aparecía
//                de golpe en cuanto terminaba de descargar (más notorio en
//                redes lentas, daba sensación de "problema de carga" en
//                vez de parte normal de la app). Ahora lleva la clase
//                .wall-avatar-loading (opacidad 0, ver CSS en index.html)
//                y su propio evento onload le añade .wall-avatar-loaded,
//                que sube la opacidad a 1 con una transición de 0.45s. Si
//                la imagen falla antes de cargar, Utils.avatarFallback()
//                (app.js v4.67) quita esas dos clases al sustituirla por
//                el emoji de repuesto, para que este no se quede invisible.
// Versión: 4.19 - FIX "sesión fantasma": el listener en tiempo real se
//                queda enganchado de fondo aunque cambies de pestaña. Si
//                marcabas una sesión como hecha estando en otra pestaña
//                (Entreno, p.ej.), la nueva publicación llegaba al
//                listener, pero como el contenedor del Muro estaba oculto
//                en ese momento, la v4.18 decidía no pintarla todavía (para
//                no crear mal los mini-mapas sobre un display:none) -- solo
//                actualizaba currentEntries en memoria. El problema: al
//                entrar luego en Comunidad → Muro, init() solo repintaba si
//                el contenedor estaba VACÍO; si ya se había visitado antes
//                (tenía tarjetas viejas), no estaba vacío, así que se
//                quedaba tal cual con las tarjetas antiguas y la sesión
//                nueva nunca llegaba a pintarse (hasta el siguiente cambio
//                real en Firestore o una recarga manual). Se añade el flag
//                _renderPendiente: el listener lo marca a true cuando se
//                salta el pintado por estar oculto, render() lo resetea en
//                cuanto pinta de verdad, e init() ahora repinta también
//                cuando ese flag está activo, no solo cuando el contenedor
//                está vacío.
...
const Wall = {
  ...
  currentEntries: [],
  _renderPendiente: false, // 🔥 v4.19: true cuando un snapshot del listener
                          // trajo cambios reales mientras el contenedor
                          // estaba oculto (ver iniciarListener()) y por eso
                          // NO se pintó -- currentEntries ya está al día,
                          // pero el DOM se quedó con lo que hubiera antes.
                          // init() lo usa para saber que tiene que repintar
                          // aunque el contenedor ya tuviera tarjetas de una
                          // visita anterior (ver FIX "sesión fantasma").
  container: null,
  isActive: false,
  loading: false,
  ...
  init() {
    ...
    // 🔥 FIX "sesión fantasma" (v4.19): antes solo se repintaba aquí si el
    // contenedor estaba vacío (primerísima visita de la sesión). Pero si ya
    // se había visitado el Muro antes y luego llegó una actualización real
    // con la pestaña oculta (ver iniciarListener(), _renderPendiente), el
    // contenedor NO estaba vacío -- tenía las tarjetas viejas -- así que
    // nunca se repintaba y la sesión nueva no aparecía hasta el siguiente
    // cambio real en Firestore o una recarga manual. Ahora también se
    // repinta si queda un render pendiente de cuando estaba oculto.
    if ((this.container.children.length === 0 || this._renderPendiente) && this.currentEntries.length > 0) {
      ...
      .onSnapshot(async (snapshot) => {
        ...
        try {
          ...
          if (this.container && this.container.offsetParent !== null) {
            ...
            // 🔥 FIX "sesión fantasma" (v4.19): currentEntries ya está al
            // día, pero como el contenedor está oculto no se pinta ahora.
            // Sin esto, init() (más abajo) solo repintaba cuando el
            // contenedor estaba vacío -- si ya se había visitado el Muro
            // antes (tiene tarjetas viejas), no está vacío, así que se
            // quedaba tal cual con las tarjetas antiguas y esta
            // actualización nunca llegaba a pintarse hasta el siguiente
            // cambio real o una recarga manual.
            this._renderPendiente = true;
          }
        } catch (e) {
  ...
  _buildEntryHTML(entry) {
    try {
      ...
      const avatarHTML = entry.photoURL
        ? `<img src="${Utils.escapeHTML(entry.photoURL)}" class="wall-avatar wall-avatar-loading" style="object-fit:cover; border:2px solid ${colorNivelAutor};" onload="this.classList.add('wall-avatar-loaded')" onerror="Utils.avatarFallback(this)">`
  ...
  render(entries) {
    if (!this.container) return;
    this._renderPendiente = false; // 🔥 v4.19: ya se pinta de verdad, no queda nada colgado
    document.getElementById('wallCargarMasContainer')?.remove();
    ...
    try {
      ...
      if (data.userId === AppState.currentUserId) {
        ...
        if (likeCount > likesLeidos) {
          ...
          // 🔥 FIX: querySelector solo quitaba el punto de la PRIMERA
          // tarjeta que encontrara con ese entry-id. Si la misma entrada
          // está montada dos veces en el DOM (la del Muro y la de "Mis
          // últimos entrenamientos" en Perfil, aunque una esté oculta),
          // se quedaba el punto rojo en la otra. Ahora se quitan todas.
          document.querySelectorAll(`.wall-item[data-entry-id="${entryId}"] .like-new-dot`).forEach(dot => dot.remove());
        }
      }
...
window.Wall = Wall;
console.log('✅ Wall v4.18 - precargarMuro() añadida (faltaba): el muro se precarga de verdad en el arranque, sin parpadeo al entrar');