// ==================== wall.js - Muro OPTIMIZADO (carga manual con caché, sin onSnapshot) ====================
// Versión: 3.0 - Sin listener en tiempo real, recarga cada 30 segundos solo si la pestaña está activa.
// ====================
...
const Wall = {
  ...
  // Inicializa el muro (sin listener en tiempo real)
  initListener() {
    this.cargarMuro(); // carga inmediata
    // Configurar refresco periódico
    if (this.refreshTimer) clearInterval(this.refreshTimer);
    this.refreshTimer = setInterval(() => {
      // Solo refrescar si la pestaña del muro está activa
      const muroTab = document.getElementById('tab-muro');
      if (muroTab && muroTab.classList.contains('active')) {
        this.cargarMuro(true);
      }
    }, this.refreshInterval);
  },

  async cargarMuro(forceRefresh = false) {
    const container = document.getElementById('wallContainer');
    if (!container) return;

    const now = Date.now();
    if (!forceRefresh && (now - this.lastRefresh) < this.refreshInterval) {
      // si se pide refrescar demasiado pronto, ignorar (pero permitir si es la primera carga)
      if (this.lastRefresh !== 0) return;
    }
    ...
    try {
      ...
      container.innerHTML = '<p style="text-align:center; padding:40px; color:var(--zone-5);">Error al cargar el muro. Inténtalo de nuevo.</p>';
    }
  },
  ...
  async toggleLike(entryId) {
    ...
    try {
      ...
      // Actualizar el contador visualmente sin recargar todo
      const btn = document.querySelector(`.wall-like-btn[data-entry-id="${entryId}"]`);
      if (btn) {
        const newCount = userLiked ? likeCount - 1 : likeCount + 1;
        btn.querySelector('.like-count').textContent = newCount;
        if (userLiked) btn.classList.remove('liked');
