...
// Versión: 4.6 - Apertura garantizada del visor, sin toasts erróneos
// ====================
    ...
    this.container.querySelectorAll('.gps-minimap-tap').forEach(el => {
      el.addEventListener('click', (e) => {
        ...
        if (entry && window.GPSTrackViewer) {
          ...
          // Fallback: intentar cargar dinámicamente o mostrar error claro
          if (window.GPSTrackViewer) {
            console.warn('Visor no disponible?', window.GPSTrackViewer);
          }
          Utils.showToast('Error al abrir el visor', 'error');
        }
      });
    });
  },
