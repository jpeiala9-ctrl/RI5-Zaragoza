...
const Wall = {
  ...
  _handleItemClick(e) {
    ...
    // de la sesión -- calentamiento/parte principal/enfriamiento, cada
    // uno con su propia zona real (ver _desglosarSesionReal en
    // calendar.js) -- en vez de la lista de "me gusta", que sigue siendo
    // lo que abre el resto de la tarjeta.
    if (e.target.closest('.wall-stats-box')) {
      const entry = (this.currentEntries || []).find(en => en.id === entryId);
      if (entry && window.construirSesionObjDesdeEntry && window.abrirModalDetalleSesion) {
        window.abrirModalDetalleSesion(window.construirSesionObjDesdeEntry(entry), true);
        return;
      }
    }
