...
// Versión: 4.4 - Sin emojis, minimapa claro, prioridad GPS, visor funcional
// ====================
...
const Wall = {
  ...
  // ===== MINIMAP SVG desde trackPoints (con fondo claro y línea ajustable) =====
    ...
    for (const entry of entries) {
      try {
        ...
        const gpsBadge = entry.hasGPS
          ? `<span style="font-size:10px; font-weight:600; letter-spacing:1px; color:#1e88e5; background:rgba(30,136,229,0.12); border:1px solid rgba(30,136,229,0.3); border-radius:20px; padding:2px 8px; margin-left:6px;">GPS</span>`
          : '';

        // Minimap solo si hay trackPoints con al menos 2 puntos
        let miniMapHTML = '';
        if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
          ...
          if (mapSVG) {
            const distReal = entry.gpsDistanceKm
              ? Number(entry.gpsDistanceKm).toFixed(2) + ' km'
              : '';
            miniMapHTML = `
