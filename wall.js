// ==================== wall.js ====================
// Versión: 4.3 - Definitiva: minimapa fiable y visor integrado
// ====================
...
const Wall = {
  ...
  _renderMiniMap(trackPoints) {
    if (!trackPoints || trackPoints.length < 2) {
      console.warn('_renderMiniMap: puntos insuficientes', trackPoints);
      return '';
    }
    let svg = '';
    if (window.GPSTracker && typeof GPSTracker.renderTrackSVG === 'function') {
      console.log('Llamando a GPSTracker.renderTrackSVG con', trackPoints.length, 'puntos');
      svg = GPSTracker.renderTrackSVG(trackPoints, 320, 130);
      if (svg && svg.trim() !== '') {
        return svg;
      } else {
        console.warn('GPSTracker.renderTrackSVG devolvió cadena vacía, usando fallback');
      }
    } else {
      console.warn('GPSTracker.renderTrackSVG no está disponible');
    }
  ...
  _miniMapFallback(points) {
    ...
    return `<svg width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"
      style="border-radius:10px; background:#eaeaea; display:block; width:100%;"
...
window.Wall = Wall;
console.log('✅ Wall v4.3 - Definitivo para el muro');