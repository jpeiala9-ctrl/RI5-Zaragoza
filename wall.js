...
const Wall = {
  ...
  iniciarActualizacionPeriodica() {
    ...
    this.refreshInterval = setInterval(() => {
      // Si el usuario ya ha pedido más páginas (currentEntries > 20), no
      // se refresca solo: eso reemplazaría todo por la primera página de
      // nuevo y le haría perder tanto lo cargado de más como la posición
      // del scroll mientras está leyendo publicaciones más antiguas.
      if (document.querySelector('#subtab-muro')?.classList.contains('active') && this.currentEntries.length <= 20) {
        this.cargarMuro();
      }
    }, 30000);
  },
  ...
  // queda algo más por cargar dentro del periodo hoy/ayer.
  ...
  // dentro del mismo periodo hoy/ayer) y los añade al final del muro sin
...
window.Wall = Wall;
console.log('✅ Wall v4.12 - centrarPublicacion() con reintentos, ya no depende de un setTimeout fijo');