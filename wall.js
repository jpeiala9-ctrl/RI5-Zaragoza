// ==================== wall.js ====================
// Versión: 4.18 - FIX "el mini-mapa GPS no aparece NUNCA en el Muro tras
//                completar una sesión (ni recargando la pestaña más
//                tarde, se queda así)": _sonIgualesEntradas (fix de v4.17
//                de aquí abajo) asumía que hasGPS pasaba de false a true
//                en la segunda escritura de la sesión. En realidad
//                calendar.js ya guarda hasGPS:true en la PRIMERA
//                escritura, antes de tener trackPoints; gps-tracker.js
//                añade trackPoints después en un update() aparte, sin
//                tocar hasGPS. Como ese booleano no cambiaba de valor
//                entre snapshots, el fix de v4.17 no detectaba nada y el
//                Muro nunca volvía a pintar esa tarjeta -- así que nunca
//                llegaba a crear su mini-mapa Leaflet en vivo, y como ese
//                mini-mapa es el que genera y cachea mapSnapshot, el
//                Perfil tampoco lo recibía nunca y se quedaba mostrando
//                solo el esquema SVG de la ruta (el "mapa en blanco").
//                Ahora _sonIgualesEntradas también compara la longitud de
//                trackPoints.
// Versión: 4.17 - FIX "el mapa GPS no aparecía en el Muro hasta recargar
//                la página": la comparación ligera de _sonIgualesEntradas
//                (v4.16) solo miraba id/likeCount/likes.length, así que la
//                segunda escritura que añade hasGPS/trackPoints/mapSnapshot
//                a una sesión recién publicada (ver gps-tracker.js, que
//                guarda el track en un paso aparte, unos instantes después
//                de crear la entrada) se consideraba "sin cambios" y nunca
//                se repintaba. Ahora también se comparan hasGPS y
//                mapSnapshot.
...
const Wall = {
  ...
  _sonIgualesEntradas(a, b) {
    ...
    for (let i = 0; i < a.length; i++) {
      ...
      // 🔥 FIX "el mapa GPS no aparece hasta recargar la página": una
      // sesión con GPS escribe el documento en 2 pasos -- primero se
      // publica SIN datos de GPS, y unos instantes después (ver
      // gps-tracker.js) se le añaden hasGPS/trackPoints/mapSnapshot con un
      // segundo update(). Sin comprobar esto aquí, esa segunda escritura
      // se consideraba "sin cambios visibles" (mismo id, mismo likeCount)
      // y nunca se llegaba a llamar a render() con los datos ya completos.
      if (!!a[i].hasGPS !== !!b[i].hasGPS) return false;
      if (!!a[i].mapSnapshot !== !!b[i].mapSnapshot) return false;
      // 🔥 v4.18: FIX "el mini-mapa no aparece NUNCA en el Muro tras hacer
      // una sesión GPS (hasta recargar la página)": el fix de v4.17 de
      // arriba asumía que hasGPS pasaba de false a true en ese segundo
      // update(). En la práctica, calendar.js ya escribe hasGPS:true en la
      // PRIMERA escritura (en cuanto sabe que la sesión fue con GPS, antes
      // incluso de tener el recorrido) -- así que hasGPS no cambia de
      // valor entre snapshots y el fix anterior no detectaba nada. El
      // segundo update() de gps-tracker.js lo único que añade de verdad es
      // el array trackPoints, así que hay que comparar también eso: si su
      // longitud cambia (típicamente de 0/ausente a N puntos), sí hay un
      // cambio visible real y hay que repintar. Sin esto, el Muro nunca
      // llegaba a crear el mini-mapa Leaflet en vivo para esa entrada, y
      // como es ESE mini-mapa el que genera y cachea mapSnapshot (ver
      // _capturarYCachearMiniMapa), el Perfil tampoco recibía nunca la
      // imagen real del mapa y se quedaba enseñando solo el esquema SVG
      // de la ruta (el "mapa en blanco").
      if ((a[i].trackPoints?.length || 0) !== (b[i].trackPoints?.length || 0)) return false;
    }
    return true;
  },
...
window.Wall = Wall;
console.log('✅ Wall v4.18 - FIX mini-mapa GPS que no aparecía nunca en el Muro: _sonIgualesEntradas ahora también compara la longitud de trackPoints');