...
const Wall = {
  ...
  async _crearMiniMapa(containerId, trackPoints, entryId = null, esPropia = false) {
    ...
    // Tile layer Esri "Light Gray Canvas" (antes CartoDB, que empezó a
    // exigir clave de API; los tiles estándar de OSM probados como
    // primer arreglo eran demasiado recargados -- ver nota en
    // gps-tracker.js/gps-track-viewer.js). crossOrigin:true es necesario
    // para poder "leer" luego el mapa como imagen (canvas) y cachearlo --
    // sin esto el navegador bloquea la lectura del canvas por seguridad
    // (CORS), aunque el mapa se vea perfectamente en pantalla. Los
    // servicios de Esri sirven CORS abierto, pero conviene comprobar tras
    // desplegar que el cacheado del mini-mapa (miniatura del Muro) sigue
    // funcionando igual que antes.
    const tileLayer = window.L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}', {
      maxZoom: 16,
      crossOrigin: true,
