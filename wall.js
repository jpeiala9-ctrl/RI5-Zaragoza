...
// Versión: 4.1 - GPS track minimap clickeable + modal ampliado
// ====================
...
const Wall = {
    ...
    for (const entry of entries) {
      try {
        ...
        if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
          ...
          if (mapSVG) {
            miniMapHTML = `
              <div class="track-minimap" data-points='${JSON.stringify(entry.trackPoints)}' data-dist="${entry.gpsDistanceKm || entry.distancia}" style="margin-top:10px; border-radius:10px; overflow:hidden; border:1px solid #2a2a2a; cursor:pointer;">
                ${mapSVG}
              </div>
            `;
          }
  ...
  abrirModalMapa(points, distancia) {
    ...
    contenido.style.cssText = `
      background: #0a0a0a; width: 95%; max-width: 500px;
    ...
    contenido.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
        ...
        <button id="closeMapModal" style="background:none; border:none; color:#aaa; font-size:24px; cursor:pointer;">✕</button>
      </div>
      <div id="mapaAmpliado" style="height: 300px; width: 100%; border-radius: 12px; margin-bottom: 16px;"></div>
      <div class="stats" style="text-align:center;">
        <div>📏 Distancia: <strong>${distancia} km</strong></div>
      </div>
    `;
    ...
    document.getElementById('closeMapModal').onclick = () => modal.remove();
