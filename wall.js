...
const Wall = {
  ...
  _sonIgualesEntradas(a, b) {
    ...
    for (let i = 0; i < a.length; i++) {
      ...
      // 🔥 FIX "el mapa GPS no aparece en el Muro hasta recargar": al
      // finalizar una carrera con GPS, la entrada se publica primero SIN
      // datos de GPS (marcarSesionRealizada) y, un instante después,
      // gps-tracker.js hace un segundo update() sobre ese mismo documento
      // añadiendo hasGPS/trackPoints (ver gps-tracker.js _guardarYPublicar).
      // Antes esta función solo comparaba id/likes, así que ese segundo
      // cambio se consideraba "misma entrada, nada que repintar" y el
      // render() con el mapa nunca llegaba a ejecutarse -- la tarjeta se
      // quedaba para siempre como si fuera una sesión sin GPS. Comparando
      // también hasGPS, ese segundo snapshot ya se trata como un cambio
      // real y dispara el render() normal (que ya sabe pintar el mapa).
