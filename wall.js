// ==================== wall.js - Muro global con minimapa GPS funcional ====================
// Versión: 4.2 - Minimapa robusto, sin emojis, eventos corregidos
// ====================
...
const Wall = {
  ...
  render(entries) {
    ...
    for (const entry of entries) {
      try {
        ...
        if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
          ...
          if (mapSVG) {
            miniMapHTML = `<div class="gps-minimap-tap" data-entry-id="${entry.id}" style="margin-top:10px;border-radius:10px;overflow:hidden;border:1px solid #2a2a2a;cursor:pointer;position:relative;">
              ${mapSVG}
              <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:flex-end;padding:8px;pointer-events:none;">
                <div style="background:rgba(0,0,0,0.72);color:#c0a060;font-size:10px;padding:4px 12px;border-radius:20px;border:1px solid rgba(192,160,96,0.35);">VER RECORRIDO</div>
              </div>
            </div>`;
          }
        }
    ...
    this.container.querySelectorAll('.gps-minimap-tap').forEach(el => {
      el.addEventListener('click', (e) => {
        ...
        if (entry && window.GPSTrackViewer) GPSTrackViewer.open(entry);
        else Utils.showToast('Visor no disponible', 'error');
      });
    });
  },

  async toggleLike(entryId, btnElement) {
    if (!AppState.currentUserId) { Utils.showToast('Inicia sesion para dar like', 'warning'); return; }
  ...
  async showLikesModal(entryId) {
    try {
      ...
      if (!doc.exists) { Utils.showToast('La publicacion ya no existe', 'error'); return; }
      const likes = doc.data().likes || [];
      if (likes.length === 0) { Utils.showToast('Nadie ha dado like a esta publicacion', 'info'); return; }
      const usersData = [];
  ...
  _createLikesModal(users) {
    ...
    modal.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; padding-bottom:10px; border-bottom:1px solid var(--border-color);"><h3 style="margin:0; color:var(--accent-yellow);">Me gusta (${users.length})</h3><button id="closeLikesModalBtn" style="background:none; border:none; font-size:24px; cursor:pointer; color:var(--text-secondary);">&times;</button></div><div id="likesList"></div>`;
    overlay.appendChild(modal);
    document.body.appendChild(overlay);
    const listContainer = modal.querySelector('#likesList');
