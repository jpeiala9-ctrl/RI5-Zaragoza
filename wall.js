          ...
          <div class="wall-item" data-entry-id="${entry.id}">
            <div class="wall-header">
              ...
              <div class="wall-user-info">
                ...
                <div class="wall-fecha">${fecha}${hora ? ' · ' + hora : ''}</div>
              </div>
              <button class="wall-like-btn ${likeClass}" data-entry-id="${entry.id}">
                ${likeClass === 'liked' ? '❤️' : '🤍'} <span class="like-count">${likeCount}</span>
              </button>
            </div>
            <div class="wall-entreno">
              <div class="wall-entreno-tipo" ${colorZona ? `style="color:${colorZona}"` : ''}>
                ${tipoEmoji} ${tipoMostrado}${zone ? ` · <span style="font-size:11px;opacity:.8">${Utils.escapeHTML(zone)}</span>` : ''}
              </div>
