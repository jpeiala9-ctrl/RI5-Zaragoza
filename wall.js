...
// Versión: 3.2 - Añadido soporte para mapas de tracks
// ====================
...
const Wall = {
  ...
  render(entries) {
        ...
        html += `
          <div class="wall-item" data-entry-id="${entry.id}">
            ...
            <div style="border:1px solid var(--border-color); border-radius:12px; padding:16px; text-align:center; background:var(--bg-primary); margin-top:4px;">
              ...
              ${entry.hasTrack ? `<div id="map-${entry.id}" class="track-mini-map" style="height:150px; margin-top:12px; border-radius:8px;"></div>` : ''}
            </div>
          </div>
        `;
      } catch (err) {
    ...
    // Cargar mapas para las entradas que tengan track
    for (const entry of entries) {
      if (entry.hasTrack && entry.trackId) {
        this.cargarMapaTrack(entry.userId, entry.trackId, `map-${entry.id}`);
      }
    }
  },

  async cargarMapaTrack(userId, trackId, containerId) {
    try {
      ...
      console.error('Error cargando mapa del track:', err);
    }
  },
