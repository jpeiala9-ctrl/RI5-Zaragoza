// ==================== wall.js - Muro global con soporte GPS y centrado mejorado ====================
// Versión: 3.3 - Añadido soporte para mostrar rutas GPS con centrado mejorado
// ====================
...
const Wall = {
  ...
  render(entries) {
    ...
    for (const entry of entries) {
      try {
        ...
        // ========== AÑADIR MAPA SI EXISTE RUTA GPS ==========
        if (entry.gpsRoute && entry.gpsRoute.points && entry.gpsRoute.points.length >= 2) {
          const mapId = `wallMap_${entry.id}`;
          html += `<div id="${mapId}" class="route-map" style="height: 200px; margin-top: 16px; border-radius: 8px; overflow: hidden;"></div>`;
          const distReal = (entry.gpsRoute.distanceMeters / 1000).toFixed(2);
          const ritmoReal = (entry.gpsRoute.durationSeconds / 60 / (entry.gpsRoute.distanceMeters / 1000)).toFixed(1);
          html += `<div style="margin-top: 8px; font-size: 12px; color: var(--gold);">📊 Real: ${distReal} km · Ritmo: ${ritmoReal} min/km</div>`;
        }
    ...
    // Inicializar mapas (con centrado mejorado)
    for (const entry of this.currentEntries) {
      if (entry.gpsRoute && entry.gpsRoute.points && entry.gpsRoute.points.length >= 2) {
        const mapId = `wallMap_${entry.id}`;
        const mapContainer = document.getElementById(mapId);
        if (mapContainer) {
          this._displayRouteWithPadding(mapId, entry.gpsRoute.points);
        }
      }
    }
  ...
  // Nueva función para mostrar el mapa con centrado y zoom ajustado
  async _displayRouteWithPadding(containerId, points) {
    ...
    try {
      ...
      // Ajustar el zoom para que se vea más cerca: usar fitBounds con padding de 30 píxeles
      ...
      console.error('Error mostrando mapa en el muro:', err);
    }
  },
