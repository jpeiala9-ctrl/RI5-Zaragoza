...
// Versión: 3.2 - Añadido método getUltimosPosts para dashboard
// ====================
    ...
    try {
      ...
      return snapshot.docs.map(doc => {
        ...
        if (data.timestamp) {
          ...
          if (diff < 60) timeAgo = 'hace ' + diff + ' seg';
          else if (diff < 3600) timeAgo = 'hace ' + Math.floor(diff / 60) + ' min';
          else if (diff < 86400) timeAgo = 'hace ' + Math.floor(diff / 3600) + ' h';
          else timeAgo = 'hace ' + Math.floor(diff / 86400) + ' d';
        }
        return {
          id: doc.id,
          username: data.username || 'Usuario',
          trainingType: data.trainingType || 'Entreno',
      ...
      console.error('Error obteniendo últimos posts:', error);
      return [];
    }
  }
  // ========== FIN NUEVO MÉTODO ==========
