...
// Versión: 1.5.1 - Integra zone, trainingName y hora sin cambios en el HTML
// ====================
    ...
    for (const entry of entries) {
      try {
        ...
        let detallesHTML = `<span>⏱️ ${duracion}'</span>
                           ...
                           <span>⚡ ${tss} TSS</span>`;
        if (zone) detallesHTML += `<span>🔥 ${Utils.escapeHTML(zone)}</span>`;
        if (hora) detallesHTML += `<span>🕒 ${hora}</span>`;
