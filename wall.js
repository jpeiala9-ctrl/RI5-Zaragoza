// ==================== wall.js ====================
// Versión: 4.5 - Miniaturas con mapa real CartoDB (light_all) igual que GPSTrackViewer
// ====================
...
const Wall = {
  ...
  // ========== FUNCIONES PARA GENERAR MINIATURAS CON MAPA REAL ==========
  ...
  async _generarTodasLasMiniaturas() {
    const placeholders = this.container.querySelectorAll('.gps-minimap-placeholder');
    for (const placeholder of placeholders) {
      const entryId = placeholder.dataset.entryId;
      const entry = this.currentEntries.find(e => e.id === entryId);
      if (!entry || !entry.trackPoints) continue;

      try {
        const imgSrc = await this._generarMiniaturaMapa(entryId, entry.trackPoints);
        // Reemplazar el placeholder por la imagen
        placeholder.innerHTML = `<img src="${imgSrc}" style="width:100%; display:block;">`;
        ...
        // Añadir evento de clic para abrir GPSTrackViewer
        placeholder.addEventListener('click', (e) => {
          e.stopPropagation();
          if (window.GPSTrackViewer) GPSTrackViewer.open(entry);
        });
      } catch (err) {
        console.warn('Error generando miniatura para', entryId, err);
        placeholder.innerHTML = '<div style="color:#888; padding:10px;">Error al cargar mapa</div>';
      }
    }
  },
  // ========== FIN FUNCIONES DE MINIATURAS ==========
  ...
  render(entries) {
    ...
    for (const entry of entries) {
      try {
        ...
        // ========== MINIATURA: placeholder (se reemplazará después) ==========
        let miniMapHTML = '';
        if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
          miniMapHTML = `
            <div class="gps-minimap-placeholder" data-entry-id="${entry.id}" style="margin-top:10px; border-radius:10px; overflow:hidden; border:1px solid #2a2a2a; cursor:pointer; position:relative; min-height:130px; background:#1a1a1a; display:flex; align-items:center; justify-content:center;">
              <span style="color:#888; font-size:11px;">Cargando mapa...</span>
            </div>
          `;
        }
        // ========== FIN MINIATURA ==========
    ...
    // ========== GENERAR MINIATURAS REALES (con mapa de CartoDB) ==========
    this._generarTodasLasMiniaturas();
    // =====================================================================
  ...
  _handleItemClick(e) {
    ...
    if (e.target.closest('.gps-minimap-placeholder')) return; // ya tiene su propio evento
...
window.Wall = Wall;
console.log('✅ Wall v4.5 - Miniaturas con mapa real CartoDB (light_all) igual que GPSTrackViewer');