...
const Wall = {
  ...
  _cargarLeaflet() {
    ...
    const intentar = (cssHref, jsSrc) => new Promise(resolve => {
      ...
      script.src = jsSrc;
      const limite = setTimeout(() => resolve(false), 5000);
      script.onload = () => { clearTimeout(limite); resolve(true); };
      script.onerror = () => { clearTimeout(limite); resolve(false); };
      document.head.appendChild(script);
    });

    // Si unpkg falla o tarda demasiado (bloqueo, red lenta, caída
    // puntual), se reintenta con jsdelivr como red de respaldo, en vez
    // de rendirse en silencio y dejar el mapa sin cargar para siempre.
    this._leafletPromise = (async () => {
      const fuentes = [
      ...
      for (const [css, js] of fuentes) {
        const ok = await intentar(css, js);
