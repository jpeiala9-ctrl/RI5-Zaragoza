// ==================== wall.js ====================
// Versión: 4.10 - Pulsar el nombre del autor en una tarjeta del muro abre su perfil
// ====================
...
const Wall = {
  ...
  _handleItemClick(e) {
    ...
    // Si el toque fue justo sobre el nombre del autor de la tarjeta, se
    // abre su perfil (o el modal de agregar amigo si aún no lo es) en
    // vez del modal de "me gusta" de la sesión.
    const nombreEl = e.target.closest('.wall-username');
    if (nombreEl) {
      const uid = nombreEl.dataset.uid;
  ...
  _createLikesModal(users) {
    ...
    for (const user of users) {
      ...
      // El nombre se pinta del color de nivel de CADA usuario (antes era
      // siempre dorado fijo para todos): así, cuando ya tenías varios
      // "me gusta" y llega uno nuevo, es más fácil identificar de un
      // vistazo quién es quién en la lista.
      ...
      userItem.innerHTML = `
        ...
        <div style="flex:1; min-width:0;">
          <div style="font-weight:bold; color:${colorNivel}; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${Utils.escapeHTML(usernameFormatted)}</div>
...
window.Wall = Wall;
console.log('✅ Wall v4.10 - Pulsar el nombre del autor en una tarjeta del muro abre su perfil');