// ==================== wall.js - Muro con modal ampliado y color según estado ====================
// Versión: 3.7 - Modal con distancia, ritmo, ciudad, fecha/hora; colores verde/naranja
// ============================================================================
...
const Wall = {
  ...
  render(entries) {
    ...
    for (const entry of entries) {
      try {
        ...
        // Determinar clase de estado (verde si normal, naranja si forzada)
        let estadoClass = '';
        if (entry.forcedComplete === true) {
          estadoClass = 'wall-item-forzado';
        } else if (entry.distancia && entry.distancia > 0) {
          // Si tiene distancia y no es forzada, se considera realizada normalmente
          estadoClass = 'wall-item-realizado';
        }
  ...
  async _openRouteModal(entry) {
    ...
    if (mapContainer) {
      ...
      // Obtener ciudad mediante geocodificación inversa
      let city = '--';
      try {
