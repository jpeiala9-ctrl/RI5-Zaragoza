// ==================== wall.js ====================
// Versión: 4.7 - Minimapa siempre visible, badge "Parcial", ciudad desde GPS
// ====================
...
const Wall = {
  ...
  _renderMiniMap(trackPoints) {
    if (!trackPoints || trackPoints.length < 2) {
      console.warn('No hay puntos para minimapa', trackPoints);
      return '';
    }
    if (window.GPSTracker && typeof GPSTracker.renderTrackSVG === 'function') {
      try {
        const svg = GPSTracker.renderTrackSVG(trackPoints, 320, 130);
        if (!svg) console.warn('renderTrackSVG devolvió vacío');
        return svg;
      } catch (e) {
        console.error('Error en renderTrackSVG', e);
        return this._miniMapFallback(trackPoints);
      }
    } else {
      console.warn('GPSTracker no disponible, usando fallback');
      return this._miniMapFallback(trackPoints);
    }
  },
    ...
    for (const entry of entries) {
      try {
        ...
        if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
          console.log(`Renderizando minimapa para entrada ${entry.id}, puntos: ${entry.trackPoints.length}`);
          const mapSVG = this._renderMiniMap(entry.trackPoints);
          if (mapSVG) {
            const firstLat = entry.firstPointLat || entry.trackPoints[0].lat;
            const firstLng = entry.firstPointLng || entry.trackPoints[0].lng;
            miniMapHTML = `
              <div class="track-minimap" data-points='${JSON.stringify(entry.trackPoints)}'
                   ...
                   data-lat="${firstLat}" data-lng="${firstLng}"
                   style="margin-top:10px; border-radius:10px; overflow:hidden; border:1px solid #2a2a2a; cursor:pointer;">
                ${mapSVG}
              </div>
            `;
          } else {
            console.warn('No se pudo generar el SVG del minimapa');
          }
        } else {
          console.log(`No se muestra minimapa para entrada ${entry.id}: hasGPS=${entry.hasGPS}, trackPoints length=${entry.trackPoints?.length}`);
        }
  ...
  async _handleMinimapClick(e) {
    ...
    let ciudad = 'Obteniendo ciudad...';
    const cacheKey = `${lat},${lng}`;
    if (this._geocodeCache.has(cacheKey)) {
      ciudad = this._geocodeCache.get(cacheKey);
    } else {
      try {
        const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=10&addressdetails=1&accept-language=es`);
        if (response.ok) {
          const data = await response.json();
          ciudad = data.address?.city || data.address?.town || data.address?.village || data.address?.municipality || 'Ubicación no disponible';
          this._geocodeCache.set(cacheKey, ciudad);
        } else {
  ...
  abrirModalMapa(points, distancia, entryData = {}) {
    const modalExistente = document.getElementById('modalMapaAmpliado');
    if (modalExistente) modalExistente.remove();

    const modal = document.createElement('div');
    modal.id = 'modalMapaAmpliado';
    modal.style.cssText = `
      position: fixed; top: 0; left: 0; width: 100%; height: 100%;
    ...
    const tiempoFormateado = entryData.tiempo ? `${entryData.tiempo} min` : '—';
    const ritmoFormateado = entryData.ritmo && entryData.ritmo !== '—' ? `${entryData.ritmo} min/km` : '—';
    
    contenido.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
        <strong style="color:#c0a060;">🏁 TRACK DE LA SESIÓN</strong>
        <button id="closeMapModal" style="background:none; border:none; color:#aaa; font-size:28px; cursor:pointer; line-height:1;">✕</button>
      </div>
      <div id="mapaAmpliado" style="height: 400px; width: 100%; border-radius: 12px; margin-bottom: 20px;"></div>
      <div class="stats" style="text-align:center; border-top: 1px solid #333; padding-top: 15px;">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 10px;">
          <div>📏 <strong>${distancia} km</strong></div>
          <div>⏱️ <strong>${tiempoFormateado}</strong></div>
        </div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 10px;">
          <div>⚡ <strong>${ritmoFormateado}</strong><br><span style="font-size:10px;">Ritmo medio</span></div>
          <div>👟 <strong>${Utils.escapeHTML(entryData.zapatillas || 'No info')}</strong></div>
        </div>
        <div style="font-size: 12px; color: #aaa; margin-top: 8px;">
          📍 ${Utils.escapeHTML(entryData.ciudad || 'Ubicación no disponible')}
        </div>
      </div>
    `;
    ...
    const map = L.map('mapaAmpliado').setView([center.lat, center.lng], 14);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', { maxZoom: 19 }).addTo(map);
    const polyline = L.polyline(points.map(p => [p.lat, p.lng]), { color: '#c0a060', weight: 4, opacity: 0.9 }).addTo(map);
    const icon = L.divIcon({ html: `<div style="width:14px;height:14px;background:#c0a060;border:3px solid #fff;border-radius:50%;"></div>`, className: '', iconAnchor: [7,7] });
    L.marker([points[0].lat, points[0].lng], { icon }).addTo(map).bindPopup('Inicio');
    L.marker([points[points.length-1].lat, points[points.length-1].lng], { icon }).addTo(map).bindPopup('Final');
...
window.Wall = Wall;
console.log('✅ Wall v4.7 - Minimapa con logs, badge parcial, ciudad');