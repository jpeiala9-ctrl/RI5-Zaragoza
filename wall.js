// ==================== wall.js ====================
// Versión: 4.18 - FIX "medio mapa sin pintar / parpadeo" (lo introdujo el
//                propio fix anterior, v4.17: forzar un render() completo
//                de TODA la lista en cuanto llegaba el GPS destruye y
//                reinicializa TODOS los mini-mapas Leaflet visibles a la
//                vez, y reinicializar varios mapas de golpe en pleno
//                reflow es justo lo que hace que Leaflet mida mal el
//                contenedor de alguno -- "medio mapa pintado", que se
//                arregla solo al girar el móvil porque eso fuerza un
//                resize real). Ahora, en vez de forzar un render()
//                completo, se sustituye SOLO el <div> de la tarjeta
//                concreta cuyo GPS acaba de llegar -- el resto de
//                tarjetas y sus mapas ya inicializados no se tocan para
//                nada. _sonIgualesEntradas() vuelve a comparar solo
//                id/likes, como antes de v4.17. Ver
//                _actualizarEntradaConMapaNuevo() y el punto donde se
//                llama, dentro de iniciarListener().
// Versión: 4.17 - FIX "el mapa GPS no aparece en el Muro hasta recargar
//                la página": _sonIgualesEntradas() (el filtro anti-
//                parpadeo de v4.16) solo comparaba id/likes, así que el
//                segundo update() de gps-tracker.js (que añade hasGPS/
//                trackPoints a una entrada publicada un instante antes
//                SIN ellos) se consideraba "nada que repintar" y el mapa
//                nunca llegaba a mostrarse. Ahora también se compara
//                hasGPS. Ver _sonIgualesEntradas().
// Versión: 4.16 - Precarga + caché real (mismo patrón que "Explorar
//                usuarios" v3.53 de friends.js y "Mis últimos
//                entrenamientos" v10.13 de profile.js): el listener
//                onSnapshot ya no se destruye/reabre en cada visita a la
//                pestaña Muro -- se abre UNA sola vez por sesión
//                (iniciarListener() es idempotente) y el HTML se pinta en
//                segundo plano al iniciar sesión (Wall.precargarMuro(),
//                llamado desde AppState.precargarDatos en app.js), así
//                que al entrar en la pestaña Muro la lista ya está puesta
//                desde antes -- sin parpadeo ni espera. Cada snapshot
//                compara las entradas nuevas con las que ya estaban en
//                pantalla (por id, likeCount y likes.length) y solo
//                repinta el DOM si algo cambió de verdad: una simple
//                reconexión del listener (móvil bloqueado/desbloqueado,
//                cambio de cobertura, visibilitychange...) ya no
//                reconstruye el HTML con las mismas tarjetas. El listener
//                se sigue cerrando solo en logout (AppState.setCurrentUser(null)).
...
const Wall = {
  ...
  // 🔥 v4.16: flag que indica si el muro ya se ha pintado al menos una vez
  // en esta sesión. Mientras siga en true, entrar/salir de la pestaña
  // Muro no vuelve a tocar el DOM ni a pedir nada a Firestore -- el
  // listener en tiempo real ya está abierto y solo manda cambios cuando
  // de verdad ocurren.
  _renderizado: false,
  // Promesa compartida para no lanzar dos cargas simultáneas si init() y
  // precargarMuro() coinciden en el tiempo.
  ...
  // 🔥 v4.16: compara dos tandas de entradas del muro de forma LIGERA
  // (id + likeCount + likes.length). Con esto, cuando el listener
  // onSnapshot dispara por cualquier motivo (una reconexión tras el móvil
  // bloqueado, un visibilitychange, un snapshot idéntico al anterior...)
  // pero el contenido visible no ha cambiado realmente, no se reconstruye
  // el HTML -- que es exactamente lo que se veía como "parpadeo" al
  // volver a la pestaña Muro.
  ...
  // 🔥 FIX "el mapa GPS no aparece hasta recargar" (v4.17) + FIX "medio
  // mapa sin pintar / parpadeo" (v4.18, esta versión): la primera vez que
  // se arregló esto se hizo forzando un render() completo en cuanto
  // hasGPS cambiaba -- pero render() destruye y reinicializa TODOS los
  // mini-mapas Leaflet visibles a la vez (_destruirMapas + innerHTML
  // entero), y reinicializar varios mapas de golpe en pleno reflow de
  // toda la lista es justo lo que hace que Leaflet mida mal el
  // contenedor de alguno de ellos ("medio mapa pintado", que se arregla
  // solo al girar el móvil porque eso fuerza un resize real). Ahora, en
  // vez de eso, se detecta y actualiza SOLO la tarjeta concreta cuyo
  // hasGPS acaba de pasar a true, sustituyendo únicamente su <div> --el
  // resto de la lista, y sus mapas ya inicializados, no se tocan para
  // nada, así que no hay ni parpadeo ni riesgo de medir mal otros mapas
  // que no tienen nada que ver con este cambio.
  _actualizarEntradaConMapaNuevo(entry) {
    if (!this.container) return;
    const el = this.container.querySelector(`.wall-item[data-entry-id="${entry.id}"]`);
    if (!el) return;
    const div = document.createElement('div');
    div.innerHTML = this._buildEntryHTML(entry).trim();
    const nuevoEl = div.firstElementChild;
    if (!nuevoEl) return;
  ...
  // 🔥 v4.16: init() es ahora idempotente y NO destruye el listener en
  // cada llamada. Antes esto era detenerListener() + iniciarListener() en
  // cada visita a la pestaña, lo que forzaba un snapshot nuevo y un
  // render() completo con las mismas entradas -- de ahí el parpadeo.
  init() {
    this.container = document.getElementById('wallContainer');
    if (!this.container) return;

    // Ya NO se destruye el listener: iniciarListener() es idempotente
    // (si ya hay uno abierto, no hace nada).
    this.iniciarListener();
    this._iniciarPurgaPeriodica();

    // Si ya estaba pintado (precarga al iniciar sesión, o una visita
    // anterior a esta pestaña dentro de la misma sesión), no se toca el
    // DOM. La lista se queda exactamente como está.
    if (this._renderizado) return;

    // Si aún no estaba pintado (p. ej. la pestaña se ha abierto antes de
    // que termine la precarga del login, o esa precarga falló), se espera
    // al primer snapshot del listener -- que es quien pinta el HTML --
    // sin mostrar nada raro por medio.
    this.precargarMuro();
    ...
    if (!this._visibilityHandlerEnganchado) {
      ...
      document.addEventListener('visibilitychange', () => {
        if (!document.hidden && document.querySelector('#subtab-muro')?.classList.contains('active') && !this._unsubscribe) {
          // Refresco de seguridad al volver de segundo plano, solo si el
          // listener se hubiera quedado caído (no es un sondeo periódico).
          this.iniciarListener();
        }
      });
    }
  ...
  // 🔥 v4.16: equivalente a SessionInvites.precargarHistorial() o
  // Profile._iniciarListenerMisEntrenamientos(): abre el listener en
  // tiempo real (idempotente), espera al primer snapshot, y deja el
  // contenedor #wallContainer pintado en segundo plano -- aunque la
  // pestaña Muro no esté activa, el <div> existe en el DOM (solo está
  // oculto por CSS), así que se puede rellenar de antemano. Cuando el
  // usuario abra de verdad la pestaña, la lista ya está puesta: no hay
  // ninguna recarga visible ni parpadeo.
  precargarMuro() {
    if (this._renderizado) return Promise.resolve();
    if (this._precargaPromise) return this._precargaPromise;

    this.container = this.container || document.getElementById('wallContainer');
    if (!this.container) return Promise.resolve();

    this.iniciarListener();
    this._iniciarPurgaPeriodica();

    // Esperar a que el listener haya pintado el muro. Se sondea el flag
    // _renderizado cada 100ms con un tope de seguridad de 8s por si algo
    // falla (red caída, error del listener...), para no quedarse colgado.
    const inicio = Date.now();
    this._precargaPromise = new Promise((resolve) => {
      const check = () => {
        if (this._renderizado || Date.now() - inicio > 8000) {
          this._precargaPromise = null;
          resolve();
          return;
        }
  ...
  // 🔥 v4.14: listener en tiempo real de Firestore (onSnapshot). A
  // diferencia de un .get() periódico (que paga sus ~20 lecturas SIEMPRE
  // aunque nada haya cambiado), onSnapshot solo factura cuando Firestore
  // detecta un cambio real en el resultado.
  // 🔥 v4.16: además, ahora es idempotente (si ya hay uno abierto no hace
  // nada) y compara las entradas nuevas con las que ya estaban en
  // pantalla antes de repintar. Con eso, una simple reconexión del
  // listener (móvil bloqueado/desbloqueado, cambio de red...) ya no
  // reconstruye el HTML con las mismas tarjetas -- que era el parpadeo
  // visible al entrar en la pestaña Muro.
  iniciarListener() {
    ...
    this._unsubscribe = firebaseServices.db
      ...
      .onSnapshot(async (snapshot) => {
        ...
        try {
          ...
          // Antes de decidir si hace falta un render() completo, se
          // detectan aparte las entradas que acaban de recibir su mapa GPS
          // (pasan de hasGPS:false a hasGPS:true) con la lista ya pintada:
          // se actualizan una a una, tarjeta por tarjeta (ver
          // _actualizarEntradaConMapaNuevo), en vez de dejar que la
          // comparación de más abajo detecte una diferencia y dispare un
          // render() completo de toda la lista.
          if (this._renderizado) {
            const anterioresPorId = {};
            this.currentEntries.forEach(e => { anterioresPorId[e.id] = e; });
            nuevasEntries.forEach(entryNuevo => {
              const anterior = anterioresPorId[entryNuevo.id];
              if (anterior && !anterior.hasGPS && entryNuevo.hasGPS) {
                entryNuevo._nivel = anterior._nivel; // no ha podido cambiar, evita otra consulta
                this._actualizarEntradaConMapaNuevo(entryNuevo);
              }
            });
          }

          // 🔥 Si el muro ya estaba pintado y las entradas visibles NO han
          // cambiado (mismos ids, mismo orden, mismos contadores de likes),
          // no se toca el DOM. Sin esto, cada reconexión del listener
          // reconstruía el HTML entero con las mismas entradas, y eso se
          // veía como un parpadeo al entrar en la pestaña.
          if (this._renderizado && this._sonIgualesEntradas(this.currentEntries, nuevasEntries)) {
  ...
  detenerListener() {
    ...
    // 🔥 v4.16: al cerrar sesión, se resetean los flags -- la próxima
    // sesión (posiblemente de otra cuenta) debe empezar de cero.
  ...
  // hubieran pedido segundos antes. Ahora se reutiliza
  // Friends._nivelesCache (la misma caché compartida y persistente que ya
  // usa la pantalla "Explorar amigos", ver friends.js) -- solo se pide a
  // Firestore el nivel de un autor la PRIMERA vez que aparece en esta
  // sesión; las siguientes veces (aquí o en Explorar) se sirve de
  // memoria, coste cero. Contrapartida asumida: si alguien sube
  ...
  async _crearMiniMapa(containerId, trackPoints, entryId = null, esPropia = false) {
    ...
    // Tile layer OpenStreetMap (vuelta atrás desde Esri, a petición del
    // usuario). crossOrigin:true es necesario para poder "leer" luego el
    // mapa como imagen (canvas) y cachearlo -- sin esto el navegador
    // bloquea la lectura del canvas por seguridad (CORS), aunque el mapa
    // se vea perfectamente en pantalla. La cajita de atribución no se ve
    // porque el mapa ya tiene attributionControl:false (más arriba, ya
    // estaba así de antes).
  ...
  // 🔥 v4.16: marca _renderizado = true en los tres puntos de salida,
  // para que a partir de este momento entrar/salir de la pestaña Muro no
  // vuelva a tocar el DOM.
  render(entries) {
    if (!this.container) return;
    document.getElementById('wallCargarMasContainer')?.remove();
...
window.Wall = Wall;
console.log('✅ Wall v4.16 - Precarga + caché real: listener abierto una sola vez por sesión + HTML pintado en segundo plano al iniciar sesión + comparación ligera antes de repintar (sin parpadeo al entrar en la pestaña)');