// ==================== wall.js - Muro con modal de mapa grande ====================
// Versión: 3.6 - Mapa sin atribución, clic en mapa abre modal con ruta tipo Strava
// ====================
  ...
  render(entries) {
    ...
    for (const entry of entries) {
      try {
        ...
        html += `
          <div class="wall-item" data-entry-id="${entry.id}" data-plan-id="${entry.planId || ''}" data-sesion-index="${entry.sesionIndex || ''}">
  ...
  async _openRouteModal(entry) {
    ...
    if (!modal) {
      ...
      modalContent.innerHTML = `
        ...
        <div id="wallRouteStats" style="text-align:center; font-size:14px;"></div>
      `;
    ...
    if (mapContainer) {
      ...
      L.circleMarker(latlngs[latlngs.length-1], { radius: 8, color: '#F44336', fillColor: '#F44336', fillOpacity: 1, weight: 2 }).addTo(map);
      mapContainer._leaflet_map = map;
      
      const distanceKm = (entry.gpsRoute.distanceMeters / 1000).toFixed(2);
      const durationMin = Math.floor(entry.gpsRoute.durationSeconds / 60);
      const durationSec = Math.round(entry.gpsRoute.durationSeconds % 60);
      const pace = (entry.gpsRoute.durationSeconds / 60 / distanceKm).toFixed(1);
      statsDiv.innerHTML = `
        📏 Distancia: ${distanceKm} km &nbsp;|&nbsp;
        ⏱️ Duración: ${durationMin}:${durationSec.toString().padStart(2,'0')} &nbsp;|&nbsp;
        ⚡ Ritmo: ${pace} min/km
      `;
    }
  },
