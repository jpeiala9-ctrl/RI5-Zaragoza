          ...
          <div class="wall-item" data-entry-id="${entry.id}">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
              <div class="wall-user-link" data-user-id="${Utils.escapeHTML(entry.userId || '')}" style="display:flex; align-items:center; gap:12px; cursor:pointer;">
                ${avatarHTML}
                <div>
                  <div class="wall-username">${Utils.escapeHTML(usernameFormatted)}${Utils.adminBadge(entry.isAdmin)}</div>
    ...
    for (const user of users) {
      ...
      userItem.innerHTML = `
        ...
        <div style="flex:1; min-width:0;">
          <div style="font-weight:bold; color:var(--accent-yellow); overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${Utils.escapeHTML(usernameFormatted)}${Utils.adminBadge(user.isAdmin)}</div>
