...
// Versión: 3.13 - Añadido listener en tiempo real para solicitudes (sin auto-inicio)
// ====================
  ...
  iniciarListenerSolicitudes() {
    ...
    this.unsubscribeSolicitudes = firebaseServices.db.collection('friendRequests')
      ...
      .onSnapshot((snapshot) => {
        ...
        // Refrescar la lista de solicitudes si la pestaña está activa
        if (document.getElementById('amigos-solicitudes')?.classList.contains('active')) {
          ...
          if (recibidasDiv && recibidasDiv.style.display !== 'none') this.cargarSolicitudesRecibidas();
          if (enviadasDiv && enviadasDiv.style.display !== 'none') this.cargarSolicitudesEnviadas();
        }
      }, (error) => {
...
// NOTA: El listener se inicia desde auth.js, no automáticamente aquí.
