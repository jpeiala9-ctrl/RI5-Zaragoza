...
const Friends = {
  ...
  // 🔥 Caché en memoria del panel "Mis alumnos", con el mismo criterio que
  // ya usan "Explorar usuarios" y el resto de listas de la app: se pide a
  // Firestore UNA vez y se reutiliza tal cual -- sin volver a leer nada
  // ni repetir la animación de "CARGANDO" -- hasta que hay un cambio real
  // (se invita, se cancela una invitación, o se quita a un alumno; ver
  // invitarAlumno/cancelarInvitacionAlumno/quitarAlumno más abajo, y los
  // listeners en tiempo real de app.js para cuando el cambio llega desde
  // el otro lado: el alumno acepta o rechaza la invitación).
  _alumnosPanelCache: null,
  ...
  async cargarPanelAlumnos(forceRefresh = false) {
    ...
    // 🔧 FIX "se ve cargando y luego de repente se ponen": antes se
    // repetía la lectura completa (y el parpadeo) cada vez que se entraba
    // en la pestaña. Ahora, si ya se cargó antes y no ha cambiado nada de
    // verdad, se repinta al instante desde memoria -- ni lectura a
    // Firestore ni animación.
    if (!forceRefresh && this._alumnosPanelCache) {
      this._renderPanelAlumnos(this._alumnosPanelCache);
      return;
    }

    // Misma animación de "CARGANDO" con letras de colores que ya usa
    // "Explorar usuarios" (Utils._animarTextoDorado vía _mostrarCargando),
    // en vez del texto fijo de antes: o está puesto al instante (caché) o
    // se ve esta animación completa y luego aparece todo de golpe.
    const esperarFinAnimacion = this._mostrarCargando(container, 'entrenadorAlumnosLoadingText');
    try {
      const userDoc = await firebaseServices.db.collection('users').doc(AppState.currentUserId).get();
      ...
      if (!friendIds.length) {
        this._alumnosPanelCache = { amigosData: [], alumnosAceptados: [], pendientePorUid: {}, niveles: {} };
...
console.log('✅ friends.js v3.56 - Explorar usuarios ya no se cuelga en CARGANDO si el canal en tiempo real está bloqueado (red de seguridad con .get() a los 9s, ver Friends._esperarPrimeraExplorar)');
