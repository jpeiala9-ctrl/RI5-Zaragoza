...
// Versión: 3.47 - Añadido sistema de compartir sesiones
// ====================
...
const Friends = {
  ...
  // ==================== COMPARTIR SESIONES ====================
  async actualizarBadgeCompartidos() {
    if (!AppState.currentUserId) return;
    try {
      const pendientes = await Storage.getSharedSessionsForUser(AppState.currentUserId, 'pending');
      this.compartidosPendientesCount = pendientes.length;
      const tab = document.getElementById('amigosCompartidosTab');
      if (tab) {
        if (this.compartidosPendientesCount > 0) {
          tab.classList.add('amigos-solicitudes-unread');
          tab.setAttribute('data-count', this.compartidosPendientesCount);
        } else {
      ...
      console.warn('Error actualizando badge compartidos', e);
    }
  },

  async cargarCompartidos() {
    const container = document.getElementById('listaCompartidos');
    if (!container || !AppState.currentUserId) return;
    try {
      const pendientes = await Storage.getSharedSessionsForUser(AppState.currentUserId, 'pending');
      if (pendientes.length === 0) {
        container.innerHTML = '<p style="text-align:center; padding:20px; color:var(--text-secondary);">No hay sesiones compartidas pendientes.</p>';
        return;
      }
      let html = '';
      for (const share of pendientes) {
        const senderData = await Storage.getUser(share.senderId);
        const senderName = senderData ? Utils.capitalizeUsername(senderData.username) : 'Usuario';
        const session = share.session;
        const tipoEmoji = {
          rodaje: '🏃‍♂️', tempo: '⚡', series: '🔁', largo: '📏', strength: '💪'
        }[session.tipo] || '🏃';
        const nombreSesion = session.detalle?.nombre || session.tipo || 'Entreno';
        const duracion = session.duracion || '?';
        const diaSemana = ['', 'LUN', 'MAR', 'MIÉ', 'JUE', 'VIE', 'SÁB', 'DOM'][share.diaSemana] || 'DÍA';

        html += `
          <div class="solicitud-item" style="flex-direction:column; align-items:stretch; gap:12px;">
            <div style="display:flex; align-items:center; gap:12px; justify-content:space-between;">
              <div style="display:flex; align-items:center; gap:10px;">
                <span style="font-size:20px;">${tipoEmoji}</span>
                <div>
                  <strong style="color:var(--accent-yellow);">${Utils.escapeHTML(senderName)}</strong>
                  <span style="font-size:13px; color:var(--text-secondary);"> te comparte:</span>
                </div>
              </div>
              <span style="font-size:12px; background:var(--bg-primary); padding:4px 12px; border-radius:20px; border:1px solid var(--border-color);">${diaSemana}</span>
            </div>
            <div style="background:var(--bg-primary); border-radius:10px; padding:12px; border-left:3px solid var(--accent-blue);">
              <div style="font-weight:500;">${Utils.escapeHTML(nombreSesion)}</div>
              <div style="font-size:13px; color:var(--text-secondary);">⏱️ ${duracion}' · ${session.tipo?.toUpperCase() || ''}</div>
              ${session.detalle?.zona ? `<div style="font-size:12px; color:var(--accent-yellow);">🔥 ${Utils.escapeHTML(session.detalle.zona)}</div>` : ''}
            </div>
            <div style="display:flex; gap:10px; justify-content:flex-end;">
              <button class="btn-amigo" onclick="Friends.aceptarSesionCompartida('${share.id}')" style="background:var(--zone-2); border-color:var(--zone-2); color:var(--bg-primary);">✅ Aceptar</button>
              <button class="btn-amigo eliminar" onclick="Friends.rechazarSesionCompartida('${share.id}')">✗ Rechazar</button>
            </div>
          </div>
        `;
      }
      ...
      console.error('Error cargando compartidos:', e);
      container.innerHTML = '<p style="text-align:center; color:var(--zone-5);">Error al cargar sesiones compartidas.</p>';
    }
  },

  async aceptarSesionCompartida(shareId) {
    if (!AppState.currentUserId) return;
    const confirmado = await Utils.confirm(
      'Aceptar sesión',
      'Esta sesión reemplazará el entrenamiento del mismo día de la semana en tu plan actual. ¿Continuar?'
    );
    if (!confirmado) return;
    Utils.showLoading();
    try {
      await Storage.acceptSharedSession(shareId, AppState.currentUserId);
      Utils.showToast('✅ Sesión añadida a tu plan', 'success');
      await this.cargarCompartidos();
      await this.actualizarBadgeCompartidos();
      if (document.getElementById('tab-plan')?.classList.contains('active')) {
        if (window.PlanGenerator) await PlanGenerator.mostrarUltimoPlanGuardado();
      }
    } catch (error) {
      console.error(error);
      Utils.showToast('Error al aceptar: ' + error.message, 'error');
    } finally {
      Utils.hideLoading();
    }
  },

  async rechazarSesionCompartida(shareId) {
    if (!AppState.currentUserId) return;
    const confirmado = await Utils.confirm('Rechazar sesión', '¿Rechazar esta sesión compartida?');
    if (!confirmado) return;
    try {
      await Storage.rejectSharedSession(shareId, AppState.currentUserId);
...
console.log('✅ friends.js v3.47 - Añadido sistema de compartir sesiones');