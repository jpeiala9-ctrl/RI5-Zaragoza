...
// Versión: 3.12 - Añadido listener en tiempo real para solicitudes de amistad
// ====================

const Friends = {
  todosUsuariosPagination: { lastDoc: null, hasMore: true, loading: false },
  unsubscribeSolicitudes: null, // para detener el listener
  ...
  initEventListeners() {
    ...
    // Delegación para los botones dinámicos (chat, eliminar amigo)
    document.addEventListener('click', (e) => {
  ...
  // --- Listener en tiempo real para solicitudes de amistad ---
  iniciarListenerSolicitudes() {
    ...
    this.unsubscribeSolicitudes = firebaseServices.db.collection('friendRequests')
      ...
      .onSnapshot((snapshot) => {
        ...
        // También podemos refrescar la lista de solicitudes si la pestaña está abierta
        if (document.getElementById('amigos-solicitudes')?.classList.contains('active')) {
          // Si la pestaña de solicitudes está activa, recargamos las listas
          const recibidasDiv = document.getElementById('listaSolicitudesRecibidas');
          const enviadasDiv = document.getElementById('listaSolicitudesEnviadas');
          if (recibidasDiv && recibidasDiv.style.display !== 'none') {
            this.cargarSolicitudesRecibidas();
          }
          if (enviadasDiv && enviadasDiv.style.display !== 'none') {
  ...
  detenerListenerSolicitudes() {
    if (this.unsubscribeSolicitudes) {
      this.unsubscribeSolicitudes();
...
if (AppState && AppState.currentUserId) {
  Friends.iniciarListenerSolicitudes();
}
