...
// Versión: 3.47 - Añadida función getAmigosLista para compartir sesiones
// ====================
...
const Friends = {
  ...
  // ==================== NUEVA FUNCIÓN PARA OBTENER LISTA DE AMIGOS ====================

  /**
   * Obtiene la lista de amigos con sus datos básicos (uid, username, photoURL)
   * @returns {Promise<Array>} Array de objetos {uid, username, photoURL}
   */
  async getAmigosLista() {
    if (!AppState.currentUserId) return [];
    ...
    try {
      ...
      for (let i = 0; i < friendIds.length; i += 10) {
        const batch = friendIds.slice(i, i + 10);
        const snapshots = await Promise.all(batch.map(fid => 
...
console.log('✅ friends.js v3.47 - Añadida función getAmigosLista para compartir sesiones');