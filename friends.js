...
// Versión: 3.24 - Optimización: lotes 'in' para niveles, caché sessionStorage (carga <150ms)
// ====================

const Friends = {
  todosUsuariosPagination: { lastDoc: null, hasMore: true, loading: false },

  // Obtener niveles en lotes de 10 usando 'in' y select('level')
  ...
  async cargarListaAmigos(forceRefresh = false) {
    ...
    if (!forceRefresh) {
      ...
      if (cached) {
        try {
          ...
          if (Date.now() - timestamp < 30 * 1000) { // 30 segundos
    ...
    try {
      ...
      // Obtener amigos con una sola consulta 'in' (lotes de 10)
      const amigos = [];
      const chunks = [];
