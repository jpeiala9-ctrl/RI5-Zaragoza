...
const Friends = {
  ...
  // 🔥 v3.50: el esqueleto pulsando de la v3.49 no convenció -- se
  // sustituye por la MISMA animación de "CARGANDO" con letras de colores
  // que ya usa el panel de soporte del admin (Utils._animarTextoDorado,
  // ver app.js). Y, como en ese mismo panel, los resultados de "Buscar"
  // (por nombre y "Explorar usuarios") se guardan en memoria y se
  // reutilizan tal cual -- sin volver a pedir nada a Firestore -- hasta
  // que haya un cambio real: enviar, cancelar, aceptar o rechazar una
  // solicitud (ver _invalidarCacheUsuarios, llamado desde esos cuatro
  // sitios). Ya no depende de un tiempo fijo como el sessionStorage de
  // 30s que había antes.
  _cacheExplorarUsuarios: null,
  _cacheExplorarVigente: false,
  _cacheBusqueda: {}, // { [termino en minúsculas]: {items, niveles, friendIds, enviadas, recibidas} }
  _cacheBusquedaVigente: false,

  _invalidarCacheUsuarios() {
    this._cacheExplorarVigente = false;
    this._cacheBusquedaVigente = false;
    this._cacheBusqueda = {};
  },
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    if (reset && this._cacheExplorarVigente && this._cacheExplorarUsuarios) {
      this._renderExplorarUsuarios(this._cacheExplorarUsuarios);
      return;
    }

    this.todosUsuariosPagination.loading = true;
    let esperarFinAnimacion = async () => {};
    if (reset) {
      // 🔥 v3.50: "CARGANDO" con letras de colores, igual que el panel de
      // soporte del admin, en vez del esqueleto (v3.49) o el texto fijo
      // de antes.
      esperarFinAnimacion = this._mostrarCargando(container, 'explorarUsuariosLoadingText');
      this.todosUsuariosPagination.lastDoc = null;
      this.todosUsuariosPagination.hasMore = true;
    } else {
      // "Cargar más": no se toca la lista ya visible, solo el botón.
    ...
    try {
      ...
      if (result.users.length === 0) {
        ...
        if (reset) {
          this._cacheExplorarUsuarios = { users: [], niveles: {}, friendIds, enviadas, recibidas };
          this._cacheExplorarVigente = true;
          await esperarFinAnimacion();
          container.innerHTML = '<p style="text-align:center; padding:20px;">No hay más usuarios</p>';
        }
        const loadMoreBtn = document.getElementById('cargarMasUsuariosBtn');
      ...
      if (reset) {
        this._cacheExplorarUsuarios = dataToCache;
        this._cacheExplorarVigente = true;
      }
      await esperarFinAnimacion();
      this._renderExplorarUsuarios(dataToCache, !reset);
    } catch (error) {
      console.error('Error cargando todos los usuarios:', error);
      if (reset) { await esperarFinAnimacion(); container.innerHTML = '<p style="text-align:center; color:var(--zone-5);">Error al cargar</p>'; }
    } finally {
      this.todosUsuariosPagination.loading = false;
      if (!reset) {
        const loadMoreBtn = document.getElementById('cargarMasUsuariosBtn');
        if (loadMoreBtn) { loadMoreBtn.disabled = false; loadMoreBtn.textContent = 'CARGAR MÁS'; }
      }
    }
  },

  // append=false (por defecto): sustituye el contenido entero del
  // contenedor (primera carga / caché). append=true: añade solo esta
  // tanda al final (botón "CARGAR MÁS"), sin tocar lo que ya había.
  _renderExplorarUsuarios({ users, niveles, friendIds, enviadas, recibidas }, append = false) {
    const container = document.getElementById('todosUsuariosList');
    if (!container) return;
    let html = '';
    ...
    if (!append && !html) html = '<p style="text-align:center; padding:20px;">No hay más usuarios</p>';
  ...
  async buscarUsuarios() {
    ...
    const terminoCache = term.toLowerCase();

    // 🔥 v3.50: "CARGANDO" con letras de colores (igual que el panel de
    // soporte del admin) en vez del esqueleto de la v3.49. Y, como el
    // input no tiene debounce (busca en cada pulsación), se guarda cada
    // término ya buscado en memoria y se reutiliza tal cual mientras no
    // haya un cambio real (ver _invalidarCacheUsuarios) -- así, si
    // borras y vuelves a escribir el mismo término, no hace falta
    // esperar de nuevo a Firestore. El token evita que la respuesta de
    // una búsqueda vieja que llega tarde pise el resultado de la más
    // reciente si has seguido escribiendo mientras tanto.
    this._busquedaToken = (this._busquedaToken || 0) + 1;
    const miToken = this._busquedaToken;

    if (this._cacheBusquedaVigente && this._cacheBusqueda[terminoCache]) {
      this._renderResultadosBusqueda(this._cacheBusqueda[terminoCache], container);
      return;
    }

    const esperarFinAnimacion = this._mostrarCargando(container, 'buscarUsuariosLoadingText');

    try {
      if (!AppState.currentUserData?.friendIds) await this._refreshCurrentUserData();
      const friendIds = AppState.currentUserData?.friendIds || [];
      const result = await Storage.searchUsersByUsername(term, 10);
      if (miToken !== this._busquedaToken) return; // llegó tarde: ya hay una búsqueda más nueva en curso
      const { enviadas, recibidas } = await this.obtenerSolicitudesPendientes();
      if (miToken !== this._busquedaToken) return;
      const uids = result.items.map(u => u.uid);
      const niveles = await this.getNivelesDirectos(uids);
      if (miToken !== this._busquedaToken) return;
      const datos = { items: result.items, niveles, friendIds, enviadas, recibidas };
      this._cacheBusqueda[terminoCache] = datos;
      this._cacheBusquedaVigente = true;
      await esperarFinAnimacion();
      if (miToken !== this._busquedaToken) return;
      this._renderResultadosBusqueda(datos, container);
    } catch (error) {
      if (miToken !== this._busquedaToken) return;
  ...
  _renderResultadosBusqueda({ items, niveles, friendIds, enviadas, recibidas }, container) {
    if (items.length === 0) { container.innerHTML = '<p style="text-align:center; padding:20px;">No se encontraron usuarios</p>'; return; }
    let html = '';
...
console.log('✅ friends.js v3.50 - "Buscar"/"Explorar usuarios" cargan con el mismo "CARGANDO" de letras del panel de soporte admin, y se cachean hasta que hay un cambio real');
