...
// Versión: 3.6 - Modal posicionado dinámicamente cerca del elemento clickeado
// ====================
...
const Friends = {
  ...
  initEventListeners() {
    document.addEventListener('click', (e) => {
      ...
      if (action === 'agregar' && uid && username) {
        this.enviarSolicitud(uid, username);
      } else if (action === 'aceptar' && uid && requestId) {
        this.aceptarSolicitud(requestId, uid, AppState.currentUserId);
      } else if (action === 'rechazar' && uid && requestId) {
        this.rechazarSolicitud(requestId);
      } else if (action === 'cancelar' && requestId) {
        this.cancelarSolicitud(requestId);
      }
    });
  },
  ...
  async obtenerSolicitudesPendientes() {
    ...
    try {
      const [enviadasSnap, recibidasSnap] = await Promise.all([
        ...
        firebaseServices.db
          .collection('friendRequests')
          .where('to', '==', AppState.currentUserId)
          .where('status', '==', 'pending')
          .get()
      ]);
  ...
  async cargarSolicitudesRecibidas() {
    ...
    try {
      ...
      for (const sol of recibidas) {
        ...
        const usernameFormatted = Utils.capitalizeUsername(fromUser?.username || 'Usuario');

        html += `
          <div class="solicitud-item">
            <div class="resultado-info" data-uid="${sol.from}" onclick="Friends.abrirModalAmigo('${sol.from}', this)">
              ${avatarHTML}
              <div>
                <div class="resultado-nombre">${usernameFormatted}</div>
                <div class="resultado-username">@${fromUser?.username || ''}</div>
              </div>
            </div>
            <div class="solicitud-botones">
              <button class="btn-amigo" data-friend-action="aceptar" data-request-id="${sol.id}" data-uid="${sol.from}" data-username="${usernameFormatted}" style="background:var(--zone-2); border-color:var(--zone-2); color:var(--bg-primary);">✓ Aceptar</button>
  ...
  async cargarSolicitudesEnviadas() {
    ...
    Utils.showLoading();
    try {
      const { enviadas } = await this.obtenerSolicitudesPendientes();
      ...
      let html = '';
      for (const sol of enviadas) {
        try {
          const toUser = await Storage.getUser(sol.to);
          const avatarHTML = `<div class="resultado-avatar" style="background:var(--bg-secondary); display:flex; align-items:center; justify-content:center;">👤</div>`;
          const username = Utils.capitalizeUsername(toUser?.username || 'Usuario desconocido');
          const userTag = toUser?.username ? `@${toUser.username}` : 'cuenta no disponible';

          html += `
            <div class="solicitud-item">
              <div class="resultado-info" data-uid="${sol.to}" onclick="Friends.abrirModalAmigo('${sol.to}', this)">
                ${avatarHTML}
                <div>
              ...
              <div class="solicitud-botones">
                <button class="btn-amigo eliminar" data-friend-action="cancelar" data-request-id="${sol.id}" data-uid="${sol.to}">✖️ Cancelar</button>
              </div>
            </div>
          `;
        } catch (err) {
          ...
          html += `
            <div class="solicitud-item">
              ...
              <div class="solicitud-botones">
                <button class="btn-amigo eliminar" data-friend-action="cancelar" data-request-id="${sol.id}">✖️ Cancelar</button>
              </div>
            </div>
          `;
        }
      ...
      if (error.code === 'failed-precondition' || error.message?.includes('index')) {
        container.innerHTML = '<p style="text-align:center; color:var(--zone-5);">Error de índice. Contacta al administrador.</p>';
      } else {
        container.innerHTML = '<p style="text-align:center; color:var(--zone-5);">Error al cargar solicitudes</p>';
      }
    } finally {
      Utils.hideLoading();
    }
      ...
      amigos.forEach(amigo => {
        ...
        html += `
          <div class="resultado-busqueda" style="justify-content:space-between;">
            <div class="resultado-info" data-uid="${amigo.uid}" onclick="Friends.abrirModalAmigo('${amigo.uid}', this)">
              ${avatarHTML}
              <div>
  ...
  // ========== FUNCIÓN CORREGIDA CON POSICIONAMIENTO DINÁMICO ==========
  async abrirModalAmigo(uid, elemento) {
    Utils.showLoading();
    try {
      ...
      const modal = document.getElementById('modalAmigo');
      const overlay = document.getElementById('modalAmigoOverlay');

      // Posicionar modal cerca del elemento clickeado
      let useCentered = true;
      if (elemento && elemento.getBoundingClientRect) {
        try {
          const rect = elemento.getBoundingClientRect();
          let modalWidth = modal.offsetWidth;
          let modalHeight = modal.offsetHeight;

          // Si el modal aún no tiene dimensiones (por estar oculto), esperamos un frame
          ...
          if (modalWidth > 0 && modalHeight > 0) {
            let top = rect.bottom + window.scrollY + 10; // justo debajo
            ...
            console.warn('Modal sin dimensiones, usando centrado');
          }
        } catch (err) {
          console.warn('Error calculando posición del modal:', err);
        }
      }
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    try {
      ...
      for (const user of result.users) {
        ...
        html += `
          <div class="resultado-busqueda">
            <div class="resultado-info" data-uid="${user.uid}" onclick="Friends.abrirModalAmigo('${user.uid}', this)">
              ${avatarHTML}
              <div>
                <div class="resultado-nombre">${usernameFormatted}</div>
      ...
      const loadMoreBtn = document.getElementById('cargarMasUsuariosBtn');
      if (loadMoreBtn) {
        loadMoreBtn.style.display = this.todosUsuariosPagination.hasMore ? 'block' : 'none';
      }
