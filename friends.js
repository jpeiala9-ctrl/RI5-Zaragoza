...
const Friends = {
  ...
  // 🔥 v3.49: mismas keyframes "riSkeletonPulse"/"riFadeInUp" que ya usa
  // session-invites.js para "Últimas sesiones creadas" (mismo id de
  // <style>, así que si el otro módulo ya las inyectó, esto no hace
  // nada). Se usan para que "Buscar" (búsqueda por nombre y explorar
  // usuarios) cargue con el mismo lenguaje visual: tarjetas esqueleto
  // pulsando desde el primer instante en vez de un hueco en blanco, y
  // fundido al llegar los resultados reales.
  _asegurarEstiloCarga() {
  ...
  _skeletonUsuarios(n = 3) {
    return Array.from({ length: n }).map((_, i) => `
      <div style="height:60px; border-radius:10px; background:var(--stat-bg); border:1px solid var(--border-color); margin-bottom:8px; animation:riSkeletonPulse 1.2s ease-in-out infinite; animation-delay:${(i * 0.15).toFixed(2)}s;"></div>
    `).join('');
  },
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    if (reset) {
      // 🔥 v3.49: mismo esqueleto pulsando que "Últimas sesiones creadas"
      // del admin, en vez del texto fijo "Cargando usuarios...".
      container.innerHTML = this._skeletonUsuarios(3);
  ...
  _renderExplorarUsuarios({ users, niveles, friendIds, enviadas, recibidas }, append = false) {
    ...
    for (const user of users) {
      ...
      html += `
        <div class="resultado-busqueda" data-uid="${user.uid}" style="animation:riFadeInUp 0.25s ease both; animation-delay:${(Math.min(idx, 9) * 0.04).toFixed(2)}s;" onclick="if(!event.target.closest('button')) Friends.abrirModalAmigo('${user.uid}')">
          <div class="resultado-info">
            ${avatarHTML}
            <div>
              <div class="resultado-nombre">${Utils.escapeHTML(usernameFormatted)}</div>
  ...
  async buscarUsuarios() {
    ...
    // 🔥 v3.49: se carga igual que "Últimas sesiones creadas" del admin --
    // esqueleto pulsando de inmediato (en vez del hueco en blanco de
    // antes mientras respondía Firestore) y fundido al llegar. Como no
    // hay debounce en el input (se busca en cada pulsación), se usa
    // además un token: si el usuario sigue escribiendo, la respuesta de
    // una búsqueda vieja que llega tarde ya no pisa el resultado de la
    // más reciente.
    this._asegurarEstiloCarga();
...
console.log('✅ friends.js v3.49 - "Buscar"/"Explorar usuarios" carga con esqueleto + fundido, igual que el historial de admin');