...
// Versión: 3.16 - CORREGIDO: Actualización del contador de amigos en perfil al eliminar amigo
// ====================
...
const Friends = {
  ...
  async _refreshCurrentUserData() {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        // CORRECCIÓN: Sincronizar friendsCount con la longitud real de friendIds
        const friendIds = newData.friendIds || [];
        const actualFriendCount = friendIds.length;
        
        // Si hay inconsistencia, actualizar Firestore
        if (newData.friendsCount !== actualFriendCount) {
          console.log(`🔧 Corrigiendo friendsCount: ${newData.friendsCount} → ${actualFriendCount}`);
          ...
          newData.friendsCount = actualFriendCount;
        }
  ...
  async aceptarSolicitud(requestId, fromUid, toUid) {
    try {
      // 1. Aceptar la solicitud de amistad (actualiza friendIds y friendsCount en Firestore)
      ...
      // 3. CORRECCIÓN: Refrescar datos del usuario actual para actualizar el contador
      ...
      // 5. Actualizar perfil si está visible
  ...
  async cargarListaAmigos() {
    ...
    try {
      // CORRECCIÓN: Refrescar datos antes de cargar la lista
      await this._refreshCurrentUserData();
  ...
  async eliminarAmigo(friendUid) {
    ...
    try {
      ...
      // CORRECCIÓN: Refrescar datos del usuario actual
      await this._refreshCurrentUserData();
      
      // Refrescar la lista de amigos
      await this.cargarListaAmigos();
      
      // Refrescar la lista de "Explorar usuarios"
      await this.cargarTodosUsuarios(true);
      
      // CORRECCIÓN: Actualizar perfil si está visible (ESTA ES LA LÍNEA QUE FALTABA)
      if (window.Profile && document.getElementById('tab-perfil')?.classList.contains('active')) {
        await Profile.cargarPerfil();
      }
      
      // También actualizar el badge de solicitudes
      await this.actualizarBadgeSolicitudes();
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    try {
      // CORRECCIÓN: Asegurar que tenemos los datos actualizados
      await this._refreshCurrentUserData();
