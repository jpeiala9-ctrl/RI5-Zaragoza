...
const Friends = {
  ...
  // Los resultados de "Explorar usuarios" (ahora la única lista) se
  // guardan en memoria y se reutilizan tal cual -- sin volver a pedir
  // nada a Firestore -- hasta que hay un cambio real (se envía, cancela,
  // acepta o rechaza una solicitud; ver _invalidarCacheUsuarios).
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    if (reset && this._cacheExplorarVigente && this._cacheExplorarUsuarios) {
      // Ya en caché y sin cambios desde la última vez: se pintan al
      // instante, respetando lo que haya escrito en el buscador -- sin
      // parpadeo ni animación.
      this._aplicarFiltroYRenderizar();
      return;
    }

    this._cargandoTodosUsuarios = true;
    const esperarFinAnimacion = this._mostrarCargando(container, 'explorarUsuariosLoadingText');
    ...
    try {
      ...
      // 🔥 v3.51: antes se pedía de 20 en 20 con un botón "CARGAR MÁS".
      // Ahora se trae la lista COMPLETA (tandas de 200, encadenadas hasta
      // que una vuelve incompleta) para que el buscador de más abajo
      // pueda filtrar en el cliente sobre TODOS los usuarios, no solo los
      // ya visibles. Límite de seguridad de 25 tandas (5000 usuarios).
      let users = [];
      let cursor = null;
      const TANDA = 200;
      for (let i = 0; i < 25; i++) {
        const result = await Storage.getAllUsers(TANDA, cursor);
        users = users.concat(result.users);
        if (result.users.length < TANDA) break;
  ...
  // Filtra la lista YA CARGADA en caché por nombre de usuario, en el
  // cliente -- sin red y sin animación de carga -- y la vuelve a pintar
  // al instante en cada pulsación (ver oninput en index.html). Si la
  // caché todavía se está cargando por primera vez, no hace nada: la
  // carga en curso ya pintará el resultado (filtrado, si ya hay texto
  // escrito) en cuanto termine.
  filtrarUsuarios() {
    if (!this._cacheExplorarVigente || !this._cacheExplorarUsuarios) return;
    this._aplicarFiltroYRenderizar();
  },

  _aplicarFiltroYRenderizar() {
    if (!this._cacheExplorarUsuarios) return;
    const term = (document.getElementById('buscarAmigosInput')?.value || '').trim().toLowerCase();
    const { users, niveles, friendIds, enviadas, recibidas } = this._cacheExplorarUsuarios;
    // Sin texto: la lista completa (esto es lo que hace que borrar lo
    // escrito vuelva a enseñar a todos los usuarios). Con texto: solo los
    // que coinciden por su @username o por su nombre visible.
    const filtrados = term
      ? users.filter(u => {
          const username = (u.username || '').toLowerCase();
          const nombre = Utils.capitalizeUsername(u.username || '').toLowerCase();
          return username.includes(term) || nombre.includes(term);
        })
      : users;
    this._renderExplorarUsuarios({ users: filtrados, niveles, friendIds, enviadas, recibidas });
  },
...
console.log('✅ friends.js v3.51 - Buscar/Explorar unificados en una sola lista: se carga completa una vez (con animación solo si no había caché) y el buscador filtra en el cliente al instante, sin animación y sin volver a tocar Firestore');
