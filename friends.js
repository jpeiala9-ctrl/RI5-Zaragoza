// ==================== friends.js - Gestión de amigos con pasaporte (avatar incluido) ====================
// Versión: 3.46 - PAGINACIÓN CLÁSICA (20 usuarios por página, no acumulación)
// ====================

const Friends = {
  todosUsuariosPagination: {
    lastDoc: null,
    hasMore: true,
    loading: false,
    currentPage: 0
  ...
  async obtenerSolicitudesPendientes() {
    ...
    try {
      ...
      const enviadas = enviadasSnap.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
      const recibidas = recibidasSnap.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
      return {
  ...
  cargarSolicitudesRecibidas() {
    this.cargarSolicitudes('recibidas');
  },
  cargarSolicitudesEnviadas() {
    this.cargarSolicitudes('enviadas');
  },
    ...
    try {
      ...
      if (!convDoc.exists) {
        const [userData, friendData] = await Promise.all([
          Storage.getUser(fromUid).catch(() => ({
            username: 'Usuario',
            profile: {}
          })),
          Storage.getUser(toUid).catch(() => ({
            username: 'Amigo',
            profile: {}
          }))
        ]);
  ...
  async cargarListaAmigos(forceRefresh = false) {
    ...
    try {
      ...
      const {
        validIds,
        changed
      } = await this._limpiarAmigosHuérfanos(AppState.currentUserId, friendIds);
  ...
  // ---------- MODAL DE PERFIL DE USUARIO (PASAPORTE CON AVATAR) ----------
  async abrirModalAmigo(uid) {
    // Si es el propio usuario, mostrar modal especial
  ...
  // ---------- EXPLORAR USUARIOS (PAGINACIÓN CLÁSICA) ----------
  async cargarTodosUsuarios(reset = false) {
    ...
    if (reset) {
      ...
      this.todosUsuariosPagination.currentPage = 0;
    }
    ...
    try {
      ...
      if (result.users.length === 0) {
        if (reset) container.innerHTML = '<p style="text-align:center; padding:20px;">No hay usuarios disponibles</p>';
      ...
      this.todosUsuariosPagination.currentPage++;

      const uids = result.users.map(u => u.uid);
      const niveles = await this.getNivelesDirectos(uids);

      // Renderizar SOLO los usuarios de esta página (no acumular)
      this._renderExplorarUsuariosPaginado({
        users: result.users,
        niveles,
        friendIds,
        enviadas,
        recibidas,
        pagina: this.todosUsuariosPagination.currentPage
      });

      // Actualizar botón
      if (loadMoreBtn) {
        if (this.todosUsuariosPagination.hasMore) {
  ...
  _renderExplorarUsuariosPaginado({
    users,
    niveles,
    friendIds,
    enviadas,
    ...
    for (const user of users) {
      ...
      if (solicitudPendiente) {
        if (solicitudEnviada) {
          ...
          boton = `<button class="btn-amigo" data-friend-action="aceptar" data-request-id="${solicitudId}" data-uid="${user.uid}" data-username="${usernameFormatted}" style="background:var(--zone-2); border-color:var(--zone-2); color:var(--bg-primary);">✓ Aceptar</button>
  ...
  _mostrarInfoPagina() {
    ...
    const totalItems = container.querySelectorAll('.resultado-busqueda').length;
    const pagina = this.todosUsuariosPagination.currentPage || 1;

    const oldInfo = document.getElementById('infoPaginaUsuarios');
    if (oldInfo) oldInfo.remove();

    const info = document.createElement('div');
    info.id = 'infoPaginaUsuarios';
    info.style.cssText = 'text-align: center; font-size: 12px; color: var(--text-secondary); margin: 10px 0; padding: 5px;';
    info.textContent = `📄 Página ${pagina} · Mostrando ${totalItems} usuarios`;

    btn.parentNode.insertBefore(info, btn);
  },

  cargarMasTodosUsuarios() {
    if (this.todosUsuariosPagination.loading) {
      Utils.showToast('Cargando...', 'info');
      return;
    }

    if (this.todosUsuariosPagination.hasMore) {
      this.cargarTodosUsuarios(false); // false = siguiente página (no reset)
    } else {
