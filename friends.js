...
const Friends = {
  ...
  async cargarEntrenadoPor() {
    ...
    try {
      ...
      entrenadores.forEach((uid, i) => {
        ...
        html += `
          <div class="resultado-busqueda" style="justify-content:space-between; margin-bottom:8px;">
            <div class="resultado-info"><div class="resultado-nombre">${Utils.escapeHTML(nombre)}</div></div>
            <button class="btn-amigo eliminar" data-trainer-action="dejar-de-ser-alumno" data-uid="${uid}" data-username="${Utils.escapeHTML(nombre)}">DEJAR DE SER SU ALUMNO</button>
          </div>
        `;
      });
      container.innerHTML = html;
  ...
  async dejarDeSerAlumno(trainerUid, nombre) {
    const confirmado = await Utils.confirm('Dejar de ser su alumno', `¿Seguro que ${nombre} deje de poder enviarte sesiones? Seguiríais siendo amigos igualmente.`);
    if (!confirmado) return;
    const ok = await Storage.dejarDeSerAlumno(trainerUid, AppState.currentUserId);
    if (!ok) { Utils.showToast('Error al dejar de ser su alumno', 'error'); return; }
    Utils.showToast(`Ya no eres alumno de ${nombre}`, 'info');
    await this._refreshCurrentUserData();
...
console.log('✅ friends.js v3.57 - "Mis alumnos" reordenado debajo de la sesión, caché persistida en sessionStorage sin animación de carga, y nuevo "Entrenado por" en el perfil para dejar de ser alumno de alguien');
