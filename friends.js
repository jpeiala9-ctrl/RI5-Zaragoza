...
html += `<div class="solicitud-item"> <div class="resultado-info" data-uid="${_esc1(otroUid)}" onclick="Friends.abrirModalAmigo('${_esc1(otroUid)}')"> ${avatarHTML} <div> <div class="resultado-nombre">${_esc1(usernameFormatted)}</div> <div class="resultado-username">${_esc1(userTag)}</div> </div> </div> <div class="solicitud-botones">${botones}</div> </div>`;
}
...
const _esc = Utils.escapeHTML.bind(Utils);
html += `<div class="resultado-busqueda" style="justify-content:space-between;"> <div class="resultado-info" data-uid="${_esc(amigo.uid)}" onclick="Friends.abrirModalAmigo('${_esc(amigo.uid)}')"> ${avatarHTML} <div> <div class="resultado-nombre ${nombreClass}">${_esc(usernameFormatted)}</div> <div class="resultado-username">@${_esc(amigo.username)}</div> </div> </div> <div style="display:flex; gap:8px;"> <button class="btn-amigo chat-btn" onclick="event.stopPropagation(); Chat.startChatWithFriend('${_esc(amigo.uid)}', '${_esc(usernameFormatted)}')">💬 CHAT</button> <button class="btn-amigo eliminar" onclick="event.stopPropagation(); Friends.eliminarAmigo('${_esc(amigo.uid)}')">✕</button> </div> </div>`;
}
container.innerHTML = html;
...
const contenido = `${avatarHTML} <div style="margin-bottom:16px;"> <div style="font-size:24px; color:var(--accent-yellow);">${esc(usernameFormatted)}</div> <div style="font-size:14px; color:var(--text-secondary);">@${esc(userData.username)}</div> </div> <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; text-align:left;"> <div><strong>Bio:</strong> ${esc(bio)}</div> <div><strong>Ciudad:</strong> ${esc(city)}</div> <div><strong>Edad:</strong> ${esc(age)}</div> <div><strong>Género:</strong> ${esc(gender)}</div> <div><strong>Peso:</strong> ${esc(weight)}</div> <div><strong>Altura:</strong> ${esc(height)}</div> </div>`;
...
else boton = `<button class="btn-amigo" data-friend-action="aceptar" data-request-id="${solicitudId}" data-uid="${user.uid}" data-username="${usernameFormatted}">✓ Aceptar</button> <button class="btn-amigo eliminar" data-friend-action="rechazar" data-request-id="${solicitudId}" data-uid="${user.uid}">✗ Rechazar</button>`;
...
html += `<div class="resultado-busqueda"> <div class="resultado-info" data-uid="${_esc2(user.uid)}" onclick="Friends.abrirModalAmigo('${_esc2(user.uid)}')"> ${avatarHTML} <div> <div class="resultado-nombre">${_esc2(usernameFormatted)}</div> <div class="resultado-username">@${_esc2(user.username)}</div> </div> </div> <div>${boton}</div> </div>`;
}
container.innerHTML += html;
...
html += `<div class="resultado-busqueda"> <div class="resultado-info" data-uid="${_esc3(user.uid)}" onclick="Friends.abrirModalAmigo('${_esc3(user.uid)}')"> ${avatarHTML} <div> <div class="resultado-nombre">${_esc3(usernameFormatted)}</div> <div class="resultado-username">@${_esc3(user.username)}</div> </div> </div> <div>${boton}</div> </div>`;
}
container.innerHTML = html;
