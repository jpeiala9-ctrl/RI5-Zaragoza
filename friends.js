...
// Versión: 3.18 - AÑADIDO: Soporte para notificaciones verdes de chat
// ====================
...
const Friends = {
  ...
  async cancelarSolicitud(requestId) {
    ...
    try {
      ...
      if (error.code === 'not-found') { 
        ...
        await this.actualizarBadgeSolicitudes();
        await this.cargarSolicitudesEnviadas();
      } else Utils.showToast('Error al cancelar', 'error'); 
    }
  },

  // ✅ FUNCIÓN CORREGIDA CON SOPORTE PARA NOTIFICACIONES VERDES
  async cargarListaAmigos() {
    ...
    try {
      ...
      // ✅ Obtener conversaciones para saber qué amigos tienen mensajes no leídos
      let amigosConMensajes = new Set();
      if (window.Chat && window.Chat.getConversations) {
        try {
          const conversations = await Chat.getConversations(AppState.currentUserId);
          conversations.forEach(conv => {
            if (conv.unreadCount > 0) {
              amigosConMensajes.add(conv.otherUserId);
            }
          });
        } catch (e) { console.warn('Error obteniendo conversaciones:', e); }
      }
      
      let html = '';
      for (const amigo of amigos) {
        const tieneMensajes = amigosConMensajes.has(amigo.uid);
        const nombreClass = tieneMensajes ? 'amigo-con-mensajes' : '';
        ...
        html += `
          <div class="resultado-busqueda" style="justify-content:space-between;">
            <div class="resultado-info" data-uid="${amigo.uid}" onclick="Friends.abrirModalAmigo('${amigo.uid}')">
              ${avatarHTML}
              <div>
                <div class="resultado-nombre ${nombreClass}">${Utils.escapeHTML(usernameFormatted)}</div>
      ...
      // ✅ Actualizar color de pestaña MIS AMIGOS
      const amigosTabs = document.querySelectorAll('.amigos-tab');
      if (amigosTabs.length >= 3) {
        const misAmigosTab = amigosTabs[2];
        if (AppState.mensajesAmigosNoLeidos > 0) {
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    try {
      ...
      if (result.users.length === 0) { if (reset) container.innerHTML = '<p style="text-align:center; padding:20px;">No hay más usuarios</p>'; this.todosUsuariosPagination.hasMore = false; this.todosUsuariosPagination.loading = false; return; }
  ...
  async buscarUsuarios() {
    ...
    try {
      ...
      container.innerHTML = html;
    } catch (error) { console.error('Error buscando usuarios:', error); container.innerHTML = '<p style="text-align:center; color:var(--zone-5);">Error en la búsqueda</p>'; }
  }
};
