...
// Versión: 3.0 - Con exploración de todos los usuarios, cancelación de solicitudes y badge
...
const Friends = {
  ...
  async cancelarSolicitud(requestId) {
    ...
    Utils.showLoading();
    try {
      await firebaseServices.db.collection('friendRequests').doc(requestId).delete();
        ...
        html += `
          <div class="solicitud-item">
            <div class="resultado-info" onclick="Friends.abrirModalAmigo('${sol.to}')">
              ${avatarHTML}
              <div>
                <div class="resultado-nombre">${toUser?.username || 'Usuario'}</div>
                <div class="resultado-username">@${toUser?.username || ''}</div>
              </div>
            </div>
            <div class="solicitud-botones">
