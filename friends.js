// ==================== friends.js - Gestión de amigos con botones de pasaporte estilo estándar ====================
// Versión: 3.36 - Botones del modal con estilo normal (como en explorar), título dinámico.
// ====================
...
const Friends = {
  ...
  // ---------- MODAL DE PERFIL DE USUARIO (PASAPORTE CON BOTONES ESTILO EXPLORAR) ----------
  async abrirModalAmigo(uid) {
    try {
      ...
      // Cambiar el título del modal por el nombre del usuario
      ...
      if (esAmigo) {
        botonAccion = `<span style="font-size:13px; color:var(--zone-2);">✓ Ya son amigos</span>`;
      } else {
        ...
        if (solicitudEnviada) {
          ...
          botonAccion = `<button class="btn-amigo" data-friend-action="cancelar" data-request-id="${reqId}" data-uid="${uid}" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:8px 16px; border-radius:10px; font-size:13px; cursor:pointer;">✖️ Cancelar solicitud</button>`;
        } else if (solicitudRecibida) {
          const reqId = recibidas.find(s => s.from === uid).id;
          botonAccion = `
            <div style="display:flex; gap:10px; justify-content:center;">
              <button class="btn-amigo" data-friend-action="aceptar" data-request-id="${reqId}" data-uid="${uid}" data-username="${Utils.escapeHTML(userName)}" style="background:var(--zone-2); border-color:var(--zone-2); color:var(--bg-primary); padding:8px 16px; border-radius:10px; font-size:13px; cursor:pointer;">✓ Aceptar</button>
              <button class="btn-amigo eliminar" data-friend-action="rechazar" data-request-id="${reqId}" data-uid="${uid}" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:8px 16px; border-radius:10px; font-size:13px; cursor:pointer;">✗ Rechazar</button>
            </div>
          `;
        } else {
          botonAccion = `<button class="btn-amigo" data-friend-action="agregar" data-uid="${uid}" data-username="${Utils.escapeHTML(userName)}" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:8px 16px; border-radius:10px; font-size:13px; cursor:pointer;">➕ Agregar amigo</button>`;
        }
      }
      ...
      const contenido = `
        <div class="passport-card" style="border:2px solid ${levelColor}; border-radius:24px; background:rgba(${parseInt(levelColor.slice(1,3),16)}, ${parseInt(levelColor.slice(3,5),16)}, ${parseInt(levelColor.slice(5,7),16)}, 0.05); box-shadow:0 8px 20px rgba(0,0,0,0.1); overflow:hidden; text-align:left;">
          <!-- Cuerpo (sin cabecera adicional, ya que el título está en h3) -->
          <div style="padding:16px 20px;">
            ...
            <!-- Botón de acción estilo explorar (centrado, tamaño normal) -->
  ...
  _renderExplorarUsuarios({ users, niveles, friendIds, enviadas, recibidas }) {
    ...
    for (const user of users) {
      ...
      if (solicitudPendiente) {
        if (solicitudEnviada) boton = `<button class="btn-amigo" data-friend-action="cancelar" data-request-id="${solicitudId}" data-uid="${user.uid}" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:8px 16px; border-radius:10px; font-size:13px; cursor:pointer;">✖️ Cancelar</button>`;
        else boton = `<button class="btn-amigo" data-friend-action="aceptar" data-request-id="${solicitudId}" data-uid="${user.uid}" data-username="${usernameFormatted}" style="background:var(--zone-2); border-color:var(--zone-2); color:var(--bg-primary); padding:8px 16px; border-radius:10px; font-size:13px; cursor:pointer;">✓ Aceptar</button>
                      <button class="btn-amigo eliminar" data-friend-action="rechazar" data-request-id="${solicitudId}" data-uid="${user.uid}" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:8px 16px; border-radius:10px; font-size:13px; cursor:pointer;">✗ Rechazar</button>`;
      } else boton = `<button class="btn-amigo" data-friend-action="agregar" data-uid="${user.uid}" data-username="${usernameFormatted}" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:8px 16px; border-radius:10px; font-size:13px; cursor:pointer;">➕ Agregar</button>`;
      html += `
