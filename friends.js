// ==================== friends.js - Gestión de amigos + NIVEL CORRECTO EN AVATAR Y TEXTO ====================
// Versión: 3.19 - El badge del nivel se genera directamente con el valor obtenido de Firestore
// ====================
...
const Friends = {
  ...
  async getNivelesMultiples(uids) {
    const resultados = {};
    const uidsSinCache = uids.filter(uid => !this.nivelCache.has(uid));
    if (uidsSinCache.length > 0) {
      try {
        for (let i = 0; i < uidsSinCache.length; i += 10) {
          const batch = uidsSinCache.slice(i, i + 10);
          const docs = await firebaseServices.db.collection('gamification')
            .where(firebaseServices.FieldPath.documentId(), 'in', batch)
            .get();
          docs.forEach(doc => {
            const nivel = doc.data().level || 1;
            this.nivelCache.set(doc.id, nivel);
            resultados[doc.id] = nivel;
          });
          batch.forEach(uid => {
            if (!resultados[uid]) {
              this.nivelCache.set(uid, 1);
              resultados[uid] = 1;
            }
          });
        }
      } catch (e) {
        console.warn('Error obteniendo niveles múltiples:', e);
        uidsSinCache.forEach(uid => {
          this.nivelCache.set(uid, 1);
          resultados[uid] = 1;
        });
      }
    ...
    uids.forEach(uid => {
      if (!resultados[uid]) resultados[uid] = this.nivelCache.get(uid) || 1;
    });
    return resultados;
  },
  ...
  async cargarSolicitudes(tipo) {
    ...
    try {
      ...
      const niveles = await this.getNivelesMultiples(uids);
      uids.forEach(uid => {
        const badge = document.getElementById(`nivel-${uid}`);
        if (badge) badge.textContent = niveles[uid];
      });
    } catch (error) { 
  ...
  async cargarListaAmigos() {
    ...
    try {
      ...
      // Limpiar amigos inválidos
      const amigosValidos = [];
      let necesitaActualizacion = false;
      const snapshots = await Promise.all(friendIds.map(fid => firebaseServices.db.collection('users').doc(fid).get()));
      const amigos = [];
      ...
      // Obtener niveles
      const niveles = await this.getNivelesMultiples(amigos.map(a => a.uid));
      let html = '';
      for (const amigo of amigos) {
        const nivel = niveles[amigo.uid] || 1;
        const photoURL = amigo.profile?.photoURL;
        const avatarHTML = photoURL
          ? `<div class="resultado-avatar-wrapper"><img src="${Utils.escapeHTML(photoURL)}" class="resultado-avatar" style="object-fit:cover;"><div class="nivel-badge" data-uid="${amigo.uid}">${nivel}</div></div>`
          : `<div class="resultado-avatar-wrapper"><div class="resultado-avatar" style="background:var(--bg-secondary); display:flex; align-items:center; justify-content:center;">👤</div><div class="nivel-badge" data-uid="${amigo.uid}">${nivel}</div></div>`;
        ...
        html += `
          <div class="resultado-busqueda" style="justify-content:space-between;">
            <div class="resultado-info" data-uid="${amigo.uid}" onclick="Friends.abrirModalAmigo('${amigo.uid}')">
              ${avatarHTML}
              <div>
                <div class="resultado-nombre">${Utils.escapeHTML(usernameFormatted)} <span style="font-size:10px; color:var(--accent-yellow);">(Nv. ${nivel})</span></div>
  ...
  async abrirModalAmigo(uid) {
    try {
      ...
      `;
      try {
        const gamificationDoc = await firebaseServices.db.collection('gamification').doc(uid).get();
        if (gamificationDoc.exists) {
          const g = gamificationDoc.data();
          ...
          if (badgesToShow.length > 0) {
            badgesHTML = '<div style="margin-top:12px; display:flex; flex-wrap:wrap; gap:6px; justify-content:center;">';
            badgesToShow.forEach(badgeId => {
              const badgeInfo = window.Gamification?.BADGES?.[badgeId];
              if (badgeInfo) {
                badgesHTML += `<span class="badge" title="${badgeInfo.description}">${badgeInfo.icon} ${badgeInfo.name}</span>`;
              } else {
                badgesHTML += `<span class="badge">🏅 ${badgeId}</span>`;
              }
            });
            badgesHTML += '</div>';
          } else {
            badgesHTML = '<p style="margin-top:12px; font-size:12px; text-align:center;">Todavía no tiene insignias</p>';
          }
          contenido += `
            <div style="margin-top:20px; padding-top:12px; border-top:1px solid var(--border-color); text-align:center;">
              <div style="display:flex; justify-content:space-around; margin-bottom:8px;">
                <div><strong>🏆 Nivel</strong><br>${level}</div>
                <div><strong>✨ XP</strong><br>${totalXP}</div>
                <div><strong>🔥 Racha</strong><br>${streak} días</div>
              </div>
              ${badgesHTML}
            </div>
          `;
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    try {
      ...
      for (const user of result.users) {
        ...
        const avatarHTML = photoURL
          ? `<div class="resultado-avatar-wrapper"><img src="${Utils.escapeHTML(photoURL)}" class="resultado-avatar" style="object-fit:cover;"><div class="nivel-badge">${nivel}</div></div>`
          : `<div class="resultado-avatar-wrapper"><div class="resultado-avatar" style="background:var(--bg-secondary); display:flex; align-items:center; justify-content:center;">👤</div><div class="nivel-badge">${nivel}</div></div>`;
