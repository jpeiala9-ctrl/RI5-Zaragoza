// ==================== friends.js - Gestión de amigos con pasaporte de running ====================
// Versión: 3.30 - Modal de perfil de amigo con diseño "Running Passport".
// ====================

const Friends = {
  todosUsuariosPagination: { lastDoc: null, hasMore: true, loading: false },

  // ---------- MÉTODOS DE GAMIFICACIÓN (para nivel, color, etc.) ----------
  ...
  // ---------- MODAL DE AMIGO CON DISEÑO PASAPORTE ----------
  async abrirModalAmigo(uid) {
    try {
      ...
      // Obtener datos de gamificación y zapatilla
      ...
      // Determinar estado de amistad y botón de acción
      const friendIds = AppState.currentUserData?.friendIds || [];
      const esAmigo = friendIds.includes(uid);
      let botonAccion = '';
      
      if (esAmigo) {
        botonAccion = `<button class="action-button chat-btn" data-uid="${uid}" data-username="${Utils.escapeHTML(Utils.capitalizeUsername(userData.username))}" style="width:auto; padding:8px 16px;">💬 CHAT</button>
                       <button class="action-button delete-friend-btn" data-uid="${uid}" style="width:auto; padding:8px 16px; border-color:var(--zone-5);">🗑️ ELIMINAR</button>`;
      } else {
        ...
        if (solicitudEnviada) {
          ...
          botonAccion = `<button class="action-button" data-friend-action="cancelar" data-request-id="${reqId}" data-uid="${uid}" style="width:auto;">✖️ Cancelar solicitud</button>`;
        } else if (solicitudRecibida) {
          const reqId = recibidas.find(s => s.from === uid).id;
          botonAccion = `<button class="action-button" data-friend-action="aceptar" data-request-id="${reqId}" data-uid="${uid}" data-username="${Utils.escapeHTML(Utils.capitalizeUsername(userData.username))}" style="width:auto; background:var(--zone-2);">✓ Aceptar</button>
                         <button class="action-button" data-friend-action="rechazar" data-request-id="${reqId}" data-uid="${uid}" style="width:auto;">✗ Rechazar</button>`;
        } else {
          botonAccion = `<button class="action-button" data-friend-action="agregar" data-uid="${uid}" data-username="${Utils.escapeHTML(Utils.capitalizeUsername(userData.username))}" style="width:auto;">➕ Agregar amigo</button>`;
        }
      }
      
      // Generar HTML del pasaporte
      const contenido = `
        <div class="passport-card" style="border:2px solid ${levelColor}; border-radius:24px; background:rgba(${parseInt(levelColor.slice(1,3),16)}, ${parseInt(levelColor.slice(3,5),16)}, ${parseInt(levelColor.slice(5,7),16)}, 0.05); box-shadow:0 8px 20px rgba(0,0,0,0.1); overflow:hidden; text-align:left;">
          ...
          <div style="padding:16px 20px 0 20px; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid ${levelColor}40;">
            <div style="display:flex; align-items:center; gap:8px;">
              ...
              <span style="font-size:12px; letter-spacing:1px; text-transform:uppercase; font-weight:500;">RUNNING PASSPORT</span>
            </div>
      ...
      // Reasignar eventos a los botones (para que funcionen los data-friend-action y los chat/eliminar)
      setTimeout(() => {
        ...
        // Botón chat (si existe)
        const chatBtn = document.querySelector('#modalAmigo .chat-btn');
        if (chatBtn) {
          chatBtn.onclick = (e) => {
            e.stopPropagation();
            Chat.startChatWithFriend(uid, Utils.capitalizeUsername(userData.username));
            this.cerrarModalAmigo();
          };
        }
        // Botón eliminar amigo (si existe)
        const deleteBtn = document.querySelector('#modalAmigo .delete-friend-btn');
        if (deleteBtn) {
          deleteBtn.onclick = async (e) => {
            e.stopPropagation();
            await this.eliminarAmigo(uid);
            this.cerrarModalAmigo();
          };
        }
