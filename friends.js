// ==================== friends.js - Gestión de amigos con título dinámico (nombre de usuario) ====================
// Versión: 3.34 - Título del modal = nombre del usuario, botones compactos.
// ====================
...
const Friends = {
  ...
  // ---------- MODAL DE PERFIL DE USUARIO (TÍTULO = NOMBRE DE USUARIO) ----------
  async abrirModalAmigo(uid) {
    try {
      ...
      const badgesStamps = badges.map(badgeId => {
        ...
        if (!badge) return '';
        return `<span class="badge-stamp" data-badge-name="${badge.name}" data-badge-desc="${badge.description}" style="display:inline-flex; align-items:center; gap:4px; background:var(--bg-card); border:1px solid ${levelColor}; border-radius:30px; padding:2px 6px; font-size:10px; margin:0 4px; cursor:pointer;">${badge.icon} ${badge.name}</span>`;
      }).filter(b => b).join('');
      ...
      if (esAmigo) {
        botonAccion = `<span style="font-size:11px; color:var(--zone-2);">✓ Ya son amigos</span>`;
      } else {
        ...
        if (solicitudEnviada) {
          ...
          botonAccion = `<button class="action-button" data-friend-action="cancelar" data-request-id="${reqId}" data-uid="${uid}" style="width:auto; padding:2px 8px; font-size:10px; border-radius:30px;">✖️ Cancelar solicitud</button>`;
        } else if (solicitudRecibida) {
          const reqId = recibidas.find(s => s.from === uid).id;
          botonAccion = `
            <div style="display:flex; gap:6px; justify-content:center;">
              <button class="action-button" data-friend-action="aceptar" data-request-id="${reqId}" data-uid="${uid}" data-username="${Utils.escapeHTML(userName)}" style="width:auto; padding:2px 8px; font-size:10px; border-radius:30px; background:var(--zone-2); border-color:var(--zone-2);">✓ Aceptar</button>
              <button class="action-button" data-friend-action="rechazar" data-request-id="${reqId}" data-uid="${uid}" style="width:auto; padding:2px 8px; font-size:10px; border-radius:30px;">✗ Rechazar</button>
            </div>
          `;
        } else {
          botonAccion = `<button class="action-button" data-friend-action="agregar" data-uid="${uid}" data-username="${Utils.escapeHTML(userName)}" style="width:auto; padding:2px 8px; font-size:10px; border-radius:30px;">➕ Agregar amigo</button>`;
        }
      }
      ...
      const contenido = `
        <div class="passport-card" style="border:2px solid ${levelColor}; border-radius:24px; background:rgba(${parseInt(levelColor.slice(1,3),16)}, ${parseInt(levelColor.slice(3,5),16)}, ${parseInt(levelColor.slice(5,7),16)}, 0.05); box-shadow:0 8px 20px rgba(0,0,0,0.1); overflow:hidden; text-align:left;">
          ...
          <div style="padding:16px 20px;">
            ...
            <!-- Botón de acción centrado y compacto -->
