...
// Versión: 3.3 - Correcciones críticas: UID válidos, índices, reset de notificaciones
// ====================

const Friends = {
  todosUsuariosPagination: { lastDoc: null, hasMore: true, loading: false },
  solicitudesPendientesCache: null, // cache para evitar múltiples consultas
  ...
  async obtenerSolicitudesPendientes(uid, como = null) {
    // Si no se pide filtro, devuelve todas las pendientes del usuario actual
    ...
    try {
      ...
      const enviadas = docs.filter(r => r.from === AppState.currentUserId);
      const recibidas = docs.filter(r => r.to === AppState.currentUserId);
      ...
      for (const sol of recibidas) {
        ...
        html += `
          <div class="solicitud-item">
            <div class="resultado-info" data-uid="${sol.from}" onclick="Friends.abrirModalAmigo('${sol.from}')">
              ${avatarHTML}
              <div>
  ...
  async cargarSolicitudesEnviadas() {
    ...
    Utils.showLoading();
    try {
      // Usamos la misma función optimizada
      ...
      for (const sol of enviadas) {
        try {
          ...
          html += `
            <div class="solicitud-item">
              <div class="resultado-info" data-uid="${sol.to}" onclick="Friends.abrirModalAmigo('${sol.to}')">
                ${avatarHTML}
                <div>
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    try {
      // Obtener todas las solicitudes pendientes de una vez para optimizar
  ...
  async buscarUsuarios() {
    ...
    try {
      ...
      // Obtener solicitudes pendientes de una vez
      const { enviadas, recibidas } = await this.obtenerSolicitudesPendientes();
  ...
  // Método para resetear notificaciones al entrar en la pestaña
  resetearNovedades() {
    AppState.actualizarNovedadesFeed(0, new Set());
    // Si el feed tiene algún marcado, se hará en Feed.marcarTodoLeido() desde switchTab
  }
};
