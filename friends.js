// ==================== friends.js - Gestión de amigos (CON FOTO Y SEGURIDAD XSS) ====================
// Versión: 3.17 - CORREGIDO: Contador de amigos sincronizado correctamente con la lista real
// ====================
...
let botones = ‘’;
if (tipo === ‘recibidas’) {
botones = `<button class="btn-amigo" data-friend-action="aceptar" data-request-id="${sol.id}" data-uid="${sol.from}" data-username="${Utils.escapeHTML(usernameFormatted)}" style="background:var(--zone-2); border-color:var(--zone-2); color:var(--bg-primary);">✓ Aceptar</button> <button class="btn-amigo eliminar" data-friend-action="rechazar" data-request-id="${sol.id}" data-uid="${sol.from}">✗ Rechazar</button>`;
} else {
...
html += `<div class="solicitud-item"> <div class="resultado-info" data-uid="${otroUid}" onclick="Friends.abrirModalAmigo('${otroUid}')"> ${avatarHTML} <div> <div class="resultado-nombre">${Utils.escapeHTML(usernameFormatted)}</div> <div class="resultado-username">${Utils.escapeHTML(userTag)}</div> </div> </div> <div class="solicitud-botones">${botones}</div> </div>`;
}
...
// ==================== OBTENER CONTEO REAL DE AMIGOS DESDE FIRESTORE ====================
async _getRealFriendCount() {
...
console.error(‘Error obteniendo conteo real de amigos:’, error);
}
return 0;
},

// ==================== SINCRONIZAR CONTADOR EN FIRESTORE ====================
async _syncFriendCountInFirestore() {
if (!AppState.currentUserId) return 0;
try {
const userDoc = await firebaseServices.db.collection(‘users’).doc(AppState.currentUserId).get();
if (userDoc.exists) {
...
```
    // Siempre actualizar Firestore con el valor correcto
    await firebaseServices.db.collection('users').doc(AppState.currentUserId).update({
      friendsCount: actualFriendCount
    });
    
    console.log(`✅ Contador de amigos sincronizado: ${actualFriendCount}`);
    return actualFriendCount;
  }
} catch (error) {
  console.error('Error sincronizando contador de amigos:', error);
}
return 0;
```
...
try {
// Primero sincronizar el contador en Firestore
const actualFriendCount = await this._syncFriendCountInFirestore();
...
```
  ...
  if (userDoc.exists) {
    ...
    newData.friendsCount = actualFriendCount; // Asegurar que el valor es correcto
...
async _actualizarPerfilSiVisible() {
if (window.Profile) {
...
// Si el perfil no está visible, forzar una actualización para cuando se abra
// actualizando el estado global
if (AppState.currentUserData) {
const realCount = await this._getRealFriendCount();
AppState.currentUserData.friendsCount = realCount;
}
}
}
},
...
try {
// 1. Aceptar la solicitud de amistad
...
```
  ...
  // 3. Sincronizar contador y refrescar datos
  await this._syncFriendCountInFirestore();
  ...
  // 5. Actualizar perfil
  await this._actualizarPerfilSiVisible();
...
try {
// Refrescar datos (ya incluye sync del contador internamente)
await this._refreshCurrentUserData();
...
```
  ...
  // Actualizar perfil después de cargar la lista
  await this._actualizarPerfilSiVisible();
...
// ==================== ELIMINAR AMIGO (CORREGIDO - SINCRONIZACIÓN COMPLETA) ====================
async eliminarAmigo(friendUid) {
const confirmed = await Utils.confirm(‘Eliminar amigo’, ‘¿Eliminar este amigo de tu lista?’);
if (!confirmed) return;
try {
// 1. Eliminar amigo en Firestore
await Storage.removeFriend(AppState.currentUserId, friendUid);
Utils.showToast(‘✅ Amigo eliminado’, ‘success’);

```
  // 2. Sincronizar explícitamente el contador en Firestore
  const nuevoContador = await this._syncFriendCountInFirestore();
  console.log(`📊 Nuevo contador después de eliminar: ${nuevoContador}`);
  
  // 3. Refrescar datos del usuario actual
  await this._refreshCurrentUserData();
  
  // 4. Refrescar la lista de amigos
  await this.cargarListaAmigos();
  
  // 5. Refrescar la lista de "Explorar usuarios"
  await this.cargarTodosUsuarios(true);
  
  // 6. Actualizar perfil - forzar actualización
  await this._actualizarPerfilSiVisible();
  
  // 7. Actualizar el badge de solicitudes
  await this.actualizarBadgeSolicitudes();
  
  // 8. Actualizar el contador en el estado global explícitamente
  if (AppState.currentUserData) {
    AppState.currentUserData.friendsCount = nuevoContador;
  }
...
const contenido = `${avatarHTML} <div style="margin-bottom:16px;"> <div style="font-size:24px; color:var(--accent-yellow);">${escape(usernameFormatted)}</div> <div style="font-size:14px; color:var(--text-secondary);">@${escape(userData.username)}</div> </div> <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; text-align:left;"> <div><strong>Bio:</strong> ${bio}</div> <div><strong>Ciudad:</strong> ${city}</div> <div><strong>Edad:</strong> ${age}</div> <div><strong>Género:</strong> ${gender}</div> <div><strong>Peso:</strong> ${weight}</div> <div><strong>Altura:</strong> ${height}</div> </div>`;
...
```
  ...
  for (const user of result.users) {
    ...
    if (solicitudPendiente) {
      if (solicitudEnviada) {
        boton = `<button class="btn-amigo" data-friend-action="cancelar" data-request-id="${solicitudId}" data-uid="${user.uid}">✖️ Cancelar</button>`;
      } else {
        boton = `<button class="btn-amigo" data-friend-action="aceptar" data-request-id="${solicitudId}" data-uid="${user.uid}" data-username="${usernameFormatted}">✓ Aceptar</button>
      ...
      boton = `<button class="btn-amigo" data-friend-action="agregar" data-uid="${user.uid}" data-username="${usernameFormatted}">➕ Agregar</button>`;
    }
