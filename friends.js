...
// Versión: 3.2 - Eventos delegados para evitar envíos masivos y mejor manejo de errores
// ====================

const Friends = {
  todosUsuariosPagination: { lastDoc: null, hasMore: true, loading: false },

  // Inicializa los listeners una sola vez al cargar la app
  initEventListeners() {
    document.addEventListener('click', (e) => {
      ...
      } else if (action === 'aceptar' && uid) {
        const requestId = target.getAttribute('data-request-id');
        if (requestId) this.aceptarSolicitud(requestId, uid, AppState.currentUserId);
      } else if (action === 'rechazar' && uid) {
        const requestId = target.getAttribute('data-request-id');
        if (requestId) this.rechazarSolicitud(requestId);
      } else if (action === 'cancelar' && uid) {
        const requestId = target.getAttribute('data-request-id');
        if (requestId) this.cancelarSolicitud(requestId);
      }
    });
  },
  ...
  async enviarSolicitud(toUid, toUsername) {
    ...
    if (!toUid || typeof toUid !== 'string' || toUid === 'undefined' || toUid === 'null') {
...
// Inicializar los listeners una vez que la página esté lista
