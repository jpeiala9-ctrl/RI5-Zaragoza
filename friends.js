// ==================== friends.js - Gestión de amigos con nivel por km y badge coloreado ====================
// Versión: 3.28 - COMPLETA: optimizada con lotes '__name__', caché, limpieza de huérfanos
// ====================

const Friends = {
  todosUsuariosPagination: { lastDoc: null, hasMore: true, loading: false },

  // ========== NUEVO: obtener niveles en lotes de 10 usando 'in' y select('level') ==========
  async getNivelesDirectos(uids) {
    ...
    for (const chunk of chunks) {
      const snapshot = await firebaseServices.db.collection('gamification')
        ...
        .select('level')
        .get();
      snapshot.forEach(doc => { niveles[doc.id] = doc.data().level || 1; });
      for (const uid of chunk) if (!niveles[uid]) niveles[uid] = 1;
    }
    return niveles;
  },
  ...
  getColorForLevel(level) {
    ...
    return '#b9f2ff'; // diamante nivel 10
  },

  // ========== CARGA PRINCIPAL DE LA LISTA DE AMIGOS (CON CACHÉ Y LIMPIEZA DE HUÉRFANOS) ==========
  async cargarListaAmigos(forceRefresh = false) {
    ...
    const cacheKey = `amigos_${AppState.currentUserId}`;
    if (!forceRefresh) {
      ...
      if (cached) {
        try {
          ...
          if (Date.now() - timestamp < 30 * 1000) {
            this.renderListaAmigos(data);
            console.log('📦 Amigos cargados desde caché');
            return;
          }
        } catch (e) {}
      }
    ...
    console.time('cargarListaAmigos');
    try {
      ...
      // ---------- LIMPIEZA DE AMIGOS HUÉRFANOS (OPTIMIZADA CON LOTES) ----------
      ...
      if (amigosValidos.length !== friendIds.length) {
        ...
        console.log(`✅ Amigos huérfanos eliminados. Ahora hay ${amigosValidos.length} amigos reales.`);
        friendIds = amigosValidos;
      }
      // --------------------------------------------------------------

      // Obtener los datos completos de los amigos (ya sabemos que todos existen)
      const amigos = [];
      const chunksData = [];
      for (let i = 0; i < friendIds.length; i += 10) {
        chunksData.push(friendIds.slice(i, i + 10));
      }
      for (const chunk of chunksData) {
        const snapshot = await firebaseServices.db.collection('users')
          .where('__name__', 'in', chunk)
          .get();
        snapshot.forEach(doc => amigos.push({ uid: doc.id, ...doc.data() }));
      }

      const uids = amigos.map(a => a.uid);
      const niveles = await this.getNivelesDirectos(uids);

      const data = { amigos, niveles };
      sessionStorage.setItem(cacheKey, JSON.stringify({ data, timestamp: Date.now() }));
      this.renderListaAmigos(data);
      console.timeEnd('cargarListaAmigos');
    } catch (error) {
  ...
  renderListaAmigos({ amigos, niveles }) {
    const container = document.getElementById('listaAmigos');
    if (!container) return;
    let html = '';
  ...
  // ========== MÉTODOS DE SOLICITUDES Y LISTENERS (ORIGINALES, PERO CON FORCE REFRESH) ==========
  initEventListeners() {
    ...
    try {
      ...
      uids.forEach(uid => {
        ...
        if (badge) {
          ...
          badge.style.textShadow = '0 0 1px black';
        }
      });
    } catch (error) { 
  ...
  // ========== CARGAR TODOS LOS USUARIOS (EXPLORAR) ==========
