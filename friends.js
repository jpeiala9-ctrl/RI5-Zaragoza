...
// Versión: 3.42 - Botón "CARGAR MÁS" corregido (permanece visible hasta el final)
// ====================
...
const Friends = {
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    try {
      ...
      // Actualizar estado del botón después de cargar
      if (loadMoreBtn) {
        if (this.todosUsuariosPagination.hasMore) {
  ...
  _mostrarContadorUsuarios() {
    ...
    const oldCounter = document.getElementById('contadorUsuariosExplorar');
    if (oldCounter) oldCounter.remove();
    
    const counter = document.createElement('div');
    counter.id = 'contadorUsuariosExplorar';
    counter.style.cssText = 'text-align: center; font-size: 12px; color: var(--text-secondary); margin: 10px 0; padding: 5px; background: var(--bg-secondary); border-radius: 8px;';
    counter.textContent = `📊 Mostrando ${totalItems} usuarios${!this.todosUsuariosPagination.hasMore ? ' (todos disponibles)' : ''}`;
