...
const Friends = {
  ...
  // Ya no hay caché de "todos los usuarios" que invalidar (v3.52): cada
  // cargarTodosUsuarios(true) trae una página nueva de Firestore. Se deja
  // como no-op para no tener que tocar las llamadas existentes tras
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    if (reset || !this._usuariosState) {
      ...
      this._usuariosState = {
        modo: 'explorar', term: '',
        users: [], lastDoc: null, agotado: false,
        friendIds: AppState.currentUserData?.friendIds || [],
        enviadas, recibidas, niveles: {}
      };
  ...
  async _cargarPaginaUsuarios(esPrimera) {
    ...
    const esperarFinAnimacion = esPrimera
    ...
    try {
      ...
      if (st.modo === 'busqueda') {
        ...
        const result = await Storage.getAllUsers(20, esPrimera ? null : st.lastDoc);
      ...
      if (esPrimera) await esperarFinAnimacion();
...
console.log('✅ friends.js v3.52 - Explorar/Buscar usuarios con paginación real (20 en 20) en vez de cargar toda la lista de golpe -- mucho más barato en lecturas de Firestore');
