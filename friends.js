// ==================== friends.js - Gestión de amigos + NIVEL VISIBLE EN AVATAR ====================
// Versión: 3.17 - Muestra el nivel del usuario en un badge sobre el avatar
// ====================

const Friends = {
  todosUsuariosPagination: { lastDoc: null, hasMore: true, loading: false },
  // Cache de niveles para evitar múltiples lecturas (opcional)
  nivelCache: new Map(),
  ...
  async getNivelesMultiples(uids) {
    ...
    if (uidsSinCache.length > 0) {
      try {
        // Obtener lotes de 10 (Firestore "in" tiene límite de 10)
        for (let i = 0; i < uidsSinCache.length; i += 10) {
          ...
          // Los que no tienen documento (nivel por defecto)
          batch.forEach(uid => {
            if (!resultados[uid]) {
  ...
  async cargarSolicitudes(tipo) {
    ...
    try {
      ...
      // Cargar niveles para estos usuarios
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    try {
      ...
      // Obtener niveles de todos los usuarios mostrados
