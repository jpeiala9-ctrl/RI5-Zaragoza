...
// Versión: 3.12 - Corregido bug: amigos aceptados siguen apareciendo en explorar usuarios
// ====================
...
const Friends = {
  ...
  // Actualiza los datos del usuario actual (friendIds, friendsCount)
  async _refreshCurrentUserData() {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        // También actualizar el contador de amigos en AppState si es necesario
        AppState.solicitudesPendientesCount = (await this.obtenerSolicitudesPendientes()).recibidas.length;
  ...
  async aceptarSolicitud(requestId, fromUid, toUid) {
    try {
      ...
      // Refrescar datos del usuario actual para actualizar friendIds
  ...
  async cargarTodosUsuarios(reset = false) {
    ...
    try {
      // Asegurar que tenemos la lista actualizada de amigos del usuario
      if (!AppState.currentUserData?.friendIds) {
        await this._refreshCurrentUserData();
      }
  ...
  async buscarUsuarios() {
    ...
    try {
      // Asegurar friendIds actualizados
      if (!AppState.currentUserData?.friendIds) {
        await this._refreshCurrentUserData();
      }
      const friendIds = AppState.currentUserData?.friendIds || [];
