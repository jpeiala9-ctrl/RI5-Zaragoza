...
// Versión: 3.13 - CORREGIDO: Ahora las invitaciones enviadas se muestran correctamente
// ====================
...
const Friends = {
  ...
  async obtenerSolicitudesPendientes() {
    ...
    try {
      ...
      // 🔍 Log de depuración - muestra cuántas solicitudes hay de cada tipo
  ...
  async enviarSolicitud(toUid, toUsername) {
    ...
    try {
      ...
      if (ok) {
        ...
        // 🔄 Refrescar la lista de solicitudes enviadas si la pestaña está activa
  ...
  async cancelarSolicitud(requestId) {
    ...
    try {
      ...
      await this.cargarTodosUsuarios(true);
      await this.actualizarBadgeSolicitudes();
      // 🔄 Refrescar la lista de solicitudes enviadas después de cancelar
      await this.cargarSolicitudesEnviadas();
      const term = document.getElementById('buscarAmigosInput')?.value.trim();
      if (term && term.length >= 2) await this.buscarUsuarios();
    } catch (error) { 
