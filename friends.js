...
const Friends = {
  ...
  async getNivelDirecto(uid) {
    try {
      // Mismo guiño que en Gamification._aplicarNivelAdmin: si el uid
      // consultado es el del propio admin que ha iniciado sesión, se
      // devuelve 10 sin ir a Firestore -- esta función se usa para pintar
      // insignias de nivel en listas de amigos y en el muro, así que sin
      // esto el admin aparecería con su nivel real ahí aunque su propio
      // dashboard ya mostrara 10.
      if (uid === AppState.currentUserId && AppState.isAdmin) return 10;
