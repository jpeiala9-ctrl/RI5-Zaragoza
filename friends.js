  ...
  async cargarSolicitudes(tipo) {
    ...
    try {
      ...
      for (const sol of solicitudes) {
        ...
        const colorNombre = colorNivel;
        html += `
          <div class="solicitud-item" data-uid="${otroUid}" onclick="if(!event.target.closest('button')) Friends.abrirModalAmigo('${otroUid}')">
            <div class="resultado-info">
              ${avatarHTML}
              <div>
                <div class="resultado-nombre" style="color:${colorNombre};">${Utils.escapeHTML(usernameFormatted)}${Utils.adminBadge(otroUser?.isAdmin)}</div>
      ...
      html += `
        <div class="resultado-busqueda" style="justify-content:space-between;" data-uid="${amigo.uid}" onclick="if(!event.target.closest('button')) Friends.abrirModalAmigo('${amigo.uid}')">
          <div class="resultado-info">
            ${avatarHTML}
            <div>
              <div class="resultado-nombre">${Utils.escapeHTML(usernameFormatted)}${Utils.adminBadge(amigo.isAdmin)}</div>
        ...
        <div style="background:var(--bg-card); border-radius:16px; border:2px solid ${levelColor}; overflow:hidden;">
          <div style="padding:20px;">
            <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 20px;">
              ${avatarHTML}
              <div>
                <div style="font-size:18px; font-weight:500; color:${levelColor};">${Utils.escapeHTML(userName)}${Utils.adminBadge(userData.isAdmin)}</div>
      ...
      html += `
        <div class="resultado-busqueda" data-uid="${user.uid}" onclick="if(!event.target.closest('button')) Friends.abrirModalAmigo('${user.uid}')">
          <div class="resultado-info">
            ${avatarHTML}
            <div>
              <div class="resultado-nombre">${Utils.escapeHTML(usernameFormatted)}${Utils.adminBadge(user.isAdmin)}</div>
