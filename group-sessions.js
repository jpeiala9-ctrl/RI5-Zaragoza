// ==================== group-sessions.js ====================
// Versión: 1.7 - Recarga forzada del tab GRUPO después de aceptar/rechazar
// ====================
...
const GroupSessions = {
  ...
  async aceptar(groupId, userId) {
    ...
    // Añadir la sesión al calendario del usuario (sharedSessions)
  ...
  // ============================================================
  // 8. RECARGAR TAB GRUPO (FORZADO)
  // ============================================================
  _recargarTabGrupo() {
    // Si el panel GRUPO está visible, recargar directamente
    ...
    if (grupoPanel && grupoPanel.classList.contains('active')) {
      ...
      // Si no está visible, no hacemos nada (se cargará al cambiar de tab)
      console.log('Tab GRUPO no activo, no se recarga automáticamente.');
    }
  },
...
console.log('✅ GroupSessions v1.7 - Recarga forzada del tab GRUPO');