...
// Versión: 3.16 - AÑADIDO: Verificación de email obligatoria y mensajes de error amigables
// ====================
...
const Auth = {
  ...
  _manejarErrorFirebase(error) {
    switch (error.code) {
      ...
      case 'auth/user-not-found':
        return '❌ No existe ninguna cuenta con este correo. ¿Quieres registrarte?';
      ...
      case 'auth/requires-recent-login':
        return '⚠️ Por seguridad, debes volver a iniciar sesión.';
      case 'auth/popup-closed-by-user':
        return '❌ Ventana cerrada. Inténtalo de nuevo.';
      ...
      case 'auth/captcha-check-failed':
        return '🤖 Verificación de seguridad fallida. Recarga la página.';
      case 'auth/web-storage-unsupported':
        return '💾 Tu navegador no soporta almacenamiento local.';
      case 'auth/missing-email':
        return '📧 Introduce un correo electrónico.';
      case 'auth/internal-error':
        return '⚠️ Error interno. Inténtalo de nuevo más tarde.';
      case 'auth/invalid-api-key':
        return '🔑 Error de configuración. Contacta con soporte.';
      case 'auth/app-deleted':
        return '❌ La aplicación ha sido eliminada.';
      case 'auth/app-not-authorized':
        return '🔒 Aplicación no autorizada.';
      case 'auth/argument-error':
        return '⚠️ Error en los datos proporcionados.';
      ...
      case 'auth/invalid-tenant-id':
        return '🏢 ID de organización inválido.';
      case 'auth/multi-factor-auth-required':
        return '🔐 Se requiere verificación adicional.';
      case 'auth/unsupported-persistence-type':
        return '💾 Tipo de persistencia no soportado.';
      default:
        ...
        console.warn('Error no mapeado:', error.code, error.message);
        return '⚠️ Ha ocurrido un error inesperado. Inténtalo de nuevo.';
    }
  },
  ...
  async registerUser() {
    ...
    try {
      ...
      Utils.showToast('📧 Te hemos enviado un correo de verificación. Revisa tu bandeja de entrada.', 'success', 6000);
      ...
      setTimeout(() => {
        Utils.showToast('📧 Recuerda verificar tu correo electrónico', 'info', 4000);
      }, 2000);
  ...
  async loginUser() {
    ...
    try {
      ...
      if (!user.emailVerified) {
        ...
        errorEl.innerText = "⚠️ Debes verificar tu correo antes de iniciar sesión.";
        errorEl.classList.add('visible');
        btn.disabled = false;
        btn.textContent = '[ ACCEDER ]';
        
        const reenviar = await Utils.confirm(
          'Correo no verificado',
          '¿Quieres que te reenviemos el email de verificación?'
        );
        if (reenviar) {
          await user.sendEmailVerification();
          Utils.showToast('📧 Email de verificación reenviado. Revisa tu bandeja de entrada.', 'success', 5000);
        }
        return;
      }
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      if (!navigator.onLine) {
        console.log('⚠️ Sin conexión, no se puede verificar sesión. Se requiere conexión para validar premium.');
        localStorage.removeItem('ri5_current_user');
        return false;
      }
...
// Inicializar listeners para limpiar errores al escribir
