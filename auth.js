...
// Versión: 3.15 - Integración de listeners en tiempo real para notificaciones
// ====================
...
const Auth = {
  ...
  async registerUser() {
    ...
    try {
      ...
      if (window.Friends) Friends.iniciarListenerSolicitudes();
      if (window.Chat) Chat.iniciarListenerChat();
      if (window.Feed) Feed.iniciarListenerFeed();
  ...
  // Nueva función: reautenticación con contraseña
  ...
  async eliminarMiCuenta() {
    // Solicitar email y contraseña para reautenticación
    ...
    try {
      // Primero, reautenticar al usuario para garantizar que es él
      const currentUser = firebaseServices.auth.currentUser;
      if (!currentUser || currentUser.email !== email) {
        throw new Error('No se ha podido verificar la sesión. Vuelve a iniciar sesión.');
      }

      // Intentar reautenticar
      await this._reauthenticateUser(password);

      // Una vez reautenticado, proceder a eliminar datos
      ...
      // Eliminar subcolecciones: historial
      ...
      // Eliminar subcolección planes y sus sesiones
      ...
      // Eliminar subcolección cálculos
      ...
      // Eliminar documentos de mensajes
      ...
      if (error.code === 'auth/wrong-password') {
        ...
        userMessage = '⚠️ Por seguridad, debes volver a iniciar sesión antes de eliminar la cuenta. Cierra sesión y vuelve a entrar.';
      } else {
  ...
  async logoutUser() {
    ...
    try {
      // DETENER TODOS LOS LISTENERS EN TIEMPO REAL
      if (window.Friends) Friends.detenerListenerSolicitudes();
      if (window.Chat) Chat.detenerListenerChat();
      if (window.Feed) Feed.detenerListenerFeed();
      if (AppState && AppState.unsubscribeMensajes) AppState.unsubscribeMensajes();
    ...
    try {
      ...
      if (!navigator.onLine) {
        ...
        if (userData) {
          ...
          // Iniciar listeners (offline, no harán nada pero se inician)
          if (window.Friends) Friends.iniciarListenerSolicitudes();
          if (window.Chat) Chat.iniciarListenerChat();
          if (window.Feed) Feed.iniciarListenerFeed();
          return true;
      ...
      // INICIAR LISTENERS EN TIEMPO REAL (después de establecer currentUserId)
      if (window.Friends) Friends.iniciarListenerSolicitudes();
      if (window.Chat) Chat.iniciarListenerChat();
      if (window.Feed) Feed.iniciarListenerFeed();
