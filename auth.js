...
// Versión: 5.14 - Corregido bucle infinito de pantalla negra de carga al recargar (hideLoading en todas las rutas de onAuthStateChanged/checkSavedSession)
// ====================
...
const Auth = {
  ...
  async verificarAhora() {
    ...
    try {
      ...
      if (user.emailVerified) {
        ...
        if (window.UI) await UI.restaurarEstado();

        if (typeof cargarDashboard === 'function') {
          try { await cargarDashboard(); } catch (e) { console.warn('Error precargando dashboard:', e); }
        }
  ...
  async loginUser() {
    ...
    try {
      ...
      // === TRANSICIÓN INMEDIATA SIN PANTALLA NEGRA ===
      document.getElementById("loginPage").style.display = "none";
      document.getElementById("verificationPage").style.display = "none";
      ...
      mainContent.style.display = "flex";
      mainContent.style.visibility = "visible";
      // Eliminar cualquier opacidad o transición residual
      ...
      if (window.UI) {
        ...
        promises.push(UI.cargarMensajesRecibidos().catch(e => console.warn('Error cargando mensajes recibidos:', e)));
      ...
      if (window.UI) promises.push(UI.restaurarEstado().catch(e => console.warn('Error restaurando estado:', e)));

      if (typeof cargarDashboard === 'function') {
        promises.push(cargarDashboard().catch(e => console.warn('Error cargando dashboard:', e)));
      }

      // No esperamos a que terminen para mostrar la app
      Promise.allSettled(promises).then(() => {
        console.log('✅ Todos los datos cargados en segundo plano');
      });
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      if (typeof cargarDashboard === 'function') {
        try { await cargarDashboard(); } catch (e) { console.warn('Error precargando dashboard en checkSavedSession:', e); }
      }
      ...
      // Red de seguridad: pase lo que pase, el overlay de carga nunca debe quedar colgado
      Utils.hideLoading();
    }
  },
...
firebaseServices.auth.onAuthStateChanged(async (user) => {
  if (user) {
      ...
      if (savedUid === uid || !savedUid) {
        ...
        try {
          ...
          if (userDoc.exists) {
            ...
            // Ocultar overlay de carga en cuanto la app es visible
            Utils.hideLoading();
        ...
        // UID guardado no coincide con el usuario autenticado: no se restaura
        Utils.hideLoading();
      }
    } else {
      Utils.hideLoading();
    ...
    if (document.getElementById("mainContent").style.display === "flex") {
      ...
      // Asegurar que se muestra el login si no hay sesión y no estamos en verificación
      document.getElementById("loginPage").style.display = "flex";
    }
  }
});
