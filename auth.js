...
const Auth = {
  ...
  async _cargarDatosYMostrarApp(uid, email, userData) {
    ...
    // Precarga (sin bloquear) la caché del modal "carga y recuperación":
    // cargarDashboard() ya deja fijado AppState.planActualId justo arriba,
    // así que a partir de aquí ya se puede calcular. Mismo motivo que la
    // precarga de gamificación: que el modal se abra relleno a la primera
    // sin pasar por "Calculando…".
    if (window.PlanGenerator && typeof PlanGenerator._refrescarCacheCargaPlan === 'function') {
      PlanGenerator._refrescarCacheCargaPlan(uid);
    }
  ...
  async _cargarDatosEnBackground(uid) {
    ...
    // Igual que en el login directo: una vez el dashboard ha fijado
    // AppState.planActualId, se precarga la caché de "carga y
    // recuperación" para esta recarga con sesión guardada también.
    if (window.PlanGenerator && typeof PlanGenerator._refrescarCacheCargaPlan === 'function') {
      PlanGenerator._refrescarCacheCargaPlan(uid);
    }
