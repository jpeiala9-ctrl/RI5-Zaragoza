...
// VERSIÓN CORREGIDA CON INTEGRACIÓN PREMIUM

const Auth = {
  intentosLogin: 0,
  ultimoIntento: 0,
  
  // ===== NUEVA FUNCIÓN: Disparar evento de actualización de usuario =====
  dispatchUserUpdated(userData) {
    // Actualizar variable global para el sistema premium
    if (typeof window !== 'undefined') {
      window.currentUser = userData;
    }
    // Disparar evento para que index.html actualice la interfaz
  ...
  async loginUser() {
    const email = document.getElementById('loginEmail').value.trim(); // Usamos email
    ...
    try {
      // Autenticar con Firebase Auth usando email y contraseña
      const userCredential = await firebaseServices.auth.signInWithEmailAndPassword(email, password);
      const user = userCredential.user;
      
      // Obtener el documento del usuario desde Firestore por su UID
      const userDoc = await firebaseServices.db.collection('users').doc(user.uid).get();
      
      if (!userDoc.exists) {
        // Si no existe, lo creamos automáticamente (por si acaso)
      ...
      // Actualizar último login y mes si es necesario
      ...
      if (Object.keys(updates).length > 0) {
        ...
        // Actualizar userData local con los cambios
        Object.assign(userData, updates);
      }
      
      // Establecer sesión en AppState
      await AppState.setCurrentUser(user.uid, email, userData);
      ...
      if (welcomeEl) {
        ...
        const estado = userData.premium ? 'PREMIUM' : 'ACCESO';
        welcomeEl.innerText = `> BIENVENIDO, ${userData.username.toUpperCase()} · ${estado} HASTA ${expiry.toLocaleDateString()}`;
      }
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      // Llamar a la función de actualización de interfaz premium si existe
