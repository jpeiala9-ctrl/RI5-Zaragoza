// ==================== auth.js - VERSIÓN COMPLETA (CON ELIMINACIÓN DE CUENTA FUNCIONAL) ====================
// Versión: 3.22 - Totalmente funcional, con reautenticación y eliminación incluso sin sesión previa
// ====================
...
const Auth = {
  ...
  async _reauthenticateUser(password) {
    ...
    // CORREGIDO: usar firebaseServices.auth.EmailAuthProvider (o firebase.auth según tu configuración)
    const credential = firebaseServices.auth.EmailAuthProvider.credential(user.email, password);
    await user.reauthenticateWithCredential(credential);
    return true;
  },

  // ==================== ELIMINAR CUENTA (funciona incluso si la sesión está cerrada) ====================
  async eliminarMiCuenta() {
    ...
    try {
      // Intentar obtener el usuario actual, si no existe, iniciar sesión con las credenciales
      let currentUser = firebaseServices.auth.currentUser;
      if (!currentUser) {
        console.log('🔐 No hay sesión activa. Iniciando sesión para eliminar la cuenta...');
        const userCred = await firebaseServices.auth.signInWithEmailAndPassword(email, password);
        currentUser = userCred.user;
      } else if (currentUser.email !== email) {
        console.log('🔄 La sesión actual no coincide. Cerrando sesión...');
        await firebaseServices.auth.signOut();
        const userCred = await firebaseServices.auth.signInWithEmailAndPassword(email, password);
        currentUser = userCred.user;
      }

      // Asegurar que la sesión es reciente (reautenticar)
      try {
        await this._reauthenticateUser(password);
      } catch (reauthError) {
        if (reauthError.code === 'auth/requires-recent-login') {
          console.log('🔄 Sesión antigua, refrescando con nuevo login...');
          await firebaseServices.auth.signOut();
          const userCred = await firebaseServices.auth.signInWithEmailAndPassword(email, password);
          currentUser = userCred.user;
          await this._reauthenticateUser(password);
      ...
      // Eliminar subcolecciones: planes y sus sesiones
      ...
      // Eliminar subcolecciones: cálculos
      ...
      if (error.code === 'auth/wrong-password') {
        ...
        userMessage = '❌ No existe ninguna cuenta con ese correo.';
