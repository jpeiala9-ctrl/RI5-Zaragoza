...
// Versión: 5.15 - CORREGIDO: espera a que los datos se carguen antes de mostrar la app
// ====================
...
const Auth = {
  ...
  async _cargarDatosYMostrarApp(uid, email, userData) {
    // 1. Guardar en AppState (sincrónico)
    await AppState.setCurrentUser(uid, email, userData);
    
    // 2. Cargar datos esenciales en paralelo (sin bloquear excesivamente)
    const promises = [];
    
    // Perfil (si no está en caché)
    if (!localStorage.getItem(`perfil_${uid}`)) {
      promises.push(Profile.cargarPerfil(true).catch(e => console.warn('Error cargando perfil:', e)));
    } else {
      // Si está en caché, cargarlo inmediatamente (ya lo hará cargarPerfil con forceRefresh=false)
      promises.push(Profile.cargarPerfil(false).catch(e => console.warn('Error cargando perfil desde caché:', e)));
    }
    
    // Gamificación
    promises.push(Gamification.getData(uid).then(data => {
      if (data) {
        sessionStorage.setItem(`gamification_${uid}`, JSON.stringify(data));
      }
    ...
    promises.push(UI.cargarMensajesRecibidos().catch(e => console.warn('Error cargando mensajes:', e)));
    promises.push(UI.cargarMensajesEnviados().catch(e => console.warn('Error cargando mensajes enviados:', e)));
    
    // Restaurar estado de pestañas/plan
    promises.push(UI.restaurarEstado().catch(e => console.warn('Error restaurando estado:', e)));
    
    // Dashboard
    promises.push(cargarDashboard().catch(e => console.warn('Error cargando dashboard:', e)));
    
    // Esperar a que todo lo esencial esté listo (máximo 2 segundos)
    ...
    console.log('✅ App mostrada con todos los datos cargados');
  },
