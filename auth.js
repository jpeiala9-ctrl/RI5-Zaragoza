...
// Registro, login, eliminación de cuenta (incluso sin sesión activa)
// Corregido: uso de firebase.auth.EmailAuthProvider (global) para reautenticación
// ====================
...
const Auth = {
  ...
  // Reautenticación (necesaria para eliminar cuenta)
  async _reauthenticateUser(password) {
    const user = firebaseServices.auth.currentUser;
    if (!user) throw new Error('No hay usuario autenticado');
    // ✅ CORRECCIÓN DEFINITIVA: usar firebase.auth (global) para EmailAuthProvider
    const credential = firebase.auth.EmailAuthProvider.credential(user.email, password);
    await user.reauthenticateWithCredential(credential);
    return true;
  },

  // ELIMINAR CUENTA (funciona incluso si el usuario ya había cerrado sesión)
  ...
  // Mostrar modal de beneficios premium
  showPremiumBenefits() {
