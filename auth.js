...
// Versión: 5.4 - Asegura que el loading se oculte siempre tras restaurar sesión
// ====================
...
const Auth = {
  ...
  showVerificationScreen(email) {
    ...
    Utils.hideLoading(); // Asegurar que el loading se oculte
  },
...
firebaseServices.auth.onAuthStateChanged(async (user) => {
  if (user) {
    ...
    // Si el email está verificado, intentamos restaurar la sesión automáticamente
    if (user.emailVerified) {
      const uid = user.uid;
      const savedUid = localStorage.getItem('ri5_current_user');
      
      // Solo restauramos si el UID coincide con el guardado o si no hay sesión activa
      if (savedUid === uid || document.getElementById("mainContent").style.display !== "flex") {
        ...
        if (userDoc.exists) {
          ...
          // Si no hay documento, redirigir a verificación
          Auth.showVerificationScreen(user.email);
        }
      }
    } else {
      // Email no verificado: mostrar pantalla de verificación
    ...
    // Usuario no autenticado: asegurar que se muestre el login
