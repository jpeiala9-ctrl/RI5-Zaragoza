// ==================== auth.js - Módulo de Autenticación FINAL ====================
  ...
  async registerUser() {
    ...
    try {
      ...
      if (welcomeEl) {
        ...
        if (userData.isAdmin) {
          welcomeEl.classList.add('admin-username');
          welcomeEl.onclick = irAlPanelAdmin;
        } else {
          welcomeEl.classList.remove('admin-username');
    ...
    try {
      ...
      if (userData.isAdmin) {
        ...
        if (!userData.expires || new Date(userData.expires) <= new Date()) {
          ...
          userData.expires = farFuture.toISOString();
        }
      }
      ...
      if (userData.isAdmin) {
        ...
        // Pequeño retraso para asegurar que el DOM y Firebase estén listos
        setTimeout(() => {
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      if (userData.isAdmin) {
        document.getElementById("adminPage").style.display = "flex";
        document.getElementById("mainContent").style.display = "none";
        // Inicializar el panel de admin con un pequeño retraso
        setTimeout(() => {
          if (window.Admin) Admin.init();
        }, 500);
      } else {
