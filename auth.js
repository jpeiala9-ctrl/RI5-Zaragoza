...
// Versión: 3.15 - Cambiada eliminación de cuenta por solicitud interna al administrador
// ====================
...
const Auth = {
  ...
  // Reautenticación con contraseña (corregido para usar firebase.auth global)
  async _reauthenticateUser(password) {
    const user = firebaseServices.auth.currentUser;
    if (!user) throw new Error('No hay usuario autenticado');
    // CORRECCIÓN: usar firebase.auth (global) en lugar de firebaseServices.auth
  ...
  // ==================== NUEVA FUNCIÓN: SOLICITUD DE ELIMINACIÓN (mensaje interno al administrador) ====================
  async eliminarMiCuenta() {
    // Solicitar email y contraseña para verificar identidad
    ...
    const confirmed = await Utils.confirm('SOLICITAR ELIMINACIÓN DE CUENTA', 
      `¿SOLICITAR la eliminación de la cuenta "${email}"?\n\nEl administrador recibirá una notificación interna y procesará tu solicitud.\n\nSe cerrará tu sesión.`);
    if (!confirmed) return;

    Utils.showLoading();

    try {
      // Verificar que el usuario actual coincide con el email
      const currentUser = firebaseServices.auth.currentUser;
      if (!currentUser || currentUser.email !== email) {
        throw new Error('El correo no coincide con tu sesión activa.');
      }

      // Reautenticar para asegurar que es el usuario legítimo
      ...
      // Enviar solicitud al administrador (colección 'solicitudesEliminacion')
      await firebaseServices.db.collection('solicitudesEliminacion').add({
        uid: uid,
        email: email,
        username: username,
        fecha: firebaseServices.Timestamp.now(),
        estado: 'pendiente',
        mensaje: `El usuario ${username} (${email}) solicita la eliminación de su cuenta.`
      });
      ...
      if (error.code === 'auth/wrong-password') {
        userMessage = '❌ Contraseña incorrecta. No se pudo verificar la identidad.';
      } else if (error.message && error.message.includes('coincide')) {
