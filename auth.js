// ==================== auth.js - LOGIN CON EMAIL (SIN FOTO) ====================
// Versión: 3.10 - Corrección de persistencia offline, creación unificada de usuario
// ====================
...
const Auth = {
  ...
  // ========== FUNCIÓN COMÚN PARA CREAR DOCUMENTO DE USUARIO ==========
  async createUserDocument(userId, email, username, extraData = {}) {
    ...
    await firebaseServices.db.collection('users').doc(userId).set(userData, { merge: true });

    // Crear documento de mensajes si no existe
  ...
  async registerUser() {
    ...
    try {
      // 1. Verificar si el username ya existe en la colección usernames
      const usernameDoc = await firebaseServices.db
      ...
      // 2. Verificar si el email ya existe en Auth
      ...
      // 7. Intentar habilitar persistencia offline después del registro
  ...
  async loginUser() {
    ...
    try {
      ...
      // ===== CORRECCIÓN: Reintentar habilitar persistencia offline =====
      await this._enablePersistence();
      // ===============================================================
  ...
  // ===== NUEVO: Método para habilitar persistencia offline de forma segura =====
  async _enablePersistence() {
    try {
      await firebaseServices.db.enablePersistence({ synchronizeTabs: true });
      console.log('✅ Persistencia offline habilitada (sincronizada)');
    } catch (err) {
      if (err.code === 'failed-precondition') {
        console.warn('⚠️ Persistencia offline limitada: múltiples pestañas abiertas. Modo online.');
      } else if (err.code === 'unimplemented') {
        console.warn('⚠️ El navegador no soporta persistencia offline.');
      } else {
        console.error('❌ Error al habilitar persistencia:', err);
      }
      // No lanzamos error, la app funciona sin persistencia
    }
  },
  // ===========================================================================
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      // Reintentar persistencia al restaurar sesión
      await this._enablePersistence();
