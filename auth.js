// ==================== auth.js - VERIFICACIÓN OBLIGATORIA (SESIÓN PERSISTENTE) ====================
// Versión: 5.16 - CORREGIDO: carga completa antes de mostrar la app / caché local para recarga rápida
// ====================
...
const Auth = {
  ...
  // ========== NUEVA FUNCIÓN: carga datos esenciales y muestra la app ==========
  async _cargarDatosYMostrarApp(uid, email, userData) {
    // 1. Guardar en AppState (sincrónico) y en caché local
    await AppState.setCurrentUser(uid, email, userData);
    localStorage.setItem('ri5_user_data', JSON.stringify(userData));
    
    // 2. Cargar datos esenciales de forma secuencial para asegurar que la UI tenga contenido
    console.log('⏳ Cargando datos esenciales...');
    
    // Perfil (si no está en caché, forzar carga)
    if (!localStorage.getItem(`perfil_${uid}`)) {
      await Profile.cargarPerfil(true).catch(e => console.warn('Error cargando perfil:', e));
    } else {
      await Profile.cargarPerfil(false).catch(e => console.warn('Error cargando perfil desde caché:', e));
    }
    
    // Gamificación
    const gamData = await Gamification.getData(uid).catch(e => console.warn('Error cargando gamificación:', e));
    if (gamData) {
      sessionStorage.setItem(`gamification_${uid}`, JSON.stringify(gamData));
    }
    
    // Último cálculo
    const calc = await Storage.getUltimoCalculo(uid).catch(e => console.warn('Error cargando último cálculo:', e));
    if (calc) {
    ...
    // Mensajes de soporte (para el badge)
    await UI.cargarMensajesRecibidos().catch(e => console.warn('Error cargando mensajes:', e));
    await UI.cargarMensajesEnviados().catch(e => console.warn('Error cargando mensajes enviados:', e));
    
    // Restaurar estado de pestañas/plan
    await UI.restaurarEstado().catch(e => console.warn('Error restaurando estado:', e));
    
    // Dashboard (fundamental para que la pestaña Inicio tenga contenido)
    await cargarDashboard().catch(e => console.warn('Error cargando dashboard:', e));
    
    console.log('✅ Datos esenciales cargados. Mostrando app...');
    
    // Ahora sí, mostrar la app
    document.getElementById("loginPage").style.display = "none";
    document.getElementById("verificationPage").style.display = "none";
    ...
    // Ocultar loading overlay por si acaso
    Utils.hideLoading();
  },
  ...
  async loginUser() {
    ...
    try {
      ...
      // Cargar datos y mostrar app (la función espera a que todo esté listo)
      ...
      btn.textContent = '[ ACCEDER ]';
      btn.classList.remove('pulse');
      // Asegurar que no se muestre la app si falla
      document.getElementById("loginPage").style.display = "flex";
      document.getElementById("mainContent").style.display = "none";
      // Ocultar loading overlay por si acaso
      Utils.hideLoading();
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      // Intentar restaurar desde caché local primero (rápido)
      const cachedUserData = localStorage.getItem('ri5_user_data');
      if (cachedUserData) {
        try {
          const userData = JSON.parse(cachedUserData);
          // Si tenemos caché, mostrar la app inmediatamente con esos datos
          ...
          console.log('✅ Sesión restaurada desde caché local');
          Utils.hideLoading();
          return true;
        } catch (e) {
      ...
      // Si no hay caché o está corrupto, cargar desde Firestore
      if (!navigator.onLine) {
      ...
      // Mostrar app inmediatamente con los datos cargados (sin esperar a que todo termine)
      await AppState.setCurrentUser(uid, userData.email, userData);
  ...
  // ========== CARGA EN SEGUNDO PLANO (para recarga) ==========
...
firebaseServices.auth.onAuthStateChanged(async (user) => {
  if (user) {
    ...
    if (user.emailVerified) {
      ...
      // Si ya hay un usuario en AppState y coincide, no hacer nada (evitar bucle)
      if (AppState.currentUserId === uid) {
        console.log('ℹ️ Usuario ya restaurado, omitiendo onAuthStateChanged');
        return;
      }
      
      // Si el uid guardado coincide o no hay uid guardado, restaurar sesión
      if (savedUid === uid || !savedUid) {
        ...
        try {
          ...
          if (userDoc.exists) {
            ...
            Auth._cargarDatosEnBackground(uid);
