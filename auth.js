...
// Versión: 3.2 - REGISTRO MEJORADO con transacción y colección usernames
...
const Auth = {
  ...
  async registerUser() {
    ...
    try {
      // --- Transacción para reservar el nombre de usuario de forma atómica ---
      const usernameDocRef = firebaseServices.db.collection('usernames').doc(username);
      
      const result = await firebaseServices.db.runTransaction(async (transaction) => {
        // Leer el documento de username
        ...
        // Guardar documento del usuario (esto se hace fuera de la transacción)
        // pero lo devolvemos para ejecutarlo después
        return { user, userData };
      });
      
      // Una vez confirmada la transacción (username reservado), creamos el documento en 'users'
      const { user, userData } = result;
      
      await firebaseServices.db.collection('users').doc(user.uid).set(userData);
      
      // Crear el documento de username (ya existe por la transacción, pero aseguramos)
      await usernameDocRef.set({ uid: user.uid, email, createdAt: firebaseServices.Timestamp.now() });
      ...
      if (error.message === 'USERNAME_EXISTS') {
        errorEl.innerText = "> EL NOMBRE DE USUARIO YA ESTÁ EN USO_";
        errorEl.classList.add('visible');
