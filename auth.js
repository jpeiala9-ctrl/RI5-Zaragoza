// ==================== auth.js - LOGIN CON EMAIL (VERIFICACIÓN OBLIGATORIA) ====================
// Versión: 4.3 - Corrección definitiva: perfil actualiza estado verificado
// ====================
  ...
  async verificarAhora() {
    ...
    try {
      ...
      if (user.emailVerified) {
        ...
        console.log('✅ Firestore actualizado: emailVerified = true');
        
        // Forzar recarga del usuario desde Firestore (ignorar caché)
        const userDoc = await firebaseServices.db.collection('users').doc(user.uid).get({ source: 'server' });
        const userData = userDoc.data();
        console.log('📄 Datos frescos de Firestore:', userData.emailVerified);
        
        // Actualizar el estado global
        await AppState.setCurrentUser(user.uid, user.email, userData);
        
        // Ocultar pantalla de verificación y mostrar app
        document.getElementById("verificationPage").style.display = "none";
        ...
        // Forzar actualización del perfil si el contenedor existe (aunque no esté activo)
...
// Detecta automáticamente cuando el usuario verifica su correo desde el enlace
firebaseServices.auth.onAuthStateChanged(async (user) => {
  if (user) {
    ...
    // Si el usuario está autenticado y su email está verificado, pero aún estamos en la pantalla de verificación...
    if (user.emailVerified && document.getElementById("verificationPage").style.display === "flex") {
      console.log('✅ Email verificado detectado automáticamente, entrando...');
      try {
        // Actualizar Firestore
        await firebaseServices.db.collection('users').doc(user.uid).update({
          emailVerified: true
        ...
        if (userDoc.exists) {
          ...
          // Actualizar perfil si el contenedor existe (aunque no esté activo)
          if (document.getElementById('perfilContainer')) {
    ...
    // Usuario no autenticado: aseguramos que se muestre el login
