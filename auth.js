...
// Versión: 3.17 - Corregida eliminación de cuenta con manejo de sesión y reautenticación
// ====================
...
const Auth = {
  ...
  // ==================== FUNCIÓN CORREGIDA PARA ELIMINAR CUENTA ====================
  async eliminarMiCuenta() {
    ...
    try {
      ...
      if (!currentUser) {
        Utils.hideLoading();
        const relogin = await Utils.confirm('SESIÓN EXPIRADA', 'Tu sesión ha expirado. ¿Quieres cerrar sesión y volver a iniciar para eliminar la cuenta?');
        if (relogin) {
          await this.logoutUser();
          Utils.showToast('Inicia sesión de nuevo y luego repite la eliminación', 'info');
        }
        return;
      }
      ...
      try {
        ...
        if (reauthError.code === 'auth/requires-recent-login') {
          Utils.hideLoading();
          const relogin = await Utils.confirm('REAUTENTICACIÓN NECESARIA', 'Por seguridad, debes volver a iniciar sesión. ¿Deseas cerrar sesión ahora?');
          if (relogin) {
            await this.logoutUser();
            Utils.showToast('Inicia sesión de nuevo y vuelve a intentar eliminar la cuenta', 'info');
          }
          return;
        } else {
          throw reauthError;
      ...
      if (error.code === 'auth/wrong-password') {
        ...
        userMessage = '⚠️ Por seguridad, debes volver a iniciar sesión antes de eliminar la cuenta.';
