// ==================== auth.js - VERSIÓN CON SOLICITUD DE ELIMINACIÓN AL ADMIN ====================
// Registro, login, y solicitud de eliminación de cuenta (el administrador borra manualmente)
// ====================
...
const Auth = {
  ...
  // Esta función ya no se usa para eliminación automática, pero la dejamos por si acaso
  ...
  // ==================== NUEVA FUNCIÓN: SOLICITUD DE ELIMINACIÓN AL ADMIN ====================
  async eliminarMiCuenta() {
    ...
    const confirmed = await Utils.confirm(
      ...
      `¿SOLICITAR la eliminación de la cuenta "${email}"?\n\nUn administrador procesará tu solicitud en un plazo de 24-48 horas.\n\nNo podrás acceder a la app hasta que se complete la eliminación.`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      // Crear solicitud en colección "deletionRequests"
      const requestData = {
        uid: uid,
        email: email,
        username: username,
        requestedAt: firebaseServices.Timestamp.now(),
        status: 'pending',
        userMessage: 'El usuario solicita la eliminación de su cuenta.'
      };
      await firebaseServices.db.collection('deletionRequests').add(requestData);
      console.log('✅ Solicitud de eliminación registrada');

      // Enviar mensaje al administrador (colección mensajes, documento admin_uid)
      const adminMessageRef = firebaseServices.db.collection('mensajes').doc('admin_' + uid);
      const adminMessage = {
        fecha: new Date().toLocaleString(),
        texto: `📢 SOLICITUD DE ELIMINACIÓN DE CUENTA\nUsuario: ${username}\nEmail: ${email}\nUID: ${uid}\n\nPor favor, procesa la solicitud en Firebase Console → Colección "deletionRequests".`,
        leido: false,
        esUsuario: true,
        timestamp: firebaseServices.Timestamp.now()
      };
      ...
      // Cerrar sesión para que el usuario no pueda seguir usando la app
      ...
      Utils.showToast('✅ Solicitud enviada al administrador. Recibirás un correo cuando tu cuenta sea eliminada.', 'success');
    } catch (error) {
