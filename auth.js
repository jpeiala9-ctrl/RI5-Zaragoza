...
// VERSIÓN CORREGIDA - BOTÓN ACCEDER FUNCIONA Y PESTAÑAS FUNCIONAN
...
const Auth = {
  ...
  switchAuthTab(tab) { 
    // CORREGIDO: Selección correcta de pestañas
  ...
  // ===== FUNCIÓN PARA DISPARAR EVENTO DE ACTUALIZACIÓN DE USUARIO =====
  dispatchUserUpdated(userData) {
    ...
    console.log('📢 Evento userUpdated disparado para:', userData?.username);
  },
  ...
  async registerUser() {
    ...
    try {
      ...
      // Crear bandeja de mensajes vacía
      await firebase.firestore().collection('mensajes').doc(user.uid).set({ mensajes: [] });
      
      await AppState.setCurrentUser(user.uid, email, userData);
      
      // Disparar evento userUpdated
      this.dispatchUserUpdated(userData);
  ...
  async loginUser() {
    ...
    try {
      // CORREGIDO: Usar firebase.auth() en lugar de firebaseServices
      ...
      let datosActualizados = { ...userData };
      
      if (userData.mesActual !== mesActual) {
        updates.calculosMes = 0;
        updates.mesActual = mesActual;
        datosActualizados.calculosMes = 0;
        datosActualizados.mesActual = mesActual;
      }
      
      // Verificar si premium ha expirado
      if (userData.premium && userData.expires) {
        const expiryDate = new Date(userData.expires);
        if (expiryDate < new Date()) {
          updates.premium = false;
          datosActualizados.premium = false;
          console.log('⚠️ Premium expirado durante login, actualizando...');
        }
      }
      ...
      await AppState.setCurrentUser(user.uid, email, datosActualizados);
      
      // Disparar evento userUpdated
      this.dispatchUserUpdated(datosActualizados);
      ...
      Utils.showToast(`✅ Bienvenido, ${datosActualizados.username}`, 'success');
      ...
      const welcomeEl = document.getElementById("userWelcome");
      if (welcomeEl) {
        const expiry = new Date(datosActualizados.expires);
        const estado = datosActualizados.premium ? 'PREMIUM' : 'ACCESO';
        welcomeEl.innerText = `> BIENVENIDO, ${datosActualizados.username.toUpperCase()} · ${estado} HASTA ${expiry.toLocaleDateString()}`;
      }
      
      const nameField = document.getElementById('name');
      if(nameField) nameField.value = datosActualizados.username;
  ...
  async eliminarMiCuenta() {
    ...
    try {
      ...
      await firebase.firestore().collection('mensajes').doc(user.uid).delete();
      await firebase.firestore().collection('mensajes').doc('admin_' + user.uid).delete();
      
      await user.delete();
      
      await AppState.setCurrentUser(null, null);
      
      this.dispatchUserUpdated(null);
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      if (userData.premium && expiry < now) {
        console.log('⚠️ Premium expirado durante verificación, actualizando...');
      ...
      localStorage.setItem('ri5_user_data', JSON.stringify(datosActualizados));
      ...
      await AppState.setCurrentUser(uid, userData.email, datosActualizados);
      ...
      if (window.AppState?.actualizarInterfazPremium) {
