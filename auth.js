...
const Auth = {
  ...
  async verificarAhora() {
    ...
    try {
      ...
      if (user.emailVerified) {
        ...
        if (esNuevo) this.mostrarBienvenida(userData);
  ...
  mostrarBienvenida(userData) {
    document.getElementById('bienvenidaModal')?.remove();
    document.getElementById('bienvenidaOverlay')?.remove();

    let fechaExpira = '';
    try {
      if (userData.expires) {
        fechaExpira = new Date(userData.expires).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' });
      }
    } catch (e) {}

    const overlay = document.createElement('div');
    overlay.id = 'bienvenidaOverlay';
    overlay.style.cssText = `
      position:fixed; inset:0; background:rgba(0,0,0,0.8); backdrop-filter:blur(4px);
      z-index:80000; display:flex; align-items:center; justify-content:center; padding:20px;
      box-sizing:border-box;
    `;

    const modal = document.createElement('div');
    modal.style.cssText = `
      background:var(--bg-secondary); border:1px solid var(--gold); border-radius:20px;
      padding:28px 24px; max-width:380px; width:100%; box-sizing:border-box;
      text-align:center; font-family:'Courier New', monospace;
    `;
    modal.innerHTML = `
      <div style="font-size:44px; margin-bottom:12px;">🎉</div>
      <div style="font-size:19px; font-weight:bold; color:var(--gold); letter-spacing:1px; margin-bottom:14px;">
        ¡BIENVENIDO/A A RI5, ${Utils.escapeHTML(userData.username || '')}!
      </div>
      <div style="font-size:14px; color:var(--text-primary); line-height:1.6; margin-bottom:18px;">
        Como regalo de bienvenida tienes <strong style="color:var(--gold);">1 mes de Premium gratis</strong>,
        con acceso a todo: cálculos ilimitados, planes de entrenamiento completos y todas las funciones de la app.
      </div>
      ${fechaExpira ? `<div style="font-size:12px; color:var(--text-secondary); margin-bottom:22px;">Válido hasta el ${fechaExpira}</div>` : '<div style="margin-bottom:22px;"></div>'}
      <button id="cerrarBienvenidaBtn" style="background:var(--gold); border:none; color:#0a0a0a; font-weight:bold; padding:12px 32px; border-radius:12px; cursor:pointer; font-size:14px; letter-spacing:1px;">EMPEZAR</button>
    `;
    ...
    document.getElementById('cerrarBienvenidaBtn').addEventListener('click', cerrar);
