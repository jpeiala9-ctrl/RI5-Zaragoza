...
// Versión: 3.13 - Mensajes de error claros y limpieza de campos
// ====================
...
try {
  ...
  // Reservar username con transacción atómica para evitar race conditions
  const usernameRef = firebaseServices.db.collection('usernames').doc(username);
  try {
    await firebaseServices.db.runTransaction(async (tx) => {
      const usernameDoc = await tx.get(usernameRef);
      if (usernameDoc.exists) throw new Error('USERNAME_TAKEN');
      tx.set(usernameRef, { uid: user.uid, createdAt: firebaseServices.Timestamp.now() });
    });
  } catch (txError) {
...
try {
  ...
  // Limpiar este UID de los friendIds de todos sus amigos
  if (friendIds.length > 0) {
    const friendsBatch = firebaseServices.db.batch();
    friendIds.forEach(friendId => {
      const friendRef = firebaseServices.db.collection('users').doc(friendId);
      friendsBatch.update(friendRef, {
        friendIds: firebaseServices.FieldValue.arrayRemove(user.uid),
        friendsCount: firebaseServices.FieldValue.increment(-1)
      });
    });
    await friendsBatch.commit().catch(e => console.warn('Error limpiando friendIds de amigos:', e));
  }
