...
// Versión: 3.8 - Añadido campo username_lowercase para búsquedas case‑insensitive
...
const Auth = {
  ...
  async registerUser() {
    ...
    try {
      ...
      const userData = { 
        username,
        username_lowercase: username.toLowerCase(), // <-- NUEVO
        email,
      ...
      // Si el error ocurrió después de crear el usuario en Auth, intentar limpiar
  ...
  async loginUser() {
    ...
    try {
      ...
      if (!userDoc.exists) {
        ...
        await firebaseServices.db.collection('users').doc(user.uid).set(userData);
        // También reservar username en usernames
        await firebaseServices.db.collection('usernames').doc(username).set({
          uid: user.uid,
          createdAt: firebaseServices.Timestamp.now()
        });
  ...
  async eliminarMiCuenta() {
    ...
    try {
      ...
      // Eliminar el username de la colección usernames
      if (username) {
