...
// Versión: 5.7 - Restauración del plan tras login
// ====================
...
const Auth = {
  ...
  async loginUser() {
    ...
    try {
      ...
      document.getElementById("loginPage").style.display = "none";
      document.getElementById("verificationPage").style.display = "none";
      // mainContent se mantiene oculto hasta tener todo cargado, para evitar
      // ver el dashboard "saltando" mientras llegan los datos uno a uno
      ...
      // Restaurar el plan guardado para este usuario
      if (window.UI) UI.restaurarEstado();

      // Precargar el dashboard de inicio completo ANTES de mostrar la app,
      // así el usuario entra directamente con todo listo (sin ver cómo carga)
      if (typeof cargarDashboard === 'function') {
        try { await cargarDashboard(); } catch (e) { console.warn('Error precargando dashboard en login:', e); }
      }
  ...
  async logoutUser() {
    ...
    try {
      ...
      // No borramos el localStorage del plan específico del usuario para que pueda restaurarse al volver
      // Solo borramos las claves generales de sesión
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      document.getElementById("loginPage").style.display = "none";
      document.getElementById("verificationPage").style.display = "none";
      // Mismo patrón que en login y onAuthStateChanged: ocultar hasta tener
      // el dashboard listo, así al abrir la app no se ve cargar nada por partes
      document.getElementById("mainContent").style.visibility = "hidden";
      document.getElementById("mainContent").style.display = "flex";
    ...
    if (user.emailVerified) {
      ...
      if (savedUid === uid || document.getElementById("mainContent").style.display !== "flex") {
        ...
        if (userDoc.exists) {
          ...
          document.getElementById("loginPage").style.display = "none";
          document.getElementById("verificationPage").style.display = "none";
          // Igual que en el login manual: ocultar mainContent hasta tener
          // el dashboard listo, para que no se vea cargar nada por partes
          document.getElementById("mainContent").style.visibility = "hidden";
          document.getElementById("mainContent").style.display = "flex";
          ...
          if (typeof cargarDashboard === 'function') {
            try { await cargarDashboard(); } catch (e) { console.warn('Error precargando dashboard al restaurar sesión:', e); }
          }
