// ==================== auth.js - VERIFICACIÓN OBLIGATORIA (DOCUMENTO CREADO TRAS VERIFICAR) ====================
// Versión: 5.0 - Registro no crea documento hasta verificar email
// ====================
...
const Auth = {
  ...
  _manejarErrorFirebase(error) {
    switch (error.code) {
      ...
      default: return `⚠️ Error inesperado: ${error.message}`;
    }
  },
  ...
  // Crear documento de usuario (SOLO después de verificar email)
  ...
  async verificarAhora() {
    ...
    try {
      ...
      if (user.emailVerified) {
        ...
        if (!userDoc.exists) {
          // Es un usuario nuevo que acaba de verificar: crear documento
          const username = localStorage.getItem('temp_username') || user.email.split('@')[0];
          
          // Verificar si el username ya está ocupado
          ...
          // Actualizar emailVerified en Firestore por si acaso
  ...
  async registerUser() {
    ...
    try {
      ...
      // Guardar temporalmente el username elegido
      localStorage.setItem('temp_username', username);
      
      // Mostrar pantalla de verificación (NO se crea documento todavía)
  ...
  async loginUser() {
    ...
    try {
      ...
      // BLOQUEO ESTRICTO: si no está verificado, mostrar pantalla y detener.
      if (!user.emailVerified) {
      ...
      if (!userDoc.exists) {
        // Caso raro: email verificado pero sin documento. Forzar verificación de nuevo.
  ...
  async checkSavedSession() {
    ...
    try {
      ...
      if (!userDoc.exists) {
        ...
        // Forzar cierre de sesión si no hay documento
        await firebaseServices.auth.signOut();
        localStorage.removeItem('ri5_current_user');
        document.getElementById("verificationPage").style.display = "none";
        document.getElementById("loginPage").style.display = "flex";
      ...
      if (verificationPage && verificationPage.style.display === "flex") {
        ...
        try {
          ...
          if (!userDoc.exists) {
            ...
            // Actualizar emailVerified por si acaso
            await firebaseServices.db.collection('users').doc(user.uid).update({ emailVerified: true });
            userDoc = await firebaseServices.db.collection('users').doc(user.uid).get();
          }
