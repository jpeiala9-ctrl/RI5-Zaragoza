...
// Versión: 3.15 - CORREGIDO: eliminado uso de localStorage como fuente de verdad para premium
// ====================
...
const Auth = {
  ...
  async createUserDocument(userId, email, username, extraData = {}) {
    ...
    // ✅ CORREGIDO: Crear documento de mensajes con try-catch y sin verificación previa
    try {
      ...
      console.log('✅ Documento de mensajes creado para usuario:', userId);
    } catch (error) {
      console.error('❌ Error creando documento de mensajes:', error);
  ...
  async loginUser() {
    ...
    try {
      ...
      if (!userDoc.exists) {
        ...
        // ✅ CORRECCIÓN: Verificar expiración de premium desde Firestore
  ...
  // ==================== SOLICITUD DE ELIMINACIÓN (solo envía mensaje al admin) ====================
  ...
  async logoutUser() {
    ...
    try {
      ...
      // ✅ Eliminar también cualquier caché de datos de usuario
  ...
  // ✅ CORRECCIÓN: checkSavedSession ya no usa localStorage como fuente de verdad para premium
  async checkSavedSession() {
    ...
    try {
      ...
      // Si no hay conexión, no podemos verificar con Firestore
      if (!navigator.onLine) {
        console.log('⚠️ Sin conexión, no se puede verificar sesión. Se requiere conexión para validar premium.');
        // No restaurar sesión sin conexión - mejor pedir login cuando haya conexión
        localStorage.removeItem('ri5_current_user');
        return false;
      }
      ...
      // ✅ FUENTE DE VERDAD: siempre desde Firestore
      const userDoc = await firebaseServices.db.collection('users').doc(uid).get();
      if (!userDoc.exists) {
      ...
      // ✅ Verificar premium desde Firestore, no desde localStorage
      const now = new Date();
      ...
      // Solo guardamos información NO crítica en localStorage (solo para UI, no para lógica de negocio)
      // No guardamos datos de premium ni expiración que puedan ser manipulados
