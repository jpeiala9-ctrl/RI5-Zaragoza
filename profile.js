...
const Profile = {
  _gpsEntries: {},
  // Mapas de mini-rutas propios del Perfil, con su propio ciclo de
  // vida — antes se compartía el array de Wall.maps, y cuando el
  // usuario visitaba el Muro, la limpieza de Wall destruía POR DETRÁS
  // los mapas del Perfil, dejando esos contenedores en un estado a
  // medias que nunca se recuperaba (el div quedaba marcado como "ya
  // tuvo mapa" y no se volvía a crear).
  _maps: [],

  _destruirMapasPerfil() {
    if (this._maps.length) {
      this._maps.forEach(map => { if (map && map.remove) { try { map.remove(); } catch (e) {} } });
      this._maps = [];
    }
  },
  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    try {
              ...
              miniMapHTML = `
                <div style="margin-top:10px; border-radius:10px; overflow:hidden; border:1px solid var(--border-color); position:relative; height:130px;">
                  <div id="${mapId}" class="gps-minimap-leaflet-profile" data-entry-id="${entryId}"
                    style="height:100%; width:100%; background:var(--stat-bg); display:flex; align-items:center; justify-content:center;">
                    <span style="font-size:11px; color:var(--text-secondary); letter-spacing:1px;">🗺️ Cargando mapa…</span>
                  </div>
      ...
      setTimeout(() => {
        ...
        // Antes de crear los mapas de este render, se destruyen los del
        // render anterior (propios del Perfil, no los del Muro) — el
        // HTML del historial se reconstruye entero cada vez, así que
        // cualquier instancia de Leaflet de la vuelta anterior queda
        // "huérfana" (atada a un <div> que ya no existe) y hay que
        // limpiarla explícitamente o se acumulan sin destruir nunca.
        this._destruirMapasPerfil();

        const inicializarMiniMapa = (mapId, entry, reintentos = 2) => {
          const mapContainer = document.getElementById(mapId);
          if (!mapContainer || mapContainer._leaflet_id || !window.Wall || typeof Wall._crearMiniMapa !== 'function') return;
          Wall._crearMiniMapa(mapId, entry.trackPoints).then(map => {
            if (map) {
              this._maps.push(map);
              return;
            }
            // Fallo real (Leaflet no cargó, contenedor desapareció, etc.):
            // nunca se deja el hueco en blanco sin explicación. Se avisa
            // y se ofrece un reintento con un toque, en vez de un
            // "a medias" silencioso que el usuario no puede distinguir
            // de que siga cargando.
            const c = document.getElementById(mapId);
            if (!c) return;
            if (reintentos > 0) {
              c.innerHTML = '<span style="font-size:11px; color:var(--text-secondary); letter-spacing:1px;">🗺️ Cargando mapa…</span>';
              setTimeout(() => inicializarMiniMapa(mapId, entry, reintentos - 1), 1200);
            } else {
              c.innerHTML = '<span style="font-size:11px; color:var(--text-secondary); letter-spacing:1px; text-align:center; padding:0 12px;">⚠️ No se pudo cargar el mapa<br><u style="cursor:pointer;">Toca para reintentar</u></span>';
              c.style.cursor = 'pointer';
              c.onclick = (e) => { e.stopPropagation(); c.onclick = null; inicializarMiniMapa(mapId, entry, 1); };
            }
          }).catch(() => {
            const c = document.getElementById(mapId);
            if (c) c.innerHTML = '<span style="font-size:11px; color:var(--text-secondary);">⚠️ No se pudo cargar el mapa</span>';
          });
        };
        ...
        Object.keys(this._gpsEntries || {}).forEach(entryId => {
          ...
          inicializarMiniMapa(mapId, entry);
        });
