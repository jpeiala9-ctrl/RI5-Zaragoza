// ==================== profile.js ====================
// Versión: 10.13 - FIX "el mapa GPS no aparecía en Mis últimos
//                entrenamientos hasta recargar la página": una sesión con
//                GPS se guarda en 2 pasos (primero sin datos de GPS, y
//                unos instantes después hasGPS/trackPoints/mapSnapshot,
//                ver gps-tracker.js). Si la tarjeta ya se había pintado
//                sin esos datos, _reconciliarMisEntrenamientos solo
//                intentaba parchear mapSnapshot sobre un contenedor de
//                mini-mapa que ni siquiera existía todavía. Ahora, si se
//                detecta que hasGPS ha pasado de no estar a estar (y la
//                tarjeta no tenía ya su mini-mapa), se reconstruye esa
//                tarjeta entera con los datos completos.
  ...
  _reconciliarMisEntrenamientos(snapshot) {
    ...
    ordenNuevo.forEach(entryId => {
      ...
      if (!el) {
        ...
        // 🔥 FIX "el mapa GPS no aparece hasta recargar la página": si esta
        // tarjeta se pintó ANTES de que llegara hasGPS/trackPoints (la
        // sesión se guarda en 2 pasos, ver gps-tracker.js) no tiene ni
        // rastro del contenedor del mini-mapa -- solo parchear la imagen
        // (más abajo) no sirve de nada porque ese contenedor no existe
        // todavía. En ese caso se reconstruye la tarjeta entera con los
        // datos ya completos, en vez de intentar un parche puntual.
        const yaTeniaGPS = el.querySelector('.gps-minimap-static') !== null;
        const ahoraTieneGPS = !!(entriesNuevas[entryId]?.hasGPS && entriesNuevas[entryId]?.trackPoints);
        if (ahoraTieneGPS && !yaTeniaGPS) {
          const div = document.createElement('div');
          div.innerHTML = this._renderEntrenamientoItemHTML(entryId, entriesNuevas[entryId]).trim();
...
console.log('✅ profile.js v10.13 - FIX mapa GPS que no aparecía en Mis últimos entrenamientos hasta recargar la página');