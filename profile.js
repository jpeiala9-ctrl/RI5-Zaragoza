...
// Versión: 3.4 - CORREGIDO: contador de amigos sincronizado con friendIds
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    try {
      ...
      // ✅ CORRECCIÓN: Sincronizar friendsCount con friendIds.length
      const friendIds = userData.friendIds || [];
      const currentCount = userData.friendsCount || 0;
      
      if (friendIds.length !== currentCount) {
        console.log(`📊 Corrigiendo friendsCount: ${currentCount} → ${friendIds.length}`);
        ...
        if (AppState.currentUserData) {
          AppState.currentUserData.friendsCount = friendIds.length;
        }
      }
        ...
        <div class="perfil-header">
          ...
          <div class="perfil-info">
            ...
            <div class="perfil-stats">
              <div class="perfil-stat"><span id="perfilFriendsCount">${userData.friendsCount || 0}</span><label>Amigos</label></div>
  ...
  async actualizarContadorAmigos() {
    const counterSpan = document.getElementById('perfilFriendsCount');
    if (!counterSpan || !AppState.currentUserId) return;
    ...
    try {
      ...
      if (AppState.currentUserData?.friendsCount !== newCount) {
        await firebaseServices.db.collection('users').doc(AppState.currentUserId).update({
          friendsCount: newCount
        }).catch(e => console.warn('Error actualizando friendsCount:', e));
        
        if (AppState.currentUserData) {
          AppState.currentUserData.friendsCount = newCount;
        }
      }
    } catch (error) {
