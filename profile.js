// ==================== profile.js - PERFIL CON MINI MURO PERSONAL + LIKES ====================
// Versión: 5.1 - Añadidos likes en "MIS ÚLTIMOS ENTRENAMIENTOS"
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    try {
      ...
      // Obtener lista de amigos y limpiar huérfanos
      ...
      // ==================== GAMIFICATION (colores y niveles) ====================
      try {
      ...
      // ==================== MIS ÚLTIMOS ENTRENAMIENTOS CON LIKES ====================
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          let entrenamientosHTML = '<div class="mis-entrenamentos-section" style="margin-top:24px; background:var(--bg-secondary); border-radius:16px; padding:16px;">';
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            try {
              ...
              if (feedDoc.exists) {
                ...
                userLiked = likes.includes(AppState.currentUserId);
              }
            } catch (err) {
            ...
            entrenamientosHTML += `
              <div class="mis-entreno-item" data-entry-id="${entryId}" style="border-bottom:1px solid var(--border-color); padding:12px 0;">
                ...
                <div class="mis-entreno-likes" style="display:flex; align-items:center; justify-content:flex-end; gap:8px; margin-top:8px;">
                  <button class="like-btn-profile" data-entry-id="${entryId}" style="background:none; border:none; cursor:pointer; display:inline-flex; align-items:center; gap:4px; font-size:14px; color:${userLiked ? '#e74c3c' : 'var(--text-secondary)'}; transition:transform 0.1s;">
          ...
          html += `
            <div style="margin-top:24px; background:var(--bg-secondary); border-radius:16px; padding:16px; text-align:center;">
      ...
      // ==================== EVENTOS DE LIKES EN EL PERFIL ====================
      // Delegación de eventos para los botones de like (se añaden después de pintar el HTML)
      setTimeout(() => {
        document.querySelectorAll('.like-btn-profile').forEach(btn => {
          // Eliminar listeners antiguos para evitar duplicados
          btn.removeEventListener('click', Profile._handleProfileLike);
          btn.addEventListener('click', Profile._handleProfileLike);
        });
      }, 0);
  ...
  // Manejador de like específico para el perfil
  async _handleProfileLike(e) {
    ...
    // Llamar a la función global de Wall (que ya maneja la lógica de like en Firestore)
    if (typeof Wall !== 'undefined' && Wall.toggleLike) {
      await Wall.toggleLike(entryId);
    } else {
      // Fallback: implementación manual si Wall no está disponible
      try {
    ...
    // Actualizar solo la UI del like en este elemento (sin recargar todo el perfil)
    try {
      const feedDoc = await firebaseServices.db.collection('globalFeed').doc(entryId).get();
      if (feedDoc.exists) {
        const feedData = feedDoc.data();
        ...
        const userLiked = likes.includes(AppState.currentUserId);
        const countSpan = btn.querySelector('.like-count-profile');
        if (countSpan) countSpan.textContent = likeCount;
        btn.style.color = userLiked ? '#e74c3c' : 'var(--text-secondary)';
      }
    } catch (err) {
      console.warn('Error actualizando UI de like:', err);
    }
  },
