// ==================== profile.js - Perfil con tarjetas estilo muro y mapa OSM ====================
// Versión: 11.3 - Mapas OpenStreetMap visibles, misma disposición que el muro
// ====================
...
const Profile = {
      ...
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            // Mapa con OpenStreetMap si existe ruta GPS
  ...
  // ========== INICIALIZAR MAPAS CON OPENSTREETMAP (SIEMPRE VISIBLE) ==========
  async _initProfileMaps() {
    // Cargar Leaflet si es necesario
      ...
      try {
        ...
        if (doc.exists && doc.data().gpsRoute) {
          ...
          if (gpsRoute.points && gpsRoute.points.length >= 2) {
            ...
            // Capa de OpenStreetMap (siempre funciona)
