// ==================== profile.js - NUEVO DISEÑO: NIVEL+ZAPATILLA, TRES MÉTRICAS, INSIGNIAS SÓLO ICONO ====================
// Versión: 8.0 - Layout: nivel y zapatilla en fila; km, sesiones, XP; insignias con tooltip.
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    try {
      ...
      // ========== NUEVA GAMIFICACIÓN: NIVEL+ZAPATILLA, TRES MÉTRICAS, INSIGNIAS SÓLO ICONO ==========
      try {
        ...
        if (gamificationData) {
          ...
          if (levelColor.startsWith('#')) {
            ...
            bgColor = levelColor.replace('rgb', 'rgba').replace(')', ', 0.12)');
          } else {
            bgColor = 'var(--bg-secondary)';
          }
          
          // Insignias: solo icono, con tooltip al hacer clic o mantener pulsado
          const badgesIconsOnly = (gamificationData.badges || []).map(badgeId => {
            const badge = Gamification.BADGES[badgeId];
            if (!badge) return '';
            return `<span class="badge-icon" data-badge-name="${badge.name}" data-badge-desc="${badge.description}" style="display:inline-block; font-size:24px; margin:0 4px; cursor:pointer;" title="${badge.name}">${badge.icon}</span>`;
          }).filter(b => b).join('');
          ...
          html += `
            <div class="gamification-section" style="margin-top:24px; padding:16px; background: ${bgColor}; border-radius:20px;">
              <!-- Fila 1: Nivel (izquierda) y Zapatilla (derecha) -->
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <div style="text-align: left;">
                  <div style="font-size:10px; letter-spacing:1px; text-transform:uppercase; color:var(--text-secondary);">Nivel</div>
                  <strong style="font-size:32px; font-weight:300; color:${levelColor};">${gamificationData.level}</strong>
                </div>
                <div style="text-align: right;">
                  <div style="font-size:10px; letter-spacing:1px; text-transform:uppercase; color:var(--text-secondary);">Zapatilla</div>
                  <strong style="font-size:14px;">${Utils.escapeHTML(shoeName)}</strong><br>
                  <span style="font-size:11px; opacity:0.8;">${shoeKm} km</span>
                </div>
              </div>
              
              <!-- Barra de progreso -->
              <div class="level-progress" style="margin:0 0 20px 0; background:var(--border-color); height:2px; border-radius:2px;">
                <div class="level-progress-fill" style="width: ${progress}%; background: ${levelColor}; height:2px; border-radius:2px;"></div>
              </div>
              
              <!-- Fila 2: Total km | Sesiones | XP (tres columnas) -->
              <div style="display: flex; justify-content: space-between; align-items: center; gap: 12px; margin-bottom: 20px;">
                <div style="flex:1; text-align: center;">
                  <div style="font-size:10px; text-transform:uppercase; color:var(--text-secondary);">📏 km</div>
                  <strong style="font-size:16px;">${gamificationData.totalDistance.toFixed(1)}</strong>
                </div>
                <div style="flex:1; text-align: center;">
                  <div style="font-size:10px; text-transform:uppercase; color:var(--text-secondary);">🎯 Sesiones</div>
                  <strong style="font-size:16px;">${gamificationData.totalSessions}</strong>
                </div>
                <div style="flex:1; text-align: center;">
                  <div style="font-size:10px; text-transform:uppercase; color:var(--text-secondary);">✨ XP</div>
                  <strong style="font-size:16px;">${gamificationData.totalXP}</strong>
                </div>
              </div>
              
              <!-- Insignias: solo iconos, con tooltip al hacer clic -->
              ${badgesIconsOnly ? `<div class="badges-icons-container" style="display: flex; justify-content: center; flex-wrap: wrap; gap: 8px; margin-bottom: 16px;">${badgesIconsOnly}</div>` : '<p style="text-align:center; font-size:11px; margin-bottom:16px;">Completa entrenamientos para desbloquear insignias</p>'}
              
              <!-- Botones -->
              <div style="display: flex; justify-content: center; gap: 12px;">
                <button id="changeShoeBtn" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:5px 16px; border-radius:30px; font-size:11px; letter-spacing:0.5px; cursor:pointer;">👟 Cambiar</button>
                <button id="historyShoeBtn" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:5px 16px; border-radius:30px; font-size:11px; letter-spacing:0.5px; cursor:pointer;">📜 Historial</button>
              </div>
            </div>
          `;
        }
      ...
      setTimeout(() => {
        ...
        // Evento para insignias: mostrar alert con nombre y descripción
        document.querySelectorAll('.badge-icon').forEach(icon => {
          icon.removeEventListener('click', Profile._showBadgeInfo);
          icon.addEventListener('click', Profile._showBadgeInfo);
          // También para móvil: touchstart
          icon.removeEventListener('touchstart', Profile._showBadgeInfo);
          icon.addEventListener('touchstart', Profile._showBadgeInfo);
        });
        
        // Botón cambiar zapatilla
  ...
  // Mostrar información de la insignia al hacer clic
  _showBadgeInfo(e) {
    const badgeSpan = e.currentTarget;
    const name = badgeSpan.getAttribute('data-badge-name');
    const desc = badgeSpan.getAttribute('data-badge-desc');
    if (name && desc) {
      alert(`${name}\n${desc}`);
    } else {
      Utils.showToast('Insignia desconocida', 'info');
    }
  },
