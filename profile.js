// ==================== profile.js - CON BOTONES DE CAMBIAR ZAPATILLA (PEQUEÑO) E HISTORIAL ====================
// Versión: 6.9 - Corregida sintaxis del setTimeout.
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    try {
      ...
      // ========== GAMIFICACIÓN CON BOTONES PEQUEÑOS E HISTORIAL ==========
      try {
        ...
        if (gamificationData) {
          ...
          html += `
            <div class="gamification-section" style="margin-top:24px; padding:20px; background: ${bgColor}; border-radius:24px; text-align:center;">
              <!-- Título: Nivel -->
              <div style="margin-bottom:20px;">
                <div style="font-size:12px; letter-spacing:2px; text-transform:uppercase; color:var(--text-secondary);">NIVEL</div>
                <strong style="font-size:48px; font-weight:300; color:${levelColor};">${gamificationData.level}</strong>
              </div>
              
              <!-- Barra de progreso -->
              <div class="level-progress" style="margin:0 0 24px 0; background:var(--border-color); height:4px; border-radius:4px;">
                <div class="level-progress-fill" style="width: ${progress}%; background: ${levelColor}; height:4px; border-radius:4px;"></div>
              </div>
              
              <!-- Datos: km total y zapatilla -->
              <div style="display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 20px;">
                <div style="flex:1;">
                  <div style="font-size:11px; opacity:0.7;">📏 Total km</div>
                  <strong style="font-size:18px;">${gamificationData.totalDistance.toFixed(1)}</strong>
                  <span style="font-size:12px;"> km</span>
                </div>
                <div style="flex:1;">
                  <div style="font-size:11px; opacity:0.7;">👟 Zapatilla</div>
                  <strong>${Utils.escapeHTML(shoeName)}</strong>
                  <div style="font-size:12px; opacity:0.7;">${shoeKm} km</div>
                </div>
              </div>
              ...
              <div style="display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 24px;">
                <div style="flex:1;">
                  <div style="font-size:11px; opacity:0.7;">🎯 Sesiones</div>
              ...
              <!-- Insignias -->
              ${badgesHTML ? `<div class="badges-container" style="margin-bottom: 16px;">${badgesHTML}</div>` : '<p style="margin-bottom:16px; font-size:12px;">Completa entrenamientos para desbloquear insignias</p>'}
              
              <!-- Botones: cambiar zapatilla (pequeño) + historial -->
              <div style="display: flex; justify-content: center; gap: 12px;">
                <button id="changeShoeBtn" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:6px 16px; border-radius:30px; font-size:11px; letter-spacing:0.5px; cursor:pointer;">👟 Cambiar</button>
                <button id="historyShoeBtn" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:6px 16px; border-radius:30px; font-size:11px; letter-spacing:0.5px; cursor:pointer;">📜 Historial</button>
              </div>
            </div>
          `;
        }
      ...
      setTimeout(() => {
        ...
        // Botón "Cambiar zapatilla" (más pequeño)
        ...
        if (historyBtn) {
          historyBtn.onclick = async () => {
            ...
            `;
            // Mostrar desde la más reciente (invertir copia)
