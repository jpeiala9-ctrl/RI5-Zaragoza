// ==================== profile.js - PERFIL CON ZAPATILLA Y NUEVO DISEÑO DE TARJETA ====================
// Versión: 6.4 - Nivel como título centrado arriba, botón cambiar zapatilla centrado abajo.
// ====================
    ...
    try {
      ...
      try {
        ...
        if (gamificationData) {
          ...
          // Convertir color a rgba para fondo sutil
          ...
          html += `
            <div class="gamification-section" style="margin-top:24px; padding:16px; background: ${bgColor}; border-radius:20px; transition: background 0.3s; text-align:center;">
              <!-- Título centrado: Nivel -->
              <div style="margin-bottom:16px;">
                <strong style="font-size:24px; color: ${levelColor};">Nivel ${gamificationData.level}</strong>
              </div>
              
              <!-- Datos intermedios: km total, zapatilla, sesiones, XP -->
              <div style="display: flex; justify-content: space-around; align-items: center; flex-wrap: wrap; gap: 12px; margin-bottom: 16px;">
                <div>
                  <div style="font-size:12px; opacity:0.7;">📏 Total km</div>
                  <strong>${gamificationData.totalDistance.toFixed(1)} km</strong>
                </div>
                <div>
                  <div style="font-size:12px; opacity:0.7;">👟 Zapatilla</div>
                  <strong>${Utils.escapeHTML(shoeName)}</strong><br>
                  <span style="font-size:12px;">${shoeKm} km</span>
                </div>
                <div>
                  <div style="font-size:12px; opacity:0.7;">🎯 Sesiones</div>
                  <strong>${gamificationData.totalSessions}</strong>
                </div>
                <div>
                  <div style="font-size:12px; opacity:0.7;">✨ XP</div>
                  <strong>${gamificationData.totalXP}</strong>
                </div>
              </div>
              
              <!-- Barra de progreso -->
              <div class="level-progress" style="margin: 8px 0 12px 0;">
                <div class="level-progress-fill" style="width: ${progress}%; background: ${levelColor}; height: 6px; border-radius: 3px;"></div>
              </div>
              
              <!-- Insignias (si hay) -->
              ${badgesHTML ? `<div class="badges-container" style="margin: 8px 0;">${badgesHTML}</div>` : '<p style="margin:8px 0; font-size:12px;">Completa entrenamientos para desbloquear insignias</p>'}
              
              <!-- Botón cambiar zapatilla centrado abajo -->
              <button id="changeShoeBtn" style="margin-top:12px; background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:8px 20px; border-radius:30px; font-size:13px; cursor:pointer; display:inline-block;">👟 Cambiar zapatilla</button>
            </div>
          `;
        }
