// ==================== profile.js - Perfil con eliminación de entradas del muro y ortofoto IGN ====================
// Versión: 10.5 - Añadido botón para eliminar publicaciones del muro
// ====================
...
const Profile = {
  async cargarPerfil(forceRefresh = false) {
    ...
    try {
      ...
      // ========== MIS ÚLTIMOS ENTRENAMIENTOS (con mapa y botón eliminar) ==========
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            const tieneGPS = entry.gpsRoute && entry.gpsRoute.points && entry.gpsRoute.points.length >= 2;
            
            entrenamientosHTML += `
              <div class="mis-entreno-wrapper" data-entry-id="${entryId}" data-plan-id="${entry.planId || ''}" data-sesion-index="${entry.sesionIndex || ''}" style="border-bottom:1px solid var(--border-color); margin-bottom:8px;">
                ...
                <div class="mis-entreno-detalle" style="display:none; padding:0 0 12px 0;">
                  <div style="display:flex; gap:16px; font-size:13px; margin-bottom:8px; flex-wrap:wrap;">
                    <span>⏱️ ${duracion}'</span>
                    <span>📏 ${distancia} km</span>
                    <span>⚡ ${tss} TSS</span>
                  </div>
                  ${tieneGPS ? `<div id="mapPerfil_${entryId}" class="route-map" style="height: 180px; margin-top: 8px; border-radius: 8px; overflow: hidden;"></div>` : ''}
                  <div style="display:flex; justify-content:space-between; align-items:center; margin-top:8px;">
                    ...
                    <button class="delete-wall-btn" data-entry-id="${entryId}" data-plan-id="${entry.planId || ''}" data-sesion-index="${entry.sesionIndex || ''}" style="background:none; border:none; cursor:pointer; font-size:14px; color:var(--zone-5);">🗑️</button>
                  </div>
                </div>
              </div>
            `;
  ...
  _toggleEntrenoDetail(e) {
    ...
    if (detailDiv) {
      ...
      if (!isVisible) {
        const mapDiv = wrapper.querySelector('.route-map');
  ...
  async _eliminarEntradaMuro(e) {
    ...
    Utils.showLoading();
    try {
      // 1. Eliminar la entrada del muro
      await firebaseServices.db.collection('globalFeed').doc(entryId).delete();

      // 2. Si tiene planId y sesionIndex, desmarcar la sesión en el plan
      if (planId && sesionIndex !== undefined && AppState.currentUserId) {
        ...
        // También eliminar la ruta GPS asociada
        await planRef.update({ [`gpsRoutes.${sesionIndex}`]: firebaseServices.FieldValue.delete() });
      }

      Utils.showToast('✅ Publicación eliminada', 'success');
      // Recargar el perfil para que desaparezca
  ...
  async _initProfileMaps() {
    const mapContainers = document.querySelectorAll('.mis-entreno-wrapper .route-map');
    for (const container of mapContainers) {
      const wrapper = container.closest('.mis-entreno-wrapper');
      if (!wrapper) continue;
      ...
      try {
        ...
        if (doc.exists && doc.data().gpsRoute) {
          ...
          if (gpsRoute.points && gpsRoute.points.length >= 2) {
            // Usar la capa de ortofoto IGN
            await GPSTracker.loadLeaflet();
  ...
  // ========== RESTO DE FUNCIONES (abrirModal, cerrarModal, etc.) ==========
  abrirModal() {
