...
// Versión: 10.4 - Usa distancia/tiempo real del GPS, minimapa claro
// ====================

const Profile = {
  _gpsEntries: {},

  // ===== MINIMAPA SVG INLINE (claro) =====
  _fallbackRenderTrackSVG(points, width = 300, height = 120) {
    ...
    return `<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"
      style="border-radius:10px;background:#f8f9fa;display:block;width:100%;"
  ...
  async cargarPerfil(forceRefresh = false) {
    ...
    try {
      ...
      // ========== MIS ÚLTIMOS ENTRENAMIENTOS (con datos GPS reales) ==========
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            const duracionReal = entry.gpsDurationMs ? Math.round(entry.gpsDurationMs / 60000) : (entry.duration || 0);
            ...
            const gpsBadge = entry.hasGPS
              ? `<span style="font-size:9px;color:#1e88e5;background:rgba(30,136,229,0.12);border:1px solid rgba(30,136,229,0.3);border-radius:20px;padding:2px 7px;margin-left:6px;letter-spacing:1px;">GPS</span>`
              : '';
            let gpsMapBlock = '';
            if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
              const mapSVG = (typeof Wall !== 'undefined' && Wall._renderMiniMap)
                ? Wall._renderMiniMap(entry.trackPoints, 300, 120)
              ...
              if (mapSVG) {
                ...
                const timeGPS = (entry.gpsDurationMs && window.GPSTrackViewer) ? GPSTrackViewer._fmtTime(entry.gpsDurationMs) : '';
                gpsMapBlock = `
                  <div class="gps-minimap-tap-profile" data-entry-id="${entryId}"
                    ...
                    <div style="position:absolute;inset:0;display:flex;flex-direction:column;
                                ...
                                pointer-events:none;">
                      <div style="background:rgba(0,0,0,0.7);color:white;font-size:10px;
              ...
              <div class="mis-entreno-wrapper" data-entry-id="${entryId}" style="border-bottom:1px solid var(--border-color); margin-bottom:8px;">
                ...
                <div class="mis-entreno-detalle" style="display:none; padding:0 0 12px 0; flex-direction:column;">
                  <div style="display:flex; gap:16px; font-size:13px; margin-bottom:8px;">
                    <span>⏱️ ${duracionReal}'</span><span>📏 ${distancia} km</span><span>⚡ ${tss} TSS</span>
                  </div>
                  ${gpsMapBlock}
        ...
        document.querySelectorAll('.gps-minimap-tap-profile').forEach(el => {
          el.addEventListener('click', (e) => {
            ...
            if (!window.GPSTrackViewer) {
              Utils.showToast('Visor no disponible', 'error');
              return;
            }
  ...
  async _mostrarModalInsignias() {
    ...
    for (const id of allBadgesOrder) {
      ...
      if (badge) {
        if (earnedBadges.includes(id)) earned.push(badge);
