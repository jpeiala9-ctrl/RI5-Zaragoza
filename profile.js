// ==================== profile.js ====================
// Versión: 10.14 - FIX mini-mapa del track GPS desaparecido en el perfil.
//                Causa: al terminar una sesión con GPS, calendar.js hace
//                primero un add() del entry con hasGPS:true pero SIN
//                trackPoints (los puntos GPS se añaden con un update()
//                posterior, hecho desde gps-tracker.js después de
//                calcular los récords por tramo). La tarjeta se pintaba
//                sin mini-mapa (porque entry.trackPoints era undefined),
//                y _reconciliarMisEntrenamientos() solo refrescaba la
//                miniatura si ya existía el contenedor del mini-mapa --
//                como no existía, la llegada posterior de los trackPoints
//                no provocaba nada y la tarjeta se quedaba sin mini-mapa
//                ni capa de tap. Ahora cada tarjeta lleva sus datos GPS
//                en atributos data-* y _reconciliarMisEntrenamientos los
//                compara, recreando la tarjeta cuando cambia la
//                estructura del mini-mapa (y solo sustituyendo la
//                miniatura cuando únicamente llega un mapSnapshot nuevo,
//                para no recrear sin necesidad).
...
const Profile = {
  ...
  // 🔥 v10.13: caché en tiempo real de "MIS ÚLTIMOS ENTRENAMIENTOS" (mismo
  // patrón que Friends._exploreCache para la página 1 de "Explorar
  // usuarios"). Antes, cada vez que se entraba en la pestaña Perfil se
  // pedían estas 5 sesiones a Firestore con un .get() suelto -- de ahí las
  // tarjetas vacías pintándose y rellenándose un instante después, en
  // CADA visita. Ahora es un listener onSnapshot abierto una sola vez por
  // sesión (_iniciarListenerMisEntrenamientos, idempotente): la primera
  // vez cuesta esa lectura (inevitable), pero a partir de ahí volver a la
  // pestaña Perfil pinta al instante desde este caché en memoria, sin
  // tocar Firestore ni ver ningún parpadeo -- y en cuanto se completa o
  // borra una sesión de verdad, el propio listener manda el dato nuevo y
  // repinta solo, esté la pestaña abierta en ese momento o no.
  _entrenamientosCache: null,
  _entrenamientosListenerUnsub: null,
  _entrenamientosListenerResolvers: [],

  _iniciarListenerMisEntrenamientos() {
    if (this._entrenamientosListenerUnsub) return;
    if (!AppState.currentUserId) return;
    this._entrenamientosCache = { snapshot: null, cargando: true };
    this._entrenamientosListenerUnsub = firebaseServices.db
      .collection('globalFeed')
      .where('userId', '==', AppState.currentUserId)
      .orderBy('timestamp', 'desc')
      .limit(5)
      .onSnapshot((snapshot) => {
        this._entrenamientosCache.snapshot = snapshot;
        this._entrenamientosCache.cargando = false;

        // Si la pestaña Perfil ya está montada (wrapper presente), se
        // repinta al momento con el dato fresco -- por ejemplo, al
        // completar una sesión nueva mientras se está viendo el perfil.
        if (document.getElementById('misEntrenamientosWrapper')) {
          this._reconciliarMisEntrenamientos(snapshot);
        }

        const resolvers = this._entrenamientosListenerResolvers;
        this._entrenamientosListenerResolvers = [];
        resolvers.forEach(resolve => resolve());
      }, (error) => {
        console.warn('Error en listener de mis últimos entrenamientos:', error);
        if (this._entrenamientosCache) this._entrenamientosCache.cargando = false;
        const resolvers = this._entrenamientosListenerResolvers;
        this._entrenamientosListenerResolvers = [];
        resolvers.forEach(resolve => resolve());
      });
  ...
  // Se resuelve en cuanto llega el primer snapshot del listener (o al
  // instante si ya había uno con datos de esta sesión).
  _esperarPrimeraCargaEntrenamientos() {
    if (!this._entrenamientosCache || !this._entrenamientosCache.cargando) return Promise.resolve();
    return new Promise(resolve => this._entrenamientosListenerResolvers.push(resolve));
  },

  // Cierra el listener (logout): evita dejarlo abierto facturando
  // lecturas de una sesión que ya ha terminado. Llamado desde
  // AppState.detenerListeners() en app.js.
  detenerListenerMisEntrenamientos() {
    if (this._entrenamientosListenerUnsub) {
      this._entrenamientosListenerUnsub();
      this._entrenamientosListenerUnsub = null;
    }
    this._entrenamientosCache = null;
    this._entrenamientosListenerResolvers = [];
  },
  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    // 🔥 "Entrenado por" vive en su propio contenedor (entrenadoPorContainer,
    // fuera del HTML que se cachea aquí abajo) así que se repinta aparte
    // cada vez que se entra en el perfil. Sin await: no debe retrasar el
    // pintado del perfil, y usa datos ya en memoria (AppState.currentUserData).
  ...
  _renderEntrenamientoItemHTML(entryId, entry) {
    ...
    // 🔥 v10.14: además del HTML, cada tarjeta lleva sus datos GPS
    // actuales como atributos data-* -- _reconciliarMisEntrenamientos
    // los usa para detectar cuándo cambió la estructura del mini-mapa y
    // recrear la tarjeta si hace falta. Esto es lo que hace que, cuando
    // llegan los trackPoints con el update() posterior al add() inicial
    // (ver gps-tracker.js _guardarYPublicar), la tarjeta pase de "sin
    // mini-mapa" a "con mini-mapa" sin tener que esperar a la siguiente
    // recarga del perfil entero.
    return `
      <div class="wall-item" data-entry-id="${entryId}" data-has-gps="${entry.hasGPS ? '1' : '0'}" data-track-count="${Array.isArray(entry.trackPoints) ? entry.trackPoints.length : 0}" data-has-snapshot="${entry.mapSnapshot ? '1' : '0'}" style="margin-bottom:16px;">
  ...
  async _actualizarMisEntrenamientos() {
    ...
    // 🔥 v10.13: ya no se hace un .get() suelto aquí -- se abre (si no
    // había ya uno de esta sesión) el listener en tiempo real y se espera
    // solo a su primer snapshot. Si el listener ya llevaba corriendo desde
    // una visita anterior a esta pestaña, _esperarPrimeraCargaEntrenamientos
    // resuelve al instante (sin ningún viaje a Firestore) y se pinta ya
    // mismo desde el caché en memoria.
    this._iniciarListenerMisEntrenamientos();
    await this._esperarPrimeraCargaEntrenamientos();
    const snapshot = this._entrenamientosCache?.snapshot;
    if (!snapshot) return; // el error, si lo hubo, ya quedó registrado en el listener
    this._reconciliarMisEntrenamientos(snapshot);
  },

  // Contiene la reconciliación incremental que antes vivía dentro de
  // _actualizarMisEntrenamientos: recibe el snapshot (del listener en
  // tiempo real) y actualiza el DOM tocando solo lo que de verdad cambió.
  // Se llama tanto desde ahí (al entrar/volver a la pestaña Perfil) como
  // directamente desde el propio listener cuando llega un dato nuevo con
  // la pestaña ya abierta.
  _reconciliarMisEntrenamientos(snapshot) {
    const wrapper = document.getElementById('misEntrenamientosWrapper');
    if (!wrapper) return;
    ...
    //
    // 🔥 v10.14: además, si los datos GPS de la entrada han cambiado
    // respecto a lo que ya está pintado (hasGPS/trackPoints/mapSnapshot),
    // se recrea la tarjeta entera -- esto es lo que hace que, cuando el
    // update() posterior al add() añade los trackPoints que faltaban, la
    // tarjeta pase de "sin mini-mapa" a "con mini-mapa" en el mismo
    // instante en que llega el dato nuevo. Si solo llegó el mapSnapshot
    // (imagen ya cacheada del mini-mapa), se sustituye únicamente la
    // miniatura, sin recrear el resto de la tarjeta -- así no se pierden
    // listeners ni se recrea lo que no ha cambiado.
    ordenNuevo.forEach(entryId => {
      let el = lista.querySelector(`[data-entry-id="${entryId}"]`);
      const entryActual = entriesNuevas[entryId] || {};

      if (!el) {
        const div = document.createElement('div');
        div.innerHTML = this._renderEntrenamientoItemHTML(entryId, entryActual).trim();
        el = div.firstElementChild;
      } else {
        const hasGPSNuevo = !!entryActual.hasGPS;
        const trackCountNuevo = Array.isArray(entryActual.trackPoints) ? entryActual.trackPoints.length : 0;
        const hasSnapshotNuevo = !!entryActual.mapSnapshot;

        const hasGPSPrevio = el.dataset.hasGps === '1';
        const trackCountPrevio = parseInt(el.dataset.trackCount || '0', 10);
        const hasSnapshotPrevio = el.dataset.hasSnapshot === '1';

        const cambioEstructura =
          hasGPSPrevio !== hasGPSNuevo ||
          trackCountPrevio !== trackCountNuevo;

        if (cambioEstructura) {
          // Recrear la tarjeta entera: cambió la estructura del mini-mapa
          // (p.ej. llegó el trackPoints que faltaba tras el add() inicial
          // con hasGPS:true pero sin puntos GPS).
          const div = document.createElement('div');
          div.innerHTML = this._renderEntrenamientoItemHTML(entryId, entryActual).trim();
          const nuevoEl = div.firstElementChild;
          el.replaceWith(nuevoEl);
          el = nuevoEl;
        } else if (hasSnapshotNuevo && !hasSnapshotPrevio) {
          // Solo llegó el mapSnapshot (imagen ya cacheada del mini-mapa):
          // reemplazar la miniatura por la imagen real, sin recrear el
          // resto de la tarjeta.
          const miniatura = el.querySelector('.gps-minimap-static');
          if (miniatura && !miniatura.querySelector('img')) {
            miniatura.innerHTML = `<img src="${entryActual.mapSnapshot}" style="height:100%; width:100%; object-fit:cover; display:block;">`;
            el.dataset.hasSnapshot = '1';
          }
        }
      }
...
console.log('✅ profile.js v10.14 - Fix mini-mapa del track GPS: cada tarjeta lleva sus datos GPS en data-* y _reconciliarMisEntrenamientos recrea la tarjeta cuando llegan los trackPoints');