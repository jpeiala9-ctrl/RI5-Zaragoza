// ==================== profile.js ====================
// Versión: 11.7 - CORREGIDO DEFINITIVAMENTE:
//                 - Eliminado onclick del HTML del mini mapa
//                 - Solo event listener de JavaScript
//                 - Un clic = una apertura, siempre
// ====================
...
const Profile = {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
              ...
              // ELIMINADO onclick del HTML - ahora solo data-entry-id
              miniMapHTML = `
  ...
  async _crearMiniMapaPerfil(container, trackPoints) {
    ...
    try {
      ...
      // Esto es CRÍTICO para que el mapa se renderice correctamente
      setTimeout(() => {
        map.invalidateSize();
      }, 100);
  ...
  //  EL MAPA SE DETECTA PRIMERO Y SE ABRE CON UN SOLO CLIC
  // ================================================================
  _handleContainerClick(e) {
    const target = e.target;
    
    // --- 1. PRIMERO: MINI MAPA ---
    // Buscamos el contenedor del mapa. Si el clic es en el mapa o en su overlay,
    // lo abrimos y SALIMOS INMEDIATAMENTE.
    ...
    if (mapElement) {
      ...
      if (entry && window.GPSTrackViewer) {
        // Abrir el visor GPS con UN SOLO CLIC
        GPSTrackViewer.open(entry);
      } else {
        console.warn('No se encontró la entrada GPS para el entryId:', entryId);
      }
      return; // SALIR - nada más se ejecuta
    }
...
console.log('✅ profile.js v11.7 - Eliminado onclick del HTML, solo event listener de JavaScript. Un clic = una apertura.');