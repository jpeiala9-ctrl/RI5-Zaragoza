...
const Profile = {
  ...
  _handleContainerClick(e) {
    ...
    if (mapElement) {
      e.preventDefault();
      e.stopPropagation(); // <-- CRUCIAL: evita que el evento suba al .wall-item
    ...
    modal.querySelectorAll('.like-user-item').forEach(item => {
      item.addEventListener('click', (e) => {
        ...
        if (uid && window.Friends) {
          // ABRIR PERFIL - NO CERRAR MODAL DE LIKES
          Friends.abrirModalAmigo(uid);
          // El modal de likes se queda abierto detrás
        }
      });
    });
...
console.log('✅ profile.js v11.3 - Modal de likes ya no se cierra al ver perfil, mini mapa con stopPropagation');