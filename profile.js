// ==================== profile.js - TARJETA "PASAPORTE DEL RUNNING" CON BORDE DEL COLOR DEL NIVEL ====================
// Versión: 9.0 - Diseño de pasaporte: borde coloreado, fondo suave, insignias como sellos.
// ====================
    ...
    try {
      ...
      try {
        ...
        if (gamificationData) {
          ...
          // Fondo muy sutil (casi transparente) para que destaque el borde
          const bgColor = levelColor.startsWith('#') 
            ? `rgba(${parseInt(levelColor.slice(1,3),16)}, ${parseInt(levelColor.slice(3,5),16)}, ${parseInt(levelColor.slice(5,7),16)}, 0.05)`
            : 'var(--bg-secondary)';
          
          // Insignias como sellos redondeados
          const badgesStamps = (gamificationData.badges || []).map(badgeId => {
            const badge = Gamification.BADGES[badgeId];
            if (!badge) return '';
            return `<span class="badge-stamp" data-badge-name="${badge.name}" data-badge-desc="${badge.description}" style="display:inline-flex; align-items:center; gap:4px; background:var(--bg-card); border:1px solid ${levelColor}; border-radius:30px; padding:4px 10px; font-size:12px; margin:0 4px; cursor:pointer;" title="${badge.name}">${badge.icon} ${badge.name}</span>`;
          }).filter(b => b).join('');
          ...
          html += `
            <div class="passport-card" style="margin-top:24px; border:2px solid ${levelColor}; border-radius:24px; background:${bgColor}; box-shadow:0 8px 20px rgba(0,0,0,0.1); overflow:hidden; backdrop-filter:blur(2px);">
              ...
              <div style="padding:16px 20px;">
                ...
                <!-- Barra de progreso estilo pasaporte (puntos) -->
                <div style="margin-bottom: 20px;">
                  ...
                  <div style="display:flex; justify-content:space-between; margin-top:4px;">
                    ...
                    <span style="font-size:8px;">${this.LEVELS_KM.find(l => l.level === gamificationData.level+1)?.kmNeeded || gamificationData.totalDistance} km</span>
                  </div>
                </div>
                
                <!-- Fila 2: Estadísticas (tres columnas) -->
                ...
                <div style="display: flex; justify-content: center; gap: 12px; margin-top:8px;">
                  <button id="changeShoeBtn" style="background:transparent; border:1px solid ${levelColor}; color:${levelColor}; padding:6px 16px; border-radius:40px; font-size:11px; letter-spacing:0.5px; cursor:pointer; transition:all 0.2s;">👟 Cambiar zapatilla</button>
                  <button id="historyShoeBtn" style="background:transparent; border:1px solid ${levelColor}; color:${levelColor}; padding:6px 16px; border-radius:40px; font-size:11px; letter-spacing:0.5px; cursor:pointer; transition:all 0.2s;">📜 Historial</button>
                </div>
              </div>
            </div>
          `;
