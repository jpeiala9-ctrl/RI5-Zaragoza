...
// Versión: 3.4 - Añadido historial de últimos 10 entrenamientos realizados
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    try {
      ...
      // Cargar los últimos 10 entrenamientos realizados
      await this.cargarUltimosEntrenos(AppState.currentUserId);
  ...
  // ========== NUEVA FUNCIÓN: ÚLTIMOS 10 ENTRENAMIENTOS REALIZADOS ==========
  async cargarUltimosEntrenos(uid) {
    const container = document.getElementById('ultimosEntrenosContainer');
    if (!container) return;
    ...
    try {
      ...
      if (snapshot.empty) {
        container.innerHTML = '<p style="text-align:center; padding:20px; color: var(--text-secondary);">Sin entrenamientos recientes</p>';
        return;
      }

      let html = '<h3 style="font-size:18px; margin: 30px 0 15px 0; text-align:center;">🏃 ÚLTIMOS ENTRENAMIENTOS</h3>';
      for (const doc of snapshot.docs) {
        const e = doc.data();
        const fecha = e.timestamp ? new Date(e.timestamp.toDate()).toLocaleDateString() : '';
        const tipoTexto = {
          rodaje: '🏃‍♂️ Rodaje',
          tempo: '⚡ Tempo',
          series: '🔁 Series',
      ...
      console.error('Error cargando últimos entrenos:', error);
      container.innerHTML = '<p style="text-align:center; color: var(--zone-5);">Error al cargar entrenamientos</p>';
    }
  },
