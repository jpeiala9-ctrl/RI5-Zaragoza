...
// Versión: 10.3 - Nuevo estilo de tarjetas para el perfil
// ====================
...
const Profile = {
  async cargarPerfil(forceRefresh = false) {
    ...
    if (!forceRefresh) {
      ...
      // Limpiar también cualquier caché de gamificación
    ...
    try {
      ...
      // Obtener datos de gamificación para mostrar nivel y estadísticas
      ...
      // Obtener datos de zonas si existen
      const ritmoBase = AppState.lastRitmoBase ? Utils.formatR(AppState.lastRitmoBase) : '—';
      const fcMax = AppState.lastFC || '—';
      
      // Obtener objetivo del plan activo
      let objetivo = '—';
      let modalidad = '—';
      let nivelPlan = '—';
      if (AppState.planGeneradoActual) {
        const distMap = { '2k': '2KM', '5k': '5KM', '10k': '10KM', 'medio': 'MEDIA', 'maraton': 'MARATÓN' };
        objetivo = distMap[AppState.planGeneradoActual.distancia] || AppState.planGeneradoActual.distancia || '—';
        modalidad = AppState.planGeneradoActual.modalidad === 'trail' ? 'Trail' : 'Asfalto';
        nivelPlan = AppState.planGeneradoActual.nivel ? AppState.planGeneradoActual.nivel.charAt(0).toUpperCase() + AppState.planGeneradoActual.nivel.slice(1) : '—';
      }
        ...
        <div class="perfil-header">
          ...
          <div class="perfil-info">
            ...
            <div class="perfil-stats">
              ...
              <div class="perfil-stat"><span>${totalSessions}</span><label>SESIONES</label></div>
              <div class="perfil-stat"><span>${totalDistance.toFixed(0)}</span><label>KM TOTALES</label></div>
            </div>
          </div>
        </div>
      `;
      ...
      html += `
        <div style="margin: 16px 0 24px 0;">
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
            <span style="font-size: 14px; font-weight: 600; color: ${levelColor};">Nivel ${level}</span>
            <span style="font-size: 12px; color: var(--text-secondary);">${progress}% → Nivel ${level + 1}</span>
          </div>
          <div class="progress-bar" style="background: var(--bg-secondary); height: 8px; border-radius: 4px; overflow: hidden;">
            <div class="progress-fill" style="width: ${progress}%; background: ${levelColor}; height: 100%;"></div>
          </div>
        </div>
      `;
      
      // Grid de datos del corredor
      html += `
        <div class="perfil-detalle-grid" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin: 20px 0;">
          <div class="perfil-detalle-item" style="background: var(--bg-secondary); border-radius: 12px; padding: 12px; text-align: center;">
            <div class="label" style="font-size: 11px; color: var(--text-secondary);">EDAD</div>
            <div class="value" style="font-size: 16px; font-weight: 600; color: var(--accent-blue);">${age}</div>
          </div>
          <div class="perfil-detalle-item" style="background: var(--bg-secondary); border-radius: 12px; padding: 12px; text-align: center;">
            <div class="label" style="font-size: 11px; color: var(--text-secondary);">FC MÁX</div>
            <div class="value" style="font-size: 16px; font-weight: 600; color: var(--accent-blue);">${fcMax} ppm</div>
          </div>
          <div class="perfil-detalle-item" style="background: var(--bg-secondary); border-radius: 12px; padding: 12px; text-align: center;">
            <div class="label" style="font-size: 11px; color: var(--text-secondary);">MARCA 6KM</div>
            <div class="value" style="font-size: 16px; font-weight: 600; color: var(--accent-blue);">${ritmoBase}</div>
          </div>
          <div class="perfil-detalle-item" style="background: var(--bg-secondary); border-radius: 12px; padding: 12px; text-align: center;">
            <div class="label" style="font-size: 11px; color: var(--text-secondary);">OBJETIVO</div>
            <div class="value" style="font-size: 16px; font-weight: 600; color: var(--accent-blue);">${objetivo}</div>
          </div>
          <div class="perfil-detalle-item" style="background: var(--bg-secondary); border-radius: 12px; padding: 12px; text-align: center;">
            <div class="label" style="font-size: 11px; color: var(--text-secondary);">MODALIDAD</div>
            <div class="value" style="font-size: 16px; font-weight: 600; color: var(--accent-blue);">${modalidad}</div>
          </div>
          <div class="perfil-detalle-item" style="background: var(--bg-secondary); border-radius: 12px; padding: 12px; text-align: center;">
            <div class="label" style="font-size: 11px; color: var(--text-secondary);">NIVEL</div>
            <div class="value" style="font-size: 16px; font-weight: 600; color: var(--accent-blue);">${nivelPlan}</div>
          </div>
        </div>
      `;
