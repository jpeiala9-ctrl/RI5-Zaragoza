...
const Profile = {
  ...
  // Inicializa el mapa Leaflet y el "tap" de una entrada concreta.
  // IMPORTANTE: Wall._crearMiniMapa es ASÍNCRONA (espera a que cargue
  // Leaflet + a que el navegador pinte el layout) antes de crear el mapa
  // de verdad. Si en ese intervalo el contenedor estaba oculto (pestaña
  // Perfil todavía no visible, p.ej. durante la precarga en segundo plano
  // al abrir la app) o desaparecía del DOM, el mapa se quedaba sin crear
  // para siempre, porque antes esta función solo se llamaba una vez por
  // entrada. Por eso:
  //  - No se intenta nada si el contenedor no es visible todavía (evita
  //    crear un mapa con tamaño 0 que luego no se puede arreglar).
  //  - Se usa un flag propio (dataset.mapaIniciando), no _leaflet_id, para
  //    saber si ya hay un intento en marcha: así esta función se puede
  //    llamar tantas veces como haga falta (se reintenta en cada
  //    actualización de la lista) sin lanzar dos mapas a la vez sobre el
  //    mismo contenedor ni tocar uno que ya está cargado.
  _inicializarMapaEntrada(entryId, entry) {
    ...
    if (!mapContainer || mapContainer._leaflet_id || mapContainer.dataset.mapaIniciando === '1') return;
    if (!window.Wall || typeof Wall._crearMiniMapa !== 'function') return;

    // Contenedor realmente visible (no oculto por display:none en algún
    // antepasado, como la propia pestaña Perfil si aún no se ha abierto).
    const visible = mapContainer.offsetParent !== null && mapContainer.offsetWidth > 0;
    if (!visible) return;

    mapContainer.dataset.mapaIniciando = '1';
    Wall._crearMiniMapa(mapId, entry.trackPoints)
      .then((mapaCreado) => {
        // Si _crearMiniMapa aborta (el contenedor desapareció del DOM
        // mientras esperaba) no llega a asignar _leaflet_id: quitamos nu
        // estro flag para que el próximo repintado lo vuelva a intentar.
        if (!mapaCreado) delete mapContainer.dataset.mapaIniciando;
      })
      .catch(() => { delete mapContainer.dataset.mapaIniciando; });
  },

  // Reintenta inicializar el mapa de TODAS las entradas GPS conocidas
  // (no solo las recién añadidas). Es barato: para una entrada cuyo mapa
  // ya está cargado, _inicializarMapaEntrada no hace nada (corta en el
  // primer if). Esto es lo que garantiza que un intento fallido (p.ej.
  // por precarga en segundo plano con la pestaña oculta) se recupere solo
  // en cuanto el usuario entra de verdad en Perfil, sin tener que tocar
  // ni recargar los mapas que ya funcionan.
  _reintentarMapasGPS() {
    Object.keys(this._gpsEntries || {}).forEach(entryId => {
      this._inicializarMapaEntrada(entryId, this._gpsEntries[entryId]);
    });
  },
  ...
  async _actualizarMisEntrenamientos() {
    ...
    //    más nuevas). Esto también libera su mapa Leaflet.
    ...
    //    (esto no afecta al mapa ni a ningún otro nodo de la entrada).
    existentes.forEach(entryId => {
    ...
    // Reintento general: crea el mapa de las entradas nuevas y, sobre
    // todo, reintenta las que se quedaron sin cargar en un intento
    // anterior (p.ej. por la precarga en segundo plano con la pestaña
    // aún oculta). Para las que ya tienen su mapa cargado esto no hace
    // absolutamente nada (corta al momento en _inicializarMapaEntrada).
    this._reintentarMapasGPS();
  },
