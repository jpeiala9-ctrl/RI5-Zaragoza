// ==================== profile.js ====================
// Versión: 12.0 - DEFINITIVA
//                 - Mini mapas con la misma estructura que wall.js
//                 - Sin overlay que interfiera con los clics
//                 - map.invalidateSize() para renderizado correcto
//                 - Un clic = una apertura, sin parpadeos
// ====================
...
const Profile = {
  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    try {
      ...
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            // ============================================================
            //  MINI MAPA - MISMA ESTRUCTURA QUE wall.js (FUNCIONA PERFECTO)
            // ============================================================
            let miniMapHTML = '';
            if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
              ...
              miniMapHTML = `
                <div class="gps-minimap-tap-profile" data-entry-id="${entryId}"
                  style="margin-top:10px;border-radius:10px;overflow:hidden; border:1px solid #2a2a2a; cursor:pointer; height:130px; background:#1a1a1a;">
      ...
      // ================================================================
      //  INICIALIZAR MINI MAPAS - IGUAL QUE wall.js
      // ================================================================
      for (const [entryId, data] of Object.entries(this._miniMapEntries)) {
        ...
        if (container && data.trackPoints && data.trackPoints.length >= 2) {
          this._crearMiniMapaPerfil(container, data.trackPoints);
        }
      }
  ...
  async _crearMiniMapaPerfil(container, trackPoints) {
    // Si Leaflet no está cargado, cargarlo
    if (!window.L) {
    ...
    try {
      ...
      // FORZAR RECALCULO DEL TAMAÑO - CRÍTICO PARA QUE EL MAPA SE VEA
      setTimeout(() => {
        map.invalidateSize();
      }, 150);
  ...
  // ================================================================
  //  EVENTOS - EL MAPA SE DETECTA PRIMERO
  // ================================================================
...
console.log('✅ profile.js v12.0 - DEFINITIVA: mini mapas con la misma estructura que wall.js, un clic = una apertura, sin parpadeos');