// ==================== profile.js ====================
// Versión: 10.14 - FIX "el mapa GPS no aparece en Perfil > Mis últimos
//                entrenamientos hasta recargar": al terminar una carrera
//                con GPS, la tarjeta se publica primero SIN datos de GPS y
//                gps-tracker.js le añade hasGPS/trackPoints un instante
//                después con un segundo update() (mismo origen que el fix
//                de wall.js v4.17). _reconciliarMisEntrenamientos solo
//                sabía parchear la IMG dentro de un .gps-minimap-static ya
//                existente -- pero si la tarjeta se creó sin GPS, ese
//                contenedor nunca existió, así que no había nada que
//                parchear y el mapa (ni la insignia 📍GPS) no aparecían
//                nunca. Ahora, si una tarjeta pasa de no tener mapa a
//                tenerlo, se regenera solo el HTML de ESA tarjeta (no de
//                toda la lista), conservando su posición.
  ...
  _reconciliarMisEntrenamientos(snapshot) {
    ...
    ordenNuevo.forEach(entryId => {
      ...
      if (!el) {
        ...
        // La entrada ya estaba en el DOM. Dos posibles novedades:
        //
        // 🔥 FIX "el mapa GPS no aparece en Perfil hasta recargar": si esta
        // tarjeta se pintó ANTES de que gps-tracker.js añadiera hasGPS/
        // trackPoints (ver el mismo fallo explicado en wall.js
        // _sonIgualesEntradas), el <div> ya existente nunca llegó a crear
        // el contenedor .gps-minimap-static porque en su momento
        // entry.hasGPS era falso -- así que el parche de abajo (que solo
        // sustituye la IMG dentro de ese contenedor) no tenía nada que
        // parchear y el mapa no aparecía nunca. En ese caso concreto (pasa
        // de no tener mapa a tenerlo) se regenera el HTML de ESTA tarjeta
        // en concreto -- no de toda la lista -- conservando su posición.
        const teniaMapaAntes = !!el.querySelector('.gps-minimap-static');
        const tieneGPSAhora = !!(entriesNuevas[entryId]?.hasGPS && Array.isArray(entriesNuevas[entryId]?.trackPoints) && entriesNuevas[entryId].trackPoints.length >= 2);
        if (tieneGPSAhora && !teniaMapaAntes) {
          const div = document.createElement('div');
          div.innerHTML = this._renderEntrenamientoItemHTML(entryId, entriesNuevas[entryId]).trim();
        ...
        // Si mientras tanto el Muro ha terminado de cachear la imagen real
        // del mapa (mapSnapshot) y esta tarjeta todavía mostraba el dibujo
        // esquemático, se sustituye SOLO la miniatura por la imagen
        // cacheada -- sin recrear el resto de la tarjeta ni perder sus
        // listeners.
...
console.log('✅ profile.js v10.14 - Mis últimos entrenamientos: el mapa GPS ya aparece aunque llegue en un segundo update() tras publicar la sesión');