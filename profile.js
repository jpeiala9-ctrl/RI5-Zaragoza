// ==================== profile.js ====================
// Versión: 10.4 - Insignias con píldoras (adaptado al nuevo sistema de gamificación)
// ====================
...
const Profile = {
    ...
    try {
      ...
      try {
        ...
        if (gamificationData) {
          ...
          // Generar elementos de insignias usando el nuevo método getBadgeElement
          let badgesElements = '';
          const earnedBadges = gamificationData.badges || [];
          for (const badgeId of earnedBadges) {
            const badgeElement = Gamification.getBadgeElement(badgeId, true);
            if (badgeElement) {
              badgesElements += `<div class="badge-pill-container" data-badge-id="${badgeId}" style="display:inline-block; margin:0 4px 8px 4px; cursor:pointer;">${badgeElement.outerHTML}</div>`;
            }
          }
          ...
          html += `
            <div class="passport-card" style="margin-top:24px; border:2px solid ${levelColor}; border-radius:24px; background:${bgColor}; box-shadow:0 8px 20px rgba(0,0,0,0.1); overflow:hidden;">
              ...
              <div style="padding:16px 20px;">
                ...
                ${badgesElements ? `<div style="margin-bottom: 16px; border-top:1px solid ${levelColor}40; padding-top:16px;">
                  ...
                  <div class="badges-icons-container" style="display: flex; flex-wrap: wrap; justify-content: center; gap: 8px;">${badgesElements}</div>
      ...
      // ========== MIS ÚLTIMOS ENTRENAMIENTOS (IDÉNTICO AL MURO) ==========
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            // --- Generación de la tarjeta IDÉNTICA a la del muro (wall.js) ---
            let fecha = '—', hora = '';
            try {
              if (entry.timestamp) {
            ...
            const likeClass = ''; // siempre sin clase 'liked'
      ...
      setTimeout(() => {
        // Evento para las tarjetas (abrir modal de likes)
        ...
        document.querySelectorAll('.gps-minimap-tap-profile').forEach(el => {
          el.addEventListener('click', (e) => {
            e.stopPropagation();
            if (!window.GPSTrackViewer) return;
        ...
        // Eventos para las insignias (ahora son contenedores .badge-pill-container)
        document.querySelectorAll('.badge-pill-container').forEach(container => {
          container.removeEventListener('click', this._mostrarModalInsignias);
          container.addEventListener('click', this._mostrarModalInsignias.bind(this));
        });
  ...
  // ========== MANEJADORES PARA TARJETAS Y LIKES ==========
  ...
  // ========== MODAL DE INSIGNIAS (ACTUALIZADO CON PÍLDORAS) ==========
  async _mostrarModalInsignias() {
    const gamificationData = await Gamification.getData(AppState.currentUserId);
    if (!gamificationData) return;
    
    const earnedBadges = gamificationData.badges || [];
    // Obtener todas las insignias disponibles ordenadas por XP (las que no tienen XP se ponen al final)
    const allBadges = Object.values(Gamification.BADGES).sort((a,b) => (a.xp || 0) - (b.xp || 0));
    ...
    for (const badge of earned) {
      const pill = Gamification.getBadgeElement(badge.id, true);
      if (pill) {
        content += `<div style="cursor:pointer;" title="${badge.description} (+${badge.xp} XP)">${pill.outerHTML}</div>`;
      } else {
        content += `<div class="gamification-badge badge-category-special">${badge.name}</div>`;
      }
    }
    content += `</div></div>`;
    ...
    if (upcoming.length > 0) {
      ...
      for (const badge of upcoming) {
        const pill = Gamification.getBadgeElement(badge.id, false);
        if (pill) {
          content += `<div style="cursor:pointer; opacity:0.7;" title="${badge.description} (+${badge.xp} XP)">${pill.outerHTML}</div>`;
        } else {
          content += `<div class="gamification-badge gamification-badge-locked badge-category-special">${badge.name}</div>`;
        }
      }
      content += `</div></div>`;
    } else {
  ...
  // ========== CAMBIO DE ZAPATILLA Y HISTORIAL ==========
...
console.log('✅ Profile v10.4 - Insignias con píldoras (adaptado al nuevo sistema de gamificación)');