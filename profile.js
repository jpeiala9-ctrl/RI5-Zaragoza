// ==================== profile.js ====================
// Versión: 10.9 - Corregido: el mini-mapa Leaflet de "Mis últimos
//                entrenamientos" se pintaba a medias cuando se creaba con
//                la pestaña Perfil todavía oculta (precarga en segundo
//                plano); ahora se reintenta al entrar de verdad a Perfil.
// ====================
...
const Profile = {
  ...
  _renderEntrenamientoItemHTML(entryId, entry) {
    ...
    if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
      ...
      // El mini-mapa ahora es un Leaflet real (mismo tile CartoDB, mismos
      // parámetros) en vez del SVG estático de antes, para que la tarjeta
      // sea visualmente idéntica a la del Muro. Se crea UNA sola vez por
      // entrada (ver _vincularToqueEntrada más abajo, con guarda por
      // dataset), nunca en cada reconciliación, para no destruir/recrear
      // mapas ya cargados y evitar el "salto"/parpadeo que eso causaba.
      miniMapHTML = `
        <div style="margin-top:10px; border-radius:10px; overflow:hidden; border:1px solid var(--border-color); position:relative; height:130px;">
          <div id="${mapId}" class="gps-minimap-leaflet" data-entry-id="${entryId}" style="height:100%; width:100%; background:var(--stat-bg);"></div>
  ...
  // El mini-mapa Leaflet (igual que el del Muro) y el toque para abrir el
  // visor interactivo (GPSTrackViewer). Ambos llevan guarda por dataset
  // para que, aunque esta función se llame en cada reconciliación para
  // TODAS las entradas (no solo las nuevas), una entrada que ya tenía su
  // mapa creado no vuelva a tocarse -- así no hay recreaciones ni saltos.
  _mapasPerfil: {},
  ...
  _vincularToqueEntrada(entryId, entry) {
    ...
    if (!mapContainer || mapContainer.dataset.mapCreated) return;
    // El perfil se precarga en segundo plano nada más iniciar sesión,
    // ANTES de que el usuario entre de verdad a la pestaña Perfil (ver
    // Profile.cargarPerfil() llamado desde app.js justo tras el login).
    // Si en ese momento la pestaña sigue oculta (display:none en algún
    // antepasado), el contenedor mide 0×0 y Leaflet inicializa el mapa mal
    // -- de ahí el "medio mapa sí, medio no" (mosaicos a medio cargar).
    // offsetParent es null en cualquier elemento dentro de algo oculto:
    // se usa como comprobación fiable de "esto se ve de verdad ahora
    // mismo". Si no es visible, NO se marca mapCreated y no se crea nada
    // todavía -- se reintenta solo cuando el usuario entra a Perfil de
    // verdad (ver el gancho en switchTab, app.js).
    if (mapContainer.offsetParent === null) return;
    mapContainer.dataset.mapCreated = '1';
    this._crearMiniMapaPerfil(mapId, entry.trackPoints).then(map => {
      if (map) this._mapasPerfil[entryId] = map;
    });
  },

  // Copia deliberada de Wall._crearMiniMapa (mismo tile CartoDB, mismos
  // parámetros, misma doble espera de layout) en vez de reutilizar
  // Wall._crearMiniMapa directamente: esa función guarda cada mapa en
  // Wall.maps para destruirlos en cada cargarMuro() (refresco cada 30s del
  // Muro) -- si los mapas de Perfil se colaran ahí, el refresco del Muro
  // los destruiría por sorpresa. Aquí se guardan aparte, en _mapasPerfil.
  async _crearMiniMapaPerfil(containerId, trackPoints) {
    if (window.Wall && typeof Wall._cargarLeaflet === 'function') await Wall._cargarLeaflet();
  ...
  async _actualizarMisEntrenamientos() {
    ...
    existentes.forEach(entryId => {
      if (!ordenNuevo.includes(entryId)) {
        ...
        if (this._mapasPerfil[entryId]) {
          try { this._mapasPerfil[entryId].remove(); } catch (e) {}
          delete this._mapasPerfil[entryId];
        }
      }
    });
...
console.log('✅ profile.js v10.9 - Fix mini-mapa a medias en pestaña Perfil oculta');