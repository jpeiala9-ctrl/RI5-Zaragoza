// ==================== profile.js - PERFIL CON TARJETA DE NIVEL ESTILO BOWERS & WILKINS ====================
// Versión: 6.6 - Diseño de gama alta: minimalista, negro/gris mate, tipografía fina, detalles metálicos.
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    try {
      ...
      // ========== GAMIFICACIÓN ESTILO BOWERS & WILKINS ==========
      try {
        ...
        if (gamificationData) {
          ...
          // Fondo oscuro/mate (dependiendo del tema) con borde sutil
          // Se usa var(--bg-card) como base, pero añadimos un borde metálico y sombra suave
          const cardStyle = `
          ...
          const badgesHTML = (gamificationData.badges || []).map(badgeId => {
            ...
            if (!badge) return '';
            return `<span class="badge" style="background: rgba(255,255,255,0.05); border: none; color: var(--text-secondary); font-size: 11px; padding: 4px 10px;">${badge.icon} ${badge.name}</span>`;
          }).filter(b => b).join('');
          ...
          html += `
            <div class="gamification-section" style="${cardStyle} text-align: center;">
              <!-- Título: Nivel con color sutil (solo el número) -->
              <div style="margin-bottom: 24px;">
                <div style="font-size: 12px; letter-spacing: 2px; text-transform: uppercase; color: var(--text-secondary); margin-bottom: 8px;">NIVEL</div>
                <div style="font-size: 48px; font-weight: 300; line-height: 1; color: ${levelColor};">${gamificationData.level}</div>
              </div>
              
              <!-- Fila 1: km total y zapatilla -->
              <div style="display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 20px; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom: 16px;">
                <div style="flex: 1; text-align: center;">
                  <div style="font-size: 11px; letter-spacing: 1px; text-transform: uppercase; color: var(--text-secondary);">Distancia</div>
                  <div style="font-size: 20px; font-weight: 300;">${gamificationData.totalDistance.toFixed(1)}<span style="font-size: 12px;"> km</span></div>
                </div>
                <div style="flex: 1; text-align: center;">
                  <div style="font-size: 11px; letter-spacing: 1px; text-transform: uppercase; color: var(--text-secondary);">Zapatilla</div>
                  <div style="font-size: 16px; font-weight: 400;">${Utils.escapeHTML(shoeName)}</div>
                  <div style="font-size: 12px; color: var(--text-secondary);">${shoeKm} km</div>
                </div>
              </div>
              
              <!-- Fila 2: sesiones y XP -->
              <div style="display: flex; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 24px;">
                <div style="flex: 1; text-align: center;">
                  <div style="font-size: 11px; letter-spacing: 1px; text-transform: uppercase; color: var(--text-secondary);">Sesiones</div>
                  <div style="font-size: 20px; font-weight: 300;">${gamificationData.totalSessions}</div>
                </div>
                <div style="flex: 1; text-align: center;">
                  <div style="font-size: 11px; letter-spacing: 1px; text-transform: uppercase; color: var(--text-secondary);">XP</div>
                  <div style="font-size: 20px; font-weight: 300;">${gamificationData.totalXP}</div>
                </div>
              </div>
              
              <!-- Barra de progreso minimalista -->
              <div class="level-progress" style="margin: 8px 0 20px 0; background: rgba(255,255,255,0.1); height: 2px; border-radius: 2px;">
                <div class="level-progress-fill" style="width: ${progress}%; background: ${levelColor}; height: 2px; border-radius: 2px;"></div>
              </div>
              
              <!-- Insignias (en línea, discretas) -->
              ${badgesHTML ? `<div class="badges-container" style="margin: 16px 0; display: flex; flex-wrap: wrap; justify-content: center; gap: 8px;">${badgesHTML}</div>` : '<p style="margin:16px 0; font-size: 11px; color: var(--text-secondary); letter-spacing: 0.5px;">Completa entrenamientos para desbloquear insignias</p>'}
              
              <!-- Botón cambiar zapatilla estilo B&W: borde metálico, texto fino, mayúsculas pequeñas -->
              <button id="changeShoeBtn" style="margin-top: 12px; background: transparent; border: 1px solid rgba(255,255,255,0.2); color: var(--text-primary); padding: 10px 24px; border-radius: 40px; font-size: 12px; letter-spacing: 1.5px; text-transform: uppercase; cursor: pointer; transition: all 0.2s; display: inline-block;">Cambiar zapatilla</button>
            </div>
          `;
        }
