...
const Profile = {
  ...
  // Inicializa el mapa Leaflet y el "tap" de una entrada concreta, SOLO si
  // aún no lo tiene (comprobación _leaflet_id / dataset.bound). Se puede
  // llamar tantas veces como se quiera sobre la misma entrada sin que
  // vuelva a cargar ni parpadear el mapa ya existente.
  ...
  async _actualizarMisEntrenamientos() {
    ...
    if (!wrapper.dataset.inicializado) {
      ...
      ordenNuevo.forEach(entryId => this._inicializarMapaEntrada(entryId, entriesNuevas[entryId]));
      return;
    }
    ...
    nuevas.slice().reverse().forEach(entryId => {
      ...
      this._inicializarMapaEntrada(entryId, entriesNuevas[entryId]);
    });
