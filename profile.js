// ==================== profile.js - TARJETA CON MODAL DE INSIGNIAS (SOLO ICONOS) Y GPS EN ENTRENAMIENTOS ====================
// Versión: 10.3 - Añadido soporte para mostrar mapas en los entrenamientos del perfil
// ====================
...
const Profile = {
  async cargarPerfil(forceRefresh = false) {
    ...
    try {
      ...
      // ========== MIS ÚLTIMOS ENTRENAMIENTOS (CON MAPA Y GPS) ==========
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            // Verificar si la entrada tiene ruta GPS
            ...
            entrenamientosHTML += `
              <div class="mis-entreno-wrapper" data-entry-id="${entryId}" style="border-bottom:1px solid var(--border-color); margin-bottom:8px;">
                ...
                <div class="mis-entreno-detalle" style="display:none; padding:0 0 12px 0;">
                  ...
                  ${tieneGPS ? `
                    <div class="gps-real-data" style="margin-top:8px; margin-bottom:8px; font-size:12px; color:var(--gold);">
                      📊 Datos reales: ${(entry.gpsRoute.distanceMeters / 1000).toFixed(2)} km · Ritmo: ${(entry.gpsRoute.durationSeconds / 60 / (entry.gpsRoute.distanceMeters / 1000)).toFixed(1)} min/km
                    </div>
                    <div id="mapPerfil_${entryId}" class="route-map" style="height: 180px; margin-top: 8px; border-radius: 8px; overflow: hidden;"></div>
                  ` : ''}
      ...
      // Inicializar eventos y mapas
      setTimeout(() => {
        // Eventos de toggle de detalle y likes
        ...
        // Inicializar mapas de GPS en los entrenamientos del perfil
  ...
  _toggleEntrenoDetail(e) {
    ...
    if (detailDiv) {
      ...
      // Si se acaba de mostrar y hay un mapa, forzar actualización de tamaño
      if (!isVisible) {
      ...
      try {
        ...
        if (doc.exists && doc.data().gpsRoute) {
          ...
          if (gpsRoute.points && gpsRoute.points.length >= 2) {
            const map = await GPSTracker.displayRoute(container.id, gpsRoute.points, { showMarkers: true });
            if (map) {
              container._leaflet_map = map;
              // Si el contenedor está oculto inicialmente, invalidateSize cuando se muestre
              const detailDiv = wrapper.querySelector('.mis-entreno-detalle');
              if (detailDiv && detailDiv.style.display !== 'flex') {
                // No hacer nada ahora, se hará en _toggleEntrenoDetail
              }
            }
          }
        }
  ...
  // ========== RESTO DE FUNCIONES (abrirModal, cerrarModal, etc.) SIN CAMBIOS ==========
  abrirModal() {
