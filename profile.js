...
// Versión: 10.3 - Añadido soporte para mapas en últimos entrenamientos
// ====================
...
const Profile = {
  async cargarPerfil(forceRefresh = false) {
    ...
    try {
            ...
            entrenamientosHTML += `
              <div class="mis-entreno-wrapper" data-entry-id="${entryId}" style="border-bottom:1px solid var(--border-color); margin-bottom:8px;">
                ...
                <div class="mis-entreno-detalle" style="display:none; padding:0 0 12px 0; flex-wrap:wrap; align-items:center; justify-content:space-between;">
                  ...
                  ${entry.hasTrack ? `<div id="map-profile-${entryId}" class="track-mini-map" style="height:150px; margin-top:8px; width:100%; border-radius:8px;"></div>` : ''}
                </div>
              </div>
            `;
          }
      ...
      setTimeout(() => {
        ...
        // Cargar mapas para cada entrenamiento con track
        for (const doc of misEntrenamientosSnapshot?.docs || []) {
          const entry = doc.data();
          if (entry.hasTrack && entry.trackId) {
            Profile._cargarMapaTrack(entry.userId, entry.trackId, `map-profile-${doc.id}`);
          }
        }
      }, 0);
  ...
  async _cargarMapaTrack(userId, trackId, containerId) {
    try {
      const trackDoc = await firebaseServices.db.collection('users').doc(userId).collection('tracks').doc(trackId).get();
      if (!trackDoc.exists) return;
      const track = trackDoc.data();
      if (!track.points || track.points.length < 2) return;
      const mapContainer = document.getElementById(containerId);
      if (!mapContainer) return;
      const map = L.map(mapContainer).setView([track.points[0].lat, track.points[0].lng], 13);
      L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OSM</a> & CartoDB'
      }).addTo(map);
      const latlngs = track.points.map(p => [p.lat, p.lng]);
      L.polyline(latlngs, { color: '#c0a060', weight: 4 }).addTo(map);
      L.marker(latlngs[0]).addTo(map).bindPopup('Inicio');
      L.marker(latlngs[latlngs.length-1]).addTo(map).bindPopup('Fin');
      map.invalidateSize();
    } catch (err) {
      console.error('Error cargando mapa del track en perfil:', err);
    }
  },
  ...
  _toggleEntrenoDetail(e) {
    ...
    if (detailDiv) {
      ...
      // Forzar recarga del mapa si es la primera vez que se abre
      if (!isVisible) {
        const mapDiv = detailDiv.querySelector('.track-mini-map');
        if (mapDiv && !mapDiv._mapLoaded) {
          const entryId = wrapper.dataset.entryId;
          // Buscar el track asociado (ya se cargaría al inicio, pero por si acaso)
          // No hacemos nada, ya se habrá cargado antes
        }
      }
    }
  },
