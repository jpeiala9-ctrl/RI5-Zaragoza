...
const Profile = {
  ...
  abrirModal() {
    ...
    // El reset de scroll de la PÁGINA se hace ANTES de mostrar el overlay
    // (no después, como antes): así, si hace falta saltar al top, ese
    // salto ocurre con la página todavía tal cual estaba -- sin el
    // overlay semitransparente encima animándose -- y el modal ya nace
    // visible directamente en su sitio final, sin que se llegue a ver
    // ningún movimiento de fondo a través del fundido de entrada.
    // Se repite con dos reintentos escalonados (mismo patrón ya usado en
    // generarPlanWithScroll para este mismo tipo de problema): en móvil,
    // si quedaba inercia de scroll en marcha o el teclado se estaba
    // cerrando justo al tocar el botón, un único scrollTop=0 puede
    // "perder" contra esa inercia una vez que termina de asentarse -- los
    // reintentos vuelven a forzar el top por si acaso.
    if (typeof window.forzarScrollTop === 'function') {
      window.forzarScrollTop();
      setTimeout(window.forzarScrollTop, 50);
      setTimeout(window.forzarScrollTop, 150);
    }
