...
const Profile = {
  ...
  _renderEntrenamientoItemHTML(entryId, entry) {
    ...
    return `
      <div class="wall-item" data-entry-id="${entryId}" style="margin-bottom:16px;">
        ...
        <div class="wall-stats-box" data-entry-id="${entryId}" style="border:1px solid var(--border-color); border-radius:12px; padding:14px; text-align:center; background:var(--bg-primary); margin-top:4px; cursor:pointer;">
          ...
          <div style="color:var(--text-secondary); font-size:12px; display:flex; justify-content:center; align-items:center; gap:12px; margin-top:4px;">
            ${zone ? `<span style="color:${colorZona}; font-weight:500;">🔥 predomina ${Utils.escapeHTML(zone)}</span>` : ''}
            ${hora ? `<span>🕒 ${hora}</span>` : ''}
          </div>
          ${entry.desglose ? `<div style="margin-top:6px; font-size:9px; color:var(--text-secondary); opacity:0.6;">▾ toca para ver el desglose por tramos</div>` : ''}
          ${miniMapHTML}
        </div>
      </div>
    `;
  ...
  async _actualizarMisEntrenamientos() {
    ...
    // Caché de TODAS las entradas (con o sin GPS), para poder abrir el
    // detalle real de la sesión (calentamiento/parte principal/
    // enfriamiento) al pulsar su recuadro de estadísticas -- _gpsEntries
    // de arriba solo guarda las que tienen track GPS.
    this._entradasCache = entriesNuevas;
  ...
  _handleItemClickProfile(item, e) {
    ...
    // Igual que en el Muro (ver wall.js::_handleItemClick): pulsar el
    // recuadro de estadísticas abre el detalle real de la sesión (con su
    // desglose calentamiento/parte principal/enfriamiento si lo tiene) en
    // vez de la lista de "me gusta".
    const statsBox = e.target.closest('.wall-stats-box');
    if (statsBox) {
      const entry = (this._entradasCache || {})[entryId];
      if (entry && window.abrirDetalleEntrenamientoCompletado) {
        window.abrirDetalleEntrenamientoCompletado(entry);
        return;
      }
    }
