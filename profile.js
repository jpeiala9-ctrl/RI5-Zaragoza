// ==================== profile.js - Gestión de perfil de usuario (CON FOTO Y SEGURIDAD XSS) ====================
// Versión: 3.6 - CORREGIDO: ELIMINADO friendsCount, SIEMPRE se usa friendIds.length
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    try {
      ...
      // ========== SOLUCIÓN RADICAL ==========
      // IGNORAR COMPLETAMENTE friendsCount
      // SIEMPRE usar friendIds.length como fuente de verdad
      const friendIds = userData.friendIds || [];
      const numeroRealDeAmigos = friendIds.length;
      
      // Actualizar el estado global con el valor correcto
      if (AppState.currentUserData) {
        AppState.currentUserData.friendIds = friendIds;
        AppState.currentUserData.friendsCount = numeroRealDeAmigos;
      }
      ...
      // ========== MOSTRAR SIEMPRE friendIds.length ==========
      let html = `
        <div class="perfil-header">
          ...
          <div class="perfil-info">
            ...
            <div class="perfil-stats">
              <div class="perfil-stat"><span id="perfilAmigosCount">${numeroRealDeAmigos}</span><label>Amigos</label></div>
      ...
      console.log(`📊 [Profile] Amigos mostrados: ${numeroRealDeAmigos} (de ${friendIds.length} IDs)`);
  ...
  // ========== MÉTODO PARA ACTUALIZAR SOLO EL CONTADOR DE AMIGOS ==========
  async actualizarContadorAmigos() {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        // Actualizar el DOM si el elemento existe
        const contadorSpan = document.getElementById('perfilAmigosCount');
        if (contadorSpan) {
          contadorSpan.textContent = numeroRealDeAmigos;
          console.log(`📊 [Profile] Contador actualizado en DOM: ${numeroRealDeAmigos}`);
        }
        
        return numeroRealDeAmigos;
      }
    } catch (error) {
      console.error('Error actualizando contador de amigos:', error);
    }
    return 0;
  },
