// ==================== profile.js ====================
// Versión: 11.5 - CORREGIDO: 
//                 - Mini mapas con Leaflet (muestran el track sobre el mapa)
//                 - Modal de likes: "Ver perfil" ya no cierra el modal
//                 - Mini mapa: stopPropagation para evitar doble ejecución
//                 - Prevención de toasts duplicados
//                 - ORDEN DE EVENTOS CORREGIDO: el mapa se detecta PRIMERO
//                 - Un solo clic en el mapa = una sola apertura
// ====================
...
const Profile = {
  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    try {
      ...
      // CONSTRUIR HTML - EXACTAMENTE IGUAL AL ORIGINAL
      ...
      // Limpiar mini mapas anteriores
      this._miniMaps.forEach(map => { try { map.remove(); } catch(e) {} });
      this._miniMaps = [];
      this._miniMapEntries = {};
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
              ...
              const mapId = `miniMapPerfil_${entryId}_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`;
              miniMapHTML = `
                <div class="gps-minimap-tap-profile" data-entry-id="${entryId}"
                  style="margin-top:10px;border-radius:10px;overflow:hidden; border:1px solid #2a2a2a; cursor:pointer; position:relative; height:130px; background:#1a1a1a;">
              ...
              // Guardar para inicializar el mapa después
              this._miniMapEntries[entryId] = { entry, mapId, trackPoints: entry.trackPoints };
            }
      ...
      // ================================================================
      //  INICIALIZAR MINI MAPAS CON LEAFLET
      // ================================================================
      if (this._miniMapEntries && Object.keys(this._miniMapEntries).length > 0) {
        // Esperar un poco para que el DOM se haya actualizado
        setTimeout(async () => {
          for (const [entryId, data] of Object.entries(this._miniMapEntries)) {
            const container = document.getElementById(data.mapId);
            if (container && data.trackPoints && data.trackPoints.length >= 2) {
              await this._crearMiniMapaPerfil(container, data.trackPoints);
            }
          }
        }, 200);
      }
      ...
      // ================================================================
      //  CONFIGURAR EVENTOS - UN SOLO DELEGADO
      //  EL ORDEN ES CRÍTICO: EL MAPA SE DETECTA PRIMERO
      // ================================================================
  ...
  // ================================================================
  //  CREAR MINI MAPA CON LEAFLET (igual que en wall.js)
  // ================================================================
  async _crearMiniMapaPerfil(container, trackPoints) {
    if (!window.L) {
    ...
    try {
      ...
      this._miniMaps.push(map);

      // Al hacer clic en el contenedor, abrir el visor completo
      container.style.cursor = 'pointer';
      // Quitar listener antiguo y añadir uno nuevo
      if (this._miniMapClickHandler) {
        container.removeEventListener('click', this._miniMapClickHandler);
      }
      this._miniMapClickHandler = (e) => {
        e.stopPropagation();
        const entryId = container.closest('.gps-minimap-tap-profile')?.dataset.entryId;
        if (entryId && this._gpsEntries[entryId]) {
          if (window.GPSTrackViewer) GPSTrackViewer.open(this._gpsEntries[entryId]);
        }
      };
      container.addEventListener('click', this._miniMapClickHandler);

    } catch(e) {
      console.warn('Error creando mini mapa en perfil:', e);
    }
  },
  ...
  //  ORDEN CORREGIDO: MAPA PRIMERO, LUEGO LIKE, LUEGO BOTONES, LUEGO TARJETA
  // ================================================================
  _handleContainerClick(e) {
    const target = e.target;
    
    // --- 1. PRIMERO: MINI MAPA (el elemento más específico) ---
    // Si el clic es en el mapa, lo abrimos y SALIMOS INMEDIATAMENTE
    // Esto evita que la tarjeta .wall-item también capture el evento
    ...
    if (mapElement) {
      ...
      return; // SALIR AQUÍ - no se ejecuta NADA más
    }
    ...
    // Solo llegamos aquí si NO se detectó el mapa, ni like, ni botones
  ...
  // ================================================================
  //  MOSTRAR LIKES - CON PREVENCIÓN DE TOASTS DUPLICADOS
  // ================================================================
  async _mostrarLikesDeEntrenamiento(entryId) {
    if (!entryId) return;
    
    if (this._executingLike) {
      console.log('⏳ Ejecución de likes en curso, ignorando...');
      return;
    }
  ...
  // ================================================================
  //  CREAR MODAL DE LIKES - CORREGIDO: "Ver perfil" NO cierra el modal
  // ================================================================
  _crearModalLikes(users) {
    ...
    // Cerrar modal de likes (solo con la X o clic fuera)
    ...
    // --- Clic en el item del usuario (abre perfil, NO cierra modal de likes) ---
    ...
    // --- Botón "Ver perfil" (abre perfil, NO cierra modal de likes) ---
  ...
  // ================================================================
  //  MODAL HISTORIAL ZAPATILLAS - CON PREVENCIÓN DE TOASTS DUPLICADOS
  // ================================================================
  async _mostrarModalHistorial() {
    if (this._executingHistory) {
      console.log('⏳ Ejecución de historial en curso, ignorando...');
      return;
    }
...
console.log('✅ profile.js v11.5 - ORDEN DE EVENTOS CORREGIDO: mapa se detecta PRIMERO, un clic = una apertura');