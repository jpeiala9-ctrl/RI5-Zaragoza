// ==================== profile.js ====================
// Versión: 10.13 - DEDUPLICACIÓN DE LECTURAS DE FIRESTORE (sin cambios de
//                comportamiento): (1) cargarPerfil() ya no llama a
//                Gamification.getCurrentShoe(uid) -- que releía el
//                documento de gamification completo solo para sacar
//                currentShoe -- y reutiliza en su lugar gamificationData,
//                que ya se había leído unas líneas antes. (2) la
//                comprobación de "amigos huérfanos" (consulta 'in' por
//                chunks sobre 'users') ya no se repite aquí con su propio
//                bucle: se delega en Friends._limpiarAmigosHuérfanos, que
//                además deduplica la promesa en vuelo por uid, así que si
//                Friends.cargarListaAmigos()/cargarPanelAlumnos() ya están
//                haciendo la misma comprobación en paralelo (típico
//                durante el login, ver AppState.precargarDatos), esta
//                llamada reutiliza esa misma consulta en vez de repetirla.
...
const Profile = {
  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    try {
      ...
      // 🔥 FIX lectura duplicada: antes esta comprobación de "amigos
      // huérfanos" se repetía aquí con su propio bucle de consultas 'in'
      // por chunks -- exactamente la misma lógica que ya existía en
      // Friends._limpiarAmigosHuérfanos, y ambas podían lanzarse EN
      // PARALELO durante el arranque (Profile.cargarPerfil() y
      // Friends.cargarListaAmigos()/cargarPanelAlumnos(), ver
      // AppState.precargarDatos), duplicando las mismas lecturas de
      // 'users'. Ahora se delega en Friends, que además deduplica la
      // promesa en vuelo por uid (mismo patrón que
      // SessionInvites._cargarHistorialSiHaceFalta): si esa comprobación
      // ya está en curso desde otro sitio, esta llamada reutiliza esa
      // misma consulta en vez de repetirla. Mismo comportamiento de
      // fallback en caso de error (se deja friendIds tal cual).
      const { validIds: amigosValidos, changed: amigosCambiaron } = await Friends._limpiarAmigosHuérfanos(AppState.currentUserId, friendIds);
      if (amigosCambiaron) {
      ...
      let html = `
        <div class="perfil-header">
          ...
          <div class="perfil-info">
            <div class="perfil-nombre">${Utils.escapeHTML(Utils.capitalizeUsername(userData.username))}${userData.isTrainer ? ' <span class="badge-entrenador">Entrenador</span>' : ''}</div>
            ...
            <div class="perfil-stats">
              ...
              <div class="perfil-stat"${userData.premium ? '' : ' onclick="showPremiumBenefits()" role="button" tabindex="0" style="cursor:pointer;"'}><span>${userData.premium ? 'PREMIUM' : 'STANDARD'}</span><label>Plan</label></div>
            </div>
          </div>
        </div>
      ...
      if (gamificationData) {
        ...
        // 🔥 FIX lectura duplicada: se reutiliza gamificationData.currentShoe
        // (ya disponible tras el Gamification.getData(...) de más arriba,
        // incluida su posible actualización tras la reparación automática)
        // en vez de llamar a Gamification.getCurrentShoe(uid), que
        // internamente vuelve a leer el documento COMPLETO de gamification
        // solo para sacar este campo.
  ...
  _renderEntrenamientoItemHTML(entryId, entry) {
    ...
    // 🔥 FIX: en el Muro, un "me gusta" sin leer se marca con un puntito
    // rojo (.like-new-dot) sobre el botón de like -- aquí, en "Mis
    // últimos entrenamientos" (Perfil), nunca se calculaba ni se pintaba,
    // así que aunque llegaras a la tarjeta correcta (por la notificación
    // de Comunidad o navegando a mano) no había ninguna marca visual que
    // dijera "este es el que tiene el like nuevo". Mismo cálculo que en
    // wall.js: likeCount - likesLeidos.
    const likesNuevos = Math.max(0, likeCount - (Number(entry.likesLeidos) || 0));
      ...
      <div class="wall-item" data-entry-id="${entryId}" style="margin-bottom:16px;">
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
          ...
          <div style="display:flex; align-items:center; gap:4px;">
            <button class="wall-like-btn" data-entry-id="${entryId}" style="position:relative; background:transparent; border:none; padding:6px 12px; border-radius:14px; cursor:pointer; display:inline-flex; align-items:center; gap:6px; font-size:14px; color:var(--text-secondary); transition:all 0.2s ease;">
  ...
  async _actualizarMisEntrenamientos() {
    ...
    ordenNuevo.forEach(entryId => {
      ...
      // 🔥 Mismo puntito rojo que en la creación de la tarjeta (ver
      // _renderEntrenamientoItemHTML), pero recalculado en caliente: si
      // llega un like nuevo mientras la pantalla ya está abierta, aparece
      // sin necesidad de recargar; si se marca como leído (p.ej. al abrir
      // "quién dio like" desde el Muro), desaparece igual de al momento.
      const likeBtn = el.querySelector('.wall-like-btn');
      if (likeBtn) {
        const likesNuevosAhora = Math.max(0, (Number(entry.likeCount) || 0) - (Number(entry.likesLeidos) || 0));
        let dot = likeBtn.querySelector('.like-new-dot');
        if (likesNuevosAhora > 0 && !dot) {
          likeBtn.insertAdjacentHTML('beforeend', `<span class="like-new-dot" style="position:absolute; top:2px; right:2px; width:9px; height:9px; border-radius:50%; background:#e74c3c; border:2px solid var(--bg-secondary);"></span>`);
        } else if (likesNuevosAhora <= 0 && dot) {
          dot.remove();
        }
      }
    });
...
console.log('✅ profile.js v10.13 - Lecturas de gamification/amigos huérfanos deduplicadas en cargarPerfil');