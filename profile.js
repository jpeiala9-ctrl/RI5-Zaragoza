...
const Profile = {
  ...
  _renderEntrenamientoItemHTML(entryId, entry) {
    ...
    return `
      <div class="wall-item" data-entry-id="${entryId}" style="margin-bottom:16px;">
        ...
        <div class="wall-stats-box" style="border:1px solid var(--border-color); border-radius:12px; padding:14px; text-align:center; background:var(--bg-primary); margin-top:4px; cursor:pointer;">
  ...
  _handleItemClickProfile(item, e) {
    ...
    if (!entryId) return;
    // Tocar la caja de estadísticas (tipo/tiempo/km/zona) abre el detalle
    // de la sesión -- calentamiento/parte principal/enfriamiento, cada uno
    // con su propia zona real (ver _desglosarSesionReal en calendar.js) --
    // en vez de la lista de "me gusta", que sigue siendo lo que abre el
    // resto de la tarjeta.
    if (e.target.closest('.wall-stats-box')) {
      this._mostrarDetalleEntrenamiento(entryId);
      return;
    }
    this._mostrarLikesDeEntrenamiento(entryId);
  },

  async _mostrarDetalleEntrenamiento(entryId) {
    if (!entryId) return;
    try {
      const doc = await firebaseServices.db.collection('globalFeed').doc(entryId).get();
      if (!doc.exists) { Utils.showToast('El entrenamiento ya no existe', 'error'); return; }
      if (window.construirSesionObjDesdeEntry && window.abrirModalDetalleSesion) {
        window.abrirModalDetalleSesion(window.construirSesionObjDesdeEntry(doc.data()), true);
      }
    } catch (error) {
      console.error('Error mostrando detalle de entrenamiento:', error);
