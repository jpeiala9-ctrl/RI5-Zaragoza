// ==================== profile.js - Perfil con tarjetas estilo muro y ortofoto IGN ====================
// Versión: 11.1 - Mapas visibles en perfil, GPS mejorado en calendario
// ====================
...
const Profile = {
  ...
  // ========== FUNCIÓN CORREGIDA PARA INICIALIZAR MAPAS (SE VE EL MAPA) ==========
  async _initProfileMaps() {
    // Asegurar que Leaflet está cargado
    if (typeof GPSTracker !== 'undefined' && GPSTracker.loadLeaflet) {
      await GPSTracker.loadLeaflet();
    } else {
      console.warn('GPSTracker no disponible');
      return;
    }
      ...
      try {
        ...
        if (doc.exists && doc.data().gpsRoute) {
          ...
          if (gpsRoute.points && gpsRoute.points.length >= 2) {
            // Forzar altura y limpiar mapa anterior
