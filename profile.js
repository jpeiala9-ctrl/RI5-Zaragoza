// ==================== profile.js ====================
// Versión: 15.0 - USA wall.js PARA LOS MAPAS
//                 - NO hay código de mapas en profile.js
//                 - Reutiliza la función de wall.js que funciona
// ====================
...
const Profile = {
  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    try {
      ...
      try {
        ...
        gamificationData = Gamification.getDefaultData();
        await firebaseServices.db.collection('gamification').doc(AppState.currentUserId).set(gamificationData).catch(() => {});
      }
      ...
      //  - SOLO HTML, SIN LÓGICA DE MAPAS
      //  - Los mapas los crea wall.js después
      // ================================================================
      ...
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            // ============================================================
            //  MINI MAPA - SOLO EL CONTENEDOR
            //  - El mapa lo creará wall.js después
            // ============================================================
            let miniMapHTML = '';
      ...
      // ================================================================
      //  CREAR MAPAS USANDO LA FUNCIÓN DE WALL.JS
      //  - wall.js ya tiene la función _crearMiniMapa que funciona
      // ================================================================
      setTimeout(() => {
        for (const [entryId, entryData] of Object.entries(this._gpsEntries)) {
          ...
          if (mapContainer && entryData.trackPoints && entryData.trackPoints.length >= 2) {
            // USAR LA FUNCIÓN DE WALL.JS si existe
            if (window.Wall && typeof Wall._crearMiniMapa === 'function') {
              Wall._crearMiniMapa(mapContainer, entryData.trackPoints);
            } else {
              // Fallback: crear el mapa manualmente
              this._crearMiniMapaFallback(mapContainer, entryData.trackPoints);
            }
          }
        }
      }, 200);
  ...
  // ================================================================
  //  FALLBACK - COPIA EXACTA DE WALL.JS _crearMiniMapa
  //  SOLO se usa si Wall no está disponible
  // ================================================================
  async _crearMiniMapaFallback(container, trackPoints) {
    if (!window.L) {
  ...
  async _mostrarLikesDeEntrenamiento(entryId) {
    if (!entryId || this._executingLike) return;
  ...
  async _mostrarModalHistorial() {
    if (this._executingHistory) return;
...
console.log('✅ profile.js v15.0 - USA wall.js PARA LOS MAPAS');