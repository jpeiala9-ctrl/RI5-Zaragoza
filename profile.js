  ...
  async cargarPerfil(forceRefresh = true) {
    ...
    try {
          ...
          <div class="passport-card" style="margin-top:24px; border:2px solid ${levelColor}; border-radius:24px; background:${bgColor}; box-shadow:0 8px 20px rgba(0,0,0,0.1); overflow:hidden;">
            ...
            <div style="padding:16px 20px;">
              ...
              <div style="display: flex; justify-content: center; gap: 12px; margin-top:8px; flex-wrap: wrap;">
                ...
                <button id="repairShoeKmBtn" title="Recalcula los km de la zapatilla actual desde tu historial real de sesiones" style="background:transparent; border:1px solid ${levelColor}; color:${levelColor}; padding:2px 10px; border-radius:14px; font-size:10px; letter-spacing:0.5px; cursor:pointer;">🔧 Recalcular km</button>
              </div>
            </div>
          </div>
        `;
      ...
      setTimeout(() => {
        ...
        // 🔥 v10.14: botón manual para recalcular YA (sin esperar a la
        // próxima sesión marcada/desmarcada) los km de la zapatilla actual
        // desde el historial real -- ver Gamification.repararKmZapatilla
        // (gamification.js v5.19). Útil para quien ya tuviera la zapatilla
        // desviada de antes de ese fix.
        const repairShoeBtn = document.getElementById('repairShoeKmBtn');
        if (repairShoeBtn) repairShoeBtn.onclick = async () => {
          repairShoeBtn.disabled = true;
          const textoOriginal = repairShoeBtn.textContent;
          repairShoeBtn.textContent = '🔧 Recalculando…';
          try {
            const nuevoKm = await Gamification.repararKmZapatilla(AppState.currentUserId);
            if (nuevoKm !== null) await this.cargarPerfil(true);
          } finally {
            repairShoeBtn.disabled = false;
            repairShoeBtn.textContent = textoOriginal;
          }
        };
      }, 0);
