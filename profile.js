...
const Profile = {
  ...
  async cargarPerfil(forceRefresh = false) {
    ...
    try {
      ...
      // ========== MIS ÚLTIMOS ENTRENAMIENTOS (con minimapa y visor) ==========
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            const gpsBadge = entry.hasGPS ? `<span style="font-size:9px;color:#1e88e5;background:rgba(30,136,229,0.12);border:1px solid rgba(30,136,229,0.3);border-radius:20px;padding:2px 7px;margin-left:6px;letter-spacing:1px;">GPS</span>` : '';
            let gpsMapBlock = '';
      ...
      setTimeout(() => {
        ...
        document.querySelectorAll('.gps-minimap-tap-profile').forEach(el => {
          el.addEventListener('click', (e) => {
            ...
            else Utils.showToast('No se encontraron datos del track', 'error');
          });
        });
