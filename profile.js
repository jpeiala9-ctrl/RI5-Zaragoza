// ==================== profile.js - TARJETA COMPACTA CON NIVEL MÁS PEQUEÑO Y ALINEACIÓN CORREGIDA ====================
// Versión: 7.1 - Nivel reducido a 36px, padding reducido, números de datos más pequeños.
// ====================
    ...
    try {
      ...
      try {
        ...
        if (gamificationData) {
          ...
          html += `
            <div class="gamification-section" style="margin-top:24px; padding:16px; background: ${bgColor}; border-radius:20px; text-align:center;">
              <!-- Título: Nivel (más pequeño) -->
              <div style="margin-bottom:12px;">
                <div style="font-size:10px; letter-spacing:1.5px; text-transform:uppercase; color:var(--text-secondary);">NIVEL</div>
                <strong style="font-size:36px; font-weight:300; color:${levelColor};">${gamificationData.level}</strong>
              </div>
              
              <!-- Barra de progreso (más fina) -->
              <div class="level-progress" style="margin:0 0 16px 0; background:var(--border-color); height:2px; border-radius:2px;">
                <div class="level-progress-fill" style="width: ${progress}%; background: ${levelColor}; height:2px; border-radius:2px;"></div>
              </div>
              
              <!-- Fila 1: Total km y Zapatilla -->
              <div style="display: flex; justify-content: space-between; align-items: center; gap: 12px; margin-bottom: 12px;">
                <div style="flex: 1; text-align: center;">
                  <div style="font-size:10px; opacity:0.7;">📏 Total km</div>
                  <strong style="font-size:16px;">${gamificationData.totalDistance.toFixed(1)}</strong>
                  <span style="font-size:11px;"> km</span>
                </div>
                <div style="flex: 1; text-align: center;">
                  <div style="font-size:10px; opacity:0.7;">👟 Zapatilla</div>
                  <strong style="font-size:14px;">${Utils.escapeHTML(shoeName)}</strong>
                  <div style="font-size:11px; opacity:0.7;">${shoeKm} km</div>
                </div>
              </div>
              ...
              <div style="display: flex; justify-content: space-between; align-items: center; gap: 12px; margin-bottom: 16px;">
                <div style="flex: 1; text-align: center;">
                  <div style="font-size:10px; opacity:0.7;">🎯 Sesiones</div>
              ...
              <!-- Insignias -->
              ${badgesHTML ? `<div class="badges-container" style="margin-bottom: 12px;">${badgesHTML}</div>` : '<p style="margin-bottom:12px; font-size:11px;">Completa entrenamientos para desbloquear insignias</p>'}
              
              <!-- Botones (compactos) -->
              <div style="display: flex; justify-content: center; gap: 10px;">
                <button id="changeShoeBtn" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:5px 14px; border-radius:30px; font-size:11px; letter-spacing:0.5px; cursor:pointer;">👟 Cambiar</button>
                <button id="historyShoeBtn" style="background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:5px 14px; border-radius:30px; font-size:11px; letter-spacing:0.5px; cursor:pointer;">📜 Historial</button>
              </div>
            </div>
          `;
        }
