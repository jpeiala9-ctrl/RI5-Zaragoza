// ==================== profile.js - PERFIL CON ZAPATILLA Y TARJETA DE NIVEL CON COLOR DE FONDO ====================
// Versión: 6.3 - Reordenada la tarjeta de gamificación y fondo sutil según nivel.
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    try {
      ...
      // ========== GAMIFICACIÓN: REORDENADA + FONDO SEGÚN NIVEL ==========
      try {
        ...
        if (gamificationData) {
          ...
          // Convertir color a rgba con opacidad 0.15 para fondo sutil
          let bgColor = levelColor;
          // Si es hex, convertirlo a rgb
          if (levelColor.startsWith('#')) {
            ...
            bgColor = levelColor.replace('rgb', 'rgba').replace(')', ', 0.15)');
          } else {
            bgColor = 'var(--bg-secondary)'; // fallback
          }
          ...
          html += `
            <div class="gamification-section" style="margin-top:24px; padding:16px; background: ${bgColor}; border-radius:16px; transition: background 0.3s;">
              <!-- Primera fila: Nivel + Zapatilla + Sesiones/XP -->
              <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; margin-bottom: 16px;">
                ...
                <div style="flex: 1; min-width: 100px; text-align: center;">
                  ...
                  <div style="font-size: 12px;">📊 ${shoeKm} km</div>
                </div>
                <div style="flex: 1; min-width: 100px; text-align: center;">
                  <strong>🎯 ${gamificationData.totalSessions}</strong>
                  <div style="font-size: 12px;">✨ ${gamificationData.totalXP} XP</div>
                </div>
              </div>
              <!-- Barra de progreso -->
              <div class="level-progress" style="margin-top: 8px;">
                <div class="level-progress-fill" style="width: ${progress}%; background: ${levelColor}; height: 8px; border-radius: 4px;"></div>
              </div>
              <!-- Insignias -->
              ${badgesHTML ? `<div class="badges-container" style="margin-top: 12px;">${badgesHTML}</div>` : '<p style="margin-top:12px; font-size:12px; text-align:center;">Completa entrenamientos para desbloquear insignias</p>'}
              <!-- Botón cambiar zapatilla -->
              <button id="changeShoeBtn" style="margin-top:12px; background:transparent; border:1px solid var(--border-color); color:var(--text-primary); padding:6px 12px; border-radius:20px; font-size:12px; cursor:pointer; width:100%;">👟 Cambiar zapatilla</button>
            </div>
          `;
        }
