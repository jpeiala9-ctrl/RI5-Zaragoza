// ==================== profile.js - PERFIL CON CACHE Y OPTIMIZACIÓN ====================
// Versión: 5.4 - Caché local + limpieza de amigos por lotes (carga <200ms)
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    // Mostrar caché inmediatamente mientras se actualiza en background
    if (cached) {
      try {
        const { html, timestamp } = JSON.parse(cached);
        if (Date.now() - timestamp < 60 * 1000) { // 1 minuto de validez
    ...
    try {
      ...
      // ========== LIMPIEZA DE AMIGOS HUÉRFANOS OPTIMIZADA (LOTES) ==========
          ...
          html += `
            <div class="gamification-section" style="margin-top:24px; padding:16px; background:var(--bg-secondary); border-radius:16px;">
              <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
                ...
                <div><strong>🔥 Racha actual: ${gamificationData.currentStreak} días</strong><br><span style="font-size:12px;">🏆 Racha máxima: ${gamificationData.longestStreak}</span></div>
      ...
      // Mis últimos entrenamientos (colapsable + modal de likes)
      try {
