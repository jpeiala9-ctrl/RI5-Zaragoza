    ...
    try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
              ...
              miniMapHTML = `
                <div style="margin-top:10px; border-radius:10px; overflow:hidden; border:1px solid #2a2a2a; position:relative;">
                  <div id="${mapId}" class="gps-minimap-leaflet-profile" data-entry-id="${entryId}"
                    style="cursor:pointer; height:130px; width:100%; background:#1a1a1a;">
                  </div>
      ...
      setTimeout(() => {
        ...
        Object.keys(this._gpsEntries || {}).forEach(entryId => {
          ...
          if (!mapContainer || mapContainer._leaflet_id) return; // ya inicializado, no se toca
          // El clic se engancha YA, sin esperar a que termine de crearse el
          // mapa (mismo motivo que en Wall: si tocabas antes de que Leaflet
          // acabara de cargar, ese primer toque no hacía nada).
