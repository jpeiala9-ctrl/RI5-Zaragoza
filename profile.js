...
const Profile = {
  ...
  abrirModal() {
    ...
    // NOTA: aquí NO se llama a forzarScrollTop(). El modal es position:fixed
    // y se centra en el viewport (top:50%/translate), así que su posición
    // en pantalla no depende en nada de cuánto scroll tenga la página de
    // detrás -- se "abre arriba" igual siempre. Forzar el scroll de la
    // PÁGINA aquí solo producía un salto visible si el usuario había
    // bajado a ver sus estadísticas antes de tocar "Editar perfil". La
    // limpieza del desplazamiento que deja el teclado virtual (al escribir
    // en un campo bajo del formulario) ya se hace en cerrarModal(), que es
    // el momento correcto: con el modal todavía delante, sin que se note.
  ...
  cerrarModal() {
    const overlay = document.getElementById('modalEditarPerfilOverlay');
    const modal = document.getElementById('modalEditarPerfil');
    // Todo el reset de scroll (el del propio modal Y el de la página) se
    // hace AQUÍ, con el overlay y el modal todavía visibles (display:block)
    // -- un elemento oculto no tiene caja de scroll real y, si el reset de
    // página se hiciera después de ocultarlos, el salto se vería sobre la
    // página ya descubierta (el mismo problema que había al abrir). Así,
    // el salto queda completamente tapado por el propio modal mientras
    // aún está delante, y lo único que el usuario ve es que se cierra.
