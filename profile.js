...
// Versión: 10.0 - Las insignias muestran solo el icono; al clic, modal con todas las insignias.
// ====================
...
const Profile = {
    ...
    try {
      ...
      try {
        ...
        if (gamificationData) {
          ...
          // INSIGNIAS: SOLO EL ICONO (sin texto)
      ...
      setTimeout(() => {
        ...
        // Evento para insignias: abrir modal con todas las insignias
  ...
  // ========== MODAL CON TODAS LAS INSIGNIAS ==========
  async _mostrarModalInsignias() {
    const gamificationData = await Gamification.getData(AppState.currentUserId);
    if (!gamificationData) return;
    
    const earnedBadges = gamificationData.badges || [];
    // Todas las insignias posibles (ordenadas manualmente por relevancia)
  ...
  // ========== MODAL PARA HISTORIAL DE ZAPATILLAS ==========
