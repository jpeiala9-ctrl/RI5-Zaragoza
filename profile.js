// ==================== profile.js ====================
// Versión: 10.9 - Arreglado el minimapa Leaflet real de "Mis últimos
//                entrenamientos": salía partido/medio en blanco porque,
//                si cargarPerfil() se disparaba dos veces seguidas, dos
//                llamadas concurrentes podían montar DOS mapas Leaflet
//                sobre el mismo contenedor a la vez, corrompiendo su
//                tamaño interno. Añadida una reserva síncrona
//                (_mapasEnCreacion) que bloquea la segunda llamada desde
//                el primer instante, más una comprobación extra de
//                container._leaflet_id como red de seguridad.
// Versión: 10.8 - "Mis últimos entrenamientos": el minimapa vuelve a ser
//                un mapa Leaflet real (teselas de calle), no solo el
//                trazo del track. Se pinta el SVG instantáneo como antes
//                mientras Leaflet carga, y el mapa real queda cacheado en
//                memoria por entrada: no se recarga hasta que 5 sesiones
//                más nuevas la saquen del top-5.
// ====================

const Profile = {
  _gpsEntries: {},
  _mapasEntradas: {}, // entryId -> instancia de mapa Leaflet ya creada (caché en memoria)
  _mapasEnCreacion: new Set(), // entryId -> reserva síncrona mientras se crea (evita dobles L.map() sobre el mismo div)
  ...
  _renderEntrenamientoItemHTML(entryId, entry) {
    ...
    if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
      ...
      // Miniatura instantánea (solo el trazo del track, sin teselas de
      // mapa real): se lee DIRECTAMENTE de localStorage si ya existía, o
      // se genera aquí mismo al vuelo -- así la tarjeta nunca se ve vacía
      // mientras Leaflet carga en segundo plano. En cuanto el mini-mapa
      // real (con teselas de calle) esté listo, _crearMiniMapaEntrada()
      // lo sustituye dentro de este mismo contenedor (mapId). Una vez
      // creado, el mapa queda cacheado en memoria (this._mapasEntradas) y
      // no se vuelve a recrear hasta que la propia entrada desaparezca
      // del top-5 (ver _actualizarMisEntrenamientos).
      let svgRuta = null;
      ...
      miniMapHTML = `
        <div style="margin-top:10px; border-radius:10px; overflow:hidden; border:1px solid var(--border-color); position:relative; height:130px;">
          <div id="${mapId}" class="gps-minimap-leaflet" data-entry-id="${entryId}" style="height:100%; width:100%; background:var(--stat-bg);">${svgRuta || ''}</div>
  ...
  _vincularToqueEntrada(entryId, entry) {
    ...
    if (tapOverlay && !tapOverlay.dataset.bound) {
      ...
      // Antes usaba Utils.bindTap, que en su touchend llama a
      // e.preventDefault(); en móvil eso deja "pegado" el :active de
      // .wall-item (scale 0.96) justo antes de abrir el modal, dando la
      // sensación de que la tarjeta se encoge y rebota. Con un click
      // normal + stopPropagation() se abre igual de directo que el resto
      // de la tarjeta (like, borrar, etc.), sin encogimiento ni rebote.
  ...
  // Carga Leaflet una sola vez (si el muro ya lo cargó antes, window.L ya
  // existe y esto no vuelve a pedir nada por red).
  _cargarLeaflet() {
  ...
  // Crea el mini-mapa Leaflet REAL (con teselas de calle) de una entrada,
  // sustituyendo dentro de su mismo contenedor al SVG instantáneo del
  // track. Si la entrada ya tiene un mapa vivo (visita anterior a la
  // pestaña Perfil, entrada que sigue en el top-5), no hace nada: esa es
  // la caché en memoria que hace que el mapa no se recargue hasta que la
  // propia tarjeta desaparezca de la lista.
  async _crearMiniMapaEntrada(entryId, trackPoints) {
    if (this._mapasEntradas[entryId]) return;
    // Reserva SÍNCRONA (antes de cualquier await): si cargarPerfil() se
    // dispara dos veces seguidas (cambio rápido de pestaña, refresco,
    // etc.), la segunda llamada a esta función para la misma entrada
    // queda bloqueada aquí mismo. Sin esto, las dos llamadas pasan de
    // largo el chequeo de _mapasEntradas (que solo se rellena al FINAL,
    // ya con el mapa creado) y acaban montando DOS L.map() sobre el
    // mismo div al mismo tiempo -- eso es justo lo que corrompe el
    // tamaño interno de Leaflet y deja el mapa partido, medio en blanco.
    if (this._mapasEnCreacion.has(entryId)) return;
    this._mapasEnCreacion.add(entryId);

    try {
      await this._cargarLeaflet();
      if (!window.L) return;
      if (this._mapasEntradas[entryId]) return; // pudo crearse mientras esperábamos

      const containerId = `miniMapProfile_${entryId}`;
      const container = document.getElementById(containerId);
      if (!container) return;
      // Defensa extra: si el contenedor ya tiene un mapa Leaflet colgado
      // (container._leaflet_id), no crear otro encima.
      if (container._leaflet_id) return;

      // Doble rAF: igual que en el muro, para que Leaflet mida el
      // contenedor ya con su tamaño final y no pinte "medio mapa negro".
      await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
      if (!document.body.contains(container)) return;
      if (this._mapasEntradas[entryId] || container._leaflet_id) return;
      ...
      // Mismo tile layer que el muro y el visor de rutas (CartoDB).
      ...
      // Segunda pasada de seguridad por si el tamaño se asienta un poco
      // más tarde (dispositivos lentos, imágenes cargando debajo, etc.).
      setTimeout(() => {
        if (!document.body.contains(container)) return;
        map.invalidateSize();
        map.fitBounds(polyline.getBounds(), { padding: [10, 10] });
      ...
      this._mapasEntradas[entryId] = map;
    } finally {
      this._mapasEnCreacion.delete(entryId);
    }
  },

  // Crea los mapas que falten (entradas nuevas del top-5). Las que ya
  // tienen mapa vivo se saltan automáticamente dentro de
  // _crearMiniMapaEntrada.
  _crearMapasFaltantes(ordenNuevo, entriesNuevas) {
    ordenNuevo.forEach(entryId => {
      const entry = entriesNuevas[entryId];
      if (entry && entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
        this._crearMiniMapaEntrada(entryId, entry.trackPoints);
      }
    });
  },

  // Destruye y libera el mapa Leaflet de una entrada que ha salido del
  // top-5 (la desplazaron 5 sesiones más nuevas).
  _destruirMapaEntrada(entryId) {
    const map = this._mapasEntradas[entryId];
    if (map && map.remove) {
      try { map.remove(); } catch (e) {}
    }
    delete this._mapasEntradas[entryId];
  },
  ...
  async _actualizarMisEntrenamientos() {
    ...
    if (snapshot.empty) {
      ...
      Object.keys(this._mapasEntradas).forEach(id => this._destruirMapaEntrada(id));
      this._gpsEntries = {};
      return;
    }
    ...
    if (!wrapper.dataset.inicializado) {
      ...
      this._crearMapasFaltantes(ordenNuevo, entriesNuevas);
      return;
    }
    ...
    existentes.forEach(entryId => {
      if (!ordenNuevo.includes(entryId)) {
        ...
        this._destruirMapaEntrada(entryId);
