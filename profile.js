...
// Versión: 10.3 - Con minimapa GPS funcional, sin emojis, soporte safe-area
// ====================

const Profile = {
  _gpsEntries: {}, // cache de entries GPS para el viewer

  // ===== MINIMAPA SVG INLINE (sin dependencias) =====
  ...
  async cargarPerfil(forceRefresh = false) {
    ...
    try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            if (entry.hasGPS && Array.isArray(entry.trackPoints) && entry.trackPoints.length >= 2) {
              const mapSVG = this._fallbackRenderTrackSVG(entry.trackPoints, 300, 120);
              if (mapSVG) {
      ...
      setTimeout(() => {
        ...
        if (changeBtn) {
          changeBtn.onclick = () => Profile._mostrarModalCambiarZapatilla();
        }
        
        const historyBtn = document.getElementById('historyShoeBtn');
        if (historyBtn) {
          historyBtn.onclick = () => Profile._mostrarModalHistorial();
        }
      }, 0);
