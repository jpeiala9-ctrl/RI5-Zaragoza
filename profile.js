...
// Versión: 3.4 - CORREGIDO: Sincronización del contador de amigos
// ====================
...
const Profile = {
  async cargarPerfil() {
    ...
    try {
      ...
      // Si hay inconsistencia, actualizar Firestore y el objeto userData
      if (userData.friendsCount !== actualFriendCount) {
        console.log(`🔧 [Profile] Corrigiendo friendsCount: ${userData.friendsCount} → ${actualFriendCount}`);
        await firebaseServices.db.collection('users').doc(AppState.currentUserId).update({
          friendsCount: actualFriendCount
        });
        userData.friendsCount = actualFriendCount;
      }

      // Actualizar el estado global
      if (AppState.currentUserData) {
        AppState.currentUserData.friendIds = friendIds;
        AppState.currentUserData.friendsCount = actualFriendCount;
      }
        ...
        <div class="perfil-header">
          ...
          <div class="perfil-info">
            ...
            <div class="perfil-stats">
              <div class="perfil-stat"><span>${actualFriendCount}</span><label>Amigos</label></div>
