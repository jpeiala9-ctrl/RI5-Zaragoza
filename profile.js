// ==================== profile.js ====================
// Versión: 10.3 - Tarjeta de últimos entrenamientos IDÉNTICA al muro, like solo muestra likes
// ====================
...
const Profile = {
  ...
  async cargarPerfil(forceRefresh = false) {
    ...
    try {
      ...
      try {
        ...
        if (!misEntrenamientosSnapshot.empty) {
          ...
          for (const doc of misEntrenamientosSnapshot.docs) {
            ...
            // En el perfil NO se permite dar like, por tanto no se calcula userLiked
            ...
            // Guardar en caché para el visor
      ...
      setTimeout(() => {
        ...
        // Evento para botones de like (abrir modal de likes)
        ...
        // Evento para los minimapas (abrir visor)
        ...
        document.querySelectorAll('.badge-icon').forEach(icon => {
          ...
          icon.removeEventListener('touchstart', this._mostrarModalInsignias);
          icon.addEventListener('touchstart', this._mostrarModalInsignias);
        });
  ...
  // ========== MODAL DE LIKES (COPIA DEL ORIGINAL) ==========
  ...
  // ========== MODAL DE INSIGNIAS (SIN CAMBIOS) ==========
  ...
  // ========== CAMBIO DE ZAPATILLA Y HISTORIAL (SIN CAMBIOS) ==========
...
console.log('✅ Profile v10.3 - Tarjetas de entrenamiento idénticas al muro, like muestra likes');