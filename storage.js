...
const Storage = {
  ...
  /**
   * Elimina SOLO la copia local del propio usuario de su conversación de
   * soporte (su subcolección privada 'users/{uid}/mensajes'). A
   * diferencia de eliminarConversacionSoporte(), NO toca la colección
   * global 'soporteMensajes' que usa el administrador: esos mensajes
   * siguen ahí para él, es quien decide si se borran del todo. Un
   * usuario normal no tiene permisos para borrar de esa colección
   * global (por eso daba error de permiso al intentarlo).
   */
  async eliminarConversacionSoportePropiaLocal(uid) {
    ...
    try {
      ...
      console.log(`🗑️ Copia local de la conversación de soporte de ${uid} eliminada (${snapshot.docs.length} mensajes)`);
      return true;
    } catch (error) {
      console.error('❌ Error eliminando la copia local de la conversación de soporte:', error);
      return false;
    }
  },
