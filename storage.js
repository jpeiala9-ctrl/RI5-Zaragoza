...
// Versión: 3.11 - Reformateado para legibilidad
// ====================
...
const Storage = {
  ...
  async getUserByUsername(username) {
    ...
    try {
      // Usamos username_lowercase que tiene índice, evitando full collection scan
  ...
  async addHistorialEntry(uid, entry) {
    ...
    try {
      await firebaseServices.db
        ...
        .add({ ...entry, timestamp: firebaseServices.Timestamp.now() });
    } catch (error) {
  ...
  async getHistorialPlanes(uid, limit = 5) {
    const result = await this.getSubcollection(uid, 'planes', { orderBy: 'fechaCreacion', direction: 'desc', limit });
    return result.items;
  },
  ...
  async setUltimoCalculo(uid, calculo) {
    ...
    try {
      ...
      await firebaseServices.db
        ...
        .collection('calculos')
        .doc(calculoId)
        .set({ ...calculo, timestamp: firebaseServices.Timestamp.now() });
      await firebaseServices.db
        .collection('users')
        .doc(uid)
  ...
  async getFeed(uid, limit = 20) {
    return this.getSubcollection(uid, 'feed', { orderBy: 'timestamp', direction: 'desc', limit });
  },
