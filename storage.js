...
const Storage = {
  ...
  async sendTrainerRequest(fromUid, toUid) {
    ...
    try {
      ...
      if (!fromUser || !toUser) return false;
      const requestId = `${fromUid}_${toUid}`;
      // 🔥 set() sin merge: si ya existía una invitación anterior a este
      // mismo alumno (rechazada, cancelada, o aceptada-y-luego-retirada
      // -- ver removeAlumno más abajo, que la deja en 'revoked'), esto la
      // reescribe entera y vuelve a 'pending' con marca de tiempo fresca.
      // reglas.js permite explícitamente este reenvío (ver el "allow
      // update" de trainerRequests: transición desde rejected/cancelled/
      // revoked hacia pending, mismo remitente y destinatario).
