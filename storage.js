...
// Versión: 3.17 - Nueva consulta de mensajes del administrador (filtro en cliente) + listener
// ====================
...
const Storage = {
  ...
  // ⬇️ NUEVA VERSIÓN: obtiene TODOS los mensajes y filtra en cliente (más fiable)
  async getAdminMessagesPaginated(limit = 10, startAfter = null) {
    try {
      // Obtener todos los documentos de la colección mensajes, ordenados por timestamp descendente
      let query = firebaseServices.db.collection('mensajes')
        .orderBy('timestamp', 'desc');
      
      // Si hay un punto de inicio, aplicamos startAfter (pero como traemos todos, lo haremos en cliente)
      // Para simplificar, traemos todos y hacemos paginación en memoria.
      // Como los mensajes no serán millones, es aceptable.
      const snapshot = await query.get();
      
      // Filtrar solo los documentos cuyo ID comienza con "admin_"
      let adminDocs = snapshot.docs.filter(doc => doc.id.startsWith('admin_'));
      
      // Ordenar por timestamp descendente (ya lo está, pero por si acaso)
      adminDocs.sort((a, b) => {
      ...
      let startIndex = 0;
      if (startAfter) {
        const startId = startAfter.id;
        startIndex = adminDocs.findIndex(doc => doc.id === startId) + 1;
        if (startIndex === 0) startIndex = adminDocs.length; // no encontrado
      }
      const paginated = adminDocs.slice(startIndex, startIndex + limit);
