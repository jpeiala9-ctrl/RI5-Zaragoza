...
// Versión: 3.21 - DEFINITIVA: getAdminMessagesPaginated devuelve mensajes planos con fromUid y mensajeIdx
// ====================
...
const Storage = {
  ...
  // Obtener mensajes recibidos por el usuario (desde admin hacia él)
  ...
  // Enviar mensaje del usuario al administrador (se guarda en admin_<uid>)
  ...
  // Enviar mensaje del administrador al usuario (se guarda en el documento del usuario)
  ...
  // Obtener mensajes enviados por el usuario (desde él hacia admin)
  ...
  // Marcar mensaje como leído (en el documento del usuario)
  ...
  // Borrar mensaje recibido por el usuario
  ...
  // Borrar mensaje enviado por el usuario (en admin_<uid>)
  ...
  // ⬇️ CORREGIDA: obtiene TODOS los mensajes y los devuelve como lista plana
  async getAdminMessagesPaginated(limit = 10, startAfter = null) {
    try {
      // Obtener todos los documentos de la colección mensajes
      const snapshot = await firebaseServices.db.collection('mensajes').get();
      
      // Filtrar solo los documentos cuyo ID comienza con "admin_"
      let adminDocs = snapshot.docs.filter(doc => doc.id.startsWith('admin_'));
      
      // Extraer los mensajes de cada documento y aplanarlos, asignando índices
      let allMessages = [];
      for (const doc of adminDocs) {
        ...
        mensajes.forEach((msg, idx) => {
          allMessages.push({
            ...
            mensajeIdx: idx,
            docId: doc.id,
            timestamp: msg.timestamp ? msg.timestamp.toDate?.() : new Date(msg.fecha)
          });
        });
      }
      ...
      let startIndex = 0;
      if (startAfter) {
        const lastTimestamp = startAfter.timestamp;
        startIndex = allMessages.findIndex(m => m.timestamp === lastTimestamp) + 1;
        if (startIndex === 0) startIndex = allMessages.length;
      }
      const paginated = allMessages.slice(startIndex, startIndex + limit);
      const lastDoc = paginated.length > 0 ? paginated[paginated.length - 1] : null;
      
      return { docs: paginated, lastDoc: lastDoc };
    } catch (error) {
