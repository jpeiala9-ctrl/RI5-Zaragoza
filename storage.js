...
// Versión: 3.12 - Feed global simplificado (sin índices)
// ====================
...
const Storage = {
  ...
  // ========== FEED GLOBAL (sin índices) ==========
  async addToFeed(uid, entry) { 
    if (!uid || !entry) return; 
    try { 
      await firebaseServices.db.collection('globalFeed').add({
        ...entry,
        userId: uid,
        timestamp: firebaseServices.Timestamp.now()
      });
      console.log('✅ Entrada añadida al feed global');
    } catch (error) { 
  ...
  async removeFromFeed(uid, planId, sesionIndex) {
    ...
    try {
      ...
      await batch.commit();
      console.log(`✅ Entrada(s) eliminada(s) del feed global para sesión ${sesionIndex} (plan ${planId})`);
    } catch (error) {
      console.error('Error removing from global feed:', error);
    }
  },
