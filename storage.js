...
// Versión: 3.12 - Corregido feed: ordenación en cliente, sin índice necesario
// ====================
...
const Storage = {
  ...
  // ========== SUBCCOLECCIONES (con ordenación en cliente para feed) ==========
  async getSubcollection(uid, subcol, options = { orderBy: 'timestamp', direction: 'desc', limit: 25, startAfter: null }) {
    ...
    try {
      ...
      // Para la subcolección "feed" NO usamos orderBy en Firestore (para evitar índice)
      // Solo se ordena en cliente después de obtener los datos.
      if (subcol !== 'feed' && options.orderBy) {
        query = query.orderBy(options.orderBy, options.direction);
      }
      ...
      // Ordenamos en cliente si es feed y se requiere orden
      if (subcol === 'feed' && options.orderBy) {
        items.sort((a, b) => {
          const aTime = a.timestamp?.toDate?.() || new Date(a.timestamp);
          const bTime = b.timestamp?.toDate?.() || new Date(b.timestamp);
          return options.direction === 'desc' ? bTime - aTime : aTime - bTime;
        });
      }
  ...
  async addToFeed(uid, entry) { 
    ...
    try { 
      ...
      friends.forEach(friend => { 
        const feedRef = firebaseServices.db.collection('users').doc(friend.uid).collection('feed').doc(); 
        batch.set(feedRef, entry); 
      }); 
      await batch.commit(); 
    } catch (error) { 
  ...
  async getFeed(uid, limit = 20, startAfter = null) { 
    return this.getSubcollection(uid, 'feed', { orderBy: 'timestamp', direction: 'desc', limit, startAfter }); 
  },
