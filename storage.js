...
// Versión: 3.16 - Añadidas funciones para sharedSessions (compartir sesiones)
// ====================
...
const Storage = {
  ...
  // ==================== SHARED SESSIONS (COMPARTIR SESIONES) ====================
  async createSharedSession(senderId, receiverIds, session, diaSemana) {
    if (!senderId || !receiverIds || !receiverIds.length || !session) return null;
    try {
      const shareId = firebaseServices.db.collection('_').doc().id;
      const docRef = firebaseServices.db.collection('sharedSessions').doc(shareId);
      await docRef.set({
        senderId,
        receiverIds,
        session,
      ...
      console.error('Error creando sesión compartida:', error);
      Utils.showToast('Error al compartir sesión', 'error');
      return null;
    }
  },

  async getSharedSessionsForUser(userId, status = 'pending') {
    if (!userId) return [];
    try {
  ...
  async acceptSharedSession(shareId, userId) {
    ...
    try {
      ...
      const data = doc.data();
      if (!data.receiverIds.includes(userId)) throw new Error('No eres destinatario');
      if (data.status !== 'pending') throw new Error('Esta sesión ya no está pendiente');

      // Obtener el plan actual del usuario
      const userRef = firebaseServices.db.collection('users').doc(userId);
      const userDoc = await userRef.get();
      let planId = userDoc.data()?.ultimoPlanId;
      let planData = null;
      ...
      // Si no hay plan, crear uno nuevo con esta sesión
      if (!planData) {
        const newPlanId = Date.now().toString() + '_' + Math.random().toString(36).substr(2, 9);
        ...
        for (let d = 1; d <= 7; d++) {
          if (d === data.diaSemana) {
            nuevasSesiones.push({
              ...
              fase: 'BASE',
              nivel: 'principiante',
              color: data.session.color || 'sesion-rodaje',
              letra: data.session.letra || 'R',
              tieneFuerza: data.session.tieneFuerza || false,
              duracion: data.session.duracion || 0
            });
          } else {
            nuevasSesiones.push({
              diaGlobal: d,
        ...
        planData = {
          params: {
            ...
            experiencia: 'si',
            objetivo: 'mejorar',
            diaLargo: data.diaSemana,
            diasEntreno: [data.diaSemana],
            planId: newPlanId,
            ritmoBase: 5,
            fcMax: 180,
            fcUmbral: 165,
          ...
          feedback: {},
          ajustes: {},
          resumen: `Sesión compartida · ${data.session.tipo || 'Entreno'}`,
        ...
        await firebaseServices.db
          ...
          .doc(planId)
          .set(planData);
        await userRef.update({ ultimoPlanId: planId });
      } else {
        // Sobrescribir la sesión del día correspondiente
        const sesiones = planData.sesiones || [];
        const diaSemana = data.diaSemana;
        let encontrado = false;
        for (let i = 0; i < sesiones.length; i++) {
          if (sesiones[i].diaSemana === diaSemana) {
            const original = sesiones[i];
            sesiones[i] = {
              ...data.session,
              diaGlobal: original.diaGlobal,
              semana: original.semana,
              diaSemana: original.diaSemana,
              fase: original.fase,
              nivel: original.nivel,
              color: data.session.color || original.color,
              letra: data.session.letra || original.letra,
              tieneFuerza: data.session.tieneFuerza || false,
              duracion: data.session.duracion || original.duracion || 0
            };
            encontrado = true;
            break;
          }
      ...
      console.error('Error aceptando sesión compartida:', error);
      Utils.showToast('Error al aceptar: ' + error.message, 'error');
      return false;
    }
  },

  async rejectSharedSession(shareId, userId) {
    if (!shareId || !userId) return false;
    try {
      const docRef = firebaseServices.db.collection('sharedSessions').doc(shareId);
      const doc = await docRef.get();
      ...
      const data = doc.data();
      if (!data.receiverIds.includes(userId)) throw new Error('No eres destinatario');
      if (data.status !== 'pending') throw new Error('Ya no está pendiente');
      await docRef.update({ status: 'rejected' });
      return true;
    } catch (error) {
      console.error('Error rechazando sesión compartida:', error);
      Utils.showToast('Error al rechazar: ' + error.message, 'error');
      return false;
    }
  }
};
