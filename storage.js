// ==================== storage.js - Versión final corregida ====================
const Storage = {
  async getUser(uid) { 
    if (!uid) return null; 
    try { 
  ...
  // ---------- Subcolecciones (con ordenación en cliente para feed) ----------
  async getSubcollection(uid, subcol, options = { orderBy: 'timestamp', direction: 'desc', limit: 25, startAfter: null }) {
    ...
    try {
      ...
      // Para el feed NO usamos orderBy en la consulta (evita índice)
      ...
      // Ordenamos en cliente si es feed
      if (subcol === 'feed' && options.orderBy) {
        items.sort((a, b) => {
