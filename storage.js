...
// Versión: 3.24 - Sistema de soporte independiente (colección soporteMensajes) CORREGIDO
// ====================
...
const Storage = {
  ...
  // ==================== NUEVO SISTEMA DE SOPORTE INDEPENDIENTE ====================
  
  // Enviar mensaje de soporte (usuario -> admin o admin -> usuario)
  ...
  // Obtener mensajes de soporte para un usuario (recibidos y enviados)
  async getMensajesSoporteUsuario(uid) {
    ...
    try {
      const snapshot = await firebaseServices.db
        ...
        .orderBy('timestamp', 'desc')
        .get();
      const enviados = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      
      const snapshot2 = await firebaseServices.db
        .collection('soporteMensajes')
        .where('toUid', '==', uid)
        .orderBy('timestamp', 'desc')
        .get();
      const recibidos = snapshot2.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      
      // Combinar y ordenar por timestamp
      const todos = [...enviados, ...recibidos];
      todos.sort((a, b) => b.timestamp?.toDate?.() - a.timestamp?.toDate?.());
      return todos;
    } catch (error) {
  ...
  // Obtener mensajes de soporte para el administrador (todos los mensajes)
  ...
  // Marcar mensaje de soporte como leído
  async marcarMensajeSoporteLeido(mensajeId) {
    if (!mensajeId) return;
    try {
