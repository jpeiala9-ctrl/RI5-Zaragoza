...
// Versión: 3.12 - Feed global simplificado
// ====================
...
const Storage = {
  ...
  async addToFeed(uid, entry) { 
    if (!uid || !entry) return; 
    try { 
      const feedRef = firebaseServices.db.collection('globalFeed').doc();
      await feedRef.set({
        ...entry,
        userId: uid,
        timestamp: firebaseServices.Timestamp.now()
  ...
  async removeFromFeed(uid, planId, sesionIndex) {
    ...
    try {
      ...
      if (!snapshot.empty) {
        ...
        console.log(`✅ Entrada eliminada del feed global para sesión ${sesionIndex} (plan ${planId})`);
      }
    } catch (error) {
