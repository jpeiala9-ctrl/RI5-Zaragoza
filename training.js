...
const Training = {
  ...
  mostrarResultados(d) { 
    ...
    d.zones.forEach((z, index) => { 
      ...
      // una acción poco frecuente). Ahora es un enlace de texto discreto
      // al final del detalle EXPANDIDO -- solo aparece cuando el usuario
      // ya ha tocado la tarjeta para ver más, que es cuando tiene sentido
      // ofrecérselo.
      const enlaceEditar = `<div style="margin-top:14px; padding-top:10px; border-top:1px solid var(--border-color);">
        <button onclick="event.stopPropagation(); Training.abrirEdicionZona(${index})" style="background:none; border:none; padding:0; width:100%; text-align:left; font-family:inherit; font-size:11px; color:var(--text-secondary); text-decoration:underline; cursor:pointer;">Editar esta zona a mano</button>
      </div>`;
      if (index === 5) {
