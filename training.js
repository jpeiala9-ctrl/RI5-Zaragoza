...
// Versión: 4.2 - Nuevo TimePicker: el tiempo de 6km en la pestaña de zonas
//                de entreno se elige con dos ruedas de scroll (minutos y
//                segundos) en vez de escribirlo a mano, para evitar
//                formatos inválidos (coma, minutos/segundos pegados...).
...
// ==================== SELECTOR DE TIEMPO (RELOJ) ====================
// Reemplaza el antiguo campo de texto libre "MM:SS" del tiempo de 6km por
// dos ruedas de scroll (minutos y segundos) con scroll-snap: el usuario
// desliza para elegir cada valor, así que es imposible escribir un
// formato inválido (coma en vez de dos puntos, "2730" pegado, etc.). El
// resultado se escribe siempre en el input oculto #time con el mismo
// formato "M:SS" que ya esperaba Utils.parseTime, así que el resto del
// flujo (validación, botón CALCULAR, cálculo de zonas) no cambia.
const TimePicker = {
  ITEM_HEIGHT: 40,
  // Rango realista para un 6km: 10-90 min es el mismo límite que ya
  // aplicaba UI.validarCampo('time') antes de este cambio.
  MINUTOS: Array.from({ length: 81 }, (_, i) => i + 10), // 10..90
  SEGUNDOS: Array.from({ length: 60 }, (_, i) => i),      // 0..59
  _inicializando: false,

  init(minInicial = 27, segInicial = 0) {
    const wrapMin = document.getElementById('timeWheelMin');
    const wrapSeg = document.getElementById('timeWheelSeg');
    if (!wrapMin || !wrapSeg) return; // esta pestaña puede no estar en el DOM en otras pantallas

    this._construirRueda(wrapMin, this.MINUTOS);
    this._construirRueda(wrapSeg, this.SEGUNDOS);

    this._inicializando = true;
    this._irA(wrapMin, this.MINUTOS, minInicial);
    this._irA(wrapSeg, this.SEGUNDOS, segInicial);
    // Pequeño margen para que los scrollTop programados de arriba no se
    // interpreten como un scroll real del usuario y disparen ya el
    // 'input' del campo (que debe quedar vacío hasta que el usuario
    // toque el selector, igual que antes con el campo de texto vacío).
    setTimeout(() => { this._inicializando = false; }, 50);

    let timeoutMin = null, timeoutSeg = null;
    wrapMin.addEventListener('scroll', () => {
      this._resaltarSeleccion(wrapMin, this.MINUTOS);
      if (this._inicializando) return;
      clearTimeout(timeoutMin);
      timeoutMin = setTimeout(() => this._onScrollFin(wrapMin, this.MINUTOS), 120);
    });
    wrapSeg.addEventListener('scroll', () => {
      this._resaltarSeleccion(wrapSeg, this.SEGUNDOS);
      if (this._inicializando) return;
      clearTimeout(timeoutSeg);
      timeoutSeg = setTimeout(() => this._onScrollFin(wrapSeg, this.SEGUNDOS), 120);
    });
  },

  _construirRueda(el, valores) {
    // Un espaciador arriba y otro abajo del alto de una fila, así la
    // primera y la última opción también pueden quedar centradas (y por
    // tanto seleccionables) bajo la franja resaltada del centro.
    const espaciador = `<div style="height:${this.ITEM_HEIGHT}px;"></div>`;
    el.innerHTML = espaciador +
      valores.map(v => `<div class="time-wheel-item" data-valor="${v}">${String(v).padStart(2, '0')}</div>`).join('') +
      espaciador;
  },

  _irA(el, valores, valor) {
    let idx = valores.indexOf(valor);
    if (idx < 0) idx = 0;
    el.scrollTop = idx * this.ITEM_HEIGHT;
    this._resaltarSeleccion(el, valores);
  },

  _indiceSeleccionado(el, valores) {
    const idx = Math.round(el.scrollTop / this.ITEM_HEIGHT);
    return Math.max(0, Math.min(valores.length - 1, idx));
  },

  _resaltarSeleccion(el, valores) {
    const idx = this._indiceSeleccionado(el, valores);
    el.querySelectorAll('.time-wheel-item').forEach((item, i) => {
      item.classList.toggle('selected', i === idx);
    });
  },

  _onScrollFin(el, valores) {
    const idx = this._indiceSeleccionado(el, valores);
    el.scrollTo({ top: idx * this.ITEM_HEIGHT, behavior: 'smooth' });
    this._resaltarSeleccion(el, valores);
    this._actualizarInputTime();
  },

  _actualizarInputTime() {
    const wrapMin = document.getElementById('timeWheelMin');
    const wrapSeg = document.getElementById('timeWheelSeg');
    const timeInput = document.getElementById('time');
    if (!wrapMin || !wrapSeg || !timeInput) return;
    const min = this.MINUTOS[this._indiceSeleccionado(wrapMin, this.MINUTOS)];
    const seg = this.SEGUNDOS[this._indiceSeleccionado(wrapSeg, this.SEGUNDOS)];
    timeInput.value = `${min}:${String(seg).padStart(2, '0')}`;
