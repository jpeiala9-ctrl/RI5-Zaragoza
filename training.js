...
const Training = {
  ...
  // Restaura el último cálculo del usuario desde localStorage y lo muestra
  restaurarUltimoCalculo(uid) {
    if (!uid) return;
    try {
      const raw = localStorage.getItem(`ri5_last_calc_${uid}`);
      if (!raw) return;
      const calc = JSON.parse(raw);
      if (!calc || !calc.zones || !calc.zones.length || !calc.pred || !calc.pred.length) return;
      AppState.setLastCalc(calc);
      this.mostrarResultados(calc);
