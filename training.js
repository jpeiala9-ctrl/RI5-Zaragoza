...
const Training = {
  ...
  mostrarResultados(d) { 
    ...
    // Referencia al cálculo que se está mostrando ahora mismo, para que
    // abrirEdicionZona()/guardarEdicionZona() puedan editarlo sin tener
    // que volver a pedirlo (funciona igual si viene recién calculado o
    // cargado del historial, ver mostrarResultadosGuardados()).
    this._ultimoCalc = d;
    d.zones.forEach((z, index) => { 
      const estiloBase = 'font-size:14px;';
      // 🔥 Zona editada a mano (8º elemento de la tupla, ver
      // abrirEdicionZona/guardarEdicionZona más abajo). No pisa ningún
      // dato existente -- el resto de la app sigue leyendo z[0]..z[6]
      // exactamente igual que antes.
      const personalizada = !!z[7];
      const badge = personalizada ? `<span style="font-size:9px; letter-spacing:0.5px; color:var(--gold); border:1px solid var(--gold); border-radius:6px; padding:2px 6px; margin-left:8px; vertical-align:middle;">PERSONALIZADA</span>` : '';
      // 🔥 El botón de editar YA NO va flotando en la esquina de la
      // tarjeta colapsada (competía en protagonismo con el propio título
      // de la zona, y se repetía siempre visible en las 6 tarjetas para
      // una acción poco frecuente). Ahora vive al final del detalle
      // EXPANDIDO -- solo aparece cuando el usuario ya ha tocado la
      // tarjeta para ver más, que es cuando tiene sentido ofrecérselo.
      // A petición del usuario, ya no es un enlace de texto subrayado
      // pegado a la izquierda: es un botón pequeño y centrado, con la
      // misma acción de siempre (Training.abrirEdicionZona).
      const enlaceEditar = `<div style="margin-top:14px; padding-top:10px; border-top:1px solid var(--border-color); display:flex; justify-content:center;">
        <button onclick="event.stopPropagation(); Training.abrirEdicionZona(${index})" style="background:none; border:1px solid var(--border-color-light); border-radius:20px; padding:6px 20px; font-family:inherit; font-size:11px; font-weight:bold; letter-spacing:0.3px; color:var(--text-secondary); cursor:pointer;">EDITAR ZONA</button>
      </div>`;
      if (index === 5) {
        ...
        html += `<div class="zone-card" style="border-left: 4px solid ${colores[index]};" onclick="this.classList.toggle('active')">
          <strong>${z[0]} – ${z[1]}</strong>${badge}<br>
          FC: >${min} lpm (máximo)
          <div class="long">${z[6].replace(/<p>/g, `<p style="margin:12px 0;">`).replace(/<strong>/g, `<strong style="${estiloBase}">`)}${enlaceEditar}</div>
        </div>`;
      } else {
  ...
  // ============================================================
  // Permite ajustar a mano la FC (lpm) y el ritmo (min/km) de una zona
  // concreta cuando el test en la calle no encaja con lo calculado por
  // fórmula. Los valores se guardan como el % de FC (sobre el umbral,
  // d.ul) y el factor de ritmo (sobre d.ritmoBase) que les correspondería
  // -- es decir, se sobrescriben z[2]/z[3]/z[4] con los valores exactos
  // que reproducen lo que el usuario ha tecleado. Así NINGÚN otro sitio
  // de la app (planes, sesiones GPS, invitaciones de admin) necesita
  // cambiar nada: todos siguen leyendo z[2]/z[3]/z[4] igual que siempre,
  // simplemente ahora esos números reflejan el ajuste manual. z[7] se usa
  // como marca "PERSONALIZADA" (ver mostrarResultados). Volver a pulsar
  // CALCULAR regenera el array de zonas desde cero (sin z[7]), así que
  // cualquier ajuste manual se pierde -- tal y como se avisa en la guía.
  abrirEdicionZona(index) {
    const d = this._ultimoCalc;
    if (!d || !d.zones || !d.zones[index]) return;
    const z = d.zones[index];
    const esZ6 = z[0] === 'Z6';
    const minActual = Math.round(d.ul * z[2]);
    const maxActual = esZ6 ? null : Math.round(d.ul * z[3]);
    const paceActual = Utils.formatR(d.ritmoBase * z[4]);

    document.getElementById('editarZonaOverlay')?.remove();

    const overlay = document.createElement('div');
    overlay.id = 'editarZonaOverlay';
    overlay.style.cssText = 'position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.8); z-index:20500; display:flex; align-items:center; justify-content:center; padding:20px;';
    overlay.addEventListener('click', (e) => { if (e.target === overlay) this.cerrarEdicionZona(); });

    const modal = document.createElement('div');
    modal.id = 'editarZonaModal';
    modal.style.cssText = 'background:var(--bg-card); border:1px solid var(--border-color); border-radius:16px; padding:20px; width:100%; max-width:380px; box-shadow:var(--shadow-lg);';
    modal.innerHTML = `
      <h3 style="margin:0 0 4px 0; text-align:center; color:var(--gold); font-size:16px;">EDITAR ${Utils.escapeHTML(z[0])} · ${Utils.escapeHTML(z[1])}</h3>
      <p style="font-size:11px; color:var(--text-secondary); text-align:center; margin:0 0 18px 0; line-height:1.4;">Ajusta a mano la FC y el ritmo de esta zona. Quedará marcada como PERSONALIZADA.</p>
      <label style="display:block; font-size:11px; color:var(--text-secondary); margin-bottom:4px;">FC mínima (lpm)</label>
      <input id="editarZonaFcMin" type="number" inputmode="numeric" value="${minActual}" style="width:100%; box-sizing:border-box; padding:10px; border-radius:8px; border:1px solid var(--border-color); background:var(--bg-primary); color:var(--text-primary); margin-bottom:12px; font-size:14px; text-align:center;">
      ${!esZ6 ? `
        <label style="display:block; font-size:11px; color:var(--text-secondary); margin-bottom:4px;">FC máxima (lpm)</label>
        <input id="editarZonaFcMax" type="number" inputmode="numeric" value="${maxActual}" style="width:100%; box-sizing:border-box; padding:10px; border-radius:8px; border:1px solid var(--border-color); background:var(--bg-primary); color:var(--text-primary); margin-bottom:12px; font-size:14px; text-align:center;">
      ` : ''}
      <label style="display:block; font-size:11px; color:var(--text-secondary); margin-bottom:4px;">Ritmo (min/km, formato M:SS)</label>
      <input id="editarZonaPace" type="text" inputmode="numeric" value="${paceActual}" placeholder="5:30" style="width:100%; box-sizing:border-box; padding:10px; border-radius:8px; border:1px solid var(--border-color); background:var(--bg-primary); color:var(--text-primary); margin-bottom:18px; font-size:14px; text-align:center;">
      <div style="display:flex; gap:12px;">
        <button class="action-button" style="flex:1; margin:0; background:transparent; border:1px solid var(--border-color-light);" onclick="Training.cerrarEdicionZona()">CANCELAR</button>
        <button class="action-button" style="flex:1; margin:0;" onclick="Training.guardarEdicionZona(${index})">GUARDAR</button>
      </div>
    `;
  ...
  // Parser de ritmo (min/km) dedicado: Utils.parseTime exige minutos entre
  // 10 y 120 (pensado para el tiempo TOTAL de 6km), lo que rechazaría
  // cualquier ritmo normal tipo "5:30". Aquí los minutos válidos son 1-30.
  _parsePaceInput(str) {
    if (!str || typeof str !== 'string') return NaN;
    str = str.trim().replace(',', ':');
    if (!str.includes(':')) {
      const val = parseFloat(str);
      return (!isNaN(val) && val > 0 && val < 30) ? val : NaN;
    }
    const partes = str.split(':');
    const m = parseInt(partes[0]), s = partes[1] ? parseInt(partes[1]) : 0;
    if (isNaN(m) || isNaN(s) || s > 59 || m < 1 || m > 30) return NaN;
    return m + s / 60;
  },

  async guardarEdicionZona(index) {
    const d = this._ultimoCalc;
    if (!d || !d.zones || !d.zones[index]) return;
    if (!d.ul || d.ul <= 0 || !d.ritmoBase || d.ritmoBase <= 0) {
      Utils.showToast('Error: faltan datos del cálculo', 'error');
      return;
    }
    const z = d.zones[index];
    const esZ6 = z[0] === 'Z6';

    const fcMin = parseInt(document.getElementById('editarZonaFcMin')?.value);
    const fcMax = esZ6 ? null : parseInt(document.getElementById('editarZonaFcMax')?.value);
    const paceDecimal = this._parsePaceInput(document.getElementById('editarZonaPace')?.value);

    if (isNaN(fcMin) || fcMin < 30 || fcMin > 240) { Utils.showToast('FC mínima no válida', 'error'); return; }
    if (!esZ6 && (isNaN(fcMax) || fcMax < 30 || fcMax > 240 || fcMax <= fcMin)) { Utils.showToast('FC máxima no válida', 'error'); return; }
    if (isNaN(paceDecimal)) { Utils.showToast('Ritmo no válido (usa M:SS, ej. 5:30)', 'error'); return; }

    z[2] = fcMin / d.ul;
    if (!esZ6) z[3] = fcMax / d.ul;
    z[4] = paceDecimal / d.ritmoBase;
    z[7] = true; // marcada como PERSONALIZADA

    AppState.setLastCalc(d);
    this._saveToLocalStorage(d);
    this.cerrarEdicionZona();
    this.mostrarResultados(d);
    Utils.showToast('✓ Zona personalizada guardada', 'success');

    if (AppState.currentUserId) {
      try {
        await Storage.setUltimoCalculo(AppState.currentUserId, d);
      } catch (e) {
        console.warn('No se pudo guardar la zona personalizada en la nube:', e);
      }
    }
  },
  // ============================================================
  //  FIN EDICIÓN MANUAL DE ZONAS
  // ============================================================
