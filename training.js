...
// Versión: 3.7 - parseTime mejorado, límite mensual implementado
// ====================
...
// Fórmula de Tanaka (2001): más precisa que 220-edad (error estándar ±7 lpm vs ±12 lpm)
const fc = Math.round(208 - 0.7 * a);
// El umbral de lactato real es individual; este 92% es una estimación orientativa
...
const calc = { name: n, age: a, fcMax: fc, ul: ul, zones: zones, pred: pred, ritmoBase: r,
  disclaimer: '⚠️ FC máx estimada con fórmula de Tanaka (208−0.7×edad). El umbral de lactato real requiere un test de esfuerzo.' };
...
if (!results) return;
let html = `<h2>MÉTRICAS</h2><div style="text-align:center; margin:20px 0;">${d.name} · ${d.age} años<br>FC MÁX: ${d.fcMax} lpm · UMBRAL: ${d.ul} lpm<br>RITMO 6km: ${Utils.formatR(d.ritmoBase)} min/km</div><div style="font-size:11px;color:var(--text-secondary);text-align:center;margin-bottom:16px;padding:8px;background:var(--bg-secondary);border-radius:8px;">⚠️ FC máx estimada (Tanaka: 208−0.7×edad). El umbral de lactato real requiere test de esfuerzo.</div><h2>PREDICCIONES</h2>`;
...
html += `<div class="zone-card" style="border-left: 4px solid ${colores[index]};" onclick="this.classList.toggle('active')"> <strong>${z[0]} – ${z[1]}</strong><br> FC: >${min} lpm (máximo) <div class="long">${z[6].replace(/<p>/g, `<p style="margin:12px 0;">`).replace(/<strong>/g, `<strong style="${estiloBase}">`)}</div> </div>`;
} else {
const min = Math.round(d.ul * z[2]), max = Math.round(d.ul * z[3]);
html += `<div class="zone-card" style="border-left: 4px solid ${colores[index]};" onclick="this.classList.toggle('active')"> <strong>${z[0]} – ${z[1]}</strong><br> FC: ${min}-${max} lpm <div class="long">${z[6].replace(/<p>/g, `<p style="margin:12px 0;">`).replace(/<strong>/g, `<strong style="${estiloBase}">`)}</div> </div>`;
}
});
