...
// Versión: 3.9 - Persistencia de resultados en localStorage por usuario
// ====================
...
const Training = {
  async startCalc() { 
    ...
    const fc = 220 - a;
    const ul = Math.round(fc * 0.92); // Umbral de lactato: 92% de FC máx (corregido)
    ...
    Training._guardarCalcLocal(calc); // ← Persistir en localStorage
  ...
  // Guarda el cálculo en localStorage vinculado al usuario actual
  _guardarCalcLocal(calc) {
    const uid = AppState.currentUserId;
    if (!uid || !calc) return;
    try {
      const calcParaGuardar = {
        name: calc.name,
        age: calc.age,
        fcMax: calc.fcMax,
        ul: calc.ul,
        ritmoBase: calc.ritmoBase,
        pred: calc.pred,
        zones: calc.zones,
        savedAt: new Date().toLocaleDateString('es-ES')
      };
      localStorage.setItem(`ri5_last_calc_${uid}`, JSON.stringify(calcParaGuardar));
    } catch (e) {
      console.warn('No se pudo guardar el cálculo localmente:', e);
    }
  },

  // Restaura el último cálculo del usuario: primero localStorage, luego Firestore como fallback
  async restaurarUltimoCalculo(uid) {
    if (!uid) return;
    try {
      // 1. Intentar desde localStorage (rápido, sin red)
      const raw = localStorage.getItem(`ri5_last_calc_${uid}`);
      if (raw) {
        const calc = JSON.parse(raw);
        if (calc && calc.zones && calc.zones.length && calc.pred && calc.pred.length) {
          AppState.setLastCalc(calc);
          this.mostrarResultados(calc);
          const footer = document.getElementById('footer');
          if (footer && calc.name && calc.savedAt) {
            footer.innerText = `${calc.name} · ${calc.savedAt} · RI5`;
          }
          return;
        }
      }
      // 2. Fallback: obtener desde Firestore (por si no hay datos en localStorage)
      if (window.Storage && Storage.getUltimoCalculo) {
        const calc = await Storage.getUltimoCalculo(uid);
        if (calc) {
          AppState.setLastCalc(calc);
          this.mostrarResultados(calc);
          this._guardarCalcLocal(calc); // Cachear para la próxima vez
        }
      }
    } catch (e) {
      console.warn('No se pudo restaurar el cálculo:', e);
    }
  },
  ...
  mostrarResultados(d) { 
    ...
    if (!results) return;
    // Escapar el nombre del usuario por seguridad
